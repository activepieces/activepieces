...
- **Hook file exports** — When a file exposes multiple React hooks for a feature, bundle them into a single named `const` keyed by hook name (e.g. `export const flowMigrationHooks = { useFoo, useBar }`) and call them as `flowMigrationHooks.useFoo()` at the call site. The hooks themselves should be declared as non-exported `function useX(...)` above the bundle. The eslint `react-hooks` plugin recognises member-expression calls whose property starts with `use`, so the rules of hooks still apply.
