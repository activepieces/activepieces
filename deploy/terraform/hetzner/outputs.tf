output "control_plane_ip" {
  description = "Public IP of the k3s control plane — static, persists across re-applies"
...
output "kubeconfig_command" {
  ...
  value       = "ssh root@${hcloud_primary_ip.control_plane.ip_address} cat /etc/rancher/k3s/k3s.yaml | sed 's/127.0.0.1/${hcloud_primary_ip.control_plane.ip_address}/g' > ~/.kube/config"
}

output "tunnel_command" {
  description = "SSH tunnel: forwards localhost:6443 → control-plane:6443 so kubectl works without exposing the API publicly"
  value       = "ssh -N -L 6443:127.0.0.1:6443 root@${hcloud_primary_ip.control_plane.ip_address}"
}
