...
export class ApPaginatorComponent implements OnInit {
	...
	ngOnInit(): void {
		this.pageSizeControl = new FormControl(DEFAULT_PAGE_SIZE, { nonNullable: true });
		...
		if (!this.route.snapshot.queryParamMap.get('limit') && !this.route.snapshot.queryParamMap.get('cursor')) {
			this.setQueryParams('');
		} else {
			const pageSize = Number.parseInt(this.route.snapshot.queryParamMap.get('limit')!);
			if (!pageSize) {
				this.router.navigate(['.'], {
					relativeTo: this.route,
	...
	setQueryParams(cursor: string) {
		const params: { [key: string]: string | number } = { limit: this.pageSizeControl.value };
		if (cursor) {
