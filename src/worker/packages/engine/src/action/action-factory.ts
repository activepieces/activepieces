...
export function createAction(jsonData: any): BaseActionHandler<any> | undefined {
    if(jsonData === undefined || jsonData === null){
        return undefined;
    }
    let currentAction: Action = jsonData as Action;
    let nextAction: BaseActionHandler<any> | undefined = createAction(jsonData['nextAction']);
    switch (currentAction.type){
        ...
        case ActionType.LOOP_ON_ITEMS:
            return new LoopOnItemActionHandler(currentAction,  createAction(jsonData['firstLoopAction']), nextAction);
    }
    throw new Error(currentAction.type + " is not implemented");
}