# AI Engineering Guide for Activepieces

> How to use our Claude Code setup to ship features from a single prompt.
...
| `FEATURE.md` (in modules) | ~60 lines each | When Claude explores the module | Entity schemas, services, data flows |
| `.claude/rules/` | 3-5 lines each | Every session | Critical safety checks (entity registration, data isolation, edition safety) |
| `.claude/skills/` | 30-65 lines each | When invoked | Step-by-step workflows (`/add-feature`, `/add-entity`, `/add-endpoint`) |
| `.claude/agents/` | ~15 lines each | When delegated | Specialized server/web/changelog agents |

**Total context per session:** ~150 lines. Claude has ~150 instruction slots — we use them all, nothing wasted.

---

## The Workflow: Prompt → Feature → Ship
...
Claude will:
- Read relevant `FEATURE.md` files in the modules you'll touch
...
**Rule of thumb:** If you've corrected Claude twice on the same thing, `/clear` and rewrite the prompt with what you learned. Don't accumulate corrections.
...
**Key insight:** Planning on Day N, implementing on Day N+1 produces better code than doing both in the same session. The overnight break lets you think about edge cases.
...
4. **Read FEATURE.md yourself.** Before starting a feature in a module, spend 2 minutes reading its FEATURE.md. You'll ask better questions.
...
**Day 1:** Read this guide. Install Claude Code. Read root `CLAUDE.md` (~55 lines).

**Day 2-3:** Pick a small feature (5-10 files). Follow the full workflow: brief → Plan Mode → implement → test → PR. Expect 4-6 hours.

**Day 4:** Pick a medium feature (15-25 files). Practice `/clear` when stuck. Use `/add-feature` skill.

**Day 5:** Try 2 parallel sessions on different features. Practice tab-switching.

**After Week 1:** You should be shipping 1 feature/day comfortably.
