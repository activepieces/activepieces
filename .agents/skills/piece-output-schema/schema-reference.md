...
- Follow the repo's "exports at the end" rule where it fits: shared `const` field-sets and helpers first, exported schema `const`s after. Type shared field-sets as `OutputSchema['fields']`.
