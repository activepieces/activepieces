...
const PLACEHOLDER_RENDERING_TYPES = new Set(['SHORT_TEXT', 'LONG_TEXT', 'DATE_TIME', 'FILE', 'SECRET_TEXT']);
...
function usage() {
  return [
    'Usage: node check-step-form.mjs <piece-dir-name> [--url http://localhost:4200] [--before <piece.json>] [--include-ai] [--json]',
    '       node check-step-form.mjs --file <piece.json> [--before <piece.json>] [--include-ai] [--json]',
    '',
    ...
    '',
    'With --before <saved.json> (the same GET saved before your change) it also errors on every prop',
    'whose type changed or that disappeared: saved steps keep the old shape and a patch upgrade opens the',
    'new widget on it, so both edits break existing flows.',
    '',
    'Exit code 1 when any finding is an error, 0 otherwise.',
  ].join('\n');
}

const FLAGS_WITH_VALUES = new Set(['--url', '--file', '--before']);
...
function migrationFindings({ before, steps, includeAi }) {
  if (!before) {
    return [];
  }
  const previousSteps = new Map(humanSteps({ piece: before, includeAi }).map((step) => [`${step.kind}:${step.name}`, step]));
  return steps.flatMap((step) => {
    const previous = previousSteps.get(`${step.kind}:${step.name}`);
    if (!previous) {
      return [];
    }
    return Object.entries(previous.props ?? {}).flatMap(([propName, previousProp]) => {
      const prop = step.props?.[propName];
      const add = (rule, message) => ({ level: 'error', rule, step: step.displayName, prop: propName, message });
      if (!prop) {
        return [add('prop-removed', `Present in ${before.version}, gone in ${step.version ?? 'this build'}; saved steps still carry its value and run() no longer reads it. Removal is a major bump.`)];
      }
      if (prop.type !== previousProp.type) {
        return [add('type-changed', `${previousProp.type} -> ${prop.type}. Saved steps keep the ${previousProp.type} value; a patch upgrade opens the ${prop.type} widget on it and the engine zips a plain object into [obj] before run(). See step-form-review.md §5.`)];
      }
      return [];
    });
  });
...
async function main() {
  ...
  const before = args.before ? JSON.parse(readFileSync(args.before, 'utf8')) : undefined;
  ...
  const findings = [
    ...
    ...migrationFindings({ before, steps, includeAi: args.includeAi }),
  ];
  if (args.json) {
