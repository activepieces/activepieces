...
- `packages/server/api/src/app/ee/helper/email/email-service.ts` — `sendIssueCreatedNotification` (`IssueCreatedArgs` shape: `emails`, `projectName`, `runUrl`, `failedStepDisplayName`, `failedStepNumber`, optional `failedStepMessage`); takes an explicit recipient list like every other email-service method — recipient assembly lives in alerts-service
- `packages/server/api/src/app/database/migration/postgres/1810000000000-AddFlowOwnerAlertsEnabledToProject.ts` — adds `project.flowOwnerAlertsEnabled` (boolean, default false)
