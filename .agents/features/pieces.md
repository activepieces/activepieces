...
- `list(params)` — returns filtered + sorted `PieceMetadataModelSummary[]` from cache; applies platform piece filters and EE filtering. Packages listed in `HIDDEN_PIECE_NAMES` (piece packages that were renamed shortly after release, e.g. `@activepieces/piece-aws-bedrock` → `@activepieces/piece-amazon-bedrock`) are dropped from the listing via `filterHiddenPieces`, because their stale rows survive in the cloud registry (`pieceSyncService.sync` upserts and never deletes) and would otherwise show as indistinguishable duplicate entries in the piece selector. Filtering happens in `fetchLatestCompatiblePiecesFromDB` (feeds listing and tool-search reindexing). The stale packages stay resolvable via `getOrThrow` by name/version so existing flows keep working. Entries must never be removed from `HIDDEN_PIECE_NAMES`: the stale DB rows live forever, so removing an entry reintroduces the indistinguishable duplicate in the selector.
