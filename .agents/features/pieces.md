...
- **PieceCategory** — enum grouping pieces (AI, CORE, COMMUNICATION, etc.). Since pieces-framework 0.34.0, `categories` accepts arbitrary custom strings alongside the enum values (typed `(PieceCategory | string)[]`), so platform-installed custom pieces can declare their own categories. The embed SDK's `embedding.categoryPriority` config matches these strings case-insensitively to render prioritized groups at the top of the step picker's Explore and Apps tabs (prioritized pieces are excluded from the stock Popular/Apps/Highlights groups to avoid duplicates) and to rank matching pieces first in picker search results. Server-side category filtering (`GET /v1/pieces?categories=`) still validates against the enum only.
