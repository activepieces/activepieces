...
- `packages/server/api/src/app/pieces/piece-bundle.ts` — resolves `GET /v1/engine/pieces/bundle` to one downloadable link: ARCHIVE by `archiveId` (platform-scoped stream), else metadata-guided REGISTRY resolution (signed S3 when cached → CDN for official → npm tarball). When `pieceMetadataService.get()` is nil the resolver distinguishes two cases via a platform-scoped row-existence check: a piece the platform knows but must not resolve (piece-set visibility, release-window gating) stays **404**, while a genuinely unknown name+version **falls back to the npm tarball redirect** — required for first-time REGISTRY installs, where extraction (and thus the bundle fetch) runs before the metadata row is created (GIT-1623). Cross-platform probes of another platform's private piece names resolve identically to unknown names (npm redirect), so the check adds no existence oracle
