...
- `list(params)` — returns filtered + sorted `PieceMetadataModelSummary[]` from cache; applies platform piece filters and EE filtering. Packages listed in `DEPRECATED_RENAMED_PIECES` (piece packages that were renamed shortly after release, e.g. `@activepieces/piece-aws-bedrock` → `@activepieces/piece-amazon-bedrock`) get a ` (Deprecated)` suffix appended to their `displayName` via `markDeprecatedRenamedPieces`, because their stale rows survive in the cloud registry (`pieceSyncService.sync` upserts and never deletes) and would otherwise show as indistinguishable duplicate entries in the piece selector. The suffix is applied in `fetchLatestCompatiblePiecesFromDB` (feeds listing and tool-search reindexing) and re-applied idempotently after locale translation in `translatePieces`, since a non-English i18n dictionary overwrites `displayName`. The stale packages stay listed and resolvable via `getOrThrow` by name/version so existing flows keep working. Entries must never be removed from `DEPRECATED_RENAMED_PIECES`: the stale DB rows live forever, so removing an entry silently drops the label and reintroduces the indistinguishable duplicate.
