...
- `packages/web/src/app/builder/test-step/` — test execution UI (action/trigger sections, sample-data viewer, CTA buttons, `json-tree-skeleton.tsx` for the shared skeleton loading state); `test-runner-context.tsx` hoists `useTestAction` + the webhook-return dialog so the bottom CTA can fire the test in-tree; `custom-test-step/mcp-tool-testing-dialog.tsx` guards against a non-array stored `inputSchema` (GIT-1598 corruption, details in `mcp.md`)
