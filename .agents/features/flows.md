...
- **Loop Batching**: The `LOOP_ON_ITEMS` action has an optional `settings.batching` (`{ mode: LoopBatchMode, size }`); when present, the loop iterates over batches (sub-arrays) instead of single items, so each iteration's `item` is an array. `ITEMS_PER_BATCH` uses `chunk()` and `NUMBER_OF_BATCHES` uses `chunkIntoParts()` (both `@activepieces/core-utils`). Absent = per-element iteration (backward compatible, no migration). Schema in `packages/core/execution/src/lib/flows/actions/action.ts`; engine in `packages/server/engine/src/lib/handler/loop-executor.ts`; form (toggle + mode select + size input + downstream-array Alert) in `packages/web/src/app/builder/step-settings/loops-settings.tsx`. See **Batch** in the automation-core glossary.
