...
- **`drawer`** — slides up from the bottom of the sidebar, occupies 60% height, sits at `z-50` over the settings form (which is constrained to the remaining 40% so it stays scrollable). Dismisses on outside-click (pointerdown listener in `step-data/step-data-panel-host.tsx`, ignoring Radix poppers, role="dialog", and resizable handles), or via the hover-revealed chevron-down pill at the drawer's top edge.
