...
- `packages/web/src/app/builder/step-settings/piece-not-available-alert.tsx` — shared destructive `Alert` rendered by both `piece-settings` and `agent-settings` when the step's piece metadata API returns 404 (piece not installed or hidden by admin), naming the piece and version instead of showing the loading skeleton forever. `pieceModelNotFound` is threaded from `builder/index.tsx` (the hook's `isNotFound`) through `StepSettingsProvider`/`StepSettingsContext` to each settings component
- `packages/web/src/app/builder/step-settings/` — step configuration panel and split/drawer layout for the step data panel; `piece-settings/connection-select.tsx` is the per-step connection picker — it renders an inline status indicator for every connection state (Connected / Error / Missing, via `getConnectionStatusDisplay`) alongside a single unified `Cable` reconnect icon button (replacing the old per-state mix of refresh icon + text button). The indicator overlay is `pointer-events-none` so clicking the status label opens the dropdown; only the reconnect button re-enables pointer events and opens the reconnect dialog. The create/reconnect dialog's scope is derived from the operation, not the selected connection: `isReconnectingGlobalConnection` (from `reconnectConnection?.scope`) is passed to the dialog, so creating a new connection always defaults to PROJECT scope while reconnecting a global connection keeps PLATFORM scope — `isSelectedConnectionGlobal` is used only for the status/reconnect-button gating (`canShowConnectionStatus`).
- `packages/web/src/app/builder/step-data/` — step data panel UI (`step-data-panel-host.tsx` / `StepDataPanelHost` — panel wrapper for drawer/split mode; `close-panel-button.tsx` / `ClosePanelButton` — shared explicit close (✕) button placed in the toolbar of both the run-details and test-step panels; `step-data-panel-header.tsx`; `step-data-panel-view-toggle.tsx` — uses `PanelRight`/`PanelBottom` icons and labels the destination layout "Side by Side" / "Bottom Panel")
- `packages/web/src/app/builder/test-step/` — test execution UI (action/trigger sections, sample-data viewer, CTA buttons, `json-tree-skeleton.tsx` for the shared skeleton loading state); `test-runner-context.tsx` hoists `useTestAction` + the webhook-return dialog so the bottom CTA can fire the test in-tree
...
- `packages/web/src/app/builder/data-selector/` — variable picker (mentions / data selector); `index.tsx` hosts main's **Data** / **Variables** tabs, with a **Friendly** / **Advanced** toggle inside the Data tab. When a step is a PIECE action/trigger, friendly mode builds its tree from the piece's `outputSchema` via `utils-schema.ts`; when the step output is a top-level array and a schema is defined, `buildTreeFromArrayWithSchema` applies the schema's `fields` to each array element and labels items via the `itemLabel` template. Both modes render through the shared `data-selector-node.tsx` (friendly mode adds per-field type icons via `field-type-icon`). Otherwise it falls back to a generic field list. The schema is fetched via `usePieceOutputSchema` (see [pieces.md](../features/pieces.md))
...
- `notes-state.tsx` — sticky notes overlay. `moveNote({ id, position, anchor? })` sends UPDATE_NOTE; link and unlink both go through it so the stored fallback `position` is refreshed to the rendered position at link/unlink time; unlink must send `anchor: null` (not `undefined` — JSON drops undefined keys and the server-side spread would keep the stale anchor). Anchored notes render at the step node's post-orientation position + offset (`buildNotesGraph` in `flow-canvas-utils.ts`, fallback to stored `position` when the step is missing); the unlink button lives in `note-node/note-tools.tsx` (shown only when anchored). Pinning is a native React Flow **drag-to-connect**: notes and steps carry perpendicular note handles (`NOTE_HANDLE_IDS` LEFT/RIGHT in `layout-consts.ts`; on steps they are offset down 10px in horizontal mode so they clear the flow lines), revealed on node hover and kept visible when connected (`useHandleConnections`). The canvas enables `nodesConnectable` and gates creation with `isValidConnection` (note→step only); `onConnect` calls `moveNote` to set the anchor, and `NoteConnectionLine` renders the in-progress line reusing the anchor edge's bezier + note color. The persistent connector is a real edge (`ApEdgeType.NOTE_ANCHOR_EDGE`, `edges/note-anchor-edge.tsx`) emitted by `buildNotesGraph`; `chooseNoteAnchorHandles` picks the closest handle pair, the edge hides while its note is dragged, and its `zIndex` sits below the nodes. Anchored notes render at the step node's post-orientation position + offset (`resolveNotePosition`, fallback to stored `position` when the step is missing). Anchor offsets are stored in final-orientation space and applied verbatim in both orientations.
...
- **`drawer`** — slides up from the bottom of the sidebar, occupies 60% height, sits at `z-50` over the settings form (which is constrained to the remaining 40% so it stays scrollable). Dismisses on outside-click (pointerdown listener in `step-data/step-data-panel-host.tsx`, ignoring Radix poppers, role="dialog", and resizable handles), or via the explicit close (✕) button (`ClosePanelButton`) in the panel toolbar.
...
- **Smart Output Viewer** (`components/custom/smart-output-viewer/`) — used by the step data panel's output pane and run details. With hints, renders a labelled friendly view driven by the hints' `fields` array (type icons, copy-to-clipboard, expandable nested values via `children` / `listItems`, automatic table view for arrays of records, formatted images / emails / dates / file sizes / durations / currencies). Without hints, falls back to a generic field list for arbitrary JSON; object keys are rendered verbatim. A Raw JSON tab is always available. File outputs — a bare file-URL string or a field whose value is an Activepieces signed file URL — render a **Download File** button (`StepFileDownloadButton`) in the friendly view only; the Raw JSON tab shows the raw link.
...
**Large lists are virtualized.** Both surfaces render their row lists through the shared `VirtualizedList` (`components/ui/virtualized-list.tsx`, built on `@tanstack/react-virtual` with dynamic row measurement). It is threshold-gated: lists at or below `DEFAULT_VIRTUALIZE_THRESHOLD` (100 items) render inline exactly as before (native expand/collapse), and only larger lists window their rows so huge sample data (e.g. a code step returning `Array(50000)`) no longer mounts one DOM node per item and freezes the panel. It virtualizes against the **nearest scrollable ancestor** (the surrounding panel / data-selector scroll container) via `scrollMargin` rather than creating its own nested scroll box — a nested scroller gets clipped when it is taller than the visible area, leaving the last rows unreachable by the scrollbar. The ancestor is found by walking up for an element with `overflow-y: auto/scroll` or the Radix ScrollArea viewport (`data-slot="scroll-area-viewport"`, matched directly because Radix sets the viewport's overflow only after it measures content). The Smart Output Viewer uses it for its generic/schema top-level array lists (`output-array-list.tsx`, `output-schema-array-list.tsx`) and the nested list / matrix / generic-array / primitive sections of `output-field-row.tsx` (and `shared-value-rendering.tsx`); the Data Selector uses it for a tree node's children list in `data-selector-node.tsx`.
