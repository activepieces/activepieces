...
**Large lists are virtualized.** Both surfaces render their row lists through the shared `VirtualizedList` (`components/ui/virtualized-list.tsx`, built on `@tanstack/react-virtual` with dynamic row measurement). It is threshold-gated: lists at or below `DEFAULT_VIRTUALIZE_THRESHOLD` (100 items) render inline exactly as before (no nested scrollbar, native expand/collapse), and only larger lists window their rows into a capped-height scroll container so huge sample data (e.g. a code step returning `Array(50000)`) no longer mounts one DOM node per item and freezes the panel. The Smart Output Viewer uses it for its generic/schema top-level array lists (`output-array-list.tsx`, `output-schema-array-list.tsx`) and the nested list / matrix / generic-array / primitive sections of `output-field-row.tsx` (and `shared-value-rendering.tsx`); the Data Selector uses it for a tree node's children list in `data-selector-node.tsx`.
