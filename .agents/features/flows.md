...
- `packages/web/src/app/builder/step-settings/` — step configuration panel and split/drawer layout for the step data panel; `piece-settings/connection-select.tsx` is the per-step connection picker — it renders an inline status indicator for every connection state (Connected / Error / Missing, via `getConnectionStatusDisplay`) alongside a single unified `Cable` reconnect icon button (replacing the old per-state mix of refresh icon + text button). The indicator overlay is `pointer-events-none` so clicking the status label opens the dropdown; only the reconnect button re-enables pointer events and opens the reconnect dialog. The create/reconnect dialog's scope is derived from the operation, not the selected connection: `isReconnectingGlobalConnection` (from `reconnectConnection?.scope`) is passed to the dialog, so creating a new connection always defaults to PROJECT scope while reconnecting a global connection keeps PLATFORM scope — `isSelectedConnectionGlobal` is used only for the status/reconnect-button gating (`canShowConnectionStatus`). `piece-settings/index.tsx` splits action/trigger props into **essential** (always visible) and **advanced** (collapsed in an `AdvancedSection` toggle): a prop is advanced only when the piece sets `advanced: true` — everything else (including optional fields, `MARKDOWN` blocks, tab-group members, and checkbox reveal targets) stays essential. `propertyGroups` with `display: 'section'` render as titled cards while keeping the advanced overflow; `display: 'builder' | 'footer'` render the filter builder (all-essential).
...
- `packages/web/src/app/builder/pieces-selector/` — piece/action browser. When a piece action/trigger declares `classification: 'READ' | 'WRITE'`, a small grey inline badge appears next to the item's display name (`generic-piece-selector-item.tsx`)
- `packages/web/src/app/builder/piece-properties/` — property input widgets: `date-range-property`, `rich-text-property`, `number-stepper`, `static-dropdown-cards`, `filter-builder-layout` (the "Add filter" builder + footer), `filter-layout` (sectioned cards), `property-group-tabs`, `mention-chips-input`; `filter-property-utils.ts` holds active-filter detection, empty-value, and chip-label helpers. `properties-utils.tsx` routes each `PropertyType` to its widget
