...
- `packages/server/api/src/app/flows/flow-version/migrations/migrate-v22-agent-predefined-inputs.ts` — schema v22→v23 migration that converts legacy flat `predefinedInput` on agent piece tools to the `{ auth?, fields }` structure
...
- **Agent tool predefined-input migration (schema v22)**: `migrate-v22-agent-predefined-inputs` normalizes the flat `{ auth, propKey: value }` predefined-input shape (stored by flows published before PR #10704) to `{ auth?, fields: { propKey: { mode, value } } }`. The migration is idempotent — inputs already in the structured form are left untouched.
...
**FlowVersion**: id, flowId, displayName, schemaVersion (current latest: `'23'`), trigger (JSONB — full flow graph), connectionIds[], agentIds[], updatedBy, valid, state (DRAFT/LOCKED), backupFiles (JSONB), notes[] (JSONB). Relations: flow, updatedByUser.
