...
`builder/index.tsx` sizes the right sidebar with a plain CSS-width drawer (no more `react-resizable-panels` / `PanelImperativeHandle`): the drawer `<div>` animates its `width` via a native CSS transition (`STAGE_TRANSITION_MS`/`STAGE_TRANSITION_EASING`) and the flow canvas flex-shrinks to match, so open/close is a single sub-pixel width tween. The resting open width is the user's persisted `userSidebarWidth` (px, stored in localStorage under `SIDEBAR_WIDTH_STORAGE_KEY`) or the default `SIDEBAR_OPEN_WIDTH`, clamped to `SIDEBAR_MAX_FRACTION` of the split. A left-edge drag handle (`role="separator"` with pointer capture, `startSidebarResize`/`moveSidebarResize`/`endSidebarResize`) lets the user resize it, committing the width on pointer release. The step data panel's own `drawer`/`split` layout (above) is toggled manually via `step-data-panel-view-toggle.tsx`; the old width-driven auto-collapse and the `useAnimateSidebar` hook have both been removed.
