...
`step-settings/index.tsx` composes the two layouts via `StepSettingsLayout`. The bottom CTA (`TestStepCTAButton`) shows under the settings form when the drawer is closed; clicking it opens the drawer and (for actions) auto-fires the test by calling `useActionTestRunner().fireTest()` from context. `ActionTestRunnerProvider` owns the React Query mutation and the return-response webhook dialog so the CTA and the in-panel button share one source of truth. The provider is wired with the Zustand-backed `selectedStep` (not the RHF form values), so `step.valid` stays in sync with the resolver-computed validity that `applyOperation` writes to canvas state — RHF never writes the resolver's `valid` back into its own form store, so reading from `form.getValues()` would observe a stale value for freshly-added steps.
