...
Steps can also be **dragged to custom positions**: dropping on an add button still moves the step within the flow (dnd-kit, unchanged), dropping anywhere else stores a position override (`canvas-state.ts`, persisted per flow + orientation). Edges compare their actual endpoints against the expected auto-layout geometry (with a small tolerance — handles render a couple of px outside node bounds) and fall back to `getSmoothStepPath` when a node was dragged; straight-line edges target the actual next-step node (not the invisible `subgraph-end`) so moved nodes stay connected, and structure (router/loop) steps emit a reconnect edge when their follower is dragged. A wand *Reset layout* control clears the overrides.
