...
- `packages/web/src/features/connections/components/revalidate-connection-button.tsx` — recheck button that calls the revalidate endpoint and patches the row's status in the query cache with a toast
...
- `POST /v1/app-connections/:id/revalidate` — run a live credential check (`engineValidateAuth`) on an existing connection and flip its status to `ACTIVE` or `ERROR`; requires `WRITE_APP_CONNECTION`, is project-scoped, and returns the connection with sensitive data stripped. Engine failures re-throw rather than marking the connection `ERROR`.
...
The project connections page (`/connections`) shows a data table with status badges, piece icons, scope indicators, and bulk-delete support. Connection display names fill the available column width and show a tooltip with the full name when truncated; hovering the piece icon reveals the connection's External ID and a copy control. Bulk-delete operates only on `PROJECT`-scope rows — `PLATFORM`-scope (global) connections are filtered out client-side and the delete button reflects only the deletable count; global connections must be deleted from the platform admin global connections page. The `new-connection-dialog` opens `create-edit-connection-dialog` which renders piece-auth-specific form fields. The "Replace" action opens `replace-connections-dialog` which calls the replace endpoint. The platform admin global connections page (`/platform/setup/connections`) uses `global-connections-hooks` and surfaces `edit-global-connection-dialog` for managing platform-scope connections and their `projectIds` assignment. The builder uses `connection-select` inside step settings to pick, create, or reconnect connections inline; active connections show a "Connected" status with a compact reconnect icon, while connections needing re-authentication keep a "Reconnect" action. Each connections-page row also has a "Recheck connection" button (RefreshCw icon) next to Reconnect that calls the revalidate endpoint — a stale connection showing `ACTIVE` flips to `ERROR` (toast on failure) so the user knows to reconnect; the button owns its own mutation and patches the row in the query cache so the rest of the table does not re-render.

The connections page also feeds Stage awareness: `use-report-connections-excerpt.ts` publishes a compact snapshot of the visible rows (display name, piece, status, flow count + total) to the workspace-shell `StageContext` via `reportStageExcerpt`, so the docked chat can resolve terse on-screen references (e.g. "the 17 0 0 in the Flows column") without re-fetching. The excerpt is recomputed from the current page and cleared on unmount; it is a no-op when the Stage context is absent (`useStageOptional`), so the page still works outside the shell.
