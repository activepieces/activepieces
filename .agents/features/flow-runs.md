...
- **syncRun (queue-routing hint)**: Non-persisted flag on `addToQueue`/`jobQueue.add` (never on `ExecuteFlowJobData` — no schema bump) marking a run whose HTTP caller is actively awaiting the response, making it eligible for the dedicated `syncJobs` queue while an `AP_WORKER_QUEUE=syncJobs` worker is online (see the sync-queue entry in [workers.md](workers.md)). `addToQueue` defaults it to `!isNil(params.workerHandlerId)`; `enqueueResume` overrides with `!isNil(workerHandlerId)` computed from its **explicit** param before merging the stored `waitpoint.workerHandlerId` fallback — so sync webhook starts and live sync-resume links qualify, while delay/approval/subflow-completion resumes (handler id inherited from pause time, caller likely timed out) stay in the shared queue even though they still best-effort deliver a response through the fallback id.
