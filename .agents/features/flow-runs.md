...
- **Waitpoint**: Row in the `waitpoint` table representing one paused step on a run. Fields: `type` (DELAY|WEBHOOK), `version` (V0|V1 — V1 is the current API), `status` (PENDING|COMPLETED), `stepName`, `resumeDateTime`, `responseToSend`, `resumePayload`, `workerHandlerId`, `httpRequestId`, `expectedCount` (nullable int). Unique on `(flow_run_id, step_name)`.
- **Join Waitpoint**: a Waitpoint with a non-null `expectedCount`. Instead of resuming on a single signal, it resumes when `COUNT(terminal children WHERE parentRunId = run)` reaches `expectedCount` — the Subflow Fan-in tally. `flow-runs-queue.ts` (`completeFanInIfDone`) runs the COUNT when a child lands terminal and once more when the parent's own PAUSED status uploads (seals the finish-before-pause race). COUNT is the sole source of truth — idempotent under retries/races — and `complete()` (pessimistic-locked, `WHERE status=PENDING`) makes the resume exactly-once. No Redis, no counter. See ADR-0010.
