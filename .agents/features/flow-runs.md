...
- `packages/server/api/src/app/flows/flow-run/stuck-run-sweeper.ts` — per-minute `SystemJobName.STUCK_RUN_SWEEP` job (registered in `app.ts`) that finalizes runs orphaned in `RUNNING` when their worker died mid-run. Liveness signal is the row's `updated` timestamp: the engine's 15 s snapshot flush (`flow-run-progress-reporter.ts`) bumps it on every live run, so `status = RUNNING AND updated < now() − (FLOW_TIMEOUT_SECONDS + 600 s grace)` can only match runs no worker is executing (the grace also covers BullMQ's 8-minute stalled-retry backoff, so a requeued second attempt is never reaped mid-flight). Matches are finalized as `TIMEOUT` through `runsMetadataQueue` — the same write path cancel uses — so `onFinish` side effects and the parent-failure cascade fire normally; no `failedStep` is set, so no issue/alert is created. Batch-capped at 250 per sweep (next tick drains the rest); backed by the partial index `idx_run_updated_where_status_running`. `startTime` is deliberately NOT the signal — it is set at queue time on retries, so a queued-backlog run would be falsely killed. Finalization uses `runsMetadataQueue.addIfNoPendingWrite` (atomic Lua `mergeIfKeyAbsent` on the `runs_metadata:<id>` hash): the sweep claims the key only if no metadata write is in flight, so it can never overwrite a run's real terminal status, while a later engine write overwriting the sweep's TIMEOUT is allowed by design (engine truth wins). No-op'd candidates resolve via the pending write itself — the metadata worker either finalizes the run or refreshes `updated`, moving it out of the stale predicate. The cron is not registered in the TESTING environment: integration fixtures seed RUNNING rows with faker-backdated `updated`, and a live sweep would reap them mid-test.
...
- Constraint: only terminal states, within the project's effective retention window (`getEffectiveExecutionDataRetentionDays`: the project's `executionDataRetentionDays` override clamped into `[PAUSED_FLOW_TIMEOUT_DAYS, EXECUTION_DATA_RETENTION_DAYS]`, or the instance value when unset, see [file-storage.md](./file-storage.md)).
