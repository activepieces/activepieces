...
- **Subflow**: A child run linked via `parentRunId`, created when a flow calls another flow as a step. If a subflow fails and propagates the failure, the parent run lookup is strictly scoped by the child's `projectId` so that cross-project/cross-tenant parent runs are ignored.
...
- **Waitpoints (V1, current):** pieces call `ctx.run.createWaitpoint({ type, ... })` + `ctx.run.waitForWaitpoint(id)`. Engine POSTs `/v1/waitpoints`; server inserts a PENDING row keyed on `(flow_run_id, step_name)`.
  ...
  - `SUBFLOW_FAILURE` propagation: When a child run fails and `failParentOnFailure` is active, it completes the parent waitpoint with an error payload. This lookup is strictly scoped by `projectId` so that cross-project/cross-tenant parent runs are ignored instead of resumed.
