...
- **Cross-project isolation (subflow parent-fail):** `markParentRunAsFailed` scopes its parent-run lookup to `{ id: parentRunId, projectId }` using the child run's authenticated `projectId`. `parentRunId`/`failParentOnFailure` ride in from spoofable webhook headers (`ap-parent-run-id`/`ap-fail-parent-on-failure`) on the public webhook endpoint, so without the scope a failed child in project A could complete a paused parent's waitpoint and resume it in project B. A cross-project parent id now returns no match and the fail becomes a no-op; legitimate subflows are always same-project (Call Flow only targets flows in the caller's project), so real usage is unaffected.
