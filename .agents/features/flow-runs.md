...
- **TOCTOU recovery (callback/metadata-worker race):** A webhook callback may read the flow run as `RUNNING` (completing the waitpoint) while the metadata worker has already written `PAUSED` but checked the waitpoint while it was still `PENDING`. After `handleResumeSignal` returns, `resumeFromWaitpoint` re-reads the current flow run status and waitpoint. If the flow is now `PAUSED` but a `COMPLETED` waitpoint persists, it enqueues the resume directly — the metadata worker already checked and won't re-check. BullMQ deduplication by `flowRun.id` prevents double-resume if both the recovery path and the metadata worker's normal PAUSED path race together.
