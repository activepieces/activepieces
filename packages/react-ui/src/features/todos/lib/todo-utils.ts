import { PopulatedTodo, TodoActivityWithUser } from "@activepieces/shared";
...
export const todoUtils = {
        ...
        switch (authorType) {
            case 'user': {
                if ('createdByUser' in todo && todo.createdByUser) {
                    return todo.createdByUser?.firstName + ' ' + todo.createdByUser?.lastName;
                }
    ...
    getAuthorType: (todo: PopulatedTodo | TodoActivityWithUser): 'agent' | 'flow' | 'user' => {
        if ('createdByUser' in todo && todo.createdByUser) {
            return 'user';
        }
        if ('user' in todo && todo.user) {
