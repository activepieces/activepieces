...
const MoveFlowDialog = ({ children, flows, onMoveTo }: MoveFlowDialogProps) => {
  ...
  const { mutate, isPending } = useMutation<PopulatedFlow[], Error, MoveFlowFormSchema>({
        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit((data) => mutate(data))}>
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <Select onValueChange={field.onChange} disabled={isLoading || folders?.length === 0}>
                    <SelectTrigger>
                      <SelectValue placeholder={t('Select Folder')} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectGroup>
                        {folders && folders.length === 0 && <SelectItem value="NULL">{t('No Folders')}</SelectItem>}
                        {folders &&
                          folders.map((folder) => (
