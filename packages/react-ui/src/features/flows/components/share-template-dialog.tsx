...
const ShareTemplateDialog: React.FC<{
  ...
  const { mutate, isPending } = useMutation<FlowTemplate, Error, { flowId: string; description: string }>({
    mutationFn: async () => {
    ...
    <Dialog open={isShareDialogOpen} onOpenChange={(open) => setIsShareDialogOpen(open)}>
      ...
      <DialogContent>
        ...
        <Form {...shareTemplateForm}>
          <form className="grid space-y-4" onSubmit={shareTemplateForm.handleSubmit(onShareTemplateSubmit)}>
            <FormField
            ...
            {shareTemplateForm?.formState?.errors?.root?.serverError && (
              <FormMessage>{shareTemplateForm.formState.errors.root.serverError.message}</FormMessage>
            )}
