import { Action, ActionErrorHandlingOptions, ActionType, CodeAction, LoopOnItemsAction, PieceAction, Trigger } from "@activepieces/shared";
...
function formatSampleData(sampleData: unknown, type: ActionType) {
    ...
    const shouldRemoveIterations = type === ActionType.LOOP_ON_ITEMS && sampleData && typeof sampleData === 'object' && 'iterations' in sampleData;
...
function buildActionWithSampleData(selectedStep: Action, sampleData: unknown): Action {
...
function buildActionWithNewCode(selectedStep: CodeAction, code: string): CodeAction {
    const clonedAction: CodeAction = JSON.parse(JSON.stringify(selectedStep));
    clonedAction.settings.sourceCode.code = code;
    return clonedAction
}

function buildActionWithNewLoopItems(selectedStep: LoopOnItemsAction, items: string): LoopOnItemsAction {
    const clonedAction: LoopOnItemsAction = JSON.parse(JSON.stringify(selectedStep));
    clonedAction.settings.items = items;
    return clonedAction
}

function buildActionWithErrorOptions<T extends Action | Trigger>(selectedStep: T, {
    continueOnFailure,
    retryOnFailure, 
}: { continueOnFailure: boolean | undefined, retryOnFailure: boolean | undefined }): Action {
    ...
    switch(clonedAction.type) {
        ...
        case ActionType.CODE: {
            const errorHandlingOptions: ActionErrorHandlingOptions = {
                continueOnFailure: {
                    value: continueOnFailure ?? clonedAction.settings.errorHandlingOptions?.continueOnFailure?.value ?? false,
                },
                retryOnFailure: {
                    value: retryOnFailure ?? clonedAction.settings.errorHandlingOptions?.retryOnFailure?.value ?? false,
                },
            };
        ...
        default: {
            throw new Error(`Action type ${selectedStep.type} should not have error handling options`);
        }
    }   
}
