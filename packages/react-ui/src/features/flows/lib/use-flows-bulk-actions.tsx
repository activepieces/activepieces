...
import { ChevronDown, CornerUpLeft, Download, Trash2, UploadCloud } from 'lucide-react'
...
import { FlowVersionState, Permission, PopulatedFlow } from '@activepieces/shared'
...
export const useFlowsBulkActions = ({
  ...
  const userHasPermissionToUpdateFlow = useAuthorization().checkAccess(Permission.WRITE_FLOW)
  const userHasPermissionToWriteFolder = useAuthorization().checkAccess(Permission.WRITE_FOLDER)
  const userHasPermissionToWroteProjectRelease = useAuthorization().checkAccess(Permission.WRITE_PROJECT_RELEASE)
  const allowPush = selectedRows.every(
    (flow) => flow.publishedVersionId !== null && flow.version.state === FlowVersionState.LOCKED,
  )
  ...
  const { mutate: exportFlows, isPending: isExportPending } = flowsHooks.useExportFlows()
  return useMemo(() => {
        ...
        render: (_, resetSelection) => {
          return (
            <div onClick={(e) => e.stopPropagation()}>
              <DropdownMenu modal={true} open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
                {selectedRows.length > 0 ? (
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      {selectedRows.length > 0 ? `${t('Actions')} (${selectedRows.length})` : t('Actions')}
                ...
                <DropdownMenuContent>
                  <PermissionNeededTooltip hasPermission={userHasPermissionToWroteProjectRelease}>
                    <PublishedNeededTooltip allowPush={allowPush}>
                      <PushToGitDialog flows={selectedRows}>
                        <DropdownMenuItem
                          disabled={!userHasPermissionToWroteProjectRelease || !allowPush}
                          onSelect={(e) => {
                            e.preventDefault()
                          }}
                        >
                  ...
                  <PermissionNeededTooltip hasPermission={userHasPermissionToUpdateFlow}>
                    <ConfirmationDeleteDialog
                      ...
                      mutationFn={async () => {
                        await Promise.all(selectedRows.map((flow) => flowsApi.delete(flow.id)))
                        setRefresh(refresh + 1)
                        resetSelection()
                        setSelectedRows([])
                        refetch()
