...
export const flowsApi = {
  ...
  update(flowId: string, request: FlowOperationRequest, showErrorToast = false) {
    return api.post<PopulatedFlow>(`/v1/flows/${flowId}`, request).catch((error) => {
      if (showErrorToast) {
        ...
        if (errorCode === ErrorCode.FLOW_IN_USE) {
          toast({
            ...
            description: t('Flow is being used by another user, please try again later.'),
            duration: Infinity,
            action: (
  ...
  get(flowId: string, request?: GetFlowQueryParamsRequest): Promise<PopulatedFlow> {
    return api.get<PopulatedFlow>(`/v1/flows/${flowId}`, request)
  },
  listVersions(flowId: string, request: ListFlowVersionRequest): Promise<SeekPage<FlowVersionMetadata>> {
    return api.get<SeekPage<FlowVersion>>(`/v1/flows/${flowId}/versions`, request)
  },
