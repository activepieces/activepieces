...
import { Flow, FlowVersion, PopulatedFlow, TriggerType } from '@activepieces/shared'
...
const zipFlows = async (flows: PopulatedFlow[]) => {
  ...
  for (const flow of flows) {
    ...
    zip.file(`${flow.version.displayName}_${flow.id}.json`, JSON.stringify(template, null, 2))
  }
  return zip
}
    ...
    switch (trigger.type) {
      case TriggerType.PIECE: {
        ...
        return cronExpression
          ? `${t('Run')} ${cronstrue.toString(cronExpression, { locale: 'en' }).toLocaleLowerCase()}`
          : t('Real time flow')
      }
