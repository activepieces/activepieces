  ...
  useSampleDataForFlow: (flowVersion: FlowVersion | undefined) => {
    return useQuery({
        ...
        queryFn: async() => {
            ...
            const singleStepSampleData = await Promise.all(steps.map(async (step) => {
                return {
                    [step.name]: await sampleDataApi.get({ flowId: flowVersion!.flowId, flowVersionId: flowVersion!.id, stepName: step.name })
                }
            }));
