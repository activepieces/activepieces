...
async function cacheFlagsIfNotCached(queryClient: QueryClient): Promise<Record<ApFlagId, unknown>> {
    const cachedFlags = queryClient.getQueryData<Record<ApFlagId, unknown>>(['flags']);
    if (cachedFlags) {
        return cachedFlags;
    }
