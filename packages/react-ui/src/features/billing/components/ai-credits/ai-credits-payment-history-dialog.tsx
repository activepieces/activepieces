...
import { PlatformAiCreditsPaymentStatus, PlatformAiCreditsPaymentType, PlatformAiCreditsPayment } from '@activepieces/shared';
import { formatUtils } from '@/lib/utils'; // Assuming formatUtils exists, or I will use Intl
      ...
      <DialogContent className="max-w-[800px]">
                            ...
                            <TableRow key={payment.id}>
                                ...
                                <TableCell>
                                    <Badge variant="outline">
                                        {payment.type === PlatformAiCreditsPaymentType.AUTO_TOPUP
                                            ? t('Auto Top-up')
                                            : t('Manual Purchase')}
                                    </Badge>
                                </TableCell>
                                <TableCell>
                                    {payment.aiCredits.toLocaleString()}
                                </TableCell>
                                <TableCell>
        ...
        {data?.next && (
            <div className="flex justify-center pt-2">
                <Button variant="outline" size="sm" onClick={() => setCursor(data.next!)}>
                    {t('Next Page')}
                </Button>
            </div>
        )}
         {data?.previous && (
            <div className="flex justify-center pt-2">
                <Button variant="outline" size="sm" onClick={() => setCursor(data.previous!)}>
                    {t('Previous Page')}
                </Button>
            </div>
        )}
...
const getStatusBadgeVariant = (status: PlatformAiCreditsPaymentStatus): BadgeProps['variant'] => {
    switch (status) {
