...
export const createServerState = (
  ...
  return {
    ...
    updateRecord: (recordIndex: number, record: Pick<ClientRecordData, 'values'>) => {
      addPromiseToQueue(async () => {
        clonedRecords[recordIndex] = await recordsApi.update(clonedRecords[recordIndex].id, {
          tableId: clonedTable.id,
    ...
    deleteRecords: (recordIndices: string[]) => {
      addPromiseToQueue(async () => {
        const recordIds = recordIndices.map((index) => clonedRecords[parseInt(index)].id)
        await recordsApi.delete({
          ids: recordIds,
        })

        // Sort indices in descending order to avoid shifting issues when splicing
        const sortedIndices = recordIndices.map((index) => parseInt(index)).sort((a, b) => b - a)
