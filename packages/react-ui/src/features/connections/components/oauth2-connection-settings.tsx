...
const OAuth2ConnectionSettings = ({
  ...
  useEffect(() => {
    ...
    if (redirectUrl) {
      form.setValue('request.value.redirect_url', redirectUrl, { shouldValidate: true });
    }
    form.setValue('request.value.client_secret', currentOAuth2Type === AppConnectionType.OAUTH2 ? '' : 'FAKE_SECRET', { shouldValidate: true });
    form.setValue('request.value.client_id', currentOAuth2Type === AppConnectionType.OAUTH2 ? '' : (pieceToClientIdMap?.[piece.name].clientId ?? ''), { shouldValidate: true });
    form.setValue('request.value.code', '', { shouldValidate: true });
    form.setValue('request.value.code_challenge', '', { shouldValidate: true });
    form.setValue('request.value.type', currentOAuth2Type!, { shouldValidate: true });
  ...
  useEffect(() => {
    ...
    const clientSecret = form.getValues().request.value.hasOwnProperty('client_secret') ? (form.getValues().request as UpsertOAuth2Request).value.client_secret : undefined;
  ...
  async function openPopup(redirectUrl: string, clientId: string) {
    ...
    form.setValue('request.value.code_challenge', codeChallenge, { shouldValidate: true });
    ...
    <Form {...form}>
      ...
      >
        ...
        <div className='border border-solid p-2 rounded-lg gap-2 flex text-center items-center justify-center h-ful'>
          ...
          {!hasCode && <Button size={"sm"} variant={"basic"} disabled={!readyToConect} onClick={async () => openPopup(redirectUrl!, form.getValues().request.value.client_id)}>Connect</Button>}
          {hasCode && <Button size={"sm"} variant={"basic"} className="text-destructive" onClick={() => {
            form.setValue('request.value.code', '', { shouldValidate: true });
            form.setValue('request.value.code_challenge', '', { shouldValidate: true });
          }}>Disconnect</Button>}
        </div>
