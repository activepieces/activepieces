...
export function InvitationCard({ invitation }: { invitation: UserInvitation }) {
  ...
  const userHasPermissionToRemoveInvitation = checkAccess(Permission.WRITE_INVITATION)
  async function deleteInvitation() {
    await userInvitationApi.delete(invitation.id)
    refetch()
  }
  return (
    <div className="flex items-center justify-between space-x-4" key={invitation.id}>
      <div className="flex items-center space-x-4">
        <Avatar className="hidden size-9 sm:flex">
          <AvatarImage src="/avatars/05.png" alt={t('Avatar')} />
          <AvatarFallback className="justify-center items-center flex">
            <span className="p-2">{invitation.email.charAt(0).toLocaleUpperCase()}</span>
          </AvatarFallback>
        </Avatar>
        <div>
      ...
      <div className="flex gap-2">
        <PermissionNeededTooltip hasPermission={userHasPermissionToRemoveInvitation}>
          ...
          >
            <Button disabled={!userHasPermissionToRemoveInvitation} variant="ghost" className="size-8 p-0">
