...
const InstallPieceDialog = ({ onInstallPiece, scope }: InstallPieceDialogProps) => {
  ...
  const { data: privatePiecesEnabled } = flagsHooks.useFlag<boolean>(ApFlagId.PRIVATE_PIECES_ENABLED)
  ...
  const handleArchiveUpload = async (file: File) => {
    if (file && file.name.endsWith('.tgz')) {
      try {
        ...
        const packageJsonMatch = text.match(/package\.json.*?{[^}]*"name"\s*:\s*"([^"]+)".*?"version"\s*:\s*"([^"]+)"/s)
        if (packageJsonMatch) {
      ...
      <DialogContent>
        ...
        <FormProvider {...form}>
          <form
            ...
            onSubmit={form.handleSubmit((data) => mutate(data as AddPieceRequestBody))}
          >
            <FormField
              name="packageType"
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectGroup>
                        <SelectItem value={PackageType.REGISTRY}>{t('NPM Registry')}</SelectItem>
                        <SelectItem value={PackageType.ARCHIVE} disabled={!isEnabled || !privatePiecesEnabled}>
