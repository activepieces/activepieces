...
import { Action, ActionType, SuggestionType, Trigger, TriggerType, flowPieceUtil } from '@activepieces/shared'

import { INTERNAL_ERROR_TOAST, toast } from '../../../components/ui/use-toast'

import { CORE_STEP_METADATA, piecesApi } from './pieces-api'
import { PieceSelectorItem, StepMetadata, StepMetadataWithSuggestions } from './types'
...
export const piecesHooks = {
  ...
  useMostRecentAndExactPieceVersion: ({ name, version, enabled = true }: UsePieceAndMostRecentPatchProps) => {
    ...
    const query = useQuery<StepMetadataWithSuggestions[], Error>({
      ...
      queryFn: async () => {
        const pieces = await piecesApi.list({
          searchQuery,
          suggestionType: type === 'action' ? SuggestionType.ACTION : SuggestionType.TRIGGER,
        })
        const piecesMetadata = pieces
          .filter((piece) => (type === 'action' && piece.actions > 0) || (type === 'trigger' && piece.triggers > 0))
          .map((piece) => {
  ...
  usePieceActionsOrTriggers: ({
    ...
    return useQuery<PieceSelectorItem[], Error>({
      queryKey: ['pieceMetadata', stepMetadata?.type, stepMetadata?.displayName],
          ...
          switch (stepMetadata.type) {
            ...
            case ActionType.PIECE: {
              ...
              return Object.values(
                stepMetadata.type === TriggerType.PIECE ? pieceMetadata.triggers : pieceMetadata.actions,
              )
            }
