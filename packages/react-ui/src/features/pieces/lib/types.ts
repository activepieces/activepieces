import { ActionBase, PieceAuthProperty, PieceMetadataModelSummary, TriggerBase } from '@activepieces/pieces-framework'
import {
  ActionType,
  FlowOperationType,
  PackageType,
...
type PrimitiveStepMetadata = BaseStepMetadata & {
  type: ActionType.CODE | ActionType.LOOP_ON_ITEMS | ActionType.ROUTER | TriggerType.EMPTY
}
...
export type StepMetadataWithSuggestions = PieceStepMetadataWithSuggestions | PrimitiveStepMetadata
...
export type PieceSelectorOperation =
  ...
  | {
      ...
      actionLocation: {
        parentStep: string
        stepLocationRelativeToParent: Exclude<StepLocationRelativeToParent, StepLocationRelativeToParent.INSIDE_BRANCH>
      }
    }
...
export type HandleSelectCallback = (piece: StepMetadata, item: PieceSelectorItem, type?: string) => void
