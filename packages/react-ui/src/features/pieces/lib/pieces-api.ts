...
export const piecesApi = {
  ...
  get(request: GetPieceRequestParams & GetPieceRequestQuery): Promise<PieceMetadataModel> {
  ...
  options<T extends PropertyType.DROPDOWN | PropertyType.MULTI_SELECT_DROPDOWN | PropertyType.DYNAMIC>(
    ...
    return api.post<ExecutePropsResult<T>>(`/v1/pieces/options`, request).catch((error) => {
      ...
      toast({
        title: t('Error'),
        description: t('An internal error occurred while fetching data, please contact support'),
        variant: 'destructive',
      })
      ...
      const defaultStateForDropdownProperty: ExecutePropsResult<PropertyType.DROPDOWN> = {
        options: {
          options: [],
          disabled: true,
          placeholder: t('An internal error occurred, please contact support'),
        },
        type: PropertyType.DROPDOWN,
      }
      return (
        propertyType === PropertyType.DYNAMIC ? defaultStateForDynamicProperty : defaultStateForDropdownProperty
      ) as ExecutePropsResult<T>
    })
  },
  mapToMetadata(type: 'action' | 'trigger', piece: PieceMetadataModelSummary | PieceMetadataModel): PieceStepMetadata {
    return {
  ...
  async getMetadata(step: Action | Trigger): Promise<StepMetadata> {
    ...
    switch (step.type) {
      ...
      case TriggerType.PIECE: {
        ...
        const metadata = await piecesApi.mapToMetadata(step.type === ActionType.PIECE ? 'action' : 'trigger', piece)
        return {
          ...metadata,
          ...spreadIfDefined('logoUrl', customLogoUrl),
        }
