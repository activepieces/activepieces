...
import { mcpHooks } from '../lib/mcp-hooks';
...
export const McpPieceDialog = React.memo(
  ({ children, mcpPieceToUpdate }: McpPieceDialogProps) => {
    ...
    const piecesOptions = pieces?.map((piece) => ({
    ...
    const form = useForm<{pieceName:string, connectionId:string|null }>({
      ...
      resolver: (values)=>{
        ...
        if(values.pieceName){
          const piece = pieces?.find((piece) => piece.name === values.pieceName);
     ...
     const selectedPiece = pieces?.find((piece)=>piece.name === form.watch('pieceName'))
     const { data: connections, isLoading: connectionsLoading, refetch: refetchConnections, isRefetching: isRefetchingConnections } =
   ...
   const connectionOptionsWithNewConnectionOption = [{label: t('+ New Connection'), value: ''},...connectionOptions]
 

   const onSubmit = (values: {pieceName:string, connectionId:string|null})=>{
    if(mcp)
    {
      ...
      <>
       
       {
        ...
        >
         ...
         <DialogHeader>
              <DialogTitle>
              {mcpPieceToUpdate? `${t('Edit Tool')} (${pieces?.find((piece)=>piece.name === mcpPieceToUpdate.pieceName)?.displayName})` : t('Add Tool')}
              </DialogTitle>
            </DialogHeader>
            ...
            <Form {...form}>
              ...
              isNil(mcpPieceToUpdate) && (
                ...
                render={({field})=>{
                  ...
                  <SearchableSelect
                      ...
                      const piece = pieces?.find((piece) => piece.name === value)!;
                      return <div className="flex gap-2 items-center">
                        <img src={piece.logoUrl} alt={piece.displayName} className="w-4 h-4 object-contain" />
             ...
             <DialogFooter>
              ...
              <DialogClose asChild>
                ...
                <Button type="submit" variant="default" loading={addOrUpdateMcpPieceMutation.isPending}>
                   {t("Confirm")}
                </Button>
