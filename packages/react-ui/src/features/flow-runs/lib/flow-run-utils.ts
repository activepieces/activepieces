import { Check, CircleCheck, CircleX, PauseCircleIcon, PauseIcon, Timer, X } from 'lucide-react'
...
export const flowRunUtils = {
  ...
  getStatusIconForStep(stepOutput: StepOutputStatus): {
    ...
    Icon: typeof Timer | typeof CircleCheck | typeof PauseCircleIcon | typeof CircleX
  } {
    switch (stepOutput) {
...
function findLoopsState(flowVersion: FlowVersion, run: FlowRun, currentLoopsState: Record<string, number>) {
  const loops = flowStructureUtil.getAllSteps(flowVersion.trigger).filter((s) => s.type === ActionType.LOOP_ON_ITEMS)
...
function findFailedStepInOutput(steps: Record<string, StepOutput>): string | null {
...
function getLoopChildStepOutput(
  ...
  while (index < parents.length) {
    ...
    if (childOutput && childOutput.output && childOutput.output.iterations[loopIndexes[currentParentName]]) {
      const stepName = index + 1 < parents.length ? parents[index + 1].name : childName
      childOutput = childOutput.output.iterations[loopIndexes[parents[index].name]][stepName] as
        | LoopStepOutput
        | undefined
    }
    index++
...
function extractStepOutput(
  ...
  const parents: LoopOnItemsAction[] = flowStructureUtil
    ...
    .filter(
      (p) => p.type === ActionType.LOOP_ON_ITEMS && flowStructureUtil.isChildOf(p, stepName),
