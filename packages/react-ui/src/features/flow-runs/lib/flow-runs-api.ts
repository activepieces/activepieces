...
export const flowRunsApi = {
  ...
  testStep(socket: Socket, request: Omit<CreateStepRunRequestBody, 'id'>): Promise<StepRunResponse> {
    const id = nanoid()
...
function getInitialRun(socket: Socket, flowVersionId: string): Promise<FlowRun> {
