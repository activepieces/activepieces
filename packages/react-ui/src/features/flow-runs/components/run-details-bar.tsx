...
const RunDetailsBar = React.memo(({ run, canExitRun, exitRun, isLoading }: RunDetailsBarProps) => {
  const { Icon, variant } = run
    ? flowRunUtils.getStatusIcon(run.status)
    : { Icon: QuestionMarkIcon, variant: 'default' }

  const { data: timeoutSeconds } = flagsHooks.useFlag<number>(ApFlagId.FLOW_RUN_TIME_SECONDS)
  const { data: memoryLimit } = flagsHooks.useFlag<number>(ApFlagId.FLOW_RUN_MEMORY_LIMIT_KB)
  ...
  return (
    ...
    >
      ...
      <div className="flex-col flex flex-grow text-foreground gap-0">
        <div className="text-sm">{getStatusText(run.status, timeoutSeconds ?? -1, memoryLimit ?? -1)}</div>
        <div className="text-xs text-muted-foreground">{run?.id ?? t('Unknown')}</div>
      </div>
      {canExitRun && (
        <Button
          variant={'outline'}
