import { Action, ActionType, FlowVersion, Trigger, isNil } from "@activepieces/shared";
...
function traverseFlow(step: Action | Trigger | undefined): ApGraph {
    ...
    switch (step.type) {
        case ActionType.BRANCH: {
            ...
            const childGraph = offsetGraph(traverseFlow(onSuccessAction), { x: -150, y: VERTICAL_OFFSET });
            if (childGraph.nodes.length > 0) {
                graph.edges.push(addEdge(stepToNode(step), childGraph.nodes[0]));
            }
            const childGraphFailure = offsetGraph(traverseFlow(onFailureAction), { x: 150, y: VERTICAL_OFFSET });
    ...
    const childGraph = offsetGraph(traverseFlow(nextAction), { x: 0, y: VERTICAL_OFFSET });
    if (childGraph.nodes.length > 0) {
        graph.edges.push(addEdge(stepToNode(step), childGraph.nodes[0]));
    }
