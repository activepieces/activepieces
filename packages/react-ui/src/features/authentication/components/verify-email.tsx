...
const VerifyEmail = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (error) => {
      if (api.isError(error) && error.response?.status === HttpStatusCode.Gone) {
        setIsExpired(true)
        setTimeout(() => navigate('/sign-in'), 3000)
      } else {
        console.error(error)
          ...
          <div className="gap-4 w-full flex flex-row items-center justify-center">
            ...
            {isPending && !isExpired && (
              <>
                ...
                <span className="text-left w-fit">{t('Verifying email...')}</span>
              </>
            )}
            ...
            {isExpired && (
              <>
                ...
                <span className="text-left w-fit">{t('invitation has expired, redirecting to sign in...')}</span>
              </>
            )}
          </div>
        </div>
