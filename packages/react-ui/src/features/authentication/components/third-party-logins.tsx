...
import { ApFlagId, ThirdPartyAuthnProviderEnum, ThirdPartyAuthnProvidersToShowMap } from '@activepieces/shared'
...
const ThirdPartyLogin = React.memo(({ isSignUp }: { isSignUp: boolean }) => {
  ...
  const { data: thirdPartyAuthProviders } = flagsHooks.useFlag<ThirdPartyAuthnProvidersToShowMap>(
    ...
    <div className="flex flex-col gap-4">
      {thirdPartyAuthProviders?.google && (
        <Button
          ...
          onClick={(e) => handleProviderClick(e, ThirdPartyAuthnProviderEnum.GOOGLE)}
        >
