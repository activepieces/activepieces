...
const passwordRules: ValidationRule[] = [
  ...
  { label: t('Lowercase'), condition:   (password:string)=> LOWERCASE_REGEX.test(password) },
  { label: t('Uppercase'), condition:  (password:string)=> UPPERCASE_REGEX.test(password) },
  { label: t('Number'), condition:  (password:string)=> NUMBER_REGEX.test(password) },
];
...
export { passwordValidation, emailRegex, passwordRules };
