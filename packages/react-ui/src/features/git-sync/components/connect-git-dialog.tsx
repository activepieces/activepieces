...
import { ConfigureRepoRequest, GitBranchType, GitRepo } from '@activepieces/ee-shared'
...
const ConnectGitDialog = ({ open, setOpen, showButton }: ConnectGitProps) => {
  ...
  const { refetch } = gitSyncHooks.useGitSync(projectId, platform.environmentsEnabled)
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen} modal={true}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <Form {...form}>
          <form className="flex flex-col" onSubmit={form.handleSubmit((data) => mutate(data))}>
            <DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="git@github.com:activepieces/activepieces.git" {...field} />
                    </FormControl>
                  </FormItem>
                )}
              />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="-----BEGIN OPENSSH PRIVATE KEY-----" {...field} />
                    </FormControl>
                    <FormDescription>{t('The SSH private key to use for authentication.')}</FormDescription>
                  </FormItem>
                )}
              />
            ...
            <DialogFooter>
              ...
              <Button type="submit" onClick={form.handleSubmit((data) => mutate(data))} loading={isPending}>
                {t('Connect')}
              </Button>
            </DialogFooter>
          </form>
