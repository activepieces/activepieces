...
export const API_BASE_URL = import.meta.env.MODE === 'cloud' ? 'https://cloud.activepieces.com' : window.location.origin
...
function globalErrorHandler(error: AxiosError) {
  if (api.isError(error)) {
    ...
    if (errorCode === ErrorCode.SESSION_EXPIRED || errorCode === ErrorCode.INVALID_BEARER_TOKEN) {
...
function request<TResponse>(url: string, config: AxiosRequestConfig = {}): Promise<TResponse> {
  const resolvedUrl = !isUrlRelative(url) ? url : `${API_URL}${url}`
  const isApWebsite = resolvedUrl.startsWith(API_URL)
  const unAuthenticated = disallowedRoutes.some((route) => url.startsWith(route))
  return axios({
    url: resolvedUrl,
    ...config,
    headers: {
      ...config.headers,
      Authorization: unAuthenticated || !isApWebsite ? undefined : `Bearer ${authenticationSession.getToken()}`,
    },
  })
    .then((response) => (config.responseType === 'blob' ? response.data : (response.data as TResponse)))
    .catch((error) => {
      if (isAxiosError(error) && !ignroedGlobalErrorHandlerRoutes.includes(url)) {
...
export const api = {
  ...
  patch: <TResponse, TBody = unknown, TParams = unknown>(url: string, body?: TBody, params?: TParams) =>
    request<TResponse>(url, {
      method: 'PATCH',
      data: body,
