...
export const TokenCheckerWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const {
    isError,
    error,
    data: isProjectValid,
  ...
  const failedToSwitchToProject = !isProjectValid && !isNil(projectIdFromParams)
...
const RedirectToCurrentProjectRoute: React.FC<RedirectToCurrentProjectRouteProps> = ({ path }) => {
  ...
  return <Navigate to={`/projects/${currentProjectId}${pathWithParamsAndSearchParams}`} replace />
}
...
export const ProjectRouterWrapper = ({ element, path }: ProjectRouterWrapperProps) => [
  {
  ...
  {
    path,
    element: <RedirectToCurrentProjectRoute path={path}>{element}</RedirectToCurrentProjectRoute>,
  },
]
