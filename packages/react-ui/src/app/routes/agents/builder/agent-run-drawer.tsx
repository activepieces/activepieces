...
export function AgentRunDrawer({ open, onOpenChange, agentRunId }: AgentRunDrawerProps) {
    const { data: agentRun } = agentRunHooks.useGet(agentRunId);

    const { variant, Icon } = agentRun ? agentRunUtils.getStatusIcon(agentRun.status) : { variant: null, Icon: Play };
                    ...
                    <div className="p-2">
                        <div className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-2 ml-4">
                                {agentRun && variant && <StatusIconWithText
                                    icon={Icon}
                                    text={formatUtils.convertEnumToHumanReadable(agentRun.status)}
                                    variant={variant}
                                />}
                            </div>
                        </div>
