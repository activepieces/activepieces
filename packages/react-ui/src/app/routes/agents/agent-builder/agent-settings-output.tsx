...
import { Agent, AgentOutputField, AgentOutputFieldType, AgentOutputType } from '@activepieces/shared';

interface AgentSettingsOutputProps {
    onChange: (outputType: AgentOutputType, outputFields: AgentOutputField[]) => void;
    agent: Agent;
}
...
export const AgentSettingsOutput = ({
    ...
    const [outputFields, setOutputFields] = useState<AgentOutputField[]>(agent.outputFields || []);
    ...
    const addOutputField = (type: AgentOutputFieldType, name: string, description: string) => {
    ...
    const removeOutputField = (id: string) => {
        const updatedFields = outputFields.filter(field => field.displayName !== id);
        setOutputFields(updatedFields);
        onChange(outputType ?? AgentOutputType.NO_OUTPUT, updatedFields);
    };

    const handleOutputTypeChange = (checked: boolean) => {
        const value = checked ? AgentOutputType.STRUCTURED_OUTPUT : AgentOutputType.NO_OUTPUT;
