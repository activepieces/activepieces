...
export const chatHooks = {
    ...
    useSendMessage(setSession: (session: ChatSession) => void) {
        ...
        return useMutation<ChatSession, Error, { message: string, sessionId: string | null }>({
            mutationFn: async (request) => {
                let chatWithSessionId = request.sessionId;
                if (isNil(request.sessionId)) {
                    const createSession = await api.post<ChatSession>('/v1/chat-sessions', {})
                    chatWithSessionId = createSession.id;
                }
                let currentSession = await api.post<ChatSession>(`/v1/chat-sessions/${chatWithSessionId}/chat`, {
