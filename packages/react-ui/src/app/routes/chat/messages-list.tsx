...
import { ApErrorParams, ChatUIResponse, ErrorCode, FileResponseInterface } from '@activepieces/shared'
...
const formatError = (projectId: string | undefined | null, flowId: string, error: ApErrorParams) => {
  switch (error.code) {
    case ErrorCode.NO_CHAT_RESPONSE:
      return projectId ? (
        <span>
          No response from the chatbot. Ensure that <strong>Respond on UI</strong> is in{' '}
          <a
        ...
        <span>
          The chatbot is not responding. It seems there might be an issue with how this chat was set up. Please contact
          the person who shared this chat link with you for assistance.
        </span>
      )
...
const ErrorBubble = ({
  ...
  <ChatBubble variant="received" className="pb-8">
    <div className="relative">
      <ChatBubbleAvatar src={chatUI?.platformLogoUrl} fallback={<BotIcon className="size-5" />} />
    ...
    return (
      <ChatMessageList ref={messagesRef} className="w-full max-w-3xl">
        ...
        {sendingError && !isSending && (
          <ErrorBubble chatUI={chatUI} flowId={flowId} sendingError={sendingError} sendMessage={sendMessage} />
        )}
