...
export const FileInputPreview = ({ file, index, onRemove }: FileInputPreviewProps) => {
  ...
  return (
    <div key={index} className="relative inline-block mr-2 mt-2 mb-3">
      {isImage && <img src={URL.createObjectURL(file)} alt={file.name} className="w-20 h-20 object-cover rounded-lg" />}
      {isVideo && <video src={URL.createObjectURL(file)} className="w-20 h-20 object-cover rounded-lg" />}
      {!isImage && !isVideo && (
