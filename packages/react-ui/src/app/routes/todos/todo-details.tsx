...
import { RightDrawer, RightDrawerContent, RightDrawerHeader, RightDrawerTitle } from '@/components/right-drawer'
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center">
                      ...
                      <TooltipTrigger>
                        {data?.run.status &&
                          (data?.run.status === FlowRunStatus.RUNNING ||
                            ...
                            data?.run.status === FlowRunStatus.STOPPED) && <Loader className="h-4 w-4 mr-2" />}
                        {data?.run.status && isFailedState(data?.run.status) && (
                          <CircleX className="h-4 w-4 text-destructive mr-2" />
                        )}
                        {data?.run.status && data?.run.status === FlowRunStatus.SUCCEEDED && (
                          <CircleCheck className="h-4 w-4 text-success mr-2" />
                        )}
                      </TooltipTrigger>
                      <TooltipContent>
                        {data?.run.status && data?.run.status === FlowRunStatus.RUNNING && (
                          <span className="text-xs">Flow is running</span>
                        )}

                        {data?.run.status && data?.run.status === FlowRunStatus.PAUSED && (
                          <span className="text-xs">Flow is paused</span>
                        )}

                        {data?.run.status && data?.run.status === FlowRunStatus.STOPPED && (
                  ...
                  {(todo.status.name === UNRESOLVED_STATUS.name || todo.status.continueFlow === false) && (
                    <DropdownMenu>
                      <DropdownMenuTrigger>
                        <Button
                          variant="outline"
                          ...
                          style={{
                            backgroundColor: STATUS_COLORS[todo.status.variant].color,
                        ...
                        >
                          {isUpdatingStatus ? (
                            ...
                            <>
                              <span className="text-sm"> {todo.status.name} </span>
                              <ChevronDown className="h-4 w-4" />
                            </>
                          )}
                        </Button>
