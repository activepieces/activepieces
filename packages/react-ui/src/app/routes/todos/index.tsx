...
import { RESOLVED_STATUS, STATUS_COLORS, Todo, TodoWithAssignee, UNRESOLVED_STATUS } from '@activepieces/shared'

import { TodoDetails } from './todo-details'

function TodosPage() {
  const [selectedRows, setSelectedRows] = useState<Array<Todo>>([])
  const [selectedTask, setSelectedTask] = useState<TodoWithAssignee | null>(null)
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => {
      ...
      const isUnresolved = status.length === 1 && status[0] === UNRESOLVED_STATUS.name ? true : false
      const isAllStatus = status.length === 0 || status.length === 2
      const statusOptions = isAllStatus ? undefined : isUnresolved ? [UNRESOLVED_STATUS.name] : [RESOLVED_STATUS.name]
  ...
  const filteredData = useMemo(() => {
    ...
    return {
      data: data.data.filter((task) => task.assignee && assigneeEmails.includes(task.assignee.email)),
      next: data.next,
  ...
  const columns: ColumnDef<RowDataWithActions<TodoWithAssignee>, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return <div className="flex items-center gap-2">{row.original.title}</div>
      },
    },
    {
      accessorKey: 'assignee',
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('Assigned to')} />,
      cell: ({ row }) => {
        return (
    ...
    {
      accessorKey: 'created',
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('Date Created')} />,
      cell: ({ row }) => {
  ...
  return (
    <div className="flex-col w-full">
      <div className="flex items-center gap-2">
        <TableTitle description={t('Manage todos for your project that are created by automations')} beta={true}>
          {t('Todos')}
        </TableTitle>
      </div>
      <DataTable
