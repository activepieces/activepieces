    ...
    parse: (markdown: string): Block[] => {
        ...
        while (i < markdown.length) {
            if (markdown.startsWith('<tool-call', i)) {
                ...
                const toolResultStr = markdown.slice(i, endIndex + '</tool-result>'.length)
...
const updateToolCallWithResult = (blocks: Block[], toolResultStr: string): void => {
    ...
    const toolCallIndex = blocks.findIndex(block => 
        block.type === 'tool-call' && block.id === toolCallId
    )
