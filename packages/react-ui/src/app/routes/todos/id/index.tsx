...
import { CheckIcon, ChevronDown, Clock2, Loader, UserRoundPen } from 'lucide-react'
...
import { MarkdownVariant, STATUS_COLORS, StatusOption, UNRESOLVED_STATUS, isNil } from '@activepieces/shared'
    ...
    !isNil(task) && (
      <div className="container mx-auto flex flex-col p-10">
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm text-muted-foreground">{formatUtils.formatDateToAgo(new Date(task.created))}</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">{t('Assigned to')}</span>
          ...
          <div className="flex items-center gap-2">
            ...
            {(task.status.name === UNRESOLVED_STATUS.name || task.status.continueFlow === false) && (
              <DropdownMenu>
                <DropdownMenuTrigger>
                  <Button variant="outline" className="h-8 flex gap-2 items-center justify-between">
                    {isUpdatingStatus ? (
