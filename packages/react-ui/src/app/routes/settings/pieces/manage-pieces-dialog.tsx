...
export const ManagePiecesDialog = React.memo(({ onSuccess }: ManagePiecesDialogProps) => {
  const [open, setOpen] = useState(false)
  const { pieces: visiblePieces, isLoading: isLoadingVisiblePieces } = piecesHooks.usePieces({
    searchQuery: '',
    includeHidden: false,
  })
  useEffect(() => {
  ...
  const { pieces: allPieces, isLoading: isLoadingAllPieces } = piecesHooks.usePieces({
    searchQuery: '',
    includeHidden: true,
  })
    ...
    <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {t('Choose which pieces you want to be available for your current project users')}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form className="grid space-y-4" onSubmit={(e) => form.handleSubmit(() => mutate())(e)}>
            <FormField
              name="pieces"
              render={({ field }) => (
