...
import { ApErrorParams, ApFlagId, ErrorCode, Permission, PlatformRole, ProjectWithLimits } from '@activepieces/shared'
  ...
  >({
    mutationFn: (request) => {
      ...
      return projectApi.update(authenticationSession.getProjectId()!, {
        ...request,
        externalId: request.externalId?.trim() !== '' ? request.externalId : undefined,
      })
    },
    onSuccess: () => {
      toast({
    ...
    <div className="flex flex-col items-center w-full gap-4">
      <div className="space-y-6 w-full">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t('Manage general settings for your project.')}</p>
        </div>
        <Separator />
        <div className="grid gap-1 mt-4">
          <Form {...form}>
            <form className="grid space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              {platform.embeddingEnabled && platformRole === PlatformRole.ADMIN && (
                <FormField
                  name="externalId"
                  render={({ field }) => (
                    <FormItem className="grid space-y-2">
                      <Label htmlFor="externalId">{t('External ID')}</Label>
                      <FormDescription>{t('Used to identify the project based on your SaaS ID')}</FormDescription>
                      <Input {...field} id="externalId" placeholder={t('org-3412321')} className="rounded-sm" />
                      <FormMessage />
                    </FormItem>
                  )}
                />
