...
export default function TeamPage() {
  ...
  const { invitations, isLoading: invitationsIsPending } = userInvitationsHooks.useInvitations()
      ...
      <div className="w-full flex flex-col items-center justify-center gap-4">
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>{t('Invite your team members to collaborate.')}</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-6 ">
            <div className="flex min-h-[35px] flex-col gap-4">
              ...
              {projectMembers && projectMembers.length === 0 && (
                <div className="text-center">{t('No members are added to this project.')}</div>
              )}

              {Array.isArray(projectMembers) &&
                projectMembers.map((member: ProjectMemberWithUser) => (
                  <div key={member.id} className="flex items-center">
                    <ProjectMemberCard member={member} onUpdate={refetchProjectMembers} />
                  </div>
                ))}
            </div>
            <Separator />
            <div className="text-2xl font-bold tracking-tight">{t('Pending Invitations')}</div>
            <div className="flex min-h-[35px] flex-col gap-4">
              ...
              {Array.isArray(invitations) &&
                invitations.map((invitation) => (
                  <InvitationCard key={invitation.id} invitation={invitation}></InvitationCard>
                ))}
            </div>
