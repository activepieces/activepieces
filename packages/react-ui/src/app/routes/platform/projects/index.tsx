...
const columns: ColumnDef<RowDataWithActions<ProjectWithLimits>>[] = [
  ...
  {
    accessorKey: 'tasks',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('Used Tasks')} />,
    cell: ({ row }) => {
      return (
  ...
  },
  {
    ...
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('Used AI Credits')} />,
    cell: ({ row }) => {
      return (
        <div className="text-left">
  ...
  {
    accessorKey: 'users',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('Active Users')} />,
    cell: ({ row }) => {
      return (
        <div className="text-left">
  ...
  {
    accessorKey: 'flows',
    header: ({ column }) => <DataTableColumnHeader column={column} title={t('Active Flows')} />,
    cell: ({ row }) => {
      return (
        <div className="text-left">
          {row.original.analytics.activeFlows} / {row.original.analytics.totalFlows}
        </div>
      )
    },
  },
...
export default function ProjectsPage() {
  ...
  const columnsWithCheckbox: ColumnDef<RowDataWithActions<ProjectWithLimits>>[] = [
    {
      id: 'select',
      header: ({ table }) => {
        const selectableRows = table.getRowModel().rows.filter((row) => row.original.id !== currentProject?.id)
        ...
        return (
          <Checkbox
            ...
            onCheckedChange={(value) => {
              ...
              if (isChecked) {
                const selectableProjects = selectableRows.map((row) => row.original)
                const newSelectedRows = [...selectableProjects, ...selectedRows]
                const uniqueRows = Array.from(new Map(newSelectedRows.map((item) => [item.id, item])).values())
                setSelectedRows(uniqueRows)
              } else {
                const filteredRows = selectedRows.filter((row) => !selectableRows.some((r) => r.original.id === row.id))
          ...
          <Tooltip>
            ...
            {isCurrentProject && (
              <TooltipContent side="right">
                {t('Cannot delete active project, switch to another project first')}
              </TooltipContent>
            )}
          </Tooltip>
        )
    ...
    () => [
      {
        render: (_, resetSelection) => {
          const canDeleteAny = selectedRows.some((row) => row.id !== currentProject?.id)
          return (
            <div onClick={(e) => e.stopPropagation()}>
              <ConfirmationDeleteDialog
                title={t('Delete Projects')}
                message={t('Are you sure you want to delete the selected projects?')}
                entityName={t('Projects')}
                mutationFn={async () => {
                  const deletableProjects = selectedRows.filter((row) => row.id !== currentProject?.id)
                  await bulkDeleteMutation.mutateAsync(deletableProjects.map((row) => row.id))
                  resetSelection()
                  setSelectedRows([])
                }}
                onError={(error) => {
              ...
              >
                {selectedRows.length > 0 && (
                  <Button className="w-full mr-2" size="sm" variant="destructive" disabled={!canDeleteAny}>
  ...
  const errorToastMessage = (error: unknown): string | undefined => {
    if (validationUtils.isValidationError(error)) {
      ...
      switch (error.response?.data?.params?.message) {
        ...
        case 'ACTIVE_PROJECT':
          return t('This project is active. Please switch to another project first.')
      }
      return undefined
    }
  }
