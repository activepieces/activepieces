...
export const ActivateLicenseDialog = ({
    ...
    const { mutate: activateLicenseKey, isPending } = useMutation({
        mutationFn: async (tempLicenseKey: string) => {
            ...
            await platformApi.verifyLicenseKey(
                tempLicenseKey.trim(),
            );
        },
        onSuccess: () => {
    ...
    return (
        ...
        onOpenChange={(open) => {
            ...
            <DialogContent onInteractOutside={(e) => e.preventDefault()}>
                <DialogHeader>
                    <DialogTitle className="text-center">{showCelebration ? t('Woohoo! It worked') : t('Activate License Key')}</DialogTitle>
                </DialogHeader>
                {!showCelebration && (
                    <Form {...form}>
                ...
                {showCelebration && (
                    <div className="celebration-lottie text-center">
                        <Lottie options={{ ...defaultOptions, animationSpeed: 4 }} height={200} width={200} />
                        <div className="text-center mt-4">
                            <p className="text-md text-gray-500">{t('Enjoy the awesome enterprise features')}</p>
                        </div>
                    </div>
                )}
