...
export const AllowedDomainDialog = ({ platform, refetch }: AllowedDomainDialogProps) => {
        ...
        <Form {...form}>
          <form
            ...
            onSubmit={form.handleSubmit((data) => {
              mutate({
                allowedAuthDomains: data.allowedAuthDomains.map((d) => d.domain),
          ...
          >
            <div className="flex flex-col gap-1">
              <div className="text-muted-foreground text-sm">
                {t('Enter the allowed domains for the users to authenticate with, Empty list will allow all domains.')}
              </div>
            </div>
                ...
                render={({ field }) => (
                  <FormItem className="grid space-y-4">
                    <div className="flex space-x-2">
                      <Input {...field} id={`allowedAuthDomains.${index}`} className="rounded-sm" />
                      <Button type="button" onClick={() => remove(index)} variant="outline" size="sm" className="h-10">
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
                  </FormItem>
            ...
            <Button type="button" onClick={() => append({ domain: '' })} variant="outline" size="sm">
              {t('Add Domain')}
            </Button>
