...
const ProviderCard = ({ providerName, providerIcon, providerDescription, button, badgesText }: ProviderCardProps) => {
  return (
    <Card className="w-full px-4 py-4">
      <div className="flex w-full gap-2 justify-center items-center">
        <div className="flex flex-col gap-2 text-center mr-2">{providerIcon}</div>
        <div className="flex flex-grow  flex-col">
          <div className="text-lg">{providerName}</div>
          <div className="text-sm text-muted-foreground">
            {providerDescription ??
              t("Allow logins through {providerName}'s single sign-on functionality.", {
      ...
      <div className="flex-col w-full">
        <div className="mb-4 flex">
          <div className="flex justify-between flex-row w-full">
            <div className="flex flex-col gap-2">
              <h1 className="text-2xl font-bold w-full">{t('Single Sign On')}</h1>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-4">
          <ProviderCard
            ...
            button={<AllowedDomainDialog platform={platform} refetch={refetch} />}
          />
          <ProviderCard
            providerName="Google"
            providerIcon={<img src={GoogleIcon} alt="icon" width={32} height={32} />}
            button={
              <NewOAuth2Dialog
          ...
          <ProviderCard
            ...
            button={<ConfigureSamlDialog platform={platform} refetch={refetch} connected={samlConnected} />}
          />
          <ProviderCard
