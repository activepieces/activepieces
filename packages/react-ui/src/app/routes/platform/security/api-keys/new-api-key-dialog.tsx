...
import { ApiKeyResponseWithValue, CreateApiKeyRequest } from '@activepieces/ee-shared'
...
export const NewApiKeyDialog = ({ children, onCreate }: NewApiKeyDialogProps) => {
  const [open, setOpen] = useState(false)
  const [apiKey, setApiKey] = useState<ApiKeyResponseWithValue | undefined>(undefined)
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{apiKey ? t('API Key Created') : t('Create New API Key')}</DialogTitle>
        </DialogHeader>
        {apiKey && (
          <div className="p-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="grid space-y-4">
                    ...
                    <FormControl>
                      <Input {...field} required placeholder={t('API Key Name')} className="rounded-sm" />
                    </FormControl>
                  </FormItem>
                )}
              />
