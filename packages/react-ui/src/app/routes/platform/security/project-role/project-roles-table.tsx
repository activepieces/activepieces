...
export const ProjectRolesTable = ({ projectRoles, isLoading, refetch }: ProjectRolesTableProps) => {
  ...
  const columns: ColumnDef<RowDataWithActions<ProjectRole>>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="text-left">{formatUtils.formatDate(new Date(row.original.updated))}</div>,
    },
    {
      accessorKey: 'userCount',
      accessorFn: (row) => row.userCount,
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('Users')} className="text-center" />,
                ...
                <TooltipTrigger>
                  ...
                  >
                    <Button variant="ghost" className="size-8 p-0">
                      {row.type === RoleType.DEFAULT ? <Eye className="size-4" /> : <Pencil className="size-4" />}
                    </Button>
                  </ProjectRoleDialog>
                </TooltipTrigger>
                ...
                <Tooltip>
                  <TooltipTrigger>
                    <ConfirmationDeleteDialog
                      ...
                      title={t('Delete Role')}
                      message={t(
                        `Deleting this role will remove ${row.userCount} project member${
                          row.userCount === 1 ? '' : 's'
                  ...
                  <TooltipContent side="bottom">{t('Delete Role')}</TooltipContent>
                </Tooltip>
              )}
            </div>
          )
