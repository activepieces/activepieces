...
import { AddSigningKeyRequestBody, AddSigningKeyResponse } from '@activepieces/ee-shared'
...
export const NewSigningKeyDialog = ({ children, onCreate }: NewSigningKeyDialogProps) => {
  const [open, setOpen] = useState(false)
  const [signingKey, setSigningKey] = useState<AddSigningKeyResponse | undefined>(undefined)
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{signingKey ? t('Signing Key Created') : t('Create New Signing Key')}</DialogTitle>
        </DialogHeader>
        {signingKey && (
          <div className="p-4">
        ...
        {!signingKey && (
          <Form {...form}>
            <form className="grid space-y-4" onSubmit={form.handleSubmit(() => mutate())}>
              <FormField
                name="displayName"
                render={({ field }) => (
                  <FormItem className="grid space-y-4">
                    <Label htmlFor="displayName">{t('Signing Key Name')}</Label>
                    <Input {...field} required id="displayName" className="rounded-sm" />
                    <FormMessage />
                  </FormItem>
                )}
              />
        ...
        <DialogFooter>
          {!signingKey ? (
            <>
              ...
              <Button disabled={isPending || !form.formState.isValid} loading={isPending} onClick={() => mutate()}>
                {t('Save')}
              </Button>
            </>
          ) : (
