...
import { CheckIcon, Folder, History, Key, Link2, Logs, Users, Wand, Workflow } from 'lucide-react'
  ...
  return (
    ...
    >
      <div className="flex flex-col  w-full">
        <TableTitle description={t('Track activities done within your platform')}>{t('Audit Logs')}</TableTitle>
        <DataTable
          emptyStateTextTitle={t('No audit logs found')}
          emptyStateTextDescription={t('Come back later when you have some activity to audit')}
          emptyStateIcon={<History className="size-14" />}
          filters={filters}
          columns={[
            {
              accessorKey: 'resource',
              header: ({ column }) => <DataTableColumnHeader column={column} title={t('Resource')} />,
              cell: ({ row }) => {
            ...
            {
              accessorKey: 'details',
              header: ({ column }) => <DataTableColumnHeader column={column} title={t('Details')} />,
              cell: ({ row }) => {
            ...
            {
              accessorKey: 'userId',
              header: ({ column }) => <DataTableColumnHeader column={column} title={t('Performed By')} />,
              cell: ({ row }) => {
            ...
            },
            {
              ...
              cell: ({ row }) => {
                return <div className="text-left">{formatUtils.convertEnumToHumanReadable(row.original.action)}</div>
              },
            },
            {
              accessorKey: 'projectId',
