...
import { calculateTaskCostHelper, calculateTotalCostHelper } from './helpers/platform-billing-helper'
...
export default function Billing() {
  ...
  const updateLimitsMutation = useMutation({
    mutationFn: (data: { tasksLimit?: number | null | undefined }) => platformBillingApi.update(data.tasksLimit),
    onSuccess: () => {
      refetch()
      toast({
        title: t('Success'),
            ...
            <Card>
              ...
              <CardContent className="p-4 mt-5">
                    ...
                    <div className="flex items-center gap-1 w-full">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">{t('Count of executed steps')}</TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
                    <div className="text-sm font-sm text-gray-500">
                      {t(`First ${platformSubscription?.subscription?.includedTasks || 1000} tasks free`)}
                    </div>
                  </div>
                ...
                <div className="text-sm mt-5 flex items-center gap-1">
                  {(tasksLimit || 0) > 0 ? (
                    <div className="flex items-center gap-1">{t(`Your tasks limit is set to ${tasksLimit}`)}</div>
                  ) : null}
          ...
          <div className="gap-4">
            <Card>
              ...
              <CardContent className="p-4 mt-5">
                <div className="flex gap-2 items-center">
                  <div className="text-sm font-sm mt-1 flex items-center gap-2 basis-1/3 flex-wrap">
                    <div className="flex items-center gap-1 w-full">{t('Current Credit Usage')}</div>
                    <div className="text-sm font-sm text-gray-500">
                      {t(`First ${platformSubscription?.subscription?.includedAiCredits || 200} credits free`)}
                    </div>
                  </div>
                  <div className="basis-2/3">
                    <Progress
                ...
                <div className="text-sm mt-5 flex items-center gap-1">
                  {(aiLimit || 0) > 0 ? (
                    <div className="flex items-center gap-1">{t(`Your AI credits limit is set to ${aiLimit}`)}</div>
                  ) : null}
                </div>
              </CardContent>
            </Card>
