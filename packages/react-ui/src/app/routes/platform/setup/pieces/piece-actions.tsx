...
const PieceActions = ({ pieceName, isEnabled }: PieceActionsProps) => {
  ...
  const { mutate: togglePiece, isPending: isTogglePending } = useMutation({
    mutationFn: async (piecename: string) => {
      const newFilteredPieceNames = platform.filteredPieceNames.includes(piecename)
    ...
    <div className="flex gap-2">
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            {filtered ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
          </Button>
        </TooltipTrigger>
        <TooltipContent>
          {filtered ? t('Hide this piece from all projects') : t('Show this piece for all projects')}
        </TooltipContent>
      </Tooltip>
      ...
      <Tooltip>
        ...
        <TooltipContent>{pinned ? t('Unpin this piece') : t('Pin this piece')}</TooltipContent>
      </Tooltip>
    </div>
  )
}
