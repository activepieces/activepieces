...
const ApplyTags = ({ selectedPieces, onApplyTags }: ApplyTagsProps) => {
  ...
  useEffect(() => {
    setSelectedTags(
      new Set(tags.map((tag) => tag.name).filter((tag) => selectedPieces.every((piece) => piece.tags?.includes(tag)))),
    )
  }, [selectedPieces, tags])
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button variant="outline" size="sm" disabled={selectedPieces.length === 0}>
          Apply Tags
        </Button>
      </PopoverTrigger>
          ...
          <CommandList>
              ...
              <CommandGroup className="max-h-[300px]">
                {tagOptions.map((option) => {
                  ...
                  const isIndeterminate =
                    selectedPieces.some((piece) => piece.tags?.includes(option.value)) &&
                    !selectedPieces.every((piece) => piece.tags?.includes(option.value)) &&
                  ...
                  return (
                    ...
                    >
                      <Checkbox checked={isIndeterminate ? 'indeterminate' : isSelected} className="mr-2"></Checkbox>
            ...
            <CreateTagDialog
              onTagCreated={(tag) => {
                ...
                setTagOptions([...tagOptions, { label: tag.name, value: tag.name }])
              }}
