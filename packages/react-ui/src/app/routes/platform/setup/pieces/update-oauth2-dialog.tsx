...
export const ConfigurePieceOAuth2Dialog = forwardRef<HTMLButtonElement, ConfigurePieceOAuth2DialogProps>(
  ({ pieceName, onConfigurationDone, isEnabled }, ref) => {
    ...
    const { oauth2App, refetch } = oauth2AppsHooks.useOAuth2AppConfigured(pieceName)
        ...
        <DialogTrigger asChild>
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                {isNil(oauth2App) ? <Unlock className="size-4" /> : <Lock className="size-4 text-destructive" />}
              </Button>
            </TooltipTrigger>
            <TooltipContent>{isNil(oauth2App) ? t('Configure OAuth2 APP') : t('Delete OAuth2 APP')}</TooltipContent>
          </Tooltip>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
          ...
          <Form {...form}>
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="grid space-y-4">
                    ...
                    <Input {...field} required id="clientSecret" className="rounded-sm" />
                    <FormMessage />
                  </FormItem>
                )}
              />
              ...
              <DialogFooter>
                ...
                <Button loading={isUpserting} disabled={!form.formState.isValid} type="submit">
                  {t('Save')}
                </Button>
              </DialogFooter>
            </form>
