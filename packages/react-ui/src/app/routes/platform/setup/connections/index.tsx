...
import { CheckIcon, Globe, Pencil, Trash } from 'lucide-react'
...
import { AppConnectionStatus, AppConnectionWithoutSensitiveData, Permission } from '@activepieces/shared'
...
const GlobalConnectionsTable = () => {
  ...
  const columns: ColumnDef<RowDataWithActions<AppConnectionWithoutSensitiveData>, unknown>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('Projects')} />,
      cell: ({ row }) => {
        return <div className="text-left">{row.original.projectIds.length}</div>
      },
    },
    {
      id: 'actions',
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => {
      ...
      return globalConnectionsApi.list({
        ...
        limit: searchParams.get(LIMIT_QUERY_PARAM) ? parseInt(searchParams.get(LIMIT_QUERY_PARAM)!) : 10,
        status: (searchParams.getAll(STATUS_QUERY_PARAM) as AppConnectionStatus[] | undefined) ?? [],
      })
    },
  })
  ...
  return (
    <div className="flex-col w-full">
      <LockedFeatureGuard
        ...
        lockDescription={t('Manage platform-wide connections to external systems.')}
        lockVideoUrl="https://cdn.activepieces.com/videos/showcase/global-connections.mp4"
      >
        <TableTitle description={t('Manage platform-wide connections to external systems.')}>
