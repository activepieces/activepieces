  ...
  const columnsWithCheckbox: ColumnDef<RowDataWithActions<FlowTemplate>>[] = [
    {
      ...
      header: ({ table }) => (
        <Checkbox
          checked={table.getRowModel().rows.length > 0 && table.getRowModel().rows.every((row) => row.getIsSelected())}
    ...
    {
      accessorKey: 'pieces',
      header: ({ column }) => <DataTableColumnHeader column={column} title={t('Pieces')} />,
      cell: ({ row }) => {
        ...
        render: (_, resetSelection) => (
          <div onClick={(e) => e.stopPropagation()}>
            <ConfirmationDeleteDialog
              ...
              message={t('Are you sure you want to delete the selected templates?')}
    ...
    >
      <div className="flex flex-col w-full">
        <div className="flex items-center justify-between flex-row">
          <TableTitle description={t('Convert the most common automations into reusable templates')}>
            {t('Templates')}
          </TableTitle>
        </div>
        <DataTable
            ...
            (row) => {
              return (
                <div className="flex items-end justify-end">
                  <Tooltip>
                    <TooltipTrigger>
                      <UpsertTemplateDialog onDone={() => refetch()} template={row}>
