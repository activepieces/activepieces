...
export const ConfigureProviderDialog = ({ open, onOpenChange }: ConfigureProviderDialogProps) => {
  ...
  const openaiProvider = platform?.copilotSettings?.providers?.[CopilotProviderType.OPENAI] as
    | OpenAiProvider
    | undefined
  ...
  const handleProviderChange = (value: CopilotProviderType) => {
    ...
    const azureProvider = platform?.copilotSettings?.providers?.[CopilotProviderType.AZURE_OPENAI] as
      | AzureOpenAiProvider
      | undefined
    const openaiProvider = platform?.copilotSettings?.providers?.[CopilotProviderType.OPENAI] as
      | OpenAiProvider
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] p-4">
        ...
        <div className="space-y-4">
          <RadioGroup value={selectedProvider} onValueChange={handleProviderChange} className="flex gap-4">
            <div className="flex items-center space-x-2">
              <RadioGroupItem value={CopilotProviderType.OPENAI} id="openai" />
              <label htmlFor="openai" className="flex items-center gap-2 cursor-pointer text-sm">
                <img src="https://cdn.activepieces.com/pieces/openai.png" alt="OpenAI" className="w-4 h-4" />
                OpenAI
              </label>
            </div>
            <div className="flex items-center space-x-2">
              <RadioGroupItem value={CopilotProviderType.AZURE_OPENAI} id="azure" />
              <label htmlFor="azure" className="flex items-center gap-2 cursor-pointer text-sm">
                <img
          ...
          <div className="flex justify-end pt-2">
            <Button onClick={handleSubmit} disabled={isSubmitting} className="h-9 px-4 text-sm">
              {isSubmitting ? (
                <>
