...
const AddCustomDomainDialog = React.memo(({ onAdd }: AddCustomDomainDialogProps) => {
  ...
  const { mutate, isPending } = useMutation<CustomDomain, Error, { domain: string }>({
    ...
    >
      ...
      <DialogContent className="max-w-[450px]">
        <DialogHeader>
          ...
          <DialogDescription>{t('Enter a domain name without a protocol (e.g. example.com)')}</DialogDescription>
          <Form {...form}>
            <form onSubmit={form.handleSubmit((data) => mutate(data))} className="gap- grid">
              <FormField
                control={form.control}
                name="domain"
                render={({ field }) => (
                  <FormItem className="grid gap-3">
                    <Input {...field} id="domain" type="text" placeholder="example.com" />
                    <FormMessage />
                  </FormItem>
                )}
              />
