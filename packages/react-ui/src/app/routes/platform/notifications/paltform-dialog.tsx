...
const PlatformDialog = ({ messages }: PlatformDialogProps) => {
    ...
    const shouldShow = messages.length > 0 && (!lastDismissed || Date.now() - parseInt(lastDismissed) > 15 * 60 * 1000);
    const platformRole = authenticationSession.getUserPlatformRole();   
    const [isOpen, setIsOpen] = useState(shouldShow && platformRole === PlatformRole.ADMIN);
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {t('There are important platform alerts that require your attention. Please check the alerts section in Platform Admin.')}
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                    <Button
