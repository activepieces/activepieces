...
import { FROM_QUERY_PARAM, LOGIN_QUERY_PARAM, PROVIDER_NAME_QUERY_PARAM, STATE_QUERY_PARAM } from '@/lib/navigation-utils';
  ...
  useEffect(() => {
    ...
    if(state && state[LOGIN_QUERY_PARAM] && code)
    {
      ...
      const handleThirdPartyLogin = async () => {
        ...
        {
          if(api.isError(e) && (e.response?.data as {code: ErrorCode})?.code === ErrorCode.INVITATION_ONLY_SIGN_UP)
          {
            toast({
              title: t('Invitation only sign up'),
              description: t('Please ask your administrator to add you to the organization.'),
            });
          }
          else {
            toast({
