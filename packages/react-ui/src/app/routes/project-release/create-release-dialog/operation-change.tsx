...
import { ProjectOperationType, ProjectSyncPlanOperation } from '@activepieces/shared'
...
export const OperationChange = React.memo(({ change, selected, onSelect }: OperationChangeProps) => {
  return (
    <>
      {change.type === ProjectOperationType.CREATE_FLOW && (
        <div className="flex gap-2 text-success items-center">
          <Checkbox checked={selected} onCheckedChange={onSelect} />
          {renderDiffInfo(change.flow.displayName, <Plus className="w-4 h-4 shrink-0" />)}
        </div>
      )}
      {change.type === ProjectOperationType.UPDATE_FLOW && (
        <div className="flex gap-2 items-center">
          <Checkbox checked={selected} onCheckedChange={onSelect} />
          {renderDiffInfo(change.targetFlow.displayName, <PencilIcon className="w-4 h-4 shrink-0" />)}
        </div>
      )}
      {change.type === ProjectOperationType.DELETE_FLOW && (
        <div className="flex gap-2 text-destructive items-center">
          <Checkbox checked={selected} onCheckedChange={onSelect} />
          {renderDiffInfo(change.flow.displayName, <Minus className="w-4 h-4 shrink-0" />)}
        </div>
      )}
    </>
  )
