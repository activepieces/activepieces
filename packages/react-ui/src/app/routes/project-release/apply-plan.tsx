...
import { DiffReleaseRequest, ProjectReleaseType, isNil } from '@activepieces/shared'
...
export const ApplyButton = ({ request, children, onSuccess, defaultName, ...props }: ApplyButtonProps) => {
  ...
  const showGitDialog = isNil(gitSync) && request.type === ProjectReleaseType.GIT
  ...
  return (
    <>
      ...
      {isConnectGitDialogOpen ? (
        <ConnectGitDialog open={isConnectGitDialogOpen} setOpen={setGitDialogOpen} showButton={false} />
      ) : (
