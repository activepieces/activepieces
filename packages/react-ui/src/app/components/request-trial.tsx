...
import { ApEdition, ApFlagId } from "../../../../shared/src";
        ...
        mutationFn: async (request) => {
            ...
            switch (edition) {
                ...
                case ApEdition.COMMUNITY: {
                    ...
                    return {
                        message: 'Please check your email for your trial key and further instructions.',
                    };
                }
                default: {
        ...
        <Drawer direction="right" open={isOpen} onOpenChange={(open) => setIsOpen(open)}>
            ...
            <DrawerContent className='h-screen top-0 right-0 left-auto mt-0 w-[600px] rounded-none py-2 px-6 gap-6 flex'>
                <DrawerHeader className="mt-4">
                    <DrawerTitle className="text-2xl">14-Day Enterprise Trial</DrawerTitle>
                </DrawerHeader>
                <Form {...form}>
                                    ...
                                    <FormControl>
                                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                                            ...
                                            <SelectContent>
                                                <SelectGroup>
                                                    {goals.map((goal) => (
                                                        <SelectItem value={goal.key} key={goal.key}>{goal.label}</SelectItem>
                                                    ))}
                                                </SelectGroup>
                                            </SelectContent>
                                        </Select>
                ...
                <div className="flex flex-col gap-5 mt-6 items-center justify-center">
                    <div className="text-lg text-muted-foreground font-semibold">Deploy your automations securely with Activepieces</div>
                ...
                <DrawerFooter className="flex flex-row items-center justify-left">
                    <Button size={"lg"} onClick={form.handleSubmit((data) => mutate(data))} loading={isPending}>Submit</Button>
                    <DrawerClose asChild>
