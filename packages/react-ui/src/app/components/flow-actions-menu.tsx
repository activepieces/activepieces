...
import { ImportFlowDialog, ImportFlowDialogProps } from '@/features/flows/components/import-flow-dialog'
...
import { FlowOperationType, FlowVersion, FlowVersionState, Permission, PopulatedFlow } from '@activepieces/shared'
...
const FlowActionMenu: React.FC<FlowActionMenuProps> = ({
  ...
  const userHasPermissionToPushToGit = checkAccess(Permission.WRITE_PROJECT_RELEASE)
  const importFlowProps: ImportFlowDialogProps = {
    insideBuilder: true,
    flowId: flow.id,
  }
  const { embedState } = useEmbedding()
  const isDevelopmentBranch = gitSync && gitSync.branchType === GitBranchType.DEVELOPMENT
  const [open, setOpen] = useState(false)
  const allowPush = flow.publishedVersionId !== null && flow.version.state === FlowVersionState.LOCKED
  ...
  const { mutate: exportFlow, isPending: isExportPending } = flowsHooks.useExportFlows()
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger className="rounded-full p-2 hover:bg-muted cursor-pointer" asChild>
        {children}
      </DropdownMenuTrigger>
      <DropdownMenuContent noAnimationOnOut={true} onCloseAutoFocus={(e) => e.preventDefault()}>
        {!readonly && (
          <>
            ...
            {!insideBuilder && (
              <RenameFlowDialog flowId={flow.id} onRename={onRename} flowName={flowVersion.displayName}>
                <DropdownMenuItem onSelect={(e) => e.preventDefault()} disabled={!userHasPermissionToUpdateFlow}>
                  <div className="flex cursor-pointer flex-row gap-2 items-center">
                    <Pencil className="h-4 w-4" />
                    <span>{t('Rename')}</span>
                  </div>
        ...
        {!embedState.hideFolders && (
          <PermissionNeededTooltip hasPermission={userHasPermissionToUpdateFlow || userHasPermissionToWriteFolder}>
        ...
        <PermissionNeededTooltip hasPermission={userHasPermissionToUpdateFlow}>
          <DropdownMenuItem disabled={!userHasPermissionToUpdateFlow} onClick={() => duplicateFlow()}>
            <div className="flex cursor-pointer  flex-row gap-2 items-center">
              {isDuplicatePending ? <LoadingSpinner /> : <Copy className="h-4 w-4" />}
        ...
        {!readonly && insideBuilder && (
          <PermissionNeededTooltip hasPermission={userHasPermissionToUpdateFlow}>
            <ImportFlowDialog {...importFlowProps}>
              <DropdownMenuItem disabled={!userHasPermissionToUpdateFlow} onSelect={(e) => e.preventDefault()}>
                <div className="flex cursor-pointer flex-row gap-2 items-center">
        ...
        <DropdownMenuItem onClick={() => exportFlow([flow])}>
          <div className="flex cursor-pointer  flex-row gap-2 items-center">
            {isExportPending ? <LoadingSpinner /> : <Download className="h-4 w-4" />}
        ...
        {!readonly && (!embedState.isEmbedded || !embedState.disableNavigationInBuilder || !insideBuilder) && (
          <PermissionNeededTooltip hasPermission={userHasPermissionToUpdateFlow}>
                ...
                <>
                  ...
                  {isDevelopmentBranch && (
                    <div className="font-bold mt-2">
                      {t('You are on a development branch, this will also delete the flow from the remote repository.')}
                    </div>
                  )}
                </>
              }
