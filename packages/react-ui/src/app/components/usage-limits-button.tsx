        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-3">
              <UsageProgress name={t('Tasks')} value={project.usage.tasks} max={project.plan.tasks} />
              <UsageProgress name={t('AI Credits')} value={project.usage.aiTokens} max={project.plan.aiTokens} />
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              {project.usage.nextLimitResetDate && (
                <div className="text-xs text-muted-foreground flex justify-between w-full">
                  <span>
                    {t('Usage resets in')} {getTimeUntilNextReset(project.usage.nextLimitResetDate)}{' '}
                  </span>
                  <FlagGuard flag={ApFlagId.SHOW_BILLING}>
                    <Link to={'/platform/setup/billing'} className="w-fit">
                      <span className="text-xs text-primary underline">{t('Manage')}</span>
                    </Link>
                  </FlagGuard>
                </div>
              )}
    ...
    <div className="flex items-center flex-col justify-between gap-2  w-full">
      <div className="w-full flex text-xs justify-between">
        ...
        <div className="text-xs">
          ...
          <span>{!isNil(max) ? formatUtils.formatNumber(max) : t('Unlimited')}</span>
        </div>
      </div>
      {!isNil(max) && <Progress value={(value / max) * 100} className="h-[6px] bg-gray-200" />}
    </div>
  )
}
