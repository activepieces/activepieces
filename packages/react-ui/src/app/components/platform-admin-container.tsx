...
import { LayoutGrid, LineChart, Server, Shield, Users, Wrench } from 'lucide-react'
...
export function PlatformAdminContainer({ children }: PlatformAdminContainerProps) {
  ...
  if (edition === ApEdition.CLOUD && !showPlatformDemo) {
    const setupGroup = items.find((item) => item.type === 'group' && item.label === t('Setup'))
  ...
  return (
    <AllowOnlyLoggedInUserOnlyGuard>
      {showPlatformAdminDashboard ? <SidebarComponent items={items}>{children}</SidebarComponent> : <Navigate to="/" />}
    </AllowOnlyLoggedInUserOnlyGuard>
  )
}
