...
const BuilderFlowStatusSection = React.memo(() => {
  ...
  const [flowVersion, flow, run] = useBuilderStateContext((state) => [state.flowVersion, state.flow, state.run])
  return (
    <>
          ...
          {flow.publishedVersionId && (
            <div className="flex items-center space-x-2">
              ...
              {(flow.publishedVersionId === flowVersion.id || flowVersion.state === FlowVersionState.DRAFT) && (
                <FlowStatusToggle flow={flow} flowVersion={flowVersion}></FlowStatusToggle>
              )}
            </div>
          )}
        </>
