...
const useAnimateSidebar = (sidebarValue: LeftSideBarType | RightSideBarType) => {
  const handleRef = useRef<ImperativePanelHandle>(null)
  const sidebarClosed = [LeftSideBarType.NONE, RightSideBarType.NONE].includes(sidebarValue)
  useEffect(() => {
...
const constructContainerKey = (flowId: string, stepName: string, triggerOrActionName?: string) => {
  return flowId + stepName + (triggerOrActionName ?? '')
}
const BuilderPage = () => {
  const { platform } = platformHooks.useCurrentPlatform()
  const [setRun, flowVersion, leftSidebar, rightSidebar, run, canExitRun, selectedStep] = useBuilderStateContext(
    (state) => [
      state.setRun,
      state.flowVersion,
      state.leftSidebar,
  ...
  const { memorizedSelectedStep, containerKey } = useBuilderStateContext((state) => {
    ...
    const triggerOrActionName =
      step?.type === TriggerType.PIECE ? (step as PieceTrigger).settings.triggerName : step?.settings.actionName
    return {
      memorizedSelectedStep: step,
      containerKey: constructContainerKey(state.flow.id, state.selectedStep, triggerOrActionName),
    }
  })
  ...
  const { versions, refetch: refetchPiece } = piecesHooks.useMostRecentAndExactPieceVersion({
    name: memorizedSelectedStep?.settings.pieceName,
    version: memorizedSelectedStep?.settings.pieceVersion,
    enabled: memorizedSelectedStep?.type === ActionType.PIECE || memorizedSelectedStep?.type === TriggerType.PIECE,
  })
        ...
        <ResizablePanel defaultSize={100} order={2} id="flow-canvas">
          <div ref={middlePanelRef} className="relative h-full w-full">
            ...
            <FlowCanvas
              ...
              lefSideBarContainerWidth={leftSidePanelRef.current?.clientWidth || 0}
            ></FlowCanvas>
            ...
            <DataSelector parentHeight={middlePanelSize.height} parentWidth={middlePanelSize.width}></DataSelector>
          </div>
        </ResizablePanel>
