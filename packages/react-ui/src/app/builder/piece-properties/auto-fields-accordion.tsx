...
const AutoFieldsAccordion = React.memo(({ props, prefixValue, disabled }: AutoFieldsAccordionProps) => {
  ...
  const getAccordionMaxHeight = () => {
    if (autoFields.length === 0) return 'max-h-48';
    if (autoFields.length <= 2) return 'max-h-48';
    if (autoFields.length <= 4) return 'max-h-72';
    return 'max-h-80';
  };

  const getScrollAreaHeight = () => {
    if (autoFields.length === 0) return 'h-32';
    if (autoFields.length <= 2) return 'h-32';
    if (autoFields.length <= 4) return 'h-48';
    return 'h-56';
  };
  ...
  const handleClearAll = () => {
    Object.entries(propertySettings).forEach(([propertyName]) => {
      propertyUtils.handlePropertyExecutionModeChange({
        ...
        property: propertySettings[propertyName].schema ?? props[propertyName],
        propertyName,
        inputName: `${prefixValue}.${propertyName}`,
        updateFormFieldSchemaWithAuto,
      });
     ...
     <div className="absolute inset-x-0 -top-8 h-8 bg-gradient-to-t from-background to-transparent pointer-events-none" />
      <div style={{
        ...
        <Accordion 
          ...
          className={`${getAccordionMaxHeight()} ${isOpen ? 'pb-4' : ''}`}
          style={{
                        ...
                        <Tooltip key={propertyName}>
                          <TooltipTrigger asChild>
                            <div className="flex items-center gap-2 w-full group" onClick={(e) => {
                              ...
                              <div className="w-full flex items-center justify-between text-xs transition-all duration-300 py-1 px-2 rounded-sm cursor-pointer border border-slate-300 group-hover:bg-white group-hover:border-slate-400">
