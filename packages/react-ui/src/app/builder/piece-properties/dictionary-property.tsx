...
export const DictionaryProperty = ({ values, onChange, disabled, useMentionTextInput }: DictionaryInputProps) => {
  const id = useRef(1)
  ...
  const valuesArrayRefUnique = valuesArrayRef.current
    .toReversed()
    .filter((el, index, self) => self.findIndex((t) => t.key === el.key) === index)
    .toReversed()
  ...
  const add = () => {
    id.current++
    const newValues = [...valuesArrayRef.current, { key: '', value: '', id: `${id.current}` }]
    valuesArrayRef.current = newValues
    updateValue(newValues)
  }

  const onChangeValue = (index: number, value: string | undefined, key: string | undefined) => {
  ...
  const updateValue = (items: DictionaryInputItem[]) => {
    onChange(items.reduce((acc, current) => ({ ...acc, [current.key]: current.value }), {}))
  }
  return (
    <div className="flex w-full flex-col gap-4">
      {valuesArrayRef.current.map(({ key, value, id }, index) => (
        <div key={'dictionary-input-' + id} className="flex items-center gap-3 items-center">
          <Input
            value={key}
            disabled={disabled}
      ...
      <Button variant="outline" size="sm" onClick={add} type="button" disabled={disabled}>
