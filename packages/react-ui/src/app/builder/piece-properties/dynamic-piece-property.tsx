...
import { ExecutePropsResult, PiecePropertyMap, PropertyType } from '@activepieces/pieces-framework'
...
const DynamicPropertiesImplementation = React.memo((props: DynamicPropertiesProps) => {
  ...
  useDeepCompareEffectNoCheck(() => {
    ...
    if (!isFirstRender.current && !deepEqual(previousValues.current, refresherValues)) {
      // the field state won't be cleared if you only unset the parent prop value
      if (propertyMap) {
        Object.keys(propertyMap).forEach((childPropName) => {
          form.setValue(`settings.input.${props.propertyName}.${childPropName}` as const, null, {
            shouldValidate: true,
          })
        })
      }
    ...
    mutate(
      ...
      {
        onSuccess: (response) => {
          const currentValue = form.getValues(`settings.input.${props.propertyName}`)
          const customizedInput = form.getValues('settings.inputUiInfo.customizedInputs')
          const defaultValue = formUtils.getDefaultValueForStep(response.options, currentValue ?? {}, customizedInput)
