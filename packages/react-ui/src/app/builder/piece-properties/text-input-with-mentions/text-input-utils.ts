...
import { Action, Trigger, assertNotNullOrUndefined, isNil } from '@activepieces/shared'
  ...
  return inputSplitToNodesContent.reduce(
    (result, node) => {
      if (node === '\n') {
        ...
        result[result.length - 1].content.push(createMentionNodeFromText(node, steps, stepsMetadata))
      } else {
...
function parseFlattenArrayPath(input: string): {
  ...
  const arrayPath = match[2].split(',').map((item) => item.trim().replace(/['"]/g, ''))
...
function parseStepAndNameFromMention(mention: string) {
  const mentionWithoutInterpolationBrackets = removeIntroplationBrackets(mention)
  const { isValid, stepName, arrayPath } = parseFlattenArrayPath(mentionWithoutInterpolationBrackets)
  if (isValid) {
    return {
      stepName,
      path: arrayPath ?? [],
...
function parseLabelFromMention(
  ...
  return {
    displayText: `${stepIdx + 1}. ${stepMetadata?.stepDisplayName ?? ''} ${path.join(' ')}`,
    serverValue: mention,
...
function createMentionNodeFromText(
  ...
  return {
    ...
    attrs: {
      id: mention,
      label: JSON.stringify(parseLabelFromMention(mention, steps, stepsMetadata)),
    },
  }
}
  ...
  const res = nodes.map((node, index) => {
    switch (node.type) {
      ...
      case TipTapNodeTypes.mention: {
        return node.attrs?.label ? JSON.parse(node.attrs.label).serverValue : ''
      }
...
const generateMentionHtmlElement = (mentionAttrs: MentionNodeAttrs) => {
  ...
  assertNotNullOrUndefined(apMentionNodeAttrs.displayText, 'apMentionNodeAttrs.displayText')
...
export const textMentionUtils = {
  ...
  convertTiptapJsonToText: ({ content }: JSONContent) => {
    const nodes = content ?? []
    const res = nodes.length === 1 && isNil(nodes[0].content) ? '' : convertTiptapJsonToText(nodes)
    return res
  },
