...
function buildInputSchemaForStep(
  ...
  switch (type) {
    case ActionType.PIECE: {
      if (piece && actionNameOrTriggerName && piece.actions[actionNameOrTriggerName]) {
    ...
    case TriggerType.PIECE: {
      if (piece && actionNameOrTriggerName && piece.triggers[actionNameOrTriggerName]) {
        return formUtils.buildSchema(
          addAuthToPieceProps(
...
function buildConnectionSchema(piece: PieceMetadataModelSummary | PieceMetadataModel) {
  const auth = piece.auth
  if (isNil(auth)) {
    return Type.Object({
      request: Type.Composite([Type.Omit(UpsertAppConnectionRequestBody, ['externalId'])]),
    })
  }
  ...
  switch (auth.type) {
    case PropertyType.SECRET_TEXT:
      return Type.Object({
        request: Type.Composite([Type.Omit(UpsertSecretTextRequest, ['externalId', 'displayName']), connectionSchema]),
      })
    case PropertyType.BASIC_AUTH:
      return Type.Object({
        request: Type.Composite([Type.Omit(UpsertBasicAuthRequest, ['externalId', 'displayName']), connectionSchema]),
      })
    case PropertyType.CUSTOM_AUTH:
      return Type.Object({
        request: Type.Composite([
          ...
          Type.Object({
            value: Type.Object({
              props: formUtils.buildSchema((piece.auth as CustomAuthProperty<any>).props),
            }),
          }),
        ]),
      })
    case PropertyType.OAUTH2:
      return Type.Object({
        request: Type.Composite([
          Type.Omit(Type.Union([UpsertOAuth2Request, UpsertCloudOAuth2Request, UpsertPlatformOAuth2Request]), [
            'externalId',
            'displayName',
          ]),
          connectionSchema,
...
export const formUtils = {
  buildPieceDefaultValue: (
    ...
    const defaultErrorOptions = {
      continueOnFailure: {
        value: selectedStep.settings.errorHandlingOptions?.continueOnFailure?.value ?? false,
      },
      retryOnFailure: {
        value: selectedStep.settings.errorHandlingOptions?.retryOnFailure?.value ?? false,
      },
    }
    switch (type) {
  ...
  return true;
      ...
      case ActionType.PIECE: {
        ...
        const requireAuth = isNil(actionName) ? false : (piece?.actions?.[actionName]?.requireAuth ?? true)

        const actionPropsWithoutAuth = isNil(actionName) ? {} : (piece?.actions?.[actionName]?.props ?? {})
        const props = addAuthToPieceProps(actionPropsWithoutAuth, piece?.auth, requireAuth)
      ...
      case TriggerType.PIECE: {
        ...
        const requireAuth = isNil(triggerName) ? false : (piece?.triggers?.[triggerName]?.requireAuth ?? true)

        const triggerPropsWithoutAuth = isNil(triggerName) ? {} : (piece?.triggers?.[triggerName]?.props ?? {})
        const props = addAuthToPieceProps(triggerPropsWithoutAuth, piece?.auth, requireAuth)
        const input = (selectedStep?.settings?.input ?? {}) as Record<string, unknown>
        const defaultValues = getDefaultValueForStep(props ?? {}, includeCurrentInput ? input : {})
          ...
          Type.Object({
            settings: Type.Composite([
              ...
              Type.Object({
                ...
                input: buildInputSchemaForStep(type, piece, actionNameOrTriggerName),
              }),
            ]),
          }),
        ])
  ...
  buildSchema: (props: PiecePropertyMap) => {
    ...
    const nonNullableUnknownPropType = Type.Not(Type.Union(nullableType), Type.Unknown())
    const propsSchema: Record<string, TSchema> = {}
    for (const [name, property] of entries) {
      switch (property.type) {
        case PropertyType.MARKDOWN:
          propsSchema[name] = Type.Optional(Type.Union([Type.Null(), Type.Undefined(), Type.Never(), Type.Unknown()]))
          break
      ...
      if (!property.required && property.type !== PropertyType.ARRAY) {
        propsSchema[name] = Type.Optional(
          Type.Union(isEmpty(propsSchema[name]) ? [Type.Any(), ...nullableType] : [propsSchema[name], ...nullableType]),
        )
      }
    }
...
export function getDefaultValueForStep(
  ...
  for (const [name, property] of entries) {
    switch (property.type) {
      case PropertyType.CHECKBOX: {
        defaultValues[name] = existingInput[name] ?? property.defaultValue ?? false
        break
      }
      case PropertyType.ARRAY: {
        const isCustomizedArrayOfProperties =
          !isNil(customizedInput) && customizedInput[name] && !isNil(property.properties)
