...
export class StepOutputStructureUtil {
  ...
  public static findPathToStep(
    ...
    const pathWithIndex = path.map((f) => {
      return {
        ...f,
        indexInDfsTraversal: StepOutputStructureUtil.findStepIndex(
          trigger,
          f.name,
        ),
      };
  ...
  public static extractStepOutput(
    ...
    const parents = StepOutputStructureUtil.findStepParents(stepName, trigger);
    if (parents) {
      return StepOutputStructureUtil.getLoopChildStepOutput(
        parents,
        loopIndexes,
        stepName,
        output,
