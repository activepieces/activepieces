...
import { PieceMetadataModel, PiecePropertyMap } from '@activepieces/pieces-framework'
import { Action, Trigger, setAtPath } from '@activepieces/shared'
...
const StepSettingsContext = createContext<StepSettingsContextState | undefined>(undefined)

export const StepSettingsProvider = ({ selectedStep, pieceModel, children }: StepSettingsProviderProps) => {
  const [formSchema, setFormSchema] = useState<TObject<any>>(Type.Object(Type.Any()))
  ...
  const updateFormSchema = useCallback((key: string, newFieldPropertyMap: PiecePropertyMap) => {
...
export const useStepSettingsContext = () => {
  ...
  if (context === undefined) {
    throw new Error('useStepSettingsContext must be used within a PieceSettingsProvider')
  }
  return context
}
