...
const CodeEditor = ({
  ...
  return (
    <div className={cn('flex flex-col gap-2 border rounded py-2 px-2 transition-all ', borderColor)} ref={containerRef}>
      <div className="flex flex-row justify-center items-center h-full">
        ...
        {codeApplicationEnabled ? (
          <Button variant="outline" className="flex gap-2" size={'sm'} onClick={applyCodeToCurrentStep}>
          ...
          allowNpmPackagesInCodeStep && (
            <AddNpmDialog onAdd={handleAddPackages}>
              <Button variant="outline" className="flex gap-2" size={'sm'} onClick={() => {}}>
      ...
      <CodeMirror
        ...
        lang="typescript"
        onChange={(value) => {
          onChange(activeTab === 'code' ? { code: value, packageJson } : { code, packageJson: value })
        }}
