...
const AddNpmDialog = ({ children, onAdd }: AddNpmDialogProps) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      ...
      const response = await api.get<{ 'dist-tags': { latest: string } }>(`https://registry.npmjs.org/${packageName}`)
      return {
        packageName,
    ...
    <Dialog open={open} onOpenChange={(open) => setOpen(open)}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>{t('Type the name of the npm package you want to add.')}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={(e) => form.handleSubmit(() => mutate())(e)} className="flex flex-col gap-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <Input {...field} id="packageName" type="text" placeholder="hello-world" className="rounded-sm" />
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormDescription>{t('The latest version will be fetched and added')}</FormDescription>
