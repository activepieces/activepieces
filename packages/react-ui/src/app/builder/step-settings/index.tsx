...
const StepSettingsContainer = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const currentStep: Trigger | Action = JSON.parse(JSON.stringify(form.getValues()))
      ...
      if (flowStructureUtil.isAction(currentStep.type) && flowStructureUtil.isAction(selectedStep.type)) {
  ...
  const [isEditingStepOrBranchName, setIsEditingStepOrBranchName] = useState(false)
            ...
            <EditableStepName
              ...
              branchName={
                !isNil(selectedBranchIndex)
                  ? modifiedStep.settings.branches?.[selectedBranchIndex]?.branchName
                  : undefined
              }
              setBranchName={(value) => {
                if (!isNil(selectedBranchIndex)) {
                  form.setValue(`settings.branches[${selectedBranchIndex}].branchName`, value)
                }
              }}
            ...
            <ScrollArea className="h-full">
              <div className="flex flex-col gap-4 px-4 pb-6">
                {stepMetadata && (
                  <PieceCardInfo
                    piece={stepMetadata}
                    customizedInputs={selectedStep.settings?.inputUiInfo?.customizedInputs}
                    actionOrTriggerDisplayName={actionOrTriggerDisplayName}
                  ></PieceCardInfo>
                )}
                {modifiedStep.type === ActionType.LOOP_ON_ITEMS && <LoopsSettings readonly={readonly}></LoopsSettings>}
                {modifiedStep.type === ActionType.CODE && <CodeSettings readonly={readonly}></CodeSettings>}
                {modifiedStep.type === ActionType.PIECE && modifiedStep && (
                  <PieceSettings step={modifiedStep} flowId={flowVersion.flowId} readonly={readonly}></PieceSettings>
                )}
                ...
                {[ActionType.CODE, ActionType.PIECE].includes(modifiedStep.type as ActionType) && (
                  <ActionErrorHandlingForm
                    hideContinueOnFailure={modifiedStep.settings.errorHandlingOptions?.continueOnFailure?.hide}
                    disabled={readonly}
                    hideRetryOnFailure={modifiedStep.settings.errorHandlingOptions?.retryOnFailure?.hide}
                  ></ActionErrorHandlingForm>
                )}
              </div>
            </ScrollArea>
