...
import { Bot, Calculator, Code2, FileJson, Globe } from 'lucide-react'
...
const FeatureItem = ({ icon, title, description }: FeatureItemProps) => (
  <div className="flex items-center gap-2.5 p-2 rounded-lg  transition-all duration-300 group">
    ...
    <div className="transition-all duration-300 ease-out ">
      <h3 className="text-sm font-medium text-black dark:text-gray-100">{title}</h3>
    ...
    <div className="flex w-full gap-3 animate-in fade-in slide-in-from-left-5 ">
      ...
      <div className="flex-1 space-y-4 mt-2">
        <div className="text-sm text-black   ">
          {t('Hello there! I am here to generate code that helps with your flow')}{' '}
          <img src={magic} className="min-w-4 h-4 object-fit inline" />
        </div>
        <p className="text-sm text-black   ">{t('Here are examples of what I am best used for: ')}</p>
        <div className="space-y-1.5">
          ...
          <FeatureItem
            ...
            description={t('Connect with external services. Best for simple integrations currently.')}
          />
        </div>
        <p className="text-sm text-foreground pt-2">{t('What would you like me to help you with?')}</p>
      </div>
    </div>
  )
}
