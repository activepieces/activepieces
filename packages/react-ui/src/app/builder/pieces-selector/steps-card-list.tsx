...
import { HandleSelectCallback, PieceStepMetadata, StepMetadata } from '@/features/pieces/lib/types'
...
export const StepsCardList: React.FC<StepsCardListProps> = ({
  ...
  const { data: actionsOrTriggers, isLoading: isLoadingSelectedPieceMetadata } = piecesHooks.usePieceActionsOrTriggers({
    stepMetadata: selectedPieceMetadata,
  })

  if ((isNil(actionsOrTriggers) || !selectedPieceMetadata) && !isLoadingSelectedPieceMetadata) {
    return (
  ...
  return (
    <ScrollArea className="h-full" viewPortClassName="h-full">
      {openCreateTodoGuideDialog && selectedPieceMetadata && actionsOrTriggers && (
        <CreateTodoGuide
      ...
      <CardList className="w-[350px] min-w-[350px] h-full gap-0" listClassName="gap-0">
        {isLoadingSelectedPieceMetadata && <CardListItemSkeleton numberOfCards={5} withCircle={false} />}
        {!isLoadingSelectedPieceMetadata && selectedPieceMetadata && actionsOrTriggers && (
          <>
            {actionsOrTriggers
              ...
              .map((item) => (
                <CardListItem
                  ...
                  onClick={() => {
                    if (
                      (selectedPieceMetadata as PieceStepMetadata).pieceName === '@activepieces/piece-todos' &&
