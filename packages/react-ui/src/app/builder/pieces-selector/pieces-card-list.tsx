      ...
      {piecesIsLoaded &&
        pieceGroups.map((group, index) => (
          <React.Fragment key={group.title}>
            ...
            {pieceGroups.length > 1 && <div className="text-sm text-muted-foreground mx-2 mt-2">{group.title}</div>}
            ...
            {group.pieces.map((pieceMetadata) => (
              <PieceCardListItem
                ...
                ref={pieceMetadata.displayName === selectedPieceMetadata?.displayName ? selectedItemRef : null}
              />
            ))}
          </React.Fragment>
        ))}

      {noResultsFound && isCopilotEnabled && operation.type !== FlowOperationType.UPDATE_TRIGGER && (
        <div className="flex flex-col gap-2 items-center justify-center h-full ">
          <WandSparkles className="w-14 h-14" />
          <div className="text-sm mb-3">{t('Let our AI assistant help you out')}</div>
          <AskAiButton varitant={'default'} operation={operation} onClick={closePieceSelector}></AskAiButton>
    ...
    return (
      <div onMouseLeave={handleMouseLeave} ref={ref}>
        <CardListItem
          ...
          selected={pieceMetadata.displayName === selectedPieceMetadata?.displayName && debouncedQuery.length === 0}
        ...
        >
          <div className="flex gap-2 items-center h-full">
            <PieceIcon
              ...
              size={'sm'}
            />
            <div className="flex-grow h-full flex items-center justify-left text-sm">{pieceMetadata.displayName}</div>
          </div>
        </CardListItem>

        {(debouncedQuery.length > 0 || isMobile) && pieceMetadata.type !== TriggerType.EMPTY && (
