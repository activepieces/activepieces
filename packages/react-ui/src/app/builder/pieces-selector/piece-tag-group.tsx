...
const PieceTagGroup = ({ selectedTag, onSelectTag, type }: PieceTagGroupProps) => {
  return (
      ...
      {Object.entries(tags).map(([tag, tagData]) => {
        ...
        const tagComponent = (
          <PieceTag
            ...
            onClick={(e) => {
              if (!isDisabled) {
                onSelectTag(selectedTag === tag ? PieceTagEnum.ALL : (tag as PieceTagEnum))
                e.stopPropagation()
              }
            }}
          ...
          >
            <div className="flex items-center gap-2">
              {tagData.icon && <img src={tagData.icon} alt={tagData.title} className="min-w-4 h-4" />}
              {tagData.title}
            </div>
          </PieceTag>
        )
