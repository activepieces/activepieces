...
function ManualTaskTestingDialog({
  ...
  const { mutate: resolveTask, isPending: isResolvingTask } = useMutation<
    StepRunResponse & {
  ...
  >({
    mutationFn: async () => {
      const response = await manualTaskApi.update(task.id, {
        status: status,
      });
        ...
        <DialogFooter className="justify-end">
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  loading={isResolvingTask}
                  disabled={
                    status.name === UNRESOLVED_STATUS.name || isResolvingTask
                  }
                  onClick={() => {
                    resolveTask();
                  }}
