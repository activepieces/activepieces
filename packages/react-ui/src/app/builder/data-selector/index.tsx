...
import { DataSelectorSizeState, DataSelectorSizeTogglers } from './data-selector-size-togglers'
import { DataSelectorTreeNode } from './type'
import { dataSelectorUtils } from './utils'

const getDataSelectorStructure: (state: BuilderState) => DataSelectorTreeNode[] = (state) => {
  const { selectedStep, flowVersion } = state
  if (!selectedStep || !flowVersion || !flowVersion.trigger) {
    return []
  }
  const pathToTargetStep = flowStructureUtil.findPathToStep(flowVersion.trigger, selectedStep)
  return pathToTargetStep.map((step) => {
    try {
      return dataSelectorUtils.traverseStep(step, state.sampleData, state.isFocusInsideListMapperModeInput)
    } catch (error) {
...
const doesElementHaveAnInputThatUsesMentions = (element: Element | null): boolean => {
  if (isNil(element)) {
    return false
  }
...
const DataSelector = ({ parentHeight, parentWidth }: DataSelectorProps) => {
  ...
  const [DataSelectorSize, setDataSelectorSize] = useState<DataSelectorSizeState>(DataSelectorSizeState.DOCKED)
  const [searchTerm, setSearchTerm] = useState('')
  const dataSelectorStructure = useBuilderStateContext(getDataSelectorStructure)
  const filteredNodes = dataSelectorUtils.filterBy(dataSelectorStructure, searchTerm)
  const [showDataSelector, setShowDataSelector] = useState(false)

  const checkFocus = useCallback(() => {
    const isTextMentionInputFocused =
      (!isNil(containerRef.current) && containerRef.current.contains(document.activeElement)) ||
          ...
          height:
            DataSelectorSize === DataSelectorSizeState.COLLAPSED
              ...
              : DataSelectorSize === DataSelectorSizeState.DOCKED
                ? '450px'
                : `${parentHeight - 100}px`,
          width: DataSelectorSize !== DataSelectorSizeState.EXPANDED ? '450px' : `${parentWidth - 40}px`,
        }}
        className="transition-all overflow-hidden"
      >
        <div className="flex items-center gap-2 px-5 py-2">
          <Input placeholder={t('Search')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}></Input>
        </div>

        <ScrollArea className="transition-all h-[calc(100%-56px)] w-full ">
          {filteredNodes &&
            filteredNodes.map((node) => (
              <DataSelectorNode depth={0} key={node.key} node={node} searchTerm={searchTerm}></DataSelectorNode>
            ))}
          {filteredNodes.length === 0 && (
            <div className="flex items-center justify-center gap-2 mt-5  flex-col">
              <SearchXIcon className="w-[35px] h-[35px]"></SearchXIcon>
              <div className="text-center font-semibold text-md">{t('No matching data')}</div>
