...
import { FlowRun, FlowVersion, TriggerType, isNil } from '@activepieces/shared'
...
const TestFlowWidget = ({ flowVersion, setRun }: TestFlowWidgetProps) => {
  ...
  const triggerHasSampleData =
    flowVersion.trigger.type === TriggerType.PIECE && !isNil(flowVersion.trigger.settings.inputUiInfo?.lastTestDate)
        ...
        <TooltipTrigger asChild>
          ...
          >
            <div className="flex justify-center items-center gap-2">
              {t('Test Flow')}
              <span className="text-[10px] tracking-widest whitespace-nowrap">{isMac ? '⌘ + D' : 'Ctrl + D'}</span>
            </div>
          </Button>
        </TooltipTrigger>
        {!triggerHasSampleData && <TooltipContent side="bottom">{t('Please test the trigger first')}</TooltipContent>}
      </Tooltip>
    )
  )
}
