...
const showChevronNextToSelection = (targetDiv: HTMLElement) => {
   ...
   root.render(<>
     <Button variant='outline' size='icon' className='absolute top-0 -left-9 z-50'
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
              mutation.addedNodes.forEach((node) => {
                if (node instanceof HTMLElement && node.children.length > 0 && node.children[0].classList.contains(NODE_SELECTION_RECT_CLASS_NAME)) {
