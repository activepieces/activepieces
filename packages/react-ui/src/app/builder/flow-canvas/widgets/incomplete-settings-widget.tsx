...
import { Action, FlowVersion, Step, flowStructureUtil } from '@activepieces/shared'
...
const filterValidOrSkippedSteps = (step: Step) =>
  ...
  (flowStructureUtil.isAction(step.type) && !(step as Action).skip && !step.valid)
const IncompleteSettingsButton: React.FC<IncompleteSettingsButtonProps> = ({ flowVersion, selectStepByName }) => {
  const invalidSteps = useMemo(
    () => flowStructureUtil.getAllSteps(flowVersion.trigger).filter(filterValidOrSkippedSteps).length,
    [flowVersion],
  )
  const { fitView } = useReactFlow()
  function onClick() {
    const invalidSteps = flowStructureUtil.getAllSteps(flowVersion.trigger).filter(filterValidOrSkippedSteps)
    if (invalidSteps.length > 0) {
      selectStepByName(invalidSteps[0].name)
      fitView(flowCanvasUtils.createFocusStepInGraphParams(invalidSteps[0].name))
    }
  }
