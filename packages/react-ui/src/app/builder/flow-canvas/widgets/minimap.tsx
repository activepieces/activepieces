...
const MinimapNode = ({node}: {node: MiniMapNodeProps}) => {
    ...
    const nodeColor = averageColorInImageUtils.useAverageColorInImage({imgUrl: stepMetadata.logoUrl ?? '', transparency: 50});
   
    return (
          <rect width={node.width} key={node.id} height={node.height} x={node.x} y={node.y} fill={nodeColor ?? 'var(--foreground)'} > 
          </rect>
    )
}
