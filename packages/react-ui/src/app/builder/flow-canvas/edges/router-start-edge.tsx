...
export const ApRouterStartCanvasEdge = ({
  ...
  const generatePath = () => {
    ...
    let path = `M ${targetX} ${targetY - flowUtilConsts.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE}`
    ...
    if (distanceBetweenSourceAndTarget >= flowUtilConsts.ARC_LENGTH) {
      ...
      if (data.drawHorizontalLine) {
        ...
        const horizontalLength =
          (Math.abs(targetX - sourceX) + 3 - 2 * flowUtilConsts.ARC_LENGTH) * (sourceX > targetX ? 1 : -1)

        // Add horizontal line and arc
        path += `h ${horizontalLength}`
        path += sourceX > targetX ? flowUtilConsts.ARC_LEFT_UP : flowUtilConsts.ARC_RIGHT_UP
      }

      if (data.drawStartingVerticalLine) {
        // Add final vertical line
        const finalVerticalLength =
          flowUtilConsts.VERTICAL_SPACE_BETWEEN_STEPS / 2 - 2 * flowUtilConsts.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE
        path += `v -${finalVerticalLength}`
      }
    } else {
      // If distance is small, just draw vertical line
      path += `v -${flowUtilConsts.ARC_LENGTH + flowUtilConsts.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE}`
    }
  ...
  const branchLabelProps =
    data.stepLocationRelativeToParent === StepLocationRelativeToParent.INSIDE_BRANCH
      ? {
          label: data.label,
  ...
  return (
    <>
      <BaseEdge path={path} style={{ strokeWidth: `${flowUtilConsts.LINE_WIDTH}px` }}></BaseEdge>
      {!data.isBranchEmpty && (
        <foreignObject
      ...
      <foreignObject
        ...
        height={flowUtilConsts.LABEL_HEIGHT + flowUtilConsts.LABEL_VERTICAL_PADDING + 'px'}
        x={targetX - (flowUtilConsts.AP_NODE_SIZE.STEP.width - 10) / 2}
        y={targetY - verticalLineLength / 2 - flowUtilConsts.AP_NODE_SIZE.ADD_BUTTON.height - 40}
