...
import { AP_NODE_SIZE, ApNodeType, flowCanvasUtils } from '../flow-canvas-utils';
...
function getEdgePath({ sourceX, sourceY, targetX, targetY, data }: ApEdgeWithButtonProps) {
  const targetYWithPlaceHolder = targetY + (flowCanvasUtils.isPlaceHolder(data.targetType) ? AP_NODE_SIZE[data.targetType].height + 10 : 0);
  if (sourceX === targetX) {
    return {
      buttonPosition: {
  ...
  return {
    buttonPosition: {
      ...
      y: targetYWithPlaceHolder - FIRST_LINE_LENGTH / 2,
    },
