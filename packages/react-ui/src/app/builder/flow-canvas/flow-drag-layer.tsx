...
import { FlowOperationType, StepLocationRelativeToParent, flowStructureUtil } from '@activepieces/shared'
...
const FlowDragLayer = ({
  ...
  const [setActiveDraggingStep, applyOperation, flowVersion, activeDraggingStep] = useBuilderStateContext((state) => [
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    setActiveDraggingStep(null)
    if (e.over && e.over.data.current && e.over.data.current.accepts === e.active.data?.current?.type) {
      const droppedAtNodeData: ApButtonData = e.over.data.current as ApButtonData
      if (
        droppedAtNodeData &&
        ...
        const isPartOfInnerFlow = flowStructureUtil.isChildOf(draggedStep, droppedAtNodeData.parentStepName)
        if (isPartOfInnerFlow) {
          toast({
            title: t('Invalid Move'),
            description: t('The destination location is a child of the dragged step'),
            duration: 3000,
          })
          return
        }
        applyOperation({
          ...
          request: {
            ...
            stepLocationRelativeToNewParent: droppedAtNodeData.stepLocationRelativeToParent,
            branchIndex:
              droppedAtNodeData.stepLocationRelativeToParent === StepLocationRelativeToParent.INSIDE_BRANCH
