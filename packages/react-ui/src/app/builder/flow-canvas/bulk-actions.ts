...
export function copySelectedNodes({ selectedNodes, flowVersion }: Pick<BuilderState, 'selectedNodes' | 'flowVersion'>) {
...
export function deleteSelectedNodes({
  ...
  selectedStep,
  exitStepSettings,
}: Pick<BuilderState, 'selectedNodes' | 'applyOperation' | 'selectedStep' | 'exitStepSettings'>) {
  applyOperation({
...
export function pasteNodes(
  ...
  const addOperations = flowOperations.getOperationsForPaste(actions, flowVersion, pastingDetails)
...
export function toggleSkipSelectedNodes({
  ...
  const steps = selectedNodes.map((node) => flowStructureUtil.getStepOrThrow(node, flowVersion.trigger)) as Action[]
