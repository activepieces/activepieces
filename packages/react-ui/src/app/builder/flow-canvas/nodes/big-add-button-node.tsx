...
import { AskAiIndicator, shouldShowAskAiIndicator } from '../edges/ask-ai-indicator'
import { flowUtilConsts } from '../utils/consts'
import { ApBigAddButtonNode } from '../utils/types'

const ApBigAddButtonCanvasNode = React.memo(({ data, id }: Omit<ApBigAddButtonNode, 'position'>) => {
  ...
  const [readonly, activeDraggingStep, showAiIndicator] = useBuilderStateContext((state) => [
    state.readonly,
            ...
            >
              <div
                ...
                className={cn('rounded bg-accent relative', {
                  ...
                  'shadow-add-button': isIsStepInsideDropzone || pieceSelectorOpen,
                  'transition-all': isIsStepInsideDropzone || pieceSelectorOpen || showDropIndicator,
                })}
              >
                {!showDropIndicator && (
                  <PieceSelector
              ...
              {showDropIndicator && (
                <div
                  style={{
                    ...
                    width: `${flowUtilConsts.AP_NODE_SIZE.STEP.width}px`,
                    top: `-${
                      flowUtilConsts.AP_NODE_SIZE.STEP.height / 2 - flowUtilConsts.AP_NODE_SIZE.BIG_ADD_BUTTON.width / 2
                    }px`,
                  }}
                  className=" absolute "
                  ref={setNodeRef}
                ...
                <g>
                  <path
                    d={`M ${flowUtilConsts.AP_NODE_SIZE.STEP.width / 2} -${
                      ...
                      2 * (flowUtilConsts.VERTICAL_SPACE_BETWEEN_STEP_AND_LINE + 5)
                    }`}
                    fill="transparent"
                    strokeWidth="1.5"
                  />
