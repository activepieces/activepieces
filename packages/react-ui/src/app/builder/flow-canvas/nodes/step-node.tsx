...
import { STEP_CONTEXT_MENU_ATTRIBUTE, flowUtilConsts } from '../utils/consts'
...
function getStepStatus(
  ...
  const stepOutput = flowRunUtils.extractStepOutput(stepName, loopIndexes, run.steps, flowVersion.trigger)
  return stepOutput?.status
}

const StepActionWrapper = React.memo(({ children }: { children: React.ReactNode }) => {
  return <div className="flex items-center gap-2 cursor-pointer">{children}</div>
})

StepActionWrapper.displayName = 'StepActionWrapper'
const ApStepCanvasNode = React.memo(({ data }: NodeProps & Omit<ApStepNode, 'position'>) => {
  const [
    selectStepByName,
    isSelected,
    isDragging,
  ...
  const pieceSelectorOperation = isAction ? FlowOperationType.UPDATE_ACTION : FlowOperationType.UPDATE_TRIGGER
  const isEmptyTriggerSelected = selectedStep === 'trigger' && step.type === TriggerType.EMPTY
    ...
    >
      ...
      <div
        className={cn('absolute left-0 top-0 pointer-events-none  rounded-sm w-full h-full', {
          'border-t-[3px] border-primary border-solid': isSelected && !isDragging,
        })}
      ></div>
      <div className="px-3 h-full w-full  overflow-hidden">
        {!isDragging && (
          <PieceSelector
            initialSelectedPiece={step.type === TriggerType.EMPTY ? undefined : stepMetadata?.displayName}
            operation={{
              type: isEmptyTriggerSelected ? FlowOperationType.UPDATE_TRIGGER : pieceSelectorOperation,
              stepName: step.name,
            }}
              ...
              <div className="flex h-full items-center justify-between gap-3 w-full">
                <div className="flex items-center justify-center min-w-[46px] h-full">
                  <div className={isSkipped ? 'opacity-80' : ''}>
                    <PieceIcon
                      logoUrl={step.settings?.inputUiInfo?.customizedInputs?.logoUrl ?? stepMetadata?.logoUrl}
                ...
                <div className="grow flex flex-col items-start justify-center min-w-0 w-full">
                  ...
                  <div className="flex justify-between w-full items-center">
                    ...
                    <div className="w-4 flex mt-0.5 items-center justify-center h-[20px]">
                      {stepOutputStatus && <StepStatusIcon status={stepOutputStatus} size="4"></StepStatusIcon>}
                      {showRunningIcon && <LoadingSpinner className="w-4 h-4 "></LoadingSpinner>}
                      {isSkipped && (
                        <Tooltip>
                      ...
                      {!step.valid && !isSkipped && (
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">{t('Incomplete settings')}</TooltipContent>
                        </Tooltip>
                      )}
                    </div>
                  </div>
        ...
        <Handle type="source" style={flowUtilConsts.HANDLE_STYLING} position={Position.Bottom} />
        <Handle type="target" style={flowUtilConsts.HANDLE_STYLING} position={Position.Top} />
      </div>
    </div>
  )
})
