...
export const AP_NODE_SIZE: Record<ApNodeType, { height: number, width: number }> = {
...
function traverseFlow(step: Action | Trigger | undefined): ApGraph {
  ...
  switch (step.type) {
    case ActionType.LOOP_ON_ITEMS: {
      ...
      const firstLoopGraph = isNil(firstLoopAction) ? buildGraph( ApNodeType.BIG_BUTTON) : traverseFlow(firstLoopAction);
      const childrenGraphs = [buildGraph( ApNodeType.LOOP_PLACEHOLDER), firstLoopGraph];
...
function buildChildrenGraph(
  ...
  const commonPartGraph = offsetGraph(
    isNil(nextAction) ? buildGraph(ApNodeType.PLACEHOLDER) : traverseFlow(nextAction),
    {
      x: 0,
      y: maximumHeight,
    },
...
function addEdge(nodeOne: ApNode, nodeTwo: ApNode): ApEdge {
  ...
  return {
    ...
    source: nodeOne.id,
    target: nodeTwo.id,
    type: nodeTwo.type === 'loopPlaceholder' ? 'apReturnEdge' : 'apEdge',
    label: nodeTwo.id,
    data: {
      addButton: nodeTwo.type === 'stepNode',
...
function boundingBox(graph: ApGraph): ApBoundingBox {
  ...
  const maxX = Math.max(
    ...graph.nodes.map((node) => node.position.x + AP_NODE_SIZE[node.type].width),
  );
  const maxY = Math.max(
    ...graph.nodes.map((node) => node.position.y + AP_NODE_SIZE[node.type].height),
  );
