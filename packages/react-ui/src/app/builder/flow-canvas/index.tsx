...
import { ActionType, FlowVersion, Step, flowStructureUtil, isNil } from '@activepieces/shared'
...
import { SELECTION_RECT_CHEVRON_ATTRIBUTE, STEP_CONTEXT_MENU_ATTRIBUTE, flowUtilConsts } from './utils/consts'
    ...
    case ActionType.ROUTER:
      return step.children.reduce((routerKey, child) => {
        const childrenKey = child
          ? flowStructureUtil.getAllSteps(child).reduce((childKey, grandChild) => `${childKey}-${grandChild.name}`, '')
          : 'null'
...
export const FlowCanvas = React.memo(
  ({
    ...
    const onContextMenu = useCallback(
      (ev: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (ev.target instanceof HTMLElement || ev.target instanceof SVGElement) {
          ...
          const stepName = stepElement?.getAttribute(`data-${STEP_CONTEXT_MENU_ATTRIBUTE}`)
          ...
          if (stepElement || targetIsSelectionRect || targetIsSelectionChevron) {
            setContextMenuType(ContextMenuType.STEP)
          } else {
            setContextMenuType(ContextMenuType.CANVAS)
          }
          if (doesSelectionRectangleExist() && !targetIsSelectionRect && !targetIsSelectionChevron) {
            document.querySelector(`.${NODE_SELECTION_RECT_CLASS_NAME}`)?.remove()
          }
        }
      },
    ...
    const onSelectionEnd = useCallback(() => {
      ...
      const selectedSteps = selectedNodes.map((node) => flowStructureUtil.getStepOrThrow(node, flowVersion.trigger))
      selectedSteps.forEach((step) => {
        if (step.type === ActionType.LOOP_ON_ITEMS || step.type === ActionType.ROUTER) {
      ...
      storeApi.getState().addSelectedNodes(selectedSteps.map((step) => step.name))
    }, [selectedNodes, storeApi, selectedStep])
    const [cursorPosition, setCursorPosition] = useState({ x: 0, y: 0 })
    return (
      <div ref={containerRef} className="size-full relative overflow-hidden z-50">
