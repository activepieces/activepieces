...
import { FlowRetryStrategy, FlowRun, Permission, PopulatedFlow, isFailedState, isNil } from '@activepieces/shared'
...
const FlowRunCard = React.memo(({ run, viewedRunId, refetchRuns }: FlowRunCardProps) => {
  const { Icon, variant } = flowRunUtils.getStatusIcon(run.status)
  const userHasPermissionToRetryRun = useAuthorization().checkAccess(Permission.WRITE_RUN)
  const projectId = authenticationSession.getProjectId()
  const [hoveringRetryButton, setHoveringRetryButton] = useState<boolean>(false)
  const [setLeftSidebar, setRun] = useBuilderStateContext((state) => [state.setLeftSidebar, state.setRun])
    ...
    >
      ...
      <div className="grid gap-2">
        ...
        {isNil(run.finishTime) ||
          (!run.finishTime && <p className="flex gap-1 text-xs text-muted-foreground">{t('Running')}...</p>)}
      </div>
