...
const FlowStepDetailsCardItem = ({ stepName, depth }: FlowStepDetailsCardProps) => {
  const [loopsIndexes, step, selectedStep, stepIndex, selectStepByName, run, flowVersion] = useBuilderStateContext(
    (state) => {
      const step = flowStructureUtil.getStepOrThrow(stepName, state.flowVersion.trigger)
      const stepIndex = flowStructureUtil.getAllSteps(state.flowVersion.trigger).findIndex((s) => s.name === stepName)
    ...
    <Collapsible open={isOpen} className="w-full">
        ...
        >
          ...
          <div className="flex items-center  w-full gap-3">
            {children.length > 0 && (
              ...
              >
                <ChevronRight size={16} className={cn('', { 'rotate-90': isOpen })} />
              </Button>
            )}
            <img
            ...
            />
            <div className="break-all truncate min-w-0 grow-1 shrink-1">{`${stepIndex + 1}. ${step?.displayName}`}</div>
            <div className="w-2"></div>
            <div className="flex gap-1 justify-end  items-center flex-grow">
              {isLoopStep && isStepSelected && (
                <span className="text-sm font-semibold animate-fade">{t('All Iterations')}</span>
              )}
              {isLoopStep && !isStepSelected && (
                <div
                  className={cn('flex gap-1 justify-end  items-center flex-grow', {
      ...
      <CollapsibleContent className="p-0">
        {children.map((stepName) => (
          <FlowStepDetailsCardItem stepName={stepName} key={stepName} depth={depth + 1}></FlowStepDetailsCardItem>
        ))}
      </CollapsibleContent>
    </Collapsible>
  )
