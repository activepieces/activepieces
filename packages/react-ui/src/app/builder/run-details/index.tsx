...
import { ApFlagId, FlowRun, FlowRunStatus, RunEnvironment, isNil } from '@activepieces/shared'
...
function getMessage(run: FlowRun | null, retentionDays: number | null) {
  ...
  if (isNil(run.logsFileId)) {
    return t('Logs are kept for {days} days after execution and then deleted.', { days: retentionDays })
  }
  return null
}
const FlowRunDetails = React.memo(() => {
  const { data: rententionDays } = flagsHooks.useFlag<number>(ApFlagId.EXECUTION_DATA_RETENTION_DAYS)

  const [setLeftSidebar, run, steps, loopsIndexes, flowVersion, selectedStep] = useBuilderStateContext((state) => {
    const steps = state.run && state.run.steps ? Object.keys(state.run.steps) : []
    return [state.setLeftSidebar, state.run, steps, state.loopsIndexes, state.flowVersion, state.selectedStep]
  })

  const selectedStepOutput = useMemo(() => {
    return run && selectedStep && run.steps
      ? flowRunUtils.extractStepOutput(selectedStep, loopsIndexes, run.steps, flowVersion.trigger)
      : null
  ...
  if (!isNil(message))
    return (
      <div className="flex flex-col justify-center items-center gap-4 w-full h-full">
        ...
        <h4 className="px-6 text-sm text-center text-muted-foreground ">{message}</h4>
      </div>
    )
      ...
      <SidebarHeader onClose={() => setLeftSidebar(LeftSideBarType.NONE)}>
        <div className="flex gap-2 items-center">
          {run && run.environment !== RunEnvironment.TESTING && (
            <Button variant="ghost" size={'sm'} onClick={() => setLeftSidebar(LeftSideBarType.RUNS)}>
              <ChevronLeft size={16} />
            </Button>
          )}
        ...
        <CardList className="p-0 h-full" listClassName="gap-0.5">
          {steps.length > 0 &&
            steps
              ...
              .map((path) => <FlowStepDetailsCardItem stepName={path} depth={0} key={path}></FlowStepDetailsCardItem>)}
          {steps.length === 0 && (
      ...
      {selectedStepOutput && (
        <>
          ...
          <ResizablePanel defaultValue={25}>
            <FlowStepInputOutput stepDetails={selectedStepOutput}></FlowStepInputOutput>
          </ResizablePanel>
        </>
      )}
    </ResizablePanelGroup>
