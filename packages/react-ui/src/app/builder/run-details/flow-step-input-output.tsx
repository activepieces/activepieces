...
import { StepOutput, flowStructureUtil } from '@activepieces/shared'

const FlowStepInputOutput = React.memo(({ stepDetails }: { stepDetails: StepOutput }) => {
  const stepOutput = stepDetails.errorMessage ?? stepDetails.output

  const [flowVersion, selectedStepName] = useBuilderStateContext((state) => [state.flowVersion, state.selectedStep])
  const selectedStep = selectedStepName ? flowStructureUtil.getStep(selectedStepName, flowVersion.trigger) : undefined
  return (
