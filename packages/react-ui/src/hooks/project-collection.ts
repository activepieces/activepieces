        ...
        onUpdate: async ({ transaction }) => {
            for (const { original, modified } of transaction.mutations) {
                const request: UpdateProjectPlatformRequest = {
                    ...
                    externalId: !isNil(modified.externalId) && modified.externalId.trim() !== '' ? modified.externalId : undefined,
                    icon: modified.icon,
                    plan: modified.plan,
                };
                await api.post<ProjectWithLimits>(`/v1/projects/${original.id}`, request);
...
export const projectCollectionUtils = {
    useCreateProject: (onSuccess: () => void, onError: (error: Error) => void) => {
        return useMutation({
            mutationFn: (request: CreatePlatformProjectRequest) => api.post<ProjectWithLimits>('/v1/projects', request),
            onSuccess: (data) => {
    ...
    useAll: () => {
        ...
        return useLiveSuspenseQuery((q) =>
            q
                ...
                .select(({ project }) => ({ ...project })), [currentUserId]
        );
    },
