...
import { ApEdition, ApFlagId, Permission, PlatformRole, isNil } from '@activepieces/shared'
...
export const useAuthorization = () => {
  ...
  const { data: projectRole, isLoading } = useQuery({
    ...
    retry: false,
    enabled: !isNil(edition) && edition !== ApEdition.COMMUNITY && !isNil(platformId),
  })
...
export const useShowPlatformAdminDashboard = () => {
  ...
  const { data: isPlatformDemo } = flagsHooks.useFlag<boolean>(ApFlagId.SHOW_PLATFORM_DEMO)
