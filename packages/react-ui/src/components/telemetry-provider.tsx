...
import { ApFlagId, TelemetryEvent, UserWithMetaInformationAndProject, isNil } from '@activepieces/shared'
...
const TelemetryProvider = ({ children }: TelemetryProviderProps) => {
  ...
  const { data: telemetryEnabled } = flagsHooks.useFlag<boolean>(ApFlagId.TELEMETRY_ENABLED)
  const { data: flagCurrentVersion } = flagsHooks.useFlag<string>(ApFlagId.CURRENT_VERSION)
  const { data: flagEnvironment } = flagsHooks.useFlag<string>(ApFlagId.ENVIRONMENT)
  ...
  return <TelemetryContext.Provider value={{ capture, reset }}>{children}</TelemetryContext.Provider>
}
