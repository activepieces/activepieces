...
import { ColumnDef as TanstackColumnDef, flexRender, getCoreRowModel, useReactTable } from '@tanstack/react-table'
...
import { SeekPage, isNil } from '@activepieces/shared'
...
interface DataTableProps<TData extends DataWithId, TValue, Keys extends string, F extends DataTableFilter<Keys>> {
...
export function DataTable<TData extends DataWithId, TValue, Keys extends string, F extends DataTableFilter<Keys>>({
              ...
              return (
                <div className="flex justify-end gap-4">
                  {actions.map((action, index) => {
                    return <React.Fragment key={index}>{action(row.original)}</React.Fragment>
                  })}
                </div>
              )
            },
  ...
  const [nextPageCursor, setNextPageCursor] = useState<string | undefined>(page?.next ?? undefined)
  const [previousPageCursor, setPreviousPageCursor] = useState<string | undefined>(page?.previous ?? undefined)
  ...
  const [tableData, setTableData] = useState<RowDataWithActions<TData>[]>(enrichPageData(page?.data ?? []))
  ...
  useEffect(() => {
    setTableData(tableData.filter((row) => !deletedRows.some((deletedRow) => deletedRow.id === row.id)))
  }, [deletedRows])
            ...
            {bulkActions.length > 0 && (
              <DataTableBulkActions
                ...
                actions={bulkActions.map((action) => ({
                  render: (selectedRows: RowDataWithActions<TData>[]) => action.render(selectedRows, resetSelection),
                }))}
              />
            )}
          </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow className="hover:bg-background">
                <TableCell colSpan={columns.length} className="h-[350px] text-center">
