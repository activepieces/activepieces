...
type ArrayInputProps = {
  ...
  disabled: boolean
  required?: boolean
  customInputNode?: (onChange: (value: string) => void, value: string, disabled: boolean) => React.ReactNode
  thinInputs?: boolean
}
...
const ArrayInput = React.memo(({ inputName, disabled, required, customInputNode, thinInputs }: ArrayInputProps) => {
  ...
  const remove = (index: number) => {
    const currentFields: ArrayField[] = form.getValues(inputName).map((value: string) => ({
      id: nanoid(),
      value,
    }))
  ...
  const updateFieldValue = (index: number, newValue: string) => {
    const newFields = fields.map((field, i) => (i === index ? { ...field, value: newValue } : field))
    setFields(newFields)
    updateFormValue(newFields)
  }
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem className="grow">
                      <FormControl>
                        {customInputNode ? (
                          customInputNode((value) => updateFieldValue(index, value), field.value as string, disabled)
                        ) : (
                          <Input
                            thin={thinInputs}
