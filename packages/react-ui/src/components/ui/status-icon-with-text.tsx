...
const statusCodeVariants = cva('inline-flex gap-1 rounded px-2.5 py-1 text-xs font-semibold', {
  variants: {
    variant: {
...
const StatusIconWithText = React.memo(({ icon: Icon, text, variant, color, textColor }: StatusIconWithTextProps) => {
  ...
  return (
    <span className={statusCodeVariants({ variant })} style={{ backgroundColor: color || undefined }}>
