...
const MultiSelectTrigger = React.forwardRef<MultiSelectTriggerElement, MultiSelectTriggerProps>(
  ({ className, children, showDeselect, onDeselect, ...props }, forwardedRef) => {
...
const MultiSelectValue = React.forwardRef<React.ElementRef<typeof Primitive.div>, MultiSelectValueProps>(
  ({ className, placeholder, maxDisplay, maxItemLength, ...props }, forwardRef) => {
    ...
    const remainingPiecesCount = maxDisplay && value.length > maxDisplay ? value.length - maxDisplay : 0
    ...
    if (!value.length || !firstRendered) {
      return <span className="pointer-events-none text-muted-foreground opacity-80">{placeholder}</span>
    }
...
>(({ className, ...props }, ref) => {
  return (
    <CommandList ref={ref} className={cn('py-1 px-0 ', className)} {...props}>
      <ScrollArea viewPortClassName="max-h-[200px]">{props.children}</ScrollArea>
    </CommandList>
  )
})
  ...
  (
    ...
    return (
      <CommandItem
        {...props}
        value={value}
        className={cn('cursor-pointer', disabled && 'text-muted-foreground cursor-not-allowed', className)}
        disabled={disabled}
        onSelect={!disabled && value ? handleClick : undefined}
        ref={forwardedRef}
      >
        <span className="mr-2 whitespace-nowrap overflow-hidden text-ellipsis">{children || label || value}</span>
