  ...
  return (
    <Alert
      className={cn('rounded-md border', {
        'bg-warning-100 text-warning-300 border-none': variant === MarkdownVariant.WARNING,
        'bg-success-100 text-success-300 border-none': variant === MarkdownVariant.TIP,
        'p-0 bg-background border-none': variant === MarkdownVariant.BORDERLESS,
      })}
    >
      {variant !== MarkdownVariant.BORDERLESS && (
        <>
          {(variant === MarkdownVariant.INFO || variant === undefined) && <Info className="w-4 h-4 mt-1" />}
          {variant === MarkdownVariant.WARNING && <AlertTriangle className="w-4 h-4 mt-1" />}
          {variant === MarkdownVariant.TIP && <Lightbulb className="w-4 h-4 mt-1" />}
        </>
      )}
...
const ApMarkdown = React.memo(({ markdown, variables, variant, className }: MarkdownProps) => {
    ...
    <Container variant={variant}>
      <ReactMarkdown
        ...
        components={{
          code(props) {
            ...
            return (
              <div className="relative w-full items-center flex bg-background border border-solid text-sm rounded block w-full gap-1 p-1.5">
                <input type="text" className="grow bg-background" value={codeContent} disabled />
                <Button
                  variant="ghost"
          ...
          h1: ({ node, ...props }) => (
            <h1 className="scroll-m-20 text-2xl font-extrabold tracking-tight lg:text-5xl" {...props} />
          ),
          h2: ({ node, ...props }) => (
            <h2 className="scroll-m-20 text-xl text-3xl font-semibold tracking-tight first:mt-0" {...props} />
          ),
          h3: ({ node, ...props }) => <h3 className="scroll-m-20 text-lg font-semibold tracking-tight" {...props} />,
          p: ({ node, ...props }) => <p className="leading-7 [&:not(:first-child)]:mt-2 w-full" {...props} />,
          ul: ({ node, ...props }) => <ul className="mt-4 ml-6 list-disc [&>li]:mt-2" {...props} />,
          ol: ({ node, ...props }) => <ol className="mt-4 ml-6 list-decimal [&>li]:mt-2" {...props} />,
          ...
          blockquote: ({ node, ...props }) => (
            <blockquote className="mt-4 first:mt-0 border-l-2 pl-6 italic" {...props} />
          ),
