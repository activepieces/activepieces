...
const FileButton = ({ fileUrl, handleDownloadFile }: FileButtonProps) => {
  ...
  return (
    <div className="flex items-center gap-0">
      ...
      >
        {readonly ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
        {t('Download File')}
      </Button>
    </div>
  )
...
const removeDoubleQuotes = (str: string): string => (str.startsWith('"') && str.endsWith('"') ? str.slice(1, -1) : str)
...
const JsonViewer = React.memo(({ json: unclearJson, title, hideDownload = false }: JsonViewerProps) => {
  ...
  useLayoutEffect(() => {
    if (typeof json === 'object') {
      ...
      const stepFileUrlsHTML = stringValuesHTML.filter(
        (el) => isStepFileUrl(el.innerHTML) || isStepFileUrl(el.parentElement!.nextElementSibling?.innerHTML),
      )

      stepFileUrlsHTML.forEach((el: Element) => {
        const fileUrl = removeDoubleQuotes(el.innerHTML).trim().replace('\n', '')
        el.className += ' hidden'
            ...
            {isProductionFile ? (
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent side="right">{t('File is not available after execution.')}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            ) : (
              <FileButton fileUrl={fileUrl} handleDownloadFile={handleDownloadFile} />
  ...
  if (isStepFileUrl(json)) {
    return <FileButton fileUrl={json} handleDownloadFile={handleDownloadFile} />
  }
          ...
          {isNil(json) ? (
            ...
            <>
              {typeof json !== 'string' && typeof json !== 'object' && (
                <pre className="text-sm whitespace-pre-wrap overflow-x-auto p-2">{JSON.stringify(json)}</pre>
              )}
              {typeof json === 'string' && (
                <pre className="text-sm whitespace-pre-wrap overflow-x-auto p-2">{json}</pre>
              )}
              {typeof json === 'object' && (
                <ReactJson
                  style={{
