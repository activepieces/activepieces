...
const ShowPoweredBy = ({ show, position = 'sticky' }: ShowPoweredByProps) => {
  useEffect(() => {
    try {
      let element: any = document.querySelector('.container.py-20');
      if (
      ...
      return () => {
        let element: any = document.querySelector(
          '.container.py-20.container2',
        );
        if (element.classList.contains('container2')) {
  ...
  return (
    ...
    >
      ...
      >
        <div className=" text-sm transition">Built by</div>
