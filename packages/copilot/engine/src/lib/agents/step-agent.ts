...
function formatAvailableActions(actions: Record<string, ActionBase>): string {
  return Object.entries(actions)
    .map(([name, action]) => `${action.displayName} (${name}): ${action.description}`)
    .join('\n');
}

function formatAvailableTriggers(triggers: Record<string, TriggerBase>): string {
  return Object.entries(triggers)
    .map(([name, trigger]) => `${trigger.displayName} (${name}): ${trigger.description}`)
    .join('\n');
}
...
export const stepAgent: StepAgent = {
  async createStep(context: StepContext): Promise<FlowStepType> {
    console.debug('Creating step with context:', context);
    ...
    if (context.stepType === 'ROUTER') {
      ...
      if (!pieceMetadata) {
        throw new Error(`Could not fetch metadata for piece ${context.pieceName}`);
      }
      ...
      stepContext = `
        ...
        ${context.stepType === 'PIECE_TRIGGER' ? `Available Triggers:
        ${formatAvailableTriggers(pieceMetadata.triggers)}` : ''}

        ${context.stepType === 'PIECE' ? `Available Actions:
    ...
    const { object } = await generateObject({
      ...
      prompt: `
        ...
        - ${context.stepType === 'PIECE_TRIGGER' ? 'Uses the correct trigger name (not display name) from the available triggers' : ''}
        - ${context.stepType === 'PIECE' ? 'Uses the correct action name (not display name) from the available actions' : ''}
        `}
        - Includes necessary input parameters using connection placeholders

        IMPORTANT:
        - The step type must be: ${context.stepType}
        ${context.stepType !== 'ROUTER' ? '- For triggers and actions, use the internal name (not display name)' : ''}
