...
export function processPieces(pieces: Piece[]): PieceSegment[] {
  ...
  for (const piece of pieces) {
    ...
    if (piece?.description) {
      segments.push({
        id: uuidv4(),
        content: `${displayName} is ${piece.description}. This ${
          piece.pieceType?.toLowerCase() || 'automation'
        } piece ${
          piece.triggers > 0
            ? `can trigger workflows with ${piece.triggers} different events`
            : ''
        }${piece.triggers > 0 && piece.actions > 0 ? ' and ' : ''}${
          piece.actions > 0
            ? `can perform ${piece.actions} different actions`
            : ''
        }. ${categoriesText}`,
        metadata: {
          pieceId: piece.id,
  ...
  console.debug(
    `Created ${segments.length} segments from ${pieces.length} pieces`
  );
  return segments;
}
