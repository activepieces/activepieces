...
export async function embedPieces(
  ...
  try {
    ...
    console.debug(
      `Successfully generated embeddings for ${embeddedPieces.length} segments`
    );
...
export async function loadEmbeddings(
  ...
  try {
    ...
    console.debug(
      `Successfully loaded ${embeddedPieces.length} embedded pieces`
    );
    return embeddedPieces;
  } catch (error) {
