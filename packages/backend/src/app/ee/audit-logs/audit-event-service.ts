...
import { Cursor, PrincipalType, SeekPage, apId, assertNotNullOrUndefined, isNil } from '@activepieces/shared'
import { PlatformId } from '@activepieces/ee-shared'
import { buildPaginator } from '../../helper/pagination/build-paginator'
import { paginationHelper } from '../../helper/pagination/pagination-utils'
import { ApplicationEventHooks, CreateAuditEventParam } from '../../helper/audit-events'
import { ApplicationEvent, ApplicationEventName } from '@activepieces/ee-shared'
...
type AuditLogService = {
    ...
    list: (params: { platformId: PlatformId, cursorRequest: Cursor | null, limit: number }) => Promise<SeekPage<ApplicationEvent>>
}
...
export const auditLogService: AuditLogService = {
    ...
    async list({ platformId, cursorRequest, limit }: { platformId: PlatformId, cursorRequest: Cursor | null, limit: number }): Promise<SeekPage<ApplicationEvent>> {
        ...
        const paginationResponse = await paginator.paginate(auditLogRepo.createQueryBuilder('audit_event').where({ platformId }))
        return paginationHelper.createPage<ApplicationEvent>(paginationResponse.data, paginationResponse.cursor)
    },
}


const saveEvent = async (request: FastifyRequest, rawEvent: CreateAuditEventParam) => {
    if (![PrincipalType.USER, PrincipalType.UNKNOWN].includes(request.principal.type)) {
        return
    }
