import { AppConnectionType, PlatformOAuth2ConnectionValue, assertNotNullOrUndefined, isNil } from '@activepieces/shared'
...
async function refresh({ pieceName, projectId, connectionValue }: RefreshOAuth2Request<PlatformOAuth2ConnectionValue>): Promise<PlatformOAuth2ConnectionValue> {
...
async function claim(
    { request, projectId, pieceName }: ClaimOAuth2Request,
...
async function getApp({ pieceName, clientId, projectId }: { clientId: string, pieceName: string, projectId: string }): Promise<OAuthAppWithSecret> {
