...
import { ConfigureRepoRequest, GitRepo, PushGitRepoRequest, PushSyncMode } from '@activepieces/shared'
...
async function planPullOperations(projectId: string, flowPath: string): Promise<FlowSyncOperation[]> {
    ...
    const deleteOperations: FlowSyncOperation[] = projectFlows.filter(f => gitFlows.findIndex(pf => pf.id === f.id) === -1).map(flow => {
        return {
...
async function planPushOperations(projectId: string, flowPath: string): Promise<FlowSyncOperation[]> {
    const projectFlows = await gitSyncHelper.fetchFlowsForProject(projectId)
    const gitFlows = await gitSyncHelper.parseFlowsFromDirectory(flowPath)
    const deleteOperations: FlowSyncOperation[] = gitFlows.filter(f => projectFlows.findIndex(pf => pf.id === f.id) === -1).map(flow => {
        return {
...
async function createGitRepoAndReturnPaths(gitRepo: GitRepo): Promise<{ flowFolderPath: string, git: SimpleGit }> {
...
async function initGitRepo(gitRepo: GitRepo, baseDir: string): Promise<SimpleGit> {
