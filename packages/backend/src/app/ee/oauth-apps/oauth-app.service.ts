...
export const oauthAppService = {
    ...
    async getWithSecret({ platformId, pieceName, clientId }: { platformId: string, pieceName: string, clientId?: string }): Promise<OAuthAppWithSecret> {
    ...
    async list({ request, platformId }: { platformId: string, request: ListOAuth2AppRequest }): Promise<SeekPage<OAuthApp>> {
        ...
        const { data, cursor } = await paginator.paginate(oauthRepo.createQueryBuilder('oauth_app').where({ platformId }))
        return paginationHelper.createPage<OAuthApp>(
            data,
            cursor,
        )
    },
