...
describe('Password Hasher', () => {
    ...
    describe('compare - Scrypt', () => {
        ...
        const hashedPassword = 'iu1iqj6i6g9D7aBiE/Qdqv88GNnV/Ea67JK1kfLmzNgxsyCL8mhUxxI5VIHM9D+62xGHuZgjrfEBF+17wxyFIQ=='
    
        it('should return true for identical scrypt passwords', async () => {
            const result = await passwordHasher.compare(plainTextPassword, `$scrypt$${hashedPassword}${SCRYPT_SEPERATOR}${salt}`)
            expect(result).toBe(true)
        })
        ...
        it('should return false for different scrypt passwords', async () => {
            const differentPassword = 'differentPassword'
            const result = await passwordHasher.compare(differentPassword, `$scrypt$${hashedPassword}${SCRYPT_SEPERATOR}${salt}`)
            expect(result).toBe(false)
        })
