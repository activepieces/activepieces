...
import { createMockUser, createMockPlatform, createAuditEvent, createMockProject } from '../../../helpers/mocks'
    ...
    describe('List Audit event API', () => {
        it('should list audit events', async () => {
            ...
            const mockAuditEvents1 = [
                createAuditEvent({ platformId: mockPlatform1.id, userId: mockUser1.id }),
                createAuditEvent({ platformId: mockPlatform1.id, userId: mockUser1.id }),
            ]
            await databaseConnection.getRepository('audit_event').save(mockAuditEvents1)

            const mockAuditEvents2 = [
                createAuditEvent({ platformId: mockPlatform2.id, userId: mockUser2.id }),
                createAuditEvent({ platformId: mockPlatform2.id, userId: mockUser2.id }),
            ]
            await databaseConnection.getRepository('audit_event').save(mockAuditEvents2)
