...
type SideNavRoute = {
  ...
  @Component({
    ...
    @if(sideNavRoute.showInSideNav$ | async)
    {
      ...
      [routerLink]="sideNavRoute.route?['/' + sideNavRoute.route]:undefined" (click)="sideNavRoute.effect ? sideNavRoute.effect() : null">
      <div
        routerLinkActive="ap-bg-primary-light   ap-transition ap-ease-out ap-rounded-[8px] hover:!ap-bg-primary-light !ap-fill-primary "
        class="hover:ap-bg-[#f5f5f5] ap-relative ap-p-3 ap-fill-title " [routerLink]="sideNavRoute.route? ['/' + sideNavRoute.route]:undefined">
        ...
        @if(sideNavRoute.showLock$ | async){
          ...
          [svgStyle]="{ width: '14px', height: '14px' }" src="assets/img/custom/lock.svg">
        </svg-icon>
        }
