...
export const doesQueryMatchStep = (step: FlowItemDetails,searchQuery:string) =>{
        return step.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        step.description.toLowerCase().includes(searchQuery.toLowerCase());
}
export const isCoreStep = (step:FlowItemDetails) => {
    return step.type === ActionType.BRANCH || ActionType.CODE || ActionType.LOOP_ON_ITEMS || TriggerType.WEBHOOK
}