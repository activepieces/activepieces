...
import { CollectionActions } from '@activepieces/ui/feature-builder-store';
...
export class ToggleInstanceStateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.instanceStateChanged$ = this.toggleFormControl.valueChanges.pipe(
      tap((res) => {
        if (res) {
          this.store.dispatch(CollectionActions.enableInstance());
        } else {
          this.store.dispatch(CollectionActions.disableInstance());
        }
      })
    );
  }
