...
export class PieceMetadataService {
    ...
    private convertToFlowItemDetails(type: TriggerType | ActionType, piece: PieceMetadataModelSummary | PieceMetadataModel): FlowItemDetails {
    ...
    listCoreFlowItemsDetailsForAction(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        const corePiecesAction$ = this.listCorePieces(searchQuery, SuggestionType.ACTION).pipe(map(pieces => pieces.filter(this.hasActions).map(this.convertToFlowItemDetails.bind(this, ActionType.PIECE))));
    ...
    listCoreFlowItemsDetailsForTrigger(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        return this.listCorePieces(searchQuery, SuggestionType.TRIGGER).pipe(map(pieces => pieces.filter(this.hasTriggers).map(this.convertToFlowItemDetails.bind(this, TriggerType.PIECE))))
    }

    listAppFlowItemsDetailsForAction(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        return this.listAppPieces(searchQuery, SuggestionType.ACTION).pipe(map(pieces => pieces.filter(this.hasActions).map(this.convertToFlowItemDetails.bind(this, ActionType.PIECE))));
    }

    listAppFlowItemsDetailsForTrigger(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        return this.listAppPieces(searchQuery, SuggestionType.TRIGGER).pipe(map(pieces => pieces.filter(this.hasTriggers).map(this.convertToFlowItemDetails.bind(this, TriggerType.PIECE))));
    }
    ...
    listAllFlowItemsDetailsForAction(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        ...
        }).pipe(map(pieces => pieces.filter(this.hasActions).map(this.convertToFlowItemDetails.bind(this, ActionType.PIECE))));
    ...
    listAllFlowItemsDetailsForTrigger(searchQuery: string | undefined): Observable<FlowItemDetails[]> {
        ...
        }).pipe(map(pieces => pieces.filter(this.hasTriggers).map(this.convertToFlowItemDetails.bind(this, TriggerType.PIECE))));
    }
    ...
    getStepDetails(step: Action | Trigger): Observable<FlowItemDetails> {
        switch (step.type) {
            ...
            case ActionType.PIECE:
                return this.getPieceMetadata(step.settings.pieceName, step.settings.pieceVersion).pipe(map(this.convertToFlowItemDetails.bind(this, step.type)));
        }
    }
