import { Action, ActionType, BranchAction, LoopOnItemsAction } from '../../../../../../shared/src/lib/flows/actions/action'
...
import { isNil } from '../../../../../../shared/src/lib/common'
...
export class FlowDrawer {
    ...
    static construct(step: Action | Trigger | undefined): FlowDrawer {
        ...
        if (step.nextAction) {
            const nextFlowDrawer = FlowDrawer.construct(step.nextAction).offset(0, FLOW_ITEM_HEIGHT + childHeight)
...
function handleLoopAction(step: LoopOnItemsAction, centerBottom: Position): FlowDrawer {
    ...
    const sideLength = firstLoopDrawer.boundingBox().width / 4.0 + SPACE_ON_EMPTY_LOOP_SIDE_HORZINTAL / 2.0

    const firstLoopOffset = {
        x: sideLength,
        y: FLOW_ITEM_HEIGHT + SPACE_BETWEEN_VERTICAL_LONG_STEP,
    }
    const firstLoopDrawerDrawerWithOffset = firstLoopDrawer.offset(firstLoopOffset.x, firstLoopOffset.y)
    const firstLoopDrawerTopCenter = firstLoopDrawerDrawerWithOffset.steps[0].center('top')

    const { line, button } = drawLineWithButton(centerBottom, firstLoopDrawerTopCenter)

    const firstLoopStepClosingLine = SvgDrawer.empty()
        .move(firstLoopDrawerTopCenter.x, firstLoopDrawerTopCenter.y + firstLoopDrawerDrawerWithOffset.boundingBox().height)
    ...
    const emptyLoopLine = SvgDrawer.empty()
        ...
        .drawVerticalLine(SPACE_BETWEEN_VERTICAL_LONG_STEP - ARC_LENGTH - SPACE_BETWEEN_VERTICAL_ARROW_LINE_HEIGHT)
        .drawArc(false, true)
        .drawHorizontalLine(-(firstLoopOffset.x - 2 * ARC_LENGTH))
        .drawArc(false, false)
        .drawVerticalLine(SPACE_BETWEEN_VERTICAL_ARROW_LINE_HEIGHT - 2 * ARC_LENGTH + firstLoopDrawer.boundingBox().height + SPACE_BETWEEN_VERTICAL_LONG_STEP / 2.0)
        .drawArc(true, false)
        .drawHorizontalLine(sideLength - 2 * ARC_LENGTH)
        .drawArc(true, true)
        .drawVerticalLine(SPACE_BETWEEN_VERTICAL_LONG_STEP / 2.0 - ARC_LENGTH)

    return firstLoopDrawerDrawerWithOffset.appendSvg(line).appendButton(button).appendSvg(emptyLoopLine).appendSvg(firstLoopStepClosingLine)
}
function handleBranchAction(step: BranchAction, centerBottom: Position): FlowDrawer {
    ...
    const sides: FlowDrawer[] = actions.map((action) => FlowDrawer.construct(action))
    const { summationWidth, maximumHeight } = calculateDimensions(sides)

    let leftStartingPoint = -((summationWidth + SPACE_BETWEEN_TWO_BRANCH * (sides.length - 1)) / 2.0)
    sides.forEach((side, index) => {
        const stepPosition = {
            x: leftStartingPoint + side.boundingBox().width / 2.0,
            y: FLOW_ITEM_HEIGHT + SPACE_BETWEEN_VERTICAL_LONG_STEP,
        }
        ...
        const secondLine = drawLine({
            ...
            {
                x: centerBottom.x,
                y: centerBottom.y + SPACE_BETWEEN_VERTICAL_LONG_STEP + maximumHeight + SPACE_BETWEEN_VERTICAL_LONG_STEP,
            })
        resultDrawer = resultDrawer.mergeChild(sideDrawer).appendSvg(line).appendSvg(secondLine).appendButton(button)

        leftStartingPoint += side.boundingBox().width + (SPACE_BETWEEN_TWO_BRANCH)
