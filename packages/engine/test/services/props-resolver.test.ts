...
describe('Props resolver', () => {
  test('Test resolve inside nested loops', async () => {
    const modifiedExecutionState = executionState
      .upsertStep(
        'step_3',
        GenericStepOutput.create({
    ...
    const { resolvedInput: secondLevelResolvedInput } = await propsResolverService.resolve({
      unresolvedInput: '{{step_7.delayForInMs}}',
      executionState: modifiedExecutionState,
    })
    expect(secondLevelResolvedInput).toEqual(20000)
    const { resolvedInput: firstLevelResolvedInput } = await propsResolverService.resolve({
      unresolvedInput: '{{step_8.delayForInMs}}',
      executionState: modifiedExecutionState,
    })
  ...
  test('flatten array path', async () => {
    const { resolvedInput } = await propsResolverService.resolve({
      unresolvedInput: "{{flattenNestedKeys(trigger, ['users','name'])}}",
      executionState,
    })
  ...
  test('merge multiple flatten array paths', async () => {
    const { resolvedInput } = await propsResolverService.resolve({
      unresolvedInput: "{{flattenNestedKeys(trigger, ['users','name'])}} {{trigger.lastNames}}",
      executionState,
    })
  ...
  test('Test resolve object', async () => {
    ...
    expect(resolvedInput).toEqual({ input: { foo: 'bar', nums: [1, 2, 5], var: 6.4 } })
  })
  ...
  test('Test resolve text with object variable', async () => {
    ...
    expect(resolvedInput).toEqual(
      'values from trigger step: {"items":[5,"a"],"name":"John","price":6.4,"users":[{"name":"Alice"},{"name":"Bob"}],"lastNames":["Smith","Doe"]}',
    )
  })
  ...
  it('should return error for invalid file inside the array properties', async () => {
    ...
    expect(errors).toEqual({
      documents: {
        properties: [
          {
            file: ['Expected file url or base64 with mimeType, received: invalid-url'],
          },
        ],
      },
    })
