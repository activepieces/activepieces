...
export const propsProcessor = {
  applyProcessorsAndValidators: async (
    ...
    const isAuthenticationProperty =
      auth &&
      (auth.type === PropertyType.CUSTOM_AUTH || auth.type === PropertyType.OAUTH2) &&
      !isNil(auth.props) &&
      requireAuth
    ...
    for (const [key, value] of Object.entries(resolvedInput)) {
      ...
      if (property.type === PropertyType.ARRAY && property.properties) {
        ...
        for (const item of arrayOfObjects) {
          const { processedInput: itemProcessedInput, errors: itemErrors } =
            await propsProcessor.applyProcessorsAndValidators(item, property.properties, undefined, false, undefined)
