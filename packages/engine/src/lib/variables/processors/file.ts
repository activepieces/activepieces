...
async function handleUrlFile(path: string): Promise<ApFile | null> {
  ...
  const filename =
    getFileName(path, fileResponse.headers['content-disposition'], fileResponse.headers['content-type']) ?? 'unknown'
...
function getFileName(path: string, disposition: string | null, mimeType: string | undefined): string | null {
  ...
  if (isNil(disposition)) {
    const fileNameFromUrl =
      url.pathname.includes('/') && url.pathname.split('/').pop()?.includes('.') ? url.pathname.split('/').pop() : null
