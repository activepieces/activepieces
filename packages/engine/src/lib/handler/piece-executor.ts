...
const executeAction: ActionHandler<PieceAction> = async ({ action, executionState, constants }) => {
  ...
  try {
    ...
    const { processedInput, errors } = await propsProcessor.applyProcessorsAndValidators(
      ...
      action.settings.inputUiInfo?.schema as Record<string, InputPropertyMap> | undefined,
    )
    ...
    const context: ActionContext = {
      ...
      generateResumeUrl: (params) => {
        const url = new URL(
          `${constants.publicApiUrl}v1/flow-runs/${constants.flowRunId}/requests/${executionState.pauseRequestId}`,
        )
    ...
    const runMethodToExecute =
      constants.testSingleStepMode && !isNil(pieceAction.test) ? pieceAction.test : pieceAction.run
    ...
    const failedStepOutput = stepOutput.setStatus(StepOutputStatus.FAILED).setErrorMessage(handledError.message)
...
const createConnectionManager = ({
  ...
  hookResponse,
  apiUrl,
}: { projectId: string; engineToken: string; hookResponse: HookResponse; apiUrl: string }): ConnectionsManager => {
  return {
