import { LoopOnItemsAction, LoopStepOutput, StepOutputStatus, isNil } from '@activepieces/shared'
...
export const loopExecutor: BaseExecutor<LoopOnItemsAction> = {
  async handle({ action, executionState, constants }) {
    ...
    if (!Array.isArray(resolvedInput.items)) {
      const failedStepOutput = stepOutput.setStatus(StepOutputStatus.FAILED).setErrorMessage(
        JSON.stringify({
