...
export function assertNull<T>(value: T | null, fieldName: string): asserts value is T {
    if (value !== null) {
        throw new Error(`${fieldName} should be null`)
    }
}

export function asserNotEmpty<T>(value: T[] | null | undefined, fieldName: string): asserts value is T[] {
