...
function getFlowBBox(step: Step | FlowAction | null | undefined, forBranch = false): CanvasBBox {
    ...
    if (step.type === FlowActionType.LOOP_ON_ITEMS) {
        ...
        const deltaLeftX =
            -(childWidth + FLOW_CANVAS_STEP_WIDTH + FLOW_CANVAS_HSPACE - FLOW_CANVAS_STEP_WIDTH / 2 - childRight) / 2 -
            FLOW_CANVAS_STEP_WIDTH / 2
        const childOffsetX = deltaLeftX + FLOW_CANVAS_STEP_WIDTH + FLOW_CANVAS_HSPACE + childLeft
        const subgraphEndY =
            FLOW_CANVAS_STEP_HEIGHT + FLOW_CANVAS_LOOP_VOFFSET + childBBox.height + FLOW_CANVAS_ARC + FLOW_CANVAS_VSPACE
        withChildMinX = Math.min(0, deltaLeftX, childOffsetX + childBBox.minX)
        withChildMaxX = Math.max(
            FLOW_CANVAS_STEP_WIDTH,
            deltaLeftX + FLOW_CANVAS_STEP_WIDTH,
            childOffsetX + childBBox.maxX,
        )
        ...
        if (children.length > 0) {
            ...
            const totalWidth =
                childBBoxes.reduce((sum, b) => sum + (b.maxX - b.minX), 0) + FLOW_CANVAS_HSPACE * (children.length - 1)
...
function buildPositions(
    ...
    if (step.type === FlowActionType.LOOP_ON_ITEMS) {
        ...
        const subgraphEndY =
            FLOW_CANVAS_STEP_HEIGHT + FLOW_CANVAS_ROUTER_VOFFSET + maxChildHeight + FLOW_CANVAS_ARC + FLOW_CANVAS_VSPACE
        buildPositions(step.nextAction, offsetX, offsetY + subgraphEndY, positions)
    } else {
