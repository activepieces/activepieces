...
export class StorageLimitError extends ExecutionError {
    ...
    constructor(key: string, maxStorageSizeInBytes: number, cause?: unknown) {
        super(
            'StorageLimitError',
            formatMessage(
                `Failed to read/write key "${key}", the value you are trying to read/write is larger than ${Math.floor(maxStorageSizeInBytes / 1024)} KB`,
            ),
            ExecutionErrorType.USER,
            cause,
        )
...
export class StorageInvalidKeyError extends ExecutionError {
    constructor(key: string, cause?: unknown) {
        super(
            ...
            formatMessage(
                `Failed to read/write key "${key}", the key is empty or longer than ${STORE_KEY_MAX_LENGTH} characters`,
            ),
            ExecutionErrorType.USER,
            cause,
        )
...
export class StorageError extends ExecutionError {
    constructor(key: string, cause?: unknown) {
        super(
            'StorageError',
            formatMessage(`Failed to read/write key "${key}" due to ${JSON.stringify(cause)}`),
...
export class FileStoreError extends ExecutionError {
    constructor(cause?: unknown) {
        super(
            'FileStoreError',
            formatMessage(`Failed to store file due to ${JSON.stringify(cause)}`),
            ExecutionErrorType.ENGINE,
            cause,
        )
    }
...
export class PausedFlowTimeoutError extends ExecutionError {
    constructor(cause?: unknown, maximumPauseDurationDays?: number) {
        super(
            ...
            `The flow cannot be paused for more than ${maximumPauseDurationDays} days`,
            ExecutionErrorType.USER,
            cause,
        )
    }
...
export class InvalidCronExpressionError extends ExecutionError {
    constructor(cronExpression: string, cause?: unknown) {
        super(
            ...
            formatMessage(`Invalid cron expression: ${cronExpression}`),
            ExecutionErrorType.USER,
            cause,
        )
    }
