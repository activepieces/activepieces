...
function mapToNewNames(flowVersion: FlowVersion, clonedActions: Action[]): Record<string, string> {
  const existingNames = flowStructureUtil.getAllSteps(flowVersion.trigger).map((step) => step.name)

  const oldStepNames = clonedActions.flatMap((clonedAction) =>
    flowStructureUtil.getAllSteps(clonedAction).map((step) => step.name),
  )

  return oldStepNames.reduce(
    (nameMap, oldName) => {
...
function replaceOldStepNameWithNewOne({ input, oldStepName, newStepName }: ReplaceOldStepNameWithNewOneProps): string {
...
function clone(step: Action, oldNameToNewName: Record<string, string>): Action {
  ...
  if ('input' in step.settings) {
    Object.keys(oldNameToNewName).forEach((oldName) => {
      ...
      settings.input = applyFunctionToValuesSync(settings.input, (value: unknown) => {
        if (isString(value)) {
