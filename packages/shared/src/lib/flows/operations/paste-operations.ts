...
export const _getOperationsForPaste = (actions: Action[], flowVersion: FlowVersion, pastingDetails: PasteLocation) => {
  const newNamesMap = addActionUtils.mapToNewNames(flowVersion, actions)
  const clonedActions: Action[] = actions.map(
    (action) =>
      flowStructureUtil.transferStep(action, (step: Action) => {
