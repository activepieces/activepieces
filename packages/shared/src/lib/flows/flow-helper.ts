...
function getAllChildSteps(action: LoopOnItemsAction | BranchAction | RouterAction): Action[] {
  switch (action.type) {
    case ActionType.LOOP_ON_ITEMS:
      return traverseInternal(action.firstLoopAction) as Action[];
    case ActionType.ROUTER:
      return action.children.map((child) => traverseInternal(child?.nextAction)).flat() as Action[];
    default:
      return [
...
function isChildOf(
  ...
  switch (parent.type) {
    ...
    case ActionType.ROUTER: {
      const children = parent.children.filter((child): child is Action => child !== null);
      return children.findIndex((c) => c.name === childStepName) > -1;
    }
    default: {
      const children = getAllChildSteps(parent);
...
function getDirectParentStep(
  ...
  if (parent.type === ActionType.ROUTER) {
    ...
    if (isChildOfRouter) {
      const directChildren = parent.children.map((child) => child?.nextAction).filter((child) => child);
      if (directChildren.at(-1)?.name === child.name) {
        return parent;
      }
...
function isStepLastChildOfParent(child: Step, trigger: Trigger): boolean {
  ...
  if (parent) {
    if (doesStepHaveChildren(parent)) {
      ...
      if (parent.type === ActionType.ROUTER) {
        const children = parent.children.map((child) => child?.nextAction).filter((child) => child);
        return children[children.length - 1]?.name === child.name;
      }
