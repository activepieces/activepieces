...
import { agentToolsName } from "./tool"
...
export const genericAgentUtils = {
    ...
    extractStructuredOutput(conversation: Conversation): Record<string, unknown> | undefined {
        for (const msg of conversation) {
            if (msg.role !== 'assistant') {
                continue
            }
            const toolCall = msg.parts.find((part) => part.type === 'tool-call' && part.toolName === agentToolsName.TASK_COMPLETION_TOOL_NAME)
            if (!isNil(toolCall) && toolCall.type === 'tool-call') {
                const input = toolCall.input as Record<string, unknown> | undefined
                return input?.['output'] as Record<string, unknown> | undefined
            }
        }
        return undefined
    },
