...
export class Oauth2Service {
  ...
  public claimWithSecret(request: ClaimTokenWithSecretRequest) {
    return this.httpClient.post<BaseOAuth2ConnectionValue>(
      environment.apiUrl + '/oauth2/claim',
      request
    );
  }

  public openPopup(
    request: OAuth2AppDetails & {
      scope: string;
      auth_url: string;
  ...
  private claimWithScretForCloud(request: {
    pieceName: string;
    code: string;
    tokenUrl: string | undefined;
  }) {
    return this.httpClient.post<BaseOAuth2ConnectionValue>(
      environment.apiUrl + '/oauth2/claim-with-cloud',
      request
    );
  }
  public openCloudAuthPopup(
    request: CloudConnectionPopupSettings
  ): Observable<any> {
    this.currentlyOpenPopUp?.close();
    const winTarget = '_blank';
    const winFeatures =
