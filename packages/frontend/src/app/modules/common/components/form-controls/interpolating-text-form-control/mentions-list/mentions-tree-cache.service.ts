...
export class MentionsTreeCacheService {
	private cache: Map<StepName, { children: MentionTreeNode[], value?: any }> = new Map();
	...
	searchForSubstringInKeyOrValue(stepName: string, substr: string): boolean {
		...
		if (tree) {
			if (tree.value && String(tree.value).toLowerCase().includes(substr.toLowerCase())) {
				return true;
			}
			else {
	...
	private searchTreeForSubstr(tree: MentionTreeNode[], substr: string): boolean {
	...
	private _markNodesToShow(tree: (MentionTreeNode & { show?: boolean })[], substr: string, markedNodesToShow: Map<string, boolean>): void {
		for (const node of tree) {
			const nodeContainsSubstring = node.propertyPath.toLowerCase().includes(substr.toLowerCase());
			...
			if (node.children) {
				...
				const showAnyChild = node.children.some(c => markedNodesToShow.get(c.propertyPath));
				markedNodesToShow.set(node.propertyPath, nodeContainsSubstring || showAnyChild);
			}
		}
	}
}
