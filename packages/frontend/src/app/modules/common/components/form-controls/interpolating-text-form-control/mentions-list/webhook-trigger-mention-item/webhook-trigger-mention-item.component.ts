...
import { TriggerType, WebhookTrigger } from 'shared';
...
export class WebhookTriggerMentionItemComponent {
	...
	emitMention() {
		if (this.pathFormGroup.valid) {
			...
			const triggerPathWithoutInterpolationDenotation = triggerPath.slice(2, triggerPath.length - 1);
			const mentionText = replaceArrayNotationsWithSpaces(
				replaceDotsWithSpaces(this.adjustItemPath(triggerPathWithoutInterpolationDenotation))
			);
			const mentionItem: MentionListItem = { value: this.pathFormGroup.controls.path.value!, label: mentionText };
	...
	adjustItemPath(triggerPathWithoutInterpolationDenotation: string): any {
		...
		return [triggerDisplayName, ...triggerPathWithoutInterpolationDenotation.split('.').slice(1)].join('.');
	}
