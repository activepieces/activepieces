...
export class CreateEditConfigModalComponent
  implements OnInit, AfterViewChecked
{
  ...
  constructor(
    ...
    public dialogData: undefined | { config: Config; index: number },
  ...
  private buildConfigForm() {
    this.configForm = this.formBuilder.group({
      ...
      type: new FormControl(ConfigType.SHORT_TEXT, {
        nonNullable: true,
  ...
  private setupConfigTypeListener() {
    this.configTypeChanged$ = this.configForm.controls.type.valueChanges.pipe(
      ...
      tap(([oldType, newType]) => {
        if (
          !this.isConfigOfTypeText(oldType) ||
          !this.isConfigOfTypeText(newType)
        ) {
  ...
  isConfigOfTypeText(configType: ConfigType) {
    return (
      configType === ConfigType.SHORT_TEXT ||
      configType === ConfigType.LONG_TEXT
    );
  }
}
