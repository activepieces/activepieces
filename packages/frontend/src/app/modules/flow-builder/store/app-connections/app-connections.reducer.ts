...
const _appConnectionsReducer = createReducer(
	initialState,
	on(BuilderActions.loadInitial, (state, { appConnections }): AppConnectionsState => {
		return {
			loaded: true,
			connections: appConnections,
		};
	}),
	on(appConnectionsActions.upsert, (state, { connection }): AppConnectionsState => {
		const clonedConnections = [...state.connections];
		const upsertedConnection = clonedConnections.find(c => c.name === connection.name);
		if (!upsertedConnection) {
			...
			const upsertedConnectionIndex = clonedConnections.findIndex(c => c.name === connection.name);
...
export function appConnectionsReducer(state: AppConnectionsState | undefined, action: Action) {
