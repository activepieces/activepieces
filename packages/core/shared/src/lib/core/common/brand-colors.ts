...
function stepValue({
  ...
  const entry = PRIMARY_CURVE.find((candidate) => candidate.step === step);
  if (entry === undefined) {
    return '';
  }
  return formatOklch({
...
function primaryRoles({
  ...
  const at = (step: string): string => stepValue({ step, hue, chromaScale });
  return {
    primary: primaryColor,
    onPrimary: onPrimaryFor({ hex: primaryColor }),
    hover: at('700'),
    ink: theme === 'dark' ? at('300') : at('700'),
    line: theme === 'dark' ? at('800') : at('200'),
    mark: theme === 'dark' ? at('400') : at('500'),
  };
}
