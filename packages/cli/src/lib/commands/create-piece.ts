...
const scaffoldPiece = async (pieceName: string, packageName: string, pieceType: string) => {
    ...
    await writeFile(path.join(baseDir, 'package.json'), JSON.stringify(packageJson, null, 2))
    ...
    await writeFile(path.join(baseDir, 'tsconfig.json'), JSON.stringify(tsconfig, null, 2))
    ...
    await writeFile(path.join(baseDir, 'tsconfig.lib.json'), JSON.stringify(tsconfigLib, null, 2))
    ...
    await writeFile(path.join(baseDir, '.eslintrc.json'), JSON.stringify(eslintConfig, null, 2))
...
export const createPiece = async (pieceName: string, packageName: string, pieceType: string) => {
    ...
    console.log(chalk.yellow(`The piece has been generated at: packages/pieces/${pieceType}/${pieceName}`))
}

export const createPieceCommand = new Command('create').description('Create a new piece').action(async () => {
    const questions = [
        {
            type: 'input',
            name: 'pieceName',
