...
export const generateTranslationFileForPieceCommand = new Command('generate-translation-file')
  ...
  .action(async () => {
    try {
      const textContent = `Hello! This is a simple text file generated by the CLI command.

Generated at: ${new Date().toISOString()}
Command: generate-translation-file-for-all-pieces
Workspace: activepieces

This file was created as part of the translation file generation process.`;

      const outputPath = join(process.cwd(), 'generated-text-file.txt');
      await writeFile(outputPath, textContent);
      console.log(chalk.yellow('✨'), `Simple text file created at: ${outputPath}`);
    } catch (error) {
      console.error(chalk.red('❌'), `Error generating text file:`, error);
    }
  });
