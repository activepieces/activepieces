...
function getPropertyValue(object: Record<string, unknown>, path: string): unknown {
    ...
    if (parsedKeys[0] === '*') {
        return Object.values(object)
            .map((item) => getPropertyValue(item as Record<string, unknown>, parsedKeys.slice(1).join('.')))
            .filter(Boolean)
            .flat()
    }
...
const generateTranslationFileFromPiece = (piece: Record<string, unknown>) => {
...
const generateTranslationFile = async (pieceName: string) => {
    ...
    try {
        ...
        const i18n = generateTranslationFileFromPiece({
            actions: (pieceFromModule as any)._actions,
            triggers: (pieceFromModule as any)._triggers,
            description: (pieceFromModule as any).description,
            displayName: (pieceFromModule as any).displayName,
            auth: (pieceFromModule as any).auth,
        })
        ...
        console.error(
            chalk.red('❌'),
            `Error generating translation file for piece ${pieceName}, make sure you built the piece`,
            error,
        )
    }
}
...
export const generateTranslationFileForAllPiecesCommand = new Command('generate-translation-file-for-all-pieces')
    ...
    .action(async ({ shardIndex, shardTotal }: { shardIndex: number; shardTotal: number }) => {
        ...
        for (const piece of pieces) {
            ...
            console.log(
                chalk.yellow('✨'),
                `Translation file for piece ${piece} created in ${(performance.now() - time) / 1000}s`,
            )
        ...
        console.log(
            chalk.yellow('✨'),
            `Total time taken to generate translation files for selected pieces: ${totalTime}s`,
        )
    })
