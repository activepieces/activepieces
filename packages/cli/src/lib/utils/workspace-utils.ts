    ...
    for (const [name, version] of Object.entries(deps)) {
        if (version.startsWith('workspace:')) {
            ...
            if (resolvedVersion) {
                ...
                throw new Error(
                    `Failed to resolve workspace dependency ${name}: ${version}. Package not found in workspace.`,
                )
            }
        } else {
...
export function stripSemverRanges(deps: Record<string, string> | undefined): Record<string, string> | undefined {
    if (!deps) {
        return deps
    }
