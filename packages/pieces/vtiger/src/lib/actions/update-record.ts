...
import { DropdownState, DynamicPropsValue, PiecePropValueSchema, Property, createAction } from "@activepieces/pieces-framework";
import { vtigerAuth } from "../..";
import { instanceLogin, VTigerAuthValue, Modules, Field, getRecordReference } from "../common";
  ...
  props: {
      ...
      options: async ({ auth, elementType}) => {
        if (!auth || !elementType) {
          return {
            disabled: true,
            options: [],
            placeholder: 'Please select the element type and setup authentication to continue.'
          }
        }
        ...
        return {
          options: response.body.result.map(
            (r) => ({ label: Modules[element](r), value: r['id'] })
          ),
          disabled: false,
        };
      },
    ...
    record: Property.DynamicProperties({
      ...
      props: async ({ auth, id, elementType }) => {
        ...
        if (id && 'id') {
          const retrieve_response = await httpClient.sendRequest<{ success: boolean, result: Record<string, unknown>}>({
            method: HttpMethod.GET,
