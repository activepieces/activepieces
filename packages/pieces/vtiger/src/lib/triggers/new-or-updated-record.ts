...
const polling: Polling<PiecePropValueSchema<typeof vtigerAuth>, { elementType?: string, watchBy?: string, limit?: number }> = {
...
const fetchRecords = async (
  { auth, propsValue, lastFetchEpochMS }: 
  { auth: Record<string, string>, propsValue: Record<string, unknown>, lastFetchEpochMS: number }
) => {
  ...
  if (response.body.success) {
    ...
    const newOrUpdatedRecords = filter(
      records, (record) => {
