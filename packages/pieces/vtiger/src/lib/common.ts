...
export const instanceLogin = async (instance_url: string, username: string, password: string, debug = false) => {
  ...
  const challenge = (await httpClient.sendRequest<{success: boolean, result: Challenge}>({
  ...
  const response = await httpClient.sendRequest<{success: boolean, result: Instance}>({
...
export const prepareHttpRequest = (
  ...
  if ('element' in record)
    data['element'] =  JSON.stringify(record['element'])
...
export const recordIdProperty = () => Property.DynamicProperties({
...
export async function getRecordReference(auth: PiecePropValueSchema<typeof vtigerAuth>, modules: string[]): Promise<DropdownState<string>> {
  ...
  const response = await httpClient.sendRequest<{success: boolean, result: Record<string, unknown>[]}>(httpRequest);

  if (response.body.success) {
    return {
      disabled: false,
      options: response.body.result.map(
        (record) => {
          return {
...
export const recordProperty = (create=true) => Property.DynamicProperties({
  displayName: "Record Fields",
  description: "Add new fields to be created in the new record",
  required: true,
  ...
  props: async ({ auth, id, elementType }) => {
    ...
    if (describe_response.body.success) {
      const generateField = async (field: Field) => {
        ...
        if (
          ['string', 'text', 'mediumtext', 'phone', 'url', 'email'].includes(field.type.name)
        ) {
          if (['mediumtext', 'url'].includes(field.type.name)) {
            fields[field.name] = Property.LongText({...params, defaultValue: defaultValue?.[field.name] as string})
          } else {
            fields[field.name] = Property.ShortText({...params, defaultValue: defaultValue?.[field.name] as string})
          }
        } else if (['picklist', 'reference', 'owner'].includes(field.type.name)) {

          let options: DropdownState<string>
          if (field.type.name === 'picklist') {
            options = { disabled: false, options: (field.type.picklistValues ?? []) }
