  ...
  validate: async ({ auth }) => {
    ...
    try {
      if (!isBaseUrl(instance_url)) {
        return {
          valid: false,
          error: "Please ensure that the website is valid and does not contain any paths, for example, https://<instance>.od2.vtiger.com  ",
        };
      }
      if (instance_url.endsWith("/")) {
        return {
          valid: false,
          error: "Please enter the URL without a trailing slash. E.g. https://<instance>.od2.vtiger.com instead of https://<instance>.od2.vtiger.com/",
        };
      }
      if (instance_url.includes("restapi/")) {
        return {
          valid: false,
          error: "Add the url without the endpoint. For example add https://<instance>.od2.vtiger.com/ instead of https://<instance>.od2.vtiger.com/restapi/v1/vtiger/default",
        };
      }
