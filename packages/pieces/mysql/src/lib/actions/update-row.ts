...
export default createAction({
    ...
    async run(context) {
        ...
        const qs = "UPDATE `" + context.propsValue.table + "` SET " + qsValues + " WHERE " + sanitizeColumnName(context.propsValue.search_column) + "=?;"
        const conn = await mysqlConnect(context.propsValue);
        try {
            const result = await conn.query(qs, [ ...values, context.propsValue.search_value ])
            return result
        } finally {
            await conn.end()
        }
