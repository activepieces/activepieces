import { HttpMethod, HttpMessageBody, httpClient, HttpResponse } from "@activepieces/pieces-common";
...
export class KimaiClient {
    ...
    constructor(private baseUrl: string, private user: string, private apiPassword: string) {
    ...
    async getActivities(project: number | undefined = undefined): Promise<ActivityResponse[]> {
        return (await this.makeRequest<ActivityResponse[]>(HttpMethod.GET, '/api/activities', {
            'project': project
        })).body;
    }
    ...
    async makeRequest<T extends HttpMessageBody>(method: HttpMethod, resourceUri: string, body: any | undefined = undefined): Promise<HttpResponse<T>> {
