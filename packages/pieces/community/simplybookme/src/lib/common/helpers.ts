...
import { getAccessToken, makeJsonRpcCall, SimplybookAuth, simplybookAuth } from './auth'
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await Promise.race([
                makeJsonRpcCall<any>(auth.props, 'getEventList', []),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Request timeout after 20s')), 20000)),
            ])
...
export const providerDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await Promise.race([
                makeJsonRpcCall<any>(auth.props, 'getUnitList', []),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Request timeout after 20s')), 20000)),
            ])
