...
export const simplybookAuth = PieceAuth.CustomAuth({
    description: 'Enter your Simplybook admin credentials to access the Admin API.',
    props: {
...
export async function getAccessToken(auth: SimplybookAuth): Promise<string> {
    try {
        ...
        if (response.body?.error) {
            ...
            else if (errorCode === -33003) helpfulMessage = 'User with this login and password not found'
            else if (errorCode === -33004) helpfulMessage = 'User is blocked'
            else if (errorCode === -33005)
                helpfulMessage = 'You are not allowed to use this application when HIPAA Custom Feature is enabled'

            throw new Error(`Simplybook authentication error: ${helpfulMessage} (code: ${errorCode})`)
        }
