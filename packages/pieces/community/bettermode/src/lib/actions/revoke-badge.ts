...
export const revokeBadgeAction = createAction({
    ...
    props: {
        badgeId: Property.Dropdown({
            ...
            required: true,
            refreshers: [],
            options: async ({ auth }) => await buildBadgesDropdown(auth?.props),
        }),
    ...
    async run(context) {
        return await revokeBadgeFromMember(context.auth.props, context.propsValue.badgeId, context.propsValue.email)
    },
})
