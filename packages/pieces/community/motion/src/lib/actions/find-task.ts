...
import { BASE_URL, projectId, statusId, userId, workspaceId } from '../common/props'
...
export const findTask = createAction({
    ...
    async run(context) {
        const { workspaceId, includeAllStatuses, name, status, assigneeId, projectId } = context.propsValue
