...
export const findConversation = createAction({
    ...
    description: 'List and search conversations for an agent with optional filtering',
    props: {
        ...
        lastUpdatedAfter: Property.DateTime({
            ...
            description: 'Only return conversations with messages after this date (optional) e.g. 2025-01-09T18:30:00Z',
            required: false,
        }),
    },
    async run(context) {
        const { project_id, name, userFilter, lastUpdatedAfter } = context.propsValue
