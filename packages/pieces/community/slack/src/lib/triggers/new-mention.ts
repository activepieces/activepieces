...
import { getChannels, multiSelectChannelInfo, usergroupIds, userIds } from '../common/props'
...
export const newMention = createTrigger({
    ...
    props: {
        ...
        removeMention: Property.Checkbox({
            ...
            description:
                'If enabled, provides a clean_text field with the user and user group mentions removed from the message.',
            required: true,
            defaultValue: false,
        }),
    },
    ...
    run: async (context) => {
        ...
        const usergroupMentioned = usergroups.some((gid) => new RegExp(`<!subteam\\^${gid}(\\|[^>]*)?>`).test(text))
        ...
        if (context.propsValue.removeMention) {
            ...
            for (const gid of usergroups) {
                cleanText = cleanText.replace(new RegExp(`<!subteam\\^${gid}(\\|[^>]*)?>`, 'g'), '')
            }
