...
const props = {
  streamers: Property.Array({
    ...
    description: 'Add the usernames or URLs of the streamers you want to monitor (e.g., loltyler1 or https://twitch.tv/faker).',
    required: true
  })
}
const polling: Polling<AppConnectionValueForAuthProperty<typeof twitchAuth>, StaticPropsValue<typeof props>> = {
  strategy: DedupeStrategy.TIMEBASED,
  items: async ({ auth, propsValue }) => {
    const { streamers } = propsValue

    const logins = (streamers as string[]).map(s => s.replace(/.*\//, '').trim().toLowerCase())

    const queryString = logins.map(login => `user_login=${encodeURIComponent(login)}`).join('&');
