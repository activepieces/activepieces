...
export class MeisterTaskClient {
  ...
  async getTasks(projectId: number): Promise<MeisterTaskTask[]> {
    try {
      return await this.makeRequest<MeisterTaskTask[]>(
        HttpMethod.GET,
  ...
  async getLabels(projectId: number): Promise<MeisterTaskLabel[]> {
    try {
      return await this.makeRequest<MeisterTaskLabel[]>(
        HttpMethod.GET,
  ...
  async getSections(projectId: number): Promise<MeisterTaskSection[]> {
    try {
      return await this.makeRequest<MeisterTaskSection[]>(
        HttpMethod.GET,
  ...
  async getAttachments(taskId: number): Promise<MeisterTaskAttachment[]> {
    try {
      return await this.makeRequest<MeisterTaskAttachment[]>(
        HttpMethod.GET,
  ...
  async getPersonById(personId: number): Promise<MeisterTaskPerson> {
    try {
      return await this.makeRequest<MeisterTaskPerson>(
        HttpMethod.GET,
        `/persons/${personId}`
      );
    } catch (e) {
  ...
  async getProjectMembers(projectId: number): Promise<MeisterTaskMember[]> {
    try {
      const r = await this.makeRequest<{ project_rights: { person_id: number }[] }>(
        HttpMethod.GET,
        `/projects/${projectId}/members`
      );

      if (!r || !Array.isArray(r.project_rights)) {
        return [];
      }

      const pIds = r.project_rights.map((p) => p.person_id);
      const p = await Promise.all(pIds.map((id) => this.getPersonById(id)));

      return p.filter((m) => m.email !== "Error");
    } catch (e) {
