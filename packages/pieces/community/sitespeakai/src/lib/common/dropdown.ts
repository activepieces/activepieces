...
export const chatbotIdDropdown = Property.Dropdown({
    ...
    async options({ auth }) {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, `/me/chatbots`)
...
export const conversationIdDropdown = Property.Dropdown<string, false, typeof SiteSpeakAuth>({
    ...
    async options({ auth, chatbotId }) {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, `/${chatbotId}/conversations`)

            return {
                disabled: false,
                options: response.map((conv: any) => ({
                    label:
                        `Speaker: ${conv.speaker}, Started: ${new Date(conv.created_at).toLocaleDateString()}` ||
                        conv.id,
                    value: conv.id,
                })),
            }
...
export const finetuneIdDropdown = Property.Dropdown<string, true, typeof SiteSpeakAuth>({
    ...
    async options({ auth, chatbotId }) {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, `/${chatbotId}/finetunes`)
