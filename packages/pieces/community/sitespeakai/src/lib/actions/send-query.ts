...
export const sendQuery = createAction({
    ...
    description: 'Sends a query to the specified chatbot and retrieves a response.',
    props: {
    ...
    async run({ auth, propsValue }) {
        ...
        const response = await makeRequest(auth.secret_text, HttpMethod.POST, `/${propsValue.chatbotId}/query`, body)
        return response
    },
})
