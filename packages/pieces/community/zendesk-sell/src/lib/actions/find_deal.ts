...
export const findDeal = createAction({
    ...
    async run(context) {
        ...
        if (deal_id) {
            const response = await callZendeskApi(HttpMethod.GET, `v2/deals/${deal_id}`, auth)
        ...
        if (deal_name) {
            ...
            const response = await callZendeskApi(HttpMethod.POST, 'v3/deals/search', auth, requestBody)
