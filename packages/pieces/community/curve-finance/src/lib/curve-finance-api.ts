...
export const CURVE_PROTOCOL_SLUG = 'curve-dex';
export const CURVE_COINGECKO_ID = 'curve-dao-token';
...
export async function getProtocolData(): Promise<DefiLlamaProtocol> {
  const response = await httpClient.sendRequest<DefiLlamaProtocol>({
    ...
    url: `${DEFILLAMA_BASE_URL}/protocol/${CURVE_PROTOCOL_SLUG}`,
  });
  return response.body;
}

export async function getCrvTokenData(): Promise<CoinGeckoPrice> {
  const response = await httpClient.sendRequest<CoinGeckoPrice>({
    method: HttpMethod.GET,
    url: `${COINGECKO_BASE_URL}/coins/${CURVE_COINGECKO_ID}`,
    queryParams: {
      localization: 'false',
      tickers: 'false',
      market_data: 'true',
