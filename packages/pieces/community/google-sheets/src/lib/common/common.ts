...
export async function findSheetName(auth: GoogleSheetsAuthValue, spreadsheetId: string, sheetId: string | number) {
...
async function getGoogleSheetRows({
    ...
    if (!headerResponse.body.values) {
        throw new Error(
            `Unable to read headers from row ${headerRow} in sheet "${sheetName}". The row appears to be empty or inaccessible.`,
        )
    }
    ...
    const labeledRowValues = mapRowsToColumnLabels(rowsResponse.body.values, startingRow, headerCount)
...
export async function getHeaderRow({ spreadsheetId, auth, sheetId }: GetHeaderRowProps): Promise<string[] | undefined> {
...
async function deleteRow(spreadsheetId: string, sheetId: number, rowIndex: number, auth: GoogleSheetsAuthValue) {
...
export function areSheetIdsValid(
    spreadsheetId: string | null | undefined,
    sheetId: string | number | null | undefined,
): boolean {
    return !isNil(spreadsheetId) && spreadsheetId !== '' && !isNil(sheetId) && sheetId !== ''
}
