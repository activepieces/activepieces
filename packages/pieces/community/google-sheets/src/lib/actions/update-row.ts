...
import { Dimension, ValueInputOption, areSheetIdsValid, objectToArray } from '../common/common'
...
export const updateRowAction = createAction({
  ...
  async run(context) {
    ...
    const formattedValues = (isFirstRowHeaders ? objectToArray(rowValuesInput) : rowValuesInput['values']).map(
    ...
    if (formattedValues.length > 0) {
      ...
      const updatedRowNumber = parseInt(updatedRowRange?.split(':')[0].substring(1) ?? '0', 10)
