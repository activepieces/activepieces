...
export const newOrUpdatedRowTrigger = createTrigger({
  ...
  props: {
    ...
    trigger_column: Property.Dropdown({
      ...
      options: async ({ auth, spreadsheetId, sheetId }) => {
        ...
        const firstRowValues = await getWorkSheetValues(authValue, spreadsheet_id, `${sheetName}!1:1`)
  ...
  async run(context) {
    ...
    for (let row = 0; row < currentValues.length; row++) {
      ...
      if (triggerColumn === ALL_COLUMNS) {
        ...
        targetValue =
          currentTriggerColumnValue !== undefined && currentTriggerColumnValue !== '' ? [currentTriggerColumnValue] : []
      }
      ...
      const oldRowHash = !isNil(oldValuesHashes) && row < oldValuesHashes.length ? oldValuesHashes[row] : undefined
  ...
  async test(context) {
    ...
    const transformedRowValues = transformWorkSheetValues(currentSheetValues, 0, headerCount).slice(-5).reverse()
