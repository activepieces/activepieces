...
import { GoogleFormsAuthValue, getAccessToken, googleFormsAuth, googleFormsCommon } from '../common/common'
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof googleFormsAuth>, { form_id: string }> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        ...
        return items
            .sort((a, b) => new Date(b.lastSubmittedTime).getTime() - new Date(a.lastSubmittedTime).getTime())
            .map((item) => ({
...
const getResponse = async (authentication: GoogleFormsAuthValue, form_id: string, startDate: string | null) => {
    let filter = {}
    if (startDate) {
        filter = {
