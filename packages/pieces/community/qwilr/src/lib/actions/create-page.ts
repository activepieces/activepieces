...
export const createPageAction = createAction({
    ...
    props: {
        ...
        acceptSettings: Property.Object({
            ...
            description:
                'Configures settings for the accept block in the template being used. Only relevant if you have an accept block.',
            required: false,
        }),
    },
    async run(context) {
        const { templateId, name, published, ownerId, tags, metadata, substitutions, acceptSettings } =
            context.propsValue
