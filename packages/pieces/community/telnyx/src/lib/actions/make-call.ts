...
export const makeCallAction = createAction({
    ...
    props: {
        connection_id: Property.ShortText({
            ...
            description: 'The Telnyx Call Control Application ID (formerly connection ID) used to place the call.',
            required: true,
        }),
        ...
        audio_url: Property.ShortText({
            ...
            description: 'Optional WAV or MP3 URL to play when the call is answered.',
            required: false,
        }),
        webhook_url: Property.ShortText({
            displayName: 'Webhook URL',
            description: 'Optional URL that overrides the default Telnyx webhook destination for this call.',
            required: false,
        }),
    ...
    async run(context) {
        const { connection_id, from, to, audio_url, webhook_url, timeout_secs } = context.propsValue
