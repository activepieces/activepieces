...
async function validateAccessToken(token: string): Promise<boolean> {
    try {
        await httpClient.sendRequest({
            ...
            url: `${CRYPTOLENS_API_BASE_URL}/auth/GetToken?token=${encodeURIComponent(token)}`,
        })
        return true
    } catch {
        return false
...
export const cryptolensAuth = PieceAuth.SecretText({
    ...
    description: 'Your Cryptolens access token. Get it from https://app.cryptolens.io/User/AccessToken',
    required: true,
    validate: async ({ auth }) => {
