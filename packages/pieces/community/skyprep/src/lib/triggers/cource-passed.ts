...
const polling: Polling<AppConnectionValueForAuthProperty<typeof skyprepAuth>, StaticPropsValue<typeof props>> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ propsValue, lastFetchEpochMS, auth }) => {
        // Fetch course progresses with "passed" status
        const response = await makeRequest(auth.secret_text, HttpMethod.POST, '/get_course_progresses', {
