...
const polling: Polling<AppConnectionValueForAuthProperty<typeof skyprepAuth>, Record<string, never>> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ propsValue, lastFetchEpochMS, auth }) => {
        const response = await makeRequest(auth.secret_text, HttpMethod.POST, '/get_users', {
        ...
        return (response || [])
            .filter((user: any) => dayjs(user.updated_at).valueOf() > lastFetchEpochMS)
            .map((user: any) => ({
