...
export const updateUser = createAction({
    ...
    async run(context) {
        ...
        if (email_notifications !== undefined) body.email_notifications = email_notifications
        if (sms_notifications !== undefined) body.sms_notifications = sms_notifications
        if (access_start_date) body.access_start_date = access_start_date
        if (access_end_date) body.access_end_date = access_end_date
        if (password_expiration_date) body.password_expiration_date = password_expiration_date

        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/update_user', body)
