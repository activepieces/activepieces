...
export const enrollAUserIntoAUserGroup = createAction({
    ...
    async run(context) {
        const { user_identifier, user_email, user_id, user_group_id, user_group_enrollment_expiration_date } =
            context.propsValue
        ...
        if (user_group_enrollment_expiration_date) {
            body.user_group_enrollment_expiration_date = user_group_enrollment_expiration_date
        }
