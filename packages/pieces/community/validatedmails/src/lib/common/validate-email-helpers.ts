...
import { ValidatedMailsValidationResponse, ValidateEmailProps } from './types'
...
export function toValidationOutput(body: Record<string, unknown>): ValidatedMailsValidationResponse {
    return {
        ...
        mx_hosts: Array.isArray(body['mx_hosts']) ? body['mx_hosts'].map((item) => String(item)) : [],
        status: asStatus(body['status']),
        reasons: Array.isArray(body['reasons']) ? body['reasons'].map((item) => String(item)) : [],
