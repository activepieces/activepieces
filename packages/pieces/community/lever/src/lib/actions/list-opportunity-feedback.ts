...
export const listOpportunityFeedback = createAction({
  ...
  description: 'Get all feedback for a given opportunity, optionally for a given template',
  auth: leverAuth,
  props: {
  ...
  async run({ auth, propsValue }) {
    ...
    if (propsValue.template) {
      return feedback.filter((form: { baseTemplateId: string }) => form.baseTemplateId === propsValue.template)
    }
    return feedback
  },
})
