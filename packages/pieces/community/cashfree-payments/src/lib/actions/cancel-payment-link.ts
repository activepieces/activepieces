...
export const cancelPaymentLink = createAction({
  ...
  description:
    'Cancel a payment link in Cashfree Payment Gateway. Only links in ACTIVE status can be cancelled.',
  auth: cashfreeAuth,
  props: {
    // Required Fields
    linkId: Property.ShortText({
      ...
      description:
        'The payment link ID that you want to cancel. Only ACTIVE links can be cancelled.',
      required: true,
    }),
  ...
  async run(context) {
    ...
    const { authType, environment, clientId, clientSecret, bearerToken } =
      context.auth as {
        authType: string;
        environment: string;
        clientId?: string;
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (response.status === 400) {
          return {
            ...
            error: response.body,
            message:
              'Bad request - Link may not be in ACTIVE status or invalid link ID',
          ...
          return {
            ...
            error: response.body,
            message:
              'Conflict - Link may already be cancelled or in a non-cancellable state',
            status: response.status,
          };
        } else {
          return {
