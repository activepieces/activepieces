...
export const getAllRefundsForOrder = createAction({
  ...
  description:
    'Fetch all refunds processed against an order in Cashfree Payment Gateway',
  requireAuth: true,
  props: {
    // Required Fields
  ...
  async run(context) {
    ...
    const { orderId, requestId, idempotencyKey } = context.propsValue;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const refunds = Array.isArray(refundsData)
          ? refundsData
          : refundsData?.refunds || [];
        ...
        const refundIds = refunds
          .map((refund: any) => refund.refund_id || refund.cf_refund_id)
          .filter(Boolean);
        const refundCharges = refunds.reduce((sum: number, refund: any) => {
          return (
            sum + (refund.refund_charge ? parseFloat(refund.refund_charge) : 0)
          );
        }, 0);
        ...
        if (response.status === 400) {
          ...
          return {
            ...
            error: response.body,
            message:
              "Forbidden - You do not have permission to access this order's refunds",
            status: response.status,
          };
        } else {
          return {
