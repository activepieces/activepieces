...
export const getOrdersForPaymentLink = createAction({
    ...
    async run(context) {
        ...
        const { environment, linkId, status, requestId, idempotencyKey } = context.propsValue
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                return {
                    ...
                    customerIds: [
                        ...new Set(orders.map((order: any) => order.customer_details?.customer_id).filter(Boolean)),
                    ],
