...
import { GoogleCalendarAuthValue, getAccessToken, googleCalendarCommon } from '.'
import {
    CalendarList,
    CalendarObject,
    GetColorsResponse,
...
export async function stopWatchEvent(body: GoogleWatchResponse, authProp: GoogleCalendarAuthValue) {
...
export async function watchEvent(
    ...
    const { body: webhook } = await httpClient.sendRequest<GoogleWatchResponse>(request)
    return webhook
}
...
export async function getColors(authProp: GoogleCalendarAuthValue): Promise<GetColorsResponse> {
    const request: HttpRequest = {
        method: HttpMethod.GET,
...
export async function getEvents(
    ...
    do {
        ...
        const { body: res } = await httpClient.sendRequest<GoogleCalendarEventList>(request)
...
export async function getEventsForDropdown(
    ...
    try {
        ...
        const request: HttpRequest = {
            method: HttpMethod.GET,
            url: `${googleCalendarCommon.baseUrl}/calendars/${encodeURIComponent(calendarId)}/events`,
            queryParams: queryParams,
            authentication: {
                type: AuthenticationType.BEARER_TOKEN,
                token: await getAccessToken(authProp),
        ...
        return response.body.items
            .map((event) => {
                ...
                const startDate = startTime ? new Date(startTime).toLocaleDateString() : ''
