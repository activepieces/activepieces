...
import { getAccessToken, googleCalendarAuth, googleCalendarCommon } from '../common'
        ...
        const newEvents = events.filter((event) => {
            ...
            if (search_fields && search_fields.length > 0) {
                ...
                return search_fields.some((field) => {
                    switch (field) {
                        case 'summary':
                            return event.summary?.toLowerCase().includes(searchTermLower) || false
                        case 'description':
                            return event.description?.toLowerCase().includes(searchTermLower) || false
                        case 'location':
                            return event.location?.toLowerCase().includes(searchTermLower) || false
                        case 'attendees':
                            return (
                                event.attendees?.some(
                                    (attendee) =>
                                        attendee.email?.toLowerCase().includes(searchTermLower) ||
                                        attendee.displayName?.toLowerCase().includes(searchTermLower),
                                ) || false
                            )
                        default:
                            return false
