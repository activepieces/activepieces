...
> = {
    ...
    items: async ({ auth, propsValue }) => {
        ...
        if (specific_event && event_id) {
            ...
            try {
                const eventResponse = await httpClient.sendRequest<GoogleCalendarEvent>(eventRequest)
                const event = eventResponse.body

                // Check if this specific event falls within our time window
                const eventStartTime = new Date(event.start?.dateTime ?? event.start?.date ?? 0).getTime()
