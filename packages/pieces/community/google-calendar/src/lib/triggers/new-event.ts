...
import { GoogleCalendarAuthValue, googleCalendarAuth, googleCalendarCommon } from '../common'
import { getLatestEvent, stopWatchEvent, watchEvent } from '../common/helper'
import { GoogleCalendarEvent, GoogleWatchResponse } from '../common/types'
...
export const newEvent = createTrigger({
    ...
    async onEnable(context) {
        ...
        await context.store.put<GoogleWatchResponse>('google_calendar_watch', response)
    },

    async onDisable(context) {
        const auth = context.auth as GoogleCalendarAuthValue
        const watch = await context.store.get<GoogleWatchResponse>('google_calendar_watch')
    ...
    async run(context) {
        ...
        if (headers['x-goog-resource-state'] === 'add') {
            ...
            if (search_filter && search_filter.trim()) {
                ...
                const matchesSearch =
                    summary.includes(searchTerm) || description.includes(searchTerm) || location.includes(searchTerm)
    ...
    async test(context) {
        ...
        const latestEvent = await getLatestEvent(context.propsValue.calendar_id!, auth)
