...
import { createSubscription, listPayers, listPlans } from '../common/client'
...
export const addSubscriptionAction = createAction({
    ...
    props: {
        ...
        sourceId: Property.ShortText({
            ...
            description:
                'Optional: Specific payment source ID to use. If omitted, a source will be automatically selected',
            required: false,
        }),
    ...
    async run(context) {
        const { planId, payerId, totalAmount, startDate, sourceId, surcharge } = context.propsValue
