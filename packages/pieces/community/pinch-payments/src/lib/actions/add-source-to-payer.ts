...
import { getPinchPaymentsToken, pinchPaymentsAuth } from '../common/auth'
...
export const addSourceToPayerAction = createAction({
    ...
    async run(context) {
        const { payerId, sourceType, useToken, token, ipAddress, bankAccountNumber, bankAccountBsb, bankAccountName } =
            context.propsValue
        ...
        if (useToken) {
            ...
            if (sourceType === 'bank-account') {
                if (!bankAccountNumber || !bankAccountBsb || !bankAccountName) {
                    throw new Error(
                        'Bank account number, BSB, and account name are required for manual bank account entry',
                    )
                }
