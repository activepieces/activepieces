...
export async function createOrUpdatePayer(credentials: PinchPaymentsAuthCredentials, payer: PinchPayer) {
    return pinchPaymentsClient(credentials, HttpMethod.POST, '/payers', payer)
}

export async function listPayers(credentials: PinchPaymentsAuthCredentials, params?: ListPayersParams) {
...
export async function listPlans(credentials: PinchPaymentsAuthCredentials, params?: ListPlansParams) {
    const queryParams: Record<string, string> = {}
    if (params?.page) queryParams['page'] = params.page.toString()
    if (params?.pageSize) queryParams['pageSize'] = params.pageSize.toString()
...
export async function deleteWebhook(credentials: PinchPaymentsAuthCredentials, webhookId: string): Promise<void> {
