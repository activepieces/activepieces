...
export const updateLead = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.mailingAddress) payload.mailing_address = propsValue.mailingAddress
        if (propsValue.mailingCity) payload.mailing_city = propsValue.mailingCity
        if (propsValue.mailingState) payload.mailing_state = propsValue.mailingState
        if (propsValue.mailingZipcode) payload.mailing_zipcode = propsValue.mailingZipcode
        if (propsValue.leadType) payload.lead_type = propsValue.leadType
        if (propsValue.leadOwnershipLevel) payload.lead_ownership_level = propsValue.leadOwnershipLevel
        ...
        if (propsValue.subscriptionPropertyAlert !== undefined)
            payload.subscription_property_alert = propsValue.subscriptionPropertyAlert
        if (propsValue.subscriptionMarketReport !== undefined)
            payload.subscription_market_report = propsValue.subscriptionMarketReport
        if (propsValue.minPriceBuyer !== undefined) payload.min_price_buyer = propsValue.minPriceBuyer
        if (propsValue.maxPriceBuyer !== undefined) payload.max_price_buyer = propsValue.maxPriceBuyer
        if (propsValue.locationDeprecated) payload.location_deprecated = propsValue.locationDeprecated
        ...
        if (propsValue.buyerZipcode) payload.buyer_zipcode = propsValue.buyerZipcode
        if (propsValue.buyerAddress) payload.buyer_address = propsValue.buyerAddress
        if (propsValue.minBedsBuyer !== undefined) payload.min_beds_buyer = propsValue.minBedsBuyer
        if (propsValue.minBathsBuyer !== undefined) payload.min_baths_buyer = propsValue.minBathsBuyer
        if (propsValue.bedsSeller !== undefined) payload.beds_seller = propsValue.bedsSeller
        if (propsValue.bathsSeller !== undefined) payload.baths_seller = propsValue.bathsSeller
        if (propsValue.priceSeller !== undefined) payload.price_seller = propsValue.priceSeller
        if (propsValue.addressSeller) payload.address_seller = propsValue.addressSeller
        if (propsValue.sqftSeller !== undefined) payload.sqft_seller = propsValue.sqftSeller
        ...
        if (propsValue.zipcodeSeller) payload.zipcode_seller = propsValue.zipcodeSeller
        if (propsValue.sendWelcomeEmail !== undefined) payload.send_welcome_email = propsValue.sendWelcomeEmail
        if (propsValue.callOptIn !== undefined) payload.call_opt_in = propsValue.callOptIn
        if (propsValue.textOptIn !== undefined) payload.text_opt_in = propsValue.textOptIn
        if (propsValue.emailOptIn !== undefined) payload.email_opt_in = propsValue.emailOptIn
        if (propsValue.numberConsent !== undefined) payload.number_consent = propsValue.numberConsent

        const response = await makeRequest(auth.secret_text, HttpMethod.PUT, `/leads/${leadId}`, payload)
