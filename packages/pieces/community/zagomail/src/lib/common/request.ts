...
async function fireHttpRequest({ method, path, body }: { method: HttpMethod; path: string; body?: unknown }) {
...
export const zagoMailApiService = {
    createSubscriber: async (publicKey: string, listUid: string, body: any) => {
    ...
    updateSubscriber: async (publicKey: string, listUid: string, subscriberUid: string, body: any) => {
        const response = await fireHttpRequest({
            method: HttpMethod.POST,
    ...
    unsubscribeSubscriber: async (publicKey: string, listUid: string, subscriberUid: string) => {
        const response = await fireHttpRequest({
            method: HttpMethod.POST,
    ...
    getSubscriberDetails: async (publicKey: string, listUid: string, subscriberUid: string) => {
        const response = await fireHttpRequest({
    ...
    getCampaignDetails: async (publicKey: string, campaignUid: string) => {
        const response = await fireHttpRequest({
            method: HttpMethod.GET,
    ...
    addTagToSubscriber: async (
        publicKey: string,
        { listUid, tagId, subscriberUid }: { listUid: string; subscriberUid: string; tagId: string },
    ) => {
        const response = await fireHttpRequest({
            method: HttpMethod.POST,
