...
export async function mongodbConnect(
    ...
    try {
        ...
        let connectionString = `${protocol}${auth.username}:${encodeURIComponent(auth.password)}@${auth.host}`
...
export async function getDatabase(client: MongoClient, databaseName?: string): Promise<Db> {
    return client.db(databaseName)
}

export async function getCollections(client: MongoClient, databaseName: string): Promise<string[]> {
    try {
...
export const mongodbCommon = {
    database: Property.ShortText({
        ...
        description: 'The MongoDB database to connect to (from your authentication)',
        required: false,
    }),
