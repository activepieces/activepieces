...
export const getMndePrice = createAction({
  name: 'get_mnde_price',
  displayName: 'Get MNDE Price',
  description: 'Fetch the current price and market data for MNDE (Marinade governance token) from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<Record<string, unknown>>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/marinade',
      queryParams: {
        localization: 'false',
        tickers: 'false',
    ...
    return {
      ...
      atl: (marketData?.['atl'] as Record<string, number>)?.['usd'],
