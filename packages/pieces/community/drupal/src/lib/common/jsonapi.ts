 ...
 */
export function getJsonApiUrl(baseUrl: string, entityType: string, bundle: string, uuid?: string): string {
...
export function fromJsonApiFormat(response: JsonApiResponse): any | any[] {
    ...
    try {
        if (Array.isArray(response.data)) {
            return response.data.filter((resource) => resource != null).map(convertJsonApiResource)
        } else {
...
function buildQueryParams(options: {
    ...
    if (options.sort) {
        const sortParam = options.sortDirection === 'ASC' ? options.sort : `-${options.sort}`
