...
export const drupalListEntitiesAction = createAction({
                ...
                if (entityInfo.entity_type === 'node') {
                    return {
                        minimal_output: Property.Checkbox({
                            ...
                            description:
                                'Return only basic fields (UUID, title, status, dates) instead of all entity data',
                            required: false,
                            defaultValue: true,
                        }),
                    } as any
    ...
    async run({ auth, propsValue }) {
        ...
        let entities = await drupal.listEntities(auth, entityInfo.entity_type, entityInfo.bundle, {
            filters,
            sort: sortField,
        ...
        entities = Array.isArray(entities) ? entities.map(removeType) : removeType(entities)
