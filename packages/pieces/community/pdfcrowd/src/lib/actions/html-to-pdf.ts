...
export const htmlToPdfAction = createAction({
    ...
    props: {
        ...
        data_options: Property.LongText({
            ...
            description:
                'Set the advanced data options as comma separated key=value pairs: csv_delimiter - The CSV data delimiter, the default is ,. xml_remove_root - Remove the root XML element from the input data. data_root - The name of the root element inserted into the input data without a root node (e.g. CSV), the default is data.',
            required: false,
        }),
        debug_log: Property.Checkbox({
            displayName: 'Debug Log',
            description:
                'Turn on debug logging to troubleshoot conversion issues. Details about the conversion process, including resource loading, rendering steps, and error messages are stored in the debug log. Use this when conversions fail or produce unexpected results.',
            required: false,
        }),
    ...
    async run(context) {
        ...
        const fileOptions = ['page_watermark', 'multipage_watermark', 'page_background', 'multipage_background']
