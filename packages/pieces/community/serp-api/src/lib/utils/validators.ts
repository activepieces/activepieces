    ...
    static validateGoogleDomain(googleDomain?: string): ValidationResult {
        ...
        if (googleDomain !== undefined) {
            if (typeof googleDomain !== 'string') {
                ...
                errors.push(
                    `Google domain is too long (maximum ${VALIDATION_CONSTANTS.MAX_GOOGLE_DOMAIN_LENGTH} characters)`,
                )
    ...
    static validateUploadDate(uploadDate?: string): ValidationResult {
        ...
        if (uploadDate !== undefined) {
            if (typeof uploadDate !== 'string') {
                ...
                !VALIDATION_CONSTANTS.VALID_UPLOAD_DATE.includes(
                    uploadDate as 'any' | 'hour' | 'today' | 'week' | 'month' | 'year',
                )
            ) {
                errors.push(
                    `Upload date parameter must be one of: ${VALIDATION_CONSTANTS.VALID_UPLOAD_DATE.join(', ')}`,
                )
            }
        }
