...
import { GoogleSearchConfig, RequestOptions, SerpApiConfig, SerpApiEngine, SerpApiResponse } from '../types'
...
export class SerpApiClient {
    ...
    async executeSearch<T = any>(config: SerpApiConfig, options: RequestOptions = {}): Promise<SerpApiResponse<T>> {
    ...
    private async makeHttpRequest<T>(
        ...
        try {
            ...
            if (response.body?.error) {
                throw createErrorFromResponse(response.status || 400, response.body, response.body.error)
            }
    ...
    private handleHttpError(error: Error): Error {
        ...
        if ((error as any).status) {
            return createErrorFromResponse((error as any).status, (error as any).body, error.message)
        }
