...
import { getNotebooksDropdown, getPagesBySectionDropdown, getSectionsByNotebookDropdown } from '../common'
...
export const appendNote = createAction({
    ...
    async run(context) {
        ...
        try {
            await client.api(`/me/onenote/pages/${page_id}/content`).patch(updateCommand)
