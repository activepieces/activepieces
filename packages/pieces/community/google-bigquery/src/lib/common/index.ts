        ...
        fields.forEach((field, i) => {
            ...
            if (value === null || value === undefined) {
                ...
                obj[field.name] = (value as Array<{ v: unknown }>)
                    .map((cell) => (typeof cell.v === 'object' ? JSON.stringify(cell.v) : String(cell.v ?? '')))
                    .join(', ')
...
export async function waitForJobResults(
    ...
    throw new Error('Query did not complete within 10 minutes. Try simplifying your query or using a smaller dataset.')
}
