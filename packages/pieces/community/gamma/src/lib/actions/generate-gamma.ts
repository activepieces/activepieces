...
export const generateGamma = createAction({
    ...
    props: {
        inputText: Property.LongText({
            ...
            description: 'Text used to generate your gamma (1-750,000 characters).',
            required: true,
        }),
        ...
        themeName: Property.ShortText({
            ...
            description: 'The theme from Gamma to be used. Defaults to workspace default.',
            required: false,
        }),
        numCards: Property.Number({
            displayName: 'Number of Cards',
            description: 'How many cards to create (default 10). Pro: 1-50, Ultra: 1-75.',
            required: false,
        }),
        ...
        additionalInstructions: Property.LongText({
            ...
            description: 'Extra specifications about the desired content and layouts (1-500 characters).',
            required: false,
        }),
        ...
        textOptions: Property.Json({
            ...
            description: 'JSON object for text attributes (e.g., amount, tone, audience, language).',
            required: false,
            defaultValue: {},
        }),
        imageOptions: Property.Json({
            displayName: 'Image Options',
            description: 'JSON object for image attributes (e.g., source, model, style).',
            required: false,
            defaultValue: {},
        }),
        ...
        sharingOptions: Property.Json({
            ...
            description: 'JSON object for sharing attributes (e.g., workspaceAccess, externalAccess).',
            required: false,
            defaultValue: {},
        }),
    },
    async run(context) {
        ...
        if (textOptions && Object.keys(textOptions).length > 0) body['textOptions'] = textOptions
        if (imageOptions && Object.keys(imageOptions).length > 0) body['imageOptions'] = imageOptions
        if (cardOptions && Object.keys(cardOptions).length > 0) body['cardOptions'] = cardOptions
        if (sharingOptions && Object.keys(sharingOptions).length > 0) body['sharingOptions'] = sharingOptions
