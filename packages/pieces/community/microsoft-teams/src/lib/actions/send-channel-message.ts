...
export const sendChannelMessageAction = createAction({
    ...
    async run(context) {
        ...
        const mentionHandles = Array.from(content.matchAll(new RegExp(mentionPattern.source, 'g')), (match) => match[2])
        ...
        if (mentionHandles.length > 0) {
            ...
            for (const member of members) {
                ...
                const keys = [typedMember.displayName, typedMember.userPrincipalName, typedMember.email].filter(
            ...
            messageContent = content.replace(mentionPattern, (match, prefix: string, handle: string) => {
                ...
                const displayName = typedMember.displayName || typedMember.userPrincipalName || typedMember.userId
