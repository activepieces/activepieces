...
export const findTeamMemberAction = createAction({
    ...
    async run(context) {
        ...
        const filter =
            searchBy == 'email'
                ? `microsoft.graph.aadUserConversationMember/email eq '${searchValue}'`
                : `microsoft.graph.aadUserConversationMember/displayName eq  '${searchValue}'`

        const response: PageCollection = await client.api(`/teams/${teamId}/members`).filter(filter).get()
