...
const parseRetryAfterMs = (error: any): number | null => {
    const headers: Record<string, string> | undefined = error?.headers ?? error?.response?.headers ?? undefined
...
const extractStatusCode = (error: any): number => {
    return error?.statusCode ?? error?.status ?? error?.response?.status ?? error?.response?.statusCode ?? 0
}
...
export const withGraphRetry = async <T>(requestFn: () => Promise<T>, options: GraphRetryOptions = {}): Promise<T> => {
