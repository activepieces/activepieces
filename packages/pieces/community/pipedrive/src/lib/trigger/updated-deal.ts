...
export const updatedDeal = createTrigger({
    ...
    async run(context) {
        ...
        const filterByValue = context.propsValue.filter_by_field_value!['field_value']
        const fieldToWatch =
            context.propsValue.field_to_watch === 'label' ? 'label_ids' : context.propsValue.field_to_watch
        ...
        if (
            ...
            const updatedDealProperties = pipedriveTransformCustomFields(customFieldsResponse, dealResponse.data)
    ...
    for (const [key, value] of Object.entries(custom_fields as Record<string, any>)) {
        ...
        switch (type) {
            ...
            case 'date':
                ...
                break
            case 'set':
                flatCustomFields[key] = value?.values?.length ? value.values.map((v: any) => v.id).join(',') : ''
                break
            case 'enum':
            case 'user':
            case 'org':
