...
import { GetField, PaginatedV1Response, PaginatedV2Response, RequestParams } from './types'
...
export function pipedriveParseCustomFields(
    ...
    const fieldTypeMap: Record<string, GetField> = customFieldsDefinitions.reduce(
        (acc, field) => {
            acc[field.key] = field
            return acc
        },
    ...
    for (const field of customFieldsDefinitions) {
        ...
        if (oldKey in rawCustomFields) {
            ...
            if (isNil(value)) {
                ...
                updatedResponseData[newKey] = field.options?.find((option) => option.id === value)?.label || null
        ...
        if (oldKey in responseData) {
            if (responseData[oldKey] === null || responseData[oldKey] === undefined) {
                ...
                updatedResponseData[newKey] =
                    field.options?.find((option) => option.id.toString() === responseData[oldKey])?.label || null
            } else if (fieldType === 'set') {
