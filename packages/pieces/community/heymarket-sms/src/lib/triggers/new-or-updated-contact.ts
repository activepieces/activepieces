...
const polling: Polling<AppConnectionValueForAuthProperty<typeof heymarketSmsAuth>, Record<string, never>> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ propsValue, auth, lastFetchEpochMS }) => {
        const apiKey = auth.secret_text

        // Convert lastFetchEpochMS to RFC 3339 format for Heymarket API
        const lastFetchDate = lastFetchEpochMS ? dayjs(lastFetchEpochMS).toISOString() : undefined
        ...
        const response = await makeRequest(apiKey, HttpMethod.POST, '/v1/contacts', requestBody)
