...
export const sendTemplateMessage = createAction({
    ...
    async run(context) {
        const { inbox_id, creator_id, template_id, phone_number, targets, text } = context.propsValue
        ...
        try {
            const response = await makeRequest(apiKey, HttpMethod.POST, '/v1/message/send', body)
