...
export const visitIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const response = (await makeRequest(api_key, HttpMethod.GET, '/visits')) as any
        return {
            disabled: false,
...
export const customerIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const api_key = auth.secret_text
        const response = (await makeRequest(api_key, HttpMethod.GET, '/customers')) as any
        return {
            disabled: false,
...
export const locationIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const api_key = auth.secret_text
        const response = (await makeRequest(api_key, HttpMethod.GET, '/locations')) as any
        return {
            disabled: false,
