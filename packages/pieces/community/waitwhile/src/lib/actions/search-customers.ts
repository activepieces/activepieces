...
export const searchCustomers = createAction({
    ...
    props: {
        q: Property.ShortText({
            ...
            description: 'Search query for name, phone, email, or customer identifier (prefix match)',
            required: false,
        }),
    ...
    async run(context) {
        const { q, locationId, state, tag, limit, page, fromDate, toDate } = context.propsValue
