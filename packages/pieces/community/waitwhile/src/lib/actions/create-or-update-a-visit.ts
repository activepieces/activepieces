...
import { customerIdDropdown, locationIdDropdown, visitIdDropdown } from '../common/props'
...
export const createOrUpdateAVisit = createAction({
    ...
    async run(context) {
        const { id, customerId, locationId, notes, partySize, startDate } = context.propsValue
        ...
        const response = await makeRequest(api_key, HttpMethod.POST, '/visits', body)
        return response
    },
})
