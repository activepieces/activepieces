...
export const getTfuelPrice = createAction({
  name: 'get_tfuel_price',
  displayName: 'Get TFUEL Price',
  description:
    'Fetch the current price and market data for TFUEL (gas/utility token) from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<CoinGeckoResponse>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/theta-fuel',
      queryParams: {
        localization: 'false',
        tickers: 'false',
