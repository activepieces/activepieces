...
const polling: Polling<AppConnectionValueForAuthProperty<typeof guideliteAuth>, StaticPropsValue<typeof props>> = {
    ...
    items: async ({ propsValue, auth, lastFetchEpochMS }) => {
        ...
        const leads = await makeRequest(auth, HttpMethod.POST, '/manage-assistant/lead', {
            assistantId,
            from: fromDate,
            to: toDate,
            country: 'all',
...
export const newLeadSubmission = createTrigger({
    ...
    description: 'Trigger when a new lead is captured through the assistant conversation',
    props,
    sampleData: {
