...
const mapActorToDropdownOption = (item: Item): DropdownOption => {
    const optionName = item.title ? `${item.title} (${item.username}/${item.name})` : `${item.username}/${item.name}`
...
export const listActors = async (apiKey: string, actorSource: string): Promise<DropdownOption[]> => {
    ...
    try {
        ...
        return [{ label: `Failed to load actors: ${error.message || error}`, value: '' }]
    }
}
...
export const listTasks = async (apiKey: string): Promise<DropdownOption[]> => {
    ...
    try {
        ...
        return [{ label: `Failed to load tasks: ${error.message || error}`, value: '' }]
    }
}
...
export const createBuildProperty = () =>
    Property.ShortText({
        displayName: 'Build',
        description:
            'Build to run. It can be either a build tag or build number. By default, the run uses the build specified in the default run configuration (typically `latest`).',
        required: false,
    })

export const createMemoryProperty = (runType: RunType) =>
...
export const createTimeoutProperty = (runType: RunType) =>
    Property.Number({
...
export const createWaitForFinishProperty = (runType: RunType) =>
    Property.Checkbox({
...
export const createActorSourceProperty = () =>
    Property.StaticDropdown({
        required: true,
...
export const createActorIdProperty = () =>
    Property.Dropdown({
        displayName: 'Actor',
        auth: apifyAuth,
...
export const createTaskIdProperty = () =>
    Property.Dropdown({
...
const createInputBodyProperty = (runType: RunType, defaultValue?: object) =>
    Property.Json({
...
export const createActorInputProperty = () =>
    Property.DynamicProperties({
        ...
        props: async (propsValue) => {
            ...
            return {
                body: createInputBodyProperty(RunType.ACTOR, defaultInputs),
            }
        },
    })

export const createTaskInputProperty = () =>
    Property.DynamicProperties({
        displayName: 'Input',
        auth: apifyAuth,
        required: true,
        ...
        props: async () => {
            return {
                body: createInputBodyProperty(RunType.TASK, {}),
            }
        },
    })
...
export const deleteWebhook = async (client: ApifyClient, webhookId: string): Promise<void> => {
    try {
...
export const generateIdempotencyKey = (id: string, eventTypes: WebhookEventType[], webhookUrl: string): string => {
...
export const createStatusesProperty = () =>
