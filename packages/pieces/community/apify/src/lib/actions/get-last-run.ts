...
export const getLastRun = createAction({
  ...
  props: {
    actorid: Property.ShortText({
        ...
        description: 'The id of the Actor to get run information [item of interest:defaultDatasetId] (compulsory)',
        required: true,
      }),
  },
  async run(context) {
    ...
    const url = 'https://api.apify.com/v2/acts/' + context.propsValue.actorid + '/runs/last?status=SUCCEEDED'
