...
export const expireShortLinkAction = createAction({
    ...
    async run({ propsValue, auth }) {
        const { linkId, domain: domainString, expiresAt, clicksLimit, expiredURL } = propsValue
        ...
        try {
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request parameters. Please check your expiration settings and try again.')
            }
