...
export const listLinksAction = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            const linkCount =
                (response as any)['count'] || ((response as any)['links'] ? (response as any)['links'].length : 0)
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request parameters. Please check your filter values and try again.')
            }
            ...
            if (error.message.includes('404')) {
                throw new Error('Domain not found. Please verify the domain exists and you have access to it.')
            }
