...
export const getLinkByPathAction = createAction({
    ...
    props: {
        ...
        path: Property.ShortText({
            ...
            description:
                'The path/slug of the short link (e.g., "abc123", "my-link"). Do not include domain or slashes.',
            required: true,
        }),
    },
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request parameters. Please check the domain and path values.')
            }
