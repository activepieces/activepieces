    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request. Please check the link ID format and try again.')
            }
            ...
            if (error.message.includes('404')) {
                throw new Error(`Short link with ID ${linkId} not found. It may have already been deleted.`)
            }
