    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request parameters. Please check your link IDs and date range.')
            }
            ...
            if (error.message.includes('404')) {
                throw new Error('Domain or links not found. Please verify the domain and link IDs exist.')
            }
