    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (error.message.includes('400')) {
                throw new Error('Invalid request parameters. Please check the country code format and URL validity.')
            }
            ...
            if (error.message.includes('409')) {
                throw new Error(`A country targeting rule for ${countryCode} already exists for this link.`)
            }
