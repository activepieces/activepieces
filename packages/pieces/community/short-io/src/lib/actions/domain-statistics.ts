    ...
    async run({ propsValue, auth }) {
        ...
        if (tz && tz.trim() !== '') {
            ...
            if (tzValue.length < 3 || tzValue.includes(' ')) {
                throw new Error(
                    'Invalid timezone format. Use standard timezone names like UTC, America/New_York, Europe/London.',
                )
            }
        }
        ...
        try {
            ...
            const periodLabel =
                period === 'custom' ? 'custom period' : period.replace(/([a-z])([A-Z])/g, '$1 $2').toLowerCase()
