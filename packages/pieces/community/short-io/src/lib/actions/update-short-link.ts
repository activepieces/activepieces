...
import { domainIdDropdown, folderIdDropdown, linkIdDropdown } from '../common/props'
...
export const updateShortLinkAction = createAction({
    ...
    props: {
        ...
        redirectType: Property.StaticDropdown({
            ...
            description: 'The HTTP status code for the redirect. The default is 302 (Found).',
            required: false,
            options: {
                disabled: false,
                options: [
    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (error.message.includes('404')) {
                throw new Error('Short link not found. Please check the link ID and try again.')
            }
