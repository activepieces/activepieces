...
export const deleteRows = createAction({
    ...
    async run(context) {
        const { table_name, filter_type, filter_column, filter_value, filter_values, count_deleted, return_deleted } =
            context.propsValue
        const { url, apiKey } = context.auth.props

        const supabase = createClient(url, apiKey)

        let deleteQuery = supabase.from(table_name as string).delete({
        ...
        switch (filter_type) {
            ...
            case 'ilike':
                if (!filter_value)
                    throw new Error('Filter value is required for case-insensitive like pattern matching')
        ...
        const { data, error, count } = return_deleted ? await deleteQuery.select() : await deleteQuery
