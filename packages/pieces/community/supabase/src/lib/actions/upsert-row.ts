...
export const upsertRow = createAction({
    ...
    async run(context) {
        const { table_name, row_data, on_conflict, count_upserted, return_upserted } = context.propsValue
        ...
        const upsertQuery = supabase.from(table_name as string).upsert(row_data, upsertOptions)

        const { data, error, count } = return_upserted ? await upsertQuery.select() : await upsertQuery
