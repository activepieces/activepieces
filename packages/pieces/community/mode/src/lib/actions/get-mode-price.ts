...
export const getModePriceAction = createAction({
  name: 'get_mode_price',
  displayName: 'Get MODE Price',
  description: 'Fetch the current price and market data for the MODE token from CoinGecko.',
  props: {
  ...
  async run(context) {
    ...
    const response = await httpClient.sendRequest({
      ...
      url: 'https://api.coingecko.com/api/v3/coins/mode',
      queryParams: {
        localization: 'false',
        tickers: 'false',
