    ...
    account_id: () =>
        Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const accounts = await whatConvertsClient.getAccounts(auth as WhatConvertsAuth)
                return {
                    disabled: false,
    ...
    profile_id: (required = true) =>
        Property.Dropdown({
            ...
            options: async ({ auth, account_id }) => {
                ...
                const profiles = await whatConvertsClient.getProfiles(auth as WhatConvertsAuth, account_id as number)
                return {
                    disabled: false,
    ...
    lead_id: () =>
        Property.Dropdown({
            ...
            options: async ({ auth, profile_id }) => {
                ...
                const response = await whatConvertsClient.findLeads(auth as WhatConvertsAuth, {
                ...
                return {
                    ...
                    options: (response.leads || []).map((lead) => {
                        const emailDetail = lead.lead_details?.find((d) => d.label === 'Email')
                        const nameDetail = lead.lead_details?.find((d) => d.label === 'First Name')
                        const label = `${
