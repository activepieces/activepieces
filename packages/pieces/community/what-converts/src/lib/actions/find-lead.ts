...
export const findLeadAction = createAction({
    ...
    async run(context) {
        ...
        const searchCriteria = [propsValue.email_address, propsValue.phone_number, propsValue.lead_type]

        if (searchCriteria.every((criterion) => !criterion)) {
            throw new Error('Please provide at least one search criteria (Email, Phone, or Lead Type).')
        }
