...
export const newFormResponse = createTrigger({
	...
	description: 'Fires when a new submission is received for a selected Fillout form.',
	props: {
	...
	async onEnable(context) {
		...
		const response = (await makeRequest(apiKey, HttpMethod.POST, '/webhook/create', undefined, {
			formId,
	...
	async onDisable(context) {
		...
		if (!isNil(webhookId)) {
			await makeRequest(apiKey, HttpMethod.POST, '/webhook/delete', undefined, { webhookId });
		}
	},
	async test(context) {
		const { formId } = context.propsValue;
		...
		const response = await makeRequest(apiKey, HttpMethod.GET, `/forms/${formId}/submissions`, {
			limit: '10',
			sort: 'desc',
		});
