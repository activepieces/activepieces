...
export const getFormResponses = createAction({
    ...
    props: {
        ...
        status: Property.StaticDropdown({
            ...
            required: false,
            options: {
                options: [{ label: 'In Progress (unfinished)', value: 'in_progress' }],
            },
            description:
                'By default, only finished submissions are returned. Select "In Progress" to get unfinished submissions instead.',
        }),
    ...
    async run(context) {
        ...
        const response = await makeRequest(apiKey, HttpMethod.GET, `/forms/${formId}/submissions`, queryParams)
