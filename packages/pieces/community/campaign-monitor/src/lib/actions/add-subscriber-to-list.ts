...
export const addSubscriberToListAction = createAction({
    ...
    async run({ propsValue, auth }) {
        const { listId, email, name, phone, consentToSendSms, consentToTrack, resubscribe, fields } = propsValue
        ...
        const payload = {
            ...
            CustomFields: Object.entries(fields).flatMap(([key, value]) =>
                Array.isArray(value) ? value.map((v) => ({ Key: key, Value: v })) : [{ Key: key, Value: value }],
            ),
        }
