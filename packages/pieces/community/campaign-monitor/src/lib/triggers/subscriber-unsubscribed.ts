...
export const subscriberUnsubscribedTrigger = createTrigger({
    ...
    async onDisable(context) {
        const storedData = await context.store.get<string>('subscriber_unsubscribed_webhook')
        ...
        for (const event of payload.Events) {
            if (event.Type === 'Deactivate' && event.State === 'Unsubscribed') {
                const response = await makeRequest(
                    { apiKey: context.auth.secret_text },
                    HttpMethod.GET,
                    `/subscribers/${context.propsValue.listId}.json?email=${encodeURIComponent(event.EmailAddress)}`,
                    payload,
                )
                result.push({
                    ...response.body,
