...
export const newClientTrigger = createTrigger({
    ...
    async onEnable(context) {
        ...
        const response = await makeRequest({ apiKey: context.auth.secret_text }, HttpMethod.GET, '/clients.json')
    ...
    async run(context) {
        ...
        // Find new clients
        const newClients = allClients.filter((client) => !existingClientIds.includes(client.ClientID))

        // Update stored client list
        const updatedClientIds = Array.from(new Set([...existingClientIds, ...allClients.map((c) => c.ClientID)]))
        await context.store.put('existing_clients', {
