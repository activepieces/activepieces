...
export const documentUpdated = createTrigger({
  ...
  description:
    'Triggers when a document is updated (content, recipients, or settings changed)',
  auth: pandadocAuth,
  props: {
    ...
    update_type_filter: Property.StaticMultiSelectDropdown({
      ...
      description:
        'Only trigger for specific types of updates (leave empty for all)',
      required: false,
      options: {
        options: [
        ...
        await httpClient.sendRequest({
          ...
          headers: {
            Authorization: `API-Key ${
              (context.auth as PandaDocAuthType).apiKey
            }`,
          },
        });
      } catch (error) {
  ...
  async run(context) {
    ...
    if (payload.event_type === 'document_updated') {
      ...
      if (
        context.propsValue.update_type_filter &&
        context.propsValue.update_type_filter.length > 0
      ) {
        const updatedFields = documentData?.updated_fields || [];
        const hasMatchingUpdate = context.propsValue.update_type_filter.some(
          (filterType: string) => {
            switch (filterType) {
              case 'content':
                return (
                  updatedFields.includes('content') ||
                  documentData?.content_date_modified
                );
              case 'recipients':
