...
export const documentStateChanged = createTrigger({
  ...
  description:
    'Triggers when a document status changes (draft, sent, completed, etc.)',
  auth: pandadocAuth,
  props: {
    filter_status: Property.StaticMultiSelectDropdown({
      displayName: 'Filter by Status',
      description:
        'Only trigger for specific status changes (leave empty for all)',
      required: false,
      options: {
        options: [
  ...
  async run(context) {
    ...
    if (payload.event_type === 'document_state_changed') {
      ...
      if (
        context.propsValue.filter_status &&
        context.propsValue.filter_status.length > 0
      ) {
