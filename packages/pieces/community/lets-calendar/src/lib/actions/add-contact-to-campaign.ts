...
export const addContactToCampaign = createAction({
    ...
    async run(context) {
        const accessToken = await getAccessToken(context.auth.props.client_key, context.auth.props.secret_key)
        ...
        const response = await makeRequest(accessToken, HttpMethod.POST, '/add-single-contact', body)
