...
export async function getAccessToken(client_key: string, secret_key: string): Promise<string> {
    try {
...
export async function makeRequest(accessToken: string, method: HttpMethod, path: string, body?: unknown) {
    try {
        const response = await httpClient.sendRequest({
            method,
            url: `${BASE_URL}${path}`,
