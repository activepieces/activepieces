...
const polling: Polling<AppConnectionValueForAuthProperty<typeof letsCalendarAuth>, Record<string, never>> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ auth }) => {
        try {
            const accessToken = await getAccessToken(auth.props.client_key, auth.props.secret_key)

            const response = (await makeRequest(accessToken, HttpMethod.GET, '/campaigns?page=1')) as CampaignsResponse
