        ...
        referenceImages: Property.Array({
            ...
            properties: {
                ...
                tag: Property.ShortText({
                    displayName: 'Tag',
                    description:
                        'Name to reference this image in your prompt using @tag (3-16 characters, alphanumeric + underscore)',
                    required: false,
                }),
            },
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        await propsValidation.validateZod(propsValue, {
            promptText: z
                ...
                .max(1000, 'Prompt text must be 1000 characters or fewer'),
            seed: z
                .number()
                .min(0, 'Seed must be at least 0')
                .max(4294967295, 'Seed must be at most 4294967295')
                .optional(),
            referenceImages: z
                .array(
                    z.object({
                        ...
                        tag: z
                            ...
                            .regex(
                                /^[a-zA-Z][a-zA-Z0-9_]*$/,
                                'Tag must start with a letter and contain only letters, numbers, and underscores',
                            )
                            .optional(),
                    }),
                )
                .max(3, 'Maximum of 3 reference images allowed')
                .optional(),
        })

        // Special validation for gen4_image_turbo
        if (
            propsValue.model === 'gen4_image_turbo' &&
            (!propsValue.referenceImages || propsValue.referenceImages.length === 0)
        ) {
