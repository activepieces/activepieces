...
const getStatusDescription = (status: string): string => {
    switch (status) {
        ...
        case 'THROTTLED':
            return 'Task is waiting for other jobs to complete'
        case 'RUNNING':
        ...
        case 'CANCELLED':
            return 'Task was cancelled or aborted'
        default:
    ...
    async run({ auth, propsValue, files }) {
        ...
        if (propsValue.downloadOutput && outputs.length > 0) {
            if (task.status !== 'SUCCEEDED') {
                console.warn(
                    `Task status is ${task.status}, but download was requested. Only succeeded tasks have downloadable outputs.`,
                )
            } else {
