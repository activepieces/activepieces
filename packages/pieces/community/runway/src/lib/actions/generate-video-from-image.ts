            ...
            props: async ({ model }) => {
                ...
                if ((model as unknown as string) === 'gen4_turbo') {
                    ...
                    ratioOptions['ratio'] = Property.StaticDropdown({
                        ...
                        required: true,
                        options: {
                            options: [{ label: 'Select a model first', value: '' }],
                        },
                    })
                }
            ...
            props: async ({ model }) => {
                ...
                if ((model as unknown as string) === 'veo3') {
                    durationOptions['duration'] = Property.StaticDropdown({
                        ...
                        defaultValue: 8,
                        options: {
                            options: [{ label: '8 seconds (required for veo3)', value: 8 }],
                        },
                    })
                } else if (
                    (model as unknown as string) === 'gen4_turbo' ||
                    (model as unknown as string) === 'gen3a_turbo'
                ) {
                    durationOptions['duration'] = Property.StaticDropdown({
                        displayName: 'Duration (seconds)',
                        required: true,
