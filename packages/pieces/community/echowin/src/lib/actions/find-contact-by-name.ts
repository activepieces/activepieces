...
export const findContactByName = createAction({
    ...
    description: 'Get a paginated list of contacts and search by name, email, or phone number',
    props: {
    ...
    async run(context) {
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.GET, `/contacts?${queryString}`)
