...
export const newDealEventTrigger = createTrigger({
  ...
  props: {
    ...
    eventSubTypes: Property.StaticMultiSelectDropdown({
      displayName: 'Events',
      description:
        'Select one or more deal events to listen for. Configure a matching notification in Outseta for each selected event, all pointing to this webhook URL.',
      required: true,
      options: {
        disabled: false,
        options: [
  ...
  async test(context) {
    ...
    const res = await client.get<{ items?: Record<string, unknown>[]; Items?: Record<string, unknown>[] }>(
      `/api/v1/crm/deals?limit=5&orderBy=${orderBy}&orderDirection=desc`
    );
