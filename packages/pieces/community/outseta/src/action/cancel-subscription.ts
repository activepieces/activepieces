...
export const cancelSubscriptionAction = createAction({
    ...
    async run(context) {
        ...
        await client.put<any>(`/api/v1/crm/accounts/cancellation/${context.propsValue.accountUid}`, body)
        ...
        if (context.propsValue.cancelImmediately) {
            ...
            await client.put<any>(`/api/v1/crm/accounts/${context.propsValue.accountUid}`, account)
        }
