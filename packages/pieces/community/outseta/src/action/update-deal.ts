...
export const updateDealAction = createAction({
    ...
    props: {
        ...
        assignedToPersonClientIdentifier: Property.ShortText({
            ...
            required: false,
            description:
                "The client identifier of the team member this deal is assigned to. Found in the person's Outseta profile.",
        }),
    ...
    async run(context) {
        ...
        if (context.propsValue.assignedToPersonClientIdentifier) {
            deal.AssignedToPersonClientIdentifier = context.propsValue.assignedToPersonClientIdentifier
        }
        ...
        if (context.propsValue.personUid) {
            deal.DealPeople = [{ Person: { Uid: context.propsValue.personUid } }]
        }

        const updated = await client.put<any>(`/api/v1/crm/deals/${context.propsValue.dealUid}`, deal)
