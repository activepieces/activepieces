...
export const changeAccountPlanAction = createAction({
    ...
    description: "Change an account's current subscription plan (upgrade, downgrade, or switch to free).",
    props: {
    ...
    async run(context) {
        ...
        if (!account.CurrentSubscription?.Uid) {
            throw new Error('This account does not have an active subscription to change.')
        }

        const subscriptionUid = account.CurrentSubscription.Uid

        const result = await client.put<any>(`/api/v1/billing/subscriptions/${subscriptionUid}/changesubscription`, {
