...
export const updateAccountAction = createAction({
    ...
    async run(context) {
        ...
        if (context.propsValue.name) {
            account.Name = context.propsValue.name
            changed = true
        }
        if (context.propsValue.accountStage != null) {
            account.AccountStage = context.propsValue.accountStage
            changed = true
        }
        if (context.propsValue.clientIdentifier) {
            account.ClientIdentifier = context.propsValue.clientIdentifier
            changed = true
        }
        if (context.propsValue.invoiceNotes) {
            account.InvoiceNotes = context.propsValue.invoiceNotes
            changed = true
        }
        ...
        const updatedAccount = await client.put<any>(`/api/v1/crm/accounts/${context.propsValue.accountUid}`, account)
