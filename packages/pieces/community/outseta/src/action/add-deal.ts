...
export const addDealAction = createAction({
    ...
    description: 'Create a deal and optionally associate it with an account or a person.',
    props: {
    ...
    async run(context) {
        ...
        if (context.propsValue.assignedToPersonClientIdentifier) {
            body['AssignedToPersonClientIdentifier'] = context.propsValue.assignedToPersonClientIdentifier
        }
        ...
        if (context.propsValue.personUid) {
            body['DealPeople'] = [{ Person: { Uid: context.propsValue.personUid } }]
        }
