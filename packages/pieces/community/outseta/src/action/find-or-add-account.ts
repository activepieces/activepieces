...
export const findOrAddAccountAction = createAction({
    ...
    description: 'Search for an account by name. If not found, create a new one.',
    props: {
        name: Property.ShortText({
            displayName: 'Name',
            required: true,
            description:
                'Account name to search for. If no exact match is found, a new account is created with this name.',
        }),
    ...
    async run(context) {
        ...
        const exactMatch = items.find((item: any) => item.Name?.toLowerCase() === context.propsValue.name.toLowerCase())
        if (exactMatch) {
            return {
                created: false,
