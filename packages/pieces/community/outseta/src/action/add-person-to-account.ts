...
export const addPersonToAccountAction = createAction({
    ...
    async run(context) {
        ...
        const account = await client.get<any>(`/api/v1/crm/accounts/${context.propsValue.accountUid}`)

        const existingMemberships = account.PersonAccount ?? []

        const alreadyLinked = existingMemberships.some((pa: any) => pa.Person?.Uid === context.propsValue.personUid)
        if (alreadyLinked) {
            throw new Error(
        ...
        const result = await client.put<any>(`/api/v1/crm/accounts/${context.propsValue.accountUid}`, {
            ...account,
