...
export const BASE_URL = `https://www.zohoapis.in/bigin/v2`;
  ...
  try {
    ...
    if (responseData && Array.isArray(responseData) && responseData.length > 0) {
      ...
      if (firstItem.status === 'error' && firstItem.code) {
        ...
        throw new Error(`Bigin API Error (${firstItem.code}): ${firstItem.message}${details}`);
      }
    }
    ...
    if (error.response?.body) {
      ...
      // Check for nested data array format errors
      if (errorBody?.data && Array.isArray(errorBody.data) && errorBody.data.length > 0) {
        ...
        if (firstError.status === 'error' && firstError.code) {
          ...
          throw new Error(`Bigin API Error (${firstError.code}): ${firstError.message}${details}`);
        }
      }
      ...
      if (Array.isArray(errorBody) && errorBody.length > 0) {
        ...
        if (firstError.status === 'error' && firstError.code) {
          throw new Error(`Bigin API Error (${firstError.code}): ${firstError.message}`);
        }
      }
      
      // Check for object format errors
      if (errorBody.code && errorBody.message) {
        throw new Error(`Bigin API Error (${errorBody.code}): ${errorBody.message}`);
      }
    }
