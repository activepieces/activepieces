...
export const updatedPipelineRecord = createTrigger({
  ...
  async run(context) {
    // Define the expected payload type based on the notification structure
    ...
    let payload: PipelineUpdatePayload = context.payload
      ?.body as PipelineUpdatePayload;
