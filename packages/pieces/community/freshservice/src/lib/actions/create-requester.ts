...
export const createRequester = createAction({
    ...
    async run(context) {
        ...
        if (props.mobile_phone_number) requesterBody['mobile_phone_number'] = props.mobile_phone_number
        if (props.department_id) requesterBody['department_ids'] = [props.department_id]
        if (props.custom_fields) requesterBody['custom_fields'] = props.custom_fields
