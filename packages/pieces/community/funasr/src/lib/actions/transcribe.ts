...
export const transcribeAction = createAction({
  ...
  props: {
    ...
    model: Property.ShortText({
      displayName: 'Model',
      description: 'The model to use for transcription (e.g. "sensevoice", "paraformer", "fun-asr-nano"). Leave empty for server default.',
      required: false,
    }),
  ...
  run: async (context) => {
    ...
    const mimeType = mime.lookup(fileData.extension ? fileData.extension : '') || 'application/octet-stream';
