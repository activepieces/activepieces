...
export const usersDropdown = Property.Dropdown({
    ...
    options: async ({ auth }: any) => {
        ...
        try {
            const response = await biginApiService.fetchUsers(auth.access_token, api_domain)
...
export const companyDropdown = Property.Dropdown({
    ...
    options: async ({ auth }: any) => {
        if (!auth) return handleDropdownError('Please Connect your account first')
        const resp = await biginApiService.fetchCompanies(auth.access_token, auth.api_domain)
        return {
...
export const contactsDropdown = Property.Dropdown({
    ...
    options: async ({ auth }: any) => {
        if (!auth) return handleDropdownError('Please Connect your account first')
        const resp = await biginApiService.fetchContacts(auth.access_token, auth.api_domain)
        return {
            options: resp.data.map((a: any) => ({
...
export const multiContactsDropdown = Property.MultiSelectDropdown({
    ...
    description: 'Provide a list of additional contacts associated with the record',
    required: false,
    refreshers: ['auth'],
    options: async ({ auth }: any) => {
        if (!auth) return handleDropdownError('Please Connect your account first')
...
export const pipelineRecordsDropdown = Property.Dropdown({
    ...
    description: 'Select a pipeline record',
    required: false,
    refreshers: ['auth'],
    options: async ({ auth }: any) => {
        if (!auth) return handleDropdownError('Please Connect your account first')
        const resp = await biginApiService.fetchPipelinesRecords(auth.access_token, auth.api_domain)
        return {
            options: resp.data.map((a: any) => ({
                label: a.Deal_Name,
...
export const layoutsDropdown = (displayName: string, description: string, module_name: string) =>
    Property.Dropdown({
        auth: biginAuth,
        displayName,
        description,
        ...
        options: async ({ auth }: any) => {
            if (!auth) return handleDropdownError('Please Connect your account first')
            const resp = await biginApiService.fetchLayouts(auth.access_token, auth.api_domain, module_name)
...
export const productsDropdown = Property.MultiSelectDropdown({
    ...
    description: 'Provide a list of products associated with the record',
    required: false,
    refreshers: ['auth'],
    options: async ({ auth }: any) => {
        if (!auth) return handleDropdownError('Please Connect your account first')
        const resp = await biginApiService.fetchProducts(auth.access_token, auth.api_domain)
...
export const tagsDropdown = (module: string, defaultValue?: string[]) =>
    Property.MultiSelectDropdown({
        ...
        options: async ({ auth }: any) => {
            if (!auth) return handleDropdownError('Please Connect your account first')
            try {
                const resp = await biginApiService.fetchTags(auth.access_token, auth.api_domain, module)
...
export const SubPipelineorStageDropdown = (
    ...
    Property.Dropdown({
        ...
        options: async ({ auth, pipeline }) => {
            if (!auth) return handleDropdownError('Please connect your account first')
            if (!pipeline) return handleDropdownError('Please select a Pipeline first')

            const { access_token, api_domain } = auth as any

            const response = await biginApiService.fetchLayouts(access_token, api_domain, 'Pipelines')
            ...
            const PipelineInfo = layout.sections.find((s: any) => s.api_name === 'Pipeline Information')

            const stageField = PipelineInfo?.fields.find((f: any) => f.field_label === field_label)
