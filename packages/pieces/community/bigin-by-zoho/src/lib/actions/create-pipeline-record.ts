...
export const createPipelineRecord = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            const response = await biginApiService.createPipelineRecord(access_token, api_domain, { data: [payload] })

            const created = response.data[0]

            if (propsValue.secondaryContacts && Array.isArray(propsValue.secondaryContacts) && created?.details?.id) {
