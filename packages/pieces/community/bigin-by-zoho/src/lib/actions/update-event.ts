...
export const updateEvent = createAction({
    ...
    props: {
        eventId: Property.Dropdown({
            ...
            options: async ({ auth }): Promise<any> => {
                ...
                try {
                    const response = await biginApiService.fetchEvents(access_token, api_domain)
        ...
        eventFields: Property.DynamicProperties({
            ...
            props: async ({ eventId, auth }: any): Promise<InputPropertyMap> => {
                ...
                const fieldsResp = await biginApiService.fetchModuleFields(access_token, api_domain, 'Events')
                ...
                if (propsValue['enableReminder']) {
                    return {
                        reminderList: Property.Array({
                            ...
                            description: 'Add multiple reminders (e.g., "1 hour", "30 minutes")',
                            required: false,
                            properties: {
        ...
        relatedTo: Property.Dropdown({
            ...
            options: async ({ auth, relatedModule }): Promise<any> => {
                ...
                const fetchMap: Record<string, () => Promise<any>> = {
                    Contacts: () => biginApiService.fetchContacts(access_token, api_domain),
                    Pipelines: () => biginApiService.fetchPipelinesRecords(access_token, api_domain),
                    Companies: () => biginApiService.fetchCompanies(access_token, api_domain),
                }

                const fetchFn = fetchMap[(relatedModule as keyof typeof fetchMap) || 'Contacts']
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.enableReminder && propsValue.reminderInfo?.['reminderList']) {
        ...
        if (propsValue.relatedTo && propsValue.relatedModule) {
            ...
            const relatedModule = relatedModuleMap[propsValue.relatedModule as keyof typeof relatedModuleMap]

            eventData.Related_To = { id: propsValue.relatedTo }
            eventData.$related_module = relatedModule
        }

        if (propsValue.tag && Array.isArray(propsValue.tag) && propsValue.tag.length > 0) {
        ...
        try {
            const response = await biginApiService.updateEvent(access_token, api_domain, payload)
