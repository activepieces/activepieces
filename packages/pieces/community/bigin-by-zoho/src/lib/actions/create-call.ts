...
export const createCall = createAction({
    ...
    async run({ auth, propsValue }) {
        try {
            ...
            if (propsValue.dialledNumber) payload.Dialled_Number = propsValue.dialledNumber
            ...
            if (propsValue.relatedTo && propsValue.relatedModule) {
                ...
                payload.$related_module =
                    relatedModuleMap[propsValue.relatedModule as keyof typeof relatedModuleMap] || 'Deals'
                payload.Related_To = { id: propsValue.relatedTo }
            }

            if (propsValue.tag) payload.Tag = propsValue.tag.map((t: any) => ({ name: t }))

            const response = await biginApiService.createCall(access_token, api_domain, { data: [payload] })
