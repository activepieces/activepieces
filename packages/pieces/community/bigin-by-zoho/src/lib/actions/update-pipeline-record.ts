...
import {
    ...
    contactsDropdown,
    layoutsDropdown,
    multiContactsDropdown,
    pipelineRecordsDropdown,
    productsDropdown,
    SubPipelineorStageDropdown,
    tagsDropdown,
    usersDropdown,
} from '../common/props'
...
export const updatePipelineRecord = createAction({
    ...
    props: {
        ...
        pipelineDetails: Property.DynamicProperties({
            ...
            props: async ({ pipelineRecordId, auth }: any) => {
                ...
                const fieldsResp = await biginApiService.fetchModuleFields(access_token, api_domain, 'Pipelines')
    ...
    async run({ propsValue, auth }) {
        ...
        if (propsValue.accountName) payload.Account_Name = { id: propsValue.accountName }
        if (propsValue.contactName) payload.Contact_Name = { id: propsValue.contactName }

        if (propsValue.subPipeline) payload['Sub Pipeline'] = propsValue.subPipeline
        ...
        if (propsValue.pipelineDetails['closingDate'])
            payload.Closing_Date = formatDateOnly(propsValue.pipelineDetails['closingDate'])
        ...
        if (propsValue.associatedProducts) {
            payload.Associated_Products = propsValue.associatedProducts.map((product: any) => {
        ...
        try {
            const response = await biginApiService.updatePipelineRecord(access_token, api_domain, { data: [payload] })

            if (propsValue.secondaryContacts && Array.isArray(propsValue.secondaryContacts)) {
                const secondaries = (propsValue.secondaryContacts as any[])
                    .map((v: any) => String(v))
                    .filter((id: string) => id !== propsValue.contactName)

                if (secondaries.length > 0) {
                    await biginApiService.updatePipelineRecord(access_token, api_domain, {
                        data: [
                            {
                                id: pipelineData.id,
