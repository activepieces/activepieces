import { createAction, InputPropertyMap, Property, PropertyContext } from '@activepieces/pieces-framework'
    ...
    props: {
        contactId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const resp = await biginApiService.fetchContacts(auth.access_token, auth.data['api_domain'])
                return {
        ...
        contactDetails: Property.DynamicProperties({
            ...
            required: true,
            props: async ({ contactId, auth }: any): Promise<InputPropertyMap> => {
                if (!contactId) return {}
