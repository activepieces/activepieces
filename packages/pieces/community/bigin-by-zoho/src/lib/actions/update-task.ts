...
export const updateTask = createAction({
    ...
    props: {
        taskId: Property.Dropdown({
            ...
            options: async (context) => {
                if (!context.auth) return handleDropdownError('Please connect your account first')
        ...
        taskDetails: Property.DynamicProperties({
            ...
            props: async ({ taskId, auth }): Promise<InputPropertyMap> => {
                ...
                const fieldsResp = await biginApiService.fetchModuleFields(access_token, api_domain, 'Tasks')
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.enableReminder && propsValue.reminderInfo) {
            ...
            if (propsValue.recurringInfo && reminderInfo.reminderType === 'days_before') {
                const daysBefore = reminderInfo.reminderDaysBefore || 1
                alarmValue += `;TRIGGER=-P${daysBefore}D;TRIGGER_TIME=${reminderTime}`
            } else if (propsValue.recurringInfo && reminderInfo.reminderType === 'weeks_before') {
        ...
        if (propsValue.relatedTo && propsValue.relatedModule) {
            ...
            const relatedModule =
                relatedModuleMap[propsValue.relatedModule as keyof typeof relatedModuleMap] || 'Contacts'
        ...
        if (taskDetails.tag && Array.isArray(taskDetails.tag) && taskDetails.tag.length > 0) {
        ...
        try {
            const response = await biginApiService.updateTask(access_token, api_domain, payload)
