...
export const updateCompanyAction = createAction({
  ...
  description: 'Update an existing company in Bigin CRM',
  props: {
  ...
  async run(context) {
    ...
    try {
      // Build company data (only include fields that are provided)
      ...
      // Update company
      const response = await client.updateCompany(companyId, companyData);
