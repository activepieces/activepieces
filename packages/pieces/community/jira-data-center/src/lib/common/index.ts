...
export async function getIssueTypes({ auth, projectId }: { auth: JiraDataCenterAuth; projectId: string }) {
...
export async function jiraPaginatedApiCall<T extends HttpMessageBody, K extends string>({
    ...
    do {
        ...
        hasMore =
            response.isLast === undefined ? response.startAt + response.maxResults < response.total : !response.isLast
    } while (hasMore)
