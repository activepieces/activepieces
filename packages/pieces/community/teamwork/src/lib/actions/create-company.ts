...
export const createCompany = createAction({
    ...
    props: {
        ...
        addressOne: Property.ShortText({
            displayName: 'Address 1',
            description: 'Primary address line.',
            required: false,
        }),
        addressTwo: Property.ShortText({
        ...
        emailOne: Property.ShortText({
            displayName: 'Email 1',
            description: 'Primary email address.',
            required: false,
        }),
        emailTwo: Property.ShortText({
            displayName: 'Email 2',
            description: 'Secondary email address.',
            required: false,
        }),
        emailThree: Property.ShortText({
            displayName: 'Email 3',
            description: 'Tertiary email address.',
            required: false,
        }),
        profile: Property.LongText({
            displayName: 'Profile',
            description: "Public company profile or 'About Us' text.",
            required: false,
        }),
        privateNotes: Property.LongText({
            displayName: 'Private Notes',
            description: 'Notes visible only to internal users.',
            required: false,
        }),
    ...
    async run({ auth, propsValue }) {
        const customFields = Object.entries(propsValue.customFields ?? {}).map(([customfieldId, value]) => ({
