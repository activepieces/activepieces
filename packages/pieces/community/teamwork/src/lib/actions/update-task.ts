...
export const updateTask = createAction({
    ...
    props: {
        ...
        'responsible-party-id': Property.MultiSelectDropdown({
            ...
            options: async ({ auth, taskId }) => {
                ...
                const options = peopleRes.data.people.map(
                    (p: { id: string; 'first-name': string; 'last-name': string }) => ({
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue['responsible-party-id'])
            item['responsible-party-id'] = propsValue['responsible-party-id'].join(',')
