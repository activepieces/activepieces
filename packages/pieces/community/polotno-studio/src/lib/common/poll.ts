...
export async function pollUntilTerminal(options: PollOptions): Promise<PollResult> {
  ...
  const started = now();
  let delay = polotnoConstants.POLL_INITIAL_DELAY_MS;

  let render = await options.client.request<RenderLike>({ path });

  while (!polotnoConstants.isTerminal(render.status)) {
    if (now() - started + delay > options.maxWaitMs) {
      return { render, timedOut: true };
    }
    await sleep(delay);
    delay = Math.min(Math.round(delay * polotnoConstants.POLL_BACKOFF_FACTOR), polotnoConstants.POLL_MAX_DELAY_MS);
