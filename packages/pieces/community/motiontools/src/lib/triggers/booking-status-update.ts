...
export const bookingStatusUpdate = createTrigger({
    ...
    description: 'Triggers when a booking status is updated.  Only available if you have a MotionToolsadmin account',
    props: {
...
`,
    ...
    async run(context) {
        ...
        if (body.event === 'modified' && body.resource_type === 'booking' && body.data) {
            return [body]
        }
        return []
    },
