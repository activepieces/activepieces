...
export const mollieCreatePaymentLink = createAction({
    ...
    props: {
        description: Property.ShortText({
            ...
            description: 'A short description of the payment link (max 255 characters)',
            required: true,
        }),

        includeAmount: Property.Checkbox({
            displayName: 'Include Fixed Amount',
            description: 'Whether to include a fixed amount or let customer enter amount',
            required: false,
            defaultValue: true,
        }),
        ...
        includeMinimumAmount: Property.Checkbox({
            ...
            description: 'Whether to set a minimum amount (only when no fixed amount)',
            required: false,
            defaultValue: false,
        }),
        ...
        allowedMethods: Property.MultiSelectDropdown({
            ...
            description: 'Payment methods allowed for this link (empty = all enabled methods)',
            required: false,
            refreshers: [],
            options: async () => {
                return {
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.includeLines && propsValue.lines && Array.isArray(propsValue.lines)) {
            const lines = (propsValue.lines as unknown[]).map((line: unknown) => {
                ...
                if (lineData['quantityUnit']) orderLine['quantityUnit'] = lineData['quantityUnit']
                ...
                if (lineData['discountAmountCurrency'] && lineData['discountAmountValue']) {
                ...
                if (lineData['productUrl']) orderLine['productUrl'] = lineData['productUrl']
        ...
        if (propsValue.includeBillingAddress) {
            ...
            if (propsValue.billingTitle) billingAddress['title'] = propsValue.billingTitle
            if (propsValue.billingGivenName) billingAddress['givenName'] = propsValue.billingGivenName
            if (propsValue.billingFamilyName) billingAddress['familyName'] = propsValue.billingFamilyName
            if (propsValue.billingOrganizationName)
                billingAddress['organizationName'] = propsValue.billingOrganizationName
            if (propsValue.billingStreetAndNumber) billingAddress['streetAndNumber'] = propsValue.billingStreetAndNumber
            if (propsValue.billingStreetAdditional)
                billingAddress['streetAdditional'] = propsValue.billingStreetAdditional
            if (propsValue.billingPostalCode) billingAddress['postalCode'] = propsValue.billingPostalCode
            if (propsValue.billingEmail) billingAddress['email'] = propsValue.billingEmail
            if (propsValue.billingPhone) billingAddress['phone'] = propsValue.billingPhone
            if (propsValue.billingCity) billingAddress['city'] = propsValue.billingCity
            if (propsValue.billingRegion) billingAddress['region'] = propsValue.billingRegion
            if (propsValue.billingCountry) billingAddress['country'] = propsValue.billingCountry
        ...
        if (propsValue.includeShippingAddress) {
            ...
            if (propsValue.shippingTitle) shippingAddress['title'] = propsValue.shippingTitle
            if (propsValue.shippingGivenName) shippingAddress['givenName'] = propsValue.shippingGivenName
            if (propsValue.shippingFamilyName) shippingAddress['familyName'] = propsValue.shippingFamilyName
            if (propsValue.shippingOrganizationName)
                shippingAddress['organizationName'] = propsValue.shippingOrganizationName
            ...
            if (propsValue.shippingStreetAdditional)
                shippingAddress['streetAdditional'] = propsValue.shippingStreetAdditional
            if (propsValue.shippingPostalCode) shippingAddress['postalCode'] = propsValue.shippingPostalCode
            if (propsValue.shippingEmail) shippingAddress['email'] = propsValue.shippingEmail
            if (propsValue.shippingPhone) shippingAddress['phone'] = propsValue.shippingPhone
            if (propsValue.shippingCity) shippingAddress['city'] = propsValue.shippingCity
            if (propsValue.shippingRegion) shippingAddress['region'] = propsValue.shippingRegion
            if (propsValue.shippingCountry) shippingAddress['country'] = propsValue.shippingCountry
        ...
        const response = await mollieCommon.makeRequest(apiKey, HttpMethod.POST, '/payment-links', paymentLinkData)
