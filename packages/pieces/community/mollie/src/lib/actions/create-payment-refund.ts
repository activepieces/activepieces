...
export const mollieCreatePaymentRefund = createAction({
    ...
    props: {
        paymentId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    const apiKey = auth
                    const payments = await mollieCommon.makeRequest(apiKey, HttpMethod.GET, '/payments?limit=250')
        ...
        reverseRouting: Property.Checkbox({
            ...
            description: 'Pull back funds routed to connected merchants (full refund only)',
            required: false,
            defaultValue: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.includeExternalReference && propsValue.externalReferenceType && propsValue.externalReferenceId) {
        ...
        if (
            ...
            const reversals = (propsValue.routingReversals as unknown[]).map((reversal: unknown) => {
