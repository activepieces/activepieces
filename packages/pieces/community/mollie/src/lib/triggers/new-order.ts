...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, Record<string, unknown>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        do {
            ...
            const url = from ? `/orders?sort=desc&limit=${limit}&from=${from}` : `/orders?sort=desc&limit=${limit}`

            const response = await mollieCommon.makeRequest<MollieOrderResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.orders || !response._embedded.orders.length) {
                break
            }
