...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, { paymentId: string }> = {
    ...
    items: async ({ auth, lastFetchEpochMS, propsValue }) => {
        ...
        do {
            ...
            const response = await mollieCommon.makeRequest<MollieChargebackResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.chargebacks || !response._embedded.chargebacks.length) {
                break
            }
        ...
        paymentId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    const options = paymentsResponse._embedded.payments.map((payment) => ({
