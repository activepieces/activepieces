...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, Record<string, unknown>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        do {
            ...
            const url = from ? `/payments?sort=desc&limit=${limit}&from=${from}` : `/payments?sort=desc&limit=${limit}`

            const response = await mollieCommon.makeRequest<MolliePaymentResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.payments || !response._embedded.payments.length) {
                break
            }
