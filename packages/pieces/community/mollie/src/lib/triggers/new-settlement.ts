...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, Record<string, unknown>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        do {
            ...
            const url = from ? `/settlements?limit=${limit}&from=${from}` : `/settlements?limit=${limit}`

            const response = await mollieCommon.makeRequest<MollieSettlementResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.settlements || !response._embedded.settlements.length) {
                break
            }
