...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, Record<string, unknown>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        do {
            ...
            const response = await mollieCommon.makeRequest<MollieCustomerResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.customers || !response._embedded.customers.length) {
                break
            }
