...
const polling: Polling<AppConnectionValueForAuthProperty<typeof mollieAuth>, { paymentId: string }> = {
    ...
    items: async ({ auth, lastFetchEpochMS, propsValue }) => {
        ...
        do {
            ...
            const response = await mollieCommon.makeRequest<MollieRefundResponse>(apiKey, HttpMethod.GET, url)

            if (!response._embedded || !response._embedded.refunds || !response._embedded.refunds.length) {
                break
            }
