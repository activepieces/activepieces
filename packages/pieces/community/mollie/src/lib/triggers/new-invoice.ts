...
export const mollieNewInvoice = createTrigger({
    ...
    async onEnable(context) {
        ...
        const response = await mollieCommon.makeRequest(context.auth, HttpMethod.POST, '/webhooks', webhookData)
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            try {
                await mollieCommon.makeRequest(context.auth, HttpMethod.DELETE, `/webhooks/${webhookId}`)
            } catch (error) {
    ...
    async run(context) {
        ...
        if (payload.id) {
            try {
                const invoice = await mollieCommon.makeRequest(context.auth, HttpMethod.GET, `/invoices/${payload.id}`)
    ...
    async test(context) {
        try {
            const response = await mollieCommon.makeRequest(context.auth, HttpMethod.GET, '/invoices?limit=1')
