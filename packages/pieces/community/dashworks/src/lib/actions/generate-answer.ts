...
export const generateAnswerAction = createAction({
    ...
    props: {
        botId: Property.ShortText({
            ...
            required: true,
            description:
                "You can find this by going to Dashworks > Bots, clicking on an existing Bot, and using the 'Copy Bot ID' button on the top right.",
        }),
        ...
        inlineSources: Property.Checkbox({
            ...
            required: false,
            description:
                'When true (default), sources will be cited inline in markdown formatting within the answer text.When false, the answer will not contain inline citations.',
        }),
    },
    async run(context) {
