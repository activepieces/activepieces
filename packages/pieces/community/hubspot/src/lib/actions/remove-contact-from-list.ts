...
export const removeContactFromListAction = createAction({
  ...
  async run(context) {
    ...
    if (contact.results.length === 0) {
      throw new Error(`No contact with email '${email}' was found. Unable to remove unknown contact from list.`)
    }
    ...
    if (parsedResponse.updated.length === 0) {
      throw new Error(`Contact with email '${email}' wasn't a member of list with id '${listId}'.`)
    }
