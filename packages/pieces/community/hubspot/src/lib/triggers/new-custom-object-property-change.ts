...
import { createTrigger, DynamicPropsValue, PiecePropValueSchema, TriggerStrategy } from '@activepieces/pieces-framework'
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof hubspotAuth>, Props> = {
    ...
    async items({ auth, propsValue, lastFetchEpochMS }) {
        ...
        do {
            ...
            if (!isNil(after) && parseInt(after) + MAX_SEARCH_PAGE_SIZE > MAX_SEARCH_TOTAL_RESULTS) {
                break
            }
        } while (after)
        ...
        const updatedCustomObjectsWithPropertyHistory = batchApiResps.flatMap((resp) => resp.results)
