...
export const hubSpotClient = {
  contacts: {
    async createOrUpdate({
      ...
      const response =
        await httpClient.sendRequest<HubSpotContactsCreateOrUpdateResponse>(
          request
        );
  ...
  lists: {
    async getStaticLists({
      token,
    }: GetStaticListsParams): Promise<HubSpotListsResponse> {
      ...
      const response = await httpClient.sendRequest<HubSpotListsResponse>(
        request
      );
      return response.body;
    },
    ...
    async addContact({
      ...
      const response =
        await httpClient.sendRequest<HubSpotAddContactsToListResponse>(request);
      return response.body;
    },
  },

  tasks: {
    async getTasksAfterLastSearch(
      accessToken: string,
