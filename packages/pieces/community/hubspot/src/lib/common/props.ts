...
import { HubspotFieldType, HubspotProperty, ListBlogsResponse, WorkflowResponse } from './types'
...
async function retriveObjectProperties(
  ...
  const groupLabels = propertyGroups.results.reduce(
    (map, group) => {
  ...
  for (const property of allProperties.results) {
    ...
    if (excludedProperties.includes(property.name) || property.modificationMetadata?.readOnlyValue || property.hidden) {
      continue
    }
    ...
    if (property.referencedObjectType) {
      props[property.name] = await createReferencedPropertyDefinition(property, propertyDisplayName, auth.access_token)
      continue
    }
...
export const standardObjectPropertiesDropdown = (
  ...
  return dropdownFunction({
    ...
    options: async ({ auth }) => {
      ...
      const defaultProperties = includeDefaultProperties ? [] : getDefaultPropertiesForObject(params.objectType)
...
export const customObjectPropertiesDropdown = (displayName: string, required: boolean, isSingleSelect = false) =>
  Property.DynamicProperties({
    ...
    props: async ({ auth, customObjectType }) => {
      ...
      // Fetch all properties for the given object type
      const allProperties = await client.crm.properties.coreApi.getAll(customObjectType as unknown as string)

      const propertyGroups = await client.crm.properties.groupsApi.getAll(customObjectType as unknown as string)
...
export const toObjectIdsDropdown = (params: DropdownParams) =>
  Property.MultiSelectDropdown({
    ...
    options: async ({ auth, toObjectType }) => {
      ...
      let after: string | undefined
      do {
        const response = await client.crm.objects.basicApi.getPage(toObjectType as string, limit, after)
