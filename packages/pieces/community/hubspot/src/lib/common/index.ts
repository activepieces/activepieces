...
export const hubspotCommon = {
	...
	dynamicProperties: Property.DynamicProperties({
		...
		props: async ({ auth, choose_props }) => {
			const all_props = choose_props as HubspotProperty[];
				...
				switch (prop.fieldType) {
					...
					case HubspotFieldType.Radio:
						if (prop.name === 'hubspot_owner_id') {
							try {
								const res =
									(await hubSpotClient.listContactOwners(auth.access_token as string)).results ??
									[];
								...
								return {
									disabled: true,
									options: [],
									placeholder: 'An error occurred while fetching contact owner list.',
								};
							}
						} else {
							fields[prop.name] = Property.StaticDropdown({
