...
export class OracleDbClient {
    ...
    public async updateRow(
        ...
        const whereClause = filterKeys.map((k) => `"${k}" = :whr_${k}`).join(' AND ')
    ...
    public async findRow(tableName: string, filter: Record<string, unknown>): Promise<unknown[]> {
        await this.connect()
    ...
    public async getColumns(tableName: string): Promise<{ label: string; value: string }[]> {
        await this.connect()
        if (!this.connection) {
            throw new Error('Database connection failed and was not established.')
        }
