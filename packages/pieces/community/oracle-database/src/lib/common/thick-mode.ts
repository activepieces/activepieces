...
const ORACLE_INSTANT_CLIENT_URL = `https://download.oracle.com/otn_software/linux/instantclient/2326100/instantclient-basic-${ORACLE_INSTANT_CLIENT_ARCH}-23.26.1.0.0.zip`
        ...
        const req = https
            .get(url, (response) => {
                ...
                if ([301, 302, 307, 308].includes(statusCode!)) {
                    ...
                    file.close()
                    fs.unlink(dest, () => {})
                    downloadFile({ url: headers.location!, dest, logs, redirectDepth: redirectDepth + 1 })
                        .then(resolve)
                        .catch(reject)
                    return
                }
