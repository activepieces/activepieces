...
export const findRowAction = createAction({
    ...
    async run(context) {
        ...
        if (Object.keys(filter).length === 0) {
            throw new Error('Filter cannot be empty. Use Run Custom SQL action to fetch all rows.')
        }

        try {
            const client = new OracleDbClient(context.auth.props)
            return await client.findRow(tableName, filter as Record<string, unknown>)
        } catch (error) {
            throw new Error(
