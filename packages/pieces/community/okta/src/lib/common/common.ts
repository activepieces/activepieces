...
export async function makeOktaRequest(auth: any, endpoint: string, method: HttpMethod = HttpMethod.GET, body?: any) {
    ...
    Property.Dropdown({
        ...
        options: async ({ auth, groupId }) => {
            try {
                ...
                const path = groupusers ? `/groups/${groupId}/users` : '/users'
                const response = await makeOktaRequest(auth, path, HttpMethod.GET, undefined)
...
export const groupIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        try {
            ...
            const response = await makeOktaRequest(auth, '/groups', HttpMethod.GET)
