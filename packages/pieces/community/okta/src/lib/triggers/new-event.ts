...
import { makeOktaRequest, oktaAuth } from '../common/common'
...
export const newEventTrigger = createTrigger({
    ...
    async onHandshake(context) {
        const challengeValue = context.payload.headers['x-okta-verification-challenge']
    ...
    async onEnable(context) {
        try {
            ...
            const existingHooks = await makeOktaRequest(context.auth, '/eventHooks', HttpMethod.GET)

            const existingHook = existingHooks.body?.find(
                (hook: any) => hook.name === hookName && hook.channel?.config?.uri === context.webhookUrl,
            )
            ...
            if (existingHook) {
                ...
                const response = await makeOktaRequest(context.auth, '/eventHooks', HttpMethod.POST, eventHookPayload)

                if (!response.body?.id) {
                    throw new Error('Failed to create event hook: ' + JSON.stringify(response.body))
                }
            ...
            if (response.status !== 200) {
                throw new Error('Failed to verify event hook: ' + JSON.stringify(response.body))
            }
        } catch (error) {
    ...
    async onDisable(context) {
        try {
            ...
            if (hookId) {
                try {
                    await makeOktaRequest(context.auth, `/eventHooks/${hookId}/lifecycle/deactivate`, HttpMethod.POST)
                ...
                try {
                    await makeOktaRequest(context.auth, `/eventHooks/${hookId}`, HttpMethod.DELETE)
        ...
        for (const event of payload.data.events) {
            if (configuredEventTypes.length > 0) {
                if (!configuredEventTypes.includes(event.eventType)) {
                    console.log(`Event type ${event.eventType} not in configured types, skipping`)
                    continue
                }
            }
    ...
    async test(context) {
        try {
            const response = await makeOktaRequest(context.auth, '/logs?limit=1', HttpMethod.GET)
