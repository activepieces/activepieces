...
import { CyberArkAuth, getAuthToken } from '../common/auth-helper'
...
export const findUser = createAction({
    ...
    props: {
        ...
        sort: Property.ShortText({
            displayName: 'Sort',
            description:
                'Sort by property (username, source, userType, location, lastname, firstname, middlename) followed by asc/desc',
            required: false,
        }),
    ...
    async run(context) {
        ...
        const url = queryString
            ? `${authData.serverUrl}/PasswordVault/API/Users?${queryString}`
            : `${authData.serverUrl}/PasswordVault/API/Users`
