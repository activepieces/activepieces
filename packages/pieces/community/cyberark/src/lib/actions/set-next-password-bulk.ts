...
export const setNextPasswordBulk = createAction({
    ...
    description: "Sets multiple accounts' credentials to use for the next CPM change",
    props: {
        bulkItems: Property.Array({
            displayName: 'Bulk Items',
            description:
                'List of account items. Each item should be a JSON object with accountId (required), changeImmediately (optional), and newCredentials (optional).',
            required: true,
        }),
    },
    async run(context) {
