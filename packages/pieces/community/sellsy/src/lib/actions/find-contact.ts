...
export const findContact = createAction({
  ...
  description: 'Search for contacts in Sellsy',
  props: {
    ...
    first_name: Property.ShortText({
      ...
      description: 'Search contacts by first name',
      required: false,
    }),
    last_name: Property.ShortText({
      displayName: 'Last Name',
      description: 'Search contacts by last name',
      required: false,
    }),
    ...
    limit: Property.Number({
      displayName: 'Limit',
      description: 'Maximum number of contacts to return (0-100)',
      required: false,
      defaultValue: 25,
    }),
  ...
  async run(context) {
    ...
    if (propsValue.first_name) {
      filters.first_name = propsValue.first_name;
    }
    if (propsValue.last_name) {
      filters.last_name = propsValue.last_name;
    }
