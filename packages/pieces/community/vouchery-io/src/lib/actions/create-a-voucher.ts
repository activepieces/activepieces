...
export const createAVoucher = createAction({
    ...
    async run(context) {
        const { campaignId, voucherCode, voucher_valid_until, customer_identifier, gift_card_value, activates_at } =
            context.propsValue
        ...
        if (voucher_valid_until) body.valid_until = new Date(voucher_valid_until).toISOString()
