...
export const findVoucher = createAction({
    ...
    description: 'Find a voucher by campaign metadata. At least one metadata must be provided',
    props: {
        ...
        customer_identifier: Property.ShortText({
            ...
            description: 'Optional: Assign the found voucher to a customer by providing their identifier',
            required: false,
        }),
    },
    async run(context) {
        const { purpose, team, channel, medium, customer_identifier } = context.propsValue
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.GET, path)
