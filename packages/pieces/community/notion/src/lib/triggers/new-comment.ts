...
const getComments = async (authentication: NotionAuthValue, page_id: string, startDate: string | null) => {
    ...
    do {
        try {
            ...
            for (const comment of response.results) {
                if (startDate && !dayjs(comment.created_time).isAfter(dayjs(startDate))) {
