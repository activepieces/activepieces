...
import { FormStructure, NotionDatabase, NotionDatabaseProperty } from '../common/types'
...
export const retrieveDatabase = createAction({
    ...
    async run(context) {
        ...
        Object.entries(database.properties).forEach(([key, property]: [string, NotionDatabaseProperty]) => {
            ...
            switch (property.type) {
                ...
                case 'status':
                    if (property.status) {
                        formStructure.statusOptions[key] = {
                            options:
                                property.status.options?.map((option: { name: string; color: string; id: string }) => ({
                                    name: option.name,
                                    color: option.color,
                                    id: option.id,
                                })) || [],
                            groups:
                                property.status.groups?.map(
                                    (group: { name: string; color: string; id: string; option_ids: string[] }) => ({
                                        name: group.name,
                                        color: group.color,
                                        id: group.id,
                ...
                default:
                    console.warn(`Unknown property type: ${property.type} for property: ${key}`)
                    break
            }
        })
        ...
        return {
            ...
            } dropdown fields and ${Object.keys(formStructure.statusOptions).length} status fields.`,
        }
    },
})
