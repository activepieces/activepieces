...
export const getPageComments = createAction({
    ...
    async run(context) {
        ...
        pageComments.sort((a: any, b: any) => new Date(a.created_time).getTime() - new Date(b.created_time).getTime())
            ...
            message:
                allComments.length === 0
                    ...
                    : `💬 Successfully retrieved ${allComments.length} comment${
                          ...
                          threadCount > 0
                              ? ` organized into ${threadCount} discussion thread${threadCount === 1 ? '' : 's'}`
                              : ''
                      }.`,
        }
    },
