...
export const getMplPrice = createAction({
  name: 'get-mpl-price',
  displayName: 'Get MPL Token Price',
  description:
    'Fetch the current price, market cap, 24h volume, and price change for the MPL governance token via CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/maple',
      queryParams: {
        localization: 'false',
        tickers: 'false',
