...
export interface CodaUpdateRowPayload {
    // Used for PUT (update single row)
    row: CodaRowEdit
}
...
export interface CodaAPIClient {
    listTables: (
        docId: string,
        params?: { limit?: number; sortBy?: string; tableTypes?: string; pageToken?: string },
    ) => Promise<CodaListTablesResponse>
    getTableDetails: (
        docId: string,
        tableIdOrName: string,
        params?: { useUpdatedTableLayouts?: boolean },
    ) => Promise<CodaGetTableDetailsResponse>
    listColumns: (
        docId: string,
        tableIdOrName: string,
        params?: { limit?: number; pageToken?: string; visibleOnly?: boolean },
    ) => Promise<CodaListColumnsResponse>
    getRow: (
        docId: string,
        tableIdOrName: string,
        rowIdOrName: string,
        params?: { useColumnNames?: boolean; valueFormat?: string },
    ) => Promise<CodaGetRowResponse>
    listRows: (
        docId: string,
        tableIdOrName: string,
        params?: {
    ...
    mutateRows: (
        docId: string,
        tableIdOrName: string,
        payload: CodaMutateRowsPayload,
        params?: { disableParsing?: boolean },
    ) => Promise<CodaMutateRowsResponse>
    updateRow: (
        docId: string,
        tableIdOrName: string,
        rowIdOrName: string,
        payload: CodaUpdateRowPayload,
        params?: { disableParsing?: boolean },
...
export const codaClient = ({ secret_text }: AppConnectionValueForAuthProperty<typeof codaAuth>): CodaAPIClient => {
    ...
    return {
        ...
        getTableDetails: async (docId, tableIdOrName, params) => {
            ...
            if (params?.useUpdatedTableLayouts !== undefined)
                queryParams['useUpdatedTableLayouts'] = params.useUpdatedTableLayouts
