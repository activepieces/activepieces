...
export const extractFileData = createAction({
    ...
    props: {
        file: Property.File({
            displayName: 'File',
            description:
                'The file to upload for extraction. Supported: PDF, images (JPEG, PNG, TIFF, BMP), Word docs (DOC, DOCX), text files (TXT)',
            required: true,
        }),
        ...
        fields: Property.Array({
            ...
            properties: {
                key: Property.ShortText({
                    ...
                    description: 'Unique identifier for this field (e.g., "name", "email")',
                    required: true,
                }),
                ...
                example: Property.ShortText({
                    ...
                    description: 'Sample value to help the AI understand the expected format',
                    required: false,
                }),
    ...
    async run(context) {
        ...
        if (!fileExtension || !SUPPORTED_FILE_TYPES.includes(fileExtension)) {
            throw new Error(`File type not supported. Supported types: ${SUPPORTED_FILE_TYPES.join(', ')}`)
        }

        if (context.propsValue.specificPageProcessing) {
            if (!context.propsValue.pageFrom || !context.propsValue.pageTo) {
                throw new Error('Page From and Page To are required when Specific Page Processing is enabled')
            }
            if (context.propsValue.pageFrom > context.propsValue.pageTo) {
        ...
        const apiFields = (context.propsValue.fields || []).map((field: any) => {
            const baseField: any = {
                ...
                type: field.type === 'array_string' || field.type === 'array_object' ? 'array' : field.type,
            }
        ...
        try {
            ...
            if (error.response) {
                ...
                switch (status) {
                    ...
                    case 400:
                        throw new Error(`Invalid request: ${body.message || JSON.stringify(body)}`)
                    default:
