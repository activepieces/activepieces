        ...
        options: async ({ auth, solutionId }) => {
            if (!auth || !solutionId) {
                return {
                    disabled: true,
                    options: [],
                    placeholder: solutionId ? 'Please select a solution first.' : 'Please enter your API key first.',
                }
            }
...
export function formatRecordFields(
    ...
    const fieldMap: Record<string, string> = tableSchema.reduce(
        (acc, field) => {
...
export function transformRecordFields(tableSchema: TableStucture[], tableValues: Record<string, any>) {
    const fieldMap: Record<string, string> = tableSchema.reduce(
        (acc, field) => {
