...
export const updatedRecordTrigger = createTrigger({
  ...
  description: 'Triggered when a record is updated in SmartSuite',
  props: {
...
Note: SmartSuite webhooks must receive payloads every 7 days to stay active.
  ...
  async run(context) {
    ...
    // Validate that this is a record updated event
    if (payload.event_type !== 'record_updated') {
      return [];
    }
