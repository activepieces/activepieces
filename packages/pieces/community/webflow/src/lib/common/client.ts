import { AuthenticationType, HttpMethod, HttpRequest, httpClient, QueryParams } from '@activepieces/pieces-common'
...
export class WebflowApiClient {
    ...
    async createCollectionItem(collectionId: string, request: Record<string, any>) {
        return await this.makeRequest(HttpMethod.POST, `/collections/${collectionId}/items`, undefined, request)
    }
    ...
    async deleteCollectionItem(collectionId: string, itemId: string) {
        return await this.makeRequest(HttpMethod.DELETE, `/collections/${collectionId}/items/${itemId}`)
    }

    async publishCollectionItem(collectionId: string, itemId: string) {
        return await this.makeRequest(HttpMethod.POST, `/collections/${collectionId}/items/publish`, undefined, {
            itemIds: [itemId],
        })
    }
    ...
    async fulfillOrder(siteId: string, orderId: string, request: Record<string, any>) {
        return await this.makeRequest(HttpMethod.POST, `/sites/${siteId}/orders/${orderId}/fulfill`, undefined, request)
    }

    async unfulfillOrder(siteId: string, orderId: string) {
        return await this.makeRequest(HttpMethod.POST, `/sites/${siteId}/orders/${orderId}/unfulfill`, undefined)
    }
