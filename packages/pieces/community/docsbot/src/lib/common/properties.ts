...
const teamProperty = ({ displayName, description }: { displayName?: string; description?: string }) =>
    Property.Dropdown({
        auth: docsbotAuth,
...
const botProperty = ({ displayName, description }: { displayName?: string; description?: string }) =>
    Property.ShortText({
        ...
        props: async ({ type }) => {
            return {
                ...
                url: Property.ShortText({
                    ...
                    required:
                        typeof type === 'string' && ['url', 'sitemap', 'youtube', 'rss'].includes(type) ? true : false,
                }),
                file: Property.ShortText({
                    ...
                    required:
                        typeof type === 'string' && ['urls', 'csv', 'document', 'wp'].includes(type) ? true : false,
                }),
                faqs: Property.Array({
                    displayName: 'FAQs',
