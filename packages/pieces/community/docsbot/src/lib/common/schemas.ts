...
export const createSource = z
    .object({
        type: z.enum(['url', 'document', 'sitemap', 'wp', 'urls', 'csv', 'rss', 'qa', 'youtube']),
    ...
    .superRefine((data, ctx) => {
        ...
        if (['url', 'sitemap', 'youtube', 'rss'].includes(data.type) && (!data.url || data.url.trim() === '')) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ['url'],
        ...
        if (['urls', 'csv', 'document', 'wp'].includes(data.type) && (!data.file || data.file.trim() === '')) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ['file'],
