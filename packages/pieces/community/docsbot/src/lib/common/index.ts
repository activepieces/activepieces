...
export const docsbotCommon = {
    ...
    endpoints: {
        ...
        createBot: (teamId: string) => `${docsbotCommon.baseUrl}/teams/${teamId}/bots`,
        ...
        listBots: (teamId: string) => `${docsbotCommon.baseUrl}/teams/${teamId}/bots`,
    },
    ...
    // Methods
    askQuestion: async ({ apiKey, teamId, botId, ...chatParams }: askQuestionRequestParams) => {
    ...
    createSource: async ({ apiKey, teamId, botId, ...sourceParams }: createSourceRequestParams) => {
        const response = await httpClient.sendRequest({
            method: HttpMethod.POST,
    ...
    createBot: async ({ apiKey, teamId, ...botParams }: createBotRequestParams) => {
        const response = await httpClient.sendRequest({
            method: HttpMethod.POST,
    ...
    createPresignedFileUploadURL: async ({ apiKey, teamId, botId, fileName }: uploadSourceFileRequestParams) => {
    ...
    uploadFileToCloudStorage: async ({ uploadUrl, file }: uploadToCloudStorageParams) => {
        const response = await httpClient.sendRequest({
