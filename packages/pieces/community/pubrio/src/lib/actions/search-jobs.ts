...
export const searchJobs = createAction({
  ...
  props: {
    ...
    titles: Property.ShortText({
      displayName: 'Job Titles',
      description: 'Comma-separated job titles',
      required: false,
    }),
    posted_dates: Property.ShortText({
  ...
  async run(context) {
    ...
    if (context.propsValue.titles)
      body['titles'] = splitComma(context.propsValue.titles);
    if (context.propsValue.posted_dates)
      body['posted_dates'] = splitComma(context.propsValue.posted_dates);
