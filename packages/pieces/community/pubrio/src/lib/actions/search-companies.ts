...
export const searchCompanies = createAction({
	...
	props: {
		...
		linkedin_urls: Property.ShortText({ displayName: 'LinkedIn URLs', description: 'Comma-separated LinkedIn company URLs', required: false }),
		...
		places: Property.ShortText({ displayName: 'Places', description: 'Comma-separated place names', required: false }),
		exclude_places: Property.ShortText({ displayName: 'Exclude Places', description: 'Comma-separated place names to exclude', required: false }),
		...
		exclude_fields: Property.ShortText({ displayName: 'Exclude Fields', description: 'Comma-separated field names to exclude', required: false }),
		...
		job_exclude_locations: Property.ShortText({ displayName: 'Job Exclude Locations', description: 'Comma-separated country codes to exclude', required: false }),
		job_posted_dates: Property.ShortText({ displayName: 'Job Posted Dates', description: 'Comma-separated date range YYYY-MM-DD', required: false }),
		...
		news_published_dates: Property.ShortText({ displayName: 'News Published Dates', description: 'Comma-separated date range YYYY-MM-DD', required: false }),
		news_galleries: Property.ShortText({ displayName: 'News Galleries', description: 'Comma-separated news gallery slugs', required: false }),
		news_gallery_ids: Property.ShortText({ displayName: 'News Gallery IDs', description: 'Comma-separated news gallery UUIDs', required: false }),
		advertisement_search_terms: Property.ShortText({ displayName: 'Advertisement Search Terms', description: 'Comma-separated keywords', required: false }),
		advertisement_target_locations: Property.ShortText({ displayName: 'Advertisement Target Locations', description: 'Comma-separated country codes', required: false }),
		advertisement_exclude_target_locations: Property.ShortText({ displayName: 'Advertisement Exclude Target Locations', description: 'Comma-separated country codes', required: false }),
		advertisement_start_dates: Property.ShortText({ displayName: 'Advertisement Start Dates', description: 'Comma-separated date range YYYY-MM-DD', required: false }),
		advertisement_end_dates: Property.ShortText({ displayName: 'Advertisement End Dates', description: 'Comma-separated date range YYYY-MM-DD', required: false }),
	...
	async run(context) {
		...
		if (context.propsValue.places) body.places = splitComma(context.propsValue.places);
		if (context.propsValue.exclude_places) body.exclude_places = splitComma(context.propsValue.exclude_places);
		...
		if (context.propsValue.revenue_min != null) body.revenue_min = context.propsValue.revenue_min;
		if (context.propsValue.revenue_max != null) body.revenue_max = context.propsValue.revenue_max;
		if (context.propsValue.founded_year_start != null) body.founded_year_start = context.propsValue.founded_year_start;
		if (context.propsValue.founded_year_end != null) body.founded_year_end = context.propsValue.founded_year_end;
		...
		if (context.propsValue.exclude_fields) body.exclude_fields = splitComma(context.propsValue.exclude_fields);
		...
		if (context.propsValue.job_exclude_locations) body.job_exclude_locations = splitComma(context.propsValue.job_exclude_locations);
		if (context.propsValue.job_posted_dates) body.job_posted_dates = splitComma(context.propsValue.job_posted_dates);
		if (context.propsValue.news_categories) body.news_categories = splitComma(context.propsValue.news_categories);
		if (context.propsValue.news_published_dates) body.news_published_dates = splitComma(context.propsValue.news_published_dates);
		...
		if (context.propsValue.advertisement_search_terms) body.advertisement_search_terms = splitComma(context.propsValue.advertisement_search_terms);
		if (context.propsValue.advertisement_target_locations) body.advertisement_target_locations = splitComma(context.propsValue.advertisement_target_locations);
		if (context.propsValue.advertisement_exclude_target_locations) body.advertisement_exclude_target_locations = splitComma(context.propsValue.advertisement_exclude_target_locations);
		if (context.propsValue.advertisement_start_dates) body.advertisement_start_dates = splitComma(context.propsValue.advertisement_start_dates);
		if (context.propsValue.advertisement_end_dates) body.advertisement_end_dates = splitComma(context.propsValue.advertisement_end_dates);
