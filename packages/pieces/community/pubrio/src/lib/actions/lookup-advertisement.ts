...
export const lookupAdvertisement = createAction({
	...
	description: 'Look up detailed advertisement information by advertisement search ID',
	props: {
		advertisement_search_id: Property.ShortText({ displayName: 'Advertisement Search ID', required: true, description: 'The advertisement search ID to look up' }),
	},
	async run(context) {
