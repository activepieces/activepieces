...
export const searchNews = createAction({
	...
	props: {
		...
		published_date_from: Property.ShortText({ displayName: 'Published Date From', description: 'YYYY-MM-DD', required: false }),
		published_date_to: Property.ShortText({ displayName: 'Published Date To', description: 'YYYY-MM-DD', required: false }),
	...
	async run(context) {
		...
		const pubFrom = context.propsValue.published_date_from;
		const pubTo = context.propsValue.published_date_to;
		if (pubFrom || pubTo) body.published_dates = [pubFrom || pubTo, pubTo || pubFrom];
