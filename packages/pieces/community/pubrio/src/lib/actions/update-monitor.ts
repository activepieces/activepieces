...
export const updateMonitor = createAction({
	...
	props: {
		...
		is_active: Property.Checkbox({ displayName: 'Is Active', required: false, defaultValue: false }),
		is_paused: Property.Checkbox({ displayName: 'Is Paused', required: false, defaultValue: false }),
		max_failure_trigger: Property.Number({ displayName: 'Max Failure Trigger', required: false }),
		max_retry_per_trigger: Property.Number({ displayName: 'Max Retry Per Trigger', required: false }),
		retry_delay_second: Property.Number({ displayName: 'Retry Delay (seconds)', required: false }),
	...
	async run(context) {
		...
		if (context.propsValue.company_filters) {
			try {
				...
				throw new Error('company_filters must be valid JSON');
			}
		}
		if (context.propsValue.signal_filters) {
			try {
				...
				throw new Error('signal_filters must be valid JSON');
			}
		}
		if (context.propsValue.people_enrichment_configs) {
			try {
				body.people_enrichment_configs = JSON.parse(context.propsValue.people_enrichment_configs);
			} catch {
				throw new Error('people_enrichment_configs must be valid JSON');
			}
		}
		if (context.propsValue.is_company_enrichment != null) body.is_company_enrichment = context.propsValue.is_company_enrichment;
		if (context.propsValue.is_people_enrichment != null) body.is_people_enrichment = context.propsValue.is_people_enrichment;
		if (context.propsValue.is_active != null) body.is_active = context.propsValue.is_active;
		if (context.propsValue.is_paused != null) body.is_paused = context.propsValue.is_paused;
