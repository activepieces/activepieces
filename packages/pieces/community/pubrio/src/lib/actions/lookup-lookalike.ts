...
export const lookupLookalike = createAction({
	...
	description: 'Look up a similar company result by domain, LinkedIn URL, or domain search ID',
	props: {
		...
		value: Property.ShortText({ displayName: 'Value', required: true, description: 'Domain, LinkedIn URL, or Domain Search ID' }),
	},
	async run(context) {
		const body: Record<string, unknown> = {
			[context.propsValue.lookup_type]: context.propsValue.value,
		};
		return await pubrioRequest(context.auth as string, HttpMethod.POST, '/companies/lookalikes/lookup', body);
	},
});
