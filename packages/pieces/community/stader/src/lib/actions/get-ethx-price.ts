...
export const getEthxPrice = createAction({
  auth: PieceAuth.None(),
  name: 'get_ethx_price',
  displayName: 'Get ETHx Price',
  description: 'Get the current USD price, market cap, and 24h trading volume of ETHx — the Stader Ethereum liquid staking token.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest({
      method: HttpMethod.GET,
      url: `${COINGECKO_BASE_URL}/coins/stader-ethx`,
      queryParams: {
        localization: 'false',
        tickers: 'false',
