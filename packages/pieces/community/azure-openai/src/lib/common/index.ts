...
export const calculateMessagesTokenSize = async (messages: string[], model: string) => {
  let tokenLength = 0
  await Promise.all(
...
export const reduceContextSize = async (messages: string[], model: string, maxTokens: number) => {
