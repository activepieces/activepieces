...
export const createSubmission = createAction({
    ...
    props: {
        ...
        form_fields: Property.DynamicProperties({
            ...
            props: async ({ auth, form_id }) => {
                ...
                try {
                    const formDetails = await makeRequest(accessToken, HttpMethod.GET, `/form/${form_id}.json`)
                            ...
                            switch (field.type?.toLowerCase()) {
                                            ...
                                            options = optionsData
                                                .map((option: any) => {
                                                    if (typeof option === 'string') {
                                                        ...
                                                        const label =
                                                            option.label || option.text || option.option || option.value
                                                        const value =
                                                            option.value || option.label || option.text || option.option
                                ...
                                case 'phone':
                                    fields[fieldKey] = Property.ShortText({
                                        ...
                                        description: field.description
                                            ? `${field.description} (Format: (XXX) XXX-XXXX)`
                                ...
                                case 'file':
                                    fields[fieldKey] = Property.File({
                                        displayName: fieldLabel,
                                        description: field.description
                                            ? `${field.description} (File will be base64 encoded automatically)`
                                            : 'Upload a file (will be base64 encoded automatically)',
                                        required: isRequired,
                                    })
                                    break
                                ...
                                case 'address': {
                                    const addressSubfields = field.visible_subfields || [
                                        'address',
                                        'city',
                                        'state',
                                        'zip',
    ...
    async run(context) {
        ...
        const { form_id, user_agent, remote_addr, payment_status, read, encryption_password, form_fields } =
            context.propsValue
        ...
        const response = await makeFormRequest(accessToken, HttpMethod.POST, `/form/${form_id}/submission`, formData)
