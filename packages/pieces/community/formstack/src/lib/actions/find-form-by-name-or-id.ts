...
export const findFormByNameOrId = createAction({
    ...
    async run(context) {
        ...
        if (isNumericId) {
            try {
                const response = await makeRequest(accessToken, HttpMethod.GET, `/form/${search_query}.json`, {})
            ...
            try {
                ...
                if (exact_match) {
                    matchingForms = matchingForms.filter(
                        (form: any) => form.name.toLowerCase() === search_query.toLowerCase(),
                    )
                }
