...
export async function makeRequest(
    ...
    const { format = RequestFormat.JSON, useQueryAuth = false } = options
    ...
    try {
        ...
        switch (status) {
            ...
            case 401:
                throw new Error(
                    `Unauthorized: Valid OAuth2 credentials were not supplied. Please check your access token.`,
                )

            case 403:
                throw new Error(
                    `Forbidden: You do not have access to this resource. Check your user permissions in Formstack.`,
                )
            ...
            case 429:
                throw new Error(
                    `Rate Limit Exceeded: You have hit the daily limit of ${RATE_LIMIT.DAILY_LIMIT} API calls. Try again in 24 hours.`,
                )
