...
const polling: Polling<AppConnectionValueForAuthProperty<typeof formStackAuth>, any> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        ...
        try {
            const formsResponse = await makeRequest(accessToken, HttpMethod.GET, '/form.json')
            ...
            return [
                {
                    ...
                    data: {
                        message:
                            'No recently created forms found. The trigger is working and will detect new forms when they are created.',
