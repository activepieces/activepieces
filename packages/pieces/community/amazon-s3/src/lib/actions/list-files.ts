...
export const listFiles = createAction({
    ...
    props: {
        prefix: Property.ShortText({
            ...
            description:
                'Filter results to a specific folder. Enter the folder path including the trailing slash (e.g. "invoices/" or "reports/2024/"). Leave empty to list all files in the bucket.',
            required: false,
        }),
        maxKeys: Property.Number({
            displayName: 'Maximum Files',
            description:
                'Maximum number of files to return (1–1000). Defaults to 1000. Use a lower number if you only need the most recent files.',
            required: false,
            defaultValue: 1000,
        }),
    },
