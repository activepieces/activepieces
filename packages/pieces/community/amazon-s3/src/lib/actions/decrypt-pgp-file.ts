...
export const decryptPgpFile = createAction({
    ...
    props: {
        ...
        passphraseArn: Property.ShortText({
            ...
            description:
                'Optional ARN of the secret in AWS Secrets Manager containing the passphrase for the private key (if the key is encrypted)',
            required: false,
        }),
        ...
        allowUnauthenticatedMessages: Property.Checkbox({
            ...
            description:
                'Allow decryption of messages without integrity protection (no MDC). WARNING: Without MDC, a malicious actor can silently modify the ciphertext, so the decrypted output may have been tampered with. Only enable this when working with legacy files where integrity protection was never used.',
            required: false,
            defaultValue: false,
        }),
        allowInsecureDecryptionWithSigningKeys: Property.Checkbox({
            ...
            description:
                'Allow decryption using a key not marked for encryption (e.g. a signing-only key). Enable this if your PGP key was not flagged for encryption use.',
            required: false,
            defaultValue: false,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            if (!privateKey.isPrivate()) {
                throw new Error(
                    'The provided key is not a private key. Please ensure you are using a private key, not a public key.',
                )
            }
            ...
            if (!privateKey.isDecrypted()) {
                if (!passphrase) {
                    throw new Error(
                        'Private key is encrypted but no passphrase was provided. Please provide a Passphrase ARN.',
                    )
                }
                try {
                    ...
                    throw new Error(
                        `Failed to unlock private key with passphrase: ${error instanceof Error ? error.message : 'Unknown error'}`,
                    )
                }
            }
