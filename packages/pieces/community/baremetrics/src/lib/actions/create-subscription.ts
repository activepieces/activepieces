...
import { BaremetricsAuth, baremetricsApiCall } from '../common/client'
...
export const createSubscription = createAction({
    ...
    props: {
            ...
            options: async ({ auth }) => {
                ...
                try {
                    const response = await baremetricsApiCall<{
                        sources: Array<{ id: string; provider: string; provider_id: string }>
                    }>({
        ...
        customer_oid: Property.Dropdown({
            ...
            options: async (propsValue) => {
                ...
                try {
                    const response = await baremetricsApiCall<{
                        customers: Array<{ oid: string; name?: string; email?: string }>
                    }>({
                        method: HttpMethod.GET,
        ...
        plan_oid: Property.Dropdown({
            ...
            options: async (propsValue) => {
                ...
                try {
                    const response = await baremetricsApiCall<{
                        plans: Array<{ oid: string; name: string; amount: number; currency: string }>
                    }>({
                        method: HttpMethod.GET,
                        path: `/${sourceId}/plans`,
                        auth: auth as BaremetricsAuth,
    ...
    async run(context) {
        const { source_id, oid, started_at, canceled_at, customer_oid, plan_oid, quantity, discount, addons } =
            context.propsValue
