...
export const newSubscriberTrigger = createTrigger({
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await makeSenderRequest(context.auth.secret_text, `/account/webhooks/${webhookId}`, HttpMethod.DELETE)
        }
    ...
    async test(context) {
        const response = await makeSenderRequest(context.auth.secret_text, '/subscribers?limit=1', HttpMethod.GET)
