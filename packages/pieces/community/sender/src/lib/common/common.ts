...
export const groupIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response: any = await makeSenderRequest(auth.secret_text, '/groups', HttpMethod.GET)
...
export const subscribersDropdown = Property.MultiSelectDropdown<string, true, typeof senderAuth>({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response: any = await makeSenderRequest(auth.secret_text, '/subscribers?limit=50', HttpMethod.GET)
