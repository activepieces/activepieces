...
export const updateBoard = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (!name && description === undefined && !privacy) {
            throw new Error('At least one field (name, description, or privacy) must be provided to update the board.')
        }
        ...
        try {
            return await makeRequest(getAccessTokenOrThrow(auth), HttpMethod.PATCH, path, body)
        } catch (error) {
