import { DedupeStrategy, getAccessTokenOrThrow, HttpMethod, Polling, pollingHelper } from '@activepieces/pieces-common'
import {
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof pinterestAuth>, Record<string, any>> = {
    ...
    items: async ({ propsValue, auth, lastFetchEpochMS }) => {
        ...
        do {
            ...
            const path = `/boards/${board_id}/pins${queryString ? `?${queryString}` : ''}`
            try {
                ...
                if (lastFetchEpochMS) {
                    const newItems = items.filter((item: any) => dayjs(item.created_at).valueOf() > lastFetchEpochMS)
                    ...
                    const hasOldItems = items.some((item: any) => dayjs(item.created_at).valueOf() <= lastFetchEpochMS)
                    if (hasOldItems) {
                        break
                    }
        ...
        pins.sort((a, b) => dayjs(b.created_at).valueOf() - dayjs(a.created_at).valueOf())
...
export const newPinOnBoard = createTrigger({
    ...
    props: {
        ...
        creative_types: Property.StaticMultiSelectDropdown({
            ...
            description: 'Filter by specific pin types. Leave empty to watch all types.',
        }),
    },
    sampleData: {
        items: [
