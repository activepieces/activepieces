...
export async function makeRequest(apiKey: string, method: HttpMethod, path: string, body?: unknown) {
    try {
        ...
        if (error.response?.status) {
            ...
            switch (status) {
                case 400:
                    throw new Error(
                        `Bad Request: ${
                            errorData?.message || 'Invalid request parameters. Please check your input and try again.'
                        }`,
                    )
                ...
                default:
                    throw new Error(
                        `Pinterest API Error (${status}): ${
                            errorData?.message || 'An unexpected error occurred while communicating with Pinterest.'
                        }`,
                    )
            }
        }
        ...
        throw new Error(
            `Unexpected Error: ${error.message || 'An unexpected error occurred while processing your request.'}`,
        )
    }
}
