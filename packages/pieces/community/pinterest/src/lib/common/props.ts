...
export const boardIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, ad_account_id }) => {
        ...
        try {
            ...
            const boards = await makeRequest(accessToken, HttpMethod.GET, url.pathname + url.search)
...
export const adAccountIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const accessToken = getAccessTokenOrThrow(auth as OAuth2PropertyValue)
            const adAccounts = await makeRequest(accessToken, HttpMethod.GET, '/ad_accounts')

            const options: DropdownOption<string>[] = adAccounts.items.map((account: any) => ({
                label: account.name,
                value: account.id,
            }))
...
export const boardSectionIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, board_id }) => {
        ...
        try {
            const accessToken = getAccessTokenOrThrow(auth as OAuth2PropertyValue)
            const boardsections = await makeRequest(accessToken, HttpMethod.GET, `/boards/${board_id}/sections`)

            const options: DropdownOption<string>[] = boardsections.items.map((section: any) => ({
                label: section.name,
                value: section.id,
            }))
