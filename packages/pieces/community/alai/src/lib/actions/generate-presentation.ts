...
export const generatePresentation = createAction({
    ...
    props: {
        ...
        themeId: Property.ShortText({
            ...
            description: 'The ID of the presentation theme to use. If not provided, a default theme will be applied.',
            required: false,
        }),
        slideRange: Property.ShortText({
            displayName: 'Slide Range',
            description: 'Target slide count: auto, 1, 2-5, 6-10, 11-15, 16-20, 21-25, 26-50.',
            required: false,
        }),
        ...
        waitForCompletion: Property.Checkbox({
            ...
            description: 'Wait for the presentation generation to complete before returning.',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        ...
        if (!generationId) {
            throw new Error(`Failed to get generation_id from response: ${JSON.stringify(response.body)}`)
        }
        ...
        while (Date.now() - startTime < maxWait) {
            ...
            if (status === 'completed') {
                ...
                throw new Error(`Generation failed: ${statusResponse.body?.error || 'Unknown error'}`)
            }
