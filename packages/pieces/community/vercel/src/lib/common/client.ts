...
export async function listAllProjects(auth: VercelAuthValue, search?: string): Promise<VercelProject[]> {
    ...
    while (pageCount < MAX_PAGES) {
        ...
        const next: number | string | null | undefined = Array.isArray(response) ? undefined : response.pagination?.next
        if (!next) {
            break
        }
...
export async function listDeployments(auth: VercelAuthValue, projectId: string): Promise<VercelDeployment[]> {
