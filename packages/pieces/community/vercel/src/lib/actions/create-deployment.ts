...
import { VercelAuthValue, vercelAuth } from '../common/auth'
...
export const createDeployment = createAction({
            ...
            props: async ({ auth, deployment_source, project }) => {
                ...
                if (deployment_source === 'redeploy') {
                    ...
                    fields['with_latest_commit'] = Property.Checkbox({
                        ...
                        description:
                            'When enabled, redeploys using the latest commit instead of the original deployment files.',
                        required: false,
                        defaultValue: false,
                    })
                    return fields
    ...
    async run(context) {
        const { project, deployment_source, source_fields, force_new, skip_auto_detection_confirmation } =
            context.propsValue
        ...
        if (deployment_source === 'redeploy') {
            ...
            if (git_type === 'github' || git_type === 'github-limited') {
                if (git_repo_id) {
                    ...
                    throw new Error(
                        'For GitHub deployments, provide either Repository ID or both Repository Organization and Repository Name.',
                    )
                }
                ...
                if (git_repo_uuid) {
                    ...
                    throw new Error(
                        'For Bitbucket deployments, provide either Repository UUID or both Organization / Workspace and Repository Name / Slug.',
                    )
                }
            } else {
