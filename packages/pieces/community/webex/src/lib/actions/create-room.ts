...
export const createRoom = createAction({
    ...
    description: 'Create a new Webex room (space). The authenticated user is automatically added as a member.',
    props: {
        ...
        teamId: Property.ShortText({
            displayName: 'Team ID',
            description: 'The ID for the team with which this room is associated (optional)',
            required: false,
        }),
        ...
        isLocked: Property.Checkbox({
            ...
            description: 'Set the space as locked/moderated. The creator becomes a moderator.',
            required: false,
        }),
    ...
    async run(context) {
        const { title, description, teamId, classificationId, isPublic, isLocked, isAnnouncementOnly } =
            context.propsValue
        ...
        if (isAnnouncementOnly && !isLocked) {
            throw new Error('Room must be locked/moderated to enable announcement-only mode')
        }
        ...
        const response = await makeRequest(context.auth.access_token, HttpMethod.POST, '/rooms', body)
