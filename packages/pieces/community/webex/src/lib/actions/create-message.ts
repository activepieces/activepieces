...
export const createMessage = createAction({
    ...
    description: 'Post a plain text or rich text message to a Webex room or send a direct message',
    props: {
        messageType: Property.StaticDropdown({
            displayName: 'Message Type',
            description: 'Choose between sending to a room or a direct message to a person',
            required: true,
            options: {
                options: [
    ...
    async run(context) {
        const { messageType, destination, text, markdown, parentId } = context.propsValue
        ...
        if (messageType === 'room') {
            ...
            if (!toPersonId && !toPersonEmail) {
                throw new Error('Either Person ID or Person Email is required for direct messages')
            }
        ...
        const response = await makeRequest(context.auth.access_token, HttpMethod.POST, '/messages', body)
