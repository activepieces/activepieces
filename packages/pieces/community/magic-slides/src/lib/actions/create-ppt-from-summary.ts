...
export const createPptFromSummary = createAction({
  ...
  name: 'create_ppt_from_summary',
  displayName: 'Create PPT from Summary',
  description: 'Creates a new presentation from a given text summary.',
  props: {
    summary: Property.LongText({
      displayName: 'Summary Text',
      description: 'The summary text to convert into a presentation.',
      required: true,
    }),
  ...
  async run(context) {
    ...
    const { summary, email, ...optionalProps } = propsValue;
    ...
    const response = await httpClient.sendRequest<{
      ...
      url: 'https://api.magicslides.app/public/api/ppt_from_summery',
      body: {
