...
import { campaignIdProp, emailsProp, leadIdsDropdownProp } from '../common/props'
...
export const addRecipientsAction = createAction({
    ...
    async run(context) {
        ...
        const stringEmails = rawEmails.filter((e): e is string => typeof e === 'string')
        const numberLeadIds = rawLeadIds.filter((id): id is number => typeof id === 'number')

        const filteredEmails = stringEmails.map((e) => e.trim()).filter((e) => e.length > 0)
        ...
        const resp = (await hunterApiCall({
            ...
            data: {
                ...
                skipped_recipients: Array<{
                    email: string
                    reason: 'duplicate' | 'invalid' | 'removed' | 'bounced' | 'unsubscribed' | 'claimed'
                }>
            }
        }
