...
export const verifyEmailAction = createAction({
    ...
    async run(context) {
        ...
        try {
            ...
            if (status === 222) {
                throw new Error('Verification failed due to an unexpected SMTP server error. Please try again later.')
            }
            if (status === 400) {
                switch (errId) {
                    case 'wrong_params':
            ...
            if (status === 429) {
                throw new Error('Rate limit exceeded (10 requests/sec, 300 req/min). Please wait and retry shortly.')
            }
            if (status === 451 && errId === 'claimed_email') {
                throw new Error('Cannot verify: the email owner requested no processing of their data.')
            }
