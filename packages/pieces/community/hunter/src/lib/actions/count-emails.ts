...
import { companyProp, domainProp, emailTypeProp } from '../common/props'
...
export const countEmailsAction = createAction({
    ...
    async run(context) {
        const { domain, company, type } = context.propsValue as {
            domain?: string
            company?: string
        ...
        try {
            ...
            const details = (httpErr.response?.body as any)?.errors?.[0]?.details || 'Unknown error'
