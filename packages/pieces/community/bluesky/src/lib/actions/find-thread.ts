...
import { extractPostInfoFromUrl, parentHeightDropdown, postUrlProperty, threadDepthDropdown } from '../common/props'
...
export const findThread = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        try {
            ...
            if (!atUri) {
                throw new Error(
                    'Could not parse the post URL. Please make sure you are using a valid Bluesky post URL like: https://bsky.app/profile/username.bsky.social/post/xxx',
                )
            }
