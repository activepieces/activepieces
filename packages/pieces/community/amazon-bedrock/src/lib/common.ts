...
export function createBedrockRuntimeClient(auth: BedrockAuth): BedrockRuntimeClient {
    ...
    try {
        ...
        if (filters?.useInferenceProfiles) {
            const [profileResponse, modelsResponse] = await Promise.all([
                ...
                filters?.inputModality ? client.send(new ListFoundationModelsCommand({})) : Promise.resolve(undefined),
            ])
            ...
            if (filters?.inputModality && modelsResponse) {
                ...
                allowedModelIds = new Set(
                    allModels
                        .filter((m) => m.inputModalities?.includes(filters.inputModality!))
                        .map((m) => m.modelId!)
                        .filter(Boolean),
                )
            }
        ...
        const response = await client.send(
            new ListFoundationModelsCommand({
                byOutputModality: filters?.showAll ? undefined : (filters?.outputModality ?? ModelModality.TEXT),
            }),
        )
        const models = response.modelSummaries ?? []

        const filtered = filters?.inputModality
            ? models.filter((m) => m.inputModalities?.includes(filters.inputModality!))
            : models
