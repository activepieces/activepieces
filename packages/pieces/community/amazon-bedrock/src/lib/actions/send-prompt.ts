import { createAction, DynamicPropsValue, Property, StoreScope } from '@activepieces/pieces-framework'
import { ContentBlock, ConversationRole, ConverseCommand, Message } from '@aws-sdk/client-bedrock-runtime'
...
export const sendPrompt = createAction({
    ...
    props: {
        ...
        systemPrompt: Property.LongText({
            ...
            required: false,
            description:
                'Instructions that set the model\'s behavior and persona (e.g. "You are a helpful assistant that replies concisely.").',
        }),
        ...
        maxTokens: Property.Number({
            ...
            required: false,
            description:
                'The maximum length of the response. One token is roughly 4 characters. 2048 tokens ≈ ~1500 words.',
            defaultValue: 2048,
        }),
        ...
        attachmentSource: Property.StaticDropdown({
            ...
            description:
                'Optionally include a file alongside your prompt. Choose whether to upload directly or reference one from S3.',
            required: false,
            options: {
                options: [
                ...
                if (attachmentSource === 'file') {
                    return {
                        file: Property.File({
                            displayName: 'File',
                            description:
                                'Supported: images (png, jpg, gif, webp), documents (pdf, csv, doc, docx, xls, xlsx, html, txt, md), videos (mp4, mov, mkv, etc.), audio (mp3, wav, aac, flac, etc.). Not all models support all file types.',
                            required: true,
                        }),
                    }
                }
                if (attachmentSource === 's3') {
                    return {
                        ...
                        s3Key: Property.ShortText({
                            ...
                            description:
                                'The path to the file in your S3 bucket (e.g. "documents/report.pdf"). The file extension determines the media type sent to the model.',
                            required: true,
                        }),
                    }
                }
    ...
    async run({ auth, propsValue, store }) {
        ...
        if (memoryKey) {
            messageHistory = (await store.get<Message[]>(memoryKey, StoreScope.PROJECT)) ?? []
        }
            ...
            const response = await client.send(
                new ConverseCommand({
                    ...
                    inferenceConfig: {
                        ...
                        topP: topP ?? undefined,
                        stopSequences:
                            stopSequences && stopSequences.length > 0 ? (stopSequences as string[]) : undefined,
                    },
                }),
            )
