...
export const customApiCall = createAction({
    ...
    props: {
        ...
        method: Property.StaticDropdown({
            displayName: 'Method',
            description:
                'HTTP method for the request. Use GET to fetch data, POST/PUT to create or update, DELETE to remove.',
            required: true,
        ...
        failsafe: Property.Checkbox({
            ...
            description:
                'When enabled, errors (e.g. 4xx/5xx responses) are returned as output instead of stopping the flow. Useful when you want to handle errors in a later step.',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        const { service, method, path, headers, queryParams, body, failsafe, timeout } = context.propsValue
