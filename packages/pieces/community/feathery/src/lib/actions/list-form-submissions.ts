...
export const listFormSubmissionsAction = createAction({
    ...
    props: {
        form_id: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const forms = await featheryCommon.apiCall<Array<{ id: string; name: string; active: boolean }>>({
    ...
    async run(context) {
        ...
        if (field_search && Array.isArray(field_search) && field_search.length > 0) {
            const fieldSearchArray = (field_search as Array<{ field_id: string; value: string }>).map((fs) => ({
        ...
        if (
            ...
            const fuzzySearch = {
                ...
                parameters: (fuzzy_search_parameters as Array<{ field_id: string; term: string; weight: number }>).map(
                    (p) => ({
                        field_id: p.field_id,
                        term: p.term,
                        weight: p.weight,
