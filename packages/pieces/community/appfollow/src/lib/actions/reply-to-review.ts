...
import { application_ext_idDropdown, collection_idDropdown, review_ID_Dropdown } from '../common/props'

export const replyToReview = createAction({
    auth: appfollowAuth,
    name: 'replyToReview',
    displayName: 'Reply to Review',
    description: 'Reply to a specific review within a date range for a selected application and collection',
    props: {
    ...
    async run(context) {
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, `/reviews/reply`, {
            ext_id: app_ext_id,
            review_id,
            answer_text,
        })
