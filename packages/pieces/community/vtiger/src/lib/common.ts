...
export async function getRecordReference(
    ...
    const vtigerInstance = await instanceLogin(auth.props.instance_url, auth.props.username, auth.props.password)
...
export const recordProperty = () =>
    Property.DynamicProperties({
        ...
        props: async ({ auth, id, elementType }) => {
            ...
            return generateElementFields(auth, elementType as unknown as string, {})
        },
    })
...
export const generateElementFields = async (
    ...
    if (describe_response.body.success) {
        ...
        const skipFields = ['id', 'modifiedtime', 'createdtime', 'modifiedby', 'created_user_id']
