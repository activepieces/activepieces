...
export const makeAPICall = createAction({
    ...
    props: {
        ...
        urlPath: Property.ShortText({
            ...
            description: "Deprecated. Use 'URL' instead. API endpoint's URL path (example: /me, /listtypes, /describe)",
            required: false,
        }),
    ...
    async run({ propsValue, auth }) {
        ...
        if (useRestAuth) {
            ...
            if (method !== HttpMethod.GET && !Object.keys(headers).some((k) => k.toLowerCase() === 'content-type')) {
