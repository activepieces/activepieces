...
import { elementTypeProperty, Field, getRecordReference, instanceLogin, Modules, VTigerAuthValue } from '../common'
    ...
    props: {
        ...
        id: Property.Dropdown({
            ...
            options: async ({ auth, elementType }) => {
                ...
                let instance = null
                while (!instance && c < 3) {
                    instance = await instanceLogin(auth.props.instance_url, auth.props.username, auth.props.password)
                    await sleep(1500)
                    c++
                }
                ...
                return {
                    options: await Promise.all(
                        response.body.result.map(async (record) => {
                            return {
                    ...
                    const generateField = async (field: Field) => {
                        ...
                        if (['string', 'text', 'mediumtext', 'phone', 'url', 'email'].includes(field.type.name)) {
                            ...
                            if (field.type.name === 'picklist') {
                                ...
                                options = await getRecordReference(auth, ['Users'])
                            } else if (field.type.refersTo) {
                                options = await getRecordReference(auth, field.type.refersTo ?? [])
                            } else {
