...
export const dynamicFormFields = Property.DynamicProperties({
    ...
    description:
        'Fill out the form fields with the data you want to submit. Field types and validation are automatically configured based on your form structure.',
    required: true,
                ...
                switch (field.Type) {
                    ...
                    case 'money':
                        props[fieldId] = Property.Number({
                            ...
                            description:
                                field.Type === 'money'
                                    ? `${fieldDescription} (Enter monetary amount)`
                                    : fieldDescription,
                            required: isRequired,
                    ...
                    case 'radio':
                        if (field.Choices && field.Choices.length > 0) {
                            const options = field.Choices.filter(
                                (choice) => choice.Label && choice.Label.trim() !== '',
                            ).map((choice) => ({
                                label: choice.Label,
                                value: choice.Label,
                            }))
                            ...
                            if (options.length > 0) {
                                props[fieldId] = Property.StaticDropdown({
                                    ...
                                    description: field.HasOtherField
                                        ? `${fieldDescription} (Includes "Other" option)`
                                        : fieldDescription,
                                    required: isRequired,
                                    options: {
                                        disabled: false,
