...
export const newFormEntryTrigger = createTrigger({
    ...
    props: {
        ...
        handshakeKey: Property.ShortText({
            ...
            description:
                'A secret key to validate webhook authenticity and prevent unauthorized requests. Recommended for production workflows.',
            required: false,
        }),
        ...
        includeMetadata: Property.Checkbox({
            ...
            description:
                'Include detailed form and field structure information with each submission. Useful for dynamic processing but increases payload size.',
            required: false,
            defaultValue: false,
        }),
    ...
    async onEnable(context) {
        ...
        try {
            ...
            console.log(
                `Wufoo webhook successfully configured for form ${formIdentifier} with hash: ${response.WebHookPutResult.Hash}`,
            )
        } catch (error: any) {
    ...
    async onDisable(context) {
        ...
        if (!isNil(webhookHash)) {
            try {
                ...
                console.warn(
                    `Warning: Failed to clean up webhook ${webhookHash} for form ${formIdentifier}:`,
                    error.message,
                )
