...
export const newFormTrigger = createTrigger({
    ...
    props: {
        pollingInterval: Property.StaticDropdown({
            ...
            description:
                'How frequently to check for new forms. More frequent checking provides faster detection but uses more API calls.',
            required: false,
            defaultValue: '5',
            options: {
                disabled: false,
        ...
        includeInactive: Property.Checkbox({
            ...
            description:
                'Include forms that are not currently active/published. Useful for monitoring all form creation activity.',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        ...
        try {
            ...
            if (nameFilter && nameFilter.trim()) {
                ...
                newForms = newForms.filter((form) => form.Name && form.Name.toLowerCase().includes(filterText))
            }
