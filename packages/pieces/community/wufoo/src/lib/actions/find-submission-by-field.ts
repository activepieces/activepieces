...
export const findSubmissionByFieldAction = createAction({
    ...
    props: {
        ...
        fieldId: Property.ShortText({
            ...
            description:
                'The form field to search in (e.g., Field1 for first name, Field218 for email). You can find Field IDs in the form builder or via the Form Fields API.',
            required: true,
        }),
        ...
        maxResults: Property.Number({
            ...
            description:
                'Limit the number of results returned (1-50). Lower numbers are faster for deduplication checks.',
            required: false,
            defaultValue: 10,
        }),
    ...
    async run(context) {
        const { formIdentifier, format, fieldId, searchValue, matchType, maxResults, sortOrder, includeMetadata } =
            context.propsValue
