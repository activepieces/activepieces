...
export const getEntryDetailsAction = createAction({
    ...
    props: {
        ...
        entryId: Property.Number({
            ...
            description:
                'Enter a specific Entry ID to retrieve just that entry. Leave blank to get multiple entries with other filters.',
            required: false,
        }),
        ...
        filterField: Property.ShortText({
            ...
            description:
                'The field ID to filter by (e.g., Field1, Field105, EntryId, DateCreated). Leave blank for no filtering.',
            required: false,
        }),
        ...
        filterValue: Property.ShortText({
            ...
            description:
                'The value to filter by. For dates, use YYYY-MM-DD format or YYYY-MM-DD HH:MM:SS for specific times.',
            required: false,
        }),
        ...
        sort: Property.ShortText({
            ...
            description:
                'Sort results by field ID (e.g., EntryId, DateCreated, Field1). Leave blank for default order.',
            required: false,
        }),
        ...
        try {
            ...
            if (entryId !== undefined && entryId !== null) {
                ...
                if (filterField && filterValue) {
                    filterCount++
                    query[`Filter${filterCount}`] =
                        `${filterField}+${filterOperator}+${encodeURIComponent(filterValue)}`
                }

                if (additionalFilterField && additionalFilterValue) {
                    filterCount++
                    query[`Filter${filterCount}`] =
                        `${additionalFilterField}+${additionalFilterOperator}+${encodeURIComponent(additionalFilterValue)}`
                }
                ...
                return {
                    ...
                    entries: entries.map((entry) => ({
                        ...
                        fieldData: Object.keys(entry)
                            .filter(
                                (key) =>
                                    !['EntryId', 'DateCreated', 'DateUpdated', 'CreatedBy', 'UpdatedBy'].includes(key),
                            )
                            .reduce(
                                (acc, key) => {
                                    acc[key] = entry[key]
