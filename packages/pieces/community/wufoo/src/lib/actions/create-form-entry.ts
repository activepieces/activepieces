...
import { dynamicFormFields, formIdentifier } from '../common/props'
    ...
    async run(context) {
        ...
        try {
            ...
            if (error.response?.status === 401) {
                throw new Error('Authentication failed. Please check your API key and subdomain.')
            }

            if (error.response?.status === 404) {
                throw new Error('Form not found. Please verify the form identifier is correct.')
            }
            ...
            throw new Error(`Failed to submit form entry: ${error.message || 'Unknown error occurred'}`)
        }
    },
})
