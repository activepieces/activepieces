...
export const newPartialFormSubmission = createTrigger({
    ...
    async run(context) {
        ...
        const transformedFields =
            Array.isArray(submission.data) && submission.data.length === 0
                ? {}
                : paperformCommon.transformSubmissionData(fields.results.fields, submission.data)
    ...
    async test(context) {
        ...
        return response.results['partial-submissions'].map((submission) => {
            ...
            const transformedFields = paperformCommon.transformSubmissionData(fields.results.fields, submission.data)
            return {
                ...submission,
                data: transformedFields,
            }
