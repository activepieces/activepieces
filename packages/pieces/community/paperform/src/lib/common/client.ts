...
import {
    PaperformCouponsResponse,
    PaperformField,
    PaperformFieldsResponse,
    PaperformFormsResponse,
    PaperformPartialSubmissionsResponse,
    PaperformProductsResponse,
    PaperformSpacesResponse,
    PaperformSubmission,
    PaperformSubmissionsResponse,
    PaperformWebhookResponse,
} from './types'
...
export const paperformCommon = {
    ...
    async getForm({ formId, auth }: { formId: string; auth: string | { apiKey: string } }) {
        return this.apiCall({
            method: HttpMethod.GET,
            url: `/forms/${formId}`,
            auth,
    ...
    async getSubmission({ submissionId, auth }: { submissionId: string; auth: string | { apiKey: string } }) {
    ...
    async getPartialSubmission({ submissionId, auth }: { submissionId: string; auth: string | { apiKey: string } }) {
    ...
    async deleteWebhook({ webhookId, auth }: { webhookId: string; auth: string | { apiKey: string } }) {
        return this.apiCall({
    ...
    transformSubmissionData(formFields: PaperformField[], submissionFields: Record<string, any>): Record<string, any> {
        const fieldMap: Record<string, string> = formFields.reduce(
            (acc, field) => {
