...
export async function fetchAppointmentTypes(accessToken: string, includeDeleted = false) {
    ...
    const response =
        await httpClient.sendRequest<Array<{ id: number; name: string; active: boolean | string }>>(request)
...
export async function fetchFormFields(accessToken: string) {
    ...
    const response =
        await httpClient.sendRequest<Array<{ id: number; name: string; fields: Array<{ id: number; name: string }> }>>(
            request,
        )
