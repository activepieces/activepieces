...
export const updateClient = createAction({
  ...
  description: 'Update an existing client in your Acuity Scheduling client list.',
  props: {
    ...
    currentPhone: Property.ShortText({
      ...
      description: "The current phone number of the client to update. Helps identify the client if names are not unique.",
      required: false,
    }),
  ...
  async run(context) {
    const props = context.propsValue as UpdateClientProps;
    ...
    if (Object.keys(body).length === 0) {
      throw new Error('At least one field to update (New First Name, New Last Name, etc.) must be provided.');
    }
