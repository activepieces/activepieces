    ...
    options: async ({ auth }) => {
        ...
        try {
            ...
            const options = response.data.map((contact) => {
                const displayName =
                    [contact.defaultFields.firstName, contact.defaultFields.lastName].filter(Boolean).join(' ') ||
                    'Unnamed Contact'
    ...
    options: async ({ auth }) => {
        ...
        try {
            ...
            const options = phoneNumbersResponse.data.map((phoneNumber) => {
                const displayName = phoneNumber.name || phoneNumber.formattedNumber || phoneNumber.number
                const numberInfo = phoneNumber.formattedNumber || phoneNumber.number
                const label = phoneNumber.name ? `${displayName} (${numberInfo})` : displayName
...
export const callDropdown = Property.Dropdown({
    ...
    options: async ({ auth, phoneNumberId, participantNumber }) => {
        ...
        try {
            const encodedParticipant = encodeURIComponent(participantNumber as string)
