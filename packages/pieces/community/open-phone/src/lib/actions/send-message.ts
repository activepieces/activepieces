...
import { OpenPhoneMessageResponse, openPhoneCommon } from '../common'
...
export const sendMessage = createAction({
    ...
    props: {
        content: Property.LongText({
            ...
            description: 'The text content of the message to be sent (1-1600 characters)',
            required: true,
        }),
        from: Property.ShortText({
            displayName: 'From Phone Number',
            description: 'OpenPhone number to send from (E.164 format: +15555555555) or phone number ID (PN123abc)',
            required: true,
        }),
        to: Property.Array({
            displayName: 'To Phone Numbers',
            description: 'Recipient phone numbers in E.164 format (e.g., +15555555555)',
            required: true,
            properties: {
    ...
    async run(context) {
        ...
        if (!content || !content.trim()) {
            throw new Error('Message content cannot be empty or contain only whitespace')
        }
        ...
        try {
            const response: OpenPhoneMessageResponse = await openPhoneCommon.makeRequest<OpenPhoneMessageResponse>(
                HttpMethod.POST,
                '/v1/messages',
                auth,
                requestBody,
