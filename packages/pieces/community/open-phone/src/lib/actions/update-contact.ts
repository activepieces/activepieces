...
export const updateContact = createAction({
    ...
    props: {
        ...
        emails: Property.Array({
            ...
            description: 'List of email addresses for the contact (replaces existing emails)',
            required: false,
            properties: {
                name: Property.ShortText({
                    displayName: 'Email Label',
                    description: 'Label for this email address (e.g., "work", "personal")',
                    required: true,
                }),
                ...
                id: Property.ShortText({
                    ...
                    description: 'The unique identifier for existing email field (required for updates)',
                    required: false,
                }),
            },
        }),
        phoneNumbers: Property.Array({
            ...
            description: 'List of phone numbers for the contact (replaces existing phone numbers)',
            required: false,
            properties: {
                name: Property.ShortText({
                ...
                value: Property.ShortText({
                    ...
                    description: 'The phone number (set to empty to remove, preferably in E.164 format)',
                    required: false,
                }),
                id: Property.ShortText({
                    displayName: 'Phone ID',
                    description: 'The unique identifier for existing phone number field (required for updates)',
                    required: false,
                }),
            },
        }),
        customFields: Property.Array({
            ...
            properties: {
                ...
                value: Property.ShortText({
                    ...
                    description: 'The value for the custom field (set to empty to remove)',
                    required: false,
                }),
                id: Property.ShortText({
                    displayName: 'Field ID',
                    description: 'The unique identifier for the custom field (required for updates)',
                    required: false,
                }),
            },
        }),
    ...
    async run(context) {
        ...
        if (externalId && externalId.trim() && (externalId.length < 1 || externalId.length > 75)) {
            throw new Error('External ID must be between 1 and 75 characters')
        }

        const requestBody: UpdateOpenPhoneContactRequest = {}

        const hasDefaultFields = firstName || lastName || company || role || emails || phoneNumbers
        if (hasDefaultFields) {
            requestBody.defaultFields = {}

            if (firstName !== undefined) {
                requestBody.defaultFields.firstName = firstName ? firstName.trim() : null
            }
            ...
            if (emails && Array.isArray(emails) && emails.length > 0) {
                requestBody.defaultFields.emails = (emails as Array<{ name: string; value?: string; id?: string }>).map(
                    (email): OpenPhoneEmail => ({
                        name: email.name.trim(),
                        value: email.value && email.value.trim() ? email.value.trim() : null,
            ...
            if (phoneNumbers && Array.isArray(phoneNumbers) && phoneNumbers.length > 0) {
                requestBody.defaultFields.phoneNumbers = (
                    ...
                    (phone): OpenPhonePhoneNumber => ({
                        ...
                        value: phone.value && phone.value.trim() ? phone.value.trim() : null,
        ...
        if (customFields && Array.isArray(customFields) && customFields.length > 0) {
            requestBody.customFields = (customFields as Array<{ key: string; value?: string; id?: string }>).map(
                (field) => ({
                    key: field.key.trim(),
        ...
        if (sourceUrl !== undefined) {
            requestBody.sourceUrl = sourceUrl && sourceUrl.trim() ? sourceUrl.trim() : null
        }

        if (externalId !== undefined) {
            requestBody.externalId = externalId && externalId.trim() ? externalId.trim() : null
        }
        ...
        try {
            const response: OpenPhoneContactResponse = await openPhoneCommon.makeRequest<OpenPhoneContactResponse>(
                HttpMethod.PATCH,
