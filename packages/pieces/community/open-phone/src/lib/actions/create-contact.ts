...
export const createContact = createAction({
    ...
    props: {
        ...
        createdByUserId: Property.ShortText({
            ...
            description: 'The unique identifier of the user who created the contact (US123abc format)',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (createdByUserId && !/^US(.*)$/.test(createdByUserId)) {
            throw new Error('Created by user ID must start with "US" (e.g., US123abc)')
        }
        ...
        if (emails && Array.isArray(emails) && emails.length > 0) {
            requestBody.defaultFields.emails = (emails as Array<{ name: string; value: string }>).map(
        ...
        if (phoneNumbers && Array.isArray(phoneNumbers) && phoneNumbers.length > 0) {
            requestBody.defaultFields.phoneNumbers = (phoneNumbers as Array<{ name: string; value: string }>).map(
        ...
        if (customFields && Array.isArray(customFields) && customFields.length > 0) {
            requestBody.customFields = (customFields as Array<{ key: string; value: string }>).map(
