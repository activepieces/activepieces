...
import { CreateOpenPhoneWebhookRequest, OpenPhoneWebhookResponse, openPhoneCommon } from '../common'
...
export const incomingCallCompleted = createTrigger({
    ...
    async onEnable(context) {
        ...
        try {
            const response: OpenPhoneWebhookResponse = await openPhoneCommon.makeRequest<OpenPhoneWebhookResponse>(
                HttpMethod.POST,
                '/v1/webhooks/calls',
                auth,
                webhookRequest,
    ...
    async onDisable(context) {
        ...
        try {
            await openPhoneCommon.makeRequest(HttpMethod.DELETE, `/v1/webhooks/calls/${webhookId}`, auth)
        } catch (error) {
