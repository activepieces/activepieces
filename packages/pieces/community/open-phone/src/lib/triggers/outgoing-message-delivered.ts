...
import { CreateOpenPhoneMessageWebhookRequest, OpenPhoneMessageWebhookResponse, openPhoneCommon } from '../common'
...
export const outgoingMessageDelivered = createTrigger({
    ...
    async onDisable(context) {
        ...
        try {
            await openPhoneCommon.makeRequest(HttpMethod.DELETE, `/v1/webhooks/messages/${webhookId}`, auth)
        } catch (error) {
