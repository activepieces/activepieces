...
const polling: Polling<AppConnectionValueForAuthProperty<typeof salesforceAuth>, { caseQueueId: string }> = {
    ...
    items: async ({ auth, lastFetchEpochMS, propsValue }) => {
        ...
        const historyRecords = (historyResponse.body?.['records'] ?? []).filter((r) => r.NewValue === caseQueueId)
