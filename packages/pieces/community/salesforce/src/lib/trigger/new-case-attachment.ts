...
const polling: Polling<AppConnectionValueForAuthProperty<typeof salesforceAuth>, Record<string, never>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        const classicQuery = `
            ...
            LIMIT 200
        `
        const classicResponse = await querySalesforceApi<{ records: { SystemModstamp: string }[] }>(
            HttpMethod.GET,
            auth,
            classicQuery,
        )
        ...
        const recentCasesResponse = await querySalesforceApi<{ records: { Id: string }[] }>(
            HttpMethod.GET,
            auth,
            recentCasesQuery,
        )
        ...
        if (recentCaseIds && recentCaseIds.length > 0) {
            const filesQuery = `
                ...
                LIMIT 200
            `
            const filesResponse = await querySalesforceApi<{ records: { SystemModstamp: string }[] }>(
                HttpMethod.GET,
                auth,
                filesQuery,
            )
