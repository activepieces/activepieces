...
const polling: Polling<AppConnectionValueForAuthProperty<typeof salesforceAuth>, { object: string | undefined }> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        ...
        const historyObject = object.endsWith('__c') ? object.replace('__c', '__History') : `${object}History`
        ...
        try {
            ...
            console.warn(
                `Could not poll for history on ${historyObject}. It might not exist or have history tracking enabled.`,
            )
            return []
        }
    },
