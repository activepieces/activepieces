...
export const salesforcesCommon = {
    account: Property.Dropdown({
        ...
        options: async ({ auth }) => {
            ...
            }>(HttpMethod.GET, auth as OAuth2PropertyValue, `SELECT Id, Name FROM Account ORDER BY Name LIMIT 100`)
            return {
                disabled: false,
        ...
        options: async ({ auth, object }) => {
            ...
            try {
                ...
                const fallbackResponse = await querySalesforceApi<{
                    records: { Id: string }[]
                }>(HttpMethod.GET, auth as OAuth2PropertyValue, `SELECT Id FROM ${object} LIMIT 20`)
                return {
                    disabled: false,
    ...
    campaign: Property.Dropdown({
        ...
        options: async ({ auth }) => {
            ...
            const response = await querySalesforceApi<{
                records: { Id: string; Name: string }[]
            }>(HttpMethod.GET, auth as OAuth2PropertyValue, 'SELECT Id, Name FROM Campaign ORDER BY Name LIMIT 200')
            return {
                disabled: false,
                options: response.body.records.map((record) => ({
                    label: record.Name,
    ...
    lead: Property.Dropdown({
        ...
        options: async ({ auth }) => {
            ...
            const response = await querySalesforceApi<{
                records: { Id: string; Name: string }[]
            }>(HttpMethod.GET, auth as OAuth2PropertyValue, 'SELECT Id, Name FROM Lead ORDER BY Name LIMIT 200')
            return {
                disabled: false,
                options: response.body.records.map((record) => ({
                    label: record.Name,
