...
export const sendInviteAction = createAction({
    ...
    props: {
        ...
        signers: Property.Array({
            displayName: 'Signers',
            description: 'One entry per signer role. Each signer needs an email, role name, and signing order.',
            required: true,
            properties: {
    ...
    async run(context) {
        const { document_id, from, signers, subject, message, cc, cc_subject, cc_message } = context.propsValue
