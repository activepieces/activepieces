...
export const customOdooApiCall = createAction({
    ...
    props: {
        model: Property.ShortText({
            displayName: 'Model',
            description: 'The Odoo model to interact with (e.g., res.partner, product.product)',
            required: true,
        }),
        method: Property.StaticDropdown({
            displayName: 'Method',
            description: 'The method to call on the model (e.g., search_read, create, write)',
            options: {
                options: [
    ...
    async run({ auth, propsValue }) {
        const {
            props: { base_url, database, username, api_key },
        } = auth
        ...
        try {
            ...
            switch (method) {
                ...
                case 'read': {
                    ...
                    const parsedIds = Array.isArray(record_ids) ? record_ids.map((id) => parseInt(id, 10)) : []
