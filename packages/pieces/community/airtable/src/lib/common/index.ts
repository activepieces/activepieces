...
async function fetchAllBases({ token }: { token: string }): Promise<AirtableBase[]> {
...
async function fetchBase({ token, baseId }: { token: string; baseId: string }): Promise<AirtableBase> {
    const request: HttpRequest = {
        method: HttpMethod.GET,
...
async function fetchTableList({ token, baseId }: { token: string; baseId: string }): Promise<AirtableTable[]> {
...
async function createRecord({ personalToken: token, fields, tableId, baseId }: Params) {
    const request: HttpRequest = {
...
async function findRecord({ personalToken: token, searchField, searchValue, tableId, baseId, limitToView }: Params) {
    const request: HttpRequest = {
        method: HttpMethod.GET,
        url: `https://api.airtable.com/v0/${baseId}/${tableId}`,
        authentication: {
...
async function updateRecord({ personalToken: token, fields, recordId, tableId, baseId }: Params) {
    const request: HttpRequest = {
...
async function deleteRecord({ personalToken: token, recordId, tableId, baseId }: Params) {
    const request: HttpRequest = {
...
async function getRecordById({ personalToken: token, baseId, tableId, recordId }: Params) {
    const request: HttpRequest = {
        method: HttpMethod.GET,
        url: `https://api.airtable.com/v0/${baseId}/${tableId}/${recordId}`,
        authentication: {
...
async function addCommentToRecord({ personalToken: token, baseId, tableId, recordId, text, parentCommentId }: Params) {
...
async function createBase({
    ...
    const response = await httpClient.sendRequest<AirtableCreateBaseResponse>(request)
...
export const airtableCommon = {
    ...
    workspaceId: Property.Dropdown<string, true, typeof airtableAuth>({
        ...
        options: async ({ auth }) => {
            ...
            const workspacePromises = bases.map((base) => fetchBase({ token: auth.secret_text, baseId: base.id }))
            const basesWithWorkspaces = await Promise.all(workspacePromises)

            const workspaces = basesWithWorkspaces.reduce(
                (acc, base) => {
                    if (base.workspaceId) {
        ...
        airtable.fields.forEach((field) => {
            if (!AirtableEnterpriseFields.includes(field.type)) {
                ...
                if (field.type === 'multipleAttachments') {
                    if (
                        allowEmpty &&
                        (!fields[key] || (Array.isArray(fields[key]) && (fields[key] as any[]).length === 0))
                    ) {
                        newFields[key] = []
    ...
    getTableSnapshot: async (params: Params) => {
        ...
        const currentTableSnapshot = (
            ...
            .sort((x: any, y: any) => new Date(x.createdTime).getTime() - new Date(y.createdTime).getTime())
