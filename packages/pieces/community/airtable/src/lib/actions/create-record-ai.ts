...
export const airtableCreateRecordAiAction = createAction({
  ...
  aiMetadata: {
    description:
      'Creates a single new record in an Airtable table from a JSON map of field-name to value. Call Get Base Schema first to learn the exact field names and types; linked-record and multi-select fields take arrays of ids/strings. Each call creates a new record, so it is not idempotent (retries duplicate).',
    idempotent: false,
  },
  props: {
