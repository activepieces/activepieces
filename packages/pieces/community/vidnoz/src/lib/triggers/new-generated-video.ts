...
const polling: Polling<AppConnectionValueForAuthProperty<typeof vidnozAuth>, StaticPropsValue<typeof props>> = {
    strategy: DedupeStrategy.LAST_ITEM,
    items: async ({ auth, propsValue }) => {
        const limit = propsValue.limit ?? 100
        const response = await vidnozClient.makeRequest<VidnozVideoListResponse>(auth.secret_text, {
