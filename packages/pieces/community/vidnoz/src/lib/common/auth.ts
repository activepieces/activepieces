...
export const vidnozClient = {
    async makeRequest<T = any>(auth: string, request: Omit<HttpRequest, 'authentication'>): Promise<T> {
        ...
        if (response.status >= 400) {
            throw new Error(
                `Vidnoz API request failed with status ${response.status}: ${JSON.stringify(response.body)}`,
            )
        }
