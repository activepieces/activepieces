...
export const generateVideoWithTemplate = createAction({
    ...
    props: {
        ...
        templateId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    const response = await vidnozClient.makeRequest<VidnozListTemplatesResponse>(auth.secret_text, {
    ...
    run: async ({ auth, propsValue }) => {
        ...
        voices.forEach((v, index) => {
            ...
            if (mode === 'file') {
                if (v['fileAssetId']) {
                    form.append(`voices[${index}][file][asset_id]`, String(v['fileAssetId']))
                }
                if (v['fileUrl']) {
            ...
            if (mode === 'silence') {
                ...
                form.append(`voices[${index}][silence][duration]`, String(duration))
            }
        })
