...
export const runAgentTaskAction = createAction({
    ...
    props: {
        ...
        maxSteps: Property.Number({
            ...
            required: false,
            description: 'Maximum number of steps the task can take. Task will fail if it exceeds this number.',
        }),
    ...
    async run(context) {
        const { prompt, proxyLocation, url, webhookUrl, dataExtractionSchema, maxSteps, engine } = context.propsValue
