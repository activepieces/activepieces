...
export const recordUpdatedTrigger = createTrigger({
    ...
    description: 'Triggers when an existing record is updated (people, companies, deals, etc.).',
    auth: attioAuth,
    props: {
    ...
    async onDisable(context) {
        const webhookData = await context.store.get<{ webhookId: string; WebhookSecret: string }>(TRIGGER_KEY)
    ...
    async test(context) {
        ...
        const filtered = response.data.filter((record) => recordMatchesFilter(record, filter_attribute, filter_value))

        return filtered.slice(0, 5)
    },
    async run(context) {
        const triggerData = await context.store.get<{ webhookId: string; WebhookSecret: string }>(TRIGGER_KEY)
