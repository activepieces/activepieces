        ...
        options: async ({ auth, linked_object }) => {
            ...
            return {
                ...
                options: response.data.map((record) => {
                    ...
                    const label = nameValues[0]?.full_name ?? nameValues[0]?.value ?? record['id']?.record_id
        ...
        options: async ({ auth }) => {
            ...
            try {
                ...
                return {
                    disabled: true,
                    options: [],
                    placeholder: 'Failed to load meetings. Ensure your API key has the "meeting:read" scope.',
                }
            }
        },
    })
        ...
        options: async ({ auth, meeting_id }) => {
            ...
            try {
                ...
                return {
                    disabled: true,
                    options: [],
                    placeholder: 'Failed to load recordings. Ensure your API key has the "call_recording:read" scope.',
                }
            }
        },
    })
    ...
    switch (type) {
        ...
        case 'record-reference': {
            ...
            return field({
                displayName: title,
                required,
                description: is_multiselect ? `Enter ${label} IDs (one per item).` : `Enter the ${label} ID.`,
            })
        }
        ...
        case 'select': {
            ...
            const dropdownType = is_multiselect ? Property.StaticMultiSelectDropdown : Property.StaticDropdown
...
export const objectFields = (isSearch = false, allOptional = false) =>
    Property.DynamicProperties({
        ...
        props: async ({ auth, objectTypeId }) => {
            ...
            for (const attribute of response) {
                ...
                props[api_slug] = await createPropertyDefinition(
                    attribute,
                    'objects',
                    objectId,
                    accessToken,
...
export const listFields = (isSearch = false, allOptional = false) =>
    Property.DynamicProperties({
        auth: attioAuth,
...
export async function formatInputFields(
    ...
    const typeMapping = attributes.reduce(
        (acc, attr) => {
            acc[attr.api_slug] = {
                type: attr.type,
        ...
        switch (fieldType) {
            case 'record-reference': {
                if (isSearch) {
                    ...
                    formattedFields[key] =
                        ids.length === 1 ? { target_record_id: { $eq: ids[0] } } : { target_record_id: { $in: ids } }
                } else {
                    const targetSlug = typeMapping[key]?.object_slug
                    const ids: string[] = Array.isArray(value) ? value : [value]
                    formattedFields[key] = ids.map((id) =>
                        targetSlug ? { target_record_id: id, target_object: targetSlug } : { target_record_id: id },
                    )
                }
                break
            }
