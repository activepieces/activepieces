...
export const updateInvoice = createAction({
  ...
  props: {
    ...
    number: Property.ShortText({ displayName: 'Invoice Number', required: false }),
  ...
  async run(context) {
    ...
    const response = await ninjapipeApiCall<Record<string, unknown>>({ auth, method: HttpMethod.PUT, path: `/invoices/${p.invoiceId}`, body });
