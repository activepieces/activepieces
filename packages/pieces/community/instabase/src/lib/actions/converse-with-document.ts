...
export const converseWithDocumentAction = createAction({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    const response = await makeInstabaseApiCall<{
                        conversations: Array<{ id: string; name: string; description?: string }>
                    }>(auth, '/v2/conversations', HttpMethod.GET)
        ...
        document_ids: Property.MultiSelectDropdown({
            ...
            options: async ({ auth, conversation_id }) => {
                ...
                try {
                    ...
                    }>(auth, `/v2/conversations/${conversation_id}`, HttpMethod.GET)
    ...
    async run({ auth, propsValue }) {
        const { conversation_id, question, document_ids, mode = 'default', fast_mode = false } = propsValue
