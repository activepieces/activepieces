...
import { CapsuleCrmAuthType, capsuleCrmAuth } from './auth'
    ...
    contact_id: (required = true) =>
        Property.Dropdown({
            ...
            options: async (props) => {
                ...
                const contacts = await capsuleCrmClient.searchContacts(auth, searchTerm)

                const options = contacts.map((contact) => {
                    const label = contact.type === 'person' ? `${contact.firstName} ${contact.lastName}` : contact.name
                    return {
