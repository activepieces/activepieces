...
export const capsuleCrmCreateTrigger = ({ name, displayName, description, event, sampleData }: TriggerParams) => {
    return createTrigger({
        ...
        async onDisable(context) {
            const storedData = await context.store.get<{ hookId: number }>(`capsule_crm_trigger_${name}`)
            if (storedData) {
