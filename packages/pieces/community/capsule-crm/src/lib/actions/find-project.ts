...
export const findProjectAction = createAction({
  ...
  description: 'Searches for projects (cases) in Capsule CRM.',
  props: {
  ...
  async run(context) {
    ...
    const response = await makeApiCall(
      context.auth,
      `${API_ENDPOINTS.SEARCH_PROJECTS}?${queryParams.toString()}`,
      'GET'
    );
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      throw new Error(`Failed to search projects: ${response.status} ${response.body?.message || ''}`);
    }
  },
});