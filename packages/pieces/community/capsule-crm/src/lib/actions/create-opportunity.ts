...
import { CreateOpportunityParams, OpportunityCustomField, OpportunityTag } from '../common/types'
...
export const createOpportunityAction = createAction({
    ...
    async run(context) {
        ...
        if (propsValue.tags) {
            opportunityData.tags = propsValue.tags.map((tag) => ({ name: tag }) as OpportunityTag)
        }
