...
import { CreateProjectParams, OpportunityCustomField, OpportunityTag } from '../common/types'
...
export const createProjectAction = createAction({
    ...
    props: {
        partyId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const contacts = await capsuleCrmClient.searchContacts(auth, '')
                return {
        ...
        stageId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const stages = await capsuleCrmClient.listStages(auth)
                return {
        ...
        customFields: Property.DynamicProperties({
            ...
            props: async ({ auth }) => {
                ...
                const customFields = await capsuleCrmClient.listCustomFields(auth)
    ...
    async run(context) {
        ...
        if (propsValue.tags) {
            projectData.tags = propsValue.tags.map((tag) => ({ name: tag }) as OpportunityTag)
        }
