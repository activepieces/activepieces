...
import { CapsuleCrmAuthType, capsuleCrmAuth } from '../common/auth'
import { capsuleCrmClient } from '../common/client'
import { OpportunityCustomField, OpportunityTag, UpdateOpportunityParams } from '../common/types'
...
export const updateOpportunityAction = createAction({
    ...
    props: {
        opportunityId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                if (!auth)
                    return { options: [], disabled: true, placeholder: 'Please connect your Capsule CRM account first' }
                const opportunities = await capsuleCrmClient.searchOpportunities(auth)
                return {
        ...
        teamId: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                if (!auth)
                    return { options: [], disabled: true, placeholder: 'Please connect your Capsule CRM account first' }
                const teams = await capsuleCrmClient.listTeams(auth)
                return {
            ...
            props: async ({ auth, opportunityId }) => {
                ...
                if (!auth || !opportunityId)
                    return {
                        options: [],
                        disabled: true,
                        placeholder: 'Please connect your Capsule CRM account first or select an opportunity',
                    }

                const opportunity = await capsuleCrmClient.getOpportunity(auth, opportunityId as unknown as number)
                const tagOptions =
        ...
        customFields: Property.DynamicProperties({
            ...
            props: async ({ auth, opportunityId }) => {
                ...
                await capsuleCrmClient.getOpportunity(auth, opportunityId as unknown as number)
                const allCustomFields = await capsuleCrmClient.listCustomFields(auth)
    ...
    async run(context) {
        ...
        return await capsuleCrmClient.updateOpportunity(auth, opportunityId, opportunityData)
    },
})
