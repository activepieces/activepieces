...
import { DUB_API_BASE, dubAuth } from '../auth'
...
export const updateLink = createAction({
    ...
    props: {
        linkId: Property.ShortText({
            displayName: 'Link ID',
            description:
                'The ID of the link to update (e.g. `clv3g2...`). Find this from the Create Link or List Links actions.',
            required: true,
        }),
        ...
        tags: Property.Array({
            ...
            description:
                "Replace the link's tags with this new list of tag names. Send an empty array to clear all tags.",
            required: false,
        }),
