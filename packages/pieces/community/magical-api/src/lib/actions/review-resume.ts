...
export const reviewResume = createAction({
    ...
    async run(context) {
        ...
        if (!requestIdToPoll) {
            ...
            response = await makeRequest(apiKey, HttpMethod.POST, REVIEW_RESUME_ENDPOINT, {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const pollResponse = await makeRequest(apiKey, HttpMethod.POST, REVIEW_RESUME_ENDPOINT, {
                request_id: requestIdToPoll,
            })
        ...
        throw new Error(
            'Timeout: Resume review is taking longer than expected. You can check the status later with this Request ID: ' +
                requestIdToPoll,
        )
    },
})
