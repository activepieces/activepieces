...
export const parseResume = createAction({
    ...
    async run(context) {
        ...
        if (!requestIdToPoll) {
            ...
            response = await makeRequest(apiKey, HttpMethod.POST, RESUME_PARSER_ENDPOINT, {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            const pollResponse = await makeRequest(apiKey, HttpMethod.POST, RESUME_PARSER_ENDPOINT, {
                request_id: requestIdToPoll,
            })
        ...
        throw new Error(
            'Timeout: Resume parsing is taking longer than expected. You can check the status later with this Request ID: ' +
                requestIdToPoll,
        )
    },
})
