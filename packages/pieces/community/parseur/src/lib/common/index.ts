...
export const parseurCommon = {
    ...
    endpoints: {
        ...
        reprocessDocument: (documentId: string) => `/document/${documentId}/process`,
        listMailboxes: '/parser',
        createWebhook: '/webhook',
        enableWebhook: (webhookId: number, mailboxId: number) => `/parser/${mailboxId}/webhook_set/${webhookId}`,
    ...
    // API methods
    listDocuments: async ({ apiKey, parserId, ...queryParams }: ListDocumentsParams) => {
        ...
        const response = await httpClient.sendRequest<ListDocumentsResponse>({
            ...
            url: parseurCommon.baseUrl + parseurCommon.endpoints.listDocuments(parserId),
    ...
    getDocument: async ({ apiKey, documentId }: GetParsedDocumentByIdParams) => {
        const response = await httpClient.sendRequest<GetParsedDocumentByIdResponse>({
            method: HttpMethod.GET,
            url: parseurCommon.baseUrl + parseurCommon.endpoints.getParsedDocumentById(documentId),
            headers: parseurCommon.getHeaders(apiKey),
        })
        return response.body
    },
    createDocument: async ({ apiKey, ...documentParams }: CreateDocumentParams) => {
    ...
    createDocumentFromFile: async ({ apiKey, file, parserId }: CreateDocumentFromFileParams) => {
        ...
        const response = await httpClient.sendRequest<CreateDocumentFromFileResponse>({
            method: HttpMethod.POST,
            url: parseurCommon.baseUrl + parseurCommon.endpoints.createDocumentFromFile(parserId),
            headers: {
    ...
    reprocessDocument: async ({ apiKey, documentId }: ReprocessDocumentParams) => {
        const response = await httpClient.sendRequest<ReprocessDocumentResponse>({
            method: HttpMethod.POST,
            url: parseurCommon.baseUrl + parseurCommon.endpoints.reprocessDocument(documentId),
            headers: parseurCommon.getHeaders(apiKey),
        })
        return response.body
    },
    ...
    enableWebhook: async ({ apiKey, webhookId, mailboxId }: EnableWebhookParams) => {
        const response = await httpClient.sendRequest<void>({
            method: HttpMethod.POST,
            url: parseurCommon.baseUrl + parseurCommon.endpoints.enableWebhook(webhookId, mailboxId),
            headers: parseurCommon.getHeaders(apiKey),
        })
    ...
    deleteWebhook: async ({ apiKey, webhookId }: DeleteWebhookParams) => {
        const response = await httpClient.sendRequest<void>({
            ...
            url: parseurCommon.baseUrl + parseurCommon.endpoints.deleteWebhook(webhookId),
            headers: parseurCommon.getHeaders(apiKey),
        })
        if (response.status === 200 || response.status === 204) {
            return { success: true }
