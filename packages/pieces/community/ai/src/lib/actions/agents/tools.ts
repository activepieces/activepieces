...
import { dynamicTool, EmbeddingModel, embed, embedMany, LanguageModel, Tool } from 'ai'
import z from 'zod'
...
function flattenMcpServers(servers: McpServerTools[], agentMcpTools: AgentMcpTool[]): FlattenedMcpResult {
...
async function constructKnowledgeBaseTools(
    ...
    const tableToolEntries = await Promise.all(
        tableTools.map(async (tableTool) => {
            ...
            try {
                const tableResponse = await api.get<{ id: string; name: string }>(
                    `v1/tables/${encodeURIComponent(tableTool.sourceId)}`,
                )
                    ...
                    filters: z
                        .array(
                            z.object({
                                ...
                                operator: z
                                    .enum(['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'co', 'exists', 'not_exists'])
...
export async function constructAgentTools(
    ...
    const {
        mcpClients,
        tools: mcpTools,
        keyToAgentTool: mcpKeyToAgentTool,
    } = flattenMcpServers(mcpServerTools, agentMcpTools)

    const agentKbTools = agentTools.filter(
        (tool): tool is AgentKnowledgeBaseTool => tool.type === AgentToolType.KNOWLEDGE_BASE,
    )
    const { tools: kbTools, keyToAgentTool: kbKeyToAgentTool } = await constructKnowledgeBaseTools(
        agentKbTools,
        context,
        embeddingConfig,
    )
    ...
    for (const agentTool of agentTools.filter(
        (t) => t.type !== AgentToolType.MCP && t.type !== AgentToolType.KNOWLEDGE_BASE,
    )) {
        const key =
            agentTool.type === AgentToolType.FLOW
                ? mcpToolNameUtils.createToolName(agentTool.toolName)
                : agentTool.toolName
        ...
        [TASK_COMPLETION_TOOL_NAME]: dynamicTool({
            ...
            inputSchema: z.object({
                ...
                ...(!isNil(structuredOutput)
                    ? {
                          output: z
                              .object(agentUtils.structuredOutputSchema(structuredOutput)?.shape ?? {})
                              .nullable()
                              .describe(
