...
import { GeneratedFile, GenerateTextResult, generateImage, generateText, ImagePart, LanguageModel, ToolSet } from 'ai'
    ...
    props: {
        ...
        advancedOptions: Property.DynamicProperties({
            ...
            props: async (propsValue): Promise<InputPropertyMap> => {
                ...
                if (providerId === AIProviderName.GOOGLE && modelId === 'gemini-2.5-flash-image-preview') {
                    options = {
...
const assertImageGenerationSuccess = (result: GenerateTextResult<ToolSet, never>): void => {
    const responseBody =
        result.response.body && typeof result.response.body === 'object' && 'candidates' in result.response.body
