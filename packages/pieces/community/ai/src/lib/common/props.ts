...
import { AIProviderModel, AIProviderName, AIProviderWithoutSensitiveData, isNil } from '@activepieces/shared'
...
export const aiProps = <T extends AIModelType>({ modelType, allowedProviders }: AIPropsParams<T>) => ({
        ...
        options: async (_, ctx) => {
            ...
            return {
                ...
                options: supportedProviders
                    ...
                    .filter((option) =>
                        allowedProviders ? allowedProviders.includes(option.value as AIProviderName) : true,
                    ),
            }
        },
    }),
