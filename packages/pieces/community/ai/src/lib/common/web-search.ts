import { InputPropertyMap, PieceAuth, Property } from '@activepieces/pieces-framework'
import { AIProviderName, spreadIfDefined } from '@activepieces/shared'
import { ProviderOptions } from '@ai-sdk/provider-utils'
import { ToolSet } from 'ai'
import { anthropicSearchTool, googleSearchTool, openaiSearchTool } from './ai-sdk'
...
function buildWebSearchOptionsProps(provider: string, params?: { showIncludeSources?: boolean }): InputPropertyMap {
    ...
    const isOpenRouterProvider = provider === AIProviderName.OPENROUTER || provider === AIProviderName.ACTIVEPIECES
...
function createWebSearchTool(provider: string, options: WebSearchOptions = {}): ToolSet {
    ...
    switch (provider) {
        case AIProviderName.ANTHROPIC: {
            ...
            if (
                ...
                (!anthropicOptions.allowedDomains || anthropicOptions.allowedDomains.length === 0)
            ) {
...
export function buildWebSearchConfig(params: {
    ...
    const isOpenRouter = provider === AIProviderName.OPENROUTER || provider === AIProviderName.ACTIVEPIECES
            ...
            providerOptions: {
                openrouter: {
                    plugins: [
                        {
                            id: 'web' as const,
                            max_results: Math.min(Math.max(webSearchOptions?.maxUses ?? 5, 1), 10),
                        },
                    ],
                },
            },
...
type AnthropicWebSearchOptions = BaseWebSearchOptions &
    UserLocationOptions & {
        allowedDomains?: { domain: string }[]
        blockedDomains?: { domain: string }[]
    }

type OpenAIWebSearchOptions = BaseWebSearchOptions &
    UserLocationOptions & {
