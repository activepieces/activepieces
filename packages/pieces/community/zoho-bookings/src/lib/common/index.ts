...
export const zohoBookingsCommon = {
    baseUrl: (location = 'zoho.com') => {
        return `https://www.zohoapis.${location.substring(5).trim()}/bookings/v1/json`
    },
...
export const bookingIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, from_time, to_time }) => {
        ...
        try {
            ...
            const appointments = await zohoBookingsCommon.fetchAppointments((auth as any).access_token, location, {
                perPage: 50,
