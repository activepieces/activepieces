...
export const bookAppointment = createAction({
    ...
    props: {
        workspace_id: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    const workspaces = await zohoBookingsCommon.fetchWorkspaces((auth as any).access_token, location)
        ...
        staff_id: Property.Dropdown({
            auth: zohoBookingsAuth,
            displayName: 'Staff',
            description: 'Select the staff member (use this OR resource_id OR group_id)',
            required: false,
        ...
        from_time: Property.DateTime({
            ...
            description: 'The starting time for the appointment (format: mm-dd-yyyy HH:mm:ss)',
            required: true,
        }),
        to_time: Property.DateTime({
            displayName: 'To Time',
            description: 'End time for resource booking (optional, format: dd-MMM-yyyy HH:mm:ss)',
            required: false,
        }),
    ...
    async run(context) {
        ...
        await propsValidation.validateZod(propsValue, zohoBookingsCommon.bookAppointmentSchema)

        // Validate that at least one of staff_id, resource_id, or group_id is provided
        if (!propsValue.staff_id && !propsValue.resource_id && !propsValue.group_id) {
            throw new Error('Either staff_id, resource_id, or group_id must be provided')
        }
        ...
        if (propsValue.additional_fields) {
            formData.append('additional_fields', JSON.stringify(propsValue.additional_fields))
        }
        if (propsValue.cost_paid) {
            formData.append('payment_info', JSON.stringify({ cost_paid: propsValue.cost_paid.toString() }))
        }
