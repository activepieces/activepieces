...
import { bookingIdDropdown, formatDateTime, zohoBookingsAuth, zohoBookingsCommon } from '../common'
...
export const rescheduleAppointment = createAction({
        ...
        service_id: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    const services = await zohoBookingsCommon.fetchServices((auth as any).access_token, location)
    ...
    async run(context) {
        ...
        if (responseBody.status !== 'success') {
            throw new Error(`Failed to reschedule appointment: ${responseBody.message || 'Unknown error'}`)
        }
