...
> = {
    ...
    items: async ({ auth, propsValue }) => {
        ...
        const tasksResponse = await makeRequest(HttpMethod.GET, `/sections/${propsValue.section}/tasks`, token)
        ...
        for (const task of tasks) {
            try {
                const attachmentsResponse = await makeRequest(HttpMethod.GET, `/tasks/${task.id}/attachments`, token)
