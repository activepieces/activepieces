...
const newLabelPolling: Polling<AppConnectionValueForAuthProperty<typeof meistertaskAuth>, { project: unknown }> = {
    strategy: DedupeStrategy.LAST_ITEM,
    items: async ({ auth, propsValue }) => {
        const token = getToken(auth)
        const response = await makeRequest(HttpMethod.GET, `/projects/${propsValue.project}/labels`, token)
