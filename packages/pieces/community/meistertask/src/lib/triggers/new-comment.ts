...
const newCommentPolling: Polling<AppConnectionValueForAuthProperty<typeof meistertaskAuth>, { task_id: unknown }> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ auth, propsValue }) => {
        const token = getToken(auth)
        const tasksResponse = await makeRequest(HttpMethod.GET, `/task/${propsValue.task_id}/comments`, token)
