...
export const findAttachment = createAction({
    ...
    async run(context) {
        ...
        const response = await makeRequest(HttpMethod.GET, `/tasks/${task_id}/attachments`, token)

        let attachments = response.body

        if (name) {
            attachments = attachments.filter((att: any) => att.name.toLowerCase().includes(name.toLowerCase()))
        }
