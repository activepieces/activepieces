...
export const findOrCreateAttachment = createAction({
    ...
    async run(context) {
        ...
        const findResponse = await makeRequest(HttpMethod.GET, `/tasks/${task_id}/attachments`, token)

        const existingAttachment = findResponse.body.find((att: any) => att.name.toLowerCase() === name.toLowerCase())
        ...
        const createResponse = await makeRequest(HttpMethod.POST, `/tasks/${task_id}/attachments`, token, {
            url: file_url,
            name,
        })
