...
export const findOrCreateLabel = createAction({
    ...
    async run(context) {
        ...
        const findResponse = await makeRequest(HttpMethod.GET, `/projects/${project}/labels`, token)

        const existingLabel = findResponse.body.find((label: any) => label.name.toLowerCase() === name.toLowerCase())
        ...
        const createResponse = await makeRequest(HttpMethod.POST, `/projects/${project}/labels`, token, body)
