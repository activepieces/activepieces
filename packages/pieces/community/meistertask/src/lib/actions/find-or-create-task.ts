...
export const findOrCreateTask = createAction({
    ...
    async run(context) {
        ...
        const findResponse = await makeRequest(HttpMethod.GET, `/sections/${section}/tasks`, token)

        const existingTask = findResponse.body.find((task: any) => task.name.toLowerCase() === name.toLowerCase())
        ...
        const createResponse = await makeRequest(HttpMethod.POST, '/tasks', token, body)
