...
import { getIssueTypes, getProjects, getUsers, jiraApiCall, jiraPaginatedApiCall, sendJiraRequest } from '.'
...
export function getUsersDropdown(data?: DropdownParams) {
  return Property.Dropdown({
    ...
    options: async ({ auth }) => {
      ...
      const users = (await getUsers(auth as JiraAuth)).filter((user) => user.accountType === 'atlassian')
      return {
...
async function fetchProjectVersionsOptions(auth: JiraAuth, projectId: string): Promise<DropdownOption<string>[]> {
...
export async function createPropertyDefinition(auth: JiraAuth, field: IssueFieldMetaData, isRequired = false) {
...
export function formatIssueFields(fieldsMetadata: IssueFieldMetaData[], fieldsInput: Record<string, any>) {
      ...
      case 'array': {
        ...
        fieldsOutput[key] =
          field.schema.items === 'string'
            ...
            : parsedArrayValue.map((item) => (field.schema.items === 'group' ? { groupId: item } : { id: item }))
        break
      }
...
export function transformCustomFields(
  ...
  const fieldsMapping = fieldsMetadata.reduce(
    (acc, field) => {
