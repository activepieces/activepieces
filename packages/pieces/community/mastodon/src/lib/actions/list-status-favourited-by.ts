...
export const listStatusFavouritedBy = createAction({
  auth: mastodonAuth,
  name: 'list_status_favourited_by',
  classification: 'SEARCH',
  displayName: 'List Accounts That Favourited a Status',
  description: 'List the accounts that favourited a status.',
  audience: 'ai',
  aiMetadata: {
    description:
      'Returns one page of accounts that favourited a given status, with cursors for further pages (pass next_max_id as Max ID, prev_since_id as Since ID). Use List Accounts That Boosted a Status for boosts. Needs the read:accounts scope. Read-only and safe to retry.',
    idempotent: true,
  },
  ...
  async run(context) {
    ...
    const { items, ...cursors } = await mastodonClient.requestPage<MastodonEntity>({
      ...
      path: `/api/v1/statuses/${encodeURIComponent(status_id)}/favourited_by`,
      operation: 'List Accounts That Favourited a Status',
      scope: 'read:accounts',
