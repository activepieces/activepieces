...
export const unfavouriteStatus = createAction({
  auth: mastodonAuth,
  name: 'unfavourite_status',
  classification: 'WRITE',
  displayName: 'Unfavourite Status',
  description: 'Remove a status from your favourites.',
  audience: 'ai',
  aiMetadata: {
    description:
      'Removes the connected account\'s favourite from a status. Safe to retry; unfavouriting a status that is not favourited changes nothing. Returns the updated status.',
    idempotent: true,
  },
  ...
  async run(context) {
    return mastodonClient.request<MastodonEntity>({
      ...
      path: `/api/v1/statuses/${encodeURIComponent(context.propsValue.status_id)}/unfavourite`,
      operation: 'Unfavourite Status',
