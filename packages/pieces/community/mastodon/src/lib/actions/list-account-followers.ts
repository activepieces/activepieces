...
export const listAccountFollowers = createAction({
  auth: mastodonAuth,
  name: 'list_account_followers',
  classification: 'SEARCH',
  displayName: 'List Account Followers',
  description: 'List the followers of an account.',
  audience: 'ai',
  aiMetadata: {
    description:
      'Returns one page of accounts following the given account, with Link-header cursors for further pages. Empty when the account hides its social graph. Use List Account Following for the reverse direction. Read-only and safe to retry.',
    idempotent: true,
  },
  ...
  async run(context) {
    ...
    const { items, ...cursors } = await mastodonClient.requestPage<MastodonEntity>({
      ...
      path: `/api/v1/accounts/${encodeURIComponent(context.propsValue.account_id)}/followers`,
      operation: 'List Account Followers',
      scope: 'read:accounts',
