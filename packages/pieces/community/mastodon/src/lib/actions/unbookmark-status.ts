...
export const unbookmarkStatus = createAction({
  auth: mastodonAuth,
  name: 'unbookmark_status',
  ...
  aiMetadata: {
    description:
      'Removes a status from the connected account\'s bookmarks. Safe to retry. Returns the updated status.',
    idempotent: true,
  },
  ...
  async run(context) {
    return mastodonClient.request<MastodonEntity>({
      ...
      path: `/api/v1/statuses/${encodeURIComponent(context.propsValue.status_id)}/unbookmark`,
