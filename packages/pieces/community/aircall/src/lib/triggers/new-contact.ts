...
export const newContact = createTrigger({
    ...
    async onEnable(context) {
        ...
        const response = await makeRequest(context.auth, HttpMethod.POST, '/webhooks', {
            url: webhookUrl,
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await makeRequest(context.auth, HttpMethod.DELETE, `/webhooks/${webhookId}`)
        }
    },
    ...
    async test(context) {
        const response = await makeRequest(context.auth, HttpMethod.GET, '/contacts?order=desc&per_page=10')
