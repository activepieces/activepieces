...
export const callIdDropdown = Property.Dropdown<number, true, typeof aircallAuth>({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth as { username: string; password: string }, HttpMethod.GET, '/calls')
...
export const numberIdDropdown = Property.Dropdown<string, true, typeof aircallAuth>({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth, HttpMethod.GET, '/numbers')
            return {
                disabled: false,
...
export const tagIdDropdown = Property.MultiSelectDropdown<number, false, typeof aircallAuth>({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth as { username: string; password: string }, HttpMethod.GET, '/tags')
