...
export const enrichCreatorByEmail = createAction({
    ...
    async run(context) {
        ...
        if (context.propsValue.exclude_platforms && context.propsValue.exclude_platforms.length > 0) {
            body.exclude_platforms = context.propsValue.exclude_platforms
        }

        if (context.propsValue.min_followers !== undefined && context.propsValue.min_followers !== null) {
