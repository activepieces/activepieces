...
export const askCometApiAction = createAction({
    ...
    props: {
        ...
        temperature: Property.Number({
            ...
            required: false,
            description: 'Controls randomness (0-2). Lower values make output more focused.',
            defaultValue: 1.0,
        }),
    ...
    async run(context) {
        ...
        const { model, prompt, systemMessage, temperature, maxTokens, topP } = context.propsValue
        ...
        try {
            const response = await httpClient.sendRequest<CometApiChatResponse>(request)
            ...
            if (errorObj.response?.status === 400) {
                const errorMessage = errorObj.response?.body?.error?.message || 'Bad request parameters'
