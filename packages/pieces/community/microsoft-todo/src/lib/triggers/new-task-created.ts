...
const polling: Polling<AppConnectionValueForAuthProperty<typeof microsoftToDoAuth>, { task_list_id: string }> = {
    ...
    items: async ({ auth, propsValue, store, lastFetchEpochMS }) => {
        ...
        const filter =
            lastFetchEpochMS === 0 ? '$top=10' : `$filter=createdDateTime gt ${dayjs(lastFetchEpochMS).toISOString()}`

        let response: PageCollection = await client.api(`/me/todo/lists/${taskListId}/tasks?${filter}`).get()
