...
import { createTodoClient, getIncompleteTasksInListDropdown, getTaskListsDropdown } from '../common'
...
export const completeTaskAction = createAction({
    ...
    async run(context) {
        ...
        try {
            ...
            const response = await client.api(`/me/todo/lists/${task_list_id}/tasks/${task_id}`).update(taskUpdate)
