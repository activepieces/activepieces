...
export async function callApi<Type>(authProp: PiecePropValueSchema<typeof weblingAuth>, request: string) {
...
export async function getCalendars(authProp: PiecePropValueSchema<typeof weblingAuth>): Promise<CalendarObject[]> {
  const response = await callApi<CalendarObject[]>(authProp, 'calendar?format=full')
  return response.body
}
...
export async function getEventsById(
  ...
  const response = await callApi<WeblingCalendarEvent[]>(authProp, request)
...
export async function getUpdatedOrNewEvents(
  ...
  const weblingChanges: WeblingChanges = await getChanges(authProp, lastFetchEpochMS / 1000)
  ...
  const updatedOrNewEvents: string[] = changedEvents.filter((event) => !deletedObjects.includes(event))

  const response = await callApi<WeblingCalendarEvent[]>(
    authProp,
    `calendarevent/${updatedOrNewEvents.join(',')}?format=full&filter=$parents.$id=${calendarId}`,
  )
  return response.body
}
