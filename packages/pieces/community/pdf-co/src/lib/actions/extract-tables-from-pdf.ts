...
export const extractTablesFromPdf = createAction({
    ...
    props: {
        ...
        templateId: Property.ShortText({
            ...
            description:
                'The ID of your Document Parser template (created in PDF.co dashboard) designed to extract the table(s).',
            required: true,
        }),
        pages: Property.ShortText({
            displayName: 'Pages',
            description:
                'Comma-separated page numbers or ranges (e.g., "0,2,5-10"). Overrides template settings if provided.',
            required: false,
        }),
    ...
    async run(context) {
        ...
        const { url, templateId, pages, pdfPassword, fileName, httpPassword, httpUsername, expiration, profiles } =
            propsValue
