...
export const getTableColumnsAction = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        let apiCall = client.api(
            `${drivePath}/items/${workbookId}/workbook/worksheets/${worksheetId}/tables/${tableId}/columns`,
        )
        ...
        const columnNames = response.value.map((column: { name: any }) => column.name)
