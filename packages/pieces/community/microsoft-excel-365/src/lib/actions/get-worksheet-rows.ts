...
import { getDrivePath, getHeaders } from '../common/helpers'
    ...
    props: {
        ...
        filterList: Property.DynamicProperties({
            ...
            props: async ({ auth, useFilter, worksheetId, workbookId, storageSource, siteId, documentId }) => {
                ...
                const filterColumnOptions: DropdownOption<number>[] = firstRow.map((header: string, index: number) => ({
                    label: header as string,
                    value: index,
                }))

                const filterOperatorOptions: DropdownOption<string>[] = Object.entries(filterOperatorLabels).map(
                    ([operator, label]) => ({
                        label,
