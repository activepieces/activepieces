...
import { createMSGraphClient, getHeaders, getLastUsedRow, numberToColumnName } from '../common/helpers'
...
export const appendMultipleRowsAction = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        const lastUsedRow = await getLastUsedRow(auth.access_token, drivePath, workbookId, worksheetId, cloud)
        ...
        const { numberFormat, formulas, formulasLocal, formulasR1C1, valueTypes, values, text, ...rest } = res
