...
export const clearColumnAction = createAction({
    ...
    async run(context) {
        const { storageSource, siteId, documentId, workbookId, worksheetId, column_index, applyTo } = context.propsValue
        ...
        if (typeof column_index !== 'number' || !Number.isInteger(column_index) || column_index < 1) {
        ...
        await client
            .api(
                `${drivePath}/items/${workbookId}/workbook/worksheets/${worksheetId}/range(address='${columnAddress}')/clear`,
            )
