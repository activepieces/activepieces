...
export const commonProps = {
    ...
    siteId: Property.Dropdown({
        ...
        options: async ({ auth, storageSource }) => {
            ...
            let response: PageCollection = await client.api('/sites?search=*&$select=displayName,id,name').get()
    ...
    workbookId: Property.Dropdown({
        ...
        options: async ({ auth, storageSource, siteId, documentId }) => {
            ...
            if (storageSource === 'sharepoint' && (!siteId || !documentId)) {
                return createEmptyOptions('please select SharePoint site and document library first.')
            }
            ...
            const drivePath = storageSource === 'onedrive' ? '/me/drive' : `/sites/${siteId}/drives/${documentId}`
    ...
    worksheetValues: Property.DynamicProperties({
        ...
        required: true,
        refreshers: ['storageSource', 'siteId', 'documentId', 'workbookId', 'worksheetId', 'isFirstRowHeaders'],
        props: async ({ auth, storageSource, siteId, workbookId, documentId, worksheetId, isFirstRowHeaders }) => {
            if (!auth || (workbookId ?? '').toString().length === 0 || (worksheetId ?? '').toString().length === 0) {
                return {}
            }
    ...
    worksheetMultiValues: Property.DynamicProperties({
        ...
        props: async ({ auth, storageSource, siteId, workbookId, documentId, worksheetId }) => {
            if (!auth || (workbookId ?? '').toString().length === 0 || (worksheetId ?? '').toString().length === 0) {
                return {}
            }
