...
async function getWorksheets(auth: OAuth2PropertyValue, workbookId: string, drivePath: string): Promise<Worksheet[]> {
    if (!workbookId) return []

    try {
        const cloud = auth.props?.['cloud'] as string | undefined
        const client = createMSGraphClient(auth.access_token, cloud)
        const response = await client.api(`${drivePath}/items/${workbookId}/workbook/worksheets`).get()
...
const polling: Polling<
    ...
    { storageSource: string; siteId?: string; documentId?: string; workbookId: string }
> = {
