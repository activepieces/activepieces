...
type ClicdataApiResponse<TResult> = ClicdataApiSuccessResponse<TResult> | ClicdataApiErrorResponse
...
function getHeadersFromAuth(auth: ClicdataAuth): Record<string, string> {
    ...
    if (auth.type === AppConnectionType.CUSTOM_AUTH) {
        const encodedEmailAndPassword = Buffer.from(`${auth.props.userEmail}:${auth.props.userPassword}`).toString(
            'base64',
        )
...
export async function clicdataApiCall<TResult>({
    ...
    const response = await httpClient.sendRequest<ClicdataApiResponse<TResult>>(request)

    if (!response.body.success) {
        const errorMsg = response.body.error?.description || response.body.error?.code || 'ClicData API error'
