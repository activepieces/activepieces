...
import { CHATWOOT_AUTH_HEADER, chatwootEndpoints } from '../common/constants'
import {
...
export const newMessage = createTrigger({
    ...
    async onEnable(context) {
        ...
        const endpoints = chatwootEndpoints(authValue.baseUrl, authValue.accountId)
        ...
        try {
            ...
            if (
                ...
                typeof body === 'object' &&
                'message' in body &&
                (body as { message?: string }).message?.includes('already been taken')
            ) {
    ...
    async onDisable(context) {
        ...
        const stored = await context.store.get<{ webhookId: number }>(WEBHOOK_STORE_KEY)
    ...
    async run(context) {
        ...
        const contactId =
            contactObj != null && typeof contactObj === 'object' && 'id' in contactObj
                ? Number((contactObj as { id: unknown })['id']) || 0
                : 0
        const contactName =
            contactObj != null && typeof contactObj === 'object' && 'name' in contactObj
                ? String((contactObj as { name: unknown })['name'])
                : 'Unknown'
        const accountId =
            account != null && typeof account === 'object' && 'id' in account
