...
export const aliasAUser = createAction({
    ...
    description: "Change a user's identifier by merging two user identities into one",
    props: {
    ...
    async run(context) {
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.PUT, '/users/reidentify', {
            id,
            new_id,
        })
