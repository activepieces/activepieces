...
import { z } from 'zod'
import { createServiceNowClient, recordDropdown, servicenowAuth, tableDropdown } from '../common/props'
...
export const findFileAction = createAction({
    ...
    async run(context) {
        const { action_type, table, record, manual_sys_id, filename, attachment_sys_id, accept_type, return_format } =
            context.propsValue
        ...
        if (action_type === 'list') {
            ...
            if (!recordId || !table) {
                throw new Error('Table and record selection are required for listing attachments')
            }
            ...
            return {
                action: 'list',
                attachments: filteredAttachments.map((attachment) => AttachmentMetaSchema.parse(attachment)),
            ...
            const result = await client.getAttachment(input.attachment_sys_id!, input.accept_type)
