    ...
    async run({ propsValue, auth }) {
        ...
        const resolveUserByEmail = async (email: string) => {
            try {
                const response = await httpClient.sendRequest({
                    ...
                    }.zendesk.com/api/v2/users/search.json?query=email:${encodeURIComponent(email)}`,
                    method: HttpMethod.GET,
                    authentication: {
                        type: AuthenticationType.BASIC,
                        username: authentication.props.email + '/token',
        ...
        if (custom_fields && typeof custom_fields === 'object') {
            try {
                ...
                for (const [propKey, value] of Object.entries(custom_fields)) {
                    ...
                    const def = fieldDefinitions.find(
                        (f) => (f.key ?? `custom_field_${f.id}`) === fieldKey || f.key === fieldKey,
                    )
                    if (!def) continue
        ...
        try {
            ...
            if (errorMessage.includes('400')) {
                throw new Error('Invalid request parameters. Please check your input values and try again.')
            }
            ...
            if (errorMessage.includes('404')) {
                throw new Error(`Ticket with ID ${ticket_id} not found. Please verify the ticket ID.`)
            }
            ...
            if (errorMessage.includes('422')) {
                throw new Error('Validation error. Please check that all field values are valid.')
            }
