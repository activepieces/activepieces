...
export const findUserAction = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        const { search_type, email, name, role, organization, tag, external_id, custom_query, sort_by, sort_order } =
            propsValue
        ...
        switch (search_type) {
            ...
            case 'organization':
                if (!organization) {
                    throw new Error('Organization is required when searching by organization.')
                }
        ...
        try {
            ...
            const users = responseBody.results.filter((result) => result.result_type === 'user')
            ...
            if (errorMessage.includes('400')) {
                throw new Error('Invalid search query. Please check your search parameters and try again.')
            }
            ...
            if (errorMessage.includes('422')) {
                throw new Error('Search query validation error. Please check your search syntax and parameters.')
            }
