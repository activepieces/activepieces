...
export const deleteUserAction = createAction({
    ...
    async run({ propsValue, auth }) {
        ...
        const { user_id, confirmation } = propsValue
        ...
        try {
            ...
            return {
                ...
                data: response.body,
                warning:
                    "Deleted users are not recoverable in Zendesk UI. For GDPR erasure, use 'Permanently Delete User' if required.",
                note: 'To comply with GDPR, you may need to use the "Permanently Delete User" endpoint for complete data removal.',
            }
        } catch (error) {
            const errorMessage = (error as Error).message
            if (errorMessage.includes('400')) {
                throw new Error('Invalid request parameters. Please check the user ID and try again.')
            }
            ...
            if (errorMessage.includes('404')) {
                throw new Error(`User with ID ${user_id} not found. Please verify the user ID.`)
            }
