...
export const addCommentToTicketAction = createAction({
  ...
  props: {
    ...
    author_email: Property.ShortText({
      ...
      description: 'Email address of the comment author. If not provided, the authenticated user will be the author.',
      required: false,
    }),
    uploads: Property.Array({
      displayName: 'Attachment Tokens',
      description: 'Array of upload tokens for file attachments. Upload files first using the Attachments API.',
      required: false,
    }),
    via_followup_source_id: Property.Number({
      displayName: 'Via Followup Source ID',
      description: 'The ID of the original ticket if this comment is from a follow-up ticket.',
      required: false,
    }),
  },
  async run({ propsValue, auth }) {
    ...
    // Set public flag (defaults to true if not specified)
    comment.public = isPublic !== false;

    // Resolve author if specified
    if (author_email) {
    ...
    // Add via followup source if provided
    ...
    // Build the ticket update object with only the comment
    const ticket = {
      comment,
    };
