...
export { zendeskAuth, ZendeskAuthValue };
  ...
  actions: [
    ...
    createCustomApiCallAction({
      baseUrl: (auth) => {
        ...
        if (!authValue) return '';
        const subdomain = getZendeskSubdomain(authValue);
        return subdomain ? `https://${subdomain}.zendesk.com/api/v2` : '';
      },
      auth: zendeskAuth,
      authMapping: async (auth) => {
        const authValue = auth as ZendeskAuthValue;
        const headers = getZendeskAuthHeader(authValue);
        return {
          Authorization: (headers['Authorization'] || headers['authorization'] || '') as string,
        };
      },
    }),
  ],
