...
export const updatedSalesInvoice = createTrigger({
  auth: xeroAuth,
  name: 'updatedSalesInvoice',
  displayName: 'Updated Sales Invoice',
  description:
    'Fires when an existing sales invoice (Accounts Receivable) is updated in Xero',
  props: {
    tenant_id: props.tenant_id,
  },
  sampleData: {
    resourceUrl:
      'https://api.xero.com/api.xro/2.0/Invoices/12345678-1234-1234-1234-123456789012',
    resourceId: '12345678-1234-1234-1234-123456789012',
    eventDateUtc: '2025-08-13T15:30:00.000Z',
    eventType: 'UPDATE',
  ...
  async run(context) {
    ...
    if (payload.events) {
      const updatedInvoiceEvents = payload.events.filter(
        (event: any) =>
          event.eventCategory === 'INVOICE' && event.eventType === 'UPDATE'
      );

      if (updatedInvoiceEvents.length > 0) {
        return updatedInvoiceEvents;
      }
    }
