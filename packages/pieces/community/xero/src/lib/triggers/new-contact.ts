...
export const newContact = createTrigger({
  ...
  description: 'Fires when a new contact is added to Xero',
  props: {
    tenant_id: props.tenant_id,
  },
  sampleData: {
    resourceUrl:
      'https://api.xero.com/api.xro/2.0/Contacts/12345678-1234-1234-1234-123456789012',
  ...
  async run(context) {
    ...
    if (payload.events) {
      const contactEvents = payload.events.filter(
        (event: any) =>
          event.eventCategory === 'CONTACT' && event.eventType === 'CREATE'
      );
