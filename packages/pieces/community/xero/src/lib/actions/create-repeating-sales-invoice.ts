...
export const xeroCreateRepeatingSalesInvoice = createAction({
    ...
    props: {
        ...
        approved_for_sending: Property.Checkbox({
            displayName: 'Approved For Sending',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        ...
        if (schedule_unit === 'WEEKLY' && !['DAYSAFTERBILLDATE', 'OFFOLLOWINGMONTH'].includes(due_date_type)) {
            throw new Error('For weekly schedules, Due Date Type must be DAYSAFTERBILLDATE or OFFOLLOWINGMONTH.')
        }
        if (schedule_unit === 'MONTHLY' && !['OFCURRENTMONTH', 'OFFOLLOWINGMONTH'].includes(due_date_type)) {
            throw new Error('For monthly schedules, Due Date Type must be OFCURRENTMONTH or OFFOLLOWINGMONTH.')
        }
