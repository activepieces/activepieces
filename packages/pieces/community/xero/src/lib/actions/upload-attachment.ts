...
export const xeroUploadAttachment = createAction({
        ...
        resource_id: Property.Dropdown({
            ...
            options: async ({ auth, propsValue, tenant_id, resource_type }) => {
                ...
                const tenantId: string | undefined =
                    typeof rawTenant === 'string' ? rawTenant : rawTenant?.value || rawTenant?.tenantId || rawTenant?.id
                const rawResourceType = resource_type ?? (propsValue as Record<string, any>)?.['resource_type']
                const resourceType = (typeof rawResourceType === 'string' ? rawResourceType : rawResourceType?.value) as
                    | ResourceType
                    | undefined
                ...
                const endpointMap: Record<
                    ResourceType,
                    { url: string; arrayKey: string; idKey: string; label: (item: any) => string }
                > = {
                    Invoices: {
                        ...
                        idKey: 'InvoiceID',
                        label: (inv) =>
                            [inv.InvoiceNumber || inv.InvoiceID, inv.Contact?.Name, inv.Status]
                                .filter(Boolean)
                                .join(' • '),
                    },
                    CreditNotes: {
                        ...
                        idKey: 'CreditNoteID',
                        label: (cn) =>
                            [cn.CreditNoteNumber || cn.CreditNoteID, cn.Contact?.Name, cn.Type, cn.Status]
                                .filter(Boolean)
                                .join(' • '),
                    },
                    PurchaseOrders: {
                        ...
                        label: (po) =>
                            [po.PurchaseOrderNumber || po.PurchaseOrderID, po.Contact?.Name, po.Status]
                                .filter(Boolean)
                                .join(' • '),
                    },
                    Quotes: {
                        ...
                        idKey: 'QuoteID',
                        label: (q) =>
                            [q.QuoteNumber || q.QuoteID, q.Contact?.Name, q.Status].filter(Boolean).join(' • '),
                    },
                    BankTransfers: {
                        ...
                        label: (bt) =>
                            [bt.BankTransferID, bt.Amount ? `Amount ${bt.Amount}` : undefined]
                                .filter(Boolean)
                                .join(' • '),
                    },
                    BankTransactions: {
                        ...
                        label: (bt) =>
                            [bt.Type, bt.BankTransactionID, bt.Contact?.Name, bt.Amount].filter(Boolean).join(' • '),
                    },
                    Contacts: {
    ...
    async run(context) {
        ...
        const inferredContentType =
            content_type || (file.extension ? `application/${file.extension}` : 'application/octet-stream')
