...
export const props = {
    ...
    payable_invoice_id: (required = false) =>
        Property.Dropdown({
            ...
            options: async ({ auth, propsValue, tenant_id }) => {
                ...
                const url = 'https://api.xero.com/api.xro/2.0/Invoices?summaryOnly=true&page=1&Statuses=AUTHORISED'
    ...
    sales_invoice_id: (required = false) =>
        Property.Dropdown({
            ...
            description:
                'Select a sales invoice with a valid status for sending email (SUBMITTED, AUTHORISED, or PAID).',
            required,
            refreshers: ['tenant_id'],
            options: async ({ auth, propsValue, tenant_id }) => {
                if (!auth)
    ...
    editable_sales_invoice_id: (required = false) =>
        Property.Dropdown({
            ...
            options: async ({ auth, propsValue, tenant_id, allow_authorised }) => {
                ...
                const statuses = allowAuthorised ? 'DRAFT,SUBMITTED,AUTHORISED' : 'DRAFT,SUBMITTED'
                const url = `https://api.xero.com/api.xro/2.0/Invoices?summaryOnly=true&page=1&Statuses=${encodeURIComponent(statuses)}&where=Type%3d%3d%22ACCREC%22`
            ...
            options: async ({ auth, propsValue, tenant_id }) => {
                ...
                if (result.status === 200) {
                    ...
                    const options = accounts
                        .filter((acc) => acc?.Type === 'BANK' || acc?.EnablePaymentsToAccount === true)
                        .slice(0, 100)
                        .map((acc) => {
            ...
            options: async ({ auth, propsValue, tenant_id }) => {
                ...
                if (result.status === 200) {
                    ...
                    const options = pos.slice(0, 100).map((po) => {
                        const labelParts = [po.PurchaseOrderNumber || po.PurchaseOrderID, po.Contact?.Name, po.Status]
                        return {
                            label: labelParts.filter(Boolean).join(' • '),
        ...
        Property.Dropdown({
            ...
            options: async ({ auth, propsValue, tenant_id }) => {
                ...
                if (result.status === 200) {
                    const accounts: any[] = result.body?.Accounts ?? []
                    const options = accounts
                        .filter((acc) => (Array.isArray(allowedTypes) ? allowedTypes.includes(acc?.Type) : true))
                        .slice(0, 200)
                        .map((acc) => {
                            const labelParts = [acc.Name || acc.Code || acc.AccountID]
                            if (acc.Code) labelParts.push(acc.Code)
