...
export const generateTextToSpeech = createAction({
    ...
    props: {
        ...
        voiceDescription: Property.LongText({
            ...
            description:
                'Natural language description of how the speech should sound (tone, accent, style, etc.). If no voice is specified, this will generate a dynamic voice.',
            required: false,
        }),
    ...
    async run(context) {
        ...
        try {
            ...
            throw new Error(
                `Text-to-speech generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
            )
        }
    },
})
