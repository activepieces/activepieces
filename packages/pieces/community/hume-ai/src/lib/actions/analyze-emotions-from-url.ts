...
export const analyzeEmotionsFromUrl = createAction({
    ...
    props: {
        urls: Property.Array({
            ...
            description:
                'URLs to media files to analyze (images, videos, audio). Supports up to 100 URLs or archives (.zip, .tar.gz)',
            required: true,
        }),
    ...
    async run(context) {
        ...
        try {
            ...
            return {
                ...
                status: 'processing',
                message:
                    'Emotion analysis job started successfully. Use "Get Emotion Results" action to retrieve predictions.',
            }
        } catch (error) {
            throw new Error(
                `Emotion analysis job failed to start: ${error instanceof Error ? error.message : 'Unknown error'}`,
            )
        }
    },
})
