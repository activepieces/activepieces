...
export const combineFiles = createAction({
    ...
    async run(context) {
        ...
        const fileData = await client.makeBinaryRequest(HttpMethod.POST, '/api/tools/combine', body)

        const fileExtension = output === 'pdf' ? 'pdf' : 'docx'
        const fileName = name ? `${name}.${fileExtension}` : `combined_file_${Date.now()}.${fileExtension}`
