...
export const splitPdf = createAction({
    ...
    async run(context) {
        ...
        const fileData = await client.makeBinaryRequest(HttpMethod.POST, '/api/tools/pdf/split', body)

        const pdfFileName = fileName.endsWith('.pdf') ? fileName.replace('.pdf', '_split.pdf') : `${fileName}_split.pdf`
