...
import { createGetResponseNewsletter, flattenGetResponseNewsletter } from '../common/client'
...
export const createNewsletterAction = createAction({
    ...
    props: {
        ...
        name: Property.ShortText({
            ...
            description: 'An internal name for the newsletter inside GetResponse. Leave empty to use the subject.',
            required: false,
        }),
    ...
    async run(context) {
        const campaignId = requireString(context.propsValue.campaignId, 'Campaign')
        const fromFieldId = requireString(context.propsValue.fromFieldId, 'From Address')
        ...
        if (!context.propsValue.htmlContent && !context.propsValue.plainContent) {
            throw new Error('Provide HTML Content, Plain Text Content, or both before creating the newsletter.')
        }
        ...
        const newsletter = await createGetResponseNewsletter({
            ...
            request: {
                ...
                content: {
                    ...(context.propsValue.htmlContent ? { html: context.propsValue.htmlContent } : {}),
                    ...(context.propsValue.plainContent ? { plain: context.propsValue.plainContent } : {}),
                },
                sendSettings: {
