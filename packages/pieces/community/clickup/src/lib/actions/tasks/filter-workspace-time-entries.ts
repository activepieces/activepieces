...
export const filterClickupWorkspaceTimeEntries = createAction({
  ...
  props: {
    ...
    assignee: clickupCommon.assignee_id(false, 'Assignee Id', 'ID of assignee for Clickup Task'),
  ...
  async run(context) {
    ...
    if (params.start_date) query['start_date'] = dayjs(params.start_date).valueOf()
