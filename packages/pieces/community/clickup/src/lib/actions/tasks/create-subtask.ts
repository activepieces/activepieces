...
import { callClickUpApi, clickupCommon, listAccessibleCustomFields } from '../../common'
...
export const createClickupSubtask = createAction({
    ...
    props: {
        ...
        assignee_id: clickupCommon.assignee_id(false, 'Assignee Id', 'ID of assignee for Clickup Subtask'),
        ...
        custom_fields: Property.DynamicProperties({
            ...
            props: async ({ list_id, auth }) => {
                ...
                const { fields: customFields } = await listAccessibleCustomFields(accessToken, list_id.toString())
    ...
    async run(configValue) {
        ...
        if (custom_fields) {
            data.custom_fields = Object.entries(custom_fields).map(([fieldId, value]) => ({
                id: fieldId,
                value,
            }))
        }
