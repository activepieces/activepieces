...
export const newEmailInFolder = createTrigger({
  ...
  async run(context) {
    ...
    const { accountId: accountIdProp, folder, includeContent, includeAttachmentInfo, limit } = context.propsValue;
  ...
  async test(context) {
    const { accountId: accountIdProp, folder, includeContent, includeAttachmentInfo } = context.propsValue;
