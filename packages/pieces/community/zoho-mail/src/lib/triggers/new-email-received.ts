...
const TRIGGER_INBOX_DATA_STORE_KEY = 'new_email_inbox_last_processed_timestamp';
...
export const newEmailReceived = createTrigger({
    ...
    async onEnable(context) {
        await context.store.put(TRIGGER_INBOX_DATA_STORE_KEY, Date.now() - (60 * 60 * 1000), StoreScope.FLOW);
        console.log('Inbox polling trigger enabled. Initial last processed timestamp set.');
    },
    async onDisable(context) {
        console.log('Inbox polling trigger disabled.');
    },
    async run(context) {
        ...
        const region = typedAuth.props?.region;
        const lastProcessedTimestamp = await context.store.get<number>(TRIGGER_INBOX_DATA_STORE_KEY, StoreScope.FLOW) || 0;
        ...
        if (maxTimestampInBatch > lastProcessedTimestamp) {
            await context.store.put(TRIGGER_INBOX_DATA_STORE_KEY, maxTimestampInBatch, StoreScope.FLOW);
        }
