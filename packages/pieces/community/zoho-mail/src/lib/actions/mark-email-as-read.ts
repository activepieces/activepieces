...
export const markEmailAsRead = createAction({
  ...
  props: {
    ...
    messageIds: Property.Array({
      ...
      description: 'The IDs of the messages to mark as read. Provide either Message IDs or Thread IDs.',
      required: false,
    }),
    threadIds: Property.Array({
      displayName: 'Thread IDs',
      description: 'The IDs of the threads to mark as read. Provide either Message IDs or Thread IDs.',
      required: false,
    }),
  },
  async run({ auth, propsValue }) {
    ...
    if (response.status !== 200) {
      throw new Error(`Failed to mark email as read: ${JSON.stringify(response.body)}`);
    }
