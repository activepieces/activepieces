...
export const findTimeEntry = createAction({
    ...
    props: {
        description: Property.ShortText({
            ...
            description: 'Search for time entries containing this text in description.',
            required: false,
        }),
        start_date: Property.ShortText({
            displayName: 'Start Date',
            description: 'Get entries from start_date (YYYY-MM-DD or RFC3339 format).',
            required: false,
        }),
        ...
        before: Property.ShortText({
            ...
            description: 'Get entries before given date (YYYY-MM-DD or RFC3339 format).',
            required: false,
        }),
    ...
    async run(context) {
        const { description, start_date, end_date, before, since, meta, include_sharing } = context.propsValue
        ...
        if (include_sharing !== undefined) queryParams['include_sharing'] = include_sharing.toString()
        ...
        if (description) {
            const matchingEntries = timeEntries.filter(
                (entry) => entry.description && entry.description.toLowerCase().includes(description.toLowerCase()),
            )
