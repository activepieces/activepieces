...
export const postScheduled = createTrigger({
    ...
    async onEnable(context) {
        const response = (await sendItRequest(context.auth.secret_text, HttpMethod.POST, '/webhooks', {
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await sendItRequest(context.auth.secret_text, HttpMethod.DELETE, `/webhooks/${webhookId}`)
        }
    },
    async run(context) {
        ...
        return [
            {
                ...
                platforms: Array.isArray(payload.data?.platforms) ? payload.data.platforms.join(', ') : null,
