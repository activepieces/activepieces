...
export const scheduleIdProperty = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = (await sendItRequest(auth.secret_text, HttpMethod.GET, '/scheduled')) as Record<
                string,
                unknown
            >

            const posts = (response['posts'] ?? response['data'] ?? response) as Record<string, unknown>[]
            ...
            return {
                ...
                options: posts
                    .map((post) => {
                        ...
                        const snippet = rawText.length > 40 ? rawText.substring(0, 40) + '…' : rawText || 'No text'
