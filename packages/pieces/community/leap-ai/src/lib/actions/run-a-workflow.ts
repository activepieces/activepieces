...
export const runAWorkflow = createAction({
    ...
    props: {
        ...
        webhook_url: Property.ShortText({
            ...
            description: 'The URL to which workflow results should be sent on completion (optional)',
            required: false,
        }),
        input: Property.Object({
            displayName: 'Input Variables',
            description: 'Variables that the workflow can use globally and their values (optional)',
            required: false,
        }),
    },
    async run(context) {
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/runs', body)
