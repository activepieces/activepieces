...
export const sendEmail = createAction({
    ...
    async run(context) {
        const { to, from, from_name, reply_to, subject, content_type, content, attachments = [] } = context.propsValue

        const attachment_data: Attachment[] = (attachments as { file: ApFile; name?: string }[]).map(
            ({ file, name }) => {
