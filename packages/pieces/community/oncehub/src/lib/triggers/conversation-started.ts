...
export const conversationStarted = createTrigger({
    ...
    description: 'Triggered when website visitor starts interacting with a chatbot.',
    props: {},
    sampleData: {
        id: 'EVNT-KN56U3YL7C',
        object: 'event',
    ...
    async onDisable(context) {
        const api_key = context.auth.secret_text
        const webhookId = await context.store.get<string>('webhookId_conversationStarted')
