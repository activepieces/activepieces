...
export const bookingScheduled = createTrigger({
    ...
    description: 'Triggered when Customer schedules a booking and when User approves a booking requested by a Customer',
    props: {},
    sampleData: {
        id: 'EVNT-KN56U3YL7C',
        object: 'event',
    ...
    async onDisable(context) {
        const api_key = context.auth.secret_text
        const webhookId = await context.store.get<string>('webhookId_bookingScheduled')
