...
export const bookingCanceledThenRescheduled = createTrigger({
    ...
    description: 'Triggered when Customer cancels a booking and then reschedules on a different booking page.',
    props: {},
    sampleData: {
        id: 'EVNT-KN56U3YL7C',
        object: 'event',
    ...
    async onDisable(context) {
        const api_key = context.auth.secret_text
        const webhookId = await context.store.get('webhookId_canceled_then_rescheduled')
