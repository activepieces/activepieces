...
export class QuickbaseClient {
    ...
    private async makeRequest<T>(endpoint: string, method: HttpMethod, data?: any): Promise<T> {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (response.status === 429) {
                    ...
                    const delay = retryAfterValue ? parseInt(retryAfterValue) * 1000 : Math.pow(2, attempt) * 1000
