...
import { QuickbaseField, QuickbaseRecordResponse } from '../common/types'
import { extractRecordValues, generateDeduplicationKey } from '../common/utils'
...
const polling: Polling<QuickbaseAuth, StaticPropsValue<typeof props>> = {
    ...
    items: async ({
        ...
        auth: QuickbaseAuth
        propsValue: StaticPropsValue<typeof props>
                        ...
                        updated_at:
                            record[
                                tableFields
                                    .find(
                                        (f) =>
                                            f.fieldType === 'timestamp' && f.label.toLowerCase().includes('modified'),
                                    )
                                    ?.id.toString() || ''
                            ]?.value || dateCreated,
                        recordId,
