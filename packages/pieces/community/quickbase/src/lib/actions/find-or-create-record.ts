...
import { appIdProp, fieldsMapperProp, mergeFieldProp, tableIdProp } from '../common/props'
import { QuickbaseCreateRecordResponse, QuickbaseField, QuickbaseRecordResponse } from '../common/types'
import { extractRecordValues, mapFieldsToRecord, validateRequiredFields } from '../common/utils'
...
export const findOrCreateRecord = createAction({
    ...
    async run(context) {
        ...
        const mergeValue =
            fields[mergeField] || fields[tableFields.find((f) => f.id.toString() === mergeField)?.label || '']
