...
export const updateCase = createAction({
    ...
    props: {
        ...
        billing_contact: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const clientOptions =
                    clientsResponse.success && Array.isArray(clientsResponse.data)
        ...
        lead_lawyer: Property.Dropdown({
            ...
            options: async ({ auth, staff }) => {
                ...
                if (response.success && Array.isArray(response.data)) {
                    const selectedStaff = response.data.filter((s: any) => staff.includes(s.id.toString()))
    ...
    async run(context) {
        ...
        if (context.propsValue.staff && Array.isArray(context.propsValue.staff)) {
            ...
            const originatingLawyerId = context.propsValue.originating_lawyer
                ? parseInt(context.propsValue.originating_lawyer)
                : null
