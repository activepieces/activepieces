...
export const updateCompany = createAction({
                ...
                if (response.success && Array.isArray(response.data)) {
                    return {
                        ...
                        options: response.data.map((company: any) => ({
                            label: `${company.name}${company.email ? ` (${company.email})` : ''} - ID: ${company.id}`,
                            value: company.id,
                        })),
                    }
                }
    ...
    async run(context) {
        ...
        if (context.propsValue.website) requestBody.website = context.propsValue.website
        if (context.propsValue.main_phone_number) requestBody.main_phone_number = context.propsValue.main_phone_number
        ...
        try {
            const response = await api.put(`/companies/${context.propsValue.company_id}`, requestBody)
