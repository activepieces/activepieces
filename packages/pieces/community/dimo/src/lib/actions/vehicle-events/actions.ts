...
import { ActionContext, CustomAuthProperty, createAction, Property } from '@activepieces/pieces-framework'
...
import { VehicleEventsBodyType, VehicleEventsParams } from './type'

async function sendVehicleEventsRequest({
    ctx,
    opKey,
}: {
    ctx: ActionContext<CustomAuthProperty<any>>
    opKey: keyof typeof VEHICLE_EVENTS_OPERATIONS
}) {
...
const upsertWebhookNumericAction = createAction({
    ...
    props: {
            ...
            options: {
                options: [
                    ...
                    { label: 'Battery Power', value: TriggerField.PowertrainTractionBatteryCurrentPower },
                    {
                        ...
                        value: TriggerField.PowertrainTractionBatteryStateOfChargeCurrent,
                    },
        ...
        triggerNumber: Property.Number({
            ...
            description:
                'Numeric value to compare against (e.g., speed in km/h, fuel percentage, battery watts, tire pressure in PSI)',
            required: true,
        }),
                ...
                options: [
                    {
                        ...
                        value: TriggerField.PowertrainTractionBatteryChargingIsCharging,
                    },
...
const upsertWebhookEventAction = createAction({
    ...
    async run(ctx) {
        const { webhookId, eventType, coolDownPeriod, targetURL, status, verificationToken, displayName, description } =
            ctx.propsValue
        const { clientId, apiKey, redirectUri } = ctx.auth.props
        const dimo = new DimoClient({ clientId, apiKey, redirectUri })
