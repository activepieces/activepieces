import { AuthenticationType, HttpError, HttpMethod, httpClient, QueryParams } from '@activepieces/pieces-common'
...
export class DimoClient {
    ...
    async sendTelemetryGraphQLRequest(input: { vehiclejwt: string; query: string; variables: Record<string, any> }) {
        const response = await httpClient.sendRequest({
            method: HttpMethod.POST,
    ...
    async getVehiclePrivileges(input: { tokenId: number }) {
        ...
        const matchingSacd = nodes.find((sacd: any) => sacd.grantee.toLowerCase() === this.clientId.toLowerCase())
    ...
    async subscribeVehiclesToWebhook(input: { developerJwt: string; webhookId: string; vehicleTokenIds: string[] }) {
