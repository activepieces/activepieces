...
export const ignitionTrigger = createTrigger({
    ...
    async onEnable(context) {
        ...
        const { vehicleTokenIds, ignitionState, coolDownPeriod, verificationToken } = context.propsValue
    ...
    async run(context) {
        ...
        if (!webhookBody || typeof webhookBody !== 'object' || webhookBody.name !== TriggerField.IsIgnitionOn) {
            return []
        }
