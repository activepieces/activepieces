...
export const fuelAbsoluteTrigger = createTrigger({
  ...
  props: {
    ...
    verificationToken: Property.ShortText({
      ...
      required: false,
      defaultValue: 'activepieces-fuel-absolute-trigger',
    }),
  },
  sampleData: {
    tokenId: 17,
  ...
  async onEnable(context) {
    const { vehicleTokenIds, comparisonType, fuelLiters, triggerFrequency, verificationToken } = context.propsValue;
    ...
    try {
      const webhookResponse = await httpClient.sendRequest({
        ...
        body: {
          ...
          status: 'Active',
          verification_token: verificationToken || 'activepieces-fuel-absolute-trigger',
        },
        headers: {
      ...
      throw new Error(`Failed to setup fuel absolute level trigger: ${error.message}`);
    }
  },
