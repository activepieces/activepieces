...
export const speedTrigger = createTrigger({
    ...
    async onEnable(context) {
        ...
        const { vehicleTokenIds, operator, speedValue, coolDownPeriod, verificationToken } = context.propsValue
    ...
    async run(context) {
        ...
        if (!webhookBody || typeof webhookBody !== 'object' || webhookBody.name !== TriggerField.Speed) {
            return []
        }
