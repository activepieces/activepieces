...
export const dimoSpeedTrigger = createTrigger({
  auth: dimoDeveloperAuth,
  name: 'trigger_speed',
  displayName: 'Vehicle Speed Alert',
  description: 'Triggers when a vehicle speed meets a specified condition.',
  ...
  props: {
    ...
    value: Property.Number({
      ...
      description: 'The speed threshold value in kilometers per hour.',
      required: true,
    }),
  ...
  async onEnable(context) {
    ...
    const webhook = await createDimoWebhook(developerJwt, {
      ...
      status: 'active',
      verificationToken,
      displayName: `Activepieces Speed Trigger (${conditionExpr})`,
    });
    ...
    if (subscribe_all !== false) {
      try {
        ...
        // Non-fatal - webhook is created, subscriptions can be added manually
        console.warn('Failed to subscribe all vehicles:', e);
      }
    }

    await context.store.put<StoredWebhookInfo>('_dimo_speed_trigger', {
  ...
  async onDisable(context) {
    ...
    const stored = await context.store.get<StoredWebhookInfo>('_dimo_speed_trigger');
    if (stored?.webhookId) {
