...
export const createDesign = createAction({
  ...
  description:
    'Create a new Canva design using a preset type or custom dimensions.',
  props: {
    ...
    customWidth: Property.Number({
      ...
      description:
        'Required when Design Type is Custom. Between 40 and 8000.',
      required: false,
    }),
  ...
  async run(context) {
    const { designTypeOption, presetName, customWidth, customHeight, title } =
      context.propsValue;
    ...
    if (designTypeOption === 'custom') {
      if (!customWidth || !customHeight) {
        throw new Error(
          'Width and height are required when Design Type is Custom.',
        );
      }
      if (
        customWidth < 40 ||
        ...
        throw new Error(
          'Width and height must be between 40 and 8000 pixels.',
        );
      }
    }
