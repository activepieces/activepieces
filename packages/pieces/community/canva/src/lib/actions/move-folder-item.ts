...
export const moveFolderItem = createAction({
  ...
  props: {
    ...
    toFolderId: Property.Dropdown({
      ...
      description:
        'The folder to move the item into. Select "Root (Projects)" to move to the top level.',
      required: true,
      refreshers: [],
      options: async ({ auth }) => {
        if (!auth) {
        ...
        const options = await listFoldersForDropdown(
          auth as Parameters<typeof listFoldersForDropdown>[0],
        );
