...
async function pollExportJob(
  ...
  throw new Error(
    'Export job timed out after 60 seconds. Check Canva for the export status.',
  );
}
...
export const exportDesign = createAction({
  ...
  description:
    'Export a Canva design to PDF, JPG, PNG, GIF, PPTX, or MP4. Returns download URLs valid for 24 hours.',
  props: {
    designId: Property.Dropdown({
      ...
      options: async ({ auth }) => {
        if (!auth) {
          return {
            disabled: true,
            placeholder: 'Connect your Canva account first.',
            options: [],
          };
        }
        const options = await listDesignsForDropdown(
          auth as Parameters<typeof listDesignsForDropdown>[0],
        );
    ...
    exportQuality: Property.StaticDropdown({
      ...
      description:
        'Quality level (Pro quality requires premium elements to be licensed).',
      required: false,
  ...
  async run(context) {
    ...
    if (exportQuality) {
      format['export_quality'] = exportQuality;
    }
    ...
    if (createResponse.job.status === 'failed') {
      throw new Error(
        `Export failed: ${createResponse.job.error?.message ?? 'Unknown error'}`,
      );
    }
    ...
    if (createResponse.job.status === 'in_progress') {
      const job = await pollExportJob(
        context.auth.access_token,
        createResponse.job.id,
      );
      if (job.status === 'failed') {
        throw new Error(
          `Export failed: ${job.error?.message ?? 'Unknown error'}`,
        );
      }
      return job;
    }
