    ...
    async run(context) {
        const company =
            context.propsValue.companyName || context.propsValue.companyDescription
                ? {
                      ...(context.propsValue.companyName ? { name: context.propsValue.companyName } : {}),
        ...
        const address =
            ...
            context.propsValue.countryCode
                ? {
                      ...(context.propsValue.addressLine1 ? { address: context.propsValue.addressLine1 } : {}),
                      ...(context.propsValue.addressLine2 ? { address_two: context.propsValue.addressLine2 } : {}),
                      ...(context.propsValue.postalCode ? { postal_code: context.propsValue.postalCode } : {}),
                      ...(context.propsValue.city ? { city: context.propsValue.city } : {}),
                      ...(context.propsValue.state ? { state: context.propsValue.state } : {}),
                      ...(context.propsValue.countryCode ? { country: { code: context.propsValue.countryCode } } : {}),
                  }
                : undefined

        const customFields = isRecord(context.propsValue.customFields) ? context.propsValue.customFields : undefined
