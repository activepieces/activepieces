...
export const createConversionAction = createAction({
    ...
    async run(context) {
        const metaData = isRecord(context.propsValue.metaData) ? context.propsValue.metaData : undefined
        ...
        return await tapfiliateApiCall({
            ...
            query: context.propsValue.overrideMaxCookieTime ? { override_max_cookie_time: 'true' } : undefined,
            body: {
                ...(context.propsValue.referralCode ? { referral_code: context.propsValue.referralCode } : {}),
                ...(context.propsValue.trackingId ? { tracking_id: context.propsValue.trackingId } : {}),
                ...(context.propsValue.clickId ? { click_id: context.propsValue.clickId } : {}),
                ...(context.propsValue.coupon ? { coupon: context.propsValue.coupon } : {}),
                ...(context.propsValue.currency ? { currency: context.propsValue.currency } : {}),
                ...(context.propsValue.assetId ? { asset_id: context.propsValue.assetId } : {}),
                ...(context.propsValue.sourceId ? { source_id: context.propsValue.sourceId } : {}),
                ...(context.propsValue.externalId ? { external_id: context.propsValue.externalId } : {}),
                amount: context.propsValue.amount,
                ...(context.propsValue.customerId ? { customer_id: context.propsValue.customerId } : {}),
                ...(context.propsValue.userAgent ? { user_agent: context.propsValue.userAgent } : {}),
