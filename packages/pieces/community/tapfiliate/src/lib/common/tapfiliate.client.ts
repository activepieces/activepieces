...
export function buildTapfiliateQuery(values?: Record<string, unknown>): QueryParams {
...
export function parseTapfiliateNextPage(linkHeader: string | string[] | undefined): string | undefined {
    const normalizedHeader = Array.isArray(linkHeader) ? linkHeader.join(',') : linkHeader
...
export async function tapfiliatePaginatedApiCall<T extends HttpMessageBody>({
    ...
    while (pageCount < TAPFILIATE_MAX_PAGES) {
        ...
        const nextPage = parseTapfiliateNextPage(response.headers?.['link'] ?? response.headers?.['Link'])
