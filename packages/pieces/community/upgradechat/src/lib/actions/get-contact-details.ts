...
export const getContactDetails = createAction({
    ...
    async run(context) {
        ...
        const queryParams = new URLSearchParams(
            Object.entries(filteredContact).map(([key, value]) => [key, String(value)]),
        )

        // Send GET request with query parameters in the URL
        const res = await httpClient.sendRequest({
            method: HttpMethod.GET,
            url: `${context.auth.props.base_url}/api/services/CRM/Contact/GetContactData?${queryParams.toString()}`,
            headers: {
