import { Account, Client, Pager, Plan, Subscription } from 'recurly'
import { RecurlyAuthType } from '../auth'
import { FlatRecord, flattenRecord, normalizeRecords } from './utils'

const MAX_DROPDOWN_ITEMS = 100

export function createRecurlyClient(auth: RecurlyAuthType): Client {
    return new Client(auth.secret_text)
}

export async function listAccounts(auth: RecurlyAuthType): Promise<Account[]> {
...
export async function listPlans(auth: RecurlyAuthType): Promise<Plan[]> {
    return await collectPagerItems(
...
export async function listAccountSubscriptions(
    ...
    return await collectPagerItems(
        createRecurlyClient(auth).listAccountSubscriptions(`code-${accountCode}`, {
            params,
        }),
        limit,
    )
...
async function collectPagerItems<T>(pager: Pager<T>, limit: number): Promise<T[]> {
    const items: T[] = []
