...
const polling: Polling<AppConnectionValueForAuthProperty<typeof foreplayCoAuth>, Record<string, any>> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        console.log(
            `[New Swipefile Ad Polling] Fetching swipefile ads, lastFetch: ${new Date(lastFetchEpochMS || 0).toISOString()}`,
        )
...
export const newSwipefileAd = createTrigger({
    ...
    sampleData: {
        ...
        media_urls: ['https://example.com/video1.mp4', 'https://example.com/image1.jpg'],
