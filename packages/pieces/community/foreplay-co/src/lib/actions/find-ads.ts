...
export const findAds = createAction({
    ...
    description: 'Search and filter ads by text, dates, platforms, and categories.',
    ...
    async run({ auth, propsValue }) {
        ...
        const fullUrl = queryString ? `/api/discovery/ads?${queryString}` : '/api/discovery/ads'
        ...
        if (responseBody.metadata && responseBody.metadata.success === true) {
            ...
            const errorMessage = responseBody.error || responseBody.metadata?.message || 'Failed to find ads'
