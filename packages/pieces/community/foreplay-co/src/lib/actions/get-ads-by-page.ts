...
export const getAdsByPage = createAction({
    ...
    description: 'Get all ads for a Facebook Page ID with filtering and pagination.',
    ...
    async run({ auth, propsValue }) {
        ...
        const fullUrl = queryString ? `/api/brand/getAdsByPageId?${queryString}` : '/api/brand/getAdsByPageId'
        ...
        if (responseBody.metadata && responseBody.metadata.success === true) {
            ...
            const errorMessage =
                responseBody.error || responseBody.metadata?.message || 'Failed to retrieve ads for page'
