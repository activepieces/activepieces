...
export class GristAPIClient {
  ...
  async listOrgs() {
    return await this.makeRequest<GristOrgResponse[]>(HttpMethod.GET, `/orgs`)
  }

  async listWorkspaces(orgId: string) {
    return await this.makeRequest<GristWorkspaceResponse[]>(HttpMethod.GET, `/orgs/${orgId}/workspaces`)
  }

  async getWorkspace(workspaceId: number) {
    return await this.makeRequest<GristWorkspaceResponse>(HttpMethod.GET, `/workspaces/${workspaceId}`)
  }

  async listDocumentTables(docId: string) {
    return await this.makeRequest<{ tables: GristTableResponse[] }>(HttpMethod.GET, `/docs/${docId}/tables`)
  }
  ...
  async addRecordsToTable(docId: string, tableId: string, request: GristCreateRecordsRequest) {
  ...
  async updateRcordsInTable(docId: string, tableId: string, request: GristUpdateRecordsRequest) {
    return await this.makeRequest<GristCreateRecordsResponse>(
      HttpMethod.PATCH,
      `/docs/${docId}/tables/${tableId}/records`,
      undefined,
  ...
  async listDocumentAttachments(docId: string, query: Query) {
    return await this.makeRequest<GristListRecordsResponse>(HttpMethod.GET, `/docs/${docId}/attachments`, {}, query)
  }

  async getDocumentAttachmentMetadata(docId: string, attachmetId: number) {
    return await this.makeRequest<GristListRecordsResponse>(HttpMethod.GET, `/docs/${docId}/attachments/${attachmetId}`)
  }
  ...
  for (const column of tableColumnSchema.columns) {
    ...
    if (columnValue !== undefined && columnValue !== '') {
      if (columnType === 'Attachments') {
        const attachmentsArray = Array.isArray(columnValue) ? columnValue : JSON.parse(columnValue)
        ...
        const refListArray = Array.isArray(columnValue) ? columnValue : JSON.parse(columnValue)
