...
export const createQrCodeAction = createAction({
    ...
    props: {
        ...
        destination: Property.DynamicProperties({
            ...
            props: async (propsValue: Record<string, unknown>) => {
                const destination_type = propsValue['destination_type'] as unknown as string
    ...
    async run(context) {
        ...
        try {
            ...
            if (props.background_color) customizations.background_color = props.background_color
            if (props.dot_pattern_color) customizations.dot_pattern_color = props.dot_pattern_color
            if (props.dot_pattern_type) customizations.dot_pattern_type = props.dot_pattern_type

            const corners: any = {}
            if (props.corner_1_shape || props.corner_1_inner_color || props.corner_1_outer_color)
                corners.corner_1 = {
                    shape: props.corner_1_shape,
                    inner_color: props.corner_1_inner_color,
                    outer_color: props.corner_1_outer_color,
                }
            if (props.corner_2_shape || props.corner_2_inner_color || props.corner_2_outer_color)
                corners.corner_2 = {
                    shape: props.corner_2_shape,
                    inner_color: props.corner_2_inner_color,
                    outer_color: props.corner_2_outer_color,
                }
            if (props.corner_3_shape || props.corner_3_inner_color || props.corner_3_outer_color)
                corners.corner_3 = {
            ...
            if (props.gradient_exclude_corners) gradient.exclude_corners = props.gradient_exclude_corners
            ...
            if (props.frame_primary_color) frameColors.primary = props.frame_primary_color
            if (props.frame_secondary_color) frameColors.secondary = props.frame_secondary_color
            if (props.frame_background_color) frameColors.background = props.frame_background_color
            if (Object.keys(frameColors).length > 0) frame.colors = frameColors
            if (props.frame_text) frame.text = { primary: { content: props.frame_text } }
            if (Object.keys(frame).length > 0) customizations.frame = frame

            const branding: any = {}
            if (props.bitly_brand !== undefined) branding.bitly_brand = props.bitly_brand
            ...
            if (props.error_correction) specSettings.error_correction = props.error_correction
            if (Object.keys(specSettings).length > 0) customizations.spec_settings = specSettings

            if (Object.keys(customizations).length > 0) body.render_customizations = customizations
