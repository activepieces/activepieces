...
export const createTask = createAction({
        ...
        task_configuration: Property.DynamicProperties({
            ...
            props: async ({ auth, task_type }) => {
                ...
                try {
                    ...
                    if (organizationOptions.length > 0) {
                        ...
                        try {
                            ...
                            userOptions =
                                usersResponse.body?.users?.map((user: any) => ({
                    ...
                    if ((task_type as unknown as string) === 'subtask') {
                        try {
                            ...
                            conversationOptions =
                                convsResponse.body?.conversations?.map((conv: any) => {
                ...
                props.organization = Property.StaticDropdown({
                    ...
                    description:
                        (task_type as unknown as string) === 'standalone'
                        ...
                        options: {
                            options:
                                teamOptions.length > 0
                                    ? teamOptions
                                    : [{ label: 'Select organization first or no teams found', value: '' }],
                        },
                    })
                                ...
                                options: {
                                    options:
                                        userOptions.length > 0
                                            ? userOptions
                                            : [{ label: 'Select organization first or no users found', value: '' }],
                                },
                            }),
                        },
                    })
                        ...
                        options: {
                            options:
                                conversationOptions.length > 0
                                    ? conversationOptions
                                    : [{ label: 'No conversations found', value: '' }],
                        },
                    })
    ...
    async run(context) {
        ...
        const { task_type, title, description, state, due_at } = propsValue
        ...
        if (task_type === 'standalone') {
            if (taskConfig['assignment_type'] === 'team' && taskConfig['team']) {
                ...
                taskConfig['assignment_type'] === 'users' &&
                taskConfig['assignees_array'] &&
                Array.isArray(taskConfig['assignees_array'])
            ) {
            ...
            if (taskConfig['conversation_selection'] === 'existing' && taskConfig['conversation']) {
                ...
                taskConfig['conversation_selection'] === 'references' &&
                taskConfig['references'] &&
                Array.isArray(taskConfig['references'])
            ) {
            ...
            if (
                taskConfig['assignees_array'] &&
                Array.isArray(taskConfig['assignees_array']) &&
                taskConfig['assignees_array'].length > 0
            ) {
                taskData['assignees'] = taskConfig['assignees_array'].map((assignee: any) => assignee.user)
            }

            if (
                taskConfig['add_users_array'] &&
                Array.isArray(taskConfig['add_users_array']) &&
                taskConfig['add_users_array'].length > 0
            ) {
