...
export const createDraftPost = createAction({
    ...
    props: {
                ...
                switch (message_type as unknown as string) {
                    case 'email':
                        props.from_email = Property.ShortText({
                            ...
                            description: 'Sender email address (must match your Missive email aliases)',
                            required: true,
                        })
                    ...
                    case 'whatsapp':
                        ...
                        if ((message_type as unknown as string) === 'whatsapp') {
                            ...
                            props.whatsapp_template_variables = Property.Object({
                                ...
                                description: 'Variables for WhatsApp template (e.g., {"1": "John", "2": "Project"})',
                                required: false,
                            })
                        }
                        break
                    ...
                    case 'instagram':
                        if (
                            ...
                            props.account_id = Property.ShortText({
                                ...
                                description: 'Account ID from Missive settings > API > Resource IDs',
                                required: true,
                            })
                        }
            ...
            props: async ({ auth }) => {
                ...
                return {
                    ...
                    team: Property.StaticDropdown({
                        ...
                        required: false,
                        options: {
                            options: teamOptions.length > 0 ? teamOptions : [{ label: 'No teams found', value: '' }],
                        },
                    }),
                    force_team: Property.Checkbox({
                        displayName: 'Force Team Assignment',
                        description: 'Force new team even if conversation is in another team',
                        required: false,
                        defaultValue: false,
                    }),
                    add_shared_labels: Property.StaticMultiSelectDropdown({
                    ...
                    add_to_team_inbox: Property.Checkbox({
                        displayName: 'Add to Team Inbox',
                        description: 'Move conversation to team inbox (requires team selection)',
                        required: false,
                        defaultValue: false,
                    }),
                    close_conversation: Property.Checkbox({
        ...
        send_at_timestamp: Property.Number({
            ...
            description: 'Unix timestamp when to send the message (required if scheduling)',
            required: false,
        }),
        auto_followup: Property.Checkbox({
            displayName: 'Auto Follow-up',
            description: 'Cancel scheduled send if there is a reply (requires scheduling)',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        ...
        switch (message_type) {
            case 'email':
                ...
                if (messageFields.to_emails && Array.isArray(messageFields.to_emails)) {
                    draftData.to_fields = messageFields.to_emails.map((recipient: any) => ({
                ...
                if (
                    ...
                    draftData.cc_fields = messageFields.cc_emails.map((recipient: any) => ({
                        address: recipient.address,
                        ...(recipient.name && { name: recipient.name }),
                    }))
                }
                ...
                if (
                    ...
                    draftData.bcc_fields = messageFields.bcc_emails.map((recipient: any) => ({
                        address: recipient.address,
                        ...(recipient.name && { name: recipient.name }),
                    }))
                }
            ...
            case 'whatsapp':
                ...
                if (message_type === 'whatsapp') {
                    ...
                    if (messageFields.whatsapp_template_variables) {
                        draftData.external_response_variables = messageFields.whatsapp_template_variables
                    }
                }
                break
            ...
            case 'custom_channel':
                ...
                if (messageFields.from_id || messageFields.from_username || messageFields.from_name) {
                    draftData.from_field = {
                ...
                if (messageFields.to_recipients && Array.isArray(messageFields.to_recipients)) {
                    draftData.to_fields = messageFields.to_recipients.map((recipient: any) => ({
        ...
        if (
            ...
            draftData.references = conversationOptions.references.map((ref: any) => ref.reference)
        }
