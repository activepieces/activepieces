...
export const getLbrPrice = createAction({
  name: 'get_lbr_price',
  displayName: 'Get LBR Price',
  description:
    'Fetch the current price and market data for the LBR governance token from CoinGecko.',
  auth: undefined,
  props: {
  ...
  async run(context) {
    ...
    const response = await httpClient.sendRequest({
      ...
      url: 'https://api.coingecko.com/api/v3/coins/lybra-finance',
      queryParams: {
        localization: 'false',
        tickers: 'false',
    ...
    return {
      ...
      high_24h: getPrice(marketData, 'high_24h'),
      low_24h: getPrice(marketData, 'low_24h'),
