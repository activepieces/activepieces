...
- **Admin rights** in that tenant, since you'll need to grant admin consent for the Graph permissions.
...
```
https://<your-activepieces-url>/api/v1/app-events/microsoft-teams-bot
```

e.g. `https://cloud.activepieces.com/api/v1/app-events/microsoft-teams-bot`, or your tunnel URL when developing locally. Azure requires a messaging endpoint to be set; Activepieces acknowledges the activities Teams delivers here and does not need them to send messages.
...
**The bot must be installed in a team before you can post to its channels.** Sending to a team the bot is not a member of fails with `403 The bot is not part of the conversation roster`.
