...
import { FATHOM_API_BASE, fathomAuth } from '../auth'
import { AGGREGATION_OPTIONS, DATE_GROUPING_OPTIONS, ENTITY_OPTIONS, FIELD_GROUPING_OPTIONS } from '../common'
...
export const getAggregation = createAction({
    ...
    async run(context) {
        ...
        if (propsValue.entity === 'event') {
            ...
            if (propsValue.entity_name) queryParams['entity_name'] = propsValue.entity_name
        } else {
            if (propsValue.entity_id) queryParams['entity_id'] = propsValue.entity_id
        }

        if (propsValue.date_grouping) queryParams['date_grouping'] = propsValue.date_grouping
        if (propsValue.field_grouping) queryParams['field_grouping'] = propsValue.field_grouping
        ...
        if (propsValue.limit != null) queryParams['limit'] = String(propsValue.limit)
        if (propsValue.filters) queryParams['filters'] = JSON.stringify(propsValue.filters)
