...
export const createAssistantRun = createAction({
    ...
    description: 'Create a run for an assistant with optional additional resources',
    props: {
    ...
    async run(context) {
        const { assistant_id, input, knowledgeBaseIds, rulesets, structures, tools, thread, create_new_thread } =
            context.propsValue
        ...
        while (runStatus !== 'SUCCEEDED') {
            ...
            runData = await makeRequest(context.auth.secret_text, HttpMethod.GET, `/assistant-runs/${assistantRunId}`)
