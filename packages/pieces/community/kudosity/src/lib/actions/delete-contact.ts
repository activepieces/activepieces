...
export const deleteContact = createAction({
    ...
    props: {
        listId: Property.ShortText({
            displayName: 'List ID',
            description: 'Numeric ID of the recipient list (found in the list page URL).',
            required: false,
        }),
    ...
    async run(context) {
        ...
        const res = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/delete-from-list.json', payload)
        return res
    },
})
