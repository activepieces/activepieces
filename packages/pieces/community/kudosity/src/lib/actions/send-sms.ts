...
export const sendSms = createAction({
    ...
    props: {
        sender: Property.ShortText({
            displayName: 'Sender',
            description: 'Sender number or alphanumeric sender that is assigned to your account',
            required: true,
        }),
        ...
        messageRef: Property.ShortText({
            ...
            description: 'Optional reference string to correlate messages with your system',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (context.propsValue.messageRef) payload.message_ref = context.propsValue.messageRef
        if (context.propsValue.trackLinks !== undefined) payload.track_links = context.propsValue.trackLinks
