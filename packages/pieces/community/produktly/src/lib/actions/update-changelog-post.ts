...
export const updateChangelogPost = createAction({
  ...
  props: {
    ...
    post_id: Property.ShortText({
      displayName: 'Post ID',
      description:
        'The ID of the post to update. Use "List Changelog Posts" to find it.',
      required: true,
    }),
    title: Property.ShortText({
      displayName: 'Title',
      description:
        'New title for the post. Leave empty to keep the current one.',
      required: false,
    }),
    description: Property.LongText({
      displayName: 'Description',
      description:
        'New body for the post. Leave empty to keep the current one.',
      required: false,
    }),
    date: Property.ShortText({
      displayName: 'Date',
      description:
        'New release date in ISO 8601 format. Leave empty to keep the current one.',
      required: false,
    }),
    ...
    tag_names: Property.Array({
      ...
      description:
        'Replace the list of tags on the post. Must match existing tags (use "List Tags" to find them).',
      required: false,
    }),
  },
