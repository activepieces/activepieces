    ...
    try {
        ...
        if (error.response) {
            ...
            switch (status) {
                case 400:
                    throw new Error(
                        `Bad request: ${error.response.body?.error?.message || 'Invalid request parameters'}`,
                    )
                case 401:
                    throw new Error(
                        `Authorization required: ${error.response.body?.error?.message || 'Invalid API credentials'}`,
                    )
                case 403:
                ...
                case 500:
                    throw new Error(
                        `Internal server error: ${error.response.body?.error?.message || 'Cloudinary server error'}`,
                    )
                default:
                    throw new Error(
                        `Request failed (${status}): ${error.response.body?.error?.message || 'Unknown error'}`,
                    )
            }
        }
