...
export const cloudinaryAuth = PieceAuth.CustomAuth({
    required: true,
    description:
        'Enter your Cloudinary account credentials. Find these in your Cloudinary Console settings under the API keys page.',
    props: {
        ...
        api_secret: PieceAuth.SecretText({
            ...
            description:
                'Your Cloudinary API Secret. Found in Console Settings → API Keys page. Keep this secure and never share publicly.',
            required: true,
        }),
        cloud_name: Property.ShortText({
            displayName: 'Cloud Name',
            description:
                'Your Cloudinary Cloud Name. Found in Console Settings → API Keys page or in your dashboard URL (e.g., "demo" from console.cloudinary.com/console/demo).',
            required: true,
        }),
    },
    validate: async ({ auth }) => {
