...
export const contactSuspendedFromCourse = createTrigger({
    ...
    async onEnable({ auth, webhookUrl, propsValue: { workspaceId }, store }) {
        ...
        try {
            const response: any = await clickfunnelsApiService.createWebhook(auth.props, workspaceId as string, {
    ...
    async onDisable({ auth, store }) {
        ...
        if (cachedData) {
            await clickfunnelsApiService.deleteWebhook(auth.props, cachedData.webhookId).then(async () => {
