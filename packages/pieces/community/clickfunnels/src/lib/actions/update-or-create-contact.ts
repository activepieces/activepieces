...
export const updateOrCreateContact = createAction({
    ...
    props: {
        ...
        emailAddress: Property.ShortText({
            displayName: 'Email',
            description: "The contact's email address. This is used to find and (update/create) the contact.",
            required: true,
        }),
        ...
        customAttributes: Property.Object({
            ...
            description:
                'A key-value object for custom contact data. Keys that are default properties on the Contact resource or variations of it will result in an error. E.g., first_name, First Name, etc. are not valid inputs.',
            required: false,
        }),
    },
    async run({ auth, propsValue }) {
        ...
        const response = await clickfunnelsApiService.upsertContact(
            auth.props,
