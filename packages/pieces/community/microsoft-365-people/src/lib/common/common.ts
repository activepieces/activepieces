...
export const microsoft365PeopleCommon = {
    ...
    listContacts: async ({ auth, queryParams }: listContactProps): Promise<Contact[]> => {
        const client = microsoft365PeopleCommon.getClient({ auth })
    ...
    updateContact: async ({ auth, contactId, contact }: updateContactProps) => {
        const client = microsoft365PeopleCommon.getClient({ auth })
        const response = await client.api(`/me/contacts/${contactId}`).patch(contact)
        return response
    },
    ...
    createContactFolder: async ({ auth, contactFolder }: createContactFolderProps) => {
        const client = microsoft365PeopleCommon.getClient({ auth })
        const response = await client.api('/me/contactFolders').post(contactFolder)
        return response
    },
    getContactFolder: async ({ auth, contactFolderId }: getContactFolderProps) => {
        const client = microsoft365PeopleCommon.getClient({ auth })
        const response = await client.api(`/me/contactFolders/${contactFolderId}`).get()
        return response
    },

    deleteContactFolder: async ({ auth, contactFolderId }: deleteContactFolderProps) => {
        const client = microsoft365PeopleCommon.getClient({ auth })
    ...
    // Dropdowns
    contactDropdown: (displayName = 'Contact', description = 'Select a Contact', required = true) =>
        Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                return {
                    placeholder: options.length === 0 ? "You don't have any contacts." : 'Select a contact',
                    options: options,
                    disabled: options.length === 0,
                }
            },
        }),
    contactFolderDropdown: (displayName = 'Contact Folder', description = 'Select an option', required = true) =>
        Property.Dropdown({
            auth: microsoft365PeopleAuth,
            displayName,
            description,
            ...
            options: async ({ auth }) => {
                ...
                const contactFolders = await microsoft365PeopleCommon.listContactFolders({
                    auth: auth as OAuth2PropertyValue,
                })
                ...
                return {
                    placeholder: options.length === 0 ? "You don't have any folders created." : 'Select a folder',
                    options: options,
                    disabled: options.length === 0,
                }
            },
    ...
    contactProperties: () => ({
        ...
        parentFolder: microsoft365PeopleCommon.contactFolderDropdown('Parent Folder', 'Select a parent folder', false),
