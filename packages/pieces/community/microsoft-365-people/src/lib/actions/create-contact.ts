...
export const createContact = createAction({
    ...
    async run({ auth, propsValue }) {
        const childrenNames: string[] =
            (propsValue.children as Array<{ name: string }> | undefined)?.map((child) => child.name) ?? []
        const emailAddresses: EmailAddress[] = (propsValue.emailAddresses as Array<EmailAddress> | undefined) ?? []
        const imAddresses: string[] =
            (propsValue.imAddresses as Array<{ address: string }> | undefined)?.map((im) => im.address) ?? []
