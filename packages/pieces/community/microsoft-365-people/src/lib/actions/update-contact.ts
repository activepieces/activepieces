...
export const updateContact = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        const childrenNames: string[] | undefined = (propsValue.children as Array<{ name: string }> | undefined)?.map(
            (child) => child.name,
        )
        const emailAddresses: EmailAddress[] | undefined = propsValue.emailAddresses as Array<EmailAddress> | undefined
