...
const paddle = createPiece({
    ...
    actions: [
        ...
        createCustomApiCallAction({
            auth: paddleAuth,
            baseUrl: (auth) =>
                auth ? paddleClient.getBaseUrl({ apiKey: auth.secret_text }) : paddleClient.getBaseUrl({ apiKey: '' }),
    ...
    triggers: [newActiveSubscription, subscriptionCanceled, subscriptionPastDue, transactionCompleted, paymentFailed],
})
