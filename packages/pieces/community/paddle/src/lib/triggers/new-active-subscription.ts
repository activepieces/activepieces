...
export const newActiveSubscription = createTrigger({
    ...
    description: 'Triggers when a subscription activates — a trial converts or the first payment succeeds.',
    props: {},
    sampleData: {
    ...
    async onDisable(context) {
        const notificationSettingId = await context.store.get<string>('notificationSettingId')
