...
async function validateAuth({ apiKey }: { apiKey: string }): Promise<void> {
...
function getBaseUrl({ apiKey }: { apiKey: string }): string {
    return apiKey.includes('_sdbx_') ? PADDLE_SANDBOX_API_BASE_URL : PADDLE_LIVE_API_BASE_URL
}
...
function buildHeaders({ apiKey }: { apiKey: string }): Record<string, string> {
    return {
...
function getNextAfter({ nextUrl }: { nextUrl: string | null }): string | undefined {
    if (!nextUrl) {
        return undefined
    }
