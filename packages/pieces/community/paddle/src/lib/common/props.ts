        ...
        options: async ({ auth, searchValue }) => {
            ...
            const { data: customers, error } = await tryCatch(() =>
                paddleClient.listCustomers({
                    ...
                    ...(normalizedSearchValue?.includes('@') ? { email: normalizedSearchValue } : {}),
                }),
            )
...
function buildAddressLabel({
    ...
    const parts = [address.country_code, address.region, address.postal_code].filter(
...
function normalizeSearchValue({ searchValue }: { searchValue: unknown }): string | undefined {
