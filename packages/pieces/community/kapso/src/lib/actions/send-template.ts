...
import { kapsoAuth, makeClient } from '../common'
import { businessAccountIdDropdown, phoneNumberIdDropdown, templateDropdown } from '../common/props'
...
export const sendTemplate = createAction({
    ...
    props: {
        ...
        to: Property.ShortText({
            ...
            description: "The recipient's phone number in international format (e.g. 15551234567).",
            required: true,
        }),
    ...
    async run(context) {
        ...
        if (headerType === 'text' && headerTextParameters && (headerTextParameters as unknown[]).length > 0) {
            components.push({
                type: 'header',
                parameters: (headerTextParameters as { parameterName: string; text: string }[]).map((p) => ({
                    type: 'text',
        ...
        if (bodyParameters && (bodyParameters as unknown[]).length > 0) {
            components.push({
                type: 'body',
                parameters: (bodyParameters as { parameterName?: string; text: string }[]).map((p) => {
                ...
                if (btn.subType === 'quick_reply') {
                    ...
                    if (btn.flowActionData) {
                        action['flow_action_data'] =
                            typeof btn.flowActionData === 'string' ? JSON.parse(btn.flowActionData) : btn.flowActionData
                    }
                    components.push({
                        type: 'button',
                        sub_type: 'flow',
