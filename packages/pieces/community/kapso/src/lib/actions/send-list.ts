...
export const sendList = createAction({
    ...
    async run(context) {
        const { phoneNumberId, to, bodyText, buttonText, sections, headerText, footerText } = context.propsValue
        const client = makeClient(context.auth.secret_text)

        const parsedSections = (sections as { title?: string; rows: unknown }[]).map((s) => ({
            title: s.title,
