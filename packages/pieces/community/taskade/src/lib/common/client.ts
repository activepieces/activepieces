...
export class TaskadeAPIClient {
  ...
  async listWorkspaceFolders(workspace_id: string): Promise<ListAPIResponse<WorkspaceFolderResponse>> {
  ...
  async createTaskDate(projectId: string, taskId: string, params: CreateTaskDateParams) {
    return await this.makeRequest(HttpMethod.PUT, `/projects/${projectId}/tasks/${taskId}/date`, undefined, params)
  }

  async listTasks(projectId: string, params: RequestParams): Promise<ListAPIResponse<TaskResponse>> {
  ...
  async completeTask(projectId: string, taskId: string) {
    return await this.makeRequest(HttpMethod.POST, `/projects/${projectId}/tasks/${taskId}/complete`, undefined, {})
  }
