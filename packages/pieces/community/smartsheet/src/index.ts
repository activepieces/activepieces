...
import { updateMultipleRows } from './lib/actions/update-multiple-rows';
...
- The Smartsheet API is only available for Business and Enterprise plans
- Tokens have different base URLs for different regions (US, EU, Australia, Gov)
`;
...
export const smartsheetAuth = PieceAuth.SecretText({
  ...
  validate: async ({ auth }) => {
    ...
    try {
      ...
      return {
        valid: false,
        error: 'Invalid access token, insufficient permissions, or region mismatch. Ensure token is for api.smartsheet.com.',
      };
    }
  },
});
...
export const smartsheet = createPiece({
  ...
  authors: ["activepieces", "TaskLinker"],
  actions: [
    addRowToSheet,
    updateRow,
    updateMultipleRows,
