...
export const smartsheetCommon = {
    ...
    column_id: Property.Dropdown({
        ...
        options: async ({ auth, sheet_id }) => {
            ...
            try {
                const columns = await getSheetColumns(auth.secret_text, sheet_id as unknown as string)
    ...
    row_id: Property.Dropdown({
        ...
        options: async ({ auth, sheet_id }) => {
            ...
            try {
                const sheet = await getSheet(auth.secret_text, sheet_id as unknown as string)
...
export async function findSheetsByName(accessToken: string, name: string): Promise<SmartsheetSheet[]> {
    const sheets = await listSheets(accessToken)
    return sheets.filter((sheet) => sheet.name.toLowerCase().includes(name.toLowerCase()))
}

export async function listReports(accessToken: string, modifiedSince?: string): Promise<SmartsheetReport[]> {
...
export async function enableWebhook(accessToken: string, webhookId: string): Promise<SmartsheetWebhook> {
    const request: HttpRequest = {
        method: HttpMethod.PUT,
...
export function verifyWebhookSignature(
    ...
    try {
        ...
        return crypto.timingSafeEqual(Buffer.from(webhookSignatureHeader, 'hex'), Buffer.from(expectedSignature, 'hex'))
    } catch (error) {
        return false
    }
}
...
export async function getAttachmentFullDetails(
    accessToken: string,
    sheetId: string,
    attachmentId: string,
): Promise<SmartsheetAttachment | null> {
    try {
        const req: HttpRequest = {
            method: HttpMethod.GET,
...
export async function getCommentFullDetails(
    accessToken: string,
    sheetId: string,
    discussionId: string,
    commentId: string,
): Promise<SmartsheetComment | null> {
    try {
        const req: HttpRequest = {
            method: HttpMethod.GET,
