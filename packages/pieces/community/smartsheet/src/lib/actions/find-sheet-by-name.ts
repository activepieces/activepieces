        ...
        try {
            ...
            if (sheet_name) {
                ...
                filteredSheets = filteredSheets.filter((sheet: any) => {
                    ...
                    return exact_match ? sheetName === searchName : sheetName.includes(searchName)
                })
            }

            // Filter by access level
            if (access_level_filter && access_level_filter.length > 0) {
                filteredSheets = filteredSheets.filter((sheet: any) => access_level_filter.includes(sheet.accessLevel))
            }
