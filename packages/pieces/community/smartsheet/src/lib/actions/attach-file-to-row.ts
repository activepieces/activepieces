...
export const attachFileToRow = createAction({
    ...
    async run(context) {
        const { sheet_id, row_id, attachment_type, file, url, attachment_name, attachment_sub_type, mime_type } =
            context.propsValue
        ...
        try {
            ...
            if (attachment_type === 'FILE') {
                ...
                // Determine MIME type
                const fileMimeType =
                    mime_type || (file?.extension ? `application/${file.extension}` : 'application/octet-stream')
