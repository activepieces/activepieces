...
import { Property, TriggerHookContext, TriggerStrategy } from '@activepieces/pieces-framework'
...
export async function subscribeWebhook(auth: string, webhookUrl: string, event: WebhookEvent): Promise<void> {
...
export async function unsubscribeWebhook(auth: string, webhookUrl: string, event: WebhookEvent): Promise<void> {
    await httpClient.sendRequest({
        method: HttpMethod.POST,
