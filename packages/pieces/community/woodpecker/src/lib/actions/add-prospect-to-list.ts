...
export const addProspectToList = createAction({
    ...
    props: {
        ...
        update: Property.Checkbox({
            ...
            description:
                'If enabled, updates existing prospect data. If disabled, existing prospects remain unchanged.',
            required: false,
            defaultValue: false,
        }),
        ...
        tags: Property.ShortText({
            displayName: 'Tags',
            description:
                'Tags starting with # separated by spaces (e.g. #VC #Startup). Appends to existing tags when updating.',
            required: false,
        }),
        set_tags: Property.ShortText({
            displayName: 'Replace Tags',
            description:
                'Replaces all existing tags with these (only when updating). Use empty string to clear all tags.',
            required: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        return await woodpeckerClient.makeRequest(auth.secret_text, HttpMethod.POST, '/v1/add_prospects_list', body)
    },
})
