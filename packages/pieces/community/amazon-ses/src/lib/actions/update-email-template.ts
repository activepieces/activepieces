...
export const updateEmailTemplate = createAction({
    ...
    async run(context) {
        const { templateName, templateFormat, subjectPart, htmlPart, textPart, preserveUnspecified, sampleData } =
            context.propsValue
        ...
        if (preserveUnspecified) {
            currentTemplate = await getEmailTemplate({ accessKeyId, secretAccessKey, region }, templateName)

            if (!currentTemplate) {
                throw new Error(`Template "${templateName}" does not exist. Cannot update non-existent template.`)
            }
        }
        ...
        if (templateFormat === 'both' && (!finalHtmlPart || !finalTextPart)) {
            throw new Error('Both HTML and text content are required when using both formats')
        }

        validateTemplateContent(finalHtmlPart, finalTextPart)

        const templateVariables = validateTemplateVariables(subjectPart, finalHtmlPart, finalTextPart)
        ...
        try {
            ...
            let preview: any = {}
            if (sampleData && Object.keys(sampleData as Record<string, string>).length > 0) {
