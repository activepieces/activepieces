...
import { SESClient, UpdateCustomVerificationEmailTemplateCommand } from '@aws-sdk/client-ses'
...
export const updateCustomVerificationEmailTemplate = createAction({
    ...
    props: {
        templateName: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                if (templates.length === 0) {
                    return {
                        disabled: false,
                        placeholder: 'No custom verification templates found. Create one first.',
                        options: [],
                    }
                }
        ...
        templateContent: Property.LongText({
            ...
            description: 'HTML content for verification email (must include verification link)',
            required: true,
        }),
    ...
    async run(context) {
        ...
        const finalFromEmail = fromEmailAddress || currentTemplate?.fromEmailAddress
        ...
        const finalSuccessUrl = successRedirectionURL || currentTemplate?.successRedirectionURL
        const finalFailureUrl = failureRedirectionURL || currentTemplate?.failureRedirectionURL
        ...
        const hasLink = contentLower.includes('href') || contentLower.includes('link')
        const hasVerificationText =
            contentLower.includes('verify') || contentLower.includes('confirm') || contentLower.includes('activate')
        ...
        try {
            ...
            return {
                ...
                recommendations: [
                    ...(hasLink ? [] : ['Consider adding verification link in template content']),
                    ...(hasVerificationText ? [] : ['Consider adding clear verification instructions']),
            ...
            const errorMessage = getCustomVerificationErrorMessage(error, templateName)
