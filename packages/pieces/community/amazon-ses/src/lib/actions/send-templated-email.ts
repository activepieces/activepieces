...
export const sendTemplatedEmail = createAction({
    ...
    props: {
        ...
        templateData: Property.Object({
            ...
            description: 'Data to replace template variables (e.g., {"firstName": "John", "company": "Acme"})',
            required: true,
        }),
    ...
    async run(context) {
        ...
        if (!templateData || Object.keys(templateData as Record<string, any>).length === 0) {
            throw new Error('Template variables are required. Provide at least one key-value pair.')
        }

        const validatedToAddresses = validateEmailAddresses(toAddresses as string[], 'To addresses')
        const validatedCcAddresses = validateEmailAddresses(ccAddresses as string[], 'CC addresses')
        const validatedBccAddresses = validateEmailAddresses(bccAddresses as string[], 'BCC addresses')
        const validatedReplyToAddresses = validateEmailAddresses(replyToAddresses as string[], 'Reply-to addresses')

        validateRecipientLimits(validatedToAddresses, validatedCcAddresses, validatedBccAddresses)

        if (returnPath && !validateEmailAddresses([returnPath], 'Return path').length) {
        ...
        try {
            ...
            throw new Error('Template data must be a valid object with key-value pairs')
        }
        ...
        try {
            ...
            const totalRecipients =
                validatedToAddresses.length + validatedCcAddresses.length + validatedBccAddresses.length
