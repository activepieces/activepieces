...
export const sendCustomVerificationEmail = createAction({
    ...
    description: 'Send verification email to add an email address to SES identities',
    props: {
    ...
    async run(context) {
        const { emailAddress, templateName, configurationSetName, validateEmailFormat, checkExistingIdentity } =
            context.propsValue

        const { accessKeyId, secretAccessKey, region } = context.auth.props

        if (validateEmailFormat) {
            const validatedEmails = validateEmailAddresses([emailAddress], 'Email address')
        ...
        if (checkExistingIdentity) {
            try {
                ...
                if (verifiedIdentities.includes(emailAddress)) {
                    console.warn(`Email address ${emailAddress} is already a verified identity`)
                }
            } catch (error) {
        ...
        const isCommonDomain = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com'].includes(
        ...
        try {
            ...
            return {
                ...
                details: {
                    ...
                    estimatedDeliveryTime: isCommonDomain ? '1-5 minutes' : '1-15 minutes',
                },
            }
        } catch (error: any) {
