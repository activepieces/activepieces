...
export const createEmailTemplate = createAction({
    ...
    async run(context) {
        const { templateName, templateFormat, subjectPart, htmlPart, textPart, checkExisting, sampleData } =
            context.propsValue
        ...
        const templateVariables = validateTemplateVariables(subjectPart, htmlPart, textPart)
        ...
        if (checkExisting) {
            try {
                ...
                if (existingTemplates.includes(templateName)) {
                    throw new Error(`Template "${templateName}" already exists. Please choose a different name.`)
                }
            } catch (error: any) {
