...
export async function getVerifiedIdentities(auth: SESAuth): Promise<string[]> {
    ...
    try {
        // Get all identities
        const identitiesResponse = await sesClient.send(new ListIdentitiesCommand({}))
        ...
        const verifiedIdentities = identities.filter(
            (identity) => verificationResponse.VerificationAttributes?.[identity]?.VerificationStatus === 'Success',
        )
...
export async function getConfigurationSets(auth: SESAuth): Promise<string[]> {
    ...
    try {
        ...
        return response.ConfigurationSets?.map((cs) => cs.Name).filter((name): name is string => !!name) || []
    } catch (error) {
 ...
 */
export function validateEmailAddresses(emails: string[] | string | undefined, fieldName: string): string[] {
    if (!emails) return []
    ...
    if (invalidEmails.length > 0) {
        throw new Error(`Invalid email addresses in ${fieldName}: ${invalidEmails.join(', ')}`)
    }
...
export function validateRecipientLimits(
    ...
    if (totalRecipients > 50) {
        throw new Error(`Too many recipients (${totalRecipients}). AWS SES allows maximum 50 recipients per email.`)
    }
}
 ...
 */
export function getSESErrorMessage(error: any, configurationSetName?: string): string {
    switch (error.name) {
        ...
        default:
            return `Failed to send email: ${error.message || 'Unknown error occurred'}`
    }
}
...
export function createIdentityDropdownOptions(identities: string[]): {
    ...
    if (identities.length === 0) {
        return {
            disabled: false,
            placeholder: 'No verified identities found. Please verify an email address or domain in AWS SES console.',
            options: [],
        }
    }
...
export async function getEmailTemplates(auth: SESAuth): Promise<string[]> {
    ...
    try {
        ...
        return (
            response.TemplatesMetadata?.map((template) => template.Name).filter((name): name is string => !!name) || []
        )
    } catch (error) {
 ...
 */
export function validateTemplateContent(htmlPart?: string, textPart?: string): void {
 ...
 */
export function getTemplateErrorMessage(error: any, templateName?: string): string {
    switch (error.name) {
        ...
        case 'ThrottlingException':
            return 'Request was throttled. Please retry after a moment.'
        default:
            return `Failed to process template: ${error.message || 'Unknown error occurred'}`
    }
}
 ...
 */
export function validateTemplateVariables(subject: string, htmlPart?: string, textPart?: string): string[] {
 ...
 */
export function createTemplatePreview(content: string, sampleData: Record<string, string> = {}): string {
    let preview = content
 ...
 */
export async function getCustomVerificationTemplates(auth: SESAuth): Promise<string[]> {
    const sesClient = createSESClient(auth)

    try {
        const response = await sesClient.send(new ListCustomVerificationEmailTemplatesCommand({}))
        return (
            response.CustomVerificationEmailTemplates?.map((template) => template.TemplateName).filter(
...
export function validateURL(url: string, fieldName: string): void {
    ...
    try {
        ...
        throw new Error(`${fieldName} must be a valid URL (e.g., https://example.com)`)
    }
}
...
export function validateCustomVerificationContent(content: string): void {
    ...
    if (contentSizeBytes > maxSizeBytes) {
        throw new Error(
            `Template content size (${Math.round(contentSizeBytes / 1024 / 1024)}MB) exceeds the 10MB limit`,
        )
    }
}
...
export function compareCustomVerificationContent(
    ...
    const changes = {
        ...
        successUrlChanged: current.successRedirectionURL !== updated.successRedirectionURL,
        failureUrlChanged: current.failureRedirectionURL !== updated.failureRedirectionURL,
    }
 ...
 */
export function getCustomVerificationErrorMessage(error: any, templateName?: string): string {
    switch (error.name) {
        ...
        case 'ThrottlingException':
            return 'Request was throttled. Please retry after a moment.'
        default:
            return `Failed to process custom verification template: ${error.message || 'Unknown error occurred'}`
    }
}
