...
import { getTeableBaseUrl, getTeableToken, TeableAuth, TeableAuthValue } from '../auth'
...
export function makeClient(auth: TeableAuthValue) {
    return new TeableClient(getTeableToken(auth), getTeableBaseUrl(auth))
}
...
export { TeableComputedFieldTypes, TeableFieldType }
