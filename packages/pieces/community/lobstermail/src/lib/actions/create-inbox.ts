...
export const createInbox = createAction({
    ...
    async run(context) {
        ...
        if (context.propsValue.displayName) body['displayName'] = context.propsValue.displayName
        if (context.propsValue.localPart) body['localPart'] = context.propsValue.localPart
