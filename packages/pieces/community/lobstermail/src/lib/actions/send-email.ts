...
import { fromAddressDropdown, lobstermailCommon } from '../common'

export const sendEmail = createAction({
    auth: lobstermailAuth,
    name: 'send_email',
    displayName: 'Send Email',
    description: 'Send an email from one of your LobsterMail inboxes. Requires a verified account (Tier 1+).',
    props: {
    ...
    async run(context) {
        const { from, to, subject, body_text, body_html, cc, in_reply_to, thread_id } = context.propsValue

        if (!body_text && !body_html) {
            throw new Error('At least one of "Body (Plain Text)" or "Body (HTML)" must be provided.')
        }
