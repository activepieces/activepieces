...
import { inboxIdDropdownOptional, lobstermailCommon } from '../common'

export const newThreadTrigger = createTrigger({
    auth: lobstermailAuth,
    name: 'new_thread',
    displayName: 'New Email Thread',
    description: 'Triggers when a brand-new conversation (thread) is started in an inbox.',
        ...
        return [
            {
                ...
                to: Array.isArray(data.recipients) ? data.recipients.join(', ') : (data.recipients ?? null),
