...
import { CohereChatRequest, CohereChatResponse, CohereErrorResponse } from '../common/types'

const COHERE_CHAT_URL = 'https://api.cohere.com/v2/chat'
const MAX_RETRIES = 3
const INITIAL_BACKOFF_MS = 1000

async function generateWithRetry(apiKey: string, request: CohereChatRequest): Promise<CohereChatResponse> {
        ...
        try {
            ...
            if (!isRetryable || attempt === MAX_RETRIES - 1) {
                if (errorAny.response?.body) {
                    ...
                    throw new Error(`Cohere API error (${statusCode}): ${errorBody.message ?? lastError.message}`)
                }
                throw lastError
            }
...
export const generateText = createAction({
    ...
    props: {
        ...
        temperature: Property.Number({
            ...
            description: 'Controls randomness (0.0 = deterministic, 1.0 = maximum randomness). Default: 0.3',
            required: false,
            defaultValue: 0.3,
        }),
