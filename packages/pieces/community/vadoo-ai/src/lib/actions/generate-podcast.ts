...
export const generatePodcast = createAction({
    ...
    async run(context) {
        ...
        const { content_source, url, text, name1, voice1, name2, voice2, theme, language, duration, tone } =
            context.propsValue

        // Validate content source requirements
        if (content_source === 'url' && !url) {
            throw new Error("URL is required when content source is 'Website/PDF URL'")
        }
        if (content_source === 'text' && !text) {
            throw new Error("Custom text is required when content source is 'Custom Text'")
        }
