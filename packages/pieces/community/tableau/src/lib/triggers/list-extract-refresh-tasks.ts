...
import { buildTableauUrl, getAuthToken, getTableauHeaders } from '../common'
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof tableauAuth>, Record<string, never>> = {
    ...
    items: async ({ auth }) => {
        ...
        const taskMatches = responseBody.matchAll(
            /<extractRefresh[^>]*id="([^"]+)"[^>]*priority="([^"]*)"[^>]*consecutiveFailedCount="([^"]*)"[^>]*type="([^"]*)"/g,
        )

        for (const match of taskMatches) {
            const [, taskId, priority, consecutiveFailedCount, type] = match

            const scheduleMatch = responseBody.match(
                new RegExp(
                    `<extractRefresh[^>]*id="${taskId}"[^>]*>[\\s\\S]*?<schedule[^>]*id="([^"]*)"[^>]*name="([^"]*)"[^>]*state="([^"]*)"[^>]*priority="([^"]*)"[^>]*createdAt="([^"]*)"[^>]*updatedAt="([^"]*)"[^>]*type="([^"]*)"[^>]*frequency="([^"]*)"[^>]*nextRunAt="([^"]*)"`,
                ),
            )
            ...
            if (scheduleMatch) {
                ...
                const cloudScheduleMatch = responseBody.match(
                    new RegExp(
                        `<extractRefresh[^>]*id="${taskId}"[^>]*>[\\s\\S]*?<schedule[^>]*frequency="([^"]*)"[^>]*nextRunAt="([^"]*)"`,
                    ),
                )
                if (cloudScheduleMatch) {
                    ...
                    const frequencyDetailsMatch = responseBody.match(
                        new RegExp(
                            `<extractRefresh[^>]*id="${taskId}"[^>]*>[\\s\\S]*?<frequencyDetails[^>]*start="([^"]*)"`,
                        ),
                    )
                    if (frequencyDetailsMatch) {
                        ...
                        const intervalMatches = [
                            ...responseBody.matchAll(
                                new RegExp(
                                    `<extractRefresh[^>]*id="${taskId}"[^>]*>.*?<interval[^>]*>.*?<weekDay>([^<]+)</weekDay>`,
                                    'gs',
                                ),
                            ),
                        ]
                        if (intervalMatches.length > 0) {
                            ;(schedule as any).frequencyDetails.intervals = intervalMatches.map(
                                (m: RegExpMatchArray) => ({ weekDay: m[1] }),
                            )
                        }
                    }
                }
            ...
            const workbookMatch = responseBody.match(
                new RegExp(`<extractRefresh[^>]*id="${taskId}"[^>]*>[\\s\\S]*?<workbook[^>]*id="([^"]+)"`),
            )
            const datasourceMatch = responseBody.match(
                new RegExp(`<extractRefresh[^>]*id="${taskId}"[^>]*>[\\s\\S]*?<datasource[^>]*id="([^"]+)"`),
            )
