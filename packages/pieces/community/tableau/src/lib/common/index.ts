...
<tsRequest>
  ${credentialsXml}
    ...
    if (!authToken || !siteId) {
        throw new Error(
            `Failed to obtain authentication token or site ID from response. Response type: ${typeof signinResponse.body}. Response: ${JSON.stringify(signinResponse.body).substring(0, 500)}`,
        )
    }
...
export async function querySites(auth: any): Promise<any[]> {
    ...
    const siteMatches = responseBody.matchAll(
        /<site[^>]*id="([^"]+)"[^>]*name="([^"]+)"[^>]*contentUrl="([^"]*)"[^>]*>/g,
    )
...
export async function queryWorkbooks(auth: any): Promise<any[]> {
    ...
    const workbookMatches = responseBody.matchAll(
        /<workbook[^>]*id="([^"]+)"[^>]*name="([^"]+)"[^>]*contentUrl="([^"]*)"[^>]*>/g,
    )
...
export async function queryViews(auth: any): Promise<any[]> {
    ...
    const viewMatches = responseBody.matchAll(
        /<view[^>]*id="([^"]+)"[^>]*name="([^"]+)"[^>]*contentUrl="([^"]*)"[^>]*viewUrlName="([^"]*)"/g,
    )
