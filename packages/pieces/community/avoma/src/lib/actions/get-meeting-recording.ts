    ...
    async run(context) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                throw new Error(
                    `Permission denied: ${response.body.detail || 'You do not have permission to access this recording'}`,
                )
            } else if (response.status === 404) {
                throw new Error(
                    `Recording not found: ${response.body.detail || 'No recording exists for this meeting'}`,
                )
            } else if (response.status === 429) {
                throw new Error(
                    `Rate limit exceeded: ${response.body.detail || 'Too many requests. Please try again later'}`,
                )
