...
export const createVendor = createAction({
  ...
  description: 'Creates a vendor in NetSuite.',
  audience: 'both',
  aiMetadata: {
    description:
      'Creates a vendor record in NetSuite. Provide a company name (for companies) or first/last name with "Is Individual" enabled (for people). Returns the new vendor id. This is a write action and is NOT safe to repeat unless an External ID is set for idempotency.',
    idempotent: false,
  },
  props: {
  ...
  async run(context) {
    ...
    return client.createRecord({ recordType: 'vendor', body });
  },
});
