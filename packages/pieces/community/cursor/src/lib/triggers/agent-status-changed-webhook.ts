...
export const agentStatusChangedWebhookTrigger = createTrigger({
    ...
    description:
        'Triggers when an agent status changes (ERROR or FINISHED). Configure this webhook URL when launching an agent.',
    type: TriggerStrategy.WEBHOOK,
    props: {
        webhookSecret: Property.ShortText({
            displayName: 'Webhook Secret',
            description:
                'Optional webhook secret for signature verification. Must match the secret used when launching the agent.',
            required: false,
        }),
    },
    sampleData: {
    ...
    async run(context) {
        ...
        const webhookSecret = context.propsValue.webhookSecret || (await context.store.get<string>('webhook_secret'))
