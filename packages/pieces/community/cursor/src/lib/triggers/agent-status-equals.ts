...
const polling: Polling<AppConnectionValueForAuthProperty<typeof cursorAuth>, { agentId: string; status: string }> = {
    ...
    items: async ({ auth, propsValue, store }) => {
        ...
        try {
            const agent = await makeCursorRequest<Agent>(auth, `/v0/agents/${agentId}`, HttpMethod.GET)
