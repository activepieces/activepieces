...
export const heartBeatCreateUser = createAction({
  ...
  props: {
    ...
    role_id: Property.Dropdown({
      ...
      refreshers: [],
      options: async ({ auth }) => {
        if (!auth) return { disabled: true, options: [], placeholder: 'Please select a connection' }
        ...
        if (response.status === 200) {
          return {
            options: response.body.map(role => ({ label: role.name, value: role.id })),
            disabled: false
          };
        }

        return { options: [], disabled: true, placeholder: 'Error loading roles.', };
      }
    }),
    group_ids: Property.MultiSelectDropdown({
      ...
      refreshers: [],
      options: async ({ auth }) => {
        if (!auth) return { disabled: true, options: [], placeholder: 'Error loading groups'}
        ...
        if (response.status === 200) {
          return {
            options: response.body.map(group => ({ label: group.name, value: group.id })),
            disabled: false
          };
        }

        return { disabled: true, options: [], placeholder: 'Error loading groups'}
      }
    }),
    profile_picture: Property.ShortText({
      displayName: "Profile Picture",
      description: "A Data URI scheme in the JPG, GIF, or PNG format. Ensure you use the proper content type (image/jpeg, image/png, image/gif) that matches the image data being provided",
      required: false,
    }),
    ...
    create_introduction_thread: Property.Checkbox({
      ...
      description: "If true and a value for bio is provided, an introduction thread for the user will be created in the channel designated for introductions in your community settings.",
      required: false
    })
  },
