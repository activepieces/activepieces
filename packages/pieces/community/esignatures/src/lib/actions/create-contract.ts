...
export const createContract = createAction({
    ...
    props: {
        ...
        title: Property.ShortText({
            ...
            description: 'Unique title for the contract (defaults to template title if not specified)',
            required: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.expiresInHours !== undefined) payload.expires_in_hours = propsValue.expiresInHours.toString()
        if (propsValue.customWebhookUrl) payload.custom_webhook_url = propsValue.customWebhookUrl
        if (propsValue.assignedUserEmail) payload.assigned_user_email = propsValue.assignedUserEmail
        if (propsValue.labels) payload.labels = propsValue.labels
        if (propsValue.test !== undefined) payload.test = propsValue.test ? 'yes' : 'no'
        if (propsValue.saveAsDraft !== undefined) payload.save_as_draft = propsValue.saveAsDraft ? 'yes' : 'no'
