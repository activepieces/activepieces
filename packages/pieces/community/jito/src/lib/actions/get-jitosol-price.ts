...
export const getJitoSolPrice = createAction({
  name: 'get_jitosol_price',
  displayName: 'Get JitoSOL Price',
  description:
    'Fetch the current price and market data for JitoSOL (Jito liquid staking token) from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<Record<string, unknown>>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/jito-staked-sol',
      queryParams: {
        localization: 'false',
        tickers: 'false',
    ...
    return {
      ...
      symbol: coin.symbol,
      name: coin.name,
      coingeckoSlug: 'jito-staked-sol',
      description: 'JitoSOL is a liquid staking token on Solana earning both staking rewards and MEV rewards',
