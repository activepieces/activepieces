...
export const getMePrice = createAction({
  name: 'get_me_price',
  displayName: 'Get ME Token Price',
  description: 'Fetches the current ME governance token price and market data from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<CoinGeckoResponse>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/magic-eden',
      queryParams: {
        localization: 'false',
        tickers: 'false',
