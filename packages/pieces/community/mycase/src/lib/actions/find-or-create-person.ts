...
import { multiCasesDropdown, peopleGroupDropdown } from '../common/props';
...
export const findOrCreatePerson = createAction({
  ...
  async run(context) {
    ...
    const existingPerson = response.find((c: any) => {
      const firstNameMatch =
        c.first_name?.toLowerCase() === propsValue.first_name?.toLowerCase();
      const lastNameMatch =
        c.last_name?.toLowerCase() === propsValue.last_name?.toLowerCase();
    ...
    return await myCaseApiService.createPerson({
