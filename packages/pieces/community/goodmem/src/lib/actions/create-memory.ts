...
import { extractAuthFromContext, getBaseUrl, getCommonHeaders, spaceIdDropdown } from '../common'
...
export const createMemory = createAction({
    ...
    async run(context) {
        ...
        if (file && file.base64) {
            ...
            const detectedMimeType = file.extension ? getMimeType(file.extension) : null
