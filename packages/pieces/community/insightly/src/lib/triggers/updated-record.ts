...
export const updatedRecord = createTrigger({
    ...
    props: {
        object_type: Property.StaticDropdown({
            ...
            description: 'The type of record to monitor for updates.',
            required: true,
            options: {
                options: [
...
6.  Set the rule to trigger **'When a record is created and every time it's updated'** (recommended) or **'Only when a record is updated'**.
...
9.  Click **Save**, then find your new workflow in the list and click **Activate**.
    ...
    sampleData: {
        "entity": {
            ...
            "DATE_UPDATED_UTC": "/Date(1489116999999-0800)/",
            "REMINDER_SENT": false,
            "TASKLINKS": []
        }
    },
