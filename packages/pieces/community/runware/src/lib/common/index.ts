...
import { GenerateImagesParams, GenerateVideoFromTextParams, ImageBackgroundRemovalParams } from './types'
...
export const runwareCommon = {
    ...
    generateImagesFromExistingImageProperties: properties.generateImagesFromExistingImage,
    ...
    generateImagesFromExistingImageSchema: schemas.generateImagesFromExistingImage,
    ...
    generateVideoFromText: async ({ apiKey, ...videoParams }: GenerateVideoFromTextParams) => {
        const client = runwareCommon.runwareClient(apiKey.secret_text)
        return await client.videoInference(videoParams)
    },
    imageBackgroundRemoval: async ({ apiKey, ...imageParams }: ImageBackgroundRemovalParams) => {
        const client = runwareCommon.runwareClient(apiKey.secret_text)
