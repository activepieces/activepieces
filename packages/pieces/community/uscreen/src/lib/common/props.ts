...
export const uscreenProps = {
    ...
    customerId: (required = true) => Property.ShortText({
        displayName: 'Customer ID or Email',
        description: "The unique ID or email address of the customer.",
        required: required,
    }),

    productType: (required = true) => Property.StaticDropdown({
    ...
    productId: (required = true) => Property.Dropdown({
        displayName: 'Product',
        ...
        options: async (context) => {
            ...
            const productType = propsValue['productType'] as 'program' | 'offer' | undefined;
