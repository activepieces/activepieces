...
export const formSubmittedTrigger = createTrigger({
    ...
    props: {
        ...
        formId: tarventCommon.audienceFormId(
            false,
            'If specified, the trigger will only fire if the selected form is submitted.',
        ),
    ...
    async onDisable(context) {
        const webhook = await context.store.get<CreateWebhookResponse>('tarvent_form_submitted')
        if (webhook != null) {
            const client = makeClient(context.auth)
