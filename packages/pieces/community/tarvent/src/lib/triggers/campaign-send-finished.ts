...
export const campaignSendFinishedTrigger = createTrigger({
    ...
    description:
        'Triggers when a campaign has been sent to a contact. WARNING: This will fire for every contact the campaign is sent to, please be careful using this trigger.',
    type: TriggerStrategy.WEBHOOK,
    props: {
        include: tarventCommon.include,
        audienceId: tarventCommon.audienceId(
            false,
            'If specified, the trigger will only fire if contact is in the selected audience.',
        ),
        groupId: tarventCommon.audienceGroupId(
            false,
            'If specified, the trigger will only fire if contact is in the selected group.',
        ),
    ...
    async onDisable(context) {
        const webhook = await context.store.get<CreateWebhookResponse>('tarvent_campaign_send_finished')
        if (webhook != null) {
            const client = makeClient(context.auth)
