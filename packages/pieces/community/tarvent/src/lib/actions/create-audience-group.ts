...
export const createAudienceGroup = createAction({
    ...
    props: {
        ...
        description: tarventCommon.name(
            'Group description',
            false,
            'Use the description to describe what the group is for. NOTE: If the group is public this description will show up in forms that have the groups question.',
        ),
        isPublic: Property.StaticDropdown({
            displayName: 'Public group',
            description:
                'Select whether the group is public or not. Public groups are shown in forms with the groups question.',
            required: true,
            options: {
                options: [
                    {
