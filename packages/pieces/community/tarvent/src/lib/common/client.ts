...
import {
    ...
    CreateSuppressionFilterResponse,
    CreateWebhookResponse,
    DeleteGroupContactResponse,
    ListAudienceDataFieldsResponse,
    ListAudienceFormsResponse,
    ListAudienceGroupAdvResponse,
    ListAudienceGroupResponse,
    ListAudiencesAdvResponse,
    ListAudiencesResponse,
    ListCampaignLinksResponse,
    ListCampaignsAdvResponse,
    ListCampaignsResponse,
    ListContactResponse,
    ListCustomEventsAdvResponse,
    ListCustomEventsResponse,
    ListJourneysAdvResponse,
    ListJourneysResponse,
    ListLandingPagesResponse,
    ListSurveysResponse,
    ListTagsResponse,
    ListTemplatesResponse,
    ListTxGroupNamesResponse,
} from './types'
...
export class TarventClient {
    ...
    async makeRequest<T extends HttpMessageBody>(body: unknown | undefined = undefined): Promise<T> {
    ...
    async createContact(
        ...
        for (const f in profileFields) {
            if (profileFields[f]) {
                profile.push({
                    dataFieldId: f,
                    value: this.isISODate(profileFields[f].toString())
                        ? this.formatISODateToYMD(profileFields[f].toString())
                        : profileFields[f].toString(),
                })
            }
        }
        const body: unknown = {
            ...
            variables: {
                input: {
                    ...
                    audienceGroupManagementType: groupAction === 'Replace' ? 'REPLACE_EXISTING' : 'ADD_NEW_GROUPS_ONLY',
                    tagManagementType: tagAction === 'Replace' ? 'REPLACE_EXISTING' : 'ADD_NEW_TAGS_ONLY',
                    firstName,
                    lastName,
                    streetAddress,
                    streetAddress2,
    ...
    ): Promise<CreateContactNoteResponse> {
        const body: unknown = {
    ...
    async addRemoveContactGroup(
        action: string | undefined,
        contactId: string | undefined,
        audienceId: string | undefined,
        groupId: string | undefined,
    ): Promise<CreateGroupContactResponse | DeleteGroupContactResponse> {
    ...
    async createSuppressionFilter(
        emailAddress: string,
        reason: string | undefined,
    ): Promise<CreateSuppressionFilterResponse> {
        const body: unknown = {
    ...
    async createTransaction(
        ...
        toEmail: string,
        ccEmail: unknown[] | undefined,
        bccEmail: unknown[] | undefined,
        subject: string,
        ...
        switch (type) {
            ...
            case 'contactGroupUpdated':
                return [
                    {
                        entityType: null,
                        eventType: parameters.action === 'Add' ? 'GROUP_ADDED' : 'GROUP_REMOVED',
                    },
                ]
            ...
            case 'contactStatusUpdated': {
                if (parameters.status === 'Any') {
                    ...
                    return [{ entityType: null, eventType: 'SUBSCRIBED' }]
                } else {
    ...
    generateWebhookInput(type: string, parameters: any): any {
        ...
        if (type === 'contactClicked' || type === 'contactOpened') {
            ...
            if (parameters.campaignType === 'Specific') {
                campaignString = `Campaign.${parameters.campaignScope.campaignId}.${
                    type === 'contactClicked' ? 'Clicked' : 'Opened'
                }${
