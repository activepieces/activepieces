...
export const oneDriveCommon = {
    ...
    parentFolderInfo: Property.MarkDown({
        value: `**Note**: If you can't find the folder in the dropdown list (which fetches up to 1000 folders), please click on the **(F)** and type the folder ID directly.\n
    ...
    async getFiles(
        ...
        if (search?.createdTime) {
            ...
            return files.filter((file) => {
                ...
                const comparison =
                    search.createdTimeOp === '<' ? fileDate.isBefore(compareDate) : fileDate.isAfter(compareDate)
                return comparison
            })
        }
