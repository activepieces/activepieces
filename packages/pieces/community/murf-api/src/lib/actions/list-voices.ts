            ...
            options: async ({ auth, locale }) => {
                ...
                const filtered = locale
                    ? voices.filter(
                          (v: any) =>
                              v.locale === locale ||
                              (Array.isArray(v.supportedLocales) && v.supportedLocales.includes(locale)),
                      )
                    : voices
    ...
    async run(context) {
        ...
        if (locale) {
            filtered = filtered.filter(
                (v: any) =>
                    v.locale === locale || (Array.isArray(v.supportedLocales) && v.supportedLocales.includes(locale)),
            )
        }
        if (style) {
            filtered = filtered.filter((v: any) => v.availableStyles?.includes(style))
        }
