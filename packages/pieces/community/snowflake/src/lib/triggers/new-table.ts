...
import { configureConnection, connect, destroy, execute, SnowflakeAuthValue, snowflakeCommonProps } from '../common'
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof snowflakeAuth>, NewTableProps> = {
    ...
    items: async ({ auth, propsValue }) => {
        ...
        try {
            const result = await execute(connection, `SHOW TABLES IN SCHEMA ${database}.${schema}`, [])
...
export const newTableTrigger = createTrigger({
    ...
    description: 'Triggers when a new table is created in the selected schema. Checked by polling every few minutes.',
    props,
    sampleData: {
        table_name: 'ORDERS',
