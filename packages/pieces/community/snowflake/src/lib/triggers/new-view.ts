...
const polling: Polling<AppConnectionValueForAuthProperty<typeof snowflakeAuth>, NewViewProps> = {
    ...
    items: async ({ auth, propsValue }) => {
        ...
        try {
            const result = await execute(connection, `SHOW VIEWS IN SCHEMA ${database}.${schema}`, [])
...
export const newViewTrigger = createTrigger({
    ...
    description: 'Triggers when a new view is created in the selected schema. Checked by polling every few minutes.',
    props,
    sampleData: {
