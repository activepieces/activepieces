...
export const insertMultipleRowsAction = createAction({
    ...
    async run(context) {
        ...
        const rowPlaceholders = parsedRows.map(() => `(${columns.map(() => '?').join(', ')})`).join(', ')

        const binds = parsedRows.flatMap((row) => columns.map((col) => row[col])) as string[]
        ...
        try {
            ...
            return {
                success: true,
                rows_inserted: result?.[0]?.['number of rows inserted'] ?? parsedRows.length,
            }
        } finally {
