...
export const mailChimpNewCampaignTrigger = createTrigger({
  ...
  async onEnable(context): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to enable campaign webhook: ${error.message || JSON.stringify(error)}`
      );
    }
  },
  ...
  async onDisable(context): Promise<void> {
    try {
      ...
      console.error(
        `Failed to disable campaign webhook: ${error.message || JSON.stringify(error)}`
      );
    }
  },
  ...
  async run(context): Promise<unknown[]> {
    try {
      ...
      console.error(
        `Error processing campaign webhook: ${error.message || JSON.stringify(error)}`
      );
      return [];
    }
  },
