...
const WEBHOOK_DATA_STORE_KEY = 'mailchimp_new_order_webhook';
...
export const newOrderTrigger = createTrigger({
    ...
    description: 'Triggers when a new order is created in a connected store.',
    type: TriggerStrategy.WEBHOOK,
    props: {
        store_id: Property.Dropdown({
            displayName: 'Store',
            description: 'The e-commerce store to monitor for new orders.',
            required: true,
            refreshers: [],
            options: async ({ auth }) => {
                if (!auth) {
    ...
    sampleData: {
        "type": "order.create",
        "fired_at": "2025-08-21T09:48:00+00:00",
        "data": {
            "id": "order_54321",
            "store_id": "store_abc",
            "customer": { "id": "cust_12345", "email_address": "new.customer@example.com" },
            "currency_code": "USD",
            "order_total": 99.99,
            "lines": [
                {
