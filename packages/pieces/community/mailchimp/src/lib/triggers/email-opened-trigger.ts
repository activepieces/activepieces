...
export const mailChimpEmailOpenedTrigger = createTrigger({
  ...
  async onEnable(context): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to enable email opened webhook: ${error.message || JSON.stringify(error)}`
      );
    }
  },
  ...
  async onDisable(context): Promise<void> {
    try {
      ...
      console.error(
        `Failed to disable email opened webhook: ${error.message || JSON.stringify(error)}`
      );
    }
  },
  ...
  async run(context): Promise<unknown[]> {
    try {
      ...
      console.error(
        `Error processing email opened webhook: ${error.message || JSON.stringify(error)}`
      );
      return [];
    }
  },
