        ...
        try {
                    ...
                    return {
                        ...
                        primary_subscriber: {
                            ...
                            member_since: subscriber.timestamp_signup
                                ? new Date(subscriber.timestamp_signup).toLocaleDateString()
                                : 'N/A',
                        ...
                        primary_subscriber: {
                            ...
                            full_name:
                                validSubscribers[0].merge_fields?.FNAME && validSubscribers[0].merge_fields?.LNAME
                                    ? `${validSubscribers[0].merge_fields.FNAME} ${validSubscribers[0].merge_fields.LNAME}`
                                    : validSubscribers[0].merge_fields?.FNAME ||
                                      validSubscribers[0].merge_fields?.LNAME ||
                                      'N/A',
                            ...
                            member_since: validSubscribers[0].timestamp_signup
                                ? new Date(validSubscribers[0].timestamp_signup).toLocaleDateString()
                                : 'N/A',
                            last_activity: validSubscribers[0].last_changed
                                ? new Date(validSubscribers[0].last_changed).toLocaleDateString()
                                : 'N/A',
            ...
            if (error.status === 400) {
                return {
                    ...
                    message:
                        'The search request was invalid. This could be due to malformed search parameters or invalid list ID.',
