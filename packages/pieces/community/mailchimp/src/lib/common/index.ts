...
export const mailchimpCommon = {
    mailChimpListIdDropdown: Property.Dropdown<string, true, typeof mailchimpAuth>({
        ...
        options: async ({ auth }) => {
            ...
            const listResponse = (await mailchimpCommon.getUserLists(authProp)) as any
    ...
    mailChimpCampaignIdDropdown: Property.Dropdown<string, true, typeof mailchimpAuth>({
        ...
        options: async ({ auth }) => {
            ...
            const authProp = auth as OAuth2PropertyValue
            const campaignResponse = (await mailchimpCommon.getUserCampaigns(authProp)) as any
    ...
    mailChimpStoreIdDropdown: Property.Dropdown<string, true, typeof mailchimpAuth>({
        ...
        options: async ({ auth }) => {
            ...
            const authProp = auth as OAuth2PropertyValue
            const storeResponse = (await mailchimpCommon.getUserStores(authProp)) as any
    ...
    disableWebhookRequest: async ({ server, token, listId, webhookId }: DisableTriggerRequestParams): Promise<void> => {
