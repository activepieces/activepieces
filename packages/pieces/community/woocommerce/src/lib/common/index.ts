...
export const wooCommon = {
  async createWebhook(name: string, webhookUrl: string, topic: string, auth: PiecePropValueSchema<typeof wooAuth>) {
  ...
  async deleteWebhook(webhookId: number, auth: PiecePropValueSchema<typeof wooAuth>) {
    const trimmedBaseUrl = auth.baseUrl.replace(/\/$/, '')
