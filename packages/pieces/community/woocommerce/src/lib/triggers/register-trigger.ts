...
export const woocommerceRegisterTrigger = ({
    ...
    createTrigger({
        ...
        async onEnable(context) {
            const res = await wooCommon.createWebhook(displayName, context.webhookUrl, topic, context.auth.props)
        ...
        async onDisable(context) {
            ...
            if (webhook != null) {
                await wooCommon.deleteWebhook(webhook.id, context.auth.props)
            }
        },
        ...
        async test(context) {
            ...
            const response = await httpClient.sendRequest<Array<{ id: number }>>(request)
