...
const polling: Polling<AppConnectionValueForAuthProperty<typeof lokaliseAuth>, { projectId: string }> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        ...
        return keys.keys
            .filter((key: { modified_at_timestamp: number }) => key.modified_at_timestamp * 1000 > lastFetchEpochMS)
            .map((key: any) => ({
