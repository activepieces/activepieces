...
export const projectDropdown = Property.Dropdown<string, true, typeof lokaliseAuth>({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, '/projects')
...
export const keyIdProp = Property.Dropdown({
    ...
    options: async ({ auth, projectId }) => {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, `/projects/${projectId}/keys`)
            const keys = response.keys

            return {
                disabled: false,
                options: keys.map((key: any) => ({
                    label: key.key_name.other || key.key_name.ios || key.key_name.android || key.key_name.web,
                    value: key.key_id,
                })),
            }
        } catch (error) {
