...
export const updateTranslation = createAction({
    ...
    props: {
        ...
        translation: Property.LongText({
            ...
            description: 'The actual translation content. Use JSON object format for plural keys',
            required: true,
        }),
        ...
        custom_translation_status_ids: Property.Array({
            ...
            description: 'Comma-separated custom translation status IDs to assign (existing statuses will be replaced)',
            required: false,
        }),
    },
    async run(context) {
        const { projectId, translationId, translation, isUnverified, isReviewed, custom_translation_status_ids } =
            context.propsValue
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.PUT, path, body)
