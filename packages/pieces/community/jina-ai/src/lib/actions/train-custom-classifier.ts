...
export const trainCustomClassifierAction = createAction({
    ...
    props: {
        model: Property.StaticDropdown({
            ...
            options: {
                options: [
                    {
                        label: 'jina-clip-v2 - Multilingual multimodal embeddings for texts and images',
                        value: 'jina-clip-v2',
                    },
                    {
                        label: 'jina-embeddings-v3 - Frontier multilingual embedding model with SOTA performance',
                        value: 'jina-embeddings-v3',
                    },
                    {
                        label: 'jina-clip-v1 - Multimodal embedding models for images and English text',
                        value: 'jina-clip-v1',
                    },
                ],
            },
    ...
    async run(context) {
        ...
        try {
            parsedTrainingData = typeof training_data === 'string' ? JSON.parse(training_data) : (training_data ?? [])
        } catch (error) {
        ...
        const trainingInput = parsedTrainingData.map((example) => {
            ...
            if (!input) {
                throw new Error('Each training example must include an "input" value for either text or image.')
            }

            return type === 'text' ? { label, text: input } : { label, image: input }
        })
