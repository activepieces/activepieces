...
export const deepSearchQueryAction = createAction({
    ...
    description: 'Answer complex questions through iterative search, reading, and reasoning with the DeepSearch API.',
    props: {
    ...
    async run(context) {
        ...
        if (response_format) {
            try {
                const parsedFormat = typeof response_format === 'string' ? JSON.parse(response_format) : response_format
