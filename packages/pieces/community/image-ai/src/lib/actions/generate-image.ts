...
export const generateImage = createAction({
  ...
  async run(context) {
    ...
    if (!image) {
      throw new Error(`Model ${context.propsValue.model} does not support image generation.`)
    }
    ...
    if (response) {
      ...
      throw new Error('Unknown error occurred. Please check image configuration.')
    }
  },
})
