...
export const sendCall = createAction({
    ...
    props: {
        ...
        firstSentence: Property.ShortText({
            ...
            description:
                'Exact opening line the agent will say when the call is answered. Leave blank to let the agent decide.',
            required: false,
        }),
        voice: Property.StaticDropdown({
            displayName: 'Voice',
            description:
                'The voice the AI agent will use. Choose from Bland AI preset voices optimised for phone quality.',
            required: false,
            defaultValue: 'maya',
            options: {
                options: [
