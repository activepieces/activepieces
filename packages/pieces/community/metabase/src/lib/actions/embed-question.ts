...
export const embedQuestion = createAction({
    ...
    description: 'Enable embedding for a Metabase question and configure parameters',
    props: {
        ...
        parameterSettings: Property.DynamicProperties<false, typeof metabaseAuth>({
            ...
            auth: metabaseAuth,
            description: 'Configure how each parameter should be handled in the embed',
            required: false,
            ...
            props: async ({ auth, questionId, enableEmbedding }) => {
                ...
                try {
                    ...
                    parameters.forEach((param) => {
                        ...
                        const currentSetting = currentEmbeddingParams[param.slug] || 'disabled'
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.parameterSettings && parameters.length > 0) {
            ...
            parameters.forEach((param) => {
                const paramSetting = propsValue.parameterSettings?.[param.slug] as string
                if (paramSetting) {
        ...
        return {
            ...
            embeddingParams: propsValue.enableEmbedding ? updatePayload.embedding_params : {},
        }
    },
})
