...
export const getGraphQuestion = createAction({
    ...
    props: {
        ...
        waitTime: Property.Number({
            ...
            description: 'How long to wait for the graph to render completely in seconds',
            required: true,
            defaultValue: 5,
        }),
    },
    async run({ auth, propsValue, files }) {
        if ('embeddingKey' in auth.props && !auth.props.embeddingKey) return 'An embedding key is needed.'

        if (propsValue.waitTime <= 0) return 'The wait time needs to be superior to 0'
        ...
        const graphName = propsValue.graphName ? propsValue.graphName + '.png' : `metabase_question_${questionId}.png`

        const iframeUrl = auth.props.baseUrl + '/embed/question/' + token + '#bordered=true&titled=true'
        ...
        try {
            ...
            if (!response || !response.ok()) {
                throw new Error(`Page load failed with status: ${response ? response.status() : 400}`)
            }
