...
export const companyIdProp = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const accessToken = await getAccessToken(auth?.props.email as string, auth?.props.secret_text as string)
            const response = await makeRequest(accessToken, HttpMethod.GET, '/company_list', { CompanySort: 'asc' })
            return {
                disabled: false,
...
export const contactIdProp = Property.Dropdown({
    ...
    options: async ({ auth, companyId }) => {
        ...
        try {
            const accessToken = await getAccessToken(auth?.props.email as string, auth?.props.secret_text as string)
            const response = await makeRequest(accessToken, HttpMethod.GET, '/contact_list', {
                CompanyId: companyId,
                PageSize: 25,
                page: 1,
                ContactSort: 'asc',
...
export const projectIdProp = Property.Dropdown({
    ...
    options: async ({ auth, companyId }) => {
        ...
        try {
            const accessToken = await getAccessToken(auth?.props.email as string, auth?.props.secret_text as string)
            const response = await makeRequest(accessToken, HttpMethod.GET, '/project_list', {
                CompanyID: companyId,
                PageSize: 25,
                page: 1,
                ProjectSort: 'asc',
