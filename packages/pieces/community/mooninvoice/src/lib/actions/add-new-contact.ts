...
export const addNewContact = createAction({
    ...
    props: {
        ...
        paymentTerms: Property.Number({
            ...
            description: 'Payment terms must be 7, 10, 15, 30, 60, 90, or 180 days only',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (hourlyRate !== undefined && hourlyRate !== null) body.HourlyRate = hourlyRate
        if (currencyCode) body.CurrencyCode = currencyCode
        if (statusCustomer !== undefined) body.StatusCustomer = statusCustomer ? 1 : 0
        if (statusVendor !== undefined) body.StatusVendor = statusVendor ? 1 : 0
        if (paymentReminder !== undefined) body.PaymentReminder = paymentReminder ? 1 : 0
        if (billingAddressStreet1) body.BillingAddressStreet1 = billingAddressStreet1
        if (billingAddressStreet2) body.BillingAddressStreet2 = billingAddressStreet2
        ...
        if (billingAddressCountry) body.BillingAddressCountry = billingAddressCountry
        if (billingAddressZip) body.BillingAddressZip = billingAddressZip
        if (shippingAddressStreet1) body.ShippingAddressStreet1 = shippingAddressStreet1
        if (shippingAddressStreet2) body.ShippingAddressStreet2 = shippingAddressStreet2
        ...
        if (shippingAddressCountry) body.ShippingAddressCountry = shippingAddressCountry
        if (shippingAddressZip) body.ShippingAddressZip = shippingAddressZip
        if (paymentTerms !== undefined && paymentTerms !== null) body.PaymentTerms = paymentTerms
        if (openingBalance !== undefined && openingBalance !== null) body.OpeningBalance = openingBalance
        if (openingBalanceDate) body.OpeningBalanceDate = openingBalanceDate
        if (bankDetails) body.BankDetails = bankDetails

        const accessToken = await getAccessToken(context.auth.props.email, context.auth.props.secret_text)
        const response = await makeRequest(accessToken, HttpMethod.POST, '/add_contact', body)
