...
export const createCreditNote = createAction({
    ...
    async run(context) {
        ...
        if (roundedAmount !== undefined && roundedAmount !== null) creditNotesFolder.rounded_amount = roundedAmount
        if (percentage !== undefined && percentage !== null) creditNotesFolder.Percentage = percentage
        ...
        const response = await makeRequest(accessToken, HttpMethod.POST, '/create_credit_notes', body)
