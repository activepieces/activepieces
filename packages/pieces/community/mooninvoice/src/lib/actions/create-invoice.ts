...
export const createInvoice = createAction({
    ...
    async run(context) {
        ...
        if (shippingCost !== undefined && shippingCost !== null) body.ShippingCost = shippingCost
        if (shippingMethod) body.ShippingMethod = shippingMethod
        if (poNo) body.PoNo = poNo
        if (discountAmount !== undefined && discountAmount !== null) body.DiscountAmount = discountAmount
        if (discountBeforeTax !== undefined) body.DiscountBeforeTax = discountBeforeTax
        if (depositeAmount !== undefined && depositeAmount !== null) body.DepositeAmount = depositeAmount
        if (amountDue !== undefined && amountDue !== null) body.AmountDue = amountDue
        if (amountPaid !== undefined && amountPaid !== null) body.AmountPaid = amountPaid
        if (totalAmount !== undefined && totalAmount !== null) body.TotalAmount = totalAmount
        ...
        if (recurringIntervalInString) body.RecurringIntervalInString = recurringIntervalInString

        // Build billing address
        if (billingStreet1 || billingStreet2 || billingCity || billingState || billingCountry || billingZip) {
            body.Billing_Add = {
        ...
        if (shippingStreet1 || shippingStreet2 || shippingCity || shippingState || shippingCountry || shippingZip) {
            body.Secondary_Add = {
        ...
        const response = await makeRequest(accessToken, HttpMethod.POST, '/create_invoice', body)
