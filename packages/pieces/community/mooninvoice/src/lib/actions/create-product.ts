...
export const createProduct = createAction({
    ...
    props: {
        ...
        productBuyTaxJson: Property.LongText({
            ...
            description: 'Array of tax IDs applied on product buy. Example: ["95A1956E-E60A-4FAC-8EC4-C88EE7431280"]',
            required: false,
        }),
        productSalesTaxJson: Property.LongText({
            displayName: 'Product Sales Tax (JSON)',
            description: 'Array of tax IDs applied on product sales. Example: ["95A1956E-E60A-4FAC-8EC4-C88EE7431280"]',
            required: false,
        }),
    },
    async run(context) {
        ...
        if (productBuyCost !== undefined && productBuyCost !== null) body.ProductBuyCost = productBuyCost
        if (productUnitCost !== undefined && productUnitCost !== null) body.ProductUnitCost = productUnitCost
        if (productType) body.ProductType = productType
        if (productQuantity !== undefined && productQuantity !== null) body.ProductQuantity = productQuantity
        if (productNotes) body.ProductNotes = productNotes
        if (productCategory) body.ProductCategory = productCategory
        if (productSelectedCurrency) body.ProductSelectedCurrency = productSelectedCurrency
        if (hsnCode) body.HSNCode = hsnCode
        if (productStockManage !== undefined) body.ProductStockManage = productStockManage ? 1 : 0
        if (showInMenu !== undefined) body.ShowInMenu = showInMenu ? 1 : 0
        if (productSerialized !== undefined) body.ProductSerialized = productSerialized ? 1 : 0
        if (productBatched !== undefined) body.ProductBatched = productBatched ? 1 : 0
        ...
        const response = await makeRequest(accessToken, HttpMethod.POST, '/add_product', body)
