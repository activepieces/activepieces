...
export const createEstimate = createAction({
    ...
    async run(context) {
        ...
        if (roundedAmount !== undefined && roundedAmount !== null) estimateFolder.rounded_amount = roundedAmount
        if (percentage !== undefined && percentage !== null) estimateFolder.Percentage = percentage
        ...
        const response = await makeRequest(accessToken, HttpMethod.POST, '/create_estimate', body)
