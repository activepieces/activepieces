    ...
    `)
        ...
        for (const dbTable of dbTables) {
            ...
            if (dbTable.schema) {
                ...
                const schemaResult = await connection.run('SELECT json_structure($sourceData) AS schema', {
                    sourceData: dbData,
                })
        ...
        const reader = await connection.runAndReadAll(context.propsValue.query, formattedArgs)
