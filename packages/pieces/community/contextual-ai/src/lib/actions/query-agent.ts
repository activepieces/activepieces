...
export const queryAgentAction = createAction({
    ...
    props: {
        ...
        conversationId: Property.ShortText({
            ...
            description:
                'Optional conversation ID to continue an existing conversation (leave empty for new conversation)',
            required: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        const messages: Array<{ role: 'user' | 'system' | 'assistant' | 'knowledge'; content: string }> = conversationId
            ? []
