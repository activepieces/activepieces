...
export const getTokenPrice = createAction({
  ...
  displayName: 'Get CYPH Token Price',
  description:
    'Fetches the current CYPH governance token price and market data from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<CoinGeckoResponse>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/cypher-protocol',
      queryParams: {
        localization: 'false',
        tickers: 'false',
