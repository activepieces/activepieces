...
export const decodeEmbeddings = (embeddingsString: Buffer) => {
  ...
  if (typeof embeddings[0] === 'string') {
    return (embeddings as string[]).map(
      (embedding) => new Float32Array(new Uint8Array(Buffer.from(embedding, 'base64')).buffer),
    )
  } else {
...
export const filteringProps = {
  must: Property.Object({
    ...
    description: 'If the point have this property in his payload it will be selected',
    required: false,
  }),
  must_not: Property.Object({
    displayName: 'Must Not Have',
    description: 'If the point have this property in his payload it will not be selected',
    required: false,
  }),
}
...
export const convertToFilter = (infosToGetPoint: {
  ...
  type Tfilter = ({ has_id: (string | number)[] } | { key: string; match: { value: any } | { any: any[] } })[]
