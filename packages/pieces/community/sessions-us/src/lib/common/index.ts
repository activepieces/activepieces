...
export function createSessionsUsWebhookTrigger(data: CreateWebhookTriggerDto) {
    return createTrigger({
        ...
        async onEnable({ auth, store, webhookUrl, propsValue }) {
            const webhookId = await createWebhook(data.trigger, auth.secret_text, webhookUrl, propsValue.permission)
