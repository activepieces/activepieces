...
export const updateCrmContactAction = createAction({
    ...
    props: {
        ...
        tags: Property.ShortText({
            displayName: 'Tags',
            description:
                'Replaces all existing tags. Comma-separated (e.g. vip, lead). Leave empty to keep current tags unchanged.',
            required: false,
        }),
    },
    async run(context) {
        ...
        const response = await whatsscaleClient(auth, HttpMethod.PATCH, `/api/crm/contacts/${contactId}`, body)
