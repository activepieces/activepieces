...
export const sendTextManualAction = createAction({
    ...
    async run(context) {
        ...
        const body = buildRecipientBody(RecipientType.MANUAL, session, recipient, chatType as ChatType)
        const response = await whatsscaleClient(auth, HttpMethod.POST, '/api/sendText', { ...body, text })
