...
describe('sendVideoToCrmContactAction', () => {
  ...
  it('calls prepareFile with the video URL', async () => {
    await (sendVideoToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('calls POST /api/sendVideo with CRM body shape (no chatId)', async () => {
    await (sendVideoToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, videoUrl: 'https://example.com/video.mp4', caption: 'Hello' },
    });
