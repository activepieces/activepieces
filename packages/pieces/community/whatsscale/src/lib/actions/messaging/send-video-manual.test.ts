...
describe('sendVideoManualAction', () => {
  ...
  it('calls prepareFile with the video URL', async () => {
    await (sendVideoManualAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', chatType: 'contact', recipient: '31649931832@c.us', videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('calls POST /api/sendVideo with correct body for contact', async () => {
    await (sendVideoManualAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', chatType: 'contact', recipient: '31649931832@c.us', videoUrl: 'https://example.com/video.mp4', caption: 'Hello' },
    });
  ...
  it('uses chatType from propsValue when building recipient body', async () => {
    await (sendVideoManualAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', chatType: 'group', recipient: '120363000000000001@g.us', videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('includes file field from preparedUrl (not raw videoUrl)', async () => {
    await (sendVideoManualAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', chatType: 'contact', recipient: '31649931832@c.us', videoUrl: 'https://example.com/raw-video.mp4', caption: undefined },
    });
