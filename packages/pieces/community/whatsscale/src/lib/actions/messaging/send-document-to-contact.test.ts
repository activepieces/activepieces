...
describe('sendDocumentToContactAction', () => {
  ...
  it('calls prepareFile with the document URL', async () => {
    await (sendDocumentToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', documentUrl: 'https://example.com/doc.pdf', filename: undefined, caption: undefined },
    });
  ...
  it('calls POST /api/sendDocument with correct body', async () => {
    await (sendDocumentToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', documentUrl: 'https://example.com/doc.pdf', filename: 'report.pdf', caption: 'Q3 Report' },
    });
  ...
  it('omits filename field when filename is empty string', async () => {
    await (sendDocumentToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', documentUrl: 'https://example.com/doc.pdf', filename: '', caption: undefined },
    });
  ...
  it('includes filename in body when provided', async () => {
    await (sendDocumentToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', documentUrl: 'https://example.com/doc.pdf', filename: 'invoice.pdf', caption: undefined },
    });
