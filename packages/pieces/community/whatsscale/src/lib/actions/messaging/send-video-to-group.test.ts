...
describe('sendVideoToGroupAction', () => {
  ...
  it('calls prepareFile with the video URL', async () => {
    await (sendVideoToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('calls POST /api/sendVideo with group chatId', async () => {
    await (sendVideoToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', videoUrl: 'https://example.com/video.mp4', caption: 'Watch this' },
    });
