...
describe('sendDocumentToCrmContactAction', () => {
  ...
  it('calls prepareFile with the document URL', async () => {
    await (sendDocumentToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, documentUrl: 'https://example.com/doc.pdf', filename: undefined, caption: undefined },
    });
  ...
  it('calls POST /api/sendDocument with CRM body shape (no chatId)', async () => {
    await (sendDocumentToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, documentUrl: 'https://example.com/doc.pdf', filename: 'report.pdf', caption: 'Q3' },
    });
  ...
  it('omits filename field when filename is empty string', async () => {
    await (sendDocumentToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, documentUrl: 'https://example.com/doc.pdf', filename: '', caption: undefined },
    });
  ...
  it('includes filename when provided', async () => {
    await (sendDocumentToCrmContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', crmContact: crmContactId, documentUrl: 'https://example.com/doc.pdf', filename: 'invoice.pdf', caption: undefined },
    });
