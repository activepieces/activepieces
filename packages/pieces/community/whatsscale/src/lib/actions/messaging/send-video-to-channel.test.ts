...
describe('sendVideoToChannelAction', () => {
  ...
  it('calls prepareFile with the video URL', async () => {
    await (sendVideoToChannelAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', channel: '120363000000000001@newsletter', videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('calls POST /api/sendVideo with channel chatId', async () => {
    await (sendVideoToChannelAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', channel: '120363000000000001@newsletter', videoUrl: 'https://example.com/video.mp4', caption: 'New update' },
    });
