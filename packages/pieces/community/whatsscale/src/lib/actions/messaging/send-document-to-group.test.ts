...
describe('sendDocumentToGroupAction', () => {
  ...
  it('calls prepareFile with the document URL', async () => {
    await (sendDocumentToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', documentUrl: 'https://example.com/doc.pdf', filename: undefined, caption: undefined },
    });
  ...
  it('calls POST /api/sendDocument with group chatId', async () => {
    await (sendDocumentToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', documentUrl: 'https://example.com/doc.pdf', filename: 'notes.pdf', caption: 'Meeting notes' },
    });
  ...
  it('omits filename field when filename is empty string', async () => {
    await (sendDocumentToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', documentUrl: 'https://example.com/doc.pdf', filename: '', caption: undefined },
    });
  ...
  it('includes filename when provided', async () => {
    await (sendDocumentToGroupAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', group: '120363000000000001@g.us', documentUrl: 'https://example.com/doc.pdf', filename: 'agenda.pdf', caption: undefined },
    });
