...
describe('sendVideoToContactAction', () => {
  ...
  it('calls prepareFile with the video URL', async () => {
    await (sendVideoToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', videoUrl: 'https://example.com/video.mp4', caption: undefined },
    });
  ...
  it('calls POST /api/sendVideo with correct body', async () => {
    await (sendVideoToContactAction as any).run({
      auth: mockAuth,
      propsValue: { session: 'test-session', contact: '31649931832@c.us', videoUrl: 'https://example.com/video.mp4', caption: 'Hello' },
    });
