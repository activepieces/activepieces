...
import { listCrmContactsOutputSchema } from '../../../output-schemas';

type ConductorCrmContactList = {
  items: ConductorCrmContact[];
  total: number;
  page: number;
  limit: number;
  hasMore: boolean;
...
export const listCrmContactsAction = createAction({
  ...
  outputSchema: listCrmContactsOutputSchema,
  props: {
  ...
  async run(context) {
    ...
    const list = response.body as ConductorCrmContactList;

    return {
      items: list.items.map(flattenCrmContact),
      total: list.total,
      page: list.page,
      limit: list.limit,
