...
import { watchChannelMessagesTrigger } from './watch-channel-messages';
...
describe('watchChannelMessagesTrigger', () => {
  describe('onEnable', () => {
    it('calls /make/hooks/subscribe with trigger_type channel and platform activepieces', async () => {
      const ctx = makeContext({ session: 'my_session' });
      await watchChannelMessagesTrigger.onEnable(ctx);
  ...
  describe('onDisable', () => {
    it('calls /make/hooks/unsubscribe with webhook_url only', async () => {
      const ctx = makeContext({ session: 'my_session' });
      await watchChannelMessagesTrigger.onDisable(ctx);
      expect(mockClient).toHaveBeenCalledWith(
        MOCK_AUTH_STRING,
        HttpMethod.POST,
  ...
  describe('run', () => {
    it('returns payload.body wrapped in an array', async () => {
      const body = { channel_id: '120363401850139775@newsletter', body: 'Announcement' };
      const ctx = makeContext({ session: 'my_session' });
      ctx.payload = { body };
      const result = await watchChannelMessagesTrigger.run(ctx);
