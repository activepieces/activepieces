...
export const pushRowsToDatasetTableAction = createAction({
    ...
    props: {
        ...
        skip_refresh: Property.Checkbox({
            ...
            description:
                'Skip refreshing the dataset after pushing data (only applies to Push and PushStreaming modes)',
            required: false,
            defaultValue: false,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            if (!targetTable) {
                throw new Error(
                    `Table '${tableName}' not found in dataset. Available tables: ${tables.map((t: any) => t.name).join(', ')}`,
                )
            }
            ...
            if (!skipRefresh) {
                refreshResponse = await httpClient
                    .sendRequest({
                        method: HttpMethod.POST,
