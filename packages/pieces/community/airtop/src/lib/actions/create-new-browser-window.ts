...
export const createNewBrowserWindowAction = createAction({
    ...
    props: {
        ...
        customResolution: Property.ShortText({
            ...
            description:
                'Custom resolution in format "widthxheight" (e.g., "1440x900"). Leave blank to use selected resolution above.',
            required: false,
        }),
    ...
    async run(context) {
        const { sessionId, url, screenResolution, customResolution, waitUntil, waitUntilTimeoutSeconds } =
            context.propsValue
        ...
        await propsValidation.validateZod(context.propsValue, {
            ...
            customResolution: z
                .string()
                .regex(/^\d+x\d+$/, 'Must be in format "widthxheight" (e.g., "1920x1080")')
                .optional(),
