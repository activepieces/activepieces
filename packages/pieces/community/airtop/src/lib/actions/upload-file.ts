...
import { AirtopSession, airtopApiCall, extractApiData } from '../common/client'
...
export const uploadFileToSessionAction = createAction({
    ...
    props: {
        ...
        sessionIds: Property.MultiSelectDropdown({
            ...
            description:
                'Select one or more sessions to make the file available on. Leave empty to make available to all sessions.',
            required: false,
            refreshers: [],
            options: async ({ auth }) => {
                if (!auth) {
                ...
                try {
                    ...
                    return {
                        ...
                        options,
                        placeholder:
                            options.length === 0 ? 'No sessions available' : 'Select sessions or leave empty for all',
                    }
                } catch (error: any) {
                    return {
                        disabled: true,
    ...
    async run({ propsValue, auth }) {
        ...
        if (sessionIds && sessionIds.length > 0) {
            await propsValidation.validateZod(
                ...
                {
                    sessionIds: z
                        .array(z.string().min(1, 'Session ID cannot be empty'))
                        .min(1, 'At least one session ID is required when providing session IDs'),
                },
            )
        }
