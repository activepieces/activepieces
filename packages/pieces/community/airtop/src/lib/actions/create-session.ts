...
export const createSessionAction = createAction({
                    ...
                    if (proxyType === 'country_sticky') {
                        return {
                            proxyCountry: Property.ShortText({
                                ...
                                description:
                                    'ISO 3166-1 alpha-2 format (e.g., "US", "GB"). Use "global" for random countries.',
                                required: false,
                                defaultValue: 'US',
                            }),
                            ...
                            proxyList: Property.Array({
                                ...
                                properties: {
                                    domainPattern: Property.ShortText({
                                        ...
                                        description:
                                            'Domain pattern (e.g., "*.example.com"). Use ? for single character, * for multiple.',
                                        required: true,
                                    }),
    ...
    async run(context) {
        const { profileName, extensionIds, useAirtopProxy, solveCaptcha, timeoutMinutes, proxyConfig, proxyAdvanced } =
