...
import { getGraphBaseUrl, getMicrosoftCloudFromAuth, microsoftCloudProperty } from './microsoft-cloud'
import {
...
export const microsoft365PlannerCommon = {
    ...
    getEtag: async ({
        ...
        id: string
        endpointType: 'planDetail' | 'listBuckets' | 'listPlanTasks' | 'bucketDetail' | 'deleteTask' | 'deleteTask'
    }) => {
        const client = microsoft365PlannerCommon.getClient({ auth })
        const endpointFn = microsoft365PlannerCommon.endpoints[endpointType] as (arg: string) => string
    ...
    listGroups: async ({ auth }: AuthenticationParams) => {
        const client = microsoft365PlannerCommon.getClient({ auth })
        const response: ListGroupsResponse = await client.api(microsoft365PlannerCommon.endpoints.listGroups).get()
        return response.value
    },
    listPlans: async ({ auth }: AuthenticationParams) => {
        const client = microsoft365PlannerCommon.getClient({ auth })
        const response: ListPlansResponse = await client.api(microsoft365PlannerCommon.endpoints.listPlans).get()
        return response.value
    },
    ...
    deleteTask: async ({ auth, id }: DeleteTaskParams) => {
        ...
        await client.api(microsoft365PlannerCommon.endpoints.deleteTask(id)).header('If-Match', etag).delete()
        return { success: true }
    },
    ...
    updateTask: async ({ auth, id, ...taskParams }: UpdateTaskParams) => {
        ...
        await client.api(microsoft365PlannerCommon.endpoints.deleteTask(id)).header('If-Match', etag).update(taskParams)
        return { success: true }
    },
    deleteBucket: async ({ auth, id }: DeleteBucketParams) => {
        ...
        await client.api(microsoft365PlannerCommon.endpoints.bucketDetail(id)).header('If-Match', etag).delete()
        return { success: true }
    },
    ...
    updatePlan: async ({ auth, id, title }: UpdatePlanParams) => {
        ...
        await client.api(microsoft365PlannerCommon.endpoints.planDetail(id)).header('If-Match', etag).update({ title })
        return { success: true }
    },
