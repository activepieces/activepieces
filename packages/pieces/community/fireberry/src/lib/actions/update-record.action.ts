...
const updateFields = Property.DynamicProperties({
    ...
    props: async ({ auth, objectType }) => {
        ...
        try {
            ...
            const picklistFields = metadata.data.filter((field) => fieldTypeMap[field.systemFieldTypeId] === 'picklist')
            ...
            for (const field of metadata.data) {
                ...
                switch (fieldType) {
                        ...
                        if (largeLists.includes(field.fieldName.toLowerCase())) {
                            ...
                            if (values.length > 0 && values.length <= 20) {
                                ...
                                props[field.fieldName] = Property.StaticDropdown({
                                    ...
                                    options: {
                                        disabled: false,
                                        options: [{ label: '-- Keep Current Value --', value: '' }, ...options],
                                    },
                                })
                            } else {
                                props[field.fieldName] = Property.ShortText({
                    ...
                    case 'lookup': {
                        ...
                        if (field.fieldName.includes('account'))
                            description = 'Account record ID (leave empty to keep current)'
                        else if (field.fieldName.includes('contact'))
                            description = 'Contact record ID (leave empty to keep current)'
                        else if (field.fieldName.includes('owner'))
                            description = 'User record ID for owner (leave empty to keep current)'
                        else if (field.fieldName.includes('product'))
                            description = 'Product record ID (leave empty to keep current)'
                        else if (field.fieldName.includes('user'))
                            description = 'User record ID (leave empty to keep current)'
