    ...
    async run({ propsValue, auth }) {
        ...
        if (contactData.phone && contactData.phone.trim()) {
            ...
            if (!e164Regex.test(contactData.phone)) {
                throw new Error(
                    'Invalid phone format. Please provide phone number in E.164 format (e.g., +60123456789).',
                )
            }
        }
        ...
        if (contactData.language && contactData.language.trim()) {
            ...
            if (!iso639Regex.test(contactData.language)) {
                throw new Error(
                    'Invalid language code. Please provide a valid ISO 639-1 two-letter language code (e.g., "en", "es").',
                )
            }
        }
        ...
        if (contactData.countryCode && contactData.countryCode.trim()) {
            ...
            if (!iso3166Regex.test(contactData.countryCode)) {
                throw new Error(
                    'Invalid country code. Please provide a valid ISO 3166-1 alpha-2 two-letter country code (e.g., "US", "MY").',
                )
            }
        }
