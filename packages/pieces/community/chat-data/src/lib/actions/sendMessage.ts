...
export const sendMessage = createAction({
    ...
    props: {
        ...
        conversationId: Property.Dropdown({
            ...
            description: 'Select an existing conversation or leave empty to start a new one',
            required: false,
            ...
            options: async ({ auth, chatbotId }) => {
                ...
                try {
                    const client = new ChatDataClient(auth.secret_text)
                    const conversations = await client.listConversations(chatbotId as string)
                    return {
                        options: [
                            { label: 'New Conversation', value: '' },
                            ...conversations.map((conversation) => ({
                                label: conversation.title || `Conversation ${conversation.id.slice(-8)}`,
        ...
        appendMessages: Property.Checkbox({
            ...
            description: 'Append this message to previous messages with the same conversationId',
            required: false,
        }),
        attachedFiles: Property.Array({
            ...
            properties: {
                ...
                content: Property.LongText({
                    ...
                    description:
                        "Pre-parsed textual content. If provided, file won't be downloaded. Required for files > 3MB.",
                    required: false,
                }),
            },
        }),
