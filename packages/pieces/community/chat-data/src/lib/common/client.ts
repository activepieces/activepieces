...
export class ChatDataClient {
    ...
    constructor(apiKey?: string) {
        ...
        this.baseURL = process.env['CHATDATA_BASE_URL'] || 'https://api.chat-data.com'
    }
    ...
    async updateChatbotSettings(payload: UpdateChatbotSettingsDto): Promise<{ status: string }> {
        try {
            const response = await httpClient.sendRequest({
                method: HttpMethod.POST,
    ...
    async listConversations(chatbotId: string): Promise<Array<{ id: string; title: string; lastMessage?: string }>> {
        try {
            const response = await httpClient.sendRequest({
                method: HttpMethod.GET,
