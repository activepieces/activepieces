...
const getLatestCursor = async (accessToken: string, path: string, recursive: boolean): Promise<string> => {
...
export const dropboxNewFolder = createTrigger({
    ...
    run: async (context) => {
        ...
        const { folders, newCursor } = await getChangedFolders(context.auth.access_token, cursor)
