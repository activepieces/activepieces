...
export const createConversationAction = createAction({
    ...
    props: {
        ...
        document_ids: Property.Array({
            ...
            description: "A list of document IDs to limit the bot's knowledge base for this conversation.",
            required: false,
        }),
    },
    async run(context) {
        ...
        // The document_ids prop gives us an array of strings, which is what the API expects.
        const docIds = document_ids as string[] | undefined;

        return await codyClient.createConversation(
            apiKey,
            bot_id,
            name,
            docIds
