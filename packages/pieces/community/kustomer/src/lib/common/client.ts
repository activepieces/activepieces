...
import { KustomerJsonObject, KustomerJsonValue, KustomerQueryParams } from './types'
...
async function getCustomer({ apiKey, customerId }: { apiKey: string; customerId: string }): Promise<KustomerJsonValue> {
    return sendRequest({
        apiKey,
        method: HttpMethod.GET,
...
function createAuthHeaders({ apiKey }: { apiKey: string }): Record<string, string> {
    return {
