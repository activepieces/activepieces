...
export const getProtocolStats = createAction({
  ...
  description: 'Get a combined summary of Filecoin chain and token statistics',
  props: {},
  async run() {
    const [chainsRes, priceRes] = await Promise.all([
      fetch('https://api.llama.fi/v2/chains'),
      fetch('https://api.coingecko.com/api/v3/simple/price?ids=filecoin&vs_currencies=usd&include_24hr_change=true&include_market_cap=true&include_24hr_vol=true'),
    ]);
    ...
    const d = prices['filecoin'];
    return {
      chain: {
        name: 'Filecoin',
        tvl: filecoin.tvl ?? 0,
        tokenSymbol: filecoin.tokenSymbol ?? 'FIL',
      },
      token: {
        price_usd: d['usd'],
