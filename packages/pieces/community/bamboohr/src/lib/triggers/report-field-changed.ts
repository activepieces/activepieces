...
export const reportFieldChanged = createTrigger({
    ...
    props: {
        ...
        fieldToMonitor: Property.Dropdown({
            ...
            options: async ({ auth, reportId }) => {
                ...
                const response = await getReportById(companyDomain, reportId as string, apiKey)
    ...
    async run(context) {
        ...
        const lastKnownState = (await context.store.get<Record<string, string[]>>('lastReportState')) || {}
