...
async function getReportById(companyDomain: string, reportId: string, apiKey: string) {
	...
	items: async ({ auth, propsValue, store }) => {
		...
		try {
			...
			if (!availableFields.has(propsValue.fieldToMonitor)) {
				console.warn(
					`Field "${propsValue.fieldToMonitor}" not found in report. Available fields: ${Array.from(
						availableFields,
					).join(', ')}`,
				);
				return [];
			...
			reportData.forEach((employee: Record<string, any>) => {
				const employeeId = String(employee['id'] || employee['employeeId'] || 'unknown');
...
export const reportFieldChangedCopy = createTrigger({
	auth: bambooHrAuth,
	name: 'reportFieldChangedCopy',
	displayName: 'Report Field Changed Copy',
	...
	async test(context) {
		...
		const items = employees.map((employee) => {
			const employeeId = String(employee['id'] || employee['employeeId'] || 'unknown');
			const currentFieldValue = employee[fieldToMonitor];
			return {
				employeeId,
				employeeName:
					employee['displayName'] ||
	...
	async run(context) {
		...
		const lastKnownState = (await context.store.get<Record<string, any>>('lastReportState')) || {};
