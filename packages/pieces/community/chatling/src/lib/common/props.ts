...
export const chatbotIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const options: DropdownOption<string>[] = response.data.chatbots.map((chatbot) => ({
            label: chatbot.name,
            value: chatbot.id,
        }))
...
export const aiModelIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, chatbotId }) => {
        ...
        }>(auth.secret_text, HttpMethod.GET, `/chatbots/${chatbotId}/ai/kb/models`)

        const options: DropdownOption<number>[] = response.data.models.map((model) => ({
            label: model.name,
            value: model.id,
        }))
...
export const languageIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, chatbotId }) => {
        ...
        }>(auth.secret_text, HttpMethod.GET, `/chatbots/${chatbotId}/ai/kb/languages`)

        const options: DropdownOption<number>[] = response.data.languages.map((language) => ({
            label: language.name,
            value: language.id,
        }))
...
export const contactIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, chatbotId }) => {
        ...
        }>(auth.secret_text, HttpMethod.GET, `/chatbots/${chatbotId}/contacts`)

        const options: DropdownOption<string>[] = response.data.contacts.map((contact) => ({
...
export const templateIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const options: DropdownOption<number>[] = response.data.templates.map((template) => ({
            label: template.name,
            value: template.id,
        }))
...
export const conversationIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth, chatbotId }) => {
        ...
        }>(auth.secret_text, HttpMethod.GET, `/chatbots/${chatbotId}/conversations`)

        const options: DropdownOption<string>[] = response.data.conversations.map((conv) => {
