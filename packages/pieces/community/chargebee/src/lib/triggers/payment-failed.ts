...
import { fetchRecentEvents, subscribeChargebeeWebhook, unsubscribeChargebeeWebhook } from '../common/webhook'
...
export const paymentFailed = createTrigger({
    ...
    description: 'Triggers when a payment attempt fails. Useful for dunning and retry workflows.',
    props: {},
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await unsubscribeChargebeeWebhook(context.auth.props.site, context.auth.props.api_key, webhookId)
        }
    },
    async test(context) {
        return fetchRecentEvents(context.auth.props.site, context.auth.props.api_key, EVENT_TYPE)
    },
    async run(context) {
