...
export function cleanObject(value: Record<string, unknown>): Record<string, unknown> {
...
export async function chargebeeRequest<TResponse>({
    ...
    try {
        ...
        const serializedBody = body && isFormEncoded ? toFormEncoded(body) : body
        ...
        const message =
            chargebeeError.response?.body?.message ?? chargebeeError.message ?? 'Chargebee API request failed.'
