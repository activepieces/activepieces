...
export const customerIo: any = createPiece({
    ...
    actions: [
        ...
        createCustomApiCallAction({
            baseUrl: (auth) => (auth ? customerIOCommon[auth.props.region || 'us'].trackUrl : ''),
            auth: customerIOAuth,
            ...
            authMapping: async (auth) => ({
                Authorization: `Basic ${Buffer.from(
                    `${(auth).props.track_site_id}:${(auth).props.track_api_key}`,
                    'utf8',
                ).toString('base64')}`,
            }),
        }),
        createCustomApiCallAction({
            baseUrl: (auth) => (auth ? customerIOCommon[auth.props.region || 'us'].apiUrl : ''),
            auth: customerIOAuth,
