...
export const deleteVector = createAction({
    ...
    props: {
        ...
        namespace: Property.ShortText({
            ...
            description: 'The namespace to delete vectors from (e.g., "example-namespace")',
            required: false,
        ...
        id: Property.ShortText({
            ...
            description: 'The ID of the vector to delete (for single vector deletion)',
            required: false,
        }),
        ids: Property.Array({
        ...
        confirmDeleteAll: Property.Checkbox({
            ...
            description: 'Check this box to confirm you want to delete ALL vectors in the namespace',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        const { indexName, indexHost, namespace, deleteMode, id, ids, confirmDeleteAll, filter } = context.propsValue
        ...
        try {
            ...
            if (deleteMode === 'one') {
                if (!id) {
                    throw new Error('You must provide a vector ID for single vector deletion.')
                }
                ...
                if (!ids || !Array.isArray(ids) || ids.length === 0) {
                    throw new Error('You must provide an array of vector IDs for multiple vector deletion.')
                }

                await ns.deleteMany(ids as string[])
                deletedCount = ids.length
                operation = `Deleted ${ids.length} vectors with IDs: ${(ids as string[]).join(', ')}`
                ...
                operation = `Deleted ALL vectors in namespace: ${namespace || 'default'}`
            } else if (deleteMode === 'filter') {
                if (!filter) {
                    throw new Error('You must provide a metadata filter for filter-based deletion.')
                }
            ...
            return {
                ...
                error: 'Unknown Error',
                message:
                    caught instanceof Error ? caught.message : 'An unexpected error occurred while deleting vectors.',
                indexName: indexName,
                namespace: namespace || 'default',
            }
        }
