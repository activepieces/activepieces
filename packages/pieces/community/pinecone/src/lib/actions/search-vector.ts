...
export const searchVector = createAction({
    ...
    props: {
        ...
        topK: Property.Number({
            displayName: 'Top K',
            description: 'The number of results to return for each query (range: 1-10000)',
            required: true,
            defaultValue: 10,
        }),
        ...
        id: Property.ShortText({
            ...
            description: 'The unique ID of the vector to be used as a query vector (max length: 512)',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (queryMethod === 'vector') {
            if (!vector || !Array.isArray(vector) || vector.length === 0) {
                throw new Error('You must provide a query vector when using vector query method.')
            }
        } else if (queryMethod === 'id') {
            if (!id || typeof id !== 'string' || id.length === 0 || id.length > 512) {
                throw new Error('You must provide a valid vector ID (1-512 characters) when using ID query method.')
            }
        }
        ...
        try {
            ...
            return {
                ...
                error: 'Unknown Error',
                message:
                    caught instanceof Error ? caught.message : 'An unexpected error occurred while searching vectors.',
                query: queryRequest,
                indexName: indexName,
                namespace: namespace || 'default',
            }
