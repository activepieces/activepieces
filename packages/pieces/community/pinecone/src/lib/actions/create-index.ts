...
export const createIndex = createAction({
    ...
    props: {
        name: Property.ShortText({
            ...
            description: 'You must pass a non-empty string for name in order to create an index',
            required: true,
        }),
        ...
        cloud: Property.StaticDropdown({
            ...
            description: 'The public cloud where you would like your index hosted (for serverless)',
            required: false,
            options: {
                options: [
                    { label: 'AWS', value: 'aws' },
        ...
        region: Property.ShortText({
            displayName: 'Region',
            description: 'The region where you would like your index to be created (for serverless)',
            required: false,
            defaultValue: 'us-west-2',
        }),
        environment: Property.ShortText({
            displayName: 'Environment',
            description: 'The environment where the index is hosted (for pod-based indexes)',
            required: false,
        }),
        ...
        shards: Property.Number({
            displayName: 'Shards',
            description: 'The number of shards. Shards split your data across multiple pods.',
            required: false,
        }),
        ...
        vectorType: Property.StaticDropdown({
            ...
            description: 'The type of vectors to store. Dense is default for most use cases.',
            required: false,
            options: {
                options: [
        ...
        waitUntilReady: Property.Checkbox({
            ...
            description: 'Wait until the index is ready to receive data before completing',
            required: false,
            defaultValue: false,
        }),
        suppressConflicts: Property.Checkbox({
            displayName: 'Suppress Conflicts',
            description: 'Do not throw if you attempt to create an index that already exists',
            required: false,
            defaultValue: false,
        }),
        tags: Property.Json({
    ...
    async run(context) {
        ...
        if (!name) {
            throw new Error('You must pass a non-empty string for `name` in order to create an index.')
        }

        if (dimension && dimension <= 0) {
            throw new Error('You must pass a positive integer for `dimension` in order to create an index.')
        }
        ...
        if (vType === 'sparse') {
            ...
            if (!dimension || dimension <= 0) {
                throw new Error('You must pass a positive `dimension` when creating a dense index.')
            }
        }
        ...
        try {
            ...
            if (indexType === 'serverless') {
                ...
                if (!podType) {
                    throw new Error('You must pass a `podType` for the pod `spec` object in order to create an index.')
                }
