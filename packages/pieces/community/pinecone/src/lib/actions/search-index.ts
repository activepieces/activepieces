        ...
        try {
            if (searchMode === 'find') {
                ...
                if (nameFilter) {
                    ...
                    filteredIndexes = allIndexes.filter((index) => index.name.toLowerCase().includes(filter))
                }
            ...
            return {
                ...
                error: 'Unknown Error',
                message:
                    caught instanceof Error ? caught.message : 'An unexpected error occurred while searching indexes.',
            }
        }
    },
})
