...
export const createCampaign = createAction({
    ...
    description: 'Create a new campaign with campaign name, subject, topic, sender name/address, and mailing list.',
    props: zohoCampaignsCommon.createCampaignProperties(),
    async run({ auth, propsValue }) {
        const location = (auth.props?.['location'] as string) || 'zoho.com'
        const accessToken = auth.access_token
        await propsValidation.validateZod(propsValue, zohoCampaignsCommon.createCampaignSchema)
        const list_details =
            propsValue.list_details?.reduce(
                (acc, curr) => {
                    acc[curr] = []
                    return acc
                },
