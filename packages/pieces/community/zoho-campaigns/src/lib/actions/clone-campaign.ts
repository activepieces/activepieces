...
export const cloneCampaign = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        await propsValidation.validateZod(propsValue, zohoCampaignsCommon.cloneCampaignSchema)
        const { campaignkey, campaignname, subject, from_name, from_add, reply_to, encode_type } = propsValue
        const campaigninfo = {
