...
export const addTagToContact = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        await propsValidation.validateZod(propsValue, zohoCampaignsCommon.addTagToContactSchema)
        const tags = await zohoCampaignsCommon.listTags({ accessToken, location })
        const tagExists =
            tags !== undefined &&
            tags.some((tagMap) => Object.values(tagMap).some((t) => t.tag_name === propsValue.tagName))
        if (!tagExists) {
