...
export const organizationSearch = createAction({
    ...
    description: 'Search for companies in the Apollo database with various filters',
    props: {
    ...
    async run({ propsValue, auth, store }) {
        ...
        if (propsValue.organization_not_locations && propsValue.organization_not_locations.length > 0) {
        ...
        if (propsValue.organization_industry_tag_ids && propsValue.organization_industry_tag_ids.length > 0) {
