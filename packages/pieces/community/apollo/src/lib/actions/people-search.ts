...
export const peopleSearch = createAction({
    ...
    description: 'Search for people in the Apollo database (does not return email/phone, use enrichment for that)',
    props: {
        ...
        person_seniorities: Property.Array({
            ...
            description: 'Filter by seniority levels (e.g., "senior", "manager", "director")',
            required: false,
        }),
        ...
        organization_num_employees_ranges: Property.Array({
            ...
            description: 'Filter by organization employee count ranges (e.g., "1,10", "250,500")',
            required: false,
        }),
    ...
    async run({ propsValue, auth, store }) {
        ...
        if (propsValue.person_seniorities && propsValue.person_seniorities.length > 0) {
        ...
        if (propsValue.organization_num_employees_ranges && propsValue.organization_num_employees_ranges.length > 0) {
            propsValue.organization_num_employees_ranges.forEach((range: unknown) => {
                queryParams.append('organization_num_employees_ranges[]', range as string)
            })
        }

        if (propsValue.q_organization_domains && propsValue.q_organization_domains.length > 0) {
            propsValue.q_organization_domains.forEach((domain: unknown) => {
                queryParams.append('q_organization_domains[]', domain as string)
            })
        }

        if (propsValue.organization_locations && propsValue.organization_locations.length > 0) {
