...
export const meetingIdDropdwon = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, '/meetings')
            ...
            return {
                ...
                options: response.meetings.map((meeting: any) => ({
                    label: meeting.title || meeting.meeting_id + meeting.timestamp_start_utc,
...
export const teamIdDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const response = await makeRequest(auth.secret_text, HttpMethod.GET, '/teams')
