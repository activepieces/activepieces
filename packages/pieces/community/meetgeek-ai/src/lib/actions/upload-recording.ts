...
export const uploadRecording = createAction({
    ...
    description: 'Upload a video or audio file for analysis and receive a notification upon completion via webhook',
    props: {
    ...
    async run(context) {
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/upload', body)
