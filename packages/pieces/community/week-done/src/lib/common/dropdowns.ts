...
import { DropdownOption, DropdownState, OAuth2PropertyValue } from '@activepieces/pieces-framework'
...
export async function weekdoneUsersDropdown(auth: OAuth2PropertyValue): Promise<DropdownState<number>> {
...
export async function weekdoneTeamsDropdown(auth: OAuth2PropertyValue): Promise<DropdownState<number>> {
...
export async function weekdoneItemsDropdown(
    ...
    const response = await weekdoneApiCall<
        WeekdoneListResponse<{ items: Array<{ id: number; description: string; type_id?: number }> }>
    >({
        auth,
        method: HttpMethod.GET,
        path: '/items',
