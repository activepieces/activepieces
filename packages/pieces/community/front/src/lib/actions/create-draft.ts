...
export const createDraft = createAction({
    ...
    props: {
        ...
        signature_id: Property.ShortText({
            ...
            description: 'The ID of the signature to use for the draft reply (if applicable).',
            required: false,
        }),
        should_add_default_signature: Property.Checkbox({
            displayName: 'Add Default Signature',
            description: 'Whether to append the default signature to the draft reply (if applicable).',
            required: false,
            defaultValue: false,
        }),
    },
    async run({ auth, propsValue }) {
        ...
        if (should_add_default_signature !== undefined)
            requestBody['should_add_default_signature'] = should_add_default_signature

        return await makeRequest(auth, HttpMethod.POST, `/channels/${channel_id}/drafts`, requestBody)
    },
})
