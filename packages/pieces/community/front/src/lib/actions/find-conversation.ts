...
import { inboxDropdown, tagsMultiSelectDropdown, teammateDropdown } from '../common/props';
...
export const findConversation = createAction({
    ...
    props: {
        ...
        custom_query: Property.ShortText({
            ...
            description: 'Enter a custom search query. This will be combined with other filters. See Front\'s search syntax documentation for details.',
            required: false,
        }),
    },
    async run(context) {
        const { inbox_id, tags, assignee_id, statuses, subject, custom_query } = context.propsValue;
        ...
        if (statuses) statuses.forEach(status => queryParts.push(`status:${status}`));
