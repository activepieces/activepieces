...
export const searchText = createAction({
  ...
  description: 'Search a namespace with a query text, query vector, or record ID and return the most similar records. Optionally rerank results based on relevance. Supports indexes with integrated embedding.',
  props: {
    ...
    vector: Property.Array({
      ...
      description: 'The query vector values (e.g., [0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3, 0.3])',
      required: false,
    }),
    ...
    fields: Property.Array({
      ...
      description: 'The fields to return in search results (e.g., ["chunk_text", "category"])',
      required: false,
    }),
    // Reranking options
  ...
  async run(context) {
    ...
    if (searchMethod === 'text' && !queryText) {
      throw new Error('You must provide query text when using text search method.');
    }
    if (searchMethod === 'vector' && (!vector || !Array.isArray(vector) || vector.length === 0)) {
      throw new Error('You must provide a query vector when using vector search method.');
    }
    if (searchMethod === 'id' && !recordId) {
      throw new Error('You must provide a record ID when using ID search method.');
    }
    ...
    try {
      ...
      if (enableRerank) {
        ...
        if (searchMethod === 'vector' || searchMethod === 'id') {
          if (!rerankQuery) {
            throw new Error('Rerank query is required when reranking vector or ID search results.');
          }
      ...
      return {
        ...
        summary: {
          ...
          readUnits: usage.read_units || usage.readUnits || 0,
          embedTokens: usage.embed_total_tokens || usage.embedTotalTokens || 0,
          rerankUnits: usage.rerank_units || usage.rerankUnits || 0,
        },
      ...
      return {
        success: false,
        error: 'Unknown Error',
        message: caught instanceof Error ? caught.message : 'An unexpected error occurred while searching records.',
        searchRequest: searchRequest,
        indexName: indexName,
        namespace: namespace,
      };
