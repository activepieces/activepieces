...
export const generateAiImage = createAction({
    ...
    description: 'Generate AI images based on text prompts using various models and styles',
    props: {
        ...
        artist: Property.ShortText({
            ...
            description: 'Art style inspired by a specific artist (e.g., "Van Gogh", "Picasso")',
            required: false,
        }),
    ...
    async run(context) {
        const { prompt, model, style, artist, dimensions, useHD, image } = context.propsValue
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/api/generate-image', payload)
