...
export const getAiTranscription = createAction({
    ...
    description: 'Generate AI transcriptions from audio files with speaker detection and enhanced quality options',
    props: {
        ...
        detect_speakers: Property.Checkbox({
            ...
            description: 'Automatically detect and identify different speakers in the audio',
            required: false,
            defaultValue: true,
        }),
        enhanced_quality: Property.Checkbox({
            displayName: 'Enhanced Quality',
            description: 'Use enhanced quality processing for better transcription accuracy',
            required: false,
            defaultValue: true,
        }),
    },
    async run(context) {
        const { url, name, audio_type, language, detect_speakers, enhanced_quality } = context.propsValue
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/api/transcriptions', payload)
