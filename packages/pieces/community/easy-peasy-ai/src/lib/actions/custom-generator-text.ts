...
export const customGeneratorText = createAction({
    ...
    description: 'Generate custom text for any purpose using the EasyPeasy AI template engine',
    props: {
        ...
        extra1: Property.LongText({
            ...
            description: 'Additional context or background information (optional, max 1000 characters)',
            required: false,
        }),
    ...
    async run(context) {
        const { keywords, extra1, outputs, language, shouldUseGPT4 } = context.propsValue
        ...
        const response = await makeRequest(context.auth.secret_text, HttpMethod.POST, '/api/generate', payload)
