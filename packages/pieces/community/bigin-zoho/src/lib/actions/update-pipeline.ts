...
export const updatePipeline = createAction({
  auth: biginZohoAuth,
  name: 'bigin_update_pipeline',
  displayName: 'Update Pipeline Record',
  description: 'Updates an existing deal/pipeline record in Bigin',
  props: {
    pipelineId: Property.ShortText({
      displayName: 'Pipeline ID',
      description: 'The ID of the deal/pipeline record to update',
      required: true,
    }),
  ...
  async run(context) {
    ...
    if (dealName) pipelineData.Deal_Name = dealName;
