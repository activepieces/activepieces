...
export const updatePipeline = createAction({
  ...
  description: 'Modify existing deal record',
  props: {
    recordId: Property.Dropdown({
      displayName: 'Pipeline Record',
      description: 'Select the pipeline record to update',
      required: true,
      refreshers: [],
      options: async ({ auth }) => {
        try {
  ...
  async run(context) {
    ...
    if (context.propsValue.dealName) body['Deal_Name'] = context.propsValue.dealName;
    if (context.propsValue.accountName) body['Account_Name'] = { id: context.propsValue.accountName };
    if (context.propsValue.subPipeline) body['Sub_Pipeline'] = context.propsValue.subPipeline;
