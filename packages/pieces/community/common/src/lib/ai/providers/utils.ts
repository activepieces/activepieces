...
type AuthHeaderOptions = { bearer: true } | { bearer: false; name: string; mapper?: HeaderValueMapper }
...
export function imageMapper(mapper: Omit<ImageModelMapper, '__tag'>): ImageModelMapper {
  return { ...mapper, __tag: ModelType.IMAGE }
}

export function chatMapper(mapper: Omit<ChatModelMapper, '__tag'>): ChatModelMapper {
  return { ...mapper, __tag: ModelType.CHAT }
}

export const hasMapper = (m: Model): m is Model & { mapper: ModelMapper<ModelType> } => {
