...
export const findChat = createAction({
    ...
    props: {
        ...
        chatgpt_autoresponse_enabled: Property.Checkbox({
            ...
            description: 'Filter by whether ChatGPT autoresponse is enabled for the chat',
            required: false,
        }),
    ...
    async run({ auth: apiKey, propsValue }) {
        ...
        const chatParams = {
            ...
            ...(responsible?.length && responsible.length > 0 ? { responsible: responsible.join(',') } : {}),
            ...rest,
        }
        ...
        if (response.status !== 'ok') {
            throw new Error(`Error fetching chat: ${response.message || 'Unknown error'}`)
        }
