...
export const sendMessageToNewChat = createAction({
    ...
    props: {
        ...
        contact: Property.DynamicProperties({
            displayName: 'Contact',
            description: 'Select the contact identifier based on the chosen contact type',
            required: true,
            refreshers: ['contactType'],
            auth: timelinesAiAuth,
            props: async ({ contactType }): Promise<DynamicPropsValue> => {
                ...
                if (contactType === 'jid') {
                    return {
                        jid: Property.ShortText({
                            displayName: 'JID',
                            description: "WhatsApp ID (JID) of a contact or group, e.g. '123456789@s.whatsapp.net'.",
                            required: true,
                        }),
                    }
                }
        ...
        attachment_template_id: Property.Number({
            ...
            description: 'Optional ID of a predefined attachment template to use when sending files.',
            required: false,
        }),
    },
