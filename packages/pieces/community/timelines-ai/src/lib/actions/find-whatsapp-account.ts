...
export const findWhatsappAccount = createAction({
    ...
    props: {
        id: Property.ShortText({
            ...
            description: 'The unique identifier of the WhatsApp account to search for.',
            required: false,
        }),
        phone: Property.ShortText({
            displayName: 'Phone Number',
            description: 'The phone number associated with the WhatsApp account to search for.',
            required: false,
        }),
    },
    async run({ auth: apiKey, propsValue: { id, phone } }) {
        ...
        if (response.status !== 'ok') {
            throw new Error(`Error fetching WhatsApp accounts: ${response.message || 'Unknown error'}`)
        }
