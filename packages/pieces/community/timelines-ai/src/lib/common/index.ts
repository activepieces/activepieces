...
export const timelinesAiCommon = {
    ...
    endpoints: {
        sendMessageToExistingChat: (chat_id: number) => `/chats/${chat_id}/messages`,
    ...
    // API Calls
    sendMessageToExistingChat: async ({ apiKey, ...messageParams }: SendMessageToExistingChatParams) => {
        const response = await httpClient.sendRequest<SendMessageToExistingChatResponse>({
            method: HttpMethod.POST,
            url:
                timelinesAiCommon.baseUrl +
                timelinesAiCommon.endpoints.sendMessageToExistingChat(messageParams.chat_id),
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
            body: messageParams,
        })
        return response.body
    },
    sendMessageToPhoneNumber: async ({ apiKey, ...messageParams }: SendMessageToPhoneNumberParams) => {
        const response = await httpClient.sendRequest<SendMessageToPhoneNumberResponse>({
            method: HttpMethod.POST,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.sendMessageToPhoneNumber,
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
            body: messageParams,
        })
        return response.body
    },
    sendMessageToJid: async ({ apiKey, ...messageParams }: SendMessageToJidParams) => {
        const response = await httpClient.sendRequest<SendMessageToJidResponse>({
            method: HttpMethod.POST,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.sendMessageToJid,
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
            body: messageParams,
        })
        return response.body
    ...
    updateChat: async ({ apiKey, ...chatParams }: UpdateChatParams) => {
        const response = await httpClient.sendRequest<UpdateChatResponse>({
            ...
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.updateChat(chatParams.chat_id.toString()),
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
            body: chatParams,
        })
        return response.body
    ...
    getChats: async ({ apiKey, ...chatParams }: GetChatsParams) => {
        const { group, read, closed, chatgpt_autoresponse_enabled, page, ...rest } = chatParams
        const queryParams = {
            ...
            ...(chatgpt_autoresponse_enabled !== undefined
                ? {
                      chatgpt_autoresponse_enabled: chatgpt_autoresponse_enabled.toString(),
                  }
                : {}),
    ...
    getMessage: async ({ apiKey, message_uid }: GetMessageParams) => {
        const response = await httpClient.sendRequest<GetMessageResponse>({
            method: HttpMethod.GET,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.getMessage(message_uid),
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
        })
        return response.body
    },
    getUploadedFile: async ({ apiKey, file_uid }: GetUploadedFileParams) => {
        const response = await httpClient.sendRequest<GetUploadedFileResponse>({
            method: HttpMethod.GET,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.getUploadedFile(file_uid),
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
        })
        return response.body
    },
    ...
    listWhatsappAccounts: async ({ apiKey }: AuthenticationParams) => {
        const response = await httpClient.sendRequest<GetWhatsappAccountsResponse>({
            method: HttpMethod.GET,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.getWhatsappAccounts,
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
        })
        return response.body
    },
    ...
    createWebhook: async ({ apiKey, ...webhookParams }: CreateWebhookParams) => {
        const response = await httpClient.sendRequest<CreateWebhookResponse>({
            method: HttpMethod.POST,
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.createWebhook,
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
            body: webhookParams,
        })
        return response.body
    ...
    deleteWebhook: async ({ apiKey, webhook_id }: DeleteWebhookParams) => {
        const response = await httpClient.sendRequest<DeleteWebhookResponse>({
            ...
            url: timelinesAiCommon.baseUrl + timelinesAiCommon.endpoints.deleteWebhook(webhook_id),
            headers: timelinesAiCommon.getHeaders(apiKey.secret_text),
        })
        return response.body
    },
