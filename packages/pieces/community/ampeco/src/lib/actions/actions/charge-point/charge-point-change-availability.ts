...
export const chargePointChangeAvailabilityAction = createAction({
    ...
    props: {
        ...
        evseNetworkId: Property.Number({
            ...
            description:
                'The id of the connector for which availability needs to change. If missing the availability of the Charge Point and all its connectors needs to change.',
            required: false,
        }),
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v1.0/{chargePoint}/change-availability',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['evseNetworkId', 'type'])
