...
export const subscriptionPlanReplaceAction = createAction({
    ...
    props: {
        ...
        planId: Property.Number({
            displayName: 'Plan Id',
            description:
                'The ID of the subscription plan that replaces the current one. The new Subscription plan will be added to the Subscription plans Restrictions for the Tariffs, where the old plan is present. Also it will be added to all Charge points that require the current Subscription plan.',
            required: true,
        }),
        ...
        replaceAt: Property.DateTime({
            ...
            description:
                'The date from which the new Subscription Plan replaces the current one. If left empty, it is replaced immediately.',
            required: false,
        }),
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/subscription-plans/v1.0/{subscriptionPlan}/replace',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['planId', 'replaceAt', 'reason'])
