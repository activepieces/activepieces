...
export const chargePointGetSecurityLogAction = createAction({
    ...
    props: {
        ...
        retries: Property.Number({
            displayName: 'Retries',
            description:
                'This specifies how many times the Charge Point must try to upload the log before giving up. If this field is not present, it is left to Charge Point to decide how many times it wants to retry.',
            required: false,
        }),
        ...
        startTime: Property.ShortText({
            ...
            description:
                'This contains the date and time of the oldest logging information to include in the diagnostics. Please provide the value in the following format `Y-m-d H:i:s`.',
            required: false,
        }),
        ...
        stopTime: Property.ShortText({
            ...
            description:
                'This contains the date and time of the latest logging information to include in the diagnostics. Please provide the value in the following format `Y-m-d H:i:s`.',
            required: false,
        }),
    },
    async run(context): Promise<ChargePointGetSecurityLogResponse> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v1.0/{chargePoint}/get-security-log',
                context.propsValue,
            )
