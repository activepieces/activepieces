...
export const chargePointTriggerMessageChargePointAction = createAction({
    ...
    props: {
        ...
        type: Property.StaticDropdown({
            displayName: 'Type',
            description:
                'The type of the message to be triggered. Messages available only for 2.0.1 chargers - LogStatusNotification, PublishFirmwareStatusNotification, SignChargingStationCertificate, SignCombinedCertificate, TransactionEvent',
            required: true,
            options: {
                options: [
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v1.0/{chargePoint}/trigger-message',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['type'])
