...
export const configurationTemplateApplyToChargePointsAction = createAction({
    ...
    props: {
        ...
        shouldPersist: Property.StaticDropdown({
            ...
            description:
                'Specifies whether the configuration template will be enforced each time the charge point boots.',
            required: false,
            options: {
                options: [
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/configuration-template/v1.0/{template}/apply-to-charge-points',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['chargePointIds', 'shouldPersist'])
