        ...
        notifications: Property.StaticMultiSelectDropdown({
            ...
            options: {
                options: [
                    ...
                    {
                        label: 'ChargePointSyncConfigurationNotification',
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/notifications/v1.0/{notification}/resend',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['notifications', 'startTime', 'endTime'])
