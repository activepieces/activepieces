...
export const transactionUpdatePaymentReferenceAction = createAction({
    ...
    props: {
        ...
        ref: Property.ShortText({
            displayName: 'Ref',
            description:
                'Reference to the payment processor or payment terminal where more information about the transaction could be checked.',
            required: true,
        }),
    },
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/transactions/v1.0/{transaction}/update-payment-reference',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['ref'])
