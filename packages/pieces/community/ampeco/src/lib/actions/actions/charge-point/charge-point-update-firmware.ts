...
export const chargePointUpdateFirmwareAction = createAction({
    ...
    props: {
        ...
        url: Property.ShortText({
            displayName: 'URL',
            description:
                'Location (URL) from which to retrieve the firmware. Either this or `firmwareVersionId` must be provided.',
            required: false,
        }),
        ...
        retrieveAfter: Property.DateTime({
            ...
            description:
                'Specifies the date and time after which the Charge Point should retrieve the new firmware. If not provided, the current *datetime* will be used.',
            required: false,
        }),
        ...
        retries: Property.Number({
            displayName: 'Retries',
            description:
                'Specifies how many times the Charge Point must try to retrieve the (new) firmware before giving up.',
            required: false,
        }),
        ...
        downloadUrlProtocol: Property.StaticDropdown({
            ...
            description:
                'Protocol to use for the firmware download URL when using firmware repository. Use HTTP only if the charge point does not support encrypted connections.',
            required: false,
            options: {
                options: [
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v2.0/{chargePoint}/update-firmware',
                context.propsValue,
            )
