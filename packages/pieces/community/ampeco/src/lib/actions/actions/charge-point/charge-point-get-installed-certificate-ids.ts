...
export const chargePointGetInstalledCertificateIdsAction = createAction({
    ...
    props: {
        ...
        certificateType: Property.StaticDropdown({
            ...
            description:
                'Indicates the type of certificates requested. Default value to be empty - the system should interpret the Null/empty value that all certificate types are requested.',
            required: false,
            options: {
                options: [
    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v2.0/{chargePoint}/sync-certificates',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['certificateType'])
