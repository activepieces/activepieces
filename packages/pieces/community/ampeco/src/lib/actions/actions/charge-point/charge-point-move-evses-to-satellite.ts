    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v1.0/{chargePoint}/move-evses-to-satellite',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['satelliteChargePointId', 'evseIds'])
