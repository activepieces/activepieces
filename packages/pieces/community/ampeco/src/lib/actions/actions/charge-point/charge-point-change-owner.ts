...
export const chargePointChangeOwnerAction = createAction({
    ...
    props: {
        ...
        userId: Property.Number({
            displayName: 'User ID',
            description:
                'This User would become the owner of the personal charge point. If left empty - no Owner would be assigned.',
            required: false,
        }),
    },
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/charge-point/v1.0/{chargePoint}/change-owner',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['userId'])
