    ...
    async run(context): Promise<unknown> {
        try {
            const url = processPathParameters(
                '/public-api/actions/electricity-meter/v1.0/{electricityMeter}/report-consumption',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['timestamp', 'currentL1', 'currentL2', 'currentL3'])
