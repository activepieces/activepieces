...
export const changeFlexibilityAssetStatusAction = createAction({
    ...
    props: {
        ...
        endsAt: Property.DateTime({
            displayName: 'Ends At',
            description:
                'ISO 8601 formatted date. If an end date is set the asset will be automatically disabled at that time. Historical time series and forecasts are not generated after the end date.',
            required: false,
        }),
    },
    async run(context): Promise<ChangeFlexibilityAssetStatusResponse> {
        try {
            const url = processPathParameters(
                '/public-api/actions/flexibility-asset/v1.0/{flexibilityAsset}/change-status',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['status', 'endsAt'])
