...
export const createRoamingCustomTariffFilterAction = createAction({
    ...
    props: {
        ...
        applicableCurrentTypes: Property.StaticMultiSelectDropdown({
            ...
            description:
                'Array of current types (AC, DC) that this filter applies to, pass an empty array to clear all current type restrictions.',
            required: false,
            options: {
                options: [
                    { label: 'AC', value: 'AC' },
        ...
        evseIdPrefix: Property.ShortText({
            ...
            description:
                'Filter EVSEs whose IDs start with specified prefix(es). Multiple prefixes separated by newlines',
            required: false,
        }),
        ...
        order: Property.Number({
            displayName: 'Order',
            description:
                'Position of the tariff filter in the ordering sequence. When not provided during creation, it will go last. If an already existing order number is provided, it will reorder the other tariff filters to take their place in the sequence.',
            required: false,
        }),
    },
    async run(context): Promise<CreateRoamingCustomTariffFilterResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/roaming-operators/v2.0/{roamingOperator}/custom-tariff-filters',
                context.propsValue,
            )
