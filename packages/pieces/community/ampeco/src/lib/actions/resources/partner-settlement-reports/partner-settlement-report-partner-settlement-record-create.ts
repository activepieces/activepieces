    ...
    async run(context): Promise<PartnerSettlementReportPartnerSettlementRecordCreateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/partner-settlement-reports/v1.0/{partnerSettlementReport}/records',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['date', 'paidAmount', 'note'])
