...
export const electricityRateUpdateAction = createAction({
    ...
    props: {
        ...
        taxPercentage: Property.Number({
            ...
            description:
                'This field is used only to indicate the applied tax. All Electricity Rate prices should be provided with the tax included in the price.',
            required: false,
        }),
    },
    async run(context): Promise<ElectricityRateUpdateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/electricity-rates/v2.0/{electricityRate}',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['name', 'utilityId', 'defaultPrice', 'taxPercentage'])
