...
export const updateRoamingCustomTariffFilterAction = createAction({
    ...
    props: {
        ...
        order: Property.Number({
            displayName: 'Order',
            description:
                'Position of the tariff filter in the ordering sequence. If an already existing order number is provided, it will reorder the other tariff filters to take their place in the sequence.',
            required: false,
        }),
    },
    async run(context): Promise<UpdateRoamingCustomTariffFilterResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/roaming-operators/v2.0/{roamingOperator}/custom-tariff-filters/{customTariffFilter}',
                context.propsValue,
            )
