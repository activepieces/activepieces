    ...
    async run(context): Promise<ListPcIdsResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/provisioning-certificates/v2.0',
                context.propsValue,
            )
            ...
            return (await makeAmpecoApiCall(context.auth, url, HttpMethod.GET, body, queryParams)) as ListPcIdsResponse
        } catch (error) {
