    ...
    async run(context): Promise<PaymentMethodCreateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/users/v1.0/{user}/payment-methods',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['tokenizedType', 'bankTransferType', 'transactionId'])
