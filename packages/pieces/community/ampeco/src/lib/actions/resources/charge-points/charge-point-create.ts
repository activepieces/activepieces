...
export const chargePointCreateAction = createAction({
  ...
  description: 'Create new charge point. (Endpoint: POST /public-api/resources/charge-points/v2.0)',
  props: {
  ...
  communicationMode: Property.StaticDropdown({
    ...
    options: {
      ...
      { label: 'via_ocpp_connected_charge_point', value: 'via_ocpp_connected_charge_point' }
      ],
    },
  }),
  },
  async run(context): Promise<ChargePointCreateResponse> {
    try {
      ...
      const body = prepareRequestBody(context.propsValue,
        ['name', 'type', 'pin', 'locationId', 'chargingZoneId', 'electricityRateId', 'subscription', 'networkType', 'status', 'managedByOperator', 'externalId', 'network', 'capabilities', 'autoStartWithoutAuthorization', 'disableAutoStartEmulation', 'security', 'modelId', 'enableAutoFaultRecovery', 'user', 'partner', 'utilityId', 'tags', 'sharingCode', 'enabledRandomisedDelay', 'noticeId', 'usesRenewableEnergy', 'ocppConnectedChargePointId', 'communicationMode']
      );
