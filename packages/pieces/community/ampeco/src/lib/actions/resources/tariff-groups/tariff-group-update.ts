...
export const tariffGroupUpdateAction = createAction({
    ...
    props: {
        ...
        tariffIds: Property.Array({
            ...
            description:
                'Ordered list of tariff ids in the group. Please note that the order is checked from bottom to top, meaning that the tariff with the largest index in that list (in other words that is the lowest in the list), that matches the requirements will apply',
            required: false,
        }),
    ...
    async run(context): Promise<TariffGroupUpdateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/tariff-groups/v1.0/{tariffGroup}',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['name', 'tariffIds', 'partnerId'])
