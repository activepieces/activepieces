...
export const bookingRequestCreateAction = createAction({
  ...
  displayName: 'Resources - Booking Requests - Booking Request Create',
  description: 'Create a new booking request. If you want to find the available slots for a given location use the Actions / Location / Check Booking Availability. (Endpoint: POST /public-api/resources/booking-requests/v1.0)',
  props: {
  ...
  async run(context): Promise<BookingRequestCreateResponse> {
    try {
      ...
      const body = prepareRequestBody(context.propsValue,
        ['requestBody_VariantType', 'type', 'userId', 'locationId', 'evseId', 'currentType', 'minPower', 'maxPower', 'connectorType', 'startAt', 'endAt', 'bookingId']
      );
