...
export const updateInstallerJobAction = createAction({
    ...
    props: {
        ...
        installerAdminId: Property.Number({
            ...
            description:
                'The ID of the installer admin if such is assigned to the installer job, if passed pin will be ignored',
            required: false,
        }),
        ...
        pin: Property.ShortText({
            displayName: 'Pin',
            description:
                'The PIN of the installer job, it is used to verify access rights to execute the installer job, if passed and no installerAdminId is provided, the pin will be assigned to the installer job',
            required: false,
        }),
    ...
    async run(context): Promise<UpdateInstallerJobResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/installer-jobs/v1.0/{installerJob}',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['installerAdminId', 'pin', 'description'])
