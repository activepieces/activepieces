...
export const roamingOperatorUpdateAction = createAction({
    ...
    props: {
        ...
        partnerId: Property.Number({
            ...
            description:
                'If a non-null value is provided, it will set enabled to true. If null is provided, it will set enabled to false.',
            required: false,
        }),
        ...
        cpoSettings__cpoQrcodePrefix: Property.ShortText({
            ...
            description:
                'Example - "http://m.intercharge.eu/qr?evseid=" if the URL in the QR Code is http://m.intercharge.eu/qr?evseid=CH*ION*E213604.',
            required: false,
        }),
    ...
    async run(context): Promise<RoamingOperatorUpdateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/roaming-operators/v2.0/{roamingOperator}',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['businessName', 'partnerId', 'enabled', 'cpoSettings'])
