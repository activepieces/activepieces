...
export const chargePointEvseUpdateAction = createAction({
    ...
    props: {
        ...
        powerOptions__maxVoltage: Property.StaticDropdown({
            ...
            description:
                'The maxVoltage of a charge point can fluctuate. Hence, when creating a charge point in the system, the maxVoltage is given as a range. For OCPI purposes it maps as follows: 220-240 = 230 110-130 = 120 400 = 400 380 = 380.',
            required: false,
            options: {
                options: [
                    { label: '230', value: '230' },
        ...
        powerOptions__phaseRotation: Property.StaticDropdown({
            ...
            description:
                '`R` stands for `L1`, </br> `S` - for `L2` </br> `T` - for `L3` </br> So for example `RST` = `L1`, `L2`, `L3`, while `RTS` = `L1`, `L3`, `L2`, etc.',
            required: false,
            options: {
                options: [
                    { label: 'RST', value: 'RST' },
    ...
    async run(context): Promise<ChargePointEvseUpdateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/charge-points/v2.0/{chargePoint}/evses/{evse}',
                context.propsValue,
            )
