...
export const circuitUserPriorityUpdateAction = createAction({
    ...
    props: {
        ...
        targetId: Property.Number({
            ...
            description:
                'The ID of the User Group the user must be part of or the ID of the Partner from which the user must have an invite from, for the set priority apply for the session.',
            required: false,
        }),
        ...
        priority: Property.Number({
            ...
            description:
                'The priority where 1 is equal to anyone else, 2 is twice as important, 0.5 is half as important.',
            required: false,
        }),
    },
    async run(context): Promise<CircuitUserPriorityUpdateResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/circuits/v2.0/{circuit}/user-priorities/{userPriority}',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['targetId', 'type', 'priority'])
