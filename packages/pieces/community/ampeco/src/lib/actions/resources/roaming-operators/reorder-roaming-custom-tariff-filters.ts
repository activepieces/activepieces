...
export const reorderRoamingCustomTariffFiltersAction = createAction({
    ...
    props: {
        ...
        filters: Property.Array({
            displayName: 'Filters',
            description:
                'Array of custom tariff filter IDs with their new positions. Only include filters that need to be moved - others will maintain their relative positions.',
            required: true,
            properties: {
                id: Property.Number({
                    displayName: 'Id',
    ...
    async run(context): Promise<ReorderRoamingCustomTariffFiltersResponse> {
        try {
            const url = processPathParameters(
                '/public-api/resources/roaming-operators/v2.0/{roamingOperator}/custom-tariff-filters/reorder',
                context.propsValue,
            )

            const queryParams = prepareQueryParams(context.propsValue, [])

            const body = prepareRequestBody(context.propsValue, ['filters'])
