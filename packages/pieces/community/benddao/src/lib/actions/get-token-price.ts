...
export const getTokenPrice = createAction({
  ...
  displayName: 'Get BEND Token Price',
  description: 'Fetches the current BEND governance token price and market data from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<CoinGeckoResponse>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/benddao',
      queryParams: {
        localization: 'false',
        tickers: 'false',
