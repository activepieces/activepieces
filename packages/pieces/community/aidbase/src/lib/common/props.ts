...
export const emailInboxDropdown = Property.Dropdown<string, false, typeof aidbaseAuth>({
    ...
    description: 'Select the email inbox to watch. Leave blank to trigger for all inboxes.',
    required: false,
    refreshers: [],
    options: async ({ auth }) => {
        if (!auth) {
...
export const ticketFormDropdown = Property.Dropdown<string, false, typeof aidbaseAuth>({
    ...
    description: 'Select the ticket form to watch. Leave blank to trigger for all forms.',
    required: false,
    refreshers: [],
    options: async ({ auth }) => {
        if (!auth) {
...
export const knowledgeItemDropdown = Property.Dropdown<string, true, typeof aidbaseAuth>({
    ...
    options: async ({ auth }) => {
        ...
        const getLabel = (item: { type: string; title?: string; base_url?: string; id: string }) => {
            switch (item.type) {
                case 'faq':
