...
async function addWebsite(apiKey: string, websiteUrl: string) {
    ...
    if (response.body.success) {
        ...
        throw new Error(response.body.message || 'Failed to add website to Aidbase.')
    }
}

async function listKnowledgeItems(apiKey: string): Promise<ListKnowledgeItemsResponse> {
    const response = await httpClient.sendRequest<{
        success: boolean
    ...
    if (response.body.success && response.body.data) {
        ...
        throw new Error(response.body.message || 'Failed to list knowledge items from Aidbase.')
    }
}

async function addFaqItem(apiKey: string, faqId: string, params: AddFaqItemParams) {
    const response = await httpClient.sendRequest<{
        success: boolean
        data?: unknown
        message?: string
    ...
    if (response.body.success) {
        ...
        throw new Error(response.body.message || 'Failed to add FAQ item to Aidbase.')
    }
}
...
async function createFaq(apiKey: string, params: CreateFaqParams) {
    ...
    if (response.body.success) {
        ...
        throw new Error(response.body.message || 'Failed to create FAQ in Aidbase.')
    }
}
...
async function listChatbots(apiKey: string): Promise<ListChatbotsResponse> {
    ...
    if (response.body.success && response.body.data?.items) {
        ...
        throw new Error(response.body.message || 'Failed to list chatbots from Aidbase.')
    }
}

async function createChatbotReply(apiKey: string, chatbotId: string, params: CreateReplyParams) {
    const response = await httpClient.sendRequest<{
        success: boolean
        data?: unknown
        message?: string
    ...
    if (response.body.success) {
        ...
        throw new Error(response.body.message || 'Failed to create chatbot reply in Aidbase.')
    }
}
...
async function startTraining(apiKey: string, knowledgeId: string) {
    ...
    if (response.body.success) {
        ...
        throw new Error(response.body.message || 'Failed to start training in Aidbase.')
    }
}
