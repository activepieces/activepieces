import { getAccessTokenOrThrow, HttpMethod, httpClient } from '@activepieces/pieces-common'
...
export const instagramCommon = {
    ...
    createVideoPost: async (page: FacebookPageDropdown, caption: string | undefined, video: string) => {
        const createContainerRequest = await httpClient.sendRequest({
            method: HttpMethod.POST,
            url: `${instagramCommon.baseUrl}/${page.id}/media`,
            body: {
        ...
        const isUploaded = await isUploadSuccessful(createContainerRequest.body.id, page.accessToken, 0)
...
async function isUploadSuccessful(containerId: string, accessToken: string, retryCount: number): Promise<boolean> {
