...
\`\`\``,
    ...
    async run(context) {
        ...
        const leads = typeof lead_list === 'string' ? JSON.parse(lead_list) : lead_list
        ...
        if (ignore_duplicate_leads_in_other_campaign) settings['ignore_duplicate_leads_in_other_campaign'] = true
        if (ignore_community_bounce_list) settings['ignore_community_bounce_list'] = true
