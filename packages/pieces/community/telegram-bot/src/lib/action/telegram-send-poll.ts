...
export const telegramSendPoll = createAction({
  ...
  props: {
    ...
    explanation_parse_mode: Property.StaticDropdown({
      displayName: 'Explanation Format',
      description:
        'How the quiz explanation should be parsed by Telegram. Defaults to plain text; only set MarkdownV2 or HTML when the explanation is correctly escaped/tagged.',
      required: false,
      options: {
        options: [
  ...
  async run(ctx) {
    ...
    if (pollType === 'quiz') {
      ...
      throw new Error(
        '"Correct Option Id" is only valid for quiz polls. Set "Poll Type" to Quiz or clear this field.'
      );
    }
    if (ctx.propsValue.open_period && ctx.propsValue.close_date) {
      throw new Error(
        '"Open Period" and "Close Date" are mutually exclusive — set at most one.'
      );
    }
