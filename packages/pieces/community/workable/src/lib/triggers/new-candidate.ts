...
export const newCandidate = createTrigger({
    ...
    description:
        'Triggers when new candidate submits application. Can be filtered by specific job and/or hiring pipeline stage.',
    props: {
        ...
        stage_slug: Property.ShortText({
            ...
            description:
                "Stage slug to filter by specific hiring pipeline stage (e.g., 'applied', 'phone_screen', 'interview', 'offer'). Leave empty to trigger for all stages.",
            required: false,
        }),
    },
    sampleData: {
    ...
    async onEnable(context) {
        ...
        const subscription = await workableCommon.subscribeWebhook(subdomain, accessToken, webhookUrl, event, {
