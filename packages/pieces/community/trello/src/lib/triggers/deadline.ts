...
import { getCardsInBoard, getCardsInList, trelloCommon } from '../common'
        ...
        return cards
            .filter(
                (card) =>
                    card.due &&
                    !card.dueComplete &&
                    dayjs(card.due).isAfter(now) &&
                    dayjs(card.due).isBefore(upcoming) &&
                    dayjs(card.due).valueOf() > lastFetchEpochMS,
            )
            .map((card) => ({
                epochMilliSeconds: dayjs(card.due).valueOf(),
                data: card,
