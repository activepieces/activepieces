...
export const cardMovedTrigger = createTrigger({
  ...
  async onEnable(context) {
    ...
    const response = await trelloCommon.create_webhook(context.auth, element_id, context.webhookUrl)
  ...
  async run(context) {
    ...
    const card = await getCardDetail(context.auth.username, context.auth.password, response_body.entities.card.id)
    return [card]
  },
  async test(context) {
    let cards: Array<Record<string, unknown>> = []
    try {
      cards = await getCardsInList(context.auth.username, context.auth.password, context.propsValue.list_id)

      cards.sort((a: any, b: any) => new Date(b.dateLastActivity).getTime() - new Date(a.dateLastActivity).getTime())
