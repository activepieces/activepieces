...
import { type CopyResponseObject, ImapFlow, type ListResponse, type MailboxLockObject } from 'imapflow'
...
async function performImapOperation(auth: ImapAuth, callback: (imapClient: ImapFlow) => Promise<unknown>) {
    ...
    try {
        ...
        throw new ImapError(imapError.message || 'Failed to perform IMAP operation')
    } finally {
        try {
