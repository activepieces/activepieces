...
import { type Attachment, fetchEmails, imapAuth, type Message, mailboxDropdown } from '../common'
...
const polling: Polling<AppConnectionValueForAuthProperty<typeof imapAuth>, StaticPropsValue<typeof props>> = {
...
async function enrichAttachments(items: Message[], files: FilesService) {
    return Promise.all(
        items.map(async (item) => {
            ...
            const convertedAttachments = attachments ? await convertAttachment(attachments, files) : []
