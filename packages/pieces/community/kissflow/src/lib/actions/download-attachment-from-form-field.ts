...
function extractAttachmentNameFromHeaders(headers?: HttpHeaders): string | null {
    let fileName = null
...
export const downloadAttachmentFromFormField = createAction({
    ...
    description: 'Downloads a specific attachment from a form field of your process.',
    auth: kissflowAuth,
    props: {
    ...
    async run(context) {
        ...
        const { processId, instanceId, activityInstanceId, fieldId, attachmentId } = context.propsValue
        ...
        const fileName = extractAttachmentNameFromHeaders(resp.headers) || 'attachment'
