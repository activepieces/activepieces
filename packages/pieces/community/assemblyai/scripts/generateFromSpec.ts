...
;(async function () {
  ...
  if (!specLocation) throw new Error('OPENAPI_SPEC_LOCATION env variable is required')
...
function createPropsFromSchema(schema: any): Record<string, any> {
  ...
  if (schema.allOf) {
    const obj = {}
    ;(schema.allOf as any[]).forEach((schema) => Object.assign(obj, createPropsFromSchema(schema)))
    return obj
  }
  ...
  for (let [key, value] of Object.entries(schema.properties) as [key: string, value: any]) {
    ...
    if (value.anyOf) {
      ...
      if (value.anyOf.findIndex((item: any) => item.type === 'string') > -1 && enumAnyOfIndex > -1) {
    ...
    } else if (value.items && value.items.type === 'string' && value.items.enum) {
      properties[key] = {
...
function createField(key: string, label: string, value: any, required: boolean) {
  const field: any = {
    displayName: label,
    type: mapType(value),
    required,
  ...
  if (value.enum) {
    ...
    field.options = {
      options: (value.enum as string[]).map((item) => {
        ...
        if (value['x-aai-enum'] && value['x-aai-enum'][item] && value['x-aai-enum'][item]['label']) {
...
function createTs(propsJson: Record<string, { type: string } & object>): string {
...
function createTsProps(propsJson: Record<string, { type: string } & object>): string {
  let result = '{\n'
  for (const key in propsJson) {
    const { type, ...prop } = propsJson[key]
    let innerProps: null | string = null
    if ('properties' in prop) {
      innerProps = createTsProps(prop['properties'] as Record<string, { type: string } & object>)
