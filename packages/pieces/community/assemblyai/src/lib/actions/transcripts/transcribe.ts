...
type TranscribeContext = ActionContext<typeof assemblyaiAuth, typeof transcribeProps>
...
function createTranscriptParams(context: TranscribeContext): TranscriptParams {
  ...
  if ('wait_until_ready' in transcriptParams) delete transcriptParams['wait_until_ready']
  if ('throw_on_error' in transcriptParams) delete transcriptParams['throw_on_error']
  if ('auth' in transcriptParams) delete transcriptParams['auth']
  return transcriptParams as TranscriptParams
}
function handleWebhookUrl(context: TranscribeContext, transcriptParams: TranscriptParams) {
  if (context.propsValue.wait_until_ready) {
...
function handlePiiAudio(context: TranscribeContext) {
  if (context.propsValue.wait_until_ready === true && context.propsValue.redact_pii_audio === true) {
    throw new Error(
