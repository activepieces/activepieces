...
export const getRedactedAudio = createAction({
  ...
  props: {
    ...
    download_file_name: Property.ShortText({
      ...
      description: 'The desired file name for storing in ActivePieces. Make sure the file extension is correct.',
      required: true,
  ...
  async run(context) {
    ...
    const redactedAudioResponse = await client.transcripts.redactedAudio(context.propsValue.id)
    if (redactedAudioResponse.status !== 'redacted_audio_ready') {
      return redactedAudioResponse
    }
    if (context.propsValue.download_file) {
      const file = await client.transcripts.redactedAudioFile(context.propsValue.id)
