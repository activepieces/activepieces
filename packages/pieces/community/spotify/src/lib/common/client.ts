...
export class SpotifyWebApi {
    ...
    async addSavedItems(request: LibraryModifyItemsRequest) {
        await this.makeRequest(HttpMethod.PUT, '/me/library', prepareQuery(request))
    }

    async removeSavedItems(request: LibraryModifyItemsRequest) {
        await this.makeRequest(HttpMethod.DELETE, '/me/library', prepareQuery(request))
    }

    async addItemsToPlaylist(id: string, request: PlaylistAddItemsRequest) {
        await this.makeRequest(HttpMethod.POST, '/playlists/' + id + '/items', undefined, request)
    }

    async removeItemsFromPlaylist(id: string, request: PlaylistRemoveItemsRequest) {
        await this.makeRequest(HttpMethod.DELETE, '/playlists/' + id + '/items', undefined, request)
    }

    async reorderPlaylist(id: string, request: PlaylistReorderItemsRequest) {
        await this.makeRequest(HttpMethod.PUT, '/playlists/' + id + '/items', undefined, request)
    }
}
