...
export const generateImage = createAction({
  ...
  props: {
    ...
    modelOptions: Property.DynamicProperties({
      ...
      props: async (propsValue) => {
        ...
        const isGeminiFlashModel = selectedModel.includes('gemini-');
        ...
        if (isGeminiFlashModel) {
          fields['inputImages'] = Property.Array({
  ...
  async run(context) {
    ...
    const inputImages = modelOptions?.['inputImages'];
    ...
    const isGeminiFlashModel = model.includes('gemini-');
    ...
    if (isGeminiFlashModel) {
      ...
      const validImages = (inputImages as any[])?.filter(
        (item) => item && item.image && item.image.data
      ) || [];

      if (validImages.length > 0) {
        const imageParts = validImages.map((item) => {
          let mimeType = 'image/png';
          const fileExt = item.image.extension?.toLowerCase() || '';
          
          if (fileExt.includes('jp')) mimeType = 'image/jpeg';
          if (fileExt.includes('webp')) mimeType = 'image/webp';

          return {
            inlineData: {
              mimeType,
              data: Buffer.from(item.image.data).toString('base64'),
            },
          };
        });
        promptContent = [{ role: 'user', parts: [{ text: prompt }, ...imageParts] }];
      } else {
        // Fallback to text-only if no valid images were found
      ...
      const images = await Promise.all(
        generatedImagesBytes.map(async (base64Data, index) => {
