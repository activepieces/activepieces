...
import { GenerateContentConfig, GoogleGenAI, Part, ThinkingConfig, ThinkingLevel } from '@google/genai'
import mime from 'mime-types'
import { GoogleVertexAIAuthValue, vertexAiAuth } from '../auth'
import { getVertexAILocationOptions, getVertexAIModelOptions } from '../common'
...
export const generateContent = createAction({
    ...
    props: {
        location: Property.Dropdown({
            ...
            auth: vertexAiAuth,
            options: async ({ auth }) => getVertexAILocationOptions(auth as GoogleVertexAIAuthValue | undefined),
        }),
        model: Property.Dropdown({
            ...
            options: async ({ auth, location }) =>
                getVertexAIModelOptions(auth as GoogleVertexAIAuthValue | undefined, location as string | undefined),
        }),
        ...
        if (candidate) {
            ...
            if (finishReason !== 'STOP') {
                if (finishReason === 'MAX_TOKENS') {
                    throw new Error(
                        `Generation stopped due to token limit. Consider removing the max output tokens or shortening your prompt.`,
                    )
                }
            }
        }
