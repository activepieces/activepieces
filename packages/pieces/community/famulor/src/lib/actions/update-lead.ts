...
export const updateLead = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (!hasUpdate) {
            throw new Error('Provide at least one field to update: campaign, phone number, status, or variables.')
        }

        const statusTrimmed = status !== undefined && status !== null ? String(status).trim() : ''
        const statusValue =
            statusTrimmed === '' ? undefined : (statusTrimmed as 'created' | 'completed' | 'reached-max-retries')
