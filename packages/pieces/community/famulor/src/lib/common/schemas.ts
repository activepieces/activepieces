...
const e164Phone = z.string().regex(/^\+[1-9]\d{1,14}$/, 'Phone number must be in E.164 format (e.g. +1234567890)')
...
const campaignWeekday = z.enum(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'])
...
export const createCampaign = {
    ...
    allowed_hours_start_time: z
        .union([z.literal(''), z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'Must be H:i (e.g. 09:00)')])
        .optional(),
    allowed_hours_end_time: z
        .union([z.literal(''), z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/, 'Must be H:i (e.g. 17:00)')])
        .optional(),
...
const supportedPurchaseCountryCodes = z.enum(['DE', 'US', 'CA', 'GB', 'AU', 'IL', 'PL', 'FI', 'NL', 'DK', 'IT'])

export const searchAvailablePhoneNumbers = {
    country_code: supportedPurchaseCountryCodes,
    contains: z.union([z.literal(''), z.string().regex(/^\d{1,10}$/, 'Use at most 10 numeric digits')]).optional(),
}
...
export const sendWhatsAppTemplate = {
    ...
    recipient_phone: z.string().regex(/^\+[1-9]\d{1,14}$/, 'Phone number must be in E.164 format (e.g. +1234567890)'),
