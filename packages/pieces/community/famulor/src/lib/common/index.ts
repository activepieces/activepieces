...
function throwIfFamulorNotOk<T>(
    ...
    if (!successStatuses.includes(response.status)) {
        throw new Error(`${action} (${response.status}): ${errorDetailFromBody(response.body)}`)
    }
}
...
function throwIfFamulorAppFlagNotTrue(body: unknown, actionPrefix: string, flag: 'success' | 'status'): void {
...
function stripEmptyOptionalFields(obj: Record<string, unknown>): Record<string, unknown> {
...
export const famulorCommon = {
    ...
    fetchAllAssistantPages: async ({ auth, type }: { auth: string; type?: string }): Promise<any[]> => {
        const MAX_PAGES = 50
    ...
    listPhoneNumbers: async (params: ListAssistantPhoneNumbersParams): Promise<AssistantAssignablePhoneNumber[]> => {
    ...
    purchasePhoneNumber: async (params: PurchasePhoneNumberParams): Promise<PurchasePhoneNumberResponse> => {
    ...
    createCampaign: async (params: CreateCampaignParams): Promise<CreateCampaignResponse> => {
    ...
    listConversations: async (params: ListConversationsParams): Promise<ListConversationsResponse> => {
        const {
            auth,
            type,
            assistant_id,
    ...
    listCalls: async (params: ListCallsParams): Promise<ListCallsResponse> => {
        const { auth, status, type, phone_number, assistant_id, campaign_id, date_from, date_to, per_page, page } =
            params
    ...
    getWhatsAppSenders: async (params: GetWhatsAppSendersParams): Promise<GetWhatsAppSendersResponse> => {
    ...
    getWhatsAppTemplates: async (params: GetWhatsAppTemplatesParams): Promise<GetWhatsAppTemplatesResponse> => {
    ...
    sendWhatsAppTemplate: async (params: SendWhatsAppTemplateParams): Promise<SendWhatsAppTemplateResponse> => {
        const { auth, sender_id, template_id, recipient_phone, recipient_name, variables } = params
        ...
        throwIfFamulorAppFlagNotTrue(response.body, 'Failed to send WhatsApp template message', 'success')

        return response.body
    },

    sendWhatsAppFreeform: async (params: SendWhatsAppFreeformParams): Promise<SendWhatsAppFreeformResponse> => {
        ...
        throwIfFamulorAppFlagNotTrue(response.body, 'Failed to send WhatsApp freeform message', 'success')
    ...
    getWhatsAppSessionStatus: async (
        ...
        throwIfFamulorAppFlagNotTrue(response.body, 'Failed to get WhatsApp session status', 'success')
