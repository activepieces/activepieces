...
const campaignPhoneNumbersMultiSelect = () =>
    Property.MultiSelectDropdown<number, false, typeof famulorAuth>({
        ...
        description: 'Optional. Caller IDs for this campaign (distinct phone number IDs).',
        required: false,
        refreshers: ['auth'],
        options: async ({ auth }) => {
            if (!auth) {
...
export const createCampaign = () => ({
    ...
    timezone: Property.ShortText({
        ...
        description: 'IANA timezone (e.g. Europe/Berlin, America/New_York). Leave empty for account default.',
        required: false,
    }),
    ...
    allowed_days: Property.StaticMultiSelectDropdown({
        ...
        description: 'Leave empty to use API default (all days). Otherwise restrict to selected days.',
        required: false,
        options: {
            options: [
    ...
    goal_completion_variable: Property.ShortText({
        ...
        description: 'Name of a boolean variable from the assistant post-call schema (max 255 characters)',
        required: false,
    }),
...
export const updateLead = () => ({
    ...
    status: Property.StaticDropdown({
        displayName: 'Status',
        description: 'Lead status: created, completed, or reached-max-retries (leave unselected to keep unchanged)',
        required: false,
        options: {
            options: [
...
export const searchAvailablePhoneNumbers = () => ({
    country_code: Property.StaticDropdown({
        ...
        description: 'ISO 3166-1 alpha-2 country code to search in (supported regions per Famulor docs).',
        required: true,
        defaultValue: 'DE',
        options: {
            options: [
    ...
    contains: Property.ShortText({
        ...
        description: 'Optional. Only return numbers that contain these digits (numeric only, max 10 digits).',
        required: false,
    }),
})
...
export const generateAiReply = () => ({
    assistant_id: assistantIdDropdown('Assistant', 'Select the assistant to generate a response with'),
...
export const createConversation = () => ({
    assistant_id: assistantUuidProperty('Assistant UUID', 'The UUID of the assistant to start the conversation with'),
...
export const getConversation = () => ({
    uuid: conversationUuidProperty('Conversation UUID', 'The unique UUID identifier of the conversation to retrieve'),
})

export const sendMessage = () => ({
    uuid: conversationUuidProperty('Conversation UUID', 'The unique UUID identifier of the conversation'),
    message: Property.LongText({
        displayName: 'Message',
        description:
...
export const deleteCall = () => ({
    call_id: Property.Number({
        displayName: 'Call ID',
        description: 'Numeric ID of the call to delete permanently (transcript, recording, and metadata are removed)',
        required: true,
    }),
})
...
export const getWhatsAppTemplates = () => ({
    sender_id: Property.Number({
        ...
        description: 'WhatsApp sender numeric ID from Get WhatsApp Senders (the sender `id` field)',
        required: true,
    }),
    status: Property.StaticDropdown({
        displayName: 'Template approval status',
        description: 'Approved templates only (default API behavior) or all templates including pending/rejected',
        required: false,
...
export const sendWhatsAppTemplate = () => ({
    ...
    recipient_phone: phoneNumberProperty('Recipient phone', 'Recipient phone number', true),
...
export const getWhatsAppSessionStatus = () => ({
    ...
    recipient_phone: phoneNumberProperty('Recipient phone', 'Recipient to check the 24-hour window for', true),
})
