...
import type { Message, MessageListParams, ToolCallMessage } from '../common/types'

const polling: Polling<AppConnectionValueForAuthProperty<typeof lettaAuth>, { agentId: string }> = {
            ...
            if (message.message_type === 'tool_call_message') {
                ...
                if (toolCallMessage.tool_calls) {
                    if (Array.isArray(toolCallMessage.tool_calls)) {
                        hasSendMessage = toolCallMessage.tool_calls.some((tc) => tc.name === 'send_message')
                    } else {
