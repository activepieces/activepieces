...
import type { AgentListParams, AgentState } from '../common/types'

const polling: Polling<AppConnectionValueForAuthProperty<typeof lettaAuth>, Record<string, never>> = {
    ...
    items: async ({ auth, lastFetchEpochMS }) => {
        ...
        return agents
            ...
            .map((agent) => ({
                epochMilliSeconds: agent.created_at ? Date.parse(agent.created_at) : Date.now(),
                data: agent,
            }))
    },
}
