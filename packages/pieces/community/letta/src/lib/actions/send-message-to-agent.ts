...
import type { LettaResponse, MessageCreateParamsNonStreaming } from '../common/types'
...
export const sendMessageToAgent = createAction({
    ...
    async run(context) {
        const { agentId, input, maxSteps } = context.propsValue
        ...
        const response: LettaResponse = await client.agents.messages.create(agentId, body)
