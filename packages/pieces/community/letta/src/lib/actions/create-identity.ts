...
import type { Identity, IdentityCreateParams, IdentityProperty } from '../common/types'
...
export const createIdentity = createAction({
    ...
    async run(context) {
        const { identifierKey, name, identityType, projectId, properties } = context.propsValue
                ...
                switch (prop.type) {
                    ...
                    case 'json':
                        try {
                            ...
                            throw new Error(
                                `Invalid JSON in property "${prop.key}": ${e instanceof Error ? e.message : 'Unknown error'}`,
                            )
                        }
                        break
                    default:
