...
import type { TallyForm, TallyFormsResponse, TallySubmissionsApiResponse, TallyWebhookResponse } from './types'
...
async function deleteWebhook({ apiKey, webhookId }: { apiKey: string; webhookId: string }): Promise<void> {
...
async function makeApiCall<T>({ method, path, apiKey, body, queryParams }: MakeApiCallParams): Promise<T> {
