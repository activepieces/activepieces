...
import type { TallyField, TallyQuestionField, TallyQuestionOption, TallyWebhookPayload } from '../common/types'
...
function buildMatrixObject({
    ...
    for (const [rowId, colIds] of Object.entries(value)) {
        result[rowTextById[rowId] ?? rowId] = Array.isArray(colIds) ? colIds.map((id) => colTextById[id] ?? id) : colIds
    }
