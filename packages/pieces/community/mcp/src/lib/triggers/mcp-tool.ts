...
export const mcpTool = createTrigger({
    ...
    props: {
        ...
        inputSchema: Property.Array({
            ...
            description:
                'Define the input parameters that this tool accepts. Parameters will be shown to users when calling the tool.',
            required: false,
            defaultValue: [
                {
                    name: '',
    ...
    async run(context) {
        ...
        const isTriggerPayloadEnvelope =
            typeof body === 'object' &&
            body !== null &&
            !Array.isArray(body) &&
        ...
        const payload = isTriggerPayloadEnvelope ? (body as Record<string, unknown>) : rawPayload
