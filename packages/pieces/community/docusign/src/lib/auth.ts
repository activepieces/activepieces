    ...
    validate: async ({ auth, server }) => {
        try {
            ...
            if (
                ...
                const oAuthBasePath = auth.environment === 'demo' ? 'account-d.docusign.com' : 'account.docusign.com'
                ...
                const consentUrl =
                    ...
                    auth.clientId +
                    '&redirect_uri=' +
                    encodeURIComponent(`${server.publicUrl.replace('/api', '')}/redirect`)
                return {
                    valid: false,
                    error: 'Consent is required, please visit this URL and grant consent: ' + consentUrl,
                }
            }
            return {
                valid: false,
