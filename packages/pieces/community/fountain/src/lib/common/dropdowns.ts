...
import { AppConnectionValueForAuthProperty, DropdownOption, PiecePropValueSchema } from '@activepieces/pieces-framework'
import { fountainAuth } from '../auth'
import { getApiUrl, getAuthHeaders } from './auth'

export async function getFunnelsDropdown(
...
export async function getLocationsDropdown(
    auth: AppConnectionValueForAuthProperty<typeof fountainAuth>,
): Promise<DropdownOption<string>[]> {
    try {
        const response = await httpClient.sendRequest({
