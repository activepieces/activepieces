...
import { createAction, DynamicPropsValue, InputPropertyMap, Property } from '@activepieces/pieces-framework'
import { firecrawlAuth } from '../auth'
import {
    downloadAndSaveCrawlScreenshots,
    FIRECRAWL_API_BASE_URL,
    forJsonOutputFormat,
    forScreenshotOutputFormat,
...
function webhookConfig(useWebhook: boolean, webhookProperties: any): any {
    ...
    if (
        webhookProperties['webhookEvents'] &&
        Array.isArray(webhookProperties['webhookEvents']) &&
        webhookProperties['webhookEvents'].length > 0
    ) {
            ...
            props: async (propsValue): Promise<InputPropertyMap> => {
                ...
                const map: InputPropertyMap = {
                    webhookUrl: Property.ShortText({
                        ...
                        description:
                            'The URL to send the webhook to. This will trigger for crawl started (crawl.started), every page crawled (crawl.page) and when the crawl is completed (crawl.completed or crawl.failed).',
                        required: true,
                    }),
