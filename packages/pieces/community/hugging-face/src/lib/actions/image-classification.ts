...
export const imageClassification = createAction({
            ...
            options: async ({ classificationMode, useCase }) => {
                ...
                if (useCase === 'search') {
                    try {
                        ...
                        return {
                            disabled: false,
                            options: getModelsByUseCase(classificationMode as string, 'general'),
                        }
                    }
                }
                ...
                return {
                    disabled: false,
                    options: getModelsByUseCase(classificationMode as string, useCase as string),
                }
            },
        }),
    ...
    async run(context) {
        ...
        const actualImageUrl = Array.isArray(imageUrl) ? imageUrl[0] || '' : imageUrl || ''

        if (classificationMode === 'zero_shot' && (!customCategories || customCategories.length === 0)) {
        ...
        try {
            let classificationResults: ImageClassificationOutput | ZeroShotImageClassificationOutput
            ...
            if (classificationMode === 'zero_shot') {
                ...
                classificationResults = await hf.zeroShotImageClassification(zeroShotArgs, {
                    retry_on_error: true,
                })
            } else {
            ...
            const categoryDistribution = enhancedResults.reduce(
                (acc, result) => {
            ...
            const analytics = {
                ...
                qualityMetrics: {
                    ...
                    lowConfidenceResults: enhancedResults.filter((r) => r.isLowConfidence).length,
                },
            ...
            return {
                ...
                businessInsights: {
                    useCase: getUseCaseDescription(useCase as string, classificationMode as string),
                    classificationTips: getClassificationTips(
                        enhancedResults,
                        classificationMode as string,
                        useCase as string,
                    ),
                    nextSteps: getNextSteps(useCase as string, topCategory, enhancedResults[0]?.confidence || 0),
                },
...
function getRecommendedAction(category: string, confidence: number, useCase: string): string {
    if (confidence > 0.8) {
        if (useCase === 'moderation') {
            return category.toLowerCase().includes('nsfw') || category.toLowerCase().includes('inappropriate')
...
function getUseCaseDescription(useCase: string, mode: string): string {
    ...
    return descriptions[useCase as keyof typeof descriptions] || 'AI-powered image classification'
}
...
function getClassificationTips(
    ...
    if (results.length === 0) {
        ...
        if (useCase === 'moderation' && results.some((r) => r.label.toLowerCase().includes('nsfw'))) {
...
function getNextSteps(useCase: string, topCategory: string, confidence: number): string[] {
    const steps: string[] = []
