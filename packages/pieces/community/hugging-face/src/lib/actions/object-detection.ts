...
export const objectDetection = createAction({
    ...
    async run(context) {
        const { useCase, model, image, confidenceThreshold, maxDetections, filterSettings, outputFormat } =
            context.propsValue
        ...
        try {
            ...
            const detectionResults: ObjectDetectionOutput = await hf.objectDetection(args, {
                retry_on_error: true,
            })
            ...
            const labelCounts = limitedResults.reduce(
                (acc, detection) => {
            ...
            const businessSummary = {
                ...
                mostFrequentObject: Object.entries(labelCounts).sort(([, a], [, b]) => b - a)[0]?.[0],
                averageConfidence: Math.round(avgConfidence * 100),
                highConfidenceDetections: limitedResults.filter((d) => d.score > 0.8).length,
            }
            ...
            const enhancedDetections = limitedResults.map((detection, index) => {
                const boxArea = (detection.box.xmax - detection.box.xmin) * (detection.box.ymax - detection.box.ymin)
                const boxCenter = {
                ...
                return {
                    ...
                    metadata: {
                        ...
                        width: Math.round((detection.box.xmax - detection.box.xmin) * 1000) / 1000,
                        height: Math.round((detection.box.ymax - detection.box.ymin) * 1000) / 1000,
                    },
                }
            })
            ...
            return {
                ...
                businessInsights: {
                    ...
                    detectionTips: getDetectionTips(enhancedDetections, actualThreshold, useCase as string),
                    nextSteps: getNextSteps(useCase as string, limitedResults.length, Object.keys(labelCounts).length),
                },
...
function getUseCaseDescription(useCase: string): string {
    ...
    return descriptions[useCase as keyof typeof descriptions] || 'AI-powered object detection'
}
...
function getDetectionTips(
    ...
    if (useCase === 'documents' && detections.some((d) => d.label.includes('table'))) {
...
function getNextSteps(useCase: string, detectionCount: number, uniqueLabels: number): string[] {
    const steps: string[] = []
