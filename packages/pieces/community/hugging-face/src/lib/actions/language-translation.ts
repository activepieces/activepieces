...
import { InferenceClient, TranslationArgs } from '@huggingface/inference'
...
export const languageTranslation = createAction({
    ...
    description: 'Translate text between languages using specialized Hugging Face translation models',
    props: {
        model: Property.Dropdown({
            auth: huggingFaceAuth,
            displayName: 'Translation Model',
            description: 'Select a translation model or search from 7000+ available models',
            required: true,
            refreshers: [],
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    return {
                        disabled: false,
                        options: allOptions.filter((option) => option.value !== '__separator__'),
                    }
                } catch (error) {
        ...
        maxLength: Property.Number({
            ...
            description: 'Maximum length of the translated text (leave empty for default)',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (!modelToUse) {
            throw new Error('Please select a model from the dropdown or enter a custom model ID')
        }
