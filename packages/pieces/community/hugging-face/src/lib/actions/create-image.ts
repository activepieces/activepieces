...
function getUseCaseDescription(useCase: string): string {
    ...
    return descriptions[useCase as keyof typeof descriptions] || 'AI-powered image generation'
}

function getQualityTips(prompt: string, negativePrompt: string | undefined, steps: number): string[] {
    const tips: string[] = []
...
function calculateEstimatedCost(model: string, width: number, height: number, steps: number): string {
