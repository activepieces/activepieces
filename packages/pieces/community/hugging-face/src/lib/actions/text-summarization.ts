...
import { InferenceClient, SummarizationArgs } from '@huggingface/inference'
...
export const textSummarization = createAction({
    ...
    props: {
            ...
            options: async ({ auth, contentType }) => {
                ...
                const getModelsByType = (type: string) => {
                    switch (type) {
                        case 'news':
                            return [
                                {
                                    label: '🔥 Facebook BART-CNN (4.5M downloads) - News optimized',
                                ...
                                {
                                    label: '📝 Google Pegasus-XSum (118K downloads) - Abstractive',
                        ...
                        case 'email':
                            return [
                                ...
                                {
                                    label: '📧 Falconsai Text Summary (30K downloads) - General text',
                                    value: 'Falconsai/text_summarization',
                                },
                                {
                                    label: '⚡ Google T5-Small (2.7M downloads) - Fast & versatile',
                        ...
                        case 'meeting':
                            return [
                                {
                                    label: '🎯 Meeting Summary (29K downloads) - Meeting optimized',
                                    value: 'knkarthick/MEETING_SUMMARY',
                                },
                                {
                                    label: '💬 BART SAMSum (104K downloads) - Conversation focused',
                        ...
                        case 'medical':
                            return [
                                {
                                    label: '🏥 Medical Summarization (45K downloads) - Medical optimized',
                        ...
                        case 'multilingual':
                            return [
                                ...
                                {
                                    label: '⚡ Google T5-Small (2.7M downloads) - Multilingual capable',
                                    value: 'google-t5/t5-small',
                                },
                                {
                ...
                try {
                    ...
                    const summarizationModels = models
                        ...
                        .map((model) => ({
                            label: `${model.id} (${model.downloads?.toLocaleString() || 0} downloads)`,
                            value: model.id,
                        }))
                        .slice(0, 50)
                    ...
                    const allOptions = [
                        ...
                        ...summarizationModels.filter(
                            (model) => !popularModels.some((popular) => popular.value === model.value),
                        ),
                    ]
        ...
        text: Property.LongText({
            ...
            description: 'The long text content you want to summarize (most models work best with 512-1024 tokens)',
            required: true,
        }),
    ...
    async run(context) {
        ...
        const parameters: {
            ...
            truncation?: 'do_not_truncate' | 'longest_first' | 'only_first' | 'only_second'
            generate_parameters?: {
                min_length?: number
                max_length?: number
                do_sample?: boolean
        ...
        return {
            ...
            businessInsights: {
                readingTimeSaved: `${Math.max(1, Math.round((originalWords - summaryWords) / 200))} minutes`, // Assume 200 words per minute
...
function getSuggestedUseCase(contentType: string): string {
    const useCases = {
        ...
        meeting: 'Great for meeting minutes, call summaries, and action item extraction',
        ...
        multilingual: 'Perfect for global content management and translation workflows',
        search: 'Custom model selection for specialized summarization needs',
    }
    return useCases[contentType as keyof typeof useCases] || 'General purpose text summarization'
}
...
function getQualityTips(originalWords: number, summaryWords: number): string[] {
    ...
    if (originalWords > 2000) {
        tips.push('📝 Very long text - consider breaking into sections for better results')
    }

    if (summaryWords / originalWords > 0.7) {
        tips.push('💡 Summary is quite long - try Brief mode for more concise results')
    }

    if (summaryWords / originalWords < 0.1) {
        tips.push('🔍 Very aggressive summarization - try Medium or Detailed for more context')
    }
