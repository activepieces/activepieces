...
import { InferenceClient, TextClassificationArgs, ZeroShotClassificationArgs } from '@huggingface/inference'
    ...
    async run(context) {
        ...
        switch (classificationMode) {
            case 'zero-shot':
                ...
                break
            case 'pretrained':
                modelToUse = pretrainedModel || 'cardiffnlp/twitter-roberta-base-sentiment-latest'
                break
            case 'search':
                if (!searchModel) {
        ...
        if (isZeroShot) {
            ...
            const classifications = Array.isArray(zeroShotResult) ? zeroShotResult : [zeroShotResult]
            ...
            return {
                ...
                highConfidencePredictions: sortedResults.filter((r: { score: number }) => r.score > 0.5),
            ...
            if (
                functionToApply &&
                (functionToApply === 'softmax' || functionToApply === 'sigmoid' || functionToApply === 'none')
            ) {
            ...
            return {
                ...
                confidenceThreshold: 0.5,
                highConfidencePredictions: results.filter((r: { score: number }) => r.score > 0.5),
