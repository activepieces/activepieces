...
export const chatCompletion = createAction({
    ...
    async run(context) {
        ...
        const systemMessage = getSystemPrompt(template, systemPrompt, useCase as string)
        if (systemMessage) {
        ...
        return {
            ...
            conversation: {
                ...
                fullConversation: [...messages, { role: 'assistant', content: assistantMessage }],
            },
            metadata: {
                model: model,
                useCase: useCase,
            ...
            metrics: {
                ...
                estimatedCost: calculateEstimatedCost(model, tokenUsage?.total_tokens || 0),
            },
            businessInsights: {
...
function getSystemPrompt(template: string | undefined, customPrompt: string | undefined, useCase: string): string {
    ...
    return defaultPrompts[useCase as keyof typeof defaultPrompts] || 'You are a helpful AI assistant.'
}
...
function getUseCaseDescription(useCase: string): string {
    ...
    return descriptions[useCase as keyof typeof descriptions] || 'General purpose chat assistance'
}
...
function getNextSteps(conversationMode: string, finishReason: string): string[] {
    const steps: string[] = []
