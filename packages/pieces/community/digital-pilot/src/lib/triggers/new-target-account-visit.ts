...
export const newTargetAccountVisitTrigger = createTrigger({
    ...
    sampleData: {
        ...
        page_views: 8,
        time_on_site: 320,
        pages_visited: ['/products', '/pricing', '/contact'],
    },
    ...
    async onEnable(context) {
        ...
        const webhook = await client.createWebhook(context.propsValue.tagId, webhookUrl, 'target_accounts')
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await client.deleteWebhook(context.propsValue.tagId, undefined, webhookId)
        }
    },
