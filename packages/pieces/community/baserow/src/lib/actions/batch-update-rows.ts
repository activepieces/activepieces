...
export const batchUpdateRowsAction = createAction({
    ...
    props: {
        ...
        rows: Property.Json({
            displayName: 'Rows',
            description: 'A JSON array of objects. Each object must include an "id" field and the fields to update.',
            required: true,
    ...
    async run(context) {
        ...
        return await client.batchUpdateRows(table_id, rows as Record<string, unknown>[])
    },
})
