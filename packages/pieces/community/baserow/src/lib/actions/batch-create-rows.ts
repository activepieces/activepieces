...
export const batchCreateRowsAction = createAction({
    ...
    description: 'Creates multiple rows in a single request. Accepts up to 200 rows.',
    auth: baserowAuth,
    props: {
        table_id: baserowCommon.tableId(),
        rows: Property.Json({
            displayName: 'Rows',
            description: 'A JSON array of objects. Each object represents a row with field names as keys.',
            required: true,
    ...
    async run(context) {
        ...
        return await client.batchCreateRows(table_id, rows as Record<string, unknown>[])
    },
})
