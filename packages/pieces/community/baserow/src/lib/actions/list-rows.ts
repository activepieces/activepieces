...
export const listRowsAction = createAction({
    ...
    props: {
        ...
        filter_type: Property.StaticDropdown({
            ...
            description: 'When AND is selected, all filters must match. When OR is selected, any filter can match.',
            required: false,
            options: {
                disabled: false,
                options: [
    ...
    async run(context) {
        const { table_id, page, limit, search, order_by, filter_type, filters } = context.propsValue
        ...
        if (filters && filters.length > 0) {
            const parsedFilters = filters.map((f) => {
                const filter = typeof f === 'string' ? JSON.parse(f) : (f as Record<string, unknown>)
                return {
        ...
        return await client.listRows(table_id!, page, limit, search, order_by, undefined, advancedFilters)
    },
})
