import { createAction, DropdownState, Property } from '@activepieces/pieces-framework'
...
export const aggregateFieldAction = createAction({
    ...
    props: {
        ...
        aggregation_type: Property.StaticDropdown({
            ...
            description: 'Sum, average, min, max, std_dev and variance only work on number fields.',
            required: true,
            options: {
                disabled: false,
                options: [
    ...
    async run(context) {
        ...
        const raw = (await client.aggregateField(view_id!, field_id!, aggregation_type!)) as { value: unknown }
