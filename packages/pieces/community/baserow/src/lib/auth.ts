...
function isDatabaseToken(auth: BaserowAuthValue): auth is BaserowAuthValue & {
    type: typeof AppConnectionType.CUSTOM_AUTH
    props: { apiUrl: string; token: string }
} {
    return auth.type === AppConnectionType.CUSTOM_AUTH && 'token' in auth.props
}
...
export type BaserowJwtAuthValue = AppConnectionValueForAuthProperty<typeof baserowJwtAuth>
