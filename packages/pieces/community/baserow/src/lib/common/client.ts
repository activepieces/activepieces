...
function emptyValueFilter(accessor: (key: string) => unknown): (key: string) => boolean {
    return (key: string) => {
...
export class BaserowClient {
    ...
    static async getJwtToken(baseUrl: string, email: string, password: string): Promise<string> {
    ...
    async listTables(): Promise<BaserowTable[]> {
        return await this.makeRequest<BaserowTable[]>(HttpMethod.GET, `/database/tables/all-tables/`)
    }
    async listTableFields(table_id: number): Promise<BaserowField[]> {
        return await this.makeRequest<BaserowField[]>(HttpMethod.GET, `/database/fields/table/${table_id}/`)
    }
    ...
    async updateRow(table_id: number, row_id: number, request: Record<string, unknown>) {
        return await this.makeRequest(
            HttpMethod.PATCH,
    ...
    async deleteRow(table_id: number, row_id: number) {
        return await this.makeRequest(HttpMethod.DELETE, `/database/rows/table/${table_id}/${row_id}/`)
    }
    async getRow(table_id: number, row_id: number) {
        return await this.makeRequest(HttpMethod.GET, `/database/rows/table/${table_id}/${row_id}/`, {
            user_field_names: 'true',
        })
    }
    async listRows(
        ...
        return await this.makeRequest(HttpMethod.GET, `/database/rows/table/${table_id}/`, query)
    }
    ...
    async batchDeleteRows(table_id: number, ids: number[]) {
        return await this.makeRequest(HttpMethod.POST, `/database/rows/table/${table_id}/batch-delete/`, undefined, {
            items: ids,
        })
    }
    async listViews(table_id: number): Promise<{ id: number; name: string; type: string }[]> {
        return await this.makeRequest(HttpMethod.GET, `/database/views/table/${table_id}/`, { type: 'grid' })
    }
    async aggregateField(view_id: number, field_id: number, aggregation_type: string) {
        return await this.makeRequest(HttpMethod.GET, `/database/views/grid/${view_id}/aggregation/${field_id}/`, {
            type: aggregation_type,
        })
    }
    async createWebhook(tableId: number, url: string, events: string[], name: string): Promise<{ id: number }> {
