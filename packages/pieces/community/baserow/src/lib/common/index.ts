...
export function makeClient(auth: PiecePropValueSchema<typeof baserowAuth>): BaserowClient {
  ...
  tableFields: (required = true) =>
    Property.DynamicProperties({
      ...
      props: async ({ auth, table_id }) => {
        ...
        try {
          const client = makeClient(auth as PiecePropValueSchema<typeof baserowAuth>)
          const tableFields = await client.listTableFields(table_id as unknown as number)
                ...
                case BaserowFieldType.DATE:
                  fields[field.name] = Property.DateTime({
                    ...
                    description: `Enter date in ${field.date_format} format ${
                      field.date_include_time ? 'and time in ' + field.date_time_format + ' hour format' : ''
                    }.`,
                  })
                  break
