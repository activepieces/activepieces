...
import { BaserowAuthValue, BaserowJwtAuthValue, baserowAuth, isDatabaseTokenAuth } from '../auth'
import { BaserowClient } from './client'
import { BaserowFieldType } from './constants'

export async function makeClient(auth: BaserowAuthValue): Promise<BaserowClient> {
    if (isDatabaseTokenAuth(auth)) {
        return new BaserowClient(auth.props.apiUrl, `Token ${auth.props.token}`)
    }
    const jwt = await BaserowClient.getJwtToken(auth.props.apiUrl, auth.props.email, auth.props.password)
    return new BaserowClient(auth.props.apiUrl, `JWT ${jwt}`)
}

export async function makeJwtClient(auth: BaserowJwtAuthValue): Promise<BaserowClient> {
    const jwt = await BaserowClient.getJwtToken(auth.props.apiUrl, auth.props.email, auth.props.password)
    return new BaserowClient(auth.props.apiUrl, `JWT ${jwt}`)
}
    ...
    rowId: (required = true) =>
        Property.Dropdown({
            ...
            options: async ({ auth, table_id }): Promise<DropdownState<number>> => {
                ...
                const client = await makeClient(auth)
                const response = (await client.listRows(table_id, undefined, 200)) as {
            ...
            props: async ({ auth, table_id }) => {
                ...
                try {
                    ...
                    for (const field of tableFields) {
                        if (!field.read_only && ![BaserowFieldType.FILE].includes(field.type)) {
                            switch (field.type) {
