...
// Default to the managed cloud; override this constant to point at a self-hosted instance.
export const CRW_API_BASE_URL = 'https://fastcrw.com/api/v1';
...
export async function polling(
  ...
  for (let attempt = 1; attempt <= maxAttempts ; attempt++) {
    ...
    const statusResponse = await httpClient.sendRequest({
      method: HttpMethod.GET,
      url: `${CRW_API_BASE_URL}/${actionType}/${jobId}`,
      headers: {
