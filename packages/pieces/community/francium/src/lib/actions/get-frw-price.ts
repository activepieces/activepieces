...
export const getFrwPrice = createAction({
  name: 'get_frw_price',
  displayName: 'Get FRW Token Price',
  description: 'Fetches the current FRW token price and market data from CoinGecko.',
  props: {},
  async run() {
    const response = await httpClient.sendRequest<CoinGeckoResponse>({
      method: HttpMethod.GET,
      url: 'https://api.coingecko.com/api/v3/coins/francium',
      queryParams: {
        localization: 'false',
        tickers: 'false',
