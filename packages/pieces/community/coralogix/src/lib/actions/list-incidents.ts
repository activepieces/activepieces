...
export const listIncidents = createAction({
    ...
    async run({ auth, propsValue }) {
        const { status, severity, applicationName, subsystemName, pageSize } = propsValue

        const params: string[] = []

        if (status && (status as string[]).length > 0) {
            ;(status as string[]).forEach((s) => params.push(`filter.status=${encodeURIComponent(s)}`))
        }
        if (severity && (severity as string[]).length > 0) {
            ;(severity as string[]).forEach((s) => params.push(`filter.severity=${encodeURIComponent(s)}`))
        }
        ...
        if (subsystemName && (subsystemName as string[]).length > 0) {
            ;(subsystemName as string[]).forEach((s) => params.push(`filter.subsystemName=${encodeURIComponent(s)}`))
        }
