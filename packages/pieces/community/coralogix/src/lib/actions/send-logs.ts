...
export const sendLogs = createAction({
    ...
    props: {
        ...
        timestamp: Property.Number({
            ...
            description: 'UTC milliseconds since 1970. Leave empty to use ingestion time.',
            required: false,
        }),
        hiResTimestamp: Property.ShortText({
            displayName: 'High Resolution Timestamp',
            description: 'UTC nanoseconds since 1970 as a string. Optional alternative to timestamp.',
            required: false,
        }),
    },
    async run({ auth, propsValue }) {
        ...
        const response = await makeRequest(auth, 'ingestion', HttpMethod.POST, `/logs/v1/singles`, [logRecord])
