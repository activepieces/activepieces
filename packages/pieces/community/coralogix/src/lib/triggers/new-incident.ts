...
export const newIncident = createTrigger({
    ...
    async run(context) {
        ...
        const params: string[] = ['filter.status=INCIDENT_STATUS_TRIGGERED', 'pagination.pageSize=100']
        if (severityFilter && severityFilter.length > 0) {
            severityFilter.forEach((s) => params.push(`filter.severity=${encodeURIComponent(s)}`))
        }
