...
export const newAlertEvent = createTrigger({
    ...
    description: 'Triggers when Coralogix sends an alert-event webhook payload to this flow.',
    props: {
...
6. Save the webhook, then trigger a test alert to verify events are received.
    ...
    sampleData: {
        ...
        fields: [
            ...
            {
                key: 'log',
                value: ['127.0.0.1 - - [17/May/2015:08:05:12 +0000] "GET / HTTP/1.1" 200 612 "-" "curl/7.64.1"'],
            },
            {
                key: 'message',
                value: ['test'],
