...
const selfHostedAuth = PieceAuth.CustomAuth({
    ...
    description: 'Connect to your own Umami instance using the same credentials you use to log in to your dashboard.',
    required: true,
    props: {
    ...
    validate: async ({ auth }) => {
        try {
            ...
            return {
                valid: false,
                error: 'Connection failed. Check that the URL is reachable and your username/password are correct.',
            }
        }
    },
})
...
const cloudAuth = PieceAuth.SecretText({
    ...
    description: 'Connect to Umami Cloud using an API key. Create one in **Settings → API Keys**.',
    required: true,
    validate: async ({ auth }) => {
        try {
            ...
            return {
                valid: false,
                error: 'Invalid API key. Check your key in Umami Cloud → Settings → API Keys.',
            }
        }
    },
})
...
export type UmamiAuthValue = AppConnectionValueForAuthProperty<typeof umamiAuth>
...
export async function getAuthHeaders(auth: UmamiAuthValue): Promise<Record<string, string>> {
    if (auth.type === AppConnectionType.SECRET_TEXT) {
