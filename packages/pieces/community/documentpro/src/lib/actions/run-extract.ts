...
export const runExtract = createAction({
    ...
    props: {
        ...
        use_ocr: Property.Checkbox({
            displayName: 'Use OCR',
            description: 'Enable OCR processing (required for gpt-3.5-turbo or OCR-related parameters)',
            required: false,
            defaultValue: false,
        }),
        ...
        start_regex: Property.ShortText({
            ...
            description: 'Regex pattern for where parsing should begin (requires use_ocr=true)',
            required: false,
        }),
        end_regex: Property.ShortText({
            displayName: 'End Regex',
            description: 'Regex pattern for where parsing should end (requires use_ocr=true)',
            required: false,
        }),
        split_regex: Property.ShortText({
            displayName: 'Split Regex',
            description: 'Regex pattern to split the document into sections (requires use_ocr=true)',
            required: false,
        }),
        use_all_matches: Property.Checkbox({
            displayName: 'Use All Matches',
            description: 'Use all regex matches instead of just the first (requires use_ocr=true)',
            required: false,
            defaultValue: false,
        }),
    },
    async run(context) {
        ...
        if (context.propsValue.chunk_by_pages) {
            params.append('chunk_by_pages', context.propsValue.chunk_by_pages.toString())
        }
        if (context.propsValue.rolling_window) {
            params.append('rolling_window', context.propsValue.rolling_window.toString())
        }
