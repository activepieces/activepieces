...
export const findSubscriber = createAction({
    ...
    props: {
        ...
        fields: Property.ShortText({
            ...
            description:
                'Comma-separated list of fields to return (e.g., id,email,firstName,lastName). Leave empty for all fields.',
            required: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (searchType === 'identifier') {
            if (!id && !email && !cellphone && !externalId) {
                throw new Error(
                    'For identifier search, please provide at least one: Contact ID, Email, Phone, or External ID',
                )
            }
