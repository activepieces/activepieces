    ...
    options: async ({ auth }) => {
        ...
        try {
            const Contacts = await makeRequest(
                auth.secret_text,
                HttpMethod.GET,
                '/Contacts?fields=id,firstName,lastName,email,cellphone',
            )
            return {
                disabled: false,
    ...
    options: async ({ auth }) => {
        ...
        try {
            const Contacts = await makeRequest(
                auth.secret_text,
                HttpMethod.GET,
                '/Contacts?fields=id,firstName,lastName,email',
            )
            const emailOptions = Contacts.filter((Contact: any) => Contact.email) // Only contacts with emails
                .map((Contact: any) => {
                    const name = `${Contact.firstName || ''} ${Contact.lastName || ''}`.trim()
