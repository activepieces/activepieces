...
const polling: Polling<
    AppConnectionValueForAuthProperty<typeof smooveAuth>,
    {
        fields?: string
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        try {
            ...
            if (fields && fields.trim()) {
                ...
                queryParams.push(
                    'fields=id,email,firstName,lastName,timestampSignup,joinSource,lastChanged,canReceiveEmails,campaignSource,ipSignup',
                )
            }
            ...
            const sortedSubscribers = newSubscribers.sort(
                (a, b) => new Date(b.timestampSignup || 0).getTime() - new Date(a.timestampSignup || 0).getTime(),
            )
...
export const newSubscriber = createTrigger({
    ...
    props: {
        fields: Property.ShortText({
            ...
            description:
                'Comma-separated fields to include (e.g., id,email,firstName,lastName,timestampSignup). Leave empty for default fields.',
            required: false,
            defaultValue:
                'id,email,firstName,lastName,timestampSignup,joinSource,campaignSource,canReceiveEmails,ipSignup',
        }),
    ...
    async test(context) {
        try {
            const response = await makeRequest(
                ...
                HttpMethod.GET,
                '/Contacts?take=1&sort=-timestampSignup',
            )
