...
> = {
    ...
    items: async ({ auth, propsValue }) => {
        try {
            ...
            const validItems = items.filter((item) => item.id).sort((a, b) => (b.id || 0) - (a.id || 0))
...
export const newListCreated = createTrigger({
    ...
    props: {
        fields: Property.ShortText({
            ...
            description:
                'Comma-separated list of fields to include (e.g., id,name,description,contactsCount). Leave empty for all fields.',
            required: false,
