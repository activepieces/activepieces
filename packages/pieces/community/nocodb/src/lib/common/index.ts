...
import { ColumnResponse, ColumnV3Response, GetTableResponse, GetTableV3Response } from './types'
...
export const nocodbCommon = {
    ...
    baseId: Property.Dropdown({
        ...
        options: async ({ auth, workspaceId }) => {
            ...
            try {
                const client = makeClient(auth)
                const response = await client.listBases((workspaceId as string) || undefined, auth.props.version || 3)
    ...
    tableId: Property.Dropdown({
        ...
        options: async ({ auth, baseId }) => {
            ...
            const client = makeClient(auth)
            const response = await client.listTables(baseId as string, auth.props.version || 3)
    ...
    columnId: Property.MultiSelectDropdown({
        ...
        options: async ({ auth, baseId, tableId }) => {
            ...
            const client = makeClient(auth)
            const authVersion = auth.props.version || 3
            const response =
                authVersion === 4
                    ? await client.getTableV3(baseId as unknown as string, tableId as unknown as string, authVersion)
                    : await client.getTable(baseId as unknown as string, tableId as unknown as string, authVersion)
    ...
    tableColumns: Property.DynamicProperties({
        ...
        props: async ({ auth, baseId, tableId }) => {
            ...
            const columns =
                authVersion === 4 ? (response as GetTableV3Response).fields : (response as GetTableResponse).columns

            for (const column of columns ?? []) {
                const uidt = authVersion === 4 ? (column as ColumnV3Response).type : (column as ColumnResponse).uidt
                ...
                switch (uidt) {
                    ...
                    case 'MultiSelect': {
                        ...
                        const options =
                            authVersion === 4
                                ...
                                : (column as ColumnResponse).colOptions?.options?.map((option) => {
                                      return {
                                          label: option.title,
                                          value: option.title,
                                      }
                    ...
                    case 'Date': {
                        const columnMeta =
                            authVersion === 4 ? (column as ColumnV3Response).options : (column as ColumnResponse).meta
                        fields[column.title] = Property.ShortText({
                            displayName: column.title,
                            required: false,
