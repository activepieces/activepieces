...
export const teamidDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        try {
            const teams = await makeRequest<{ id: string; name: string }[]>(auth.secret_text, HttpMethod.GET, '/teams')
            return {
                disabled: false,
...
export const tableFields = Property.DynamicProperties({
    ...
    props: async ({ auth, teamid, dbid, tid }) => {
        ...
        try {
            ...
            }>(auth as unknown as string, HttpMethod.GET, `/teams/${teamid}/databases/${dbid}/tables/${tid}`)
