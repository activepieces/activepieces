...
export const newPdf = createTrigger({
  auth: placidAuth,
  name: 'new_pdf',
  displayName: 'New PDF',
  description: 'Fires when a PDF is generated from a template',
  type: TriggerStrategy.WEBHOOK,
  props: {
    template: {
      ...templateDropdown,
      displayName: 'Template (Optional)',
      description: 'Filter events by a specific template. Leave empty to receive all PDF generation events.',
      required: false,
    },
  },
  sampleData: {
    id: 'pdf_123456789',
    status: 'finished',
    image_url: 'https://placid.app/generated/document.pdf',
  ...
  async run(context) {
    ...
    // Only trigger on completed PDF generation events
    if (payload.status === 'finished' && payload.image_url && payload.id.startsWith('pdf_')) {
