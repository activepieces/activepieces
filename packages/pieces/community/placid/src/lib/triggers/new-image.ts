...
export const newImage = createTrigger({
  auth: placidAuth,
  name: 'new_image',
  displayName: 'New Image',
  description: 'Fires when a new image is generated from a template',
  type: TriggerStrategy.WEBHOOK,
  props: {
    template: {
      ...templateDropdown,
      displayName: 'Template (Optional)',
      description: 'Filter events by a specific template. Leave empty to receive all image generation events.',
      required: false,
    },
  },
  sampleData: {
    id: 'img_123456789',
    status: 'finished',
    image_url: 'https://placid.app/generated/image.png',
  ...
  async run(context) {
    ...
    // Only trigger on completed image generation events
    if (payload.status === 'finished' && payload.image_url) {
