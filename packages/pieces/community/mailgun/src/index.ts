...
3. Copy your **Private API Key** (starts with \`key-\`)`,
    ...
    validate: async ({ auth }) => {
        const baseUrl = auth.region === 'eu' ? 'https://api.eu.mailgun.net' : 'https://api.mailgun.net'
...
export const mailgun = createPiece({
    ...
    actions: [
        ...
        createCustomApiCallAction({
            baseUrl: (auth) => {
                return auth?.props.region === 'eu' ? 'https://api.eu.mailgun.net/v3' : 'https://api.mailgun.net/v3'
            },
            auth: mailgunAuth,
