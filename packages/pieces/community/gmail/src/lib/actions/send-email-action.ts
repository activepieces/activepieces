...
import { createGoogleClient, getUserEmail, gmailAuth } from '../auth'
...
export const gmailSendEmailAction = createAction({
    ...
    async run(context) {
        ...
        const replyTo = context.propsValue['reply_to']?.filter((email) => email !== '')
        const receiver = context.propsValue['receiver']?.filter((email) => email !== '')
        ...
        const senderEmail = context.propsValue.from || (await getUserEmail(context.auth, authClient))
        if (senderEmail) {
        ...
        if (attachments && attachments.length > 0) {
            const attachmentOption: Attachment[] = attachments.map(({ file, name }) => {
        ...
        const encodedPayload = Buffer.from(mailBody).toString('base64').replace(/\+/g, '-').replace(/\//g, '_')
