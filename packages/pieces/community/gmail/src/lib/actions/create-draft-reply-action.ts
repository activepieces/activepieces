...
export const gmailCreateDraftReplyAction = createAction({
  ...
  aiMetadata: {
    description:
      'Builds a draft reply to an existing email and saves it (unsent) in the thread, optionally quoting the original message. Use this when a human should review and send the response rather than sending it automatically; requires the Gmail message ID of the email being replied to. Not idempotent: each call creates a new draft.',
    idempotent: false,
  },
