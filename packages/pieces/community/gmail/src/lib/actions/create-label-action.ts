...
const GMAIL_COLORS = [
  ...
  { label: 'Very Light Gray', textColor: '#efefef', backgroundColor: '#efefef' },
  ...
  { label: 'Very Light Green', textColor: '#c6f3de', backgroundColor: '#c6f3de' },
  { label: 'Light Blue', textColor: '#c9daf8', backgroundColor: '#c9daf8' },
  { label: 'Light Purple', textColor: '#e4d7f5', backgroundColor: '#e4d7f5' },
  { label: 'Very Light Pink', textColor: '#fcdee8', backgroundColor: '#fcdee8' },
];
...
export const gmailCreateLabelAction = createAction({
  ...
  props: {
    name: Property.ShortText({
      ...
      description: 'The name of the label to create (max 50 characters, no < > " characters)',
      required: true,
    }),
  ...
  async run(context) {
    const { name, messageListVisibility, labelListVisibility, useCustomColor, textColor, backgroundColor } = context.propsValue;
    ...
    try {
      ...
      if (useCustomColor && (!textColor || !backgroundColor)) {
        throw new Error('Both text color and background color are required when using custom colors');
      }

      const existingLabelsResponse = await GmailRequests.getLabels(context.auth);
      const existingLabels = existingLabelsResponse.body.labels || [];
      const duplicateLabel = existingLabels.find(
        (label) => label.name.toLowerCase() === name.toLowerCase() && label.type === 'user'
      );
      ...
      const labelData: {
        ...
        labelListVisibility: labelListVisibility as 'labelShow' | 'labelShowIfUnread' | 'labelHide',
      };
      ...
      const newLabel = await GmailRequests.createLabel(labelData);
      ...
      return {
        ...
        label: {
          ...
          messagesTotal: newLabel.messagesTotal || 0,
          messagesUnread: newLabel.messagesUnread || 0,
          threadsTotal: newLabel.threadsTotal || 0,
          threadsUnread: newLabel.threadsUnread || 0,
        },
      ...
      if (error.response?.status === 400) {
        ...
        if (errorMessage.includes('Invalid label name')) {
          throw new Error('Invalid label name. Please check the name format and try again');
        }
        if (errorMessage.includes('color')) {
          throw new Error('Invalid color values. Please select valid Gmail colors');
        }
        throw new Error(`Gmail API Error: ${errorMessage}`);
      }

      if (error.response?.status === 403) {
        throw new Error('Insufficient permissions to create labels. Please check your Gmail API permissions');
      }
