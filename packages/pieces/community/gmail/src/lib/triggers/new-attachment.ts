...
export const gmailNewAttachmentTrigger = createTrigger({
  ...
  props: {
    ...
    file_type: Property.ShortText({
      displayName: 'File Type Filter',
      description: 'Filter by file extension (e.g., pdf, xlsx, docx). Leave empty for all attachments.',
      required: false,
    }),
  },
  sampleData: {},
...
async function pollAttachmentMessages({
  ...
  if (props.file_type) query.push(`filename:${props.file_type}`);
  ...
  for (const message of messagesResponse.data.messages || []) {
    ...
    const convertedAttachments = await convertAttachment(parsedMailResponse.attachments, files);
