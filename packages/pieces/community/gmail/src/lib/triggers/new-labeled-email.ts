import { createTrigger, FilesService, TriggerStrategy } from '@activepieces/pieces-framework'
import { google } from 'googleapis'
import { createGoogleClient, gmailAuth } from '../auth'
                ...
                if (history.labelsAdded) {
                    ...
                    for (const messageAdded of history.messagesAdded) {
                        if (
                            ...
                            messageAdded.message.labelIds?.includes(context.propsValue.label.id)
                        ) {
                            labeledMessages.set(messageAdded.message.id, history.id?.toString() || '')
                        }
                    }
                }
            }
