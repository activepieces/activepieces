...
export const uploadDocument = createAction({
    ...
    description:
        'Upload a document or provide a webpage URL to parse. Supported formats: PDF, DOCX, Image, Txt File (maximum 10 pages per document).',
    props: {
        bucket_id: Property.ShortText({
            displayName: 'Bucket ID',
            description:
                'The ID of the bucket where the data will be stored. You can find this in your dashboard under Manage Bucket.',
            required: true,
        }),
        file: Property.File({
            displayName: 'File',
            description:
                'The file to upload. Supported formats: PDF, DOCX, Image, Txt File (maximum 10 pages per document). This will be ignored if webpage URL is provided.',
            required: false,
        }),
