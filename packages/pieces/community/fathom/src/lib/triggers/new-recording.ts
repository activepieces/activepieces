...
export const newRecording = createTrigger({
    ...
    description: 'Fires when a meeting is recorded (i.e. a new meeting recording is produced)',
    props: {
    ...
    async onEnable(context) {
        ...
        const webhookParams: CreateWebhookRequest = {
            ...
            triggeredFor: context.propsValue.triggered_for as (
                | 'my_recordings'
    ...
    async onDisable(context) {
        const webhookInfo = await context.store.get<WebhookInformation>('_new_recording_webhook')
