...
import { customerDropdown, locationDropdown, taxRateDropdown, variantDropdown } from '../common/props'
...
export const createSalesOrder = createAction({
    ...
    async run(context) {
        ...
        if (additional_order_rows) {
            const additionalRows = Array.isArray(additional_order_rows) ? additional_order_rows : []
