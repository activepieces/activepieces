...
export const createContact = createAction({
            ...
            options: async ({ auth, tagSource }) => {
                if (!auth || tagSource !== 'existing') {
                    return {
                        ...
                        placeholder:
                            tagSource === 'new'
                                ? 'Not needed when creating new tags'
                                : tagSource === 'none'
                                  ? 'Not needed when no tags selected'
    ...
    async run(context) {
        const { email, locale, dynamicContactFields, customFields, tagSource, existingTags, newTagNames } =
            context.propsValue
