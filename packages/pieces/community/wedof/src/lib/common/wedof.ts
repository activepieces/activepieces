...
export const wedofCommon = {
    ...
    subscribeWebhook: async (events: string[], webhookUrl: string, apiKey: string, name: string) => {
        const request = {
        ...
        if (response.status !== 201) {
            ...
            if (response.body && typeof response.body === 'object') {
                ...
                if (errorBody.violations && Array.isArray(errorBody.violations)) {
                    const violations = errorBody.violations.map((v: any) => `${v.propertyPath}: ${v.title}`).join(', ')
    ...
    handleWebhookSubscription: async (events: string[], context: any, name: string) => {
    ...
    controlState: Property.StaticMultiSelectDropdown({
        ...
        description: "Permet de n'obtenir que les dossiers dans l'état de contrôle considéré",
        required: false,
        options: {
            options: [
                {
