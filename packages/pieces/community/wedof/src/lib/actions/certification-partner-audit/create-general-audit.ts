...
export const createGeneralAudit = createAction({
  ...
  props: {
    ...
    suspend: Property.StaticDropdown({
      ...
      description:
        "Indique si le partenariat doit être suspendu en cas de non-conformité (ne s'applique que pour les certifications actives)",
      required: false,
      defaultValue: true,
      options: {
        disabled: false,
  ...
  async run(context) {
    ...
    return (
      await httpClient.sendRequest({
        ...
        url: wedofCommon.baseUrl + '/certifications/' + context.propsValue.certifInfo + '/partners/audits',
        body: message,
        headers: {
          'Content-Type': 'application/json',
