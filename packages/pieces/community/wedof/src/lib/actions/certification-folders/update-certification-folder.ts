...
export const updateCertificationFolder = createAction({
    ...
    description: "Met à jour certaines informations modifiables d'un dossier de certification",
    props: {
            ...
            props: async ({ fieldsToUpdate }) => {
                ...
                if (selectedFields.includes('enrollmentDate')) {
                    fields['enrollmentDate'] = Property.DateTime({
                        ...
                        description:
                            'Date au format YYYY-MM-DD - peut être modifié dans les états toRegister, registered, toTake, toControl',
                        required: false,
                    })
                }
                ...
                if (selectedFields.includes('examinationDate')) {
                    fields['examinationDate'] = Property.DateTime({
                        ...
                        description:
                            'Date au format YYYY-MM-DD - peut être modifié dans les états registered, toTake, toRetake, toControl',
                        required: false,
                    })
                }
                ...
                if (selectedFields.includes('examinationPlace')) {
                    fields['examinationPlace'] = Property.ShortText({
                        ...
                        description:
                            "Lieu de l'examen de certification - peut être modifié dans les états registered, toTake, toControl, toRetake",
                        required: false,
                    })
                }
                ...
                if (selectedFields.includes('examinationCenterZipCode')) {
                    fields['examinationCenterZipCode'] = Property.ShortText({
                        ...
                        description:
                            "Code postal du centre d'examen principal - peut être modifié dans tous les états sauf success",
                        required: false,
                    })
                }
                ...
                if (selectedFields.includes('examinationType')) {
                    fields['examinationType'] = Property.StaticDropdown({
                        ...
                        description:
                            "Type de passage de l'examen - peut être modifié dans les états registered, toTake, toControl, toRetake",
                        required: false,
                        options: {
                            disabled: false,
                            options: [
                ...
                if (selectedFields.includes('verbatim')) {
                    fields['verbatim'] = Property.ShortText({
                        ...
                        description:
                            'Information complémentaire sur la certification - peut être modifié dans tous les états sauf success',
                        required: false,
                    })
                }
                    ...
                    fields['accessModality'] = Property.StaticDropdown({
                        ...
                        options: {
                            ...
                            options: [
                                ...
                                {
                                    label: 'Formation initiale apprentissage',
                                    value: 'FORMATION_INITIALE_APPRENTISSAGE',
                                },
                                {
                                    label: 'Formation continue hors contrat de professionnalisation',
                                    value: 'FORMATION_CONTINUE_HORS_CONTRAT_DE_PROFESSIONNALISATION',
                                },
                                {
                                    label: 'Formation continue contrat de professionnalisation',
                ...
                if (selectedFields.includes('type')) {
                    fields['type'] = Property.StaticDropdown({
                        ...
                        description:
                            "Initiative à laquelle l'inscription a été réalisée - peut être modifié dans tous les états sauf success",
                        required: false,
                        options: {
                            disabled: false,
                            options: [
                ...
                if (selectedFields.includes('tags')) {
                    fields['tags'] = Property.Array({
                        displayName: 'Tags',
                        description:
                            'Liste de tags associée au dossier de certification, uniquement pour le certificateur',
                        required: false,
                    })
                }
                ...
                if (selectedFields.includes('badgeAssertion')) {
                    fields['badgeAssertion'] = Property.ShortText({
                        ...
                        description:
                            "Lien vers le badge de la certification - peut être mis à jour par le certificateur et à l'état success",
                        required: false,
                    })
                }
    ...
    async run(context) {
        ...
        selectedFields.forEach((fieldName) => {
            switch (fieldName) {
                case 'enrollmentDate':
                    message['enrollmentDate'] = enrollmentDate ? dayjs(enrollmentDate).format('YYYY-MM-DD') : null
                    break
                case 'examinationDate':
                    message['examinationDate'] = examinationDate ? dayjs(examinationDate).format('YYYY-MM-DD') : null
                    break
        ...
        return (
            await httpClient.sendRequest({
                ...
                body: message,
                url: wedofCommon.baseUrl + '/certificationFolders/' + context.propsValue['externalId'],
                headers: {
