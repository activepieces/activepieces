...
export const certificationFolderSelected = createTrigger({
    ...
    description: "Se déclenche lorsque l'événement choisi se produit sur un dossier de certification",
    props: {
    ...
    async onEnable(context) {
        ...
        await wedofCommon.handleWebhookSubscription(context.propsValue.scope, context, name)
    },

    async onDisable(context) {
        const id = await context.store.get('_webhookId')
        if (id !== null && id !== undefined) {
            await wedofCommon.unsubscribeWebhook(id as string, context.auth.secret_text)
