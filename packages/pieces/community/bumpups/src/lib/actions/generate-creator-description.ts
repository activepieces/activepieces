...
export const generateCreatorDescription = createAction({
  ...
  description: 'Generates a description of a given YouTube video using the bump-1.0 model.',
  props: {
    url: Property.ShortText({
      displayName: 'Video URL',
      required: true,
      description: 'The YouTube video URL to generate a description for',
    }),
    model: Property.StaticDropdown({
      displayName: 'Model',
      required: true,
      description: 'The AI model to use for generating the description',
      options: {
        options: [
  ...
  async run({ auth, propsValue }) {
    ...
    try {
      const response = await httpClient.sendRequest({
        ...
        url: 'https://api.bumpups.com/creator/description',
        headers: {
