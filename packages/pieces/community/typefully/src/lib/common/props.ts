...
import { TypefullyPaginatedResponse, TypefullySocialSet, TypefullyTag } from './types'

async function fetchAllPages<T>(apiKey: string, resourceUri: string): Promise<T[]> {
    const allResults: T[] = []
    let offset = 0
    const limit = 50
...
export const socialSetDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const socialSets = await fetchAllPages<TypefullySocialSet>(auth.secret_text, '/social-sets')
...
export const tagsMultiSelectDropdown = Property.MultiSelectDropdown({
    ...
    options: async ({ auth, social_set_id }) => {
        ...
        const tags = await fetchAllPages<TypefullyTag>(auth.secret_text, `/social-sets/${social_set_id}/tags`)
