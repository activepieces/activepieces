...
import { FeatureType, StartDocumentAnalysisCommand } from '@aws-sdk/client-textract'
...
export const startDocumentAnalysis = createAction({
    ...
    props: {
        ...
        s3Key: Property.ShortText({
            ...
            description:
                'The path to the document in your S3 bucket (e.g. "reports/multi-page.pdf"). Supports multi-page PDFs and TIFF files.',
            required: true,
        }),
        featureTypes: Property.StaticMultiSelectDropdown({
            displayName: 'Feature Types',
            description:
                'Select what to extract. TABLES extracts table data. FORMS extracts key-value pairs. SIGNATURES detects signatures.',
            required: true,
            options: {
                options: [
        ...
        outputS3Bucket: Property.ShortText({
            ...
            description:
                'S3 bucket where Textract will save the raw analysis output. Leave blank to retrieve results using "Get Document Analysis Results" instead.',
            required: false,
        }),
        outputS3Prefix: Property.ShortText({
            displayName: 'Output Folder Path (Optional)',
            description:
                'Folder path within the output bucket where results will be saved (e.g. "textract-output/"). Only used if Output S3 Bucket is set.',
            required: false,
        }),
    },
    async run(context) {
        const { s3Bucket, s3Key, featureTypes, clientRequestToken, outputS3Bucket, outputS3Prefix } = context.propsValue
