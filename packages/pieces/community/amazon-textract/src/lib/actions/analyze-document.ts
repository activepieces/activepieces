...
import { buildDocumentInput, createTextractClient, formatTextractError, parseAnalysisBlocks } from '../common'
            ...
            props: async ({ source }): Promise<DynamicPropsValue> => {
                if (source === 's3') {
                    return {
                        ...
                        s3Key: Property.ShortText({
                            ...
                            description:
                                'The path to the file in your S3 bucket (e.g. "documents/invoice.pdf"). PDF and TIFF are supported for single-page documents only. For multi-page PDFs, use the Start Document Analysis action instead.',
                            required: true,
                        }),
                    }
                }
                return {
                    file: Property.File({
                        displayName: 'File',
                        description:
                            'The document to analyze. Only JPEG and PNG are supported for direct upload (max 5 MB). For PDF or TIFF files, use the "From S3 bucket" option instead.',
                        required: true,
                    }),
                }
            },
