...
import { buildDocumentInput, createTextractClient, formatTextractError, parseExpenseDocuments } from '../common'
            ...
            props: async ({ source }): Promise<DynamicPropsValue> => {
                ...
                return {
                    file: Property.File({
                        displayName: 'File',
                        description:
                            'The receipt or invoice to analyze. Only JPEG and PNG are supported for direct upload (max 5 MB). For PDF or TIFF files, use the "From S3 bucket" option instead.',
                        required: true,
                    }),
                }
            },
