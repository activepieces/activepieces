...
import { buildDocumentInput, createTextractClient, formatTextractError, parseTextBlocks } from '../common'
...
export const detectDocumentText = createAction({
            ...
            props: async ({ source }): Promise<DynamicPropsValue> => {
                ...
                return {
                    file: Property.File({
                        displayName: 'File',
                        description:
                            'The document to read. Only JPEG and PNG are supported for direct upload (max 5 MB). For PDF or TIFF files, use the "From S3 bucket" option instead.',
                        required: true,
                    }),
                }
            },
    ...
    async run(context) {
        ...
        try {
            ...
            const response = await client.send(new DetectDocumentTextCommand({ Document: documentInput }))
