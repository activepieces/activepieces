...
import { buildDocumentInput, createTextractClient, formatTextractError, parseIdentityDocuments } from '../common'
            ...
            props: async ({ source }): Promise<DynamicPropsValue> => {
                if (source === 's3') {
                    return {
                        ...
                        s3Key: Property.ShortText({
                            ...
                            description:
                                'The path to the image in your S3 bucket (e.g. "id-documents/passport.jpg"). Only JPEG and PNG are supported — PDF and TIFF are not accepted by this action.',
                            required: true,
                        }),
                    }
                }
                return {
                    file: Property.File({
                        displayName: 'File',
                        description:
                            'The ID document to analyze. Only JPEG and PNG are supported (max 5 MB). PDF and TIFF are not supported by this action.',
                        required: true,
                    }),
                }
            },
