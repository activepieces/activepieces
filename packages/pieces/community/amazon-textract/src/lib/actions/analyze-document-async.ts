...
import { createTextractClient, formatTextractError, parseAnalysisBlocks } from '../common'
...
export const analyzeDocumentAsync = createAction({
    ...
    props: {
        ...
        s3Key: Property.ShortText({
            ...
            description:
                'The path to the file in your S3 bucket (e.g. "reports/contract.pdf"). Supports JPEG, PNG, PDF, and TIFF. Multi-page PDFs and TIFFs are fully supported.',
            required: true,
        }),
        featureTypes: Property.StaticMultiSelectDropdown({
            displayName: 'What to Extract',
            description:
                'Choose what to extract. Tables: structured grid data. Forms: labelled fields and their values (e.g. "Name: John"). Signatures: signature locations. Layout: document structure like titles, headers, and sections.',
            required: true,
            options: {
                options: [
        ...
        queries: Property.Array({
            ...
            description:
                'Ask plain-English questions about the document and get direct answers (e.g. "What is the invoice total?" or "What is the patient name?").',
            required: false,
        }),
        clientRequestToken: Property.ShortText({
            displayName: 'Deduplication Token (Optional)',
            description:
                'A unique string (e.g. "invoice-2024-001") to prevent accidentally running the same job twice. If you submit the same token again within 5 minutes, AWS returns the original job result instead of starting a new one.',
            required: false,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            if (queries && queries.length > 0) {
                const queryStrings = queries.filter((q): q is string => typeof q === 'string' && q.trim().length > 0)
