...
export function buildDocumentInput(
    ...
    throw new Error('You must provide either a file or an S3 bucket and key.')
}
...
function getChildIds(block: Block, relationshipType: RelationshipType): string[] {
    return block.Relationships?.filter((r) => r.Type === relationshipType).flatMap((r) => r.Ids ?? []) ?? []
}
...
function getBlockText(block: Block, blockIndex: Map<string, Block>): string {
    ...
    return wordIds
        ...
        .filter((b): b is Block => b !== undefined && b.BlockType === BlockType.WORD)
        .map((b) => b.Text ?? '')
        .join(' ')
        .trim()
}

function getValueBlock(keyBlock: Block, blockIndex: Map<string, Block>): Block | undefined {
    const valueIds = getChildIds(keyBlock, RelationshipType.VALUE)
    for (const id of valueIds) {
        const block = blockIndex.get(id)
        if (block?.BlockType === BlockType.KEY_VALUE_SET && block.EntityTypes?.includes(EntityType.VALUE)) {
            return block
        }
    }
    return undefined
...
export function parseAnalysisBlocks(blocks: Block[]): AnalyzeDocumentResult {
        ...
        switch (block.BlockType) {
            ...
            case BlockType.KEY_VALUE_SET:
                if (block.EntityTypes?.includes(EntityType.KEY)) {
                    ...
                    const valueText = valueBlock ? getBlockText(valueBlock, blockIndex) : ''
                    if (keyText) {
                        keyValuePairs.push({
                            key: keyText,
                            value: valueText,
    ...
    for (const tableBlock of tableMap.values()) {
        ...
        for (const cellId of cellIds) {
            ...
            const isHeader = cellBlock.EntityTypes?.includes(EntityType.COLUMN_HEADER) ?? false
    ...
    const pageCount = Math.max(0, ...blocks.filter((b) => b.BlockType === BlockType.PAGE).map((b) => b.Page ?? 0))
...
export function parseExpenseDocuments(expenseDocs: ExpenseDocument[]): AnalyzeExpenseResult {
...
export function parseIdentityDocuments(identityDocs: IdentityDocument[]): AnalyzeIDResult {
