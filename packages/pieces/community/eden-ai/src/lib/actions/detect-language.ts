...
export const detectLanguageAction = createAction({
  ...
  description: 'Detect the language used in a text using various language detection providers via Eden AI.',
  props: {
    ...
    settings: Property.Json({
      ...
      description: 'Optional JSON specifying provider-specific model overrides, e.g., {"google": "google_model"}',
      required: false,
    }),
  ...
  async run(context) {
    const response = await edenApiCall<any>({
      ...
      body: {
        ...
        settings: context.propsValue.settings,
