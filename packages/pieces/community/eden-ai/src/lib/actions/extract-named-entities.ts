...
export const extractNamedEntitiesAction = createAction({
  name: 'edenai-extract-named-entities',
  auth: edenAuth,
  displayName: 'Extract Named Entities',
  description:
    'Extract named entities from text using various NLP providers via Eden AI.',
  props: {
    ...
    providers: Property.Array({
      ...
      description:
        'One or more providers (e.g., ["openai", "google"]) or providers with model (e.g., ["openai/gpt-4"])',
      required: true,
    }),
  ...
  async run(context) {
    ...
    try {
      ...
      return {
        success: true,
        message: 'Named entity extraction completed successfully',
        data: response,
      };
    } catch (error: any) {
      ...
      throw new Error(`Failed to extract named entities: ${error.message}`);
    }
  },
});
