...
export const generateAudioAction = createAction({
  ...
  description:
    'Convert text to speech using various text-to-speech providers via Eden AI.',
  props: {
    ...
    providers: Property.Array({
      ...
      description:
        'One or more providers (e.g., ["google", "microsoft"]) or providers with model (e.g., ["google/text-to-speech-v1"])',
      required: true,
    }),
    language: Property.ShortText({
      displayName: 'Language',
      description:
        'Language code for the text (e.g., "en-US", "fr-FR", "es-ES").',
      required: false,
    }),
    option: Property.ShortText({
      displayName: 'Voice Option',
      description:
        'Voice option/style (e.g., "MALE", "FEMALE", specific voice name).',
      required: false,
    }),
