...
import { ColumnDefinition, Drive, DriveItem, List, ListItem, Site } from '@microsoft/microsoft-graph-types'
import { microsoftSharePointAuth } from '../auth'
import { getGraphBaseUrl } from './microsoft-cloud'

const createDriveDropdown = (params: { displayName: string; refreshers: string[] }) =>
    Property.Dropdown({
...
export const microsoftSharePointCommon = {
    ...
    itemId: Property.Dropdown({
        ...
        options: async ({ auth, siteId, driveId }) => {
            ...
            let response: PageCollection = await client
                .api(`/sites/${siteId}/drives/${driveId}/root/children?$select=id,name,folder,file`)
                .get()
    ...
    pageId: Property.Dropdown({
        ...
        options: async ({ auth, siteId }) => {
            ...
            const options: DropdownOption<string>[] = []
            let response: PageCollection = await client.api(`/sites/${siteId}/pages?$select=id,title`).get()
            while (response.value.length > 0) {
