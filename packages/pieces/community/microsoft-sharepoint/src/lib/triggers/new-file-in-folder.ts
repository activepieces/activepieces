...
export const newFileInFolderTrigger = createTrigger({
    ...
    props: {
        ...
        folderId: Property.Dropdown({
            ...
            description:
                'The folder to watch for new files. Select "Root Folder" for the top-level folder of the drive.',
            required: true,
    ...
    async onEnable(context) {
        ...
        try {
            const effectiveFolderId =
                folderId === 'root' ? (await client.api(`/drives/${driveId}/root`).get()).id : folderId
