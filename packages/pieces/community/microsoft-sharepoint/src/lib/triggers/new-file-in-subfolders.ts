...
export const newFileInSubfoldersTrigger = createTrigger({
        ...
        parentFolderId: Property.Dropdown({
            ...
            options: async ({ auth, siteId, driveId }) => {
                ...
                let response: PageCollection = await client
                    .api(`/drives/${driveId}/root/children?$filter=folder ne null&$select=id,name`)
                    .get()
    ...
    async onEnable(context) {
        ...
        try {
            const effectiveParentId =
                parentFolderId === 'root' ? (await client.api(`/drives/${driveId}/root`).get()).id : parentFolderId

            const subfoldersResponse = await client
                .api(`/drives/${driveId}/items/${effectiveParentId}/children?$filter=folder ne null&$select=id`)
                .get()
        ...
        if (subscriptionIds && subscriptionIds.length > 0) {
            ...
            const deletionPromises = subscriptionIds.map((id) =>
                client
                    ...
                    .delete()
                    .catch((err) => console.error(`Failed to delete subscription ${id}:`, err)),
            )
    ...
    async run(context) {
        ...
        const validNotifications = notifications.filter((notif) => notif.clientState === clientState)
