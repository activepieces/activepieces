...
export const jobFailed = createTrigger({
  ...
  onEnable: async (context) => {
    ...
    const request: HttpRequest = {
      ...
      url: `${cloudconvertCommon.baseUrl((context.auth).props?.['region'] || 'auto')}/webhooks`,
      body: {
        url: webhookUrl,
        events: ['job.failed']
      },
  ...
  onDisable: async (context) => {
    ...
    const listRequest: HttpRequest = {
      ...
      url: `${cloudconvertCommon.baseUrl((context.auth).props?.['region'] || 'auto')}/users/me/webhooks`,
      authentication: {
        type: AuthenticationType.BEARER_TOKEN,
        token: context.auth.access_token,
      },
      ...
      if (listResponse.status === 200 && listResponse.body.data) {
        ...
        if (webhook) {
          const deleteRequest: HttpRequest = {
            ...
            url: `${cloudconvertCommon.baseUrl((context.auth).props?.['region'] || 'auto')}/webhooks/${webhook.id}`,
            authentication: {
              type: AuthenticationType.BEARER_TOKEN,
              token: context.auth.access_token,
            },
