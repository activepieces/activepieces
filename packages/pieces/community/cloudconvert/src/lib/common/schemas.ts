...
export const convertFileSchema = z
    .object({
        ...
        input_format: z
            ...
            .nullable()
            .optional()
            .refine((val) => !val || val === 'auto' || val.length > 0, {
    ...
    .refine(
        ...
        {
            message:
                'File is required for upload method, URL is required for URL method, Stored File ID is required for stored file method',
            path: ['file'],
        },
    )
...
export const optimizeFileSchema = z
    .object({
        import_method: z.enum(['upload', 'url', 'stored_file']),
        file: z
            .object({
