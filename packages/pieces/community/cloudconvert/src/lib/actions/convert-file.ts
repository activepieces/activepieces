...
import { CloudConvertClient, cloudconvertAuth, convertFileProps, convertFileSchema } from '../common'
    ...
    async run(context) {
        ...
        try {
            ...
            if (supportedFormats.length === 0) {
                ...
                throw new Error(
                    `Conversion from ${input_format || 'auto-detected format'} to ${output_format} is not supported by CloudConvert. Available input formats for ${output_format}: ${allOutputFormats.map((f: any) => f.input_format).join(', ')}`,
                )
            }
            ...
            if (import_method === 'url') {
                ...
                if (!context.server?.apiUrl) {
                    throw new Error(
                        'Server API URL is not available. Please check your Activepieces server configuration.',
                    )
                }
                ...
                try {
                    ...
                    throw new Error(
                        `Invalid file URL constructed: ${fileUrl}. URL Error: ${urlError instanceof Error ? urlError.message : String(urlError)}`,
                    )
                }
                jobTasks['import-file'] = {
                    operation: 'import/url',
                ...
                if (uploadResponse.status < 200 || uploadResponse.status >= 300) {
                    throw new Error(
                        `Failed to upload file: HTTP ${uploadResponse.status} - ${uploadResponse.body?.message || 'Upload failed'}`,
                    )
                }
                ...
                while (attempts < maxAttempts) {
                    ...
                    if (currentJob.status === 'finished') {
                        ...
                        const outputFilename =
                            exportTaskData?.result?.files?.[0]?.filename || `converted-${output_format}`
                        ...
                        if (store_file && downloadUrl) {
                            try {
                                ...
                                try {
                                    ...
                                    throw new Error(
                                        `Invalid download URL format: ${downloadUrl} - ${urlError instanceof Error ? urlError.message : String(urlError)}`,
                                    )
                                }
