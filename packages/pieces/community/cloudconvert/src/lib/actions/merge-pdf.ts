...
import { CloudConvertClient, cloudconvertAuth, mergePdfSchema } from '../common'
...
export const mergePdf = createAction({
    ...
    async run(context) {
        ...
        const { import_method, files, filename, engine, engine_version, timeout, wait_for_completion, store_file } =
            context.propsValue
            ...
            files.forEach((file: any, index: number) => {
                ...
                if (import_method === 'url') {
                    ...
                    if (!file.stored_file_id) {
                        throw new Error(
                            `Stored File ID is required for file ${index + 1} when using stored file import method`,
                        )
                    }
                    ...
                    throw new Error(
                        'Upload method for multiple files is not yet supported with the new job format. Please use URL or stored file methods.',
                    )
                } else {
