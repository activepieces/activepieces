...
export const createTranscription = createAction({
    ...
    description: 'Initiate a pre-recorded transcription job from an audio or video URL',
    props: {
        ...
        languages: Property.StaticDropdown({
            ...
            description: 'Select the language spoken in the audio for better transcription accuracy',
            required: false,
            options: {
        ...
        code_switching: Property.Checkbox({
            ...
            description: 'Enable code switching to handle multiple languages in the audio',
            required: false,
            defaultValue: false,
        }),
    ...
    async run(context) {
        ...
        const response = await makeRequest(apiKey, HttpMethod.POST, '/pre-recorded', body)

        const transcription = await makeRequest(apiKey, HttpMethod.GET, `/pre-recorded/${response.id}`)
