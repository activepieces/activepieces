...
export const updatedOpportunityStage = createTrigger({
    ...
    async onDisable(context) {
        ...
        if (cachedWebhookData) {
            await CopperApiService.deleteWebhook(context.auth, cachedWebhookData.webhookId).then(async () => {
    ...
    async run(context) {
        ...
        const idChanged = Array.isArray(updatedAttrs.stage_id) && updatedAttrs.stage_id[0] !== updatedAttrs.stage_id[1]
        const labelChanged = Array.isArray(updatedAttrs.stage) && updatedAttrs.stage[0] !== updatedAttrs.stage[1]
