    ...
    async run(context) {
        ...
        const normEmails = (Array.isArray(emails) ? emails : [])
            .map((row) => ({
                email: String(row?.email ?? '')
                    .trim()
                    .toLowerCase(),
                category: isNonEmptyStr(row?.category) ? String(row.category).trim() : undefined,
            }))
            .filter((e) => isNonEmptyStr(e.email))

        if (normEmails.length === 0) throw new Error('Please provide at least one valid email.')
        ...
        const normPhones = (Array.isArray(phone_numbers) ? phone_numbers : [])
            .map((p) => ({
                ...
                category: isNonEmptyStr(p?.category) ? String(p.category).trim() : undefined,
            }))
            .filter((p) => isNonEmptyStr(p.number))

        const address = [address_street, address_city, address_state, address_postal_code, address_country].some(
            isNonEmptyStr,
        )
            ? {
                  street: isNonEmptyStr(address_street) ? address_street.trim() : undefined,
                  city: isNonEmptyStr(address_city) ? address_city.trim() : undefined,
                  state: isNonEmptyStr(address_state) ? address_state.trim() : undefined,
                  postal_code: isNonEmptyStr(address_postal_code) ? address_postal_code.trim() : undefined,
                  country: isNonEmptyStr(address_country) ? address_country.trim() : undefined,
              }
            : undefined
