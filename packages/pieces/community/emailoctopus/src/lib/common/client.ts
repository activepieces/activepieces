...
export class EmailOctopusClient {
    ...
    async getLists(): Promise<EmailOctopusList[]> {
        const response = await this.makeRequest<{ data: EmailOctopusList[] }>(HttpMethod.GET, '/lists')
        return response.data
    }

    async getList(listId: string): Promise<EmailOctopusListDetails> {
        return await this.makeRequest<EmailOctopusListDetails>(HttpMethod.GET, `/lists/${listId}`)
    }

    async getCampaigns(): Promise<EmailOctopusCampaign[]> {
        const response = await this.makeRequest<{ data: EmailOctopusCampaign[] }>(HttpMethod.GET, '/campaigns')
        return response.data
    }
}
