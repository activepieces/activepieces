...
export const submitAsyncJob = createAction({
  name: 'submit_async_job',
  displayName: 'Async AI',
  description: 'Submits a new async inference job.',
  ...
  props: {
    ...
    webhook_url: Property.ShortText({
      ...
      description:
        "Paste the complete Webhook URL from your 'On Job Completed' trigger flow. It must start with https://",
      required: false,
    }),
    // --- Key Differentiator Field (Visible) ---
    completion_windows: Property.ShortText({
      displayName: 'Completion Windows',
      description:
        'Optional. Max time to wait for job completion.',
      required: false,
    }),
    notification_email: Property.ShortText({
      displayName: 'Notification Email',
      description:
        'Optional. Email address to receive notifications.',
      required: false,
    }),
  ...
  async run(context) {
    ...
    // Add webhook_url, completion_windows, and notification_email to the body
    // These are expected in the request body according to the API schema
    if (webhook_url) {
      // Validate that webhook_url is a valid URL
      try {
        new URL(webhook_url);
        requestBody['webhook_url'] = webhook_url;
      } catch (error) {
        throw new Error(
          `Invalid webhook_url: "${webhook_url}". Please provide a complete URL starting with http:// or https://. ` +
          `You can copy the webhook URL from the "On Job Completed" trigger (it should look like https://...).`
        );
      }
    }
    
    if (completion_windows) {
      requestBody['completion_window'] = completion_windows;
    }
    
    if (notification_email) {
      requestBody['notification_email'] = notification_email;
    }
    
    // Build URL (no query parameters needed)
    const url = `${BASE_URL}/chat/completions`;

    // Log the complete request details
    console.log('=== CHEAPESTINFERENCE REQUEST ===');
    console.log('URL:', url);
    ...
    console.log('Body (requestBody):', JSON.stringify(requestBody, null, 2));
    console.log('Messages Array:', JSON.stringify(messagesArray, null, 2));
    console.log('Advanced Params:', advanced_params ? JSON.stringify(advanced_params, null, 2) : 'none');
    console.log('================================');
    ...
    console.log('=== CHEAPESTINFERENCE RESPONSE ===');
