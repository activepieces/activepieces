...
import { findStockItemActionOutputSchema } from '../output-schemas';

export const findStockItemAction = createAction({
  auth: sageAccountingAuth,
  name: 'find_stock_item',
  classification: 'SEARCH',
  displayName: 'Find Stock Item',
  description: 'Searches for stock items in Sage Accounting by description or item code.',
  audience: 'both',
  aiMetadata: {
    description:
      'Search Sage Accounting stock items by description or item code (partial match). Returns zero or more matches. Read-only, safe to retry.',
    idempotent: true,
  },
  outputSchema: findStockItemActionOutputSchema,
  props: {
    ...
    updatedOrCreatedSince: Property.DateTime({
      ...
      description: 'Only return stock items changed since this date/time.',
      required: false,
    }),
