...
export const updateProductAction = createAction({
  ...
  description: 'Updates an existing product in Sage Accounting.',
  audience: 'both',
  aiMetadata: {
    description:
      'Update fields on an existing Sage Accounting product, identified by its ID. Only the fields you provide are changed. Safe to retry with the same values.',
    idempotent: true,
  },
  outputSchema: updateProductActionOutputSchema,
  props: {
    product: sageAccountingDropdowns.productById,
  ...
  async run(context) {
    ...
    return await sageAccountingClient.apiCall<SageAccountingRef>({
      ...
      path: `${sageAccountingClient.paths.products}/${product}`,
      body: {
        product: {
