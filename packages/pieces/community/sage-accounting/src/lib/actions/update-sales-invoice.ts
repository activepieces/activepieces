...
import { updateSalesInvoiceActionOutputSchema } from '../output-schemas';

export const updateSalesInvoiceAction = createAction({
  auth: sageAccountingAuth,
  name: 'update_sales_invoice',
  classification: 'WRITE',
  displayName: 'Update Sales Invoice',
  description: 'Updates an existing sales invoice in Sage Accounting.',
  audience: 'both',
  aiMetadata: {
    description:
      'Update fields on an existing Sage Accounting sales invoice, identified by its ID. Only the fields you provide are changed. Providing line items replaces the entire line list. Safe to retry with the same values.',
    idempotent: true,
  },
  outputSchema: updateSalesInvoiceActionOutputSchema,
  propertyGroups: [
    {
      key: 'mainAddress',
      display: 'section',
  ...
  props: {
    invoice: sageAccountingDropdowns.salesInvoiceById,
  ...
  async run(context) {
    ...
    return await sageAccountingClient.apiCall<SageAccountingRef>({
      ...
      path: `${sageAccountingClient.paths.salesInvoices}/${invoice}`,
      body: {
        sales_invoice: {
