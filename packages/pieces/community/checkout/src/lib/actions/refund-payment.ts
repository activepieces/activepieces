    ...
    async run(context) {
        ...
        if (destination_country && destination_account_number && destination_bank_code) {
            ...
            if (account_holder_first_name && account_holder_last_name) {
                ...
                if (account_holder_address_line1 || account_holder_city || account_holder_country) {
                    ...
                    if (account_holder_address_line1)
                        body['destination']['account_holder']['billing_address']['address_line1'] =
                            account_holder_address_line1
                    if (account_holder_address_line2)
                        body['destination']['account_holder']['billing_address']['address_line2'] =
                            account_holder_address_line2
                    if (account_holder_city)
                        body['destination']['account_holder']['billing_address']['city'] = account_holder_city
                    if (account_holder_state)
                        body['destination']['account_holder']['billing_address']['state'] = account_holder_state
                    if (account_holder_zip)
                        body['destination']['account_holder']['billing_address']['zip'] = account_holder_zip
                    if (account_holder_country)
                        body['destination']['account_holder']['billing_address']['country'] = account_holder_country
                }
            }
        ...
        try {
            ...
            if (error.response?.status === 422) {
                throw new Error(
                    `Invalid data: ${error.response.body?.error_codes?.join(', ') || 'Please check your input data'}`,
                )
            }
            throw error
        }
