...
export const checkoutComAuth = PieceAuth.SecretText({
    ...
    description:
        'Your Checkout.com secret key. Use sandbox key (sk_sbox_...) for testing or production key (sk_...) for live transactions. You can find it in the Checkout.com dashboard under Developers > API keys.',
    required: true,
    validate: async ({ auth }) => {
        try {
            ...
            if (!secretKey.startsWith('sk_')) {
                return {
                    valid: false,
                    error: 'Invalid API key format. Must start with "sk_" for production or "sk_sbox_" for sandbox.',
                }
            }
            ...
            if (e.response?.status === 401) {
                return {
                    valid: false,
                    error: 'Invalid Checkout.com secret key. Please check your API key in the Checkout.com dashboard.',
                }
            } else if (e.response?.status === 403) {
                return {
                    valid: false,
                    error: 'Insufficient permissions. Please ensure your API key has the required permissions.',
                }
            } else {
                return {
                    valid: false,
