...
import { leadDropdown, lossReasonDropdown, pipelineDropdown, statusDropdown, userDropdown } from '../common/props'
...
export const updateLeadAction = createAction({
    ...
    async run(context) {
        const { leadId, name, price, statusId, pipelineId, responsible_user_id, loss_reason_id } = context.propsValue
        ...
        const result = await makeRequest({ apiToken, subdomain }, HttpMethod.PATCH, `/leads/${leadId}`, updatePayload)
