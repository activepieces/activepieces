...
export const newContactAddedTrigger = createTrigger({
    ...
    async onEnable(context) {
        ...
        const webhook = await makeRequest({ subdomain, apiToken }, HttpMethod.POST, `/webhooks`, {
    ...
    async onDisable(context) {
        ...
        if (webhookId) {
            await makeRequest({ subdomain, apiToken }, HttpMethod.DELETE, `/webhooks`, {
                destination: context.webhookUrl,
            })
        }
    },
    ...
    async test(context) {
        ...
        const response = await makeRequest(
            { subdomain, apiToken },
            HttpMethod.GET,
            '/contacts?limit=5&order[updated_at]=desc',
        )
