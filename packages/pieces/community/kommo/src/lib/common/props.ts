...
export const pipelineDropdown = (required = false) =>
    Property.Dropdown({
        auth: kommoAuth,
        ...
        options: async ({ auth }) => {
            ...
            const options: DropdownOption<number>[] = (pipelines._embedded?.pipelines || []).map((pipeline: any) => ({
                label: pipeline.name,
                value: pipeline.id,
            }))
...
export const statusDropdown = (required = false) =>
    Property.Dropdown({
        auth: kommoAuth,
        displayName: 'Status',
        required,
        ...
        options: async ({ auth, pipelineId }) => {
            ...
            const options: DropdownOption<number>[] = (statuses._embedded?.statuses || []).map((status: any) => ({
                label: status.name,
                value: status.id,
            }))
...
export const userDropdown = (required = false) =>
    Property.Dropdown({
        auth: kommoAuth,
...
export const lossReasonDropdown = (required = false) =>
    Property.Dropdown({
        auth: kommoAuth,
        ...
        options: async ({ auth }) => {
            ...
            const options: DropdownOption<number>[] = (reasons._embedded?.loss_reasons || []).map((reason: any) => ({
                label: reason.name,
                value: reason.id,
            }))
...
export const companyDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const options: DropdownOption<string>[] = (companies._embedded?.companies || []).map((company: any) => ({
            label: company.name,
...
export const contactDropdown = Property.Dropdown({
    ...
    options: async ({ auth }) => {
        ...
        const options: DropdownOption<number>[] = (contacts._embedded?.contacts || []).map((contact: any) => ({
            label: contact.name,
            value: contact.id,
        }))
