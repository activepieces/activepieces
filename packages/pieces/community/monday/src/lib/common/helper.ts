...
export function generateColumnIdTypeMap(columns: MondayColumn[]): ColumnIdTypeMap {
...
export const convertMondayColumnToActivepiecesProp = (column: MondayColumn) => {
  switch (column.type) {
    ...
    case MondayColumnType.DEPENDENCY:
      return Property.ShortText({
        displayName: column.title,
        description: 'A list of item IDs from the same board. Example: [188392, 20339]',
        required: false,
      })
    case MondayColumnType.DROPDOWN: {
      const labels: { id: string; name: string }[] = JSON.parse(column.settings_str).labels
...
export const convertPropValueToMondayColumnValue = (columnType: string, propValue: DynamicPropsValue) => {
  switch (columnType) {
    case MondayColumnType.CHECKBOX:
      return {
  ...
  switch (columnValue.type) {
    ...
    case MondayColumnType.SUBTASKS: {
      ...
      if (!isEmpty(JSON.parse(columnValue.value))) {
        JSON.parse(columnValue.value).linkedPulseIds.map((item: { linkedPulseId: number }) => {
    ...
    case MondayColumnType.PEOPLE: {
      const people: number[] = []
      if (!isEmpty(columnValue.value)) {
        JSON.parse(columnValue.value).personsAndTeams.map((item: { id: number; kind: string }) => {
          people.push(item.id)
        })
      }
      return people
