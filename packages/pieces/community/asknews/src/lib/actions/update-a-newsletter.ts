...
export const updateANewsletter = createAction({
    ...
    async run(context) {
        ...
        if (audienceId !== undefined && audienceId !== '') requestBody.audience_id = audienceId
        if (resendApiKey !== undefined && resendApiKey !== '') requestBody.resend_api_key = resendApiKey
        if (isPublic !== undefined) requestBody.public = isPublic
        if (isActive !== undefined) requestBody.active = isActive
        if (expiresAt !== undefined && expiresAt !== '') requestBody.expires_at = expiresAt
