...
export const searchNews = createAction({
    ...
    async run(context) {
        const { query, nArticles, method, returnType, historical, hoursBack, categories, similarityScoreThreshold } =
            context.propsValue
        ...
        if (similarityScoreThreshold) queryParams.similarity_score_threshold = similarityScoreThreshold

        const queryString = new URLSearchParams(queryParams).toString()
        const response = await makeRequest(context.auth.secret_text, HttpMethod.GET, `/news/search?${queryString}`)
