...
export const getArticleById = createAction({
    ...
    async run(context) {
        ...
        const queryParams = articleIds.map((id: string) => `article_ids=${encodeURIComponent(id)}`).join('&')
        const endpoint = `/news?${queryParams}`

        const response = await makeRequest(context.auth.secret_text, HttpMethod.GET, endpoint)
