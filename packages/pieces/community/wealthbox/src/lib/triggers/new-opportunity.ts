...
const polling: Polling<any, any> = {
    ...
    items: async ({ propsValue, lastFetchEpochMS, auth }) => {
        ...
        if (propsValue.min_probability !== undefined)
            searchParams.append('min_probability', propsValue.min_probability.toString())
        if (propsValue.max_probability !== undefined)
            searchParams.append('max_probability', propsValue.max_probability.toString())

        if (propsValue.target_close_after)
            searchParams.append('target_close_after', dayjs(propsValue.target_close_after).toISOString())
        if (propsValue.target_close_before)
            searchParams.append('target_close_before', dayjs(propsValue.target_close_before).toISOString())
        ...
        const url = queryString
            ? `${WEALTHBOX_API_BASE}/opportunities?${queryString}`
            : `${WEALTHBOX_API_BASE}/opportunities`
        ...
        try {
            ...
            throw new Error(
                `Failed to poll new opportunities: ${error instanceof Error ? error.message : 'Unknown error'}`,
            )
        }
    },
}
