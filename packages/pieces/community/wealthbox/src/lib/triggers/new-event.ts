...
const polling: Polling<any, any> = {
    ...
    items: async ({ propsValue, lastFetchEpochMS, auth }) => {
        ...
        if (propsValue.start_date_min)
            searchParams.append('start_date_min', dayjs(propsValue.start_date_min).toISOString())
        if (propsValue.start_date_max)
            searchParams.append('start_date_max', dayjs(propsValue.start_date_max).toISOString())
