...
import { fetchContacts, fetchHouseholds, handleApiError, WEALTHBOX_API_BASE } from '../common'
...
export const addHouseholdMember = createAction({
    ...
    async run(context) {
        ...
        try {
            ...
            throw new Error(
                `Failed to add member to household: ${error instanceof Error ? error.message : 'Unknown error'}`,
            )
        }
    },
})
