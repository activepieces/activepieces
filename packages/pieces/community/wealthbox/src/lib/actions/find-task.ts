...
export const findTask = createAction({
    ...
    description:
        'Finds existing tasks using comprehensive search filters. Search by assignment, resource, completion status, and date ranges.',
    props: {
    ...
    async run(context) {
        ...
        if (!hasSearchCriteria) {
            throw new Error(
                'At least one search criteria must be provided (Task ID, resource, assignment, status, or date filters)',
            )
        }
