...
import { fetchContacts, fetchTags, handleApiError, WEALTHBOX_API_BASE } from '../common'
...
export const findContact = createAction({
    ...
    description:
        'Locate a contact by name, email, phone, or advanced filters. Comprehensive contact search with dynamic filtering options.',
    props: {
        name: Property.ShortText({
            displayName: 'Name',
            description:
                'Search by name (supports partial matches across prefix, first, middle, last, suffix, nickname, and full name for households/companies)',
            required: false,
        }),
        ...
        return_single_result: Property.Checkbox({
            ...
            description:
                'If checked, returns only the first matching contact. If unchecked, returns all matching contacts.',
            required: false,
            defaultValue: false,
        }),
    },
    ...
    async run(context) {
        ...
        if (!hasSearchCriteria) {
            throw new Error(
                'At least one search criteria must be provided (ID, name, email, phone, external ID, or filters)',
            )
        }
