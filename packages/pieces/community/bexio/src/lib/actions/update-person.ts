...
export const updatePersonAction = createAction({
                ...
                try {
                    ...
                    return {
                        ...
                        options: persons.map((person) => {
                            const name = person.name_2 ? `${person.name_2} ${person.name_1}` : person.name_1
        ...
        country_id: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    const countries =
                        await client.get<
                            Array<{ id: number; name: string; name_short: string; iso3166_alpha2: string }>
                        >('/2.0/country')
        ...
        user_id: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                try {
                    ...
                    const users =
                        await client.get<
                            Array<{ id: number; firstname: string | null; lastname: string | null; email: string }>
                        >('/3.0/users')
    ...
    async run(context) {
        ...
        if (
            props['contact_group_ids'] &&
            Array.isArray(props['contact_group_ids']) &&
            props['contact_group_ids'].length > 0
        ) {
            requestBody['contact_group_ids'] = props['contact_group_ids'].join(',')
        }
        if (
            props['contact_branch_ids'] &&
            Array.isArray(props['contact_branch_ids']) &&
            props['contact_branch_ids'].length > 0
        ) {
