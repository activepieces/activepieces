...
export const createSalesInvoiceAction = createAction({
                ...
                try {
                    ...
                    return {
                        ...
                        options: contacts.map((contact) => {
                            const name = contact.name_2 ? `${contact.name_2} ${contact.name_1}` : contact.name_1
            ...
            props: async ({ auth }) => {
                ...
                if (auth) {
                    try {
                        ...
                        accounts = await client
                            .get<Array<{ id: number; account_no: string; name: string }>>('/accounts')
                            .catch(() => [])
                        const taxesResponse = await client
                            .get<
                                Array<{
                                    id: number
                                    name: string
    ...
    async run(context) {
        ...
        if (positionFields && positionFields['positions'] && Array.isArray(positionFields['positions'])) {
            requestBody['positions'] = (positionFields['positions'] as Array<Record<string, unknown>>).map(
                (position) => {
