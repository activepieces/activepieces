...
export const createManualEntryAction = createAction({
            ...
            props: async ({ type, auth }, ctx) => {
                ...
                if (auth) {
                    try {
                        ...
                        accounts =
                            await client.get<Array<{ id: number; account_no: string; name: string }>>('/accounts')
    ...
    async run(context) {
        ...
        const entryType = context.propsValue.type as
            | 'manual_single_entry'
        ...
        const response = await client.post<BexioManualEntryResponse>('/accounts/manual_entries', requestBody)
