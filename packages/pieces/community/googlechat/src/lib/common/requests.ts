...
export const googleChatAPIService = {
    ...
    async AddASpaceMember({ accessToken, spaceId, userId }: { accessToken: string; spaceId: string; userId: string }) {
        return await fireHttpRequest({
            method: HttpMethod.POST,
    ...
    async getSpace({ accessToken, spaceId }: { accessToken: string; spaceId: string }) {
        return await fireHttpRequest({
            method: HttpMethod.GET,
            entity: 'chat',
            path: `/v1/${spaceId}`,
    ...
    async listMessages(accessToken: string, spaceId: string, pageSize = 50) {
        return await fireHttpRequest({
            method: HttpMethod.GET,
            entity: 'chat',
            accessToken,
    ...
    async fetchWorkSpaceSubscriptions({ accessToken, event_type }: { accessToken: string; event_type: string }) {
        const response = await fireHttpRequest({
            method: HttpMethod.GET,
            entity: 'workspaceevents',
            accessToken,
