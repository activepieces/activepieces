...
export const sendMessageSchema = {
    ...
    messageReplyOption: z.enum(['REPLY_MESSAGE_FALLBACK_TO_NEW_THREAD', 'REPLY_MESSAGE_OR_FAIL']).optional(),
    customMessageId: z
        .string()
        .regex(/^[a-z0-9-]+$/, 'Custom message ID must contain only lowercase letters, numbers, and hyphens')
...
export const spaceIdSchema = z.string().regex(/^spaces\/[a-zA-Z0-9_-]+$/, 'Space ID must be in format: spaces/{space}')
...
export const userResourceNameSchema = z
    .string()
    .regex(/^people\/[a-zA-Z0-9_-]+$/, 'User resource name must be in format: people/{person}')

export const customMessageIdSchema = z
    .string()
    .regex(
        /^client-[a-z0-9-]+$/,
...
export const addSpaceMemberSchema = {
    spaceId: z
        ...
        .regex(/^spaces\/[a-zA-Z0-9_-]+$/, 'Space ID must be in format: spaces/{space}'),
    personId: z
        .string()
        .min(1, 'Person ID is required')
        .regex(/^people\/[a-zA-Z0-9_-]+$/, 'Person ID must be in format: people/{person}'),
}
...
export const getDirectMessageDetailsSchema = {
    directMessageId: z
        ...
        .regex(/^spaces\/[a-zA-Z0-9_-]+$/, 'Direct message ID must be in format: spaces/{space}'),
}
...
export const newMentionTriggerSchema = {
    ...
    spaceMemberId: z
        .string()
        .regex(/^users\/[a-zA-Z0-9_-]+$/, 'Space member ID must be in format: users/{user}')
        .optional(),
}
