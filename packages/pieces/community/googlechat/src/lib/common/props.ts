...
export const projectsDropdown = (refreshers: string[]) =>
    Property.Dropdown({
        ...
        async options({ auth }: any) {
            ...
            try {
                const projects = await googleChatAPIService.fetchProjects(auth.access_token)
...
export const spacesDropdown = ({ refreshers, required = false }: { refreshers: string[]; required?: boolean }) =>
    Property.Dropdown({
        auth: googleChatApiAuth,
        displayName: 'Space',
        ...
        async options({ auth }: any) {
            ...
            try {
                const spaces = await googleChatAPIService.fetchSpaces(auth.access_token)
...
export const allSpacesDropdown = ({ refreshers, required = false }: { refreshers: string[]; required?: boolean }) =>
    Property.Dropdown({
        auth: googleChatApiAuth,
        displayName: 'Space',
        description: `Select a Space${required ? '' : ', leave empty for all spaces'}`,
        ...
        async options({ auth }: any) {
            ...
            try {
                const spaces = await googleChatAPIService.fetchAllSpaces(auth.access_token)
...
export const directMessagesDropdown = ({
    ...
    Property.Dropdown({
        ...
        async options({ auth }: any) {
            ...
            try {
                const spaces = await googleChatAPIService.fetchDirectMessages(auth.access_token)
...
export const spacesMembersDropdown = (refreshers: string[]) =>
    Property.Dropdown({
        ...
        async options({ auth, spaceId }) {
            ...
            try {
                const members = await googleChatAPIService.fetchSpaceMembers(auth.access_token, spaceId)
...
export const peoplesDropdown = (refreshers: string[]) =>
    Property.Dropdown({
        ...
        async options({ auth }) {
            ...
            try {
                const members = await googleChatAPIService.fetchPeople(auth.access_token)

                return {
                    options: members
                        .map((member: any) => {
                            const nameObj = member.names?.find((n: any) => n.metadata.primary) || member.names?.[0]
...
export const threadsDropdown = ({ refreshers, required = false }: { refreshers: string[]; required?: boolean }) =>
    Property.Dropdown({
        auth: googleChatApiAuth,
        displayName: 'Thread',
        ...
        async options({ auth, spaceId }) {
            ...
            try {
                const threads = await googleChatAPIService.fetchThreads(auth.access_token, spaceId)
