...
export const newMention = createTrigger({
    ...
    async onEnable({ auth, propsValue, webhookUrl, store }) {
        ...
        const targetResource = `//chat.googleapis.com/${spaceId ? spaceId : 'spaces/-'}`
    ...
    async run(context) {
        const messageData = JSON.parse(
            Buffer.from((context.payload.body as any).message.data, 'base64').toString('utf-8'),
        )
        ...
        const mentions = messageData.message.annotations.filter((a: any) => a.type === 'USER_MENTION')
        ...
        if (spaceMemberId) {
            const isMatch = mentions.some((m: any) => m.userMention?.user?.name === spaceMemberId)
