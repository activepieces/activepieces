...
export const searchMessages = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        const response = await googleChatAPIService.listMessages(auth.access_token, spaceId as string, limit)

        const messages = response.messages || []

        const filtered = messages.filter((msg: any) => msg.text?.toLowerCase().includes(keyword.toLowerCase()))
