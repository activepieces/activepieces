...
export const findMember = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        try {
            const people = await googleChatAPIService.fetchPeople(accessToken)

            const person = people.find((p: any) =>
                p?.emailAddresses?.some((e: any) => e.value.toLowerCase() === email.toLowerCase()),
            )
            ...
            const spaceMembers = await googleChatAPIService.fetchSpaceMembers(accessToken, spaceId as string)

            const matchingMember = spaceMembers.find((m: any) => m.member?.name === spaceMemberId)
