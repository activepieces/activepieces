...
const polling: Polling<
    AppConnectionValueForAuthProperty<typeof microsoftOutlookAuth>,
    {
        sender?: string
        recipient?: string
    }
> = {
    ...
    items: async ({ auth, lastFetchEpochMS, propsValue }) => {
        ...
        const messages = []
        const filter =
            lastFetchEpochMS === 0 ? '$top=10' : `$filter=receivedDateTime gt ${dayjs(lastFetchEpochMS).toISOString()}`
        ...
        if (propsValue?.recipient) {
            filteredMessages = filteredMessages.filter((message) => {
                const hasRecipient = message.toRecipients?.some((recipient) => {
