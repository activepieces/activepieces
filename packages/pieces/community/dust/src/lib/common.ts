import { HttpMessageBody, HttpMethod, httpClient } from '@activepieces/pieces-common'
import { Property } from '@activepieces/pieces-framework'
import { DustAPI } from '@dust-tt/client'
import { DustAuthType, dustAuth } from '..'
...
export async function getConversationContent(conversationId: string, timeout: number, auth: DustAuthType) {
    const getConversation = async (conversationId: string) => {
        return httpClient.sendRequest({
            method: HttpMethod.GET,
            url: `${DUST_BASE_URL[auth.region || 'us']}/${auth.workspaceId}/assistant/conversations/${conversationId}`,
            headers: {
    ...
    while (!['succeeded', 'failed'].includes(getConversationStatus(conversation.body)) && retries < maxRetries) {
