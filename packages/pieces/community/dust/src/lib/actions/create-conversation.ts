...
import { assistantProp, createClient, getConversationContent, timeoutProp, timezoneProp, usernameProp } from '../common'
...
export const createConversation = createAction({
    ...
    props: {
        ...
        fileId: Property.ShortText({
            displayName: 'File ID',
            required: false,
            description: 'ID of the file to be added to the conversation (takes precedence over Fragment)',
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.query) {
            return await getConversationContent(conversationId, propsValue.timeout, auth.props)
        } else {
