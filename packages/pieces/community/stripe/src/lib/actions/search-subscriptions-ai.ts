...
export const stripeSearchSubscriptionsAi = createAction({
  ...
  description:
    'Search subscriptions by price ID, status, customer ID and other filters.',
  audience: 'ai',
  aiMetadata: {
    description:
      "Lists and filters Stripe subscriptions by price ID, status, customer ID, and creation date range, optionally expanding full customer details. Use to find subscriptions matching criteria or audit a customer's subscriptions; use Get Subscription when you already have the sub_ ID. Read-only and idempotent.",
    idempotent: true,
  },
  props: {
  ...
  async run(context) {
    ...
    while (hasMore && requestCount < maxRequests) {
      ...
      const subscriptions = subscriptionsResponse.body
          ...
          (subscription: Stripe.Subscription) => {
            return subscription.items.data.some(
              (item: Stripe.SubscriptionItem) =>
                ...
                priceIdArray.includes(item.price.id)
            );
          }
        );
      }
        ...
        if (include_customer_details) {
          if (
            ...
            try {
              ...
              resultSubscription.customer =
                customerResponse.body as Stripe.Customer;
            } catch (error) {
              console.warn(
                `Failed to fetch customer details for ${subscription.customer}:`,
                error
              );
            }
          }
