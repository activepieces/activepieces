...
import { DropdownState, PiecePropValueSchema, Property } from '@activepieces/pieces-framework'
...
export const stripeCommon = {
    ...
    subscribeWebhook: async (eventName: string, webhookUrl: string, apiKey: string): Promise<{ id: string }> => {
    ...
    customer: Property.Dropdown({
        ...
        options: async ({ auth, searchValue }): Promise<DropdownState<string>> => {
            ...
            const request: HttpRequest = {
                ...
                url: searchValue ? `${baseUrl}/customers/search` : `${baseUrl}/customers`,
            ...
            if (response.status === 200) {
                return {
                    disabled: false,
                    options: response.body.data.map((customer: StripeCustomerSearchResult) => ({
                        value: customer.id,
            ...
            if (response.status === 200) {
                return {
                    ...
                    options: response.body.data.map((price) => {
                        const label = price.nickname ? `${price.nickname} (${price.product.name})` : price.product.name
                        return {
                            value: price.id,
                            label: label,
                        }
            ...
            try {
                ...
                return {
                    ...
                    options: subscriptions.data.map((sub) => {
                        ...
                        const customerInfo = customer?.name || customer?.email || 'Unknown Customer'
    ...
    paymentIntent: Property.Dropdown({
        ...
        options: async ({ auth, searchValue }): Promise<DropdownState<string>> => {
            ...
            if (response.status === 200) {
                return {
                    disabled: false,
                    options: response.body.data.map((intent: StripePaymentIntentSearchResult) => {
                        const customerInfo = intent.customer?.name || intent.customer?.email || 'Unknown Customer'
