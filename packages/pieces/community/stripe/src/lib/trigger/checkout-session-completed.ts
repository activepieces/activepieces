...
export const stripeCheckoutSessionCompleted = createTrigger({
    ...
    async onEnable(context) {
        ...
        await context.store.put<StripeWebhookInformation>('_checkout_session_completed_trigger', {
            webhookId: webhook.id,
        })
    },

    async onDisable(context) {
        const webhookInfo = await context.store.get<StripeWebhookInformation>('_checkout_session_completed_trigger')
        if (webhookInfo !== null && webhookInfo !== undefined) {
            await stripeCommon.unsubscribeWebhook(webhookInfo.webhookId, context.auth.secret_text)
        }
    },
    async test(context) {
