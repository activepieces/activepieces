...
export const updateCompany = createAction({
    ...
    async run(context) {
        const { companyId, name, domainName, address, employees } = context.propsValue
        ...
        if (Object.keys(body).length === 0) {
            return await twentyRequest(context.auth, HttpMethod.GET, `/rest/companies/${companyId}`)
        }

        return await twentyRequest(context.auth, HttpMethod.PATCH, `/rest/companies/${companyId}`, body)
    },
})
