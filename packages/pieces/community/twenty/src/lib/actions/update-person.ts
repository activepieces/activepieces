...
import { parseSingleRecord, twentyRequest } from '../common'
...
export const updatePerson = createAction({
    ...
    async run(context) {
        ...
        if (Object.keys(body).length === 0) {
            return await twentyRequest(context.auth, HttpMethod.GET, `/rest/people/${personId}`)
        }

        return await twentyRequest(context.auth, HttpMethod.PATCH, `/rest/people/${personId}`, body)
    },
})
