...
export const findScreen = createAction({
    ...
    async run(context) {
        ...
        const screens =
            (await makeRequest<any[]>(context.auth.secret_text, HttpMethod.GET, `/projects/${projectId}/screens`)) || []

        if (screenName) {
            const filtered = screens.filter((screen) => screen.title.toLowerCase().includes(screenName.toLowerCase()))
