import { createAction, ActionContext, Property } from "@activepieces/pieces-framework";
...
export const findCustomerAction = createAction({
    ...
    async run(context: ActionContext<typeof quickbooksAuth>) {
        ...
        const realmId = context.auth.data['realmId'];

        if (!realmId) {
            throw new Error('Realm ID not found in authentication data. Please reconnect your account.');
        }

        const apiUrl = quickbooksCommon.getApiUrl(context.auth.data['environment'], realmId);
        const query = `SELECT * FROM Customer WHERE DisplayName = '${search_term.replace(/'/g, "\\'")}' MAXRESULTS 1`;
        ...
        if (response.body?.QueryResponse?.['Customer'] && response.body.QueryResponse['Customer'].length > 0) {
