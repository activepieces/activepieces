...
import { QuickbooksAccount, QuickbooksVendor, QuickbooksPurchase, QuickbooksRef } from "../lib/types";

// Define expected structure of auth data
...
export const createExpenseAction = createAction({
    ...
    props: {
        accountRef: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                const query = `SELECT Id, Name, AccountType FROM Account WHERE AccountType = 'Bank' OR AccountType = 'Credit Card' STARTPOSITION 1 MAXRESULTS 1000`;
                ...
                if (response.body.Fault) {
                    throw new Error(`QuickBooks API Error fetching accounts: ${response.body.Fault.Error.map((e: { Message: string; }) => e.Message).join(', ')}`);
                }
        ...
        entityRef: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                if (response.body.Fault) {
                    throw new Error(`QuickBooks API Error fetching vendors: ${response.body.Fault.Error.map((e: { Message: string; }) => e.Message).join(', ')}`);
                }
        ...
        lineItems: Property.Array({
            ...
            properties: {
                ...
                detailType: Property.StaticDropdown({
                    ...
                    options: {
                        options: [
                            { label: 'Account Based Expense Line Detail', value: 'AccountBasedExpenseLineDetail' },
                        ]
                    },
                ...
                expenseAccountId: Property.ShortText({
                    ...
                    description: "Enter the ID of the Expense Account. Required for AccountBasedExpenseLineDetail.",
                    required: true,
                }),
            }
        }),
    ...
    async run(context: ActionContext<typeof quickbooksAuth>) {
        ...
        const lines = (props['lineItems'] as any[]).map(line => {
            ...
            if (line['detailType'] === 'AccountBasedExpenseLineDetail') {
                if (!line['expenseAccountId']) {
                    throw new Error('Expense Category/Account ID is required for Account Based Expense Line Detail.');
                }
        ...
        if (response.body.Fault) {
            throw new Error(`QuickBooks API Error creating expense: ${response.body.Fault.Error.map((e: any) => e.Message).join(', ')} - Detail: ${response.body.Fault.Error.map((e: any) => e.Detail).join(', ')}`);
        }
