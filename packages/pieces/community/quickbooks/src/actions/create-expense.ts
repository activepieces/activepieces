...
import { QuickbooksAccount, QuickbooksPurchase, QuickbooksRef, QuickbooksVendor } from '../lib/types'
...
export const createExpenseAction = createAction({
    ...
    async run(context) {
        ...
        if (response.body.Fault) {
            throw new Error(
                `QuickBooks API Error creating expense: ${response.body.Fault.Error.map((e: any) => e.Message).join(
                    ', ',
                )} - Detail: ${response.body.Fault.Error.map((e: any) => e.Detail).join(', ')}`,
            )
        }
