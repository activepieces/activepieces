...
export const findInvoiceAction = createAction({
    ...
    async run(context: ActionContext<typeof quickbooksAuth>) {
        ...
        const query = `SELECT * FROM Invoice WHERE DocNumber = '${invoice_number.replace(/'/g, "\\'")}' MAXRESULTS 1`;
        ...
        if (response.body?.QueryResponse?.['Invoice'] && response.body.QueryResponse['Invoice'].length > 0) {
