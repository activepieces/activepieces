import { createAction, ActionContext, Property, OAuth2PropertyValue } from "@activepieces/pieces-framework";
...
export const createInvoiceAction = createAction({
        ...
        customerRef: Property.Dropdown({
            ...
            options: async ({ auth }) => {
                ...
                if (response.body.Fault) {
                    throw new Error(`QuickBooks API Error: ${response.body.Fault.Error.map((e: { Message: string; }) => e.Message).join(', ')}`);
                }
        ...
        lineItems: Property.Array({
            ...
            properties: {
                ...
                itemId: Property.ShortText({
                    ...
                    description: 'Enter the ID of the Item (Product/Service). Required for SalesItemLineDetail.',
                    required: true,
                }),
                ...
                unitPrice: Property.Number({
                    ...
                    description: 'If specified, Amount will be Qty * UnitPrice. If Amount is also specified, Amount overrides calculation.',
                    required: false,
                }),
            }
        }),
    ...
    async run(context: ActionContext<typeof quickbooksAuth>) {
        ...
        if (response.body.Fault) {
            throw new Error(`QuickBooks API Error: ${response.body.Fault.Error.map((e: any) => e.Message).join(', ')} - ${response.body.Fault.Error.map((e: any) => e.Detail).join(', ')}`);
        }
