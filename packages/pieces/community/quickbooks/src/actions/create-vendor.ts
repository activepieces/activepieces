...
export const createVendorAction = createAction({
	...
	props: {
		...
		termRef: Property.Dropdown({
			...
			options: async ({ auth }) => {
				...
				const response = await httpClient.sendRequest<QuickbooksEntityResponse<QuickbooksTerm>>({
				...
				const terms = response.body.QueryResponse?.['Term'] ?? [];
				return {
					disabled: false,
	...
	async run(context) {
		...
		if (response.body.Fault) {
			throw new Error(
				`QuickBooks API Error creating vendor: ${response.body.Fault.Error.map(
					(e) => e.Message,
				).join(', ')} - Detail: ${response.body.Fault.Error.map((e) => e.Detail).join(', ')}`,
			);
		}

		return response.body.Vendor;
	},
});
