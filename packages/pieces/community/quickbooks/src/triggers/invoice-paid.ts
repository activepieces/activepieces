...
6.  **Select Events:** Choose the **Payment** entity and select the desired operations (e.g., Create, Update). **Ensure you select at least the 'Create' event for this trigger.** This trigger fires when a payment is recorded.
...
export const invoicePaid = createTrigger({
    auth: quickbooksAuth,
    name: 'invoice_paid', // Or new_payment?
    displayName: 'Invoice Paid / Payment Received (Webhook)',
    description: 'Triggers when a Payment is created or updated in QuickBooks (requires webhook setup).',
    props: {
    ...
    async onEnable(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<void> {
        ...
        await context.store.put('quickbooks_payment_verifier_token', verifierToken); // Unique store key
        console.info(`QuickBooks 'Invoice Paid / New Payment' Webhook Enabled. User needs to configure endpoint in QuickBooks UI: ${context.webhookUrl}`);
    },
    async onDisable(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<void> {
        await context.store.delete('quickbooks_payment_verifier_token'); // Unique store key
        console.info(`QuickBooks 'Invoice Paid / New Payment' Webhook Disabled. Please manually remove the webhook from your QuickBooks Developer Dashboard: ${context.webhookUrl}`);
    },
    async run(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<QuickBooksEntity[]> {
        ...
        const storedToken = await context.store.get<string>('quickbooks_payment_verifier_token'); // Unique store key
        ...
        console.log('QuickBooks Payment Webhook Payload Received & Verified:', context.payload.body);
        ...
        for (const notification of notificationPayload.eventNotifications) {
            if (notification.dataChangeEvent?.entities) {
                for (const entity of notification.dataChangeEvent.entities) {
                    if (entity.name === 'Payment') { // Filter for Payment entity
        ...
        if (relevantEntities.length === 0) {
            console.log("No relevant Payment entities found in the webhook payload.");
            return [];
        }

        // Note: The payload only contains the ID. User will need a subsequent
        // 'Find Payment' action step to get full details like linked invoice, amount etc.
        console.log(`Returning ${relevantEntities.length} payment events.`);
                    ...
                    "entities": [
                        {
                            ...
                            "id": "456",
                            "operation": "Create",
                            "lastUpdated": "2024-07-31T13:00:00Z"
                        }
                    ]
                }
            }
