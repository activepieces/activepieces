...
6.  **Select Events:** Choose the **Deposit** entity and select the desired operations (e.g., Create, Update). **Ensure you select at least the 'Create' event for this trigger.** This trigger fires when a deposit is recorded.
...
export const newDeposit = createTrigger({
    auth: quickbooksAuth,
    name: 'new_deposit',
    displayName: 'New or Updated Deposit (Webhook)',
    description: 'Triggers when a Deposit is created or updated in QuickBooks (requires webhook setup).',
    props: {
    ...
    async onEnable(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<void> {
        ...
        await context.store.put('quickbooks_deposit_verifier_token', verifierToken); // Unique store key
        console.info(`QuickBooks 'New Deposit' Webhook Enabled. User needs to configure endpoint in QuickBooks UI: ${context.webhookUrl}`);
    },
    async onDisable(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<void> {
        await context.store.delete('quickbooks_deposit_verifier_token'); // Unique store key
        console.info(`QuickBooks 'New Deposit' Webhook Disabled. Please manually remove the webhook from your QuickBooks Developer Dashboard: ${context.webhookUrl}`);
    },
    async run(context: TriggerHookContext<typeof quickbooksAuth, Record<string, any>, TriggerStrategy.WEBHOOK>): Promise<QuickBooksEntity[]> {
        ...
        const storedToken = await context.store.get<string>('quickbooks_deposit_verifier_token'); // Unique store key
        ...
        console.log('QuickBooks Deposit Webhook Payload Received & Verified:', context.payload.body);
        ...
        for (const notification of notificationPayload.eventNotifications) {
            if (notification.dataChangeEvent?.entities) {
                for (const entity of notification.dataChangeEvent.entities) {
                    if (entity.name === 'Deposit') { // Filter for Deposit entity
        ...
        if (relevantEntities.length === 0) {
            console.log("No relevant Deposit entities found in the webhook payload.");
            return [];
        }

        // Note: The payload only contains the ID. User will likely need a subsequent
        // action step to get full details.
        console.log(`Returning ${relevantEntities.length} deposit events.`);
                    ...
                    "entities": [
                        {
                            ...
                            "id": "101",
                            "operation": "Create",
                            "lastUpdated": "2024-07-31T15:00:00Z"
                        }
                    ]
                }
            }
