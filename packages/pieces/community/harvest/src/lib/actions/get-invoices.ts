import { getAccessTokenOrThrow, HttpMethod, propsValidation } from '@activepieces/pieces-common'
...
export const getInvoices = createAction({
    ...
    async run(context) {
        ...
        const response = await callHarvestApi(HttpMethod.GET, `invoices`, getAccessTokenOrThrow(context.auth), params)
