...
export const searchObjects = createAction({
    ...
    async run(context) {
        const { bucket, prefix, matchGlob, delimiter, includeFoldersAsPrefixes, versions, pageToken, maxResults } =
            context.propsValue
        ...
        try {
            ...
            if (error.response?.status === 403) {
                throw new Error(
                    'Access denied. You need storage.objects.list permission to search objects in this bucket.',
                )
            }
