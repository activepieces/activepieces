...
import { aclEntityProperty, bucketDropdown, objectDropdown, projectIdProperty } from '../common/props'
        ...
        try {
            ...
            if (error.response?.status === 400) {
                if (entity.includes('user-') && entity.includes('@')) {
                    throw new Error(
                        'Cannot remove OWNER access from the object owner. The only way to remove owner access is to delete or replace the object.',
                    )
                }
                throw new Error(
                    "Bad request. This bucket may have uniform bucket-level access enabled, which doesn't support object ACLs. Use IAM policies instead.",
                )
            }
            if (error.response?.status === 403) {
                throw new Error(
                    'Access denied. You need storage.objects.setIamPolicy permission or OWNER ACL permission on the object.',
                )
            }
