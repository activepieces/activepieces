...
import { bucketNameProperty, locationProperty, projectIdProperty, storageClassProperty } from '../common/props'
...
export const createBucket = createAction({
    ...
    async run(context) {
        const { projectId, name, location, storageClass, versioning, uniformBucketLevelAccess, labels } =
            context.propsValue
