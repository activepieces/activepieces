...
export const searchBuckets = createAction({
    ...
    async run(context) {
        const { projectId, prefix, includeSoftDeleted, projection, pageToken, maxResults } = context.propsValue
        ...
        try {
            ...
            if (error.response?.status === 403) {
                throw new Error(
                    'Access denied. You need storage.buckets.list permission to search buckets in this project.',
                )
            }
