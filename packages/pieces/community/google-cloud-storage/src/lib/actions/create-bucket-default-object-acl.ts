...
import { aclEntityProperty, bucketDropdown, objectAclRoleProperty, projectIdProperty } from '../common/props'
    ...
    async run(context) {
        ...
        try {
            ...
            if (error.response?.status === 400) {
                throw new Error(
                    "Bad request. This bucket may have uniform bucket-level access enabled, which doesn't support default object ACLs. Use IAM policies instead.",
                )
            }
            if (error.response?.status === 403) {
                throw new Error(
                    'Access denied. You need storage.buckets.update permission to modify bucket default object ACLs.',
                )
            }
