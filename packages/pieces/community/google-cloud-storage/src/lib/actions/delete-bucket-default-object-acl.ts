...
export const deleteBucketDefaultObjectAcl = createAction({
  ...
  description: 'Remove a default ACL setting from a bucket.',
  props: {
  ...
  async run({ auth, propsValue }) {
    ...
    await client.deleteBucketDefaultObjectAcl(propsValue.bucket, entity);

    return {
        success: true,
        message: `Default object ACL entry for entity "${entity}" was removed successfully from bucket "${propsValue.bucket}".`
    };
  },
});