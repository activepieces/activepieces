...
export const createBucketAcl = createAction({
  ...
  description: 'Add an ACL entry at the bucket level.',
  props: {
    ...
    role: Property.StaticDropdown({
        ...
        options: {
            options: [
                { label: 'Reader (Can list objects)', value: 'READER' },
                { label: 'Writer (Can create/overwrite/delete objects)', value: 'WRITER' },
  ...
  async run({ auth, propsValue }) {
    ...
    return await client.createBucketAcl(propsValue.bucket, {
        entity: entity,
        role: propsValue.role as 'READER' | 'WRITER' | 'OWNER',
    });
  },
});