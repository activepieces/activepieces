...
import { aclEntityProperty, bucketAclRoleProperty, bucketDropdown, projectIdProperty } from '../common/props'
    ...
    async run(context) {
        ...
        try {
            ...
            if (error.response?.status === 400) {
                throw new Error(
                    "Bad request. This bucket may have uniform bucket-level access enabled, which doesn't support bucket ACLs. Use IAM policies instead.",
                )
            }
