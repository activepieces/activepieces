...
export const sendFromTemplate = createAction({
	...
	props: {
		...
		template_params: Property.DynamicProperties({
			...
			props: async (propsValue) => {
				...
				if (
					propsValue &&
					propsValue['listTemplate'] &&
					propsValue['listTemplate']['template'] &&
					propsValue['listTemplate']['template']['components'] &&
					propsValue['listTemplate']['template']['components'].length > 0
				) {
					...
					for (let i = 0; i < template.length; i++) {
						...
						if (element['type'] == 'HEADER') {
							...
							Object.assign(properties, {
								headerMarkdown: Property.MarkDown({
							...
							if (examples && examples['body_text'] && examples['body_text'].length > 0) {
								Object.assign(properties, {
									templateParameters: Property.MarkDown({
	...
	async run(context) {
		...
		const { to, listTemplate, template_params, phone_number_id } = context.propsValue;

		const components = [];

		if (
			listTemplate['template'] &&
			listTemplate['template']['components'] &&
			listTemplate['template']['components'].length > 0
		) {
