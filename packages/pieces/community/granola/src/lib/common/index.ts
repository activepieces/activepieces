...
function flattenNote(note: GranolaNote) {
    return {
        ...
        scheduled_start_time: note.calendar_event?.scheduled_start_time ?? null,
        scheduled_end_time: note.calendar_event?.scheduled_end_time ?? null,
        attendees: note.attendees ? note.attendees.map((a) => a.name ?? a.email).join(', ') : null,
        folders: note.folder_membership ? note.folder_membership.map((f) => f.name).join(', ') : null,
...
function flattenNoteWithTranscript(note: GranolaNote) {
    ...
    return {
        ...flat,
        transcript: note.transcript ? note.transcript.map((t) => `[${t.speaker.source}] ${t.text}`).join('\n') : null,
    }
}

export type { GranolaListResponse, GranolaNote }
