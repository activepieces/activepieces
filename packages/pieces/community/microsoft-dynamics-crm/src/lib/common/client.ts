...
export class DynamicsCRMClient {
  ...
  async updatedRecord(entityUrlPath: string, recordId: string, request: unknown) {
    return await this.makeRequest(
      HttpMethod.PATCH,
  ...
  async getRecord(entityUrlPath: string, recordId: string) {
    return await this.makeRequest(HttpMethod.GET, `/${entityUrlPath}(${recordId})`)
  }

  async deleteRecord(entityUrlPath: string, recordId: string) {
    return await this.makeRequest(HttpMethod.DELETE, `/${entityUrlPath}(${recordId})`)
  }
  async fetchEntityTypes(): Promise<EntityTypeResponse> {
    // fetch entity type data
    return await this.makeRequest<EntityTypeResponse>(HttpMethod.GET, `/EntityDefinitions`, undefined, {
      $select: ['EntitySetName', 'LogicalName'].join(','),
    })
  }
  async fetchEntityTypeAttributes(entitySetName: string): Promise<EntityTypeAttributesResponse> {
    return await this.makeRequest<EntityTypeAttributesResponse>(HttpMethod.GET, `/EntityDefinitions`, undefined, {
      $select: ['PrimaryIdAttribute', 'PrimaryNameAttribute', 'LogicalName'].join(','),
      $filter: `EntitySetName eq '${entitySetName}'`,
    })
  }
  async fetchEntityAttributes(entityName: string): Promise<EntityAttributeResponse> {
