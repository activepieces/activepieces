...
import { fieldIdDropdown, KnackGetObjectResponse, knackTransformFields, objectDropdown } from '../common/props'
    ...
    async run({ propsValue, auth }) {
        ...
        try {
            ...
            if (response.records && response.records.length > 0) {
                ...
                const transformedRecord = knackTransformFields(objectDetails, response.records[0])
                return {
                    found: true,
