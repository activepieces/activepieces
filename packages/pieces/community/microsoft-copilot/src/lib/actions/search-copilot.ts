...
export const searchCopilot = createAction({
    ...
    props: {
        query: Property.LongText({
            ...
            description: 'Natural language query to search for relevant files. Maximum 1,500 characters.',
            required: true,
        }),
        ...
        resourceMetadata: Property.Json({
            ...
            description: 'Array of metadata field names to include in results. Example: ["title", "author"]',
            required: false,
        }),
    },
    async run(context) {
        ...
        if (filterExpression || resourceMetadata) {
            ...
            if (resourceMetadata) {
                ...
                if (metadataArray.length > 0) {
                    requestBody.dataSources.oneDrive.resourceMetadataNames = metadataArray
                }
            }
        }
