...
import { getAccessToken, googleDriveAuth } from '../auth'
    ...
    async run(context) {
        ...
        if (context.propsValue.downloadFiles) {
            ...
            for (const fileWithLevel of filesWithLevel) {
                ...
                try {
                    const fileUrl = await downloadFileFromDrive(context.auth, context.files, file.id, file.name)
                    downloadedFiles.push(fileUrl)
                } catch (error) {
                    console.warn(
                        `Failed to download file ${file.name}: ${error instanceof Error ? error.message : 'Download failed'}`,
                    )
                }
            }
