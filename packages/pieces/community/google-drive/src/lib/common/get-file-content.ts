...
async function getMimeType(auth: GoogleDriveAuthValue, fileId: string): Promise<string> {
    const accessToken = await getAccessToken(auth)
    const mimeType = (
        await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=mimeType&supportsAllDrives=true`, {
            headers: {
