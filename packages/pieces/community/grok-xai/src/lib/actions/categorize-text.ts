...
export const categorizeText = createAction({
    ...
    props: {
        ...
        customInstructions: Property.LongText({
            ...
            defaultValue:
                'Analyze the text and assign it to the most appropriate category based on its content, tone, and context.',
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        const categoryDescriptions = categoriesArray.map((cat) => `${cat.name}: ${cat.description}`).join('\n')
