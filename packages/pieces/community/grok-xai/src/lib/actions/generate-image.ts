...
import { createModelProperty } from '../common/utils'
...
export const generateImage = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        const { prompt, model, numberOfImages, responseFormat } = propsValue
        ...
        try {
            ...
            if (error.message?.includes('timeout')) {
                throw new Error(
                    'Image generation timed out. The request may have been too complex. Try simplifying your prompt.',
                )
            }
