...
import { Bucket, Cluster, Collection, connect, DurabilityLevel, GetResult, MutationResult, Scope } from 'couchbase'
...
export async function createCouchbaseClient(auth: CouchbaseAuthValue): Promise<Cluster> {
...
export function formatGetResult(result: GetResult, documentId: string): { id: string; content: unknown; cas: string } {
    return {
        id: documentId,
...
export const collectionDropdown = Property.Dropdown({
    ...
    options: async ({ auth, bucket, scope }) => {
        ...
        try {
            ...
            const selectedScope = scopes?.find((s) => s.name === (scope || '_default'))
...
export const couchbaseCommonProps = {
    ...
    query: Property.LongText({
        ...
        description:
            'SELECT statement with optional positional parameters ($1, $2, etc.). Do not include LIMIT or OFFSET.',
        required: true,
    }),
