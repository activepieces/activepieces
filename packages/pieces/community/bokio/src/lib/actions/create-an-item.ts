...
export const createAnItem = createAction({
    ...
    async run(context) {
        ...
        if (dynamicProps?.['unitPrice'] !== undefined && dynamicProps?.['unitPrice'] !== null) {
            body.unitPrice = dynamicProps['unitPrice']
        }

        if (dynamicProps?.['taxRate'] !== undefined && dynamicProps?.['taxRate'] !== null) {
            body.taxRate = dynamicProps['taxRate']
        }

        const response = await makeRequest(api_key, HttpMethod.POST, `/companies/${companyId}/items`, body)
