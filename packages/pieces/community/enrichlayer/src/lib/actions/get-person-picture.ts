...
export const getPersonPicture = createAction({
    ...
    description: 'Get the profile picture URL of a person from cached profiles (0 credits)',
    props: {
        person_profile_url: Property.ShortText({
            displayName: 'Person Profile URL',
            description: 'Professional network person URL (e.g., https://www.linkedin.com/in/williamhgates/)',
            required: true,
        }),
    },
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.PERSON_PICTURE, {
