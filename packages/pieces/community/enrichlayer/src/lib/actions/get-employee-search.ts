...
export const getEmployeeSearch = createAction({
    ...
    props: {
        ...
        keyword_regex: Property.ShortText({
            ...
            description: 'Job title keyword in regex format (e.g., ceo|cto). Prefer Boolean keyword instead.',
            required: false,
        }),
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.EMPLOYEE_SEARCH, {
