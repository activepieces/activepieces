...
export const getPersonProfile = createAction({
    ...
    description: 'Get structured data of a Person Profile from a professional network URL (1 credit)',
    props: {
        ...
        extra: Property.StaticDropdown({
            ...
            description: 'Enrich with gender, birth date, industry, and interests (+1 credit)',
            required: false,
            options: {
                options: [
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.PERSON_PROFILE, {
