...
export const getReverseEmailLookup = createAction({
    ...
    props: {
        email: Property.ShortText({
            ...
            description: 'Email address to look up (e.g., johndoe@enrichlayer.com)',
            required: true,
        }),
        lookup_depth: Property.StaticDropdown({
            displayName: 'Lookup Depth',
            description: 'Superficial: no credits if no results. Deep: credits used regardless.',
            required: false,
            options: {
                options: [
        ...
        enrich_profile: Property.StaticDropdown({
            ...
            description: 'Enrich the result with cached profile data (+1 credit)',
            required: false,
            options: {
                options: [
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.REVERSE_EMAIL_LOOKUP, {
