...
export const getReversePhoneLookup = createAction({
    ...
    description: 'Find social media profiles from a phone number (3 credits)',
    props: {
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.REVERSE_PHONE_LOOKUP, {
