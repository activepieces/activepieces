...
export const getPersonalEmail = createAction({
    ...
    description: 'Find personal email addresses from a social media profile (1 credit per email returned)',
    props: {
        ...
        page_size: Property.ShortText({
            ...
            description: 'Maximum emails returned (0 = no limit, default: 0)',
            required: false,
        }),
    },
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.PERSONAL_EMAIL, {
