...
export const getPersonLookup = createAction({
    ...
    description: 'Look up a person by name and company information (2 credits)',
    props: {
        ...
        title: Property.ShortText({
            displayName: 'Title',
            description: 'Title the person holds at their current job (e.g., Co-chair)',
            required: false,
        }),
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.PERSON_LOOKUP, {
