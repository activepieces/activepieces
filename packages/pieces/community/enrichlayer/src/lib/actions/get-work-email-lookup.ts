...
export const getWorkEmailLookup = createAction({
    ...
    props: {
        profile_url: Property.ShortText({
            ...
            description: 'Professional network profile URL (e.g., https://sg.linkedin.com/in/williamhgates)',
            required: true,
        }),
        callback_url: Property.ShortText({
            displayName: 'Callback URL',
            description: 'Webhook URL to notify when the request finishes processing (optional)',
            required: false,
        }),
    },
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.WORK_EMAIL_LOOKUP, {
