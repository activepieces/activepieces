...
export const getJobSearch = createAction({
    ...
    description: 'List jobs posted by a company with optional filters (2 credits)',
    props: {
        search_id: Property.ShortText({
            displayName: 'Company Search ID',
            description: 'The search_id of the company (from Company Profile API)',
            required: false,
        }),
        ...
        geo_id: Property.ShortText({
            displayName: 'Geo ID',
            description: 'Geographic location ID (e.g., 92000000 for worldwide)',
            required: false,
        }),
    },
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.JOB_SEARCH, {
