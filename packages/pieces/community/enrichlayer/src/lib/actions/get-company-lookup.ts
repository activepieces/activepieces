...
export const getCompanyLookup = createAction({
    ...
    description: 'Resolve a Company Profile from company name, domain, or location (2 credits)',
    props: {
        company_name: Property.ShortText({
            displayName: 'Company Name',
            description: 'Name of the company. Requires either Company Name or Company Domain.',
            required: false,
        }),
        ...
        enrich_profile: Property.StaticDropdown({
            ...
            description: 'Enrich the result with cached company profile data (+1 credit)',
            required: false,
            options: {
                options: [
    ...
    async run(context) {
        return await enrichlayerApiCall(context.auth.secret_text as string, ENDPOINTS.COMPANY_LOOKUP, {
