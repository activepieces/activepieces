...
export const ghostcms = createPiece({
  ...
  actions: [
    ...
    createCustomApiCallAction({
      baseUrl: (auth) => `${(auth as { baseUrl: string }).baseUrl}/ghost/api/admin`,
      auth: ghostAuth,
      authMapping: async (auth) => ({
        Authorization: `Ghost ${common.jwtFromApiKey((auth as { apiKey: string }).apiKey)}`,
      }),
    }),
  ],
  triggers: [memberAdded, memberEdited, memberDeleted, postPublished, postScheduled, pagePublished],
})
