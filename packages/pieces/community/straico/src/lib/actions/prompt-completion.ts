...
export const promptCompletion = createAction({
    ...
    props: {
        ...
        fileUrls: Property.Array({
            ...
            required: false,
            description:
                'URLs of files to be processed by the model (maximum 4 URLs), previously uploaded via the File Upload endpoint',
        }),
        ...
        imageUrls: Property.Array({
            ...
            required: false,
            description:
                'URLs of images to be processed by the model, previously uploaded via the File Upload endpoint',
        }),
        displayTranscripts: Property.Checkbox({
            displayName: 'Display Transcripts',
            required: false,
            description:
                'If true, returns transcripts of the files. Note: Either File URLs or YouTube URLs are required when this is enabled',
            defaultValue: false,
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue.displayTranscripts === true && !propsValue.fileUrls?.length && !propsValue.youtubeUrls?.length) {
        ...
        const response = await httpClient.sendRequest<{
            ...
            body: {
                ...
                ...(propsValue.displayTranscripts !== undefined &&
