...
export const getStrkPriceAction = createAction({
  name: 'get_strk_price',
  displayName: 'Get STRK Price',
  description: 'Fetch the current price and market data for StarkNet (STRK) token from CoinGecko.',
  props: {
  ...
  async run(context) {
    ...
    const response = await httpClient.sendRequest({
      ...
      url: 'https://api.coingecko.com/api/v3/coins/starknet',
      queryParams: {
        localization: 'false',
        tickers: 'false',
