...
export const createJob = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (
            propsValue['scheduled_start'] ||
            propsValue['scheduled_end'] ||
            propsValue['arrival_window'] !== undefined
        ) {
            jobData['schedule'] = {}
        ...
        const response = await makeHousecallProRequest(auth, '/jobs', HttpMethod.POST, jobData)
