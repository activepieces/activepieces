...
export const createEstimate = createAction({
    ...
    props: {
        estimate_number: Property.Number({
            ...
            required: false,
            description: 'Unique estimate number. If blank, one will be automatically generated.',
        }),
    ...
    async run({ auth, propsValue }) {
        ...
        if (propsValue['estimate_number'] !== undefined) body['estimate_number'] = propsValue['estimate_number']
        ...
        const response = await makeHousecallProRequest(auth, '/estimates', HttpMethod.POST, body)
