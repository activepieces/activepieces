...
function normalizeBaseUrl({
  ...
  if (parsed.search || parsed.hash) {
    throw new Error(`${fieldName} must not include query parameters or a fragment.`);
  }

  if (parsed.pathname !== '/') {
    throw new Error(`${fieldName} must contain only the server origin, without a path.`);
  }
...
async function apiCall<T extends HttpMessageBody>({
  ...
  const baseUrl = normalizeBaseUrl({ value: apiUrl, fieldName: 'API URL' });
