...
export const createPost = createAction({
    ...
    props: {
        ...
        user_email: Property.ShortText({
            ...
            description:
                'Email of an existing community member to create this post as. If empty, posts as the authenticated admin.',
            required: false,
        }),
    },
    async run(context) {
        ...
        if (tiptap_body_json && typeof tiptap_body_json === 'object' && (tiptap_body_json as any).body) {
            ...
            throw new Error(
                "Invalid body input. Provide either 'Post Body (Plain Text)' or a valid 'Tiptap Body JSON'. If both are empty, an empty post will be created.",
            )
        }
