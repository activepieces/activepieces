...
import {
    firstSeenAtFromField,
    firstSeenAtUntilField,
    lastSeenAtFromField,
    lastSeenAtUntilField,
    limitField,
    makeClient,
    pageField,
} from '../common'
...
export const findJobOpeningsAction = createAction({
    ...
    props: {
        onet_codes: Property.Array({
            ...
            description:
                'O*NET codes to filter by, such as "17-2071.00" for "Electrical Engineers" or "15-1254.00" for "Web Developers". For full list of possible codes see: https://www.onetonline.org/find/all',
            required: false,
        }),
        page: pageField,
        limit: limitField,
    ...
    async run(context) {
        ...
        try {
            const response = await client.findJobOpenings(
                prepareQuery({
                    page,
                    limit,
                    onet_codes,
