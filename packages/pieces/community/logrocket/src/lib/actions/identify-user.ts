...
export const identifyUser = createAction({
    ...
    props: {
        ...
        appId: Property.ShortText({
            displayName: 'App ID',
            description:
                'Your LogRocket app/project ID (found in Settings > General Settings, or in your App ID after the slash)',
            required: true,
        }),
        ...
        traits: Property.Object({
            displayName: 'Traits',
            description:
                'User traits as key-value pairs (e.g., {"plan": "free", "age": 43}). Values will be converted to strings.',
            required: false,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            throw new Error(`Failed to identify user: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    },
})
