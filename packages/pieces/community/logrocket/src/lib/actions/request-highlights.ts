...
export const requestHighlights = createAction({
    ...
    props: {
        orgId: Property.ShortText({
            ...
            description:
                'Your LogRocket organization ID (found in Settings > General Settings, or in your App ID before the slash)',
            required: true,
        }),
        projectId: Property.ShortText({
            displayName: 'Project ID',
            description:
                'Your LogRocket project ID (found in Settings > General Settings, or in your App ID after the slash)',
            required: true,
        }),
        ...
        webhookUrl: Property.ShortText({
            ...
            description:
                'URL where highlights results will be posted when ready. You can get this from the "Highlights Ready" trigger.',
            required: true,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            throw new Error(`Failed to request highlights: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    },
})
