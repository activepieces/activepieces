...
export const createProductFromProductInfo = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        const hasUrl = !!url
        const hasProductInfo = !!(name || description || target_audience || media?.length)

        if (!hasUrl && !hasProductInfo) {
            throw new Error('You must provide either a URL or product information (name, description, etc.)')
        }
        ...
        await propsValidation.validateZod(propsValue, {
            ...
            target_audience: z.string().min(1, 'Target audience cannot be empty').optional(),
            media: z
                .array(
                    z.object({
