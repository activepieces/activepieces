...
export const createAiAvatarPhoto = createAction({
    ...
    async run({ auth, propsValue }) {
        const { age, appearance, aspect_ratio, avatar_style, background, ethnicity, gender, image_url, model } =
            propsValue

        await propsValidation.validateZod(propsValue, {
            image_url: z.string().url('Image URL must be a valid URL').optional(),
            appearance: z.string().min(1, 'Appearance description cannot be empty').optional(),
            background: z.string().min(1, 'Background description cannot be empty').optional(),
        })
