...
export const createVideoFromTemplate = createAction({
        ...
        template_id: Property.Dropdown({
            ...
            async options({ auth, template_type }) {
                ...
                try {
                    ...
                    return {
                        options: templates.map((template: { id: number; name: string }) => ({
                            label: template.name,
                            value: template.id,
                        })),
                    }
        ...
        variables: Property.Array({
            ...
            properties: {
                ...
                name: Property.ShortText({
                    ...
                    description: 'Name of the variable to replace (as defined in template)',
                    required: true,
                }),
        ...
        avatar_id: Property.Dropdown({
            ...
            async options({ auth, avatar_type }) {
                ...
                try {
                    ...
                    return {
                        options: avatars.map((avatar: { avatar_id: number; name: string }) => ({
                            label: avatar.name,
    ...
    async run({ auth, propsValue }) {
            ...
            variables && variables.length > 0
                ? (
                      ...
                      if (variable.type === 'text' || variable.type === 'script') {
                          if (!variable.content) {
                              throw new Error(`Content is required for ${variable.type} variables`)
                          }
                          properties.content = variable.content
                      } else if (variable.type === 'image' || variable.type === 'video') {
                          if (!variable.url) {
                              throw new Error(`URL is required for ${variable.type} variables`)
                          }
        ...
        if (response.body.code !== 0) {
            ...
            const message = errorMessages[response.body.code] || `API Error: ${response.body.msg}`
