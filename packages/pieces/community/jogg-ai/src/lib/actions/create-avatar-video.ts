...
export const createAvatarVideo = createAction({
        ...
        avatar_id: Property.Dropdown({
            ...
            async options({ auth, avatar_type }) {
                ...
                try {
                    ...
                    return {
                        options: avatars.map((avatar: { avatar_id: number; name: string; status?: number }) => ({
                            label: `${avatar.name}${
                                avatar.status !== undefined ? ` (${avatar.status === 1 ? 'Ready' : 'Processing'})` : ''
                            }`,
    ...
    async run({ auth, propsValue }) {
        const { screen_style, avatar_id, avatar_type, voice_id, script, audio_url, aspect_ratio, caption, video_name } =
            propsValue
