...
export const videoGenerationFailed = createTrigger({
    ...
    async onEnable(context) {
        try {
            ...
            throw new Error(
                `Failed to register webhook with JoggAI: ${error instanceof Error ? error.message : 'Unknown error'}`,
            )
        }
    },
    ...
    async run(context) {
        ...
        if (!payload.event_id || !payload.event || !payload.timestamp || !payload.data?.project_id) {
