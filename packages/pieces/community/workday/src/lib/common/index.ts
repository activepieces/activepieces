...
export async function fetchAllPages<T>(
    ...
    while (hasMore) {
        ...
        const total = typeof response.body['total'] === 'number' ? response.body['total'] : allItems.length
        offset += limit
...
function buildSoapEnvelope(username: string, password: string, operationXml: string): string {
