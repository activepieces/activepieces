...
export const findRecordsWql = createAction({
    ...
    description: 'Executes a Workday Query Language (WQL) query and returns the results.',
    props: {
    ...
    async run(ctx) {
        const response = await workdayWqlRequest(ctx.auth as OAuth2PropertyValue, ctx.propsValue.query)
