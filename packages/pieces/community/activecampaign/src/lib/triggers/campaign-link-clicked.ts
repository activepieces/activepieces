...
export const campaignLinkClickedTrigger = createTrigger({
  ...
  description: 'Triggers when a contact clicks a link in a campaign message (will only run once for each unique link).',
  ...
  async onDisable(context) {
    const webhook = await context.store.get<CreateWebhookResponse>('activecampaign_campaign_link_clicked')
    if (webhook != null) {
      const client = makeClient(context.auth)
