...
const polling: Polling<AppConnectionValueForAuthProperty<typeof presentonAuth>, Record<string, never>> = {
    strategy: DedupeStrategy.TIMEBASED,
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        const apiKey = auth.secret_text

        const response: any = await makeRequest(apiKey, HttpMethod.GET, '/ppt/presentation/all')
