...
export const generatePresentations = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (slides_markdown) body['slides_markdown'] = slides_markdown as unknown as string[]
        ...
        try {
            const response = await makeRequest(apiKey, HttpMethod.POST, '/ppt/presentation/generate/async', body)
            ...
            while (Date.now() - start < timeoutMs) {
                const statusResp = await makeRequest(apiKey, HttpMethod.GET, `/ppt/presentation/status/${taskId}`)
