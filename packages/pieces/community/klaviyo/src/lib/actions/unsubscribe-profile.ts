...
export const unsubscribeProfile = createAction({
    ...
    props: {
        ...
        unsubscribe_sms_transactional: Property.Checkbox({
            ...
            description: 'Unsubscribe selected profiles from SMS transactional messages.',
            required: false,
            defaultValue: false,
        }),
    },
    async run({ auth, propsValue }) {
        ...
        const profileDetails = await Promise.all(
            (profile_ids as string[]).map((id) =>
                makeRequest(auth as unknown as KlaviyoAuthValue, HttpMethod.GET, `/profiles/${id}`, {}),
            ),
        )
        ...
        const profileData = (profile_ids as string[]).map((profileId, index) => {
            ...
            if ((unsubscribe_sms || unsubscribe_sms_transactional) && attrs.phone_number) {
