...
export const subscribeProfile = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        if (!subscribe_email && !subscribe_sms) {
            throw new Error('At least one subscription channel must be selected (Email or SMS).')
        }
        if (historical_import && !consented_at) {
            throw new Error('Consent Date is required when Historical Import is enabled.')
        }
        ...
        const profileData = (profile_ids as string[]).map((profileId) => {
            ...
            if (subscribe_sms && attrs.phone_number) profileAttrs.phone_number = attrs.phone_number
