...
export const createProfileAction = createAction({
	...
	async run(context) {
		const { email, phoneNumber, externalId, firstName, lastName, organization, title, properties } = context.propsValue;
		...
		return await client.createProfile(profileAttributes);
	},
});
