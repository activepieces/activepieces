...
export const createProfile = createAction({
    ...
    async run({ auth, propsValue }) {
        ...
        const hasLocationData =
            address1 || address2 || city || country || region || zip || timezone || ip || latitude || longitude
        if (hasLocationData) {
            attributes['location'] = {}
        ...
        return await makeRequest(auth as KlaviyoAuthValue, HttpMethod.POST, endpoint, body)
    },
})
