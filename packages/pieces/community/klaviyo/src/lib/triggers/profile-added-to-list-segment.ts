...
import { KlaviyoAuthValue, klaviyoAuth } from '../common/auth'
...
const polling: Polling<KlaviyoAuthValue, StaticPropsValue<typeof props>> = {
    ...
    items: async ({ auth, propsValue, lastFetchEpochMS }) => {
        ...
        const basePath = targetType === 'list' ? `/lists/${targetId}/profiles` : `/segments/${targetId}/profiles`
        ...
        let response = await makeRequest(auth, HttpMethod.GET, `${basePath}?${queryParams.toString()}`)
        ...
        return allProfiles.map((profile) => {
            ...
            return {
                ...
                data: {
                    ...
                    full_name:
                        [profile.attributes.first_name, profile.attributes.last_name].filter(Boolean).join(' ') || null,
