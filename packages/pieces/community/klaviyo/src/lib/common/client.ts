...
import { getAuthorizationHeader, KlaviyoAuthValue } from './auth'
...
export async function makeRequest(auth: KlaviyoAuthValue, method: HttpMethod, path: string, body?: unknown) {
    try {
        ...
        if (error.response?.status >= 400 && error.response?.status < 500) {
            throw new Error(`Client error: ${error.response?.body?.message || JSON.stringify(error.response?.body)}`)
        }
