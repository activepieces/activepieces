...
import { AppConnectionValueForAuthProperty, OAuth2AuthorizationMethod, PieceAuth } from '@activepieces/pieces-framework'
    ...
    PieceAuth.SecretText({
        ...
        validate: async ({ auth }) => {
            try {
                ...
                return {
                    valid: false,
                    error: 'Invalid Private API Key. Check your key in Klaviyo → Settings → API Keys.',
                }
            }
        },
    }),
...
export type KlaviyoAuthValue = AppConnectionValueForAuthProperty<typeof klaviyoAuth>
