...
export const addOrUpdateContact = createAction({
    ...
    props: {
                    ...
                    for (const iteration of PROPERTY_ITERATIONS) {
                        for (const spec of response.body.specs) {
                            ...
                            if (spec.multivalue > 1) {
                                ...
                                const PropType = LONG_TEXT_TYPES.includes(spec.dataType)
                                    ? Property.LongText
                                    : Property.ShortText
        ...
        instant: Property.Checkbox({
            displayName: 'Instant',
            description:
                'Process contact immediately instead of queuing. Only enable this when you need to add an event for this contact in the next step. Not recommended for high-volume operations.',
            required: false,
            defaultValue: false,
        }),
    },
