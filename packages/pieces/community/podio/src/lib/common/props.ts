        ...
        try {
            ...
            switch (refType) {
                case 'item': {
                    ...
                    return {
                        options:
                            itemResponse.items?.map((item: any) => ({
                                label: item.title || `Item ${item.item_id}`,
                                value: item.item_id,
                            })) || [],
                    }
                        ...
                        if (statusResponse?.items) {
                            return {
                                options: statusResponse.items
                                    ...
                                    .map((item: any) => ({
                                        label:
                                            (item.data.value || item.data.text || '').substring(0, 60) +
                                                (item.data.value?.length > 60 ? '...' : '') ||
                                            `Status ${item.data.status_id}`,
            ...
            return {
                options: files.map((file: any) => {
                    ...
                    if (file.size) {
                        ...
                        const sizeText = sizeInKB > 1024 ? `${Math.round(sizeInKB / 1024)}MB` : `${sizeInKB}KB`
                        label += ` (${sizeText})`
                    }
