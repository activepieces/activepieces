...
export async function podioApiCall<T = any>(options: PodioApiCallOptions): Promise<T> {
    ...
    try {
        ...
        switch (statusCode) {
            case 400:
                throw new Error(
                    `Bad Request: ${getErrorMessage(errorBody) || 'Invalid request parameters. Please check your input values.'}`,
                )

            case 401:
                throw new Error(
                    'Authentication failed. Your Podio access token is invalid or expired. Please reconnect your Podio account.',
                )

            case 403:
                throw new Error(
                    `Access denied: ${getErrorMessage(errorBody) || 'You do not have permission to perform this action. Check your user permissions in Podio.'}`,
                )

            case 404:
                throw new Error(
                    `Not found: ${getErrorMessage(errorBody) || 'The requested resource was not found. Please verify the ID is correct.'}`,
                )

            case 409:
                throw new Error(
                    `Conflict: ${getErrorMessage(errorBody) || 'The item has been modified by someone else. Please refresh and try again.'}`,
                )
            ...
            case 422:
                throw new Error(
                    `Validation error: ${getErrorMessage(errorBody) || 'The provided data is invalid. Please check required fields and formats.'}`,
                )
            ...
            default:
                throw new Error(
                    `Request failed (${statusCode}): ${getErrorMessage(errorBody) || error.message || 'Unknown error occurred'}`,
                )
        }
    }
}
