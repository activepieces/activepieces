...
export const createItemAction = createAction({
    ...
    props: {
        ...
        legacyFields: Property.Object({
            ...
            description:
                'Advanced: Use this for complex field configurations. Format: {"field_id": {"value": "content"}}. Only use if the dynamic fields above don\'t meet your needs.',
            required: false,
        }),
        ...
        recurrence: Property.Object({
            ...
            description:
                'The recurrence for the task, if any. Format: {"name": "weekly|monthly|yearly", "config": {...}, "step": 1, "until": "date"}',
            required: false,
        }),
    ...
    async run(context) {
        ...
        if (appFields && Object.keys(appFields).length > 0) {
            try {
                ...
                throw new Error(
                    `Failed to process app fields: ${error instanceof Error ? error.message : 'Unknown error'}`,
                )
            }
        }
        ...
        if (Object.keys(formattedFields).length === 0) {
            throw new Error(
                'At least one field value is required to create an item. Please configure the app fields or provide legacy field data.',
            )
        }
