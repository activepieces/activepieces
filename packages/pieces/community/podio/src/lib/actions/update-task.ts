...
import {
    dynamicAppProperty,
    dynamicOrgProperty,
    dynamicRefIdProperty,
    dynamicRefTypeProperty,
    dynamicSpaceProperty,
    dynamicTaskProperty,
    getAccessToken,
    hookProperty,
    podioApiCall,
...
export const updateTaskAction = createAction({
    ...
    props: {
        ...
        reminder: Property.Object({
            ...
            description:
                'Optional reminder on this task. Format: {"remind_delta": minutes_before_due_date}. If empty, existing reminder is deleted.',
            required: false,
        }),
        hook: hookProperty,
    ...
    async run(context) {
        ...
        if (refType && !refId) {
            throw new Error(
                'Reference Object is required when Reference Type is specified. Please select an object from the dropdown.',
            )
        }

        if (!refType && refId) {
            throw new Error(
                'Reference Type is required when Reference Object is specified. Please select a reference type first.',
            )
        }
        ...
        if ((refType === 'status' || refType === 'task') && !spaceId) {
            throw new Error(
                'Space selection is required when referencing status updates or tasks. Please select a space first.',
            )
        }
        ...
        if (reminder && typeof reminder === 'object' && reminder['remind_delta'] !== undefined) {
            if (typeof reminder['remind_delta'] !== 'number' && reminder['remind_delta'] !== null) {
                throw new Error(
                    'Reminder remind_delta must be a number (minutes before due date) or null to delete existing reminder.',
                )
            }
        }
        ...
        if (labels && Array.isArray(labels) && labels.length > 0) {
            body.labels = labels.filter(
                (label: unknown) => (typeof label === 'string' && label.trim().length > 0) || typeof label === 'number',
            )
            updateCount++
        }

        if (fileIds && Array.isArray(fileIds) && fileIds.length > 0) {
            body.file_ids = fileIds.filter(
        ...
        if (updateCount === 0) {
            throw new Error(
                'At least one field must be provided to update the task. Please specify which fields you want to modify.',
            )
        }
