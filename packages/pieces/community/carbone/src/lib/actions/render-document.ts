...
export const renderDocumentAction = createAction({
    ...
    async run(context) {
        ...
        const contentType = renderResponse.headers?.['content-type'] ?? 'application/octet-stream'
        const contentDispositionHeader = renderResponse.headers?.['content-disposition'] ?? ''
