...
export const listTemplatesAction = createAction({
    ...
    async run(context) {
        ...
        if (includeVersions !== undefined) queryParams['includeVersions'] = String(includeVersions)
        ...
        if (!response.body.success) {
            throw new Error(`Failed to list templates: ${response.body.error ?? 'Unknown error'}`)
        }
