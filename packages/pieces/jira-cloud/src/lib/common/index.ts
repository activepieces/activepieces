import { AuthenticationType, HttpMessageBody, HttpMethod, HttpRequest, QueryParams, httpClient } from '@activepieces/pieces-common';
...
export async function executeJql({
    ...
    queryParams,
    body
}: { auth: JiraAuth, jql: string, sanitizeJql: boolean, url: string, method: HttpMethod, queryParams?: QueryParams, body?: HttpMessageBody }) {
    let reqJql = jql;
    if (sanitizeJql) {
        const sanitizeResult = (await sendJiraRequest({
            auth: auth,
            url: 'jql/sanitize',
...
export async function searchIssuesByJql({ auth, jql, maxResults, sanitizeJql }: { auth: JiraAuth, jql: string, maxResults: number, sanitizeJql: boolean }) {
    return (await executeJql({
        auth,
        url: 'search',
        method: HttpMethod.POST,
        jql,
