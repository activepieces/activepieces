import { PiecePropValueSchema, Property, StaticPropsValue, TriggerStrategy, createTrigger } from "@activepieces/pieces-framework";
import { Polling, DedupeStrategy, pollingHelper } from "@activepieces/pieces-common";
...
const polling: Polling<PiecePropValueSchema<typeof jiraCloudAuth>, { jql?: string, sanitizeJql?: boolean }> = {
    ...
    items: async ({ auth, lastFetchEpochMS, propsValue }) => {
        ...
        const searchQuery = `${jql ? jql + ' AND ' : ''}updated > '${dayjs(lastFetchEpochMS).format('YYYY-MM-DD HH:mm')}'`
