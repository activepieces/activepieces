...
export const translateText = createAction({
	...
	props: {
        ...
        split_sentences: Property.StaticDropdown({
            ...
            description: 'Sets whether the translation engine should first split the input into sentences. For text translations where tag_handling is not set to html, the default value is 1, meaning the engine splits on punctuation and on newlines.',
            required: false,
            options: {
                options: [
                    {value: '0' , label: 'No splitting at all, whole input is treated as one sentence'},
                    {value: '1' , label: 'Splits on punctuation and on newlines'},
                    {value: 'nonewlines' , label: 'Splits on punctuation only, ignoring newlines'}
                ]
            }
        }),
        ...
        formality: Property.StaticDropdown({
            ...
            options: {
                options: [
                    ...
                    {value: 'prefer_more' , label: 'For a more formal language if available, otherwise fallback to default formality'},
                    {value: 'prefer_less' , label: 'For a more informal language if available, otherwise fallback to default formality'},
                ]
            }
        }),
