...
export const textToImage = createAction({
    ...
    props: {
        ...
        accept_format: Property.StaticDropdown({
            ...
            description: 'json will return the image as base64, and png will return the image as binary',
            required: true,
            options: {
                options: [
                    {
    ...
    async run(context) {
        const { prompt, cfg_scale, clip_guidance_preset, height, width, samples, steps, accept_format, style_preset, engine_id } = context.propsValue;
        ...
        try {
            const response = await axios.post(`${apiHost}/v1/generation/${engineId}/text-to-image`, requestBody, {
                headers: {
