...
export const campaignChatEmailCompletedTrigger = createTrigger({
    auth: PieceAuth.None(),
    name: 'campaign_chat_email_completed_trigger', // Unique name across the piece.
    displayName: 'Campaign Chat Completed With Email Address', // Display name on the interface.
    description: 'Triggers when the campaign chat was completed but with the received client email.', // Description for the action
    props: {
    ...
    async test(ctx) {
        return await pollingHelper.test(polling, {
            auth: ctx.auth,
            store: ctx.store,
            propsValue: Object.assign(ctx.propsValue, {type: 'campaign_chats_email_latest'})
        });
    },
    async onEnable(ctx) {
