...
export function prepareQueryRequest(request?: ListRequest|Record<string,any|undefined>): QueryParams {
    const params: QueryParams = {}
    if(!request)
        return params
    const requestObj = request as Record<string, any>
    Object.keys(request).filter(k => k != 'filter').filter(emptyValueFilter(k => requestObj[k])).forEach((k: string) => {
    ...
    if(request.filter) {
        ...
        Object.keys(request.filter).filter(emptyValueFilter(k => filter[k])).forEach(k => {
