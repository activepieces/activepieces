...
import { KizeoFormsDataUsers, KizeoFormsExports, KizeoFormsForms, KizeoFormsList, KizeoFormsLists } from "./models";
...
export const kizeoFormsCommon = {
      ...
      try {
        ...
        if (lists) {
          return {
            disabled: false,
            options: lists.map(
              (lists) => ({ value: lists.id, label: lists.name })
            )
          }
        }
      } catch (e) {
  ...
  async fetchList({ token, listId }: { token: string, listId: string }): Promise<KizeoFormsList> {
