    ...
    implements Omit<PieceBase, 'version' | 'name'>
{
    ...
    constructor(
        ...
        if (
            !semver.valid(minimumSupportedRelease) ||
            semver.lt(minimumSupportedRelease, MINIMUM_SUPPORTED_RELEASE_AFTER_LATEST_CONTEXT_VERSION)
        ) {
...
type BackwardCompatiblePieceMetadata = Omit<
    PieceMetadata,
    'name' | 'version' | 'authors' | 'i18n' | 'getContextInfo'
> & {
