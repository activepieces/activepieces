    ...
    for (const [name, property] of entries) {
        switch (property.type) {
            ...
            case PropertyType.NUMBER:
                propsSchema[name] = z.union([property.required ? z.string().min(1) : z.string(), z.number()])
                break
            case PropertyType.STATIC_DROPDOWN:
            case PropertyType.DROPDOWN:
                propsSchema[name] = z.unknown().refine((val) => val !== null && val !== undefined, {
            ...
            case PropertyType.ARRAY: {
                ...
                propsSchema[name] = z.union([
                    property.required ? z.array(arrayItemSchema).min(1) : z.array(arrayItemSchema),
            ...
            case PropertyType.STATIC_MULTI_SELECT_DROPDOWN:
                propsSchema[name] = z.union([
                    property.required ? z.array(z.any()).min(1) : z.array(z.any()),
                    property.required ? z.string().min(1) : z.string(),
                ])
                break
