...
export const tablesCommon = {
    ...
    createFieldValidations(tableFields: Field[]) {
        ...
        tableFields.forEach((field) => {
            switch (field.type) {
                case FieldType.NUMBER:
                    fieldValidations[field.externalId] = z
                        .union([
                            z.number(),
    ...
    async convertTableExternalIdToId(
        tableId: string,
        context: { server: { apiUrl: string; token: string }; project: { id: string } },
    ) {
...
const fetchAllTables = async (context: {
    server: { apiUrl: string; token: string }
    project: { id: string }
}): Promise<Table[]> => {
    const res = await httpClient.sendRequest({
