...
export function pageRangeToIndexes(startPage: number, endPage: number, totalPages: number) {
    if (startPage > endPage) {
        throw Error(`Range start (${startPage}) has to be less than range end (${endPage})`)
    }
    ...
    if (startPage > totalPages || endPage > totalPages) {
        throw Error('Range start/end has to be less or equal to the total number of pages')
    }
...
export const extractPdfPages = createAction({
    ...
    async run(context) {
        try {
            ...
            const pageIndexes = context.propsValue.pageRanges.flatMap((pageRange: any) =>
