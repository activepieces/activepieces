...
export const delayUntilAction = createAction({
  ...
  aiMetadata: { description: 'Suspends the flow until one absolute date/time (ISO and other parseable formats) and then continues with the next step; waits longer than a minute suspend the run rather than sleeping in-process. Choose this when the resume point is a known calendar instant, and prefer Delay For when you only know a relative duration. Requires the target timestamp, which must be parseable and in the future - an unparseable value or one that has already passed throws instead of continuing - and the wait cannot exceed the instance paused-flow timeout; idempotent, it changes no data.', idempotent: true },
  ...
  props: {
    ...
    delayUntilTimestamp: Property.DateTime({
      ...
      description:
        'Specifies the date and time until which the execution of the next action should be delayed. It supports multiple formats, including ISO format. Must be in the future — a timestamp that has already passed fails the step.',
      required: true,
    }),
  },
  ...
  async run(ctx) {
    ...
    if (ctx.executionType == ExecutionType.RESUME) {
      // MUST stay above the past-timestamp check: on resume the target is in
      // the past by definition, so checking first would fail every paused run
      // exactly when it wakes up.
      return {
        delayTill: delayTill,
        success: true,
      };
    ...
    if (delayInMs <= 0) {
      throw new Error(
        `Date and Time is in the past: "${ctx.propsValue.delayUntilTimestamp}" resolved to ${delayTill.toISOString()}, which has already passed. Provide a future timestamp.`
      );
    }
  ...
  async test(ctx) {
    const delayTill = parseTimestampOrThrow(ctx.propsValue.delayUntilTimestamp);
    if (delayTill.getTime() - Date.now() <= 0) {
      throw new Error(
        `Date and Time is in the past: "${ctx.propsValue.delayUntilTimestamp}" resolved to ${delayTill.toISOString()}, which has already passed. Provide a future timestamp.`
      );
    }
