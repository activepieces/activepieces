...
export const parseUrl = createAction({
    ...
    props: {
        ...
        returnArrays: Property.Checkbox({
            ...
            description:
                'If checked (recommended), all query parameters are returned as JSON arrays to safely group duplicate keys (e.g., {"tags": ["shoes", "hats"]}). If unchecked, standard web behavior applies, returning only the first value as text.',
            required: false,
            defaultValue: true,
        }),
    },
    async run(context) {
        ...
        try {
            ...
            throw new Error(
                `Failed to parse URL. Please ensure it is a valid, absolute URL. Details: ${(error as Error).message}`,
            )
        }
    },
})
