...
export async function deliverSubflowResponse({
    ...
    if (acknowledgement.body?.discarded === true) {
        throw new Error(
            'The calling flow was no longer waiting for a response, so this response was discarded. Open the calling run to see why it stopped waiting.'
        );
    }
}
