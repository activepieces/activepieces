...
import { BlobReader, BlobWriter, EntryGetDataOptions, getMimeType, ZipReader } from '@zip.js/zip.js'
    ...
    async run(context) {
        ...
        for (const entry of entries) {
            if (!entry.directory) {
                ...
                const blob = await entry.getData(new BlobWriter(mimeType), getDataOptions)
