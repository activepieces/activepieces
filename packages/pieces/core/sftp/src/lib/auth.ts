...
export const sftpAuth = PieceAuth.CustomAuth({
    props: {
        ...
        password: PieceAuth.SecretText({
            ...
            description:
                'The password to authenticate with. Either this or private key is required. When using a private key, this field is used as the passphrase to decrypt the key.',
            required: false,
        }),
        ...
        algorithm: Property.StaticMultiSelectDropdown({
            ...
            description:
                'The host key algorithm to use for SFTP connection. Only needed if you want to override the default algorithms (e.g., to support ssh-dss).',
            required: false,
            options: {
                options: [
