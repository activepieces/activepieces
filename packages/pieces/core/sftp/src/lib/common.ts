...
export async function getClient<T extends Client | FTPClient>(auth: {
    ...
    const {
        ...
        host,
        port,
        allow_unauthorized_certificates,
    ...
    if (protocolBackwardCompatibility === 'sftp') {
        ...
        if (privateKey) {
            ...
            if (
                ...
                const beginMatch = processedKey.match(
                    /(-----BEGIN [A-Z0-9 ]+ KEY-----\n(?:Proc-Type:[^\n]+\n)?(?:DEK-Info:[^\n]+\n\n)?)/,
                )
                ...
                if (beginMatch && endMatch) {
                    ...
                    const base64Content = processedKey.replace(header, '').replace(footer, '').replace(/\s+/g, '') // Remove all whitespace from base64
