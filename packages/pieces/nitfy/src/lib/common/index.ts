...
import { HttpMethod, HttpMessageBody, HttpResponse, httpClient, AuthenticationType } from "@activepieces/pieces-common";
...
export const nitfyCommon = {
    ...
    status: Property.Dropdown({
        description: 'Nitfy Status to create the task in',
        displayName: 'Status',
        required: true,
        refreshers: [ 'project'],
        options: async ({ auth , project }) => {
            ...
            const response = (await callNitfyApi<{
                ...
                hasMore: boolean,
            }>(HttpMethod.GET, `taskgroups?project_id=${project}&archived=false`, accessToken, undefined )).body;
    ...
    milestone: Property.Dropdown({
        description: 'Nitfy Milestone to create the task in',
        ...
        options: async ({ auth , project }) => {
            ...
            const response = (await callNitfyApi<{
                ...
                hasMore: boolean,
            }>(HttpMethod.GET, `milestones?project_id=${project}&is_list=true`, accessToken, undefined )).body;
