...
export const updateTeam = createAction({
    ...
    async run(context) {
        ...
        if (context.propsValue.workers) options.workers = context.propsValue.workers;
        if (context.propsValue.hub) options.hub = context.propsValue.hub;
        if (context.propsValue.enableSelfAssignment) options.enableSelfAssignment = context.propsValue.enableSelfAssignment;

        return await onfleetApi.teams.update(context.propsValue.team as string, options);
    }
})