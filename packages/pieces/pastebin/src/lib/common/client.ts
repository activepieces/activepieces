...
export class PastebinClient {
    ...
    async makeRequest(script: string, option: string, body: Record<string, any>): Promise<string> {
        ...
        Object.keys(body).filter(k => body[k] !== undefined && body[k] !== null).forEach(k => req.append('api_' + k, body[k]))
