...
class ActivepiecesEmbedded {
  ...
  private _openNewWindowForConnections({
    ...
    connectionName,
    newWindow,
  }: { pieceName: string; connectionName?: string; newWindow: newWindowFeatures }) {
  ...
  private _checkForClientRouteChanges = (source: Window) => {
    window.addEventListener('message', (event: MessageEvent<ActivepiecesClientRouteChanged>) => {
      if (event.data.type === ActivepiecesClientEventName.CLIENT_ROUTE_CHANGED && event.source === source) {
  ...
  extractActivepiecesRouteFromUrl({ vendorUrl }: { vendorUrl: string }) {
    ...
    return this._extractRouteAfterPrefix(
      vendorUrl,
      prefixStartsWithSlash ? this._parentOrigin + this._prefix : `${this._parentOrigin}/${this._prefix}`,
    )
  }
