...
class ActivepiecesEmbedded {
  ...
  _resolveNewConnectionDialogClosed?: (
    result: ActivepiecesNewConnectionDialogClosed['data']
  ) => void;
  ...
  private _initializeConnectionsIframe = ({
    jwtToken,
  }: {
    jwtToken: string;
  }) => {
    return this._addGracePeriodBeforeMethod({
      ...
      method: () => {
        this._connectionsIframe = this.connectToEmbed({
          jwtToken,
          iframeContainer: document.body,
          callbackAfterAuthentication: () => {
            this._connectionsIframeInitialized = true;
          },
        });
        const connectionsIframeStyle = [
          'display:none',
          'position:fixed',
          'top:0',
          'left:0',
      ...
      errorMessage:
        'document body not found while trying to add connections iframe',
    });
  };
  ...
  private connectToEmbed({
    ...
    const iframe = this._createIframe({
      src: `${this._instanceUrl}/embed?${jwtTokenQueryParamName}=${jwtToken}`,
    });
  ...
  async connect({ pieceName }: { pieceName: string }) {
    return this._addGracePeriodBeforeMethod({
      ...
      method: () => {
        if (
          !this._connectionsIframe ||
          !this._doesFrameHaveWindow(this._connectionsIframe)
        ) {
          console.error(
            'Activepieces: connections iframe not found, please make sure you enabled embedded dialiogs in the configure method'
          );
          return;
        }
