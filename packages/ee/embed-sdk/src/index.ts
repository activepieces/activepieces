...
export type ActivepiecesClientEvent = ActivepiecesClientInit | ActivepiecesClientRouteChanged
...
type EmbeddingParam = {
    ...
    builder?: {
        ...
        homeButtonIcon: 'back' | 'logo'
        homeButtonClickedHandler?: (data: { route: string }) => void
    }
    dashboard?: {
        hideSidebar?: boolean
        hideFlowsPageNavbar?: boolean
...
class ActivepiecesEmbedded {
    ...
    configure({ jwtToken, instanceUrl, embedding, prefix }: ConfigureParams) {
    ...
    private _initializeBuilderAndDashboardIframe = ({ containerSelector }: { containerSelector: string }) => {
    ...
    private _setupInitialMessageHandler(
        targetWindow: Window,
        initialRoute: string,
                        ...
                        const apEvent: ActivepiecesVendorInit = {
                            ...
                            data: {
                                ...
                                emitHomeButtonClickedEvent:
                                    this._embeddingState?.builder?.homeButtonClickedHandler !== undefined,
    ...
    private _createConfigurationFinishedListener = (
        targetWindow: Window,
        callbackAfterConfigurationFinished?: () => void,
    ) => {
        const configurationFinishedHandler = (event: MessageEvent<ActivepiecesClientConfigurationFinished>) => {
            if (
                event.data.type === ActivepiecesClientEventName.CLIENT_CONFIGURATION_FINISHED &&
    ...
    private _createAuthenticationFailedListener = (targetWindow: Window) => {
        const authenticationFailedHandler = (event: MessageEvent<ActivepiecesClientAuthenticationFailed>) => {
            if (
                event.data.type === ActivepiecesClientEventName.CLIENT_AUTHENTICATION_FAILED &&
                event.source === targetWindow
            ) {
    ...
    private _createAuthenticationSuccessListener = (targetWindow: Window) => {
        const authenticationSuccessHandler = (event: MessageEvent<ActivepiecesClientAuthenticationSuccess>) => {
            if (
                event.data.type === ActivepiecesClientEventName.CLIENT_AUTHENTICATION_SUCCESS &&
                event.source === targetWindow
            ) {
    ...
    private _addConnectionIframe({ pieceName, connectionName }: { pieceName: string; connectionName?: string }) {
        ...
        connectionsIframe.style.cssText = [
            'display:none',
            'position:fixed',
            'top:0',
            'left:0',
    ...
    private _openNewWindowForConnections({
        pieceName,
        connectionName,
        newWindow,
    }: {
        ...
        this._setupInitialMessageHandler(
            popup,
            `/embed/connections?${NEW_CONNECTION_QUERY_PARAMS.name}=${pieceName}&randomId=${Date.now()}&${NEW_CONNECTION_QUERY_PARAMS.connectionName}=${connectionName || ''}`,
        )
        return popup
    }
    async connect({
        ...
        return this._addGracePeriodBeforeMethod({
            ...
            method: async () => {
                const target = newWindow
                    ? this._openNewWindowForConnections({ pieceName, connectionName, newWindow })
                    : this._addConnectionIframe({ pieceName, connectionName })
    ...
    private _checkForClientRouteChanges = (source: Window) => {
        window.addEventListener('message', (event: MessageEvent<ActivepiecesClientRouteChanged>) => {
            if (
    ...
    private _checkForBuilderHomeButtonClicked = (source: Window) => {
        window.addEventListener('message', (event: MessageEvent<ActivepiecesBuilderHomeButtonClicked>) => {
            if (
                event.data.type === ActivepiecesClientEventName.CLIENT_BUILDER_HOME_BUTTON_CLICKED &&
                event.source === source
            ) {
    ...
    private _setConnectionIframeEventsListener(target: Window | HTMLIFrameElement) {
        const connectionRelatedMessageHandler = (
            event: MessageEvent<
    ...
    private async fetchEmbeddingAuth(params: { jwtToken: string } | undefined) {
        ...
        const response = await this.request(
            {
                path: '/managed-authn/external-token',
                method: 'POST',
                body: {
    ...
    async request(
        ...
        return fetch(
            `${this._removeTrailingSlashes(this._instanceUrl)}/api/v1/${this._removeStartingSlashes(path)}${queryParamsString}`,
            {
                method,
