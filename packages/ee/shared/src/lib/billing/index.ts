...
export function getPriceId(stripeKey: string | undefined) {
  return (
    {
    [PlanName.PLUS]: {
        [BilingCycle.MONTHLY]: stripeKey?.startsWith('sk_test') ? 'price_test_plus_monthly' : 'price_live_plus_monthly',
        [BilingCycle.ANNUAL]: stripeKey?.startsWith('sk_test') ? 'price_test_plus_annual' : 'price_live_plus_annual',
    },
    [PlanName.BUSINESS]: {
        [BilingCycle.MONTHLY]: stripeKey?.startsWith('sk_test') ? 'price_test_business_monthly' : 'price_live_business_monthly',
        [BilingCycle.ANNUAL]: stripeKey?.startsWith('sk_test') ? 'price_test_business_annual' : 'price_live_business_annual',
    },
    ADDONS: {
        EXTRA_USERS: stripeKey?.startsWith('sk_test') ? 'price_test_users' : 'price_live_users',
        EXTRA_FLOWS: stripeKey?.startsWith('sk_test') ? 'price_test_flows' : 'price_live_flows',
        EXTRA_AI_CREDITS: stripeKey?.startsWith('sk_test') ? 'price_test_ai_credits' : 'price_live_ai_credits',
    }
}
  )
}
