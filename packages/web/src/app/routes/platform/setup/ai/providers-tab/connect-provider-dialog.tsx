...
import { apId, formErrors, MarkdownVariant } from '@activepieces/shared';
...
import { MockProviderConfig } from '../mock/fixtures';
...
export function ConnectProviderDialog({
  ...
  editing?: MockProviderConfig;
  defaultProvider?: AIProviderName;
  onConnected: (config: MockProviderConfig) => void;
}) {
  return (
...
function ConnectProviderForm({
  ...
  const handleSubmit = (values: ConnectFormValues) => {
    ...
    const secret =
      secretKeys.map((key) => values.credentials[key] ?? '').find(Boolean) ??
      '';
    window.setTimeout(() => {
      setConnecting(false);
      if (secret.length < MIN_DEMO_KEY_LENGTH) {
        form.setError('root.serverError', {
          type: 'manual',
          message: t('The provider rejected these credentials.'),
        });
        return;
      }
      onConnected({
        ...
        modelScope: editing?.modelScope ?? 'all',
        modelIds: editing?.modelIds ?? [],
        projectScope: editing?.projectScope ?? 'all',
