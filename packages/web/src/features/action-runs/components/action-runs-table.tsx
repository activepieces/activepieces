...
import { actionRunsApi } from '@/features/action-runs/api/action-runs-api';
...
import { ActionRunDetailSheet } from './action-run-detail-sheet';
import {
  actionRunsColumns,
  formatSource,
  SelectedRow,
} from './action-runs-columns';

const ACTION_RUN_STATUSES = [
...
export const ActionRunsTable = () => {
  ...
  const [selectedRun, setSelectedRun] = useState<ActionRunListItem | null>(
    null,
  );
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['action-runs-table', searchParams.toString(), projectId],
    ...
    refetchInterval: (query) => {
      const allRuns = query.state.data?.data as ActionRunListItem[] | undefined;
    ...
    () => [
      {
        ...
        title: t('Status'),
        accessorKey: 'status',
        options: ACTION_RUN_STATUSES.map((status) => ({
      ...
      },
      {
        ...
        title: t('Source'),
        accessorKey: 'source',
        options: Object.values(ActionRunSource).map((source) => ({
  ...
  const archiveRuns = useMutation({
    mutationFn: () =>
      actionRunsApi.bulkArchive({
        projectId,
        actionRunIds: selectedAll
          ? undefined
          : selectedRows.map((row) => row.id),
        excludeActionRunIds: selectedAll ? Array.from(excludedRows) : undefined,
        status:
        ...
        source:
          searchParams.getAll('source').length > 0
            ? (searchParams.getAll('source') as ActionRunSource[])
            : undefined,
        userId:
  ...
  const bulkActions: BulkAction<ActionRunListItem>[] = useMemo(
    () => [
      {
  ...
  return (
    <>
      ...
      >
        ...
        <span>
          {t(
            'These are individual actions run outside of a flow by your chat agents, MCP servers, or the API.',
          )}
        </span>
      </div>
      <DataTable
