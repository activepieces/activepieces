...
function PieceCell({ run }: { run: ActionRunListItem }) {
  const { kind, actionName, pieceName, archivedAt } = run;
  const { pieceModel } = piecesHooks.usePiece({
    name: pieceName ?? '',
    enabled: kind === ActionRunKind.PIECE && !isNil(pieceName),
  });

  if (kind === ActionRunKind.CODE) {
    return (
...
export const actionRunsColumns = ({
  selectedRows,
  setSelectedRows,
  selectedAll,
  setSelectedAll,
  ...
  setExcludedRows,
  data,
}: ActionRunsColumnsProps): ColumnDef<
  RowDataWithActions<ActionRunListItem>
>[] => [
  {
    id: 'select',
    accessorKey: 'select',
...
export function formatSource(source: string): string {
  if (source === ActionRunSource.MCP || source === ActionRunSource.API) {
    return source;
  }
...
type ActionRunsColumnsProps = {
  data: SeekPage<ActionRunListItem> | undefined;
