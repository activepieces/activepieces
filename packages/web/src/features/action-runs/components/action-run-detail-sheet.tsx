...
import { actionRunsApi } from '../api/action-runs-api';

import { formatSource } from './action-runs-columns';

export const ActionRunDetailSheet = ({
  run,
  open,
  onOpenChange,
}: ActionRunDetailSheetProps) => {
  ...
  const { data: fullRun } = useQuery({
    queryKey: ['action-run-detail', run?.id],
    queryFn: () =>
      actionRunsApi.get({
        id: run!.id,
  ...
  const { pieceModel } = piecesHooks.usePiece({
    name: run?.pieceName ?? '',
    enabled: run?.kind === ActionRunKind.PIECE && !isNil(run?.pieceName),
  });
      ...
      <SheetContent className="w-[520px] sm:max-w-[520px] flex flex-col p-0">
        <SheetHeader className="px-6 py-4 border-b shrink-0">
          <SheetTitle className="text-base flex items-center gap-2">
            {!isNil(run) && run.kind === ActionRunKind.CODE && (
              <PieceIcon
                size="sm"
                border={true}
                showTooltip={false}
...
function buildTitle({
  ...
  pieceDisplayName,
  actionDisplayName,
}: BuildParams & { run: ActionRunListItem | null }): string {
  if (isNil(run)) {
    return t('Run Details');
  }
  if (run.kind === ActionRunKind.CODE) {
    return t('Code');
  }
...
function buildOverviewRows({
  ...
  pieceDisplayName,
  actionDisplayName,
}: BuildParams & { run: ActionRunListItem }): OverviewRow[] {
  const duration =
  ...
  if (run.kind === ActionRunKind.PIECE && !isNil(pieceDisplayName)) {
...
type ActionRunDetailSheetProps = {
  run: ActionRunListItem | null;
  open: boolean;
