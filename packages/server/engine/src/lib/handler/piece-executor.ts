        ...
        async () => {
            ...
            const context: ActionContext<PieceAuthProperty, InputPropertyMap> = {
                ...
                generateResumeUrl: (params) => {
                    const url = new URL(
                        `${constants.publicApiUrl}v1/flow-runs/${constants.flowRunId}/requests/${executionState.pauseRequestId}${params.sync ? '/sync' : ''}`,
                    )
            ...
            const runMethodToExecute =
                testSingleStepMode && !isNil(pieceAction.test) ? pieceAction.test : pieceAction.run
