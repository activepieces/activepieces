...
const executeAction: ActionHandler<PieceAction> = async ({ action, executionState, constants }) => {
    ...
    const { data: executionStateResult, error: executionStateError } = await utils.tryCatchAndThrowOnEngineError((async () => {
        ...
        if (params.hookResponse.type === 'stopped') {
            ...
            const succeeded = stepOutput.setOutput(output).setSensitiveOutputFields(utils.sensitiveOutputFields(pieceAction.outputSchema)).setStatus(StepOutputStatus.SUCCEEDED).setDuration(stepEndTime - stepStartTime)
        ...
        if (params.hookResponse.type === 'paused') {
            const paused = stepOutput.setOutput(output).setSensitiveOutputFields(utils.sensitiveOutputFields(pieceAction.outputSchema)).setStatus(StepOutputStatus.PAUSED).setDuration(stepEndTime - stepStartTime)
