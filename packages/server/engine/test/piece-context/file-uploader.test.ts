...
describe('file-uploader service', () => {
    ...
    it('throws when data is a plain Object', async () => {
        ...
        await expect(files.write({ fileName: 'test.txt', data: {} as any })).rejects.toThrow(
            'Expected file data to be a Buffer, but received [object Object]',
        )
    })

    it('throws when data is a string', async () => {
        const files = createFileUploader(SERVICE_PARAMS)
        await expect(files.write({ fileName: 'test.txt', data: 'hello' as any })).rejects.toThrow(
            'Expected file data to be a Buffer, but received string',
        )
    })

    it('throws when data is undefined', async () => {
        const files = createFileUploader(SERVICE_PARAMS)
        await expect(files.write({ fileName: 'test.txt', data: undefined as any })).rejects.toThrow(
            'Expected file data to be a Buffer, but received undefined',
        )
    })
    ...
    it('throws when file exceeds size limit', async () => {
        ...
        await expect(files.write({ fileName: 'big.bin', data: twoMbBuffer })).rejects.toThrow(FileSizeError)
    })

    it('direct upload returns result url', async () => {
        const mockUrl = 'http://localhost:3000/files/abc123'
        vi.spyOn(global, 'fetch').mockResolvedValue(
            new Response(JSON.stringify({ url: mockUrl, uploadUrl: null }), {
                status: 200,
    ...
    it('signed URL upload posts metadata then PUTs to signed URL', async () => {
        ...
        vi.spyOn(global, 'fetch')
            .mockResolvedValueOnce(
                new Response(JSON.stringify({ url: resultUrl, uploadUrl: signedUrl }), {
                    status: 200,
                    headers: { 'Content-Type': 'application/json' },
                }),
            )
    ...
    it('throws FileStoreError when upload URL is missing in signed mode', async () => {
        ...
        vi.spyOn(global, 'fetch').mockResolvedValue(
            new Response(JSON.stringify({ url: 'https://s3.example.com/files/abc123', uploadUrl: null }), {
                status: 200,
                headers: { 'Content-Type': 'application/json' },
            }),
        )
        ...
        const files = createFileUploader(SERVICE_PARAMS)
        await expect(files.write({ fileName: 'test.txt', data: Buffer.from('hello') })).rejects.toThrow(FileStoreError)
    })
    ...
    it('throws FileStoreError when signed URL PUT fails', async () => {
        ...
        vi.spyOn(global, 'fetch')
            .mockResolvedValueOnce(
                new Response(JSON.stringify({ url: 'https://s3.example.com/files/abc123', uploadUrl: signedUrl }), {
                    status: 200,
                    headers: { 'Content-Type': 'application/json' },
                }),
            )
