...
describe('pieceInstaller', () => {
    ...
    it('individual fallback always passes --filter path regardless of includeFilters', async () => {
        ...
        mockInstall
            .mockRejectedValueOnce(new Error('batch error'))  // batch (includeFilters: false → no filter)
            .mockResolvedValueOnce({ output: '' })             // piece-filter-a individual
            .mockResolvedValueOnce({ output: '' })             // piece-filter-b individual
        ...
        // Individual calls must always include the --filter path
