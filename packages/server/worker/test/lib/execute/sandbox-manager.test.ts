...
describe('sandbox-manager canReuseSandbox', () => {
    ...
    it('AP_REUSE_SANDBOX=false overrides trusted mode → sandbox NOT reusable', async () => {
        mockSystemGet.mockReturnValue('false')
        mockGetSettings.mockReturnValue(buildSettings({
            executionMode: ExecutionMode.SANDBOX_CODE_ONLY,
            environment: ApEnvironment.PRODUCTION,
        }))
    ...
    it('AP_REUSE_SANDBOX=true overrides non-trusted mode → sandbox reusable', async () => {
        mockSystemGet.mockReturnValue('true')
        mockGetSettings.mockReturnValue(buildSettings({
            executionMode: ExecutionMode.SANDBOX_PROCESS,
            environment: ApEnvironment.PRODUCTION,
        }))
