...
async function main(): Promise<void> {
    ...
    if (!apiKey) {
        fail('AP_API_KEY is not set. Run via `npm run agent-evals:live` (it sources .env.dev), and ensure the dev backend is running.')
    }
...
main().catch((error) => {
    console.error(chalk.red(`  agent-evals:live failed: ${error instanceof Error ? error.message : String(error)}`))
    process.exit(2)
})
