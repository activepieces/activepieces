import { PersistedAgentPartType, PersistedAgentRole, PersistedToolCallStatus } from '@activepieces/shared'
...
function toolCall({ toolName, input = {}, actionName, pieceName, outputText = '', status = PersistedToolCallStatus.COMPLETED }: {
    ...
    return { type: PersistedAgentPartType.TOOL_CALL, toolCallId: toolName, toolName, input: fullInput, status, output: { content: [{ type: 'text', text: outputText }] } }
}
...
describe('liveTagger.tag', () => {
    ...
    it('treats an ACTION_RECEIPT success as goal met even if no execute text is present', () => {
        const uiMessages = [assistant([
            ...
            { type: PersistedAgentPartType.ACTION_RECEIPT, toolCallId: 'r1', actionDisplayName: 'Send Message', pieceName: 'slack', status: 'success', timestamp: 't' },
        ])]
        const score = liveTagger.tag({ scenario: scenario(), uiMessages })
        expect(score.executeSucceeded).toBe(true)
        expect(score.gaveUp).toBe(false)
