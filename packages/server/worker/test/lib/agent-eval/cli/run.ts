...
async function run({ candidatePath, onProgress }: { candidatePath?: string, onProgress?: (message: string) => void }): Promise<RunResult> {
    ...
    try {
        for (const fixture of fixtures) {
            ...
            const baseline = await agentEvalRunner.evaluateFixture({ fixture, systemPrompt: sources.baseline.systemPrompt, guides: sources.baseline.guides })
            let candidate = baseline
            if (sources.changed) {
                onProgress?.(`evaluating ${fixture.id} · ${sources.candidate.label}`)
                candidate = await agentEvalRunner.evaluateFixture({ fixture, systemPrompt: sources.candidate.systemPrompt, guides: sources.candidate.guides })
            }
