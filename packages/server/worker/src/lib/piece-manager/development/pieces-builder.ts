...
import { ApLock, PiecesSource, filePiecesUtils, memoryLock } from '@activepieces/server-shared'
import { WebsocketClientEvent, debounce, isNil } from '@activepieces/shared'
...
async function handleFileChange(
  packages: string[],
  pieceProjectName: string,
  piecePackageName: string,
  io: Server,
  log: FastifyBaseLogger,
): Promise<void> {
  log.info(chalk.blueBright.bold('👀 Detected changes in pieces. Waiting... 👀 ' + pieceProjectName))
  ...
  try {
    ...
    log.info(chalk.green.bold('✨ Changes are ready! Please refresh the frontend to see the new updates. ✨'))
  }
}
...
export async function piecesBuilder(
  app: FastifyInstance,
  io: Server,
  packages: string[],
