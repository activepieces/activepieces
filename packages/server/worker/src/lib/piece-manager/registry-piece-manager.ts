...
import { PackageType, PiecePackage, PrivatePiecePackage, getPackageArchivePathForPiece } from '@activepieces/shared'
...
export class RegistryPieceManager extends PieceManager {
  protected override async installDependencies({ projectPath, pieces, log }: InstallParams): Promise<void> {
    ...
    try {
      ...
      await Promise.all(dependencies.map((pkg) => cache.setCache(pkg.alias, CacheState.READY)))
    } finally {
