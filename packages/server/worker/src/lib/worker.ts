...
async function startPollingWorkers(apiClient: WorkerToApiContract): Promise<void> {
    ...
    // Reuse the runtime across reconnects. This previously awaited runtime.shutdown()
    // before recreating it, which deadlocks whenever a prior-generation job is still
    // in-flight on that runtime (e.g. a long chat turn): the await never resolves, so
    // polling never restarts and the worker silently stops consuming jobs after any
    // transient Socket.IO disconnect.
    const activeRuntime = runtime ?? selectRuntime({ concurrency, log: logger })
