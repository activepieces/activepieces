import { WebhookJobData, pinoLogging } from '@activepieces/server-shared'
import {
  EngineHttpResponse,
  FlowStatus,
...
export const webhookExecutor = (log: FastifyBaseLogger) => ({
  async consumeWebhook(data: WebhookJobData, engineToken: string, workerToken: string): Promise<void> {
    ...
    const disabledFlow =
      flowVersionToRun === GetFlowVersionForWorkerRequestType.LOCKED && populatedFlowToRun.status !== FlowStatus.ENABLED
