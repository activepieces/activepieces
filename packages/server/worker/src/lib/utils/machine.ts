  ...
  init: async (_settings: WorkerMachineHealthcheckResponse) => {
    settings = {
      ...
      ...spreadIfDefined(
        ...
        environmentVariables.getNumberEnvironment(WorkerSystemProp.FLOW_WORKER_CONCURRENCY),
      ),
      ...spreadIfDefined(
        'SCHEDULED_WORKER_CONCURRENCY',
        environmentVariables.getNumberEnvironment(WorkerSystemProp.SCHEDULED_WORKER_CONCURRENCY),
      ),
    }
...
async function getContainerMemoryUsage() {
  ...
  const usedRamInBytes = memUsageExists
    ? parseInt(await fs.promises.readFile(memUsagePath, 'utf8'))
