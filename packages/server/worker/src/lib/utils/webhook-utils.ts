...
import { EventPayload, FlowId, FlowVersion, PopulatedFlow, TriggerType } from '@activepieces/shared'
...
export const webhookUtils = (log: FastifyBaseLogger) => ({
  ...
  async getWebhookUrl({ flowId, simulate, publicApiUrl }: GetWebhookUrlParams): Promise<string> {
  ...
  async extractPayload({
    ...
    return triggerConsumer.extractPayloads(engineToken, log, {
      projectId,
      flowVersion,
      payload,
      simulate,
  ...
  savePayloadsAsSampleData({ flowVersion, projectId, workerToken, payloads }: SaveSampleDataParams): void {
  ...
  async handshake({ populatedFlow, payload, engineToken }: HandshakeParams): Promise<WebhookResponse | null> {
