  ...
  async executeAction(engineToken, operation) {
    ...
    switch (step.type) {
      ...
      case ActionType.CODE: {
        const codes = pieceEngineUtil(log)
...
async function execute<Result extends EngineHelperResult>(
  ...
  if (isNil(engineWorkers)) {
    engineWorkers = new EngineWorker(
      log,
      Math.max(
        workerMachine.getSettings().FLOW_WORKER_CONCURRENCY,
        workerMachine.getSettings().SCHEDULED_WORKER_CONCURRENCY,
      ),
      enginePath,
      {
