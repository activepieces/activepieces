...
export const isolateEngineRunner = (log: FastifyBaseLogger): EngineRunner => ({
  ...
  async extractPieceMetadata(
    ...
    return execute(EngineOperationType.EXTRACT_PIECE_METADATA, sandbox, operation, log)
  },
  async executeTrigger(engineToken, operation) {
    log.debug({ hookType: operation.hookType, projectId: operation.projectId }, '[EngineHelper#executeTrigger]')
...
const execute = async <Result extends EngineHelperResult>(
  ...
  try {
    log.trace({ operationType, sandboxId: sandbox.boxId }, '[EngineHelper#execute]')
