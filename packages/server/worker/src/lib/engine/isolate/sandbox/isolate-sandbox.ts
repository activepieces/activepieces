...
import { PiecesSource, fileExists, getEngineTimeout } from '@activepieces/server-shared'
import {
  EngineOperation,
  EngineOperationType,
  EngineResponse,
...
export class IsolateSandbox {
  ...
  public async runOperation(
    ...
    try {
      const timeout = getEngineTimeout(
        operationType,
        workerMachine.getSettings().FLOW_TIMEOUT_SECONDS,
        workerMachine.getSettings().TRIGGER_TIMEOUT_SECONDS,
      )
      const dirsToBindArgs = this.getDirsToBindArgs()
      const propagatedEnvVars = Object.entries(this.getEnvironmentVariables()).map(
    ...
    const result = {
      ...
      verdict,
      output,
      standardOutput: await readFile(this.getSandboxFilePath('_standardOutput.txt'), {
        encoding: 'utf-8',
      }),
      standardError: await readFile(this.getSandboxFilePath('_standardError.txt'), {
        encoding: 'utf-8',
      }),
    }
  ...
  public async assignCache({
    ...
    log.debug({ boxId: this.boxId, globalCachePath, globalCodesPath, flowVersionId }, '[IsolateSandbox#assignCache]')
  }
  ...
  private getDirsToBindArgs(): string[] {
    ...
    if (this._customPiecesPath) {
      dirsToBind.push(
        `--dir=${IsolateSandbox.sandboxCustomPiecesCachePath}=${path.resolve(this._customPiecesPath, 'node_modules')}`,
      )
    }
    if (this._flowVersionId) {
      dirsToBind.push(
        `--dir=${path.join(IsolateSandbox.sandboxCodesCachePath, this._flowVersionId)}=${path.resolve(globalCodesCachePath, this._flowVersionId)}`,
      )
    }
