...
export const pieceEngineUtil = (log: FastifyBaseLogger) => ({
  ...
  async extractFlowPieces({ flowVersion, engineToken }: ExtractFlowPiecesParams): Promise<PiecePackage[]> {
    const steps = flowStructureUtil.getAllSteps(flowVersion.trigger)
    const pieces = steps
      .filter((step) => step.type === TriggerType.PIECE || step.type === ActionType.PIECE)
      .map((step) => {
        const { packageType, pieceType, pieceName, pieceVersion } = step.settings as
          | PieceTriggerSettings
          | PieceActionSettings
    ...
    switch (packageType) {
      ...
      case PackageType.REGISTRY: {
        ...
        const version = exactVersion
          ? pieceVersion
          : (
              await engineApiService(engineToken, log).getPiece(pieceName, {
  ...
  async lockSingleStepPieceVersion(params: LockFlowVersionParams): Promise<FlowVersion> {
    ...
    const pieceVersions = pieces.reduce(
      (acc, piece, index) => ({
        ...acc,
...
async function getPieceVersionAndArchiveId(
  engineToken: string,
  piece: BasicPieceInformation,
  log: FastifyBaseLogger,
): Promise<{ pieceVersion: string; archiveId?: string }> {
