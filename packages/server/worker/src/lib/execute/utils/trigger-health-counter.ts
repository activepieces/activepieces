...
export async function bumpTriggerHealthCounter({ apiClient, log, flowVersion, platformId, status }: BumpTriggerHealthCounterParams): Promise<void> {
    ...
    if (error) {
        log.warn({ error: String(error), piece: { name: pieceName }, flowVersion: { id: flowVersion.id } }, 'Failed to bump trigger health counter')
    }
}

type BumpTriggerHealthCounterParams = {
