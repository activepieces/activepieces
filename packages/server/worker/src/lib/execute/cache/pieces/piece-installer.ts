...
import { cacheRootPaths, getGlobalCacheCommonPath, getGlobalCachePathLatestVersion } from '../cache-paths'
...
export const pieceInstaller = (log: ApLogger, apiClient: WorkerToApiContract) => ({
    async install({ pieces, includeFilters, cacheRoot }: InstallParams): Promise<void> {
        const groupedPieces = groupPiecesByPackagePath(pieces, cacheRoot)
...
function getCustomPiecesPath(platformId: string, cacheRoot?: string): string {
    switch (workerSettings.getSettings().EXECUTION_MODE) {
        ...
        case ExecutionMode.SANDBOX_CODE_AND_PROCESS:
            return path.resolve(latestVersionPathFor(cacheRoot), 'custom_pieces', platformId)
        case ExecutionMode.UNSANDBOXED:
        case ExecutionMode.SANDBOX_CODE_ONLY:
            return commonPathFor(cacheRoot)
        default:
            throw new Error('Invalid execution mode')
    }
}

function commonPathFor(cacheRoot?: string): string {
    return isNil(cacheRoot) ? getGlobalCacheCommonPath() : cacheRootPaths(cacheRoot).common
}

function latestVersionPathFor(cacheRoot?: string): string {
    return isNil(cacheRoot) ? getGlobalCachePathLatestVersion() : cacheRootPaths(cacheRoot).latestVersion
}
...
function groupPiecesByPackagePath(pieces: PiecePackage[], cacheRoot?: string): Record<string, PiecePackage[]> {
    return groupBy(pieces, (piece) => {
        switch (piece.packageType) {
            case PackageType.ARCHIVE:
                return getCustomPiecesPath(piece.platformId, cacheRoot)
