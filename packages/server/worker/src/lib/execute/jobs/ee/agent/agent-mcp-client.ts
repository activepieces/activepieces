...
function errorMessageText(error: unknown): string {
    ...
    return agentWorkerTools.extractResultText(error)
}
...
function classifyMcpAuthError({ error, result, toolName }: {
    ...
    if (isObject(result) && result['isError'] === true) {
        const text = agentWorkerTools.extractResultText(result)
...
function injectSelectedAuth({ name, args, getSelectedAuth }: {
    ...
    const externalId = getSelectedAuth({ pieceName: agentWorkerTools.normalizePieceName(pieceName) })
    ...
    for (const [name, tool] of Object.entries(mcpToolSet)) {
        ...
        result[name] = Object.assign({}, tool, {
            execute: async (rawArgs: unknown, options?: ToolExecutionOptions<undefined>) => {
                ...
                return agentWorkerTools.truncateLargeResult(toolResult)
            },
        })
    }
