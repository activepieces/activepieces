...
export const executeAgentRunJob: JobHandler<ExecuteAgentRunJobData, FireAndForgetJobResult> = {
    ...
    async execute(ctx: JobContext, data: ExecuteAgentRunJobData): Promise<FireAndForgetJobResult> {
        ...
        try {
            ...
            // Every request the turn makes reports the health of the key the app handed us. The
            // worker classifies nothing — it forwards the raw HTTP signal so one classifier decides
            // for every process — and the report is fire-and-forget, never a reason a turn fails.
