...
export async function runAgentTurn({ slots, systemPrompt, messages, tier, phaseState, abortSignal, log, sinks, stopWhen }: RunAgentTurnParams): Promise<AgentTurnResult> {
        ...
        return streamText({
            ...
            prepareStep: ({ steps }) => {
                ...
                return {
                    ...
                    activeTools: agentToolPhases.activeToolsForPhase({ phase: phaseState.phase, allToolNames: Object.keys(slot.tools) }),
    ...
    for (;;) {
        ...
        const sanitizedTail = agentAiUtils.sanitizeTruncatedAssistantTail(stepMessages)
...
function wrapToolsWithFailureGuard({ tools, log, failureCounts }: { tools: ToolSet, log: AgentTurnLogger, failureCounts: Map<string, number> }): ToolSet {
    ...
    for (const [name, toolDef] of Object.entries(tools)) {
        ...
        guarded[name] = {
            ...
            execute: async (input: unknown, options: ToolCallOptions) => {
                ...
                const isFailure = text.length > 0 && agentToolClassification.hasFailureTextPrefix(text)
