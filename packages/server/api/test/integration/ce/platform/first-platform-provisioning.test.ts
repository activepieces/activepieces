...
describe('First platform provisioning', () => {
    ...
    it('reports the signup it finished for a platform whose owner link never landed', async () => {
        ...
        const signedUpCalls = () => trackProject.mock.calls.filter(([{ event }]) => event.name === TelemetryEventName.SIGNED_UP)
        await vi.waitFor(() => expect(signedUpCalls()).toHaveLength(1), { timeout: 2000 })
        expect(signedUpCalls()[0][0].projectId).toBe(response.projectId)
    })
