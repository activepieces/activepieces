...
describe('Record API', () => {
    describeWithAuth(
        ...
        (setup) => {
            ...
            it('should create a record with a numeric value coerced to string', async () => {
                ...
                const response = await ctx.post('/v1/records', {
                    tableId: table.id,
                    records: [[{ fieldId: field.id, value: 0 }]],
                })
            ...
            it('should accept null value without coercing to "null" string', async () => {
                ...
                const response = await ctx.post('/v1/records', {
                    tableId: table.id,
                    records: [[{ fieldId: field.id, value: null }]],
                })
        ...
        (setup) => {
            it('should update cell value', async () => {
                ...
                const response = await ctx.post(`/v1/records/${record.id}`, {
                    tableId: table.id,
                    cells: [{ fieldId: field.id, value: 'new-value' }],
                })
            ...
            it('should add new cell to existing record (upsert)', async () => {
                ...
                const response = await ctx.post(`/v1/records/${record.id}`, {
                    tableId: table.id,
                    cells: [{ fieldId: field2.id, value: 'new-cell-value' }],
                })
    ...
    describeWithAuth(
        'GET /v1/records (List with filters)',
        () => app!,
        (setup) => {
