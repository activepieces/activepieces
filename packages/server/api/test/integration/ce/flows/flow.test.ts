...
describe('Flow API', () => {
    ...
    describe('Export Flow Template endpoint', () => {
        it('Exports a flow template using an API key', async () => {
            ...
            expect(responseBody).toHaveProperty('flowTemplates')
            expect(responseBody.flowTemplates).toHaveLength(1)
            expect(responseBody.flowTemplates[0]).toHaveProperty('trigger')
        })
    })
})
