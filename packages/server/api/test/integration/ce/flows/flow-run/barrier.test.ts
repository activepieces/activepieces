...
describe('barrier release predicate', () => {
    it('releases once every signal has been received, and not before', async () => {
        ...
        const { barrier } = await createFanOutBarrier({ flowRunId: flowRun.id, items: [1, 2, 3, 4], batchSize: 2 })
        const signals = await listSignals(barrier.id)
        expect(signals).toHaveLength(2)

        await barrierService(app.log).receive({ signalId: signals[0].id, status: BarrierSignalStatus.SUCCEEDED })
        await barrierService(app.log).evaluate({ barrierId: barrier.id, projectId: ctx.project.id })
        expect((await db.findOneByOrFail<{ status: WaitpointStatus }>('waitpoint', { id: barrier.id })).status).toBe(WaitpointStatus.PENDING)

        await barrierService(app.log).receive({ signalId: signals[1].id, status: BarrierSignalStatus.SUCCEEDED })
        await barrierService(app.log).evaluate({ barrierId: barrier.id, projectId: ctx.project.id })
    ...
    it('releases early once requiredSuccesses favourable signals have landed', async () => {
        const { flowRun } = await createParentRun(FlowRunStatus.RUNNING)
        const { barrier } = await barrierService(app.log).create({
    ...
    it('releases on the first unfavourable signal when releaseOnFirstFailure is set', async () => {
        ...
        await barrierService(app.log).receive({ signalId: signals[0].id, status: BarrierSignalStatus.REJECTED })
        await barrierService(app.log).evaluate({ barrierId: barrier.id, projectId: ctx.project.id })
    ...
    it('reports the inline signals so a failure reads as a batch range', async () => {
        const { flowRun } = await createParentRun(FlowRunStatus.RUNNING)
        const { barrier } = await createFanOutBarrier({ flowRunId: flowRun.id, items: [1, 2, 3, 4], batchSize: 2 })
        const signals = await listSignals(barrier.id)

        for (const signal of signals) {
            await barrierService(app.log).receive({ signalId: signal.id, status: BarrierSignalStatus.FAILED, result: { runId: 'run-1' } })
        }
        await barrierService(app.log).evaluate({ barrierId: barrier.id, projectId: ctx.project.id })

        const summary = await readSummary(barrier.id)
        expect(summary.failed).toBe(2)
        expect(summary.signals?.map((signal) => signal.sequence).sort()).toEqual([0, 1])
    })
})
...
describe('signal identity', () => {
    it('rejects a second signal on the same (waitpointId, sequence)', async () => {
        ...
        const { barrier } = await createFanOutBarrier({ flowRunId: flowRun.id, items: [1, 2], batchSize: 1 })

        await expect(databaseConnection().getRepository('waitpoint_signal').insert({
            id: apId(),
    ...
    it('lets two null sequences coexist on one barrier', async () => {
        ...
        const { barrier } = await barrierService(app.log).create({
            ...
            stepName: 'approval',
            version: 'V1',
            signalLabels: ['a@example.com', 'b@example.com'],
        })
        const signals = await listSignals(barrier.id)
        expect(signals).toHaveLength(2)
    ...
    it('overwrites the outcome when the same signal is received twice', async () => {
        ...
        await barrierService(app.log).receive({ signalId: signal.id, status: BarrierSignalStatus.FAILED })
        await barrierService(app.log).receive({ signalId: signal.id, status: BarrierSignalStatus.SUCCEEDED })
...
describe('resume guards', () => {
    ...
    it('refuses a by-run resume while the run holds a pending barrier', async () => {
        const { flowRun } = await createParentRun()
        await createFanOutBarrier({ flowRunId: flowRun.id, items: [1, 2], batchSize: 1 })
    ...
    it('lets the module release a barrier and consumes the waitpoint exactly once', async () => {
        const { flowRun } = await createParentRun()
        const { barrier } = await createFanOutBarrier({ flowRunId: flowRun.id, items: [1, 2], batchSize: 2 })
        const [signal] = await listSignals(barrier.id)
