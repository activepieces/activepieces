...
describe('MCP Tools integration', () => {
    ...
    it('87. ap_run_action — returns error when auth-required action has no connection', { timeout: 120_000 }, async () => {
        const ctx = await createTestContext(app)
        const mcp = makeMcp(ctx.project.id)
    ...
    it('88. ap_run_action — rejects connectionExternalId containing special characters', { timeout: 120_000 }, async () => {
        const ctx = await createTestContext(app)
        const mcp = makeMcp(ctx.project.id)
    ...
    it('89. ap_run_action — accepts a plain connectionExternalId without validation error', { timeout: 120_000 }, async () => {
        const ctx = await createTestContext(app)
        const mcp = makeMcp(ctx.project.id)
