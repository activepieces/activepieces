...
import {
    checkIfSolutionExistsInDb,
    createMockConnection,
    createMockFlow,
    createMockFlowRun,
    createMockFlowVersion,
    createMockSolutionAndSave,
    createMockUser,
...
function deletionJobIds(platformId: string, projectIds: string[]) {
    return [...projectIds.map((id) => `hard-delete-project-${id}`), `hard-delete-platform-${platformId}`]
}
    ...
    describe('get platform endpoint', () => {
        ...
        describe('delete platform endpoint', () => {
            ...
            it('deletes a platform by id', async () => {
                ...
                const ownerSolution = await createMockSolutionAndSave({
                    projectId: firstAccount.mockProject.id,
                    platformId: firstAccount.mockPlatform.id,
                    userId: firstAccount.mockOwner.id,
                })

                const secondSolution = await createMockSolutionAndSave({
                    projectId: secondAccount.mockProject.id,
                    platformId: secondAccount.mockPlatform.id,
                    userId: secondAccount.mockOwner.id,
                })
                ...
                it('deletes enabled flows and their related data', async () => {
                    ...
                    await databaseConnection()
                        .getRepository('flow')
                        .update(enabledFlow.id, { publishedVersionId: flowVersionId })
                ...
                it("doesn't delete user identity if it has other users", async () => {
                    ...
                    // assert
                    expect(response?.statusCode).toBe(StatusCodes.NO_CONTENT)
                    await waitForDeletionJobs(
                        deletionJobIds(firstAccount.mockPlatform.id, [firstAccount.mockProject.id]),
                    )
                    const userIdentityExists = await db.findOneBy('user_identity', {
                        id: firstAccount.mockUserIdentity.id,
                    })
