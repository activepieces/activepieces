...
describe('admin add platform endpoint', () => {
    ...
    // arrange
        ...
        expect(responseBody.logoIconUrl).toBe(
            'https://cdn.activepieces.com/brand/logo.svg',
        )
        expect(responseBody.fullLogoUrl).toBe(
            'https://cdn.activepieces.com/brand/full-logo.png',
        )
        expect(responseBody.favIconUrl).toBe(
    ...
    // arrange
        ...
        const updatedMockProject = await databaseConnection
    ...
    // arrange
        ...
        const user = await databaseConnection
            .getRepository('user')
            .findOneByOrFail({ id: mockUser.id })
