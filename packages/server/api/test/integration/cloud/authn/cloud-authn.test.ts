import { CustomDomain, OtpType } from '@activepieces/ee-shared'
import {
  DefaultProjectRole,
  InvitationStatus,
  InvitationType,
...
describe('Authentication API', () => {
  describe('Sign up Endpoint', () => {
    ...
    it('auto verify invited users to continue platform sign up', async () => {
      ...
      const editorRole = (await databaseConnection()
        .getRepository('project_role')
        .findOneByOrFail({ name: DefaultProjectRole.EDITOR })) as ProjectRole
...
async function createMockPlatformAndDomain({
  platform,
  domain,
}: { platform: Partial<Platform>; domain?: Partial<CustomDomain> }): Promise<{
  mockUser: User
