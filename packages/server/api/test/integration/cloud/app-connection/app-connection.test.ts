import { DefaultProjectRole, PackageType, PlatformRole, PrincipalType, ProjectRole } from '@activepieces/shared'
...
describe('AppConnection API', () => {
  describe('Upsert AppConnection endpoint', () => {
    it.each([DefaultProjectRole.ADMIN, DefaultProjectRole.EDITOR])('Succeeds if user role is %s', async (testRole) => {
      // arrange
  ...
  describe('List AppConnections endpoint', () => {
    it.each([DefaultProjectRole.ADMIN, DefaultProjectRole.EDITOR, DefaultProjectRole.VIEWER])(
      'Succeeds if user role is %s',
      async (testRole) => {
        // arrange
        const { mockPlatform, mockProject } = await mockAndSaveBasicSetup()
        const { mockUser } = await mockBasicUser({
