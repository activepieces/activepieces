import { ApiKeyResponseWithValue, UpdateProjectMemberRoleRequestBody } from '@activepieces/ee-shared'
import {
  DefaultProjectRole,
  Permission,
  Platform,
...
beforeEach(async () => {
  stripeHelper(mockLog).createCustomer = jest.fn().mockResolvedValue(faker.string.uuid())
...
describe('Project Member API', () => {
  describe('Update project member role', () => {
    it('should update a project role for a member', async () => {
      const {
        mockOwner: mockUserOne,
        mockPlatform: mockPlatformOne,
      ...
      const mockProjectMemberOne = createMockProjectMember({
        platformId: mockPlatformOne.id,
        projectId: mockProjectOne.id,
  ...
  describe('List project members Endpoint', () => {
    describe('List project members from api', () => {
      it('should return project members', async () => {
        ...
        const projectRole = (await databaseConnection()
          ...
          .findOneByOrFail({ name: DefaultProjectRole.VIEWER })) as ProjectRole
    ...
    describe('List project members by user', () => {
      it.each([
        ...
        const projectRole = (await databaseConnection()
          .getRepository('project_role')
          .findOneByOrFail({ name: testRole })) as ProjectRole
  ...
  describe('Delete project member Endpoint', () => {
    it('Deletes project member', async () => {
      ...
      const projectRole = (await databaseConnection()
        .getRepository('project_role')
        .findOneByOrFail({ name: DefaultProjectRole.ADMIN })) as ProjectRole
    ...
    it.each([DefaultProjectRole.EDITOR, DefaultProjectRole.VIEWER, DefaultProjectRole.OPERATOR])(
      'Fails if user role is %s',
      async (testRole) => {
        // arrange
        const { mockPlatform, mockProject, mockMember } = await createBasicEnvironment()
