import { DefaultProjectRole, PieceType, PiecesFilterType, ProjectRole, apId } from '@activepieces/shared'
...
beforeEach(async () => {
  stripeHelper(mockLog).createCustomer = jest.fn().mockResolvedValue(faker.string.alphanumeric())
})
...
describe('Managed Authentication API', () => {
  describe('External token endpoint', () => {
    ...
    it('Sync Pieces when exchanging external token', async () => {
      ...
      await databaseConnection().getRepository('piece_metadata').save(mockPieceMetadata1)
      ...
      await databaseConnection().getRepository('tag').save(mockTag)
      ...
      await databaseConnection().getRepository('piece_tag').save(mockPieceTag)
