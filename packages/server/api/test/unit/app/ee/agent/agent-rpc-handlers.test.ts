...
async function callUpdateChatProgress(input: { conversationId: string, runId?: string, uiMessages: unknown[], messages?: unknown[] }): Promise<void> {
    ...
    await agentRpcHandlers(noopLogger as never).updateChatProgress(input)
}

describe('agentRpcHandlers.updateChatProgress — incremental LLM message persistence', () => {
    beforeEach(() => {
        mockSet.mockClear()
        mockWhere.mockClear()
...
async function callSaveChatMessages(input: { conversationId: string, runId?: string, messages: unknown[], uiMessages: unknown[] }): Promise<void> {
    const { agentRpcHandlers } = await import('../../../../../src/app/ee/agent/agent-rpc-handlers')
    await agentRpcHandlers(noopLogger as never).saveChatMessages(input as never)
}

describe('agentRpcHandlers.saveChatMessages — no-shrink guard against context loss', () => {
    beforeEach(() => {
        mockSet.mockClear()
