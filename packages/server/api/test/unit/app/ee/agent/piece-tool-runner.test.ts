...
const { mockGetOrThrow, mockExecuteActionRunAction, mockResolveProperty, mockCompleter } = vi.hoisted(() => ({
    mockGetOrThrow: vi.fn(),
    mockExecuteActionRunAction: vi.fn(),
...
vi.mock('../../../../../src/app/mcp/tools/flow-run-utils', () => ({
    executeActionRunAction: mockExecuteActionRunAction,
}))
...
describe('pieceToolRunner.runFromInstruction', () => {
    beforeEach(() => {
        ...
        mockExecuteActionRunAction.mockResolvedValue({ content: [{ type: 'text', text: 'sent' }] })
    })

    it('executes the action with the input the model filled in', async () => {
        await run()

        expect(mockExecuteActionRunAction).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('does not pin a version on execution, because the adhoc path validates against the latest', async () => {
        ...
        const call = mockExecuteActionRunAction.mock.calls[0]?.[0] ?? {}
    ...
    it('fails clearly when the piece has no such action', async () => {
        ...
        expect(mockExecuteActionRunAction).not.toHaveBeenCalled()
    })
