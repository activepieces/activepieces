...
const {
    ...
    return {
        ...
        mockFilterPlatformsWithInfraSetupTelemetryEnabled: vi.fn(),
    }
})
...
vi.mock('../../../../../src/app/platform/platform-configuration.service', () => ({
    platformConfigurationService: vi.fn(() => ({
        filterPlatformsWithInfraSetupTelemetryEnabled: mockFilterPlatformsWithInfraSetupTelemetryEnabled,
    })),
}))
...
describe('licenseKeyUsageReportService', () => {
    beforeEach(() => {
        ...
        mockFilterPlatformsWithInfraSetupTelemetryEnabled.mockImplementation(
            ({ platformIds }: { platformIds: string[] }) => Promise.resolve(platformIds),
        )
    })
    ...
    describe('reportAllPlatforms', () => {
        ...
        it('should send no setup report for a platform that turned deployment-setup telemetry off, while still reporting its usage', async () => {
            mockFilterPlatformsWithInfraSetupTelemetryEnabled.mockResolvedValue(['platform-2'])
            mockQueries({
                licenseKeys: [
                    { platformId: 'platform-1', licenseKey: 'key-1' },
                    { platformId: 'platform-2', licenseKey: 'key-2' },
            ...
            expect(capturesOf('total_runs_per_day').map((capture) => capture.licenseKey)).toEqual(['key-1', 'key-2'])
            expect(capturesOf('platform_setup_report').map((capture) => capture.licenseKey)).toEqual(['key-2'])
        })

        it('should skip collecting the deployment setup entirely when every platform turned it off', async () => {
            mockFilterPlatformsWithInfraSetupTelemetryEnabled.mockResolvedValue([])
            mockAppMachineList.mockResolvedValue([appInstance()])
            mockQueries({ licenseKeys: [{ platformId: 'platform-1', licenseKey: 'key-123' }] })
            ...
            expect(mockAppMachineList).not.toHaveBeenCalled()
            expect(mockWorkerMachineFind).not.toHaveBeenCalled()
        })
