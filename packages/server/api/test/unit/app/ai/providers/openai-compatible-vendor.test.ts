...
describe('openAiCompatibleVendor', () => {
    ...
    it('prefers an explicit base url so a China-region platform can override the default', async () => {
        respondWith(['moonshot-v1-8k'])
        const vendor = openAiCompatibleVendor({ name: 'Moonshot AI', provider: AIProviderName.MOONSHOT })

        await vendor.listModels({ apiKey: 'k' }, { baseUrl: 'https://api.moonshot.cn/v1' })
    ...
    it('does not double the slash when the base url carries a trailing one', async () => {
        ...
        await vendor.listModels({ apiKey: 'k' }, { baseUrl: 'https://api.z.ai/api/paas/v4/' })
