...
import { activepiecesAiCaller } from '../../../../src/app/ai/activepieces-ai-caller'

function refusalMessageOf(pieceVersion?: string): string {
    const { error } = tryCatchSync(() => activepiecesAiCaller.assertReportsCost({ pieceVersion }))
...
describe('activepiecesAiCaller.assertReportsCost', () => {
    ...
    it('allows a caller that reports the cost-reporting version', () => {
        expect(tryCatchSync(() => activepiecesAiCaller.assertReportsCost({ pieceVersion: '0.11.0' })).error).toBeNull()
    })

    it('allows a caller on a later version, since it also reports cost', () => {
        expect(tryCatchSync(() => activepiecesAiCaller.assertReportsCost({ pieceVersion: '0.14.2' })).error).toBeNull()
    })
})
