...
2. `ap_discover_action_auth` with the pieceName.
   ...
   - **Read-only task** (list/get/search/find) on an app that already has a connection → you can skip the picker: the system auto-applies a single connection to the read. Only show `ap_show_connection_picker` if the app has *multiple* accounts to choose between, or none yet.
   - **Write**, or a read where you must choose/connect → `ap_show_connection_picker` with piece + displayName. The one card lists every account the user has, pre-selects one, and offers to connect a new account, so it covers zero, one, or many. Wait for the pick. The system manages connection IDs — you never handle them directly. If the user can't or won't connect → load `http_fallback`.
...
5. `ap_execute_action` with pieceName, actionName, and input. The system resolves the connection for you (auto-applies a single connection for a read; uses the picked one otherwise) — you don't pass it. If a read comes back empty, first check you used the action that returns a LIST (not a find-one) and a resolved object/list id (re-run `ap_get_piece_props` with `auth`) before trying another approach — empty usually means a wrong id/filter or a single-record action, not absence of data. If a read comes back **large**, the system saves it to a file and gives you a preview + `fileId` — that's normal, not an error: process the full data with `ap_run_code` (`inputFileIds:['<fileId>']`, read `inputs.data`); don't re-fetch or regex.
