...
import { ExecutionType, RunEnvironment, ScheduleType, isNil } from '@activepieces/shared'
...
async function updateLegacyRepeatableJobKey(repeatableJobs: RepeatableJob[], log: FastifyBaseLogger) {
  ...
  for (const repeatableJob of repeatableJobs) {
    const nextJob = currentJobs.find((job) => !isNil(job) && job.repeatJobKey === repeatableJob.key && !isNil(job.data))
...
async function getJobsToMigrate(): Promise<(Job<ScheduledJobData> | undefined)[]> {
  return (await bullMqGroups[QueueName.SCHEDULED].getJobs()).filter(
    (job) => !isNil(job?.data) && job.data.schemaVersion !== LATEST_JOB_DATA_SCHEMA_VERSION,
  )
}
