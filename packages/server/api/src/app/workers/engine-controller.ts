...
export const flowEngineWorker: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.post('/update-run', UpdateRunProgress, async (request) => {
    const { runId, workerHandlerId, runDetails, httpRequestId, executionStateBuffer, executionStateContentLength } =
      request.body
  ...
  app.post('/update-flow-response', UpdateFlowResponseParams, async (request) => {
    ...
    await engineResponseWatcher(request.log).publish(httpRequestId, workerHandlerId, runResponse)
    return {}
  })
  ...
  app.post('/remove-stale-job', RemoveFlowRequest, async (request) => {
    ...
    const flow = isNil(flowId)
      ? null
      : await flowService(request.log).getOnePopulated({
          projectId: request.principal.projectId,
  ...
  app.get('/files/:fileId', GetFileRequestParams, async (request, reply) => {
    ...
    return reply.type('application/zip').status(StatusCodes.OK).send(data)
  })
}

async function getFlowResponse(result: FlowRunResponse): Promise<EngineHttpResponse> {
