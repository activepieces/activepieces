...
export const refillJobsWithEngineToken = (log: FastifyBaseLogger) => ({
  async run(): Promise<void> {
    ...
    if (isNil(workerJobsQueue)) {
      log.error('[refillJobsWithEngineToken] BullMQ queue is not initialized')
      return
    }
    log.info('[refillJobsWithEngineToken] Starting migration')
    let migratedJobs = 0
    for (let start = 0; ; start += BATCH_SIZE) {
      const jobs = await workerJobsQueue.getJobs(undefined, start, start + BATCH_SIZE)
