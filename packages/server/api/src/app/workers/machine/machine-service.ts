import { MachineInformation, WorkerMachineStatus, WorkerMachineWithStatus, WorkerPrincipal } from '@activepieces/shared'
  ...
  async list(): Promise<WorkerMachineWithStatus[]> {
    const workers = await workerRepo()
      ...
      .where('machine.updated > :updated', {
        updated: new Date(dayjs().subtract(OFFLINE_THRESHOLD, 'ms').toISOString()),
      })
      .getMany()
    return workers.map((worker) => {
      ...
      return {
        ...worker,
        status: isOnline ? WorkerMachineStatus.ONLINE : WorkerMachineStatus.OFFLINE,
      }
    })
  },
}
