...
export class AddToolSearchIndexTable1797000000000 implements Migration {
    name = 'AddToolSearchIndexTable1797000000000'
    breaking = false
    release = '0.85.4'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!vectorAvailable[0]?.available) {
            log.warn('[AddToolSearchIndexTable1797000000000] Skipping tool_search_index table creation — pgvector extension is not installed. The tool-search engine will run on the keyword floor until pgvector is available and a new migration creates the table.')
            return
        }
