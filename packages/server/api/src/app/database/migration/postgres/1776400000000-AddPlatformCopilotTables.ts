...
export class AddPlatformCopilotTables1776400000000 implements Migration {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`
            CREATE TABLE "platform_copilot_registry" (
                "id" character varying(21) NOT NULL,
                "created" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
                "updated" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
                "platformId" character varying NOT NULL,
                ...
                CONSTRAINT "PK_platform_copilot_registry" PRIMARY KEY ("id")
            )
        `)
        await queryRunner.query(`
            CREATE UNIQUE INDEX "idx_platform_copilot_registry_platform_id"
            ON "platform_copilot_registry" ("platformId")
        `)
        await queryRunner.query(`
            CREATE UNIQUE INDEX "idx_platform_copilot_registry_api_key_hash"
            ON "platform_copilot_registry" ("copilotApiKeyHash")
        `)
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query('DROP INDEX "idx_platform_copilot_registry_api_key_hash"')
        await queryRunner.query('DROP INDEX "idx_platform_copilot_registry_platform_id"')
        await queryRunner.query('DROP TABLE "platform_copilot_registry"')
