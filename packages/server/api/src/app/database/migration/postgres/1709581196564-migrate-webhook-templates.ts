...
export class MigrateWebhookTemplate1709581196564 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const { id } of flowVersionsIds) {
      ...
      if (step.type === 'WEBHOOK') {
        ...
        await queryRunner.query('UPDATE flow_template SET template = $1 WHERE id = $2', [endResult, flowVersion.id])
      }
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    log.info('rolling back  MigrateWebhookTemplate1709581196564, finished flows ' + count)
  }
}
