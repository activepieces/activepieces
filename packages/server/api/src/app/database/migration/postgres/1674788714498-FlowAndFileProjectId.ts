...
export class FlowAndFileProjectId1674788714498 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (let i = 0; i < flows.length; ++i) {
            ...
            const collection = await queryRunner.query('SELECT * FROM collection WHERE id = $1', [
                currentFlow.collectionId,
            ])
            currentFlow.projectId = collection[0].projectId
            await queryRunner.query('UPDATE flow SET "projectId" = $1 WHERE id = $2', [
            ...
            while (action !== undefined && action !== null) {
                if (action.type === 'CODE') {
                    ...
                    if (packagedFileId !== undefined && packagedFileId !== null) {
                        const packagedFileToUpdate = await queryRunner.query('SELECT * FROM file WHERE id = $1', [
                            packagedFileId,
                        ])
                        if (packagedFileToUpdate.length === 0) {
                            ...
                            await queryRunner.query('UPDATE file SET "projectId" = $1 WHERE id = $2', [
                                packagedFileToUpdate[0].projectId,
                                packagedFileId,
                            ])
                        }
                    }
                    ...
                    if (sourceFileId !== undefined && sourceFileId !== null) {
                        const sourceFileToUpdate = await queryRunner.query('SELECT * FROM file WHERE id = $1', [
                            sourceFileId,
                        ])
                        if (sourceFileToUpdate.length === 0) {
                            ...
                            await queryRunner.query('UPDATE file SET "projectId" = $1 WHERE id = $2', [
                                sourceFileToUpdate[0].projectId,
                                sourceFileId,
                            ])
                        }
                    }
        ...
        for (let i = 0; i < flowRuns.length; ++i) {
            ...
            if (currentFlowRun.logsFileId !== undefined && currentFlowRun.logsFileId !== null) {
                const logFlowRunFile = await queryRunner.query('SELECT * FROM file WHERE id = $1', [
                    currentFlowRun.logsFileId,
                ])
                logFlowRunFile[0].projectId = currentFlowRun.projectId
                await queryRunner.query('UPDATE file SET "projectId" = $1 WHERE id = $2', [
                    logFlowRunFile[0].projectId,
                    logFlowRunFile[0].id,
                ])
            }
        }
