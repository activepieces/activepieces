...
export class MigrateWebhook1709581196563 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const { id } of flowVersionsIds) {
      ...
      if (step.type === 'WEBHOOK') {
        ...
        await queryRunner.query(`UPDATE ${FLOW_VERSION_TABLE} SET trigger = $1 WHERE id = $2`, [result, flowVersion.id])
      }
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    log.info('rolling back  MigrateWebhook1709581196563, finished flows ' + count)
  }
}
