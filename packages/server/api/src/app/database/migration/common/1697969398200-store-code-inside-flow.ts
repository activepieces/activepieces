...
const traverseAndUpdateSubFlow = async (
  ...
  switch (root.type) {
    case 'BRANCH':
      updated =
        (await traverseAndUpdateSubFlow(updater, root.onSuccessAction, queryRunner, flowId, flowVersionId)) || updated
      updated =
        (await traverseAndUpdateSubFlow(updater, root.onFailureAction, queryRunner, flowId, flowVersionId)) || updated
      break
    case 'LOOP_ON_ITEMS':
      updated =
        (await traverseAndUpdateSubFlow(updater, root.firstLoopAction, queryRunner, flowId, flowVersionId)) || updated
      break
    case 'CODE':
