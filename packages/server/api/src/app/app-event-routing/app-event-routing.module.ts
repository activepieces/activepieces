    ...
    fastify.all(
        ...
        async (
            ...
            const eventsQueue = listeners.map(async (listener) => {
                ...
                const flowToRunRequest: GetFlowVersionForWorkerRequest = {
                    flowId: listener.flowId,
                    type: GetFlowVersionForWorkerRequestType.LOCKED,
                }
                const flowToRun = await flowService(request.log).getFlow(listener.projectId, flowToRunRequest, request.log)
