...
async function handleSampleDataDeletion(projectId: ProjectId, flowVersion: FlowVersion, operation: FlowOperationRequest, log: FastifyBaseLogger): Promise<void> {
    ...
    switch (operation.type) {
        ...
        {
            const stepsToDelete = Array.isArray(operation.request) ? operation.request.map(r=>flowStructureUtil.getStepOrThrow(r.name, flowVersion.trigger)) : [flowStructureUtil.getStepOrThrow(operation.request.name, flowVersion.trigger)]
