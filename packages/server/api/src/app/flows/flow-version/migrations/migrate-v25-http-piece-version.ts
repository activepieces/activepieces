...
export const migrateV25HttpPieceVersion: Migration = {
    ...
    migrate: async (flowVersion: FlowVersion): Promise<FlowVersion> => {
        const newVersion = flowStructureUtil.transferFlow(flowVersion, (step) => {
            if (step.type !== FlowActionType.PIECE || step.settings.pieceName !== HTTP_PIECE_NAME || step.settings.pieceVersion !== BROKEN_PIECE_VERSION) {
                return step
            }
            return {
                ...step,
...
const BROKEN_PIECE_VERSION = '0.11.11'
const FIXED_PIECE_VERSION = '0.11.16'
