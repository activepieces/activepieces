...
export const moveAgentsToFlowVerion: Migration = {
    ...
    migrate: async (flowVersion: FlowVersion): Promise<FlowVersion> => {
        ...
        const agentsAndMcpPromises = await Promise.all(
            flowStructureUtil.getAllSteps(flowVersion.trigger).map(
                async (
                    step,
                ): Promise<{
                    ...
                    if (step.type === FlowActionType.PIECE && step.settings.pieceName === '@activepieces/piece-agent') {
                        const agentResults = await db.query('SELECT * FROM agent WHERE "externalId" = $1', [
                        ...
                        const tools = dbTools.map(
                            (tool: {
                                ...
                                if (tool.type === 'PIECE') {
                                    const pieceMetadata =
                                        typeof tool.pieceMetadata === 'string'
                                            ? JSON.parse(tool.pieceMetadata)
                                            : tool.pieceMetadata
                                    return {
                                        type: tool.type,
        ...
        const newVersion = flowStructureUtil.transferFlow(flowVersion, (step) => {
            if (step.type === FlowActionType.PIECE && step.settings.pieceName === '@activepieces/piece-agent') {
                ...
                const agentAndTools = agentsAndMcpPromises.find(
                    (agentAndMcp) => agentAndMcp?.agent?.externalId === step.settings.input['agentId'],
                )
                ...
                step.settings = {
                    ...
                    input: {
                        ...
                        [AgentPieceProps.STRUCTURED_OUTPUT]:
                            typeof agent?.outputFields === 'string'
                                ? JSON.parse((agent?.outputFields as string) || '[]')
                                : [],
