      ...
      case FlowOperationType.ADD_ACTION:
        switch (clonedRequest.request.action.type) {
          ...
          case ActionType.PIECE: {
            const result = await validateAction(clonedRequest.request.action.settings, projectId, platformId, log)
          ...
          case ActionType.ROUTER:
            clonedRequest.request.action.valid = routerSettingsValidator.Check(clonedRequest.request.action.settings)
            break
      ...
      case FlowOperationType.UPDATE_ACTION:
        switch (clonedRequest.request.type) {
          case ActionType.LOOP_ON_ITEMS:
            ...
            break
          case ActionType.PIECE: {
            const result = await validateAction(clonedRequest.request.settings, projectId, platformId, log)
          ...
          case ActionType.ROUTER:
            clonedRequest.request.valid = routerSettingsValidator.Check(clonedRequest.request.settings)
            break
          case ActionType.CODE: {
            break
          }
      ...
      case FlowOperationType.UPDATE_TRIGGER:
        switch (clonedRequest.request.type) {
          ...
          case TriggerType.PIECE: {
            const result = await validateTrigger(clonedRequest.request.settings, projectId, platformId, log)
            clonedRequest.request.valid = result.valid
...
function validateProps(props: PiecePropertyMap, input: Record<string, unknown> | undefined): ValidationResult {
  ...
  const cleanInput = !isNil(input)
    ? Object.fromEntries(Object.keys(props).map((key) => [key, input?.[key]]))
    : undefined
