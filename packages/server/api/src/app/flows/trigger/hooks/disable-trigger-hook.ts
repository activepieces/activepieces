...
import { UserInteractionJobType, exceptionHandler } from '@activepieces/server-shared'
...
export const disablePieceTrigger = async (
  ...
  try {
    const result = await userInteractionWatcher(log).submitAndWaitForResponse<
      EngineHelperResponse<EngineHelperTriggerResult<TriggerHookType.ON_DISABLE>>
    >({
