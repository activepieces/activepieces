...
export const sampleDataService = (log: FastifyBaseLogger) => ({
  async runAction({ projectId, flowVersionId, stepName }: RunActionParams): Promise<Omit<StepRunResponse, 'id'>> {
    const flowVersion = await flowVersionService(log).getOneOrThrow(flowVersionId)
    const step = flowStructureUtil.getActionOrThrow(stepName, flowVersion.trigger)

    const { result, standardError, standardOutput } = await userInteractionWatcher(log).submitAndWaitForResponse<
      EngineHelperResponse<EngineHelperActionResult>
    >({
      projectId,
      flowVersion,
  ...
  async getOrReturnEmpty(params: GetSampleDataParams): Promise<unknown> {
    ...
    const fileId =
      fileType === FileType.SAMPLE_DATA
        ? step.settings.inputUiInfo?.sampleDataFileId
  ...
  async deleteForStep(params: DeleteSampleDataForStepParams): Promise<void> {
    await fileRepo()
      ...
      .andWhere("metadata->>'flowVersionId' = :flowVersionId", {
