...
export const flowRunLogsController: FastifyPluginAsyncZod = async (app) => {
        ...
        '/logs',
        {
            ...
            onRequest: async (request, reply) => {
                ...
                if (decodedToken.behavior === UploadLogsBehavior.REDIRECT_TO_S3) {
                    ...
                    const s3SignedUrl = await s3Helper(request.log).putS3SignedUrl({
                        s3Key: fileMetadata.s3Key!,
                        contentEncoding: CONTENT_ENCODING_ZSTD,
                    })
                    request.log.info(
                        {
                            s3Key: fileMetadata.s3Key,
    ...
    app.get(
        ...
        async (request, reply) => {
            ...
            const data =
                file.location === FileLocation.DB ? file.data : await s3Helper(request.log).getFile(file.s3Key!)
