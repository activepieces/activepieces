...
const calculateDelayForPausedRun = (resumeDateTimeIsoString: string): number => {
  const now = dayjs()
...
export const flowRunSideEffects = (log: FastifyBaseLogger) => ({
  ...
  async start({
    ...
    log.info(`[FlowRunSideEffects#start] flowRunId=${flowRun.id} executionType=${executionType}`)
  ...
  async pause({ flowRun }: PauseParams): Promise<void> {
    log.info(`[FlowRunSideEffects#pause] flowRunId=${flowRun.id} pauseType=${flowRun.pauseMetadata?.type}`)
