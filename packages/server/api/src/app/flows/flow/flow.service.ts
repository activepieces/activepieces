...
const getFolderIdFromRequest = async ({
  projectId,
  folderId,
  folderName,
  log,
...
export const flowService = (log: FastifyBaseLogger) => ({
  async create({ projectId, request, externalId }: CreateParams): Promise<PopulatedFlow> {
    ...
    telemetry(log)
      ...
      .catch((e) => log.error(e, '[FlowService#create] telemetry.trackProject'))
  ...
  async list({
    ...
    const filteredPopulatedFlows = name
      ? populatedFlows.filter((flow) => flow.version.displayName.match(new RegExp(`^.*${name}.*`, 'i')))
      : populatedFlows
  ...
  async update({ id, userId, projectId, platformId, operation, lock = true }: UpdateParams): Promise<PopulatedFlow> {
    const flowLock = lock
  ...
  async updateStatus({ id, projectId, newStatus, entityManager }: UpdateStatusParams): Promise<PopulatedFlow> {
  ...
  async updateFailureCount({ flowId, projectId, success }: UpdateFailureCountParams): Promise<void> {
    ...
    if (newFailureCount >= TRIGGER_FAILURES_THRESHOLD) {
      ...
      rejectedPromiseHandler(
        telemetry(log).trackProject(projectId, {
