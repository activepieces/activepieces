...
import { ActivepiecesError, apId, ApId, assertNotNullOrUndefined, ErrorCode, ExecutioOutputFile, FileType, FlowRun, FlowRunStatus, flowStructureUtil, isNil, Issue, IssueStatus, ListIssuesParams, PopulatedIssue, SeekPage, spreadIfDefined, StepOutputStatus, TelemetryEventName } from '@activepieces/shared'
...
const extractFailedStepId = async (flowRun: FlowRun, log: FastifyBaseLogger) => {
    ...
    const failedStep = Object.entries(executionState.steps).find(
        ([_, step]) => step.status === StepOutputStatus.FAILED,
    )
