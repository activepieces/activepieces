...
async function addPieceToServer(
    ...
    for (const action of toolActionsNames) {
        ...
        const extractedActionName = mcpToolNaming.extractActionName(actionMetadata.name, toolPieceMetadata.pieceName)
        const actionName = mcpToolNaming.fixTool(extractedActionName, mcpTool.id, McpToolType.PIECE)
        const toolSchema = {
