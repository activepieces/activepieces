import { McpServer, McpToolDefinition, RouterActionSettingsWithValidation, SourceCode } from '@activepieces/shared'
                ...
                switch (params.stepType) {
                    ...
                    case 'PIECE_TRIGGER': {
                        ...
                        if (!params.pieceName || !componentName) {
                            const missing = !params.pieceName
                                ? 'pieceName'
                            ...
                            return {
                                content: [
                                    {
                                        type: 'text',
                                        text: `❌ ${missing} is required for ${componentType} validation.`,
                                    },
                                ],
                            }
                        }
                        return await validatePieceComponent({
                    ...
                    case 'CODE':
                        return validateWithSchema({
                            ...
                            data: {
                                sourceCode: { code: params.sourceCode ?? '', packageJson: params.packageJson ?? '{}' },
                                input: {},
                            },
                            label: 'CODE',
                        })
                    case 'LOOP_ON_ITEMS':
                        return validateWithSchema({
                            schema: loopValidator,
                            data: { items: params.loopItems ?? '' },
                            label: 'LOOP_ON_ITEMS',
                        })
                    case 'ROUTER':
                        return validateWithSchema({
                            schema: RouterActionSettingsWithValidation,
...
const validateStepConfigInput = z.object({
    stepType: z
        .enum(['PIECE_ACTION', 'PIECE_TRIGGER', 'CODE', 'LOOP_ON_ITEMS', 'ROUTER'])
    ...
    actionName: z.string().optional().describe('For PIECE_ACTION: action name (e.g. "send_channel_message").'),
    triggerName: z.string().optional().describe('For PIECE_TRIGGER: trigger name (e.g. "new_mention").'),
    input: z
    ...
    sourceCode: z.string().optional().describe('For CODE: the JavaScript/TypeScript source code.'),
    packageJson: z.string().optional().describe('For CODE: package.json content as JSON string.'),
    loopItems: z.string().optional().describe('For LOOP_ON_ITEMS: expression for items to iterate over.'),
    settings: z
        .record(z.string(), z.unknown())
        .optional()
...
async function validatePieceComponent({
    pieceName,
    componentName,
    componentType,
    input,
    ...
    projectId,
    log,
}: ValidatePieceParams): Promise<McpResult> {
    ...
    if (diagnosis.missing.length === 0) {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `✅ Valid configuration for ${componentType.toUpperCase()} "${normalized}/${componentName}".${uiHint}`,
                },
            ],
        }
    }
