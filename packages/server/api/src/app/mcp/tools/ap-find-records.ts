        ...
        .array(
            z.object({
                ...
                value: z
                    .string()
                    .optional()
                    .describe('Filter value (required for all operators except exists/not_exists)'),
            }),
        )
        .optional()
        .describe('Optional filters. All filters are combined with AND logic.'),
                    ...
                    for (const filter of filters) {
                        if (
                            ...
                            resolved.errors.push(
                                `Filter on "${filter.fieldName}" with operator "${filter.operator}" requires a value.`,
                            )
                        }
                    }
