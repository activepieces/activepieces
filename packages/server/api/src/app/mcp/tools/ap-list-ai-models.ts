...
const listAiModelsInput = z.object({
    provider: providerSchema
        .optional()
        .describe('Filter by provider name. Omit to list all configured providers and their models.'),
})
                ...
                if (providers.length === 0) {
                    return {
                        content: [
                            {
                                type: 'text',
                                text: 'No AI providers configured. Ask a platform admin to add one in Settings → AI Providers.',
                            },
                        ],
                    }
                }
                ...
                if (filteredProviders.length === 0) {
                    ...
                    return {
                        content: [
                            {
                                type: 'text',
                                text: `Provider "${filterProvider}" is not configured. Available providers: ${available}`,
                            },
                        ],
                    }
                }
                ...
                const sections = await Promise.all(
                    filteredProviders.map(async (p) => {
                        try {
                            ...
                            const overflow =
                                textModels.length > MAX_MODELS_PER_PROVIDER
                            ...
                            log.warn(
                                { err, provider: p.provider },
                                'ap_list_ai_models: failed to fetch models for provider',
                            )
