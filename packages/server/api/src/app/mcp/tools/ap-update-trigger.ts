    ...
    return {
        ...
        inputSchema: {
            ...
            pieceName: z
                .string()
                .describe(
                    'The piece name for the trigger (e.g. "@activepieces/piece-gmail"). Use ap_list_pieces to get valid values.',
                ),
            pieceVersion: z
                .string()
                .describe('The piece version (e.g. "~0.1.0"). Use ap_list_pieces to get valid values.'),
            triggerName: z
                .string()
                .describe(
                    'The trigger name within the piece (e.g. "new_email"). Use ap_list_pieces with includeTriggers=true to get valid values.',
                ),
            input: z
                .record(z.string(), z.unknown())
                .optional()
                .describe(`Input settings for the trigger (key-value pairs). ${mcpUtils.STEP_REFERENCE_HINT}`),
            auth: z
                .string()
                .optional()
                .describe(
        ...
        execute: async (args) => {
            ...
            const existingPieceSettings =
                existingTrigger.type === FlowTriggerType.PIECE &&
            ...
            try {
                ...
                if (!trigger.valid) {
                    const diagnosis = await diagnoseMissingTriggerInputs({
                        pieceName,
                        pieceVersion,
                        triggerName,
                        input,
                    ...
                    const hint =
                        diagnosis ??
                        'Check that triggerName is correct and all required inputs are provided. Use ap_list_connections to get a valid connection externalId for auth.'
                    return {
                        content: [
                            {
                                type: 'text',
                ...
                return {
                    content: [
                        { type: 'text', text: `✅ Successfully updated trigger to "${pieceName}/${triggerName}".` },
                    ],
                }
            } catch (err) {
...
async function diagnoseMissingTriggerInputs({
    pieceName,
    pieceVersion,
    triggerName,
    input,
    ...
    try {
        ...
        const { parts, missing, uiRequired, hasAuth } = mcpUtils.diagnosePieceProps({
            props: trigger.props,
            input,
