...
export async function createMcpServer({
  ...
  const pieces = await Promise.all(
    pieceNames.map(async (pieceName) => {
  ...
  pieces.flatMap((piece) =>
    Object.values(piece.actions).map((action) => {
      ...
      const pieceConnectionExternalId = connections.find(
        (connection) => connection.pieceName === piece.name,
      )?.externalId
      ...
      server.tool(
        ...
        async (params) => {
          ...
          const result = await userInteractionWatcher(logger).submitAndWaitForResponse<
            EngineHelperResponse<ExecuteActionResponse>
          >({
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `✅ Successfully executed ${action.displayName}\n\n` +
                    `${action.description}\n\n` +
              ...
              content: [
                {
                  type: 'text',
                  text:
                    `❌ Error executing ${action.displayName}\n\n` +
                    `${action.description}\n\n` +
