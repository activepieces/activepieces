import { AppSystemProp, UserInteractionJobType, exceptionHandler } from '@activepieces/server-shared'
import {
  ActivepiecesError,
  ApEdition,
  ApEnvironment,
...
export const appConnectionService = (log: FastifyBaseLogger) => ({
  ...
  async list({
    ...
    const queryBuilder = appConnectionsRepo().createQueryBuilder('app_connection').where(querySelector)
    ...
    const promises = data.map(async (encryptedConnection) => {
      ...
      const owner = isNil(apConnection.ownerId)
        ? null
        : await userService.getMetaInformation({
  ...
  async getOwners({
    projectId,
    platformId,
  }: { projectId: ProjectId; platformId: PlatformId }): Promise<AppConnectionOwners[]> {
...
const engineValidateAuth = async (params: EngineValidateAuthParams, log: FastifyBaseLogger): Promise<void> => {
  ...
  const engineResponse = await userInteractionWatcher(log).submitAndWaitForResponse<
    EngineHelperResponse<EngineHelperValidateAuthResult>
  >({
  ...
  if (engineResponse.status !== EngineResponseStatus.OK) {
    log.error(engineResponse, '[AppConnectionService#engineValidateAuth] engineResponse')
    throw new ActivepiecesError({
