import { ActivepiecesError, ErrorCode, UserIdentity, apId, isNil } from '@activepieces/shared'
...
export const userIdentityService = (log: FastifyBaseLogger) => ({
  ...
  async getBasicInformation(
    id: string,
  ): Promise<Pick<UserIdentity, 'email' | 'firstName' | 'lastName' | 'trackEvents' | 'newsLetter'>> {
