...
export const recordService = {
  async create({ request, projectId }: CreateParams): Promise<PopulatedRecord[]> {
  ...
  async list({ tableId, projectId, filters }: ListParams): Promise<SeekPage<PopulatedRecord>> {
  ...
  async importCsv({ projectId, tableId, request }: ImportCsvParams): Promise<PopulatedRecord[]> {
    ...
    for await (const row of parser) {
      const record: { value: string; fieldId: string }[] = fields.reduce(
        (acc, field, idx) => {
...
async function formatRecordsAndFetchField({
  ...
  tableId,
  projectId,
}: { records: RecordSchema[]; tableId: string; projectId: string }): Promise<PopulatedRecord[]> {
  const fields = await fieldService.getAll({
    tableId,
    projectId,
  })
