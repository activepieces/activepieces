...
export const redisSystemJobSchedulerService = (log: FastifyBaseLogger): SystemJobSchedule => ({
  async init(): Promise<void> {
    ...
    await Promise.all([systemJobsQueue.waitUntilReady(), systemJobWorker.waitUntilReady()])
  ...
  async upsertJob({ job, schedule }): Promise<void> {
    ...
    const patternChanged =
      !isNil(existingJob) && schedule.type === 'repeated' ? schedule.cron !== existingJob.opts.repeat?.pattern : false
...
async function removeDeprecatedJobs() {
  const deprecatedJobs = ['trigger-data-cleaner', 'logs-cleanup-trigger']
  const allSystemJobs = await systemJobsQueue.getJobSchedulers()
  const deprecatedJobsFromQueue = allSystemJobs.filter(
    (f) => !isNil(f) && (deprecatedJobs.includes(f.key) || deprecatedJobs.some((d) => f.key.startsWith(d))),
  )
