...
import { ApEdition, ApEnvironment, ExecutionMode, FileLocation, PieceSyncMode, isNil } from '@activepieces/shared'
  ...
  if ([ApEdition.CLOUD, ApEdition.ENTERPRISE].includes(edition) && test !== ApEnvironment.TESTING) {
    ...
    if (![ExecutionMode.SANDBOXED, ExecutionMode.SANDBOX_CODE_ONLY].includes(executionMode)) {
      throw new Error(
        JSON.stringify({
          message:
            'Execution mode UNSANDBOXED is no longer supported in this edition, check the documentation for recent changes',
          docUrl: 'https://www.activepieces.com/docs/install/configuration/overview',
        }),
      )
    }
