...
const acquireRedisLock = async (key: string, timeout: number, log: FastifyBaseLogger): Promise<ApLock> => {
  try {
...
const acquireLock = async ({ key, timeout = 3000, log }: AcquireLockParams): Promise<ApLock> => {
  switch (queueMode) {
    case QueueMode.REDIS:
