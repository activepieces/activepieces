...
import { ConnectionState, Flow, FlowState, PopulatedFlow, ProjectState, flowMigrations } from '@activepieces/shared'
...
export const gitSyncHelper = (_log: FastifyBaseLogger) => ({
  async getStateFromGit({ flowPath, connectionsFolderPath }: GetStateFromGitParams): Promise<ProjectState> {
    try {
      ...
      for (const connection of connections) {
        const connectionState = JSON.parse(await fs.readFile(path.join(connectionsFolderPath, connection), 'utf-8'))
  ...
  async upsertFlowToGit({
    fileName,
    flow,
    flowFolderPath,
    connections,
    connectionsFolderPath,
  }: UpsertFlowIntoProjectParams): Promise<void> {
    try {
