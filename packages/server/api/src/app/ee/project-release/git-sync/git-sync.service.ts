...
export const gitRepoService = (_log: FastifyBaseLogger) => ({
  ...
  async onFlowDeleted({
    ...
    projectId,
    log,
  }: { flowId: string; userId: string; projectId: string; log: FastifyBaseLogger }): Promise<void> {
  ...
  async push({ id, userId, request, log }: PushParams): Promise<void> {
    ...
    switch (request.type) {
      case GitPushOperationType.PUSH_FLOW: {
        ...
        await Promise.all(
          request.flowIds.map(async (flowId) => {
        ...
        await gitHelper.commitAndPush(
          git,
          gitRepo,
          request.commitMessage ?? `chore: updated flows ${request.flowIds.join(', ')}`,
        )
        break
      }
      case GitPushOperationType.DELETE_FLOW: {
        ...
        if (deleted) {
          await gitHelper.commitAndPush(
            git,
            gitRepo,
            request.commitMessage ?? `chore: deleted flow ${request.flowIds[0]} from user interface`,
          )
        }
        break
      }
