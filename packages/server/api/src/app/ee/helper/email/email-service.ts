...
import { ApEdition, InvitationType, UserIdentity, UserInvitation, assertNotNullOrUndefined } from '@activepieces/shared'
...
export const emailService = (log: FastifyBaseLogger) => ({
  ...
  async sendReminderJobHandler(job: {
    ...
    const issues = await issuesService(log).list({
