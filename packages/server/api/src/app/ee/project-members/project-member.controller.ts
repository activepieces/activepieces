...
import { Permission, PrincipalType, SERVICE_KEY_SECURITY_OPENAPI, SeekPage } from '@activepieces/shared'
...
export const projectMemberController: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.delete('/:id', DeleteProjectMemberRequest, async (request, reply) => {
    await projectMemberService(request.log).delete(request.principal.projectId, request.params.id)
