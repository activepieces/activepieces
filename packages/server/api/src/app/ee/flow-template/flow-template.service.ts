...
export const flowTemplateService = {
  upsert: async (
    ...
    { description, type, template, blogUrl, tags, id }: CreateFlowTemplateRequest,
  ...
  list: async (
    ...
    const templates = await templateRepo().createQueryBuilder('flow_template').where(commonFilters).getMany()
