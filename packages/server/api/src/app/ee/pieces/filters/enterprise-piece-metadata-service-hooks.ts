import { FilteredPieceBehavior, PiecesFilterType, isNil } from '@activepieces/shared'
...
export const enterprisePieceMetadataServiceHooks: PieceMetadataServiceHooks = {
  async filterPieces(params) {
    ...
    const piecesAfterDefaultFilter = await defaultPieceHooks.filterPieces({
      ...params,
      pieces: resultPieces,
    })
...
async function filterBasedOnProject(projectId: string, pieces: PieceMetadataSchema[]): Promise<PieceMetadataSchema[]> {
