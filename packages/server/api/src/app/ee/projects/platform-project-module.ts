...
export const platformProjectModule: FastifyPluginAsyncTypebox = async (app) => {
  systemJobHandlers.registerJobHandler(
    SystemJobName.HARD_DELETE_PROJECT,
    async (job: SystemJobData<SystemJobName.HARD_DELETE_PROJECT>): Promise<void> => {
      app.log.info({
        name: 'PlatformProjectSideEffects#hardDeleteProjectJobHandler',
