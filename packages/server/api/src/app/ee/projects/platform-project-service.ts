...
async function getProjects(
  params: GetAllParams & { projectIds?: string[] },
  ...
  const projects: ProjectWithLimits[] = await Promise.all(data.map((project) => enrichProject(project, log)))
...
async function enrichProject(project: Project, log: FastifyBaseLogger): Promise<ProjectWithLimits> {
  ...
  return {
    ...
    usage: await usageService(log).getUsageForBillingPeriod(project.id, BillingEntityType.PROJECT),
    analytics: {
      activeFlows,
      totalFlows,
      totalUsers,
