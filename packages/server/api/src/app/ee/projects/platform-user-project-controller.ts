import { assertNotNullOrUndefined, PrincipalType, ProjectWithLimitsWithPlatform } from '@activepieces/shared'
...
export const usersProjectController: FastifyPluginAsyncZod = async (fastify) => {
    fastify.get('/platforms', ListProjectsForPlatforms, async (request) => {
        ...
        const projects = await Promise.all(
            platforms.map(async (platform) => {
                const platformUser = await userService(request.log).getOneByIdentityAndPlatform({
                    identityId: loggedInUser.identityId,
                    platformId: platform.id,
                })
                assertNotNullOrUndefined(platformUser, `Platform user not found for platform ${platform.id}`)
                const projects = await platformProjectService(request.log)
                    .getForPlatform({
                        platformId: platform.id,
