...
export const projectLimitsService = (log: FastifyBaseLogger) => ({
    ...
    async checkAICreditsExceededLimit({ projectId, videoModelRequestCost }: { projectId: string, videoModelRequestCost: number }): Promise<boolean> {
        if (edition === ApEdition.COMMUNITY) {
            return false
        }
        ...
        try {
            ...
            const projectAICreditUsage = await platformUsageService(log).getProjectUsage({ projectId, metric: 'ai_credits', startDate, endDate }) + videoModelRequestCost
            const platformAICreditUsage = await platformUsageService(log).getPlatformUsage({ platformId, metric: 'ai_credits', startDate, endDate }) + videoModelRequestCost
