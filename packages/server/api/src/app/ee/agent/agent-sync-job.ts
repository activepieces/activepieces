...
import { AgentConversation, AgentHistoryMessage, ApEdition, PersistedAgentMessage, PersistedAgentPart, PersistedAgentPartType, PersistedAgentRole, PersistedToolCallStatus } from '@activepieces/shared'
...
function resolveMessages(conversation: AgentConversation): PersistedAgentMessage[] {
    ...
    return convertToPersistedFormat(agentHistory.reconstruct(rawMessages))
}

function convertToPersistedFormat(messages: AgentHistoryMessage[]): PersistedAgentMessage[] {
    return messages.map((msg) => {
        const parts: PersistedAgentPart[] = []

        if (msg.content) {
            parts.push({ type: PersistedAgentPartType.TEXT, text: msg.content })
        }

        if (msg.thoughts) {
            parts.push({ type: PersistedAgentPartType.REASONING, text: msg.thoughts })
        }
        ...
        return {
            role: msg.role === 'user' ? PersistedAgentRole.USER : PersistedAgentRole.ASSISTANT,
            parts,
        }
    })
}
...
function countToolCallsInLatestTurn(messages: PersistedAgentMessage[]): number {
    ...
    return turn.reduce((sum, message) => sum + message.parts.filter((part) => part.type === PersistedAgentPartType.TOOL_CALL).length, 0)
}
