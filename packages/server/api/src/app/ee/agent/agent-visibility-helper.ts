...
async function resolveChatEnabledForUser({ userId, platform, isEmbedded }: {
    ...
    const [cloudRolloutOpen, userHasChatted] = isCloud
        ? await Promise.all([agentRolloutService.isRolloutOpen(), agentRolloutService.hasUserChatted({ userId })])
        : [false, false]
...
export const agentVisibilityGuard: onRequestAsyncHookHandler = async (request) => {
