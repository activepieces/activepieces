...
type SimulationStatus = AgentConversationStatus | typeof SIMULATION_TIMEOUT_STATUS
...
export const agentEvalModule: FastifyPluginAsyncZod = async (app) => {
    app.addHook('preHandler', checkApiKeyPreHandler)
    await app.register(agentEvalController, { prefix: '/v1/agents' })
}

const agentEvalController: FastifyPluginAsyncZod = async (app) => {
    app.get('/eval/prompt-sources', PromptSourcesRoute, async (_request, reply) => {
        return reply.status(StatusCodes.OK).send(agentPrompt.sources)
    })
    ...
    app.post('/eval/simulate', SimulateRoute, async (request, reply) => {
        ...
        for (const turn of turns) {
            ...
            if (settled.status === AgentConversationStatus.ERROR || settled.status === SIMULATION_TIMEOUT_STATUS) {
                break
            }
    ...
    app.post('/eval/turn/start', EvalTurnStartRoute, async (request, reply) => {
        ...
        if (!isNil(conversationId)) {
            ...
            if (existing.status === AgentConversationStatus.STREAMING) {
    ...
    app.get('/eval/conversations/:conversationId/state', EvalStateRoute, async (request, reply) => {
        ...
        const conversation = await agentHelpers.conversationRepo().findOneBy({ id: request.params.conversationId })
...
async function waitForSimulationResult({ conversationId, platformId, userId, priorAssistantTurns, log }: { conversationId: string, platformId: string, userId: string, priorAssistantTurns: number, log: FastifyBaseLogger }): Promise<{ status: SimulationStatus, uiMessages: unknown[] | null }> {
    ...
    for (let attempt = 0; attempt < SIMULATION_MAX_ATTEMPTS; attempt++) {
        ...
        const settled = readSettledState(await agentHelpers.conversationRepo().findOneBy({ id: conversationId, platformId, userId }), priorAssistantTurns)
        if (settled) {
            return settled
        }
    }
...
function readSettledState(conversation: { status: AgentConversationStatus, uiMessages: unknown[] | null } | null, priorAssistantTurns: number): { status: AgentConversationStatus, uiMessages: unknown[] | null } | null {
    if (!conversation) {
        return null
    }
    const uiMessages = conversation.uiMessages ?? null
    if (conversation.status === AgentConversationStatus.ERROR) {
    ...
    if (conversation.status === AgentConversationStatus.IDLE && countAssistantTurns(uiMessages) > priorAssistantTurns) {
        return { status: conversation.status, uiMessages }
    }
    return null
}
