...
export async function configuredToolConversationOrThrow({ conversationId }: { conversationId: string }): Promise<{ projectId: string, platformId: string, source: AgentRunSource, agentId: string | null }> {
    ...
    return { projectId: conversation.projectId, platformId: conversation.platformId, source: conversation.source, agentId: conversation.agentId ?? null }
}
