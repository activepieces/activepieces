...
import { ApEdition, ApEnvironment, ProjectUsage, isNil } from '@activepieces/shared'
...
export const usageService = (log: FastifyBaseLogger) => ({
  async getUsageForBillingPeriod(entityId: string, entityType: BillingEntityType): Promise<ProjectUsage> {
    ...
    const teamMembers =
      entityType === BillingEntityType.PROJECT
...
async function checkUsageLimit({
  ...
  try {
    ...
    const { consumedProjectUsage, consumedPlatformUsage } = await increaseProjectAndPlatformUsage({
      projectId,
      incrementBy,
      usageType,
    })
    ...
    const platformLimit =
      usageType === BillingUsageType.TASKS ? platformBilling.tasksLimit : platformBilling.aiCreditsLimit
...
async function increaseProjectAndPlatformUsage({
  projectId,
  incrementBy,
  usageType,
}: { projectId: string; incrementBy: number; usageType: BillingUsageType }): Promise<{
