...
import { TASKS_PAYG_PRICE_ID, stripeHelper } from './stripe-helper'
...
export const platformBillingModule: FastifyPluginAsyncTypebox = async (app) => {
  systemJobHandlers.registerJobHandler(SystemJobName.PLATFORM_USAGE_REPORT, async () => {
    ...
    const platforms: { platformId: string }[] = await projectRepo()
    ...
    for (const { platformId } of platforms) {
      ...
      if (
        isNil(platformBilling.stripeSubscriptionId) ||
        platformBilling.stripeSubscriptionStatus !== ApSubscriptionStatus.ACTIVE
      ) {
        continue
      }
      ...
      const { tasks, aiTokens } = await usageService(log).getUsageForBillingPeriod(
        platformId,
        BillingEntityType.PLATFORM,
      )

      log.info(
        { platformId, tasks, aiTokens, includedTasks: platformBilling.includedTasks },
        'Sending usage record to stripe',
      )
