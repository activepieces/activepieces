...
import { ActivepiecesError, ErrorCode, PrincipalType, assertNotNullOrUndefined } from '@activepieces/shared'
...
export const platformBillingController: FastifyPluginAsyncTypebox = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest) => {
      ...
      const { tasks, aiTokens } = await usageService(request.log).getUsageForBillingPeriod(
        platform.id,
        BillingEntityType.PLATFORM,
      )
      return {
  ...
  fastify.post('/portal', {}, async (request) => {
    return {
      portalLink: await stripeHelper(request.log).createPortalSessionUrl({
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const projectBilling = await platformBillingService(request.log).getOrCreateForPlatform(
      ...
      await platformBillingService(request.log).update({
        platformId: request.principal.platform.id,
  ...
  fastify.patch(
    ...
    async (request) => {
      ...
      return platformBillingService(request.log).update({
        platformId,
        tasksLimit: request.body.tasksLimit,
      })
    },
  )
}
