import { AI_CREDITS_USAGE_THRESHOLD, ApSubscriptionStatus, BillingCycle, getPlanLimits, PlanName  } from '@activepieces/ee-shared'
import { AppSystemProp, exceptionHandler } from '@activepieces/server-shared'
import { AiOverageState, ALL_PRINCIPAL_TYPES, isNil, TelemetryEventName } from '@activepieces/shared'
            ...
            try {
                ...
                if (!isUnknowSubscription) {
                    ...
                    switch (webhook.type) {
                        ...
                        {
                            const customerHasSubscribed = await haveActiveSubs(subscription.customer as string, stripe)
                            if (isTrialSubscription && customerHasSubscribed)  {
                                break
                            }
                            const result = await processSubscriptionEvent({ subscription, webhook, request, platformId, isTrialSubscription })
                            await telemetry(request.log).trackPlatform(platformId, {
                                name: TelemetryEventName.USER_DELTED_SUBSCRIPTION,
                                payload: result,
                            }).catch((e) => {
                                request.log.error(e, '[Telemetry#trackPlatform] USER_DOWNGRADE_TO_FREE_PLAN')
                            })
                            break
                        }
                        case 'customer.subscription.created':
                        {
                            ...
                            else if (isTrialSubscription && !customer.deleted && customer.email) {
                                await sendTrialRelatedEmails(customer.email, platformId, request.log) 
                            }
                            const result = await processSubscriptionEvent({ subscription, webhook, request, platformId, isTrialSubscription })
                            if (!isTrialSubscription) {
                                await telemetry(request.log).trackPlatform(platformId, {
                                    name: TelemetryEventName.USER_UPGRADE_TO_PAID_PLAN,
                                    payload: result,
                                }).catch((e) => {
                                    request.log.error(e, '[Telemetry#trackPlatform] USER_UPGRADE_TO_PAID_PLAN')
                                })
                            }
                            break
                        }
                        case 'customer.subscription.updated': {
                            ...
                            const result = await processSubscriptionEvent({ subscription, webhook, request, platformId, isTrialSubscription })
                            const isUpgradeFromLastPlan = PlatformPlanHelper.isUpgradeExperience({
                                currentActiveFlowsLimit: platformPlan.activeFlowsLimit ?? 0,
                                currentProjectsLimit: platformPlan.projectsLimit ?? 0,
                                currentUserSeatsLimit: platformPlan.userSeatsLimit ?? 0,
                                newPlan: result.plan,
                                currentPlan: platformPlan.plan as PlanName,
                                newActiveFlowsLimit: result.limits.activeFlowsLimit ?? undefined,
                                newProjectsLimit: result.limits.projectsLimit ?? undefined,
                                newUserSeatsLimit: result.limits.userSeatsLimit ?? undefined,
                                newCycle: result.cycle,
                                currentCycle: platformPlan.stripeBillingCycle as BillingCycle,
                            })
                            await telemetry(request.log).trackPlatform(platformId, {
                                name: TelemetryEventName.USER_UPDATED_PLAN,
                                payload: {
                                    ...result,
                                    isUpgradeFromLastPlan,
                                },
                            }).catch((e) => {
                                request.log.error(e, '[Telemetry#trackPlatform] USER_UPDATED_PLAN')
                            })
                            break
                        }
...
const haveActiveSubs = async (customerId: string, stripe: Stripe) => {
...
const cancelTrialSubscription = async (customerId: string, stripe: Stripe) => {
...
const addThresholdOnAiCreditsItem = async (stripe: Stripe, subscription: Stripe.Subscription): Promise<void> => {
...
const processSubscriptionEvent = async ( { subscription, webhook, request, platformId, isTrialSubscription }: { subscription: Stripe.Subscription, webhook: Stripe.Event, request: FastifyRequest, platformId: string, isTrialSubscription: boolean })=>{
