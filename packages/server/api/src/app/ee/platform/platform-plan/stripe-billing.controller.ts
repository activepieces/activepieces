...
export const stripeBillingController: FastifyPluginAsyncZod = async (fastify) => {
    fastify.post('/stripe/webhook', WebhookRequest, async (request: FastifyRequest, reply) => {
        try {
            ...
            const webhook = stripe.webhooks.constructEvent(payload, signature, webhookSecret)
            ...
            switch (webhook.type) {
                case 'checkout.session.completed': {
                    ...
                    if (session.metadata.type === StripeCheckoutType.AI_CREDIT_AUTO_TOP_UP) {
                        ...
                        const platformId = session.metadata.platformId as string
                        await platformAiCreditsService(request.log).handleAutoTopUpCheckoutSessionCompleted(
                            platformId,
                            paymentMethodId,
                        )
                    }
                ...
                case 'invoice.paid': {
                    ...
                    if (invoice.metadata.type === StripeCheckoutType.AI_CREDIT_AUTO_TOP_UP) {
                        ...
                        await platformAiCreditsService(request.log).aiCreditsPaymentSucceeded(
                            platformId,
                            amountInUsd,
                            StripeCheckoutType.AI_CREDIT_AUTO_TOP_UP,
                        )
                    }
                    break
                }
                ...
                case 'customer.subscription.updated': {
                    ...
                    const { startDate, endDate, cancelDate } = await stripeHelper(
                        request.log,
                    ).getSubscriptionCycleDates(subscription)

                    const extraActiveFlows =
                        subscription.items.data.find((item) => ACTIVE_FLOW_PRICE_ID === item.price.id)?.quantity ?? 0
