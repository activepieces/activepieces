...
export const platformPlanController: FastifyPluginAsyncZod = async (app) => {
    ...
    app.post('/checkout', CheckoutRequest, async (request) => {
        ...
        rejectedPromiseHandler(platformPlanTelemetry(request.log).onCheckoutStarted({ platformId, planId: request.body.planId }), request.log)
    ...
    app.post('/cancel', CancelRequest, async (request) => {
        ...
        rejectedPromiseHandler(platformPlanTelemetry(request.log).onCancelled({ platformId }), request.log)
    ...
    app.post('/reactivate', ReactivateRequest, async (request) => {
        ...
        rejectedPromiseHandler(platformPlanTelemetry(request.log).onReactivated({ platformId }), request.log)
        await provider.refreshEntitlements(platformId)
    })
