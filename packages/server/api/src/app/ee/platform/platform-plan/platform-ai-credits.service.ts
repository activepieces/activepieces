...
import { OpenRouterApikey, openRouterApi } from './openrouter/openrouter-api'
...
export const platformAiCreditsService = (log: FastifyBaseLogger) => ({
    async init(): Promise<void> {
        systemJobHandlers.registerJobHandler(
            SystemJobName.AI_CREDIT_UPDATE_CHECK,
            async ({ apiKeyHash, platformId }) => {
    ...
    async updateAutoTopUp(
        platformId: string,
        request: UpdateAICreditsAutoTopUpParamsSchema,
    ...
    async initializeStripeAiCreditsPayment(
        platformId: string,
        { aiCredits }: CreateAICreditCheckoutSessionParamsSchema,
    ): Promise<{ stripeCheckoutUrl: string }> {
    ...
    async aiCreditsPaymentSucceeded(
        platformId: string,
        amount: number,
        _paymentType: StripeCheckoutType,
    ): Promise<void> {
...
async function getOpenRouterUsageCached(
    apiKeyHash: string,
    log: FastifyBaseLogger,
): Promise<Pick<OpenRouterApikey, 'usage' | 'limit' | 'limit_remaining' | 'usage_monthly'>> {
...
async function tryResetPlanIncludedCredits(
    plan: PlatformPlan,
    apiKeyHash: string,
    log: FastifyBaseLogger,
): Promise<void> {
...
async function tryAutoTopUpPlan(plan: PlatformPlan, apiKeyHash: string, log: FastifyBaseLogger): Promise<boolean> {
    ...
    if (!isNil(plan.maxAutoTopUpCreditsMonthly) && plan.maxAutoTopUpCreditsMonthly > 0) {
        const totalAmountThisMonth = await stripeHelper(log).getAutoTopUpInvoicesTotalThisMonth(
