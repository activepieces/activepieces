...
export const adminPlatformService = (log: FastifyBaseLogger) => ({
  async add({ userId, projectId, name, domain }: AdminAddPlatformParams): Promise<Platform> {
  ...
  retryRuns: async ({ createdAfter, createdBefore }: AdminRetryRunsRequestBody): Promise<void> => {
