...
import { ActivepiecesError, ErrorCode, ProjectPlan, apId, isNil } from '@activepieces/shared'
...
export const projectLimitsService = {
  async upsert(planLimits: Partial<FlowPlanLimits>, projectId: string): Promise<ProjectPlan> {
  ...
  async getPiecesFilter(projectId: string): Promise<Pick<ProjectPlan, 'piecesFilterType' | 'pieces'>> {
    const plan = await projectPlanRepo()
      .createQueryBuilder()
      .select(['"piecesFilterType"', 'pieces'])
