import { ActivepiecesError, ErrorCode, PrincipalType, VerifyLicenseKeyRequestBody, isNil } from '@activepieces/shared'
...
export const licenseKeysController: FastifyPluginAsyncTypebox = async (app) => {
  app.post('/', GenerateTrialRequest, async (req, res) => {
    const { email, companyName, selfHosting, ultimatePlan, productionKey } = req.body as {
      email: string
      companyName: string
      selfHosting: boolean
    ...
    const { message } = await licenseKeysTrialService(req.log).requestTrial({
      email,
      companyName,
      selfHosting,
      ultimatePlan,
