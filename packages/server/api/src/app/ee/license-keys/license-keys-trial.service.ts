...
import { ActivepiecesError, ErrorCode, LicenseKeyEntity, Platform, isNil } from '@activepieces/shared'
...
export const licenseKeysTrialService = (
  ...
  async requestTrial({
    ...
    try {
      ...
      if (selfHosting) {
        const { trialLicenseKey } = await generateSelfHostingTrialLicense(
          email,
          companyName,
          disabledFeatures,
          isEmbeddingsEnabled,
        ...
        const { developmentTrialLicenseKey, productionTrialLicenseKey, subdomain } = await generateCloudTrialLicense(
          email,
          platform.id,
          companyName,
          disabledFeatures,
      ...
      return {
        message: `Failed to request trial. Please try again later or contact support. (${e instanceof Error ? e.message : 'Unknown error'})`,
      }
    }
  },

  async extendTrial({ email, days }: ExtendTrialParams): Promise<void> {
...
async function generateSelfHostingTrialLicense(
  email: string,
  companyName: string,
  ...
  const trialLicenseKey = await generateLicenseKey(
    email,
    companyName,
    disabledFeatures,
    isEmbeddingsEnabled,
    productionKey ? 'production' : 'development',
    log,
  )
  return { trialLicenseKey: trialLicenseKey.key }
}

async function generateCloudTrialLicense(
  workEmail: string,
  platformId: string,
  companyName: string,
  disabledFeatures: string[],
  ...
  developmentTrialLicenseKey: string
  productionTrialLicenseKey: string
  subdomain: string
}> {
  ...
  const developmentTrialLicenseKey = await generateLicenseKey(
    workEmail,
    companyName,
    disabledFeatures,
    isEmbeddingsEnabled,
  ...
  const productionTrialLicenseKey = await generateLicenseKey(
    workEmail,
    companyName,
    disabledFeatures,
    isEmbeddingsEnabled,
...
async function addCNAMERecord(subdomain: string, target: string, log: FastifyBaseLogger): Promise<boolean> {
  try {
    ...
    if (cnameData.success) {
      ...
      const errorMessage =
        cnameData.errors?.map((error: { message: string }) => error.message).join(', ') || 'Unknown error'
