...
import { Cursor, PrincipalType, SeekPage, apId, isNil } from '@activepieces/shared'
...
export const auditLogService = (log: FastifyBaseLogger) => ({
  ...
  sendWorkerEvent(projectId: string, params: AuditEventParam): void {
    rejectedPromiseHandler(
      projectService.getOneOrThrow(projectId).then((project) => {
        rejectedPromiseHandler(
          saveEvent(
            {
...
async function saveEvent(info: MetaInformation, rawEvent: AuditEventParam, log: FastifyBaseLogger): Promise<void> {
  ...
  const user = info.userId
    ? await userService.getOneOrFail({
        id: info.userId,
      })
    : undefined
  const identity = !isNil(user?.identityId)
    ? await userIdentityService(log).getOneOrFail({
        id: user.identityId,
      })
    : undefined
