...
async function sendEmail(
    ...
    telemetry
        ...
        .catch((e) =>
            logger.error(e, '[usageService#handleAlerts] telemetry.trackProject'),
        )
    ...
    emailService
        ...
        .catch((e) =>
            logger.error(e, '[usageService#handleAlerts] emailService.send'),
        )
}
...
async function handleAlerts({
    ...
    const planTasks = projectPlan.tasksPerDay
    ...
    for (const emailTemplate of alertingEmails) {
        ...
        const newThreshold = calculateThreshold(
            consumedTask + numberOfTasks,
            planTasks,
        )

        if (
            threshold < emailTemplate.threshold &&
      newThreshold >= emailTemplate.threshold
        ) {
            if (!emailTemplate.sendForDailyLimit && projectPlan.tasksPerDay) {
                continue
            }

            const project = await projectService.getOneOrThrow(
