...
export const plansService = {
    ...
    async removeDailyTasksAndUpdateTasks({
        projectId,
        tasks,
    }: {
        projectId: ProjectId
...
async function createInitialPlan({
    ...
    try {
        ...
        const defaultPlanFlow = await getDefaultFlowPlan({
...
async function getDefaultFlowPlan({
    platformId,
    email,
}: {
    platformId: string | null
    email: string
}): Promise<FlowPlanLimits> {
