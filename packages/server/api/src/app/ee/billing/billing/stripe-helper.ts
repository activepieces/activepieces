...
function parseStripeSubscription(sub: Stripe.Subscription): FlowPlanLimits {
    ...
    for (const plan of sub.items.data) {
        const productType =
      plan.price.metadata.productType ?? StripeProductType.PRO
        switch (productType) {
            case StripeProductType.PRO: {
                const { tasks, minimumPollingInterval, connections, teamMembers } =
          plan.price.metadata
