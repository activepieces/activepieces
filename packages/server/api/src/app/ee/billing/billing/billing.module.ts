...
const billingController: FastifyPluginAsyncTypebox = async (fastify) => {
    fastify.get('/', async (request) => {
        return {
            ...
            usage: await projectUsageService.getUsageByProjectId(
                request.principal.projectId,
            ),
            plan: await plansService.getOrCreateDefaultPlan({
                projectId: request.principal.projectId,
            }),
            customerPortalUrl: await stripeHelper.createPortalSessionUrl({
                projectId: request.principal.projectId,
            }),
        }
    })
    ...
    fastify.post(
        ...
        async (request: FastifyRequest<Record<string, never>>, reply) => {
            const payloadString = request.rawBody
            const sig = request.headers['stripe-signature'] as string
            try {
                await handleWebhook({
                    payload: payloadString as string,
                    signature: sig,
                })
...
function isSubscriptionPlatformOrCustom(
    subscription: Stripe.Subscription,
): boolean {
    const customProduct = subscription.items.data.find((item) => {
        return (
            item.price.metadata.productType === 'CUSTOM' ||
            item.price.metadata.productType === 'PLATFORM'
        )
    })
