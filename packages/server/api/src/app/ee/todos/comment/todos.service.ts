  ...
  async getOneOrThrow(params: GetParams): Promise<TodoComment> {
    ...
    if (!todoComment) {
      throw new ActivepiecesError({
        ...
        params: {
          entityType: 'todo_comment',
          entityId: params.id,
          message: 'Todo comment by id not found',
        },
      })
    }
    return todoComment
...
async function enrichTodoCommentWithUser(comment: TodoComment, log: FastifyBaseLogger): Promise<TodoCommentWithUser> {
