...
const buildConnectionCheckKey = (platformId: string, connectionId: string) =>
    `${KEY_PREFIX}:check:${platformId}:${connectionId}`

const buildSecretValueKey = (platformId: string, connectionId: string, path: string) =>
    `${KEY_PREFIX}:secret:${platformId}:${connectionId}:${path}`

export const secretManagerCache = {
    async getConnectionStatus(platformId: string, connectionId: string): Promise<boolean | undefined> {
        const result = await distributedStore.get<boolean>(buildConnectionCheckKey(platformId, connectionId))
        return result ?? undefined
    },
    async setConnectionStatus(platformId: string, connectionId: string, value: boolean): Promise<void> {
        await distributedStore.put(buildConnectionCheckKey(platformId, connectionId), value, ONE_HOUR_SECONDS)
    },
    async getSecretValue(platformId: string, connectionId: string, path: string): Promise<string | undefined> {
    ...
    async setSecretValue(platformId: string, connectionId: string, path: string, value: string): Promise<void> {
    ...
    async invalidateConnectionEntries(platformId: string, connectionId?: string): Promise<void> {
