import { ApEdition, FlowRun, RunEnvironment, isFailedState, isFlowUserTerminalState, isNil } from '@activepieces/shared'
...
export const platformRunHooks = (log: FastifyBaseLogger): FlowRunHooks => ({
  async onFinish(flowRun: FlowRun): Promise<void> {
    ...
    const { consumedProjectUsage } = await usageService(log).increaseProjectAndPlatformUsage({
      projectId: flowRun.projectId,
      incrementBy: flowRun.tasks,
...
async function sendQuotaAlertIfNeeded({
  ...
  log,
}: SendQuotaAlertIfNeededParams): Promise<void> {
