...
export const platformMustHaveFeatureEnabled =
    (handler: (platform: PlatformWithoutSensitiveData) => boolean): onRequestAsyncHookHandler =>
    async (request, _res) => {
...
export const projectMustBeTeamType: onRequestAsyncHookHandler = async (request, _res) => {
...
export const platformToEditMustBeOwnedByCurrentUser: onRequestAsyncHookHandler = async (request, _res) => {
    if (
        !request.params ||
