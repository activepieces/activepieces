...
import { UserInteractionJobType, apVersionUtil } from '@activepieces/server-shared'
import {
  ALL_PRINCIPAL_TYPES,
  ApEdition,
  FileType,
...
const basePiecesController: FastifyPluginAsyncTypebox = async (app) => {
  ...
  app.get('/:scope/:name', GetPieceParamsWithScopeRequest, async (req): Promise<PieceMetadata> => {
  ...
  app.post('/options', OptionsPieceRequest, async (req) => {
    ...
    const { result } = await userInteractionWatcher(req.log).submitAndWaitForResponse<
      EngineHelperResponse<EngineHelperPropResult>
    >({
