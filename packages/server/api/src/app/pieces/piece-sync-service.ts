...
export const pieceSyncService = (log: FastifyBaseLogger) => ({
    async setup(): Promise<void> {
        systemJobHandlers.registerJobHandler(SystemJobName.PIECES_SYNC, async function syncPiecesJobHandler(): Promise<void> {
            await pieceSyncService(log).sync({ notifyOtherInstances: true })
        })
        rejectedPromiseHandler(pieceSyncService(log).sync({ notifyOtherInstances: false }), log)
    ...
    async sync({ notifyOtherInstances }: { notifyOtherInstances: boolean }): Promise<void> {
        ...
        try {
            ...
            const added = await installNewPieces(cloudPieces, dbPieces, log, notifyOtherInstances)
...
async function installNewPieces(cloudPieces: PieceRegistryResponse[], dbPieces: PieceMetadataOnly[], log: FastifyBaseLogger, notifyOtherInstances: boolean): Promise<number> {
