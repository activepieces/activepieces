...
export const pieceSyncService = (log: FastifyBaseLogger) => ({
    ...
    async sync({ publishCacheRefresh }: { publishCacheRefresh: boolean }): Promise<void> {
        ...
        try {
            ...
            const [dbPieces, cloudPieces] = await Promise.all([
                pieceRepos().find({
                    select: {
                        name: true,
                        version: true,
...
async function deletePiecesIfNotOnCloud(
    dbPieces: PieceMetadataOnly[],
    cloudPieces: PieceRegistryResponse[],
    log: FastifyBaseLogger,
): Promise<number> {
    const cloudMap = new Map<string, true>(
        cloudPieces.map((cloudPiece) => [`${cloudPiece.name}:${cloudPiece.version}`, true]),
    )
    const piecesToDelete = dbPieces.filter(
        (piece) => piece.pieceType === PieceType.OFFICIAL && !cloudMap.has(`${piece.name}:${piece.version}`),
    )
    await pieceMetadataService(log).bulkDelete(
        piecesToDelete.map((piece) => ({ name: piece.name, version: piece.version })),
    )
    return piecesToDelete.length
}

async function installNewPieces(
    cloudPieces: PieceRegistryResponse[],
    dbPieces: PieceMetadataOnly[],
    log: FastifyBaseLogger,
    _publishCacheRefresh: boolean,
): Promise<number> {
    ...
    for (let done = 0; done < newPiecesToFetch.length; done += batchSize) {
        ...
        await Promise.all(
            currentBatch.map(async (piece) => {
                const url = `${CLOUD_API_URL}/${piece.name}${piece.version ? '?version=' + piece.version : ''}`
                const response = await fetch(url)
                if (!response.ok) {
                    log.warn(
                        { pieceName: piece.name, version: piece.version, status: response.status },
                        '[pieceSyncService#installNewPieces] Error reading piece metadata',
                    )
                    return
                }
                const pieceMetadata = await response.json()
                const { error } = await tryCatch(() =>
                    pieceMetadataService(log).create({
                        pieceMetadata,
                ...
                if (error) {
                    log.debug(
                        { pieceName: piece.name, version: piece.version },
                        '[pieceSyncService#installNewPieces] Piece already exists, skipping',
                    )
                }
            }),
        )
