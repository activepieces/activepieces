...
export const todoService = (_log: FastifyBaseLogger) => ({
  ...
  async getOne(params: GetParams): Promise<Todo | null> {
    return repo().findOneBy({
      id: params.id,
      ...spreadIfDefined('platformId', params.platformId),
  ...
  async resolve(params: ResolveParams) {
    ...
    if (status.continueFlow === false) {
      throw new ActivepiecesError({
        code: ErrorCode.VALIDATION,
        params: {
          message: `Todo cannot be resolved because the continueFlow is set to false for the status: ${status.name}`,
        },
      })
    }
  ...
  async list(params: ListParams): Promise<SeekPage<TodoWithAssignee>> {
    ...
    if (params.statusOptions) {
      if (params.statusOptions[0] === UNRESOLVED_STATUS.name) {
        ...
        query = query.andWhere(
          "status->>'name' != :statusName AND (status->>'continueFlow' IS NULL OR status->>'continueFlow' != :continueFlow)",
          {
            statusName: UNRESOLVED_STATUS.name,
            continueFlow: 'false',
          },
...
async function enrichTodoWithAssignee(todo: Todo): Promise<TodoWithAssignee> {
