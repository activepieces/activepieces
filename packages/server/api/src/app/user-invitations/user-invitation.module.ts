...
import {
    ...
    platformMustHaveFeatureEnabled,
    projectMustBeTeamType,
} from '../ee/authentication/ee-authorization'
...
const invitationController: FastifyPluginAsyncZod = async (app) => {
    app.post('/', UpsertUserInvitationRequestParams, async (request, reply) => {
        ...
        switch (type) {
            case InvitationType.PROJECT:
                ...
                await assertPrincipalHasPermissionToProject(
                    app,
                    request,
                    reply,
                    request.principal,
        ...
        const status = (await shouldAutoAcceptInvitation(request.principal, request.body, request.log))
    ...
    app.post('/accept', AcceptUserInvitationRequestParams, async (request, reply) => {
        const invitation = await userInvitationsService(request.log).getOneByInvitationTokenOrThrow(
...
const getProjectRoleAndAssertIfFound = async (
    platformId: string,
...
async function shouldAutoAcceptInvitation(
    principal: Principal,
    request: SendUserInvitationRequest,
