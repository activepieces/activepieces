...
const invitationController: FastifyPluginAsyncTypebox = async (app) => {
  app.post('/', UpsertUserInvitationRequestParams, async (request, reply) => {
    ...
    const status =
      request.principal.type === PrincipalType.SERVICE ? InvitationStatus.ACCEPTED : InvitationStatus.PENDING
...
const getProjectIdAndAssertPermission = async (
  app: FastifyInstance,
...
async function assertPrincipalHasPermissionToProject(
