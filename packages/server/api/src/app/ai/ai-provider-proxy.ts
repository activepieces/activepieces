...
export const proxyController: FastifyPluginAsyncTypebox = async (fastify, _opts) => {
  fastify.all('/:provider/*', ProxyRequest, async (request, reply) => {
    ...
    try {
      ...
      rejectedPromiseHandler(
        telemetry(request.log).trackProject(projectId, {
      ...
      await reply
        ...
        .type(responseContentType ?? 'text/plain')
        .send(data)
    } catch (error) {
...
const calculateHeaders = (
  ...
  const cleanedHeaders = Object.entries(requestHeaders).reduce(
    (acc, [key, value]) => {
      if (value !== undefined && !forbiddenHeaders.includes(key.toLowerCase()) && !key.toLowerCase().startsWith('x-')) {
