...
export const aiProviderService = {
  ...
  async upsert(
    platformId: string,
    aiConfig: Omit<AiProviderConfig, 'id' | 'created' | 'updated' | 'platformId'>,
  ): Promise<AiProviderWithoutSensitiveData> {
    const existingProvider = await this.getOrThrow({
      platformId,
      provider: aiConfig.provider,
    }).catch(() => null)
    const existingHeaders = existingProvider?.config.defaultHeaders ?? {}
    const nonEmptyHeaders = Object.fromEntries(
      Object.entries(aiConfig.config.defaultHeaders).filter(([_, v]) => !isNil(v)),
    )
    const newHeaders = { ...existingHeaders, ...nonEmptyHeaders }
    const encryptedConfig = encryptUtils.encryptObject({
      ...aiConfig.config,
...
// TODO (@abuaboud) clean up this function
async function fallbackToCloudPlatformList(
  platformId: PlatformId,
): Promise<SeekPage<AiProviderWithoutSensitiveData> | null> {
  const isCloudEdition = system.getEdition() === ApEdition.CLOUD
  if (isCloudEdition) {
