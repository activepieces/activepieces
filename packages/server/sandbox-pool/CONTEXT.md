...
Glossary for the execution-cache / sandbox-pool domain. Terms only — no implementation details.
...
- **Code-only payload:** v1 ships compiled `index.js` per code step (what the engine imports); on hit the worker writes them to the code cache and provisions with `codeSteps: []` (skips compile); pieces install via the manifest as normal.

Key files: `cache/flow/flow-provisioning.ts` (resolve hit/miss + publish), `cache/flow/flow-bundle-store.ts` (fetch/publish + codec), `cache/flow/code/code-cache.ts` (compiled-step layout), `local-pool-runtime.ts` (lifecycle), `worker-rpc-service.ts` (`getFlowBundle`/`uploadFlowBundle`), `file.service.ts` (`getLocationForFile` → S3 for `FLOW_BUNDLE`), `worker-contract.ts` (contract).
...
### Flow Provisioning
`cache/flow/flow-provisioning.ts`. The deep seam over "turn a `flowVersionId` into `{ flowVersion, pieces, codeSteps }`." `resolve()` hides the Flow Bundle hit vs. fetch-resolve-and-mark-for-publish decision (and the disable-on-missing-piece path, formerly `flow-artifacts.ts`, now inlined); `publishBundle()` is the best-effort write after a miss. `local-pool-runtime` keeps only sandbox lifecycle: resolve → acquire → install → publish.
