...
- **Connection-piece binding (opt-in, CE+EE)**: by design, any piece a flow author points at a connection can read it in full plaintext — the `Read Connection` piece exists specifically for this, it's not a bug. `AP_ENFORCE_CONNECTION_PIECE_BINDING=true` (default `false`) locks a connection to the piece that created it, checked server-side at the one chokepoint every connection fetch passes through (`app-connection-worker-controller.ts`'s `GET /v1/worker/app-connections/:externalId`, keyed off the engine-supplied `requestingPieceName` query param). Pieces with no fixed destination (HTTP, GraphQL, Webhook, and self-hosted/DB-style pieces — see `generic-destination-pieces.ts`'s hand-maintained allowlist) get connection access blocked outright even when correctly bound, since there's no host to restrict them to. Code/Loop/Router steps have no piece identity at all, so they lose all connection access under this flag — this subsumes what would otherwise need a bespoke Code-step-specific block. `Read Connection` gets no exemption (it's never "bound" to any connection by construction) and goes fully non-functional the instant the flag is on — that's intended, not a bug to route around. Hard cutover: applies to existing connections/flows immediately, no grace period, no warn-only mode.
