...
- `AP_ENFORCE_CONNECTION_PIECE_BINDING` (default `false`) makes a step resolve only connections whose `pieceName` equals the step's own piece. The engine appends `?pieceName=` to `GET /v1/worker/app-connections/:externalId` and the **server** rejects a mismatch with `MCP_PIECE_CONNECTION_MISMATCH` (the engine turns that into a USER-level `ConnectionPieceMismatchError`). Code / loop / router steps have no piece and send no name, so they are never gated — turning the flag on does not stop a code step from reading any connection.
