...
- **OTP** (`EMAIL_VERIFICATION`, `PASSWORD_RESET`, `EMAIL_LOGIN`): 10-min expiry + 10-min resend window; states PENDING/CONFIRMED; one row per `(identityId, type)`, DB-enforced. The first two carry a `randomUUID()` delivered as a link; `EMAIL_LOGIN` carries a 6-digit code the member types, and its row counts `attempts` so it dies after five wrong guesses. See [000015](../../decisions/000016-email-sign-in-is-a-typed-code-on-the-existing-otp-primitive.md).
