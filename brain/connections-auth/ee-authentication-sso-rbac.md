...
- **OTP** (`EMAIL_VERIFICATION`, `PASSWORD_RESET`): per-type expiry (`OTP_EXPIRATION_MS` in `otp-service.ts`: 24h verification, 10-min reset); states PENDING/CONFIRMED. Resend re-delivers the existing pending OTP value and refreshes `updated`, restarting the expiry clock so the newest email always carries a full validity window — a new value is only generated once the OTP is expired or confirmed (GIT-1733: the old early-return made resend a silent 204 no-op).
