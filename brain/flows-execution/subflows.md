...
- **Process in Batches** (core step, not a piece): resolves an `items` expression, `chunk`s it by `batchSize`
  ...
  - Output is a fan-in **summary** (`expected`/`succeeded`/`failed`/`canceled`/`stillRunning`/`notStarted`/
    `failedToDispatch`/`timedOut` plus an `exceptions` array), never per-iteration results — the deliberate
    reason it is not a `LOOP_ON_ITEMS` mode. The barrier mechanics live in
...
- **"Fan-out" now means two unrelated code paths, and `lib/fan-out.ts` is only one of them.** The piece's bounded-window dispatcher is the CSV one; `PROCESS_IN_BATCHES` has its own loop in `process-in-batches-executor.ts` with the same `maxInFlight: 5` shape but a different transport, its own resumability, and no Callable Flow trigger anywhere in it. A change to the piece's dispatcher does not reach the step, and vice versa.
