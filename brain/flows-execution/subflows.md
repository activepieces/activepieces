...
### Caller authentication (HMAC signature, in the piece)
A Callable Flow (`>= 0.7.0`) only runs for callers that prove they are in the **same project** — verified **inside the piece**, exactly like a generic webhook piece checks a Stripe/GitHub signature, so there are **no server-side webhook changes**. The shared key is a per-project secret the server derives on demand: `GET /v1/engine/subflow-signing-secret` (engine-principal auth, in `engine-controller.ts`) returns `HMAC(jwtSecret, "subflow-signing:" + projectId)` — derived, zero-config, never stored, never user-visible. Both halves fetch it with their own engine token (`fetchSubflowSigningSecret` in the piece's `common.ts`):
- `Call Flow` / `Stream CSV to Subflows` sign the POST body with `signSubflowPayload` (HMAC-SHA256 over a **canonical**, key-sorted JSON so it survives the JSON round-trip) and send it in the `ap-subflow-signature` header. Stream CSV fetches the secret once and reuses it across batches.
- The `callableFlow` trigger's `run()` recomputes the HMAC with **its own** project's secret and `timingSafeEqual`-compares (`verifySubflowSignature`); on mismatch/absence it returns `[]` (the flow simply does not run). Same project → same secret → verifies; cross-project or external caller → different/no secret → dropped.

Because the piece now depends on the `/v1/engine/subflow-signing-secret` endpoint, `0.7.0` sets `minimumSupportedRelease: '0.89.0'` — the platform won't run it against an older engine that lacks the endpoint. Gating is intrinsic to the piece version and needs no server check: only `>= 0.7.0` code signs and verifies. Because a flow pins each piece step to an **exact** version, caller and callee upgrade independently — old→old and new→new both work; new parent → old subflow works (old trigger ignores the header); an old parent (no signature) calling a `>= 0.7.0` subflow is **dropped** until its Call Flow action is upgraded (normal piece-version migration).

Trade-offs baked in: the derived secret is readable by any piece running in that project (same trust boundary as the already-exposed engine token), and rejection is **late** — the webhook still returns `200` and a job is queued before `run()` drops it (no `401`). Verification happens on real webhook invocations only; builder trigger-testing uses the mock `test()` payload.
