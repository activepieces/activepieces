...
A **Subflow** is a flow invoked by another flow instead of by its own external trigger — a reusable function at flow granularity. The `@activepieces/piece-subflows` core piece supplies both halves: the **Callable Flow** trigger that makes a flow callable, and the actions a parent uses to reach it. A parent calls a subflow once (**Call Flow**, optionally waiting for a response through a waitpoint) or fans out many calls from one streaming step (**Stream CSV to Subflows**). The **Respond** action sends data back to a waiting parent. Every *piece* call is a webhook `POST` to `/v1/webhooks/:flowId`.

There is a second fan-out shape that is **not** a piece and does not use that transport: the
**Process in Batches** core step (`FlowActionType.PROCESS_IN_BATCHES`), which fans a list out to child runs of
the *parent's own* flow version and waits for all of them. See the fan-out section below.
...
- **Process in Batches** (core step, not a piece): resolves an `items` expression, `chunk`s it by `batchSize`
  (default 10, minimum 1), creates a fan-in barrier carrying the intended child count and a sha-256 of
  `{ seed, items }`, dispatches each batch through `POST /v1/flow-runs/dispatch` at `maxInFlight: 5`, seals,
  and pauses. The released summary comes back on the resume payload and the step either succeeds with it or
  fails on `failed > 0 || timedOut`, per Continue on Failure.
  - **Children run the parent's flow version from the body's entry step** (`action.firstLoopAction`), not a
    separate callable flow — no Callable Flow trigger, no dropdown, no published-version resolution. That is
    the whole reason it cannot ride the webhook transport the piece uses.
  - What travels is the union of `{{ stepX.* }}` references across the body's steps, plus the batch itself
    seeded as the step's own output, so `{{ thisStep.items }}` inside the body is the slice.
  - Output is a batch **summary** — the shared `BarrierSummary` (`total`/`succeeded`/`failed`/`rejected`/
    `canceled`/`notDispatched`/`stillRunning`/`timedOut` plus a `signals` array covering every batch, or
    `signalsTruncated` above 100) spread onto `{ barrierId, totalItems, batchSize }` — never per-iteration results, the deliberate
    reason it is not a `LOOP_ON_ITEMS` mode. The step hands its whole item list to the server in one call and
    a server-side worker job creates the children; see [Server-side fan-out](./server-side-fan-out.md). The
    barrier mechanics live in
    [Fan-in entry points](./fan-in-entry-points.md); the four sizing ceilings live in
    [the step's user docs](../../docs/flows/process-in-batches.mdx).
...
- **"Fan-out" now means two unrelated code paths, and `lib/fan-out.ts` is only one of them.** The piece's bounded-window dispatcher is the CSV one and runs **in the engine**; `PROCESS_IN_BATCHES` dispatches **server-side**, from `fan-out-dispatcher-job.ts` on the `BARRIER_JOBS` queue, with a different transport, its own resumability, and no Callable Flow trigger anywhere in it. The engine side of the step is now one `POST /v1/waitpoints` and a pause. A change to the piece's dispatcher does not reach the step, and vice versa.
- **`MAX_DISPATCHES_IN_FLIGHT = 5` throttles dispatch, not execution — there is no concurrency knob for batches.** The `p-limit` window in `fan-out-dispatcher-job.ts` bounds how many children the server-side worker job creates at once; every child it creates is queued in BullMQ immediately, so all N batches *run* as fast as worker capacity allows. Turning it up buys nothing for throughput and only makes the dispatcher a noisier neighbour.
- **MCP batch authoring is half-open, and the tests assert both halves.** `ap_add_step` / `ap_update_step` can create and edit a `PROCESS_IN_BATCHES` step (`loopItems` + `batchSize`, default 10); `ap_build_flow` and `ap_validate_step_config` still reject the type, and `ap_build_flow` still rejects `INSIDE_BATCH` as a nesting location. The negative "cannot author a batch" assertions live in `test/integration/ce/mcp/mcp-tools.test.ts` — when you open a surface, that file fails until you flip its test, which is the intended tripwire. One asymmetry left standing: `ap_add_step` writes `errorHandlingOptions` on a batch step but `ap_update_step` refuses `continueOnFailure`/`retryOnFailure` on anything but CODE/PIECE.
- **Batch children are hidden from every list surface, so "the subflow ran but I can't find the run" is expected.** Run lists filter `parentWaitpointId IS NULL`; a batch child is reachable only from its parent's run detail or by id. A *CSV* subflow child is a normal run and stays listed — the two fan-outs differ here.
...
- `packages/server/engine/src/lib/handler/process-in-batches-executor.ts` — the Process in Batches step (core, not a piece)
- `packages/server/api/src/app/flows/flow-run/flow-run-service.ts` — `dispatchChild`, the batch transport
