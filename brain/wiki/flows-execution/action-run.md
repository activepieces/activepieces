...
- A watcher timeout maps to `TIMEOUT`, **never** `INTERNAL_ERROR` — reporting "the engine crashed while loading or executing the piece" when nothing ran sends the calling agent off debugging the piece instead of retrying. The MCP timeout text also tells the agent the write may have landed, so it checks before re-running.
