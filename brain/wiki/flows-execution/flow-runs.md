...
- **12 statuses**: 3 non-terminal (QUEUED, RUNNING, PAUSED) + 9 terminal (SUCCEEDED, FAILED, TIMEOUT, CANCELED, QUOTA_EXCEEDED, PROJECT_INACTIVE, MEMORY_LIMIT_EXCEEDED, INTERNAL_ERROR, LOG_SIZE_EXCEEDED).
...
- **The trigger step's status IS the raw-vs-extracted discriminator for retry** — there is no separate field (a `payload` field was tried and removed as redundant). `FAILED` means "`output` holds a raw event, re-run `run()` on it" (`executeTrigger: true`); `SUCCEEDED` means "`output` is already the trigger's result, replay as-is" (`executeTrigger: false`). So any code that *fabricates* a trigger step without the engine having run — the admission gates below are the only ones today — must pick the status from where its payload came: raw for sync webhooks, extracted for anything sourced from the worker RPC `submitPayloads` (which passes post-`TriggerHookType.RUN` output). Get it wrong on a polling trigger and retry re-polls against an already-advanced `lastPoll` cursor, so the run gets `undefined` or an unrelated newer item *and* silently consumes those fresh items' own runs.
- **Two admission gates share one helper.** `createAdmissionBlockedRun({ status, ... })` writes a terminal run + its synthetic trigger log without ever enqueuing a job; `QUOTA_EXCEEDED` (out of credits) and `PROJECT_INACTIVE` (project suspended — see the Projects page) are its only callers. The project gate runs **first** and short-circuits the credit gate, so a suspended tenant never pays a billing-provider round-trip. Both statuses are excluded from the `onFinish` credit charge, and neither is in `FAILED_STATES` for `PROJECT_INACTIVE` — it is not a flow failure, so it raises no issue or alert. Replay is "retry on latest version"; `isFailedState` only gates the *from-failed-step* option, which correctly stays hidden for it.
