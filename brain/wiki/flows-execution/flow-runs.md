...
- **Waitpoint** — row per paused step: `type` (DELAY|WEBHOOK), `version` (V0|V1), `status` (PENDING|COMPLETED), unique on `(flow_run_id, step_name)`. Fan-in barriers add `is_fan_in` (bool), `expected_children` (int, null until sealed), `fan_in_baseline` (JSONB `{succeeded, failed, canceled}`, immutable watermark captured at create).
...
- **A fan-in barrier's only backstop is a Redis job, and nothing reconciles it from Postgres.** The barrier row carries `resumeDateTime`, but the sole thing that acts on it is the `resume-delay-<waitpointId>` BullMQ job — no periodic sweep scans for sealed-PENDING barriers whose deadline has passed. Lose Redis and the barrier hangs until retention deletes the run. `refill-paused-jobs.ts` looks like it recovers this on next boot but does not: it filters `waitpoint.type !== PauseType.DELAY`, and barriers are created as `WEBHOOK`.
