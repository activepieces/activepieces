...
- **A run has exactly one resumable program counter.** Resume rebuilds a single execution context from the log file and re-enters the executor at one point, and `loop-executor.ts` returns out of the *whole* loop on any non-RUNNING verdict. Combined with the `(flow_run_id, step_name)` unique waitpoint key, a step can only ever have one waitpoint pending at a time — sequential loops work because iteration N+1 reuses the row after N completes. Anything wanting concurrent independent waits (parallel fan-out) needs a second resumable unit; see decision 000015.
