...
- **A step that contains other steps cannot be a piece.** Nesting is declared in the typed `FlowAction` union (`firstLoopAction` on LOOP_ON_ITEMS, `children` on ROUTER, `continueOnFailureBranches`) and the canvas hard-switches on `step.type` to lay out the subgraph (`flow-canvas-util.ts`), so piece metadata has no way to express a body. "Build a piece with Loop-on-Items UI" is therefore always a new `FlowActionType`: ~52 files across 4 packages branch on `LOOP_ON_ITEMS` (17 server/api, 16 web, 13 core/execution, 6 server/engine), plus a `LATEST_FLOW_SCHEMA_VERSION` bump and its migration. The pieces-only alternative is dispatching to a *separate* flow behind a Callable Flow trigger — cheap, but no nested section. See [fan-in entry points](./fan-in-entry-points.md) for the worked case (`BULK_PROCESS` vs. the CSV piece).
