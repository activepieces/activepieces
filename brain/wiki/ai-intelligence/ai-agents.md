...
- **The enums and pure functions have exactly one home: `core/piece-types/src/lib/agent-primitives.ts`** — deliberately zod-free so `core-execution` can re-export it without dragging `zod/mini` into server bundles. Do not re-declare `AgentToolType`, `McpAuthType`, `buildAuthHeaders`, `TASK_COMPLETION_TOOL_NAME`, or `mcpToolNameUtils` in `core-execution`; re-export them. They used to be duplicated byte-for-byte across both packages, which was silently load-bearing: if `createToolName` drifted, the tool names `migrate-v16` persisted would stop matching runtime names and every piece/flow/MCP call on a migrated flow would degrade to `ToolCallType.UNKNOWN`. `mcp-tool-name-util.test.ts` now asserts both entry points resolve to the *same object*, so a re-fork fails the test rather than shipping.
