...
- **`packages/pieces/community/ai` pinning an exact `ai` patch (`6.0.170`) while `packages/pieces/framework` floats `^6.0.0` (resolved `6.0.240` as of 2026-08) breaks the piece's own build, not just cross-major mismatches.** Bun's isolated linker installs the two as separate copies, so `tools.ts`'s `Record<string, Tool>` return spreads a `Tool` built from the framework's resolved version (`context.agent.tools()`) together with a `Tool` from piece-ai's own pinned `dynamicTool` — structurally close enough to read as duplicated, not assignable, in the `tsc` error (`Type '{ [x: string]: Tool | Tool; }' is not assignable to type 'Record<string, Tool>'`). Unlike the CI gap noted above, this hits the **"Build all pieces projects"** CI step, which runs unconditionally on every PR (not gated on the diff touching `packages/pieces/**`) — so it goes red for unrelated PRs the moment something else in the workspace bumps the resolved `ai@6.x`. Fixed on `fix/ai-sdk7-pieces-engine-lockstep` (#14605), which moves `piece-ai`, `pieces-framework`, and the engine to `ai@7` together instead of re-pinning within v6.
