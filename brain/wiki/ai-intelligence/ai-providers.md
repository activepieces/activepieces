...
- **One credential per provider is baked in at three layers, not one.** `ai_provider` carries `UNIQUE(platformId, provider)` (`ai-provider-entity.ts`, index `idx_ai_provider_platform_id_provider`), `getConfigOrThrow({ platformId, provider })` looks a row up **by provider name**, and `GET /:provider/config` + `GET /:provider/models` are keyed by name too — the last with `securityAccess.engine()`, so the worker is a caller. Note `POST`/`DELETE` already key by **id**. Any "several keys for one provider" design (e.g. a cheap Anthropic key for all projects plus a premium one for a few) therefore needs a migration dropping that index *and* a rewrite of those name-keyed lookups with an engine-side story for which credential a run picks — it is not a frontend change. There is also no per-project scoping of AI providers today: `projectIds` exists on app connections, secret managers and piece sets, but nothing in `ai/`, and `project_plan` has no AI limit.
