...
- **Chat model tiers are Activepieces-only.** `ACTIVEPIECES_CHAT_TIERS` (`fast`/`smart`/`premium`, shown as Fast/Expert/Heavy) hold OpenRouter-shaped Anthropic ids, so they are only meaningful for the ACTIVEPIECES and OPENROUTER chat providers. Any provider that declares `ALLOWED_CHAT_MODELS_BY_PROVIDER` (openai, anthropic, google) must select a real model id from that list instead. Read it through `aiProviderUtils.getCuratedChatModels({ provider })` — the one accessor both the server resolver and the chat dropdown share; it returns `{ id, label }`, pairing each curated id with a display name from `CHAT_MODEL_LABELS` (falling back to the id). Labels are hardcoded on purpose: `AIProviderModel.name` from `listModels` is the vendor's display name for anthropic/google but OpenAI's `/v1/models` returns ids only, so the live list cannot supply a friendly name for every provider — both in `chatHelpers.resolveModelIdForProvider` and in the chat model dropdown. `conversation.modelName` is a free string that carries either a tier id or a real model id; a legacy tier id resolves to the tier's equivalent model when the provider ships it, else the provider's first model. Naively stripping the tier's vendor prefix for every provider is what once sent `claude-haiku-4-5` to OpenAI.
