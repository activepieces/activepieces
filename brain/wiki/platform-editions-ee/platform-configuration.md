...
- **A `platformId` column is not a foreign key.** Verified against the dev schema: only 18 FKs actually reference `platform(id)`, but 9 tables carry a `platformId` with **no FK at all** — `user`, `file`, `app_connection`, `piece_metadata`, `project_member`, `project_role`, `user_invitation`, `mcp_oauth_token`, `mcp_oauth_authorization_code`. Those never block a delete and never cascade; they orphan silently. Any teardown must delete them explicitly. Do not infer cascade behaviour from the presence of the column.
- **What actually blocks `DELETE FROM platform` today** is four constraints: `project` and `signing_key` are `RESTRICT`, `tag` and `piece_tag` are `NO ACTION`. The other 14 FKs already `CASCADE`. So platform deletion fails for any tenant with a signing key or a tag, not — as it appears — for any tenant with a second member.
...
- Any new entity carrying `platformId` should declare its FK `ON DELETE CASCADE`, or it silently breaks (or silently orphans on) platform deletion. Nothing in CI checks this. See [000024](../../decisions/000024-delete-platform-is-a-cloud-owner-action-purged-by-one-cascading-job.md).
