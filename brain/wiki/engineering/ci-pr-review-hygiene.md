...
## Gotchas
- **Greptile's dashboard filters run before mention handling.** The author filter that skips `dependabot`/`renovate` PRs is evaluated first, so if it ever over-matches (an inverted include-list, a blank entry, a `*bot*` glob), every PR shows "skipped due to filters" in the dashboard and `@greptileai review` does not override it. On 2026-07-27 that silently killed review on every PR from #14437 onward; the fix is in the dashboard, never in `.github/`.
