...
- **Async**: offload payload → queue `EXECUTE_WEBHOOK` → return 200. Note `handleAsync` calls `offloadPayload` **unconditionally**, not `maybeOffloadPayload` — the inline threshold (`AP_WEBHOOK_PAYLOAD_INLINE_THRESHOLD_KB`, default 512KB) applies to the `addToQueue` path, not to async webhook ingest. Job carries a `JobPayload` union (`inline` or `ref`); the **engine** resolves it at execution time (workers no longer fetch payloads).
...
### Gotchas
- **Postgres is already synchronously on the async ingest path, so the durability gap is cheap to close.** [[000010-async-webhook-ack-is-redis-durable-not-postgres-durable]] justifies the Redis-only ACK by "keeping Postgres off the ingest hot path", but that premise no longer holds: `offloadPayload` → `fileService.save` ends in `fileRepo().save(...)` on **every** branch, S3 included (S3 stores the blob, Postgres always stores the row). So an async webhook already pays a Postgres INSERT before its 200, and already fails when Postgres is down. What it does *not* write is the routing intent — `flowId`, `flowVersionId`, `requestId` — which lives only in the Redis job. The accepted-but-unrecoverable window is therefore a missing-columns problem, not a missing-write problem: the `file` row is a durable receipt with the wrong fields on it, and `metadata` is a free nullable jsonb column on that same INSERT.
