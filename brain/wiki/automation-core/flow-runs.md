...
- **Fan-in barrier** (subflow join): a piece dispatches N children (`ap-parent-run-id`, no `callbackUrl`) around a single `createWaitpoint({ isFanIn: true })` → `sealFanIn({ expectedChildren, failedToDispatch, timeoutAt })` — `expectedChildren` is the count the dispatch loop actually got accepted, so a partial dispatch still seals and waits on what landed instead of leaving the barrier unsealed. At seal, on every child terminal transition, and at timeout, the barrier re-evaluates `sealed ∧ no non-terminal child ∧ (terminal − baseline) >= expected` from one indexed `GROUP BY` over `flow_run.parentRunId`; the first evaluator to win `complete()`'s pessimistic lock resumes the parent once with a `{expected, succeeded, failed, canceled, stillRunning, failedToDispatch, timedOut}` summary. No counter, no poll — the barrier row is never written from a child's transaction. Keyed on the one-PENDING-waitpoint-per-run invariant — see decision 000013. Piece: subflows `Fan Out to Subflows`.
...
- **A retried run has its waitpoints cleared** — `FROM_FAILED_STEP` calls `deleteByFlowRunId`, so a re-executed step cannot inherit the previous attempt's waitpoint (or a fan-in barrier's stale baseline).
- **Mixing a still-running fire-and-forget fan-out with a wait-for-all fan-in in one run throws at barrier create**, and so does a fan-in that follows an earlier fan-in which timed out leaving stragglers.
