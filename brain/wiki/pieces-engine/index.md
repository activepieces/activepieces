...
- **Gotchas**:
  - **`runInChildProcess` guards `typeof child.send !== 'function'` before sending, and that guard must stay.** It reads like dead code — `send` is always there on a healthy spawn — but Node assigns it per-instance inside `setupChannel()`, which `ChildProcess.prototype.spawn` skips entirely when the spawn fails with `EMFILE`/`ENFILE`. Calling it then throws a synchronous `TypeError` that settles the promise *before* the `'error'` handler can reject with the real errno, which only arrives a tick later. Deleting the guard silently restores that. Only EMFILE/ENFILE behave this way — `EAGAIN` and `ENOENT` still define `send`. Pinned by `no-op-code-sandbox.test.ts`, which fails with the literal historical string `child.send is not a function` if the guard goes.
  - Historically that masking was total, which is why the isolate 64-fd cap above went unnoticed for three and a half years: fd exhaustion surfaced as `TypeError: o.send is not a function` blamed on a random CODE step, nothing reached the logs at all, and the worker's own wide event recorded `"outcome": "success"`. The only artifact anywhere was the customer's failure-alert email quoting a bogus stack. It looked fleet-wide and random — many workers, many platforms, a different CODE step each time — because every sandboxed engine shares the same cap. Expect `spawn EMFILE` now instead.
