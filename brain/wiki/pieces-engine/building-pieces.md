...
- **Whether a `{{ }}` input keeps its native type depends on whether the whole input is exactly one token.** `resolveInputAsync` (`packages/server/engine/src/lib/variables/props-resolver.ts`) branches five ways and each yields a different type, so the same property can hand `run()` an array on one flow and a string on another. Whole-input single token (`'{{ trigger.body.units }}'`, the `inputContainsOnlyOneTokenToResolve` branch) **preserves the native type** — real array, boolean, object, number. A token inside text (`'a {{ x }} b'`) always collapses to a string, `JSON.stringify`ing non-strings in place. `{{ flattenNestedKeys(…) }}` returns `string[]`. An `ap-formula-v1::{…}` wrapper returns whatever the formula returns, `''` on nullish. An input with no `{{ }}` at all is returned **untouched**, so a hand-typed or fx-stringified literal like `'["year","month"]'` reaches the processor as that exact string. A token that throws or resolves nowhere is swallowed by `evalInScope` and becomes `''` — never nil, which is why processors must map `''` to `undefined` on optional props. Type-check at the top of `run()`; don't assume the shape the property declares.
