...
- **OutputSchema** — optional per-action/trigger structured render hint (`fields`, `itemLabel`); set by the piece author, consumed by the builder's Smart Output Viewer and data selector. Opt-in and non-breaking. A top-level field marked `sensitive: true` is redacted in run logs and builder display (resolved via `value` if set, else `key`); the real value still reaches downstream steps at runtime.
