...
- **`/current` is not the widest door onto piece-set resolution — `GET /v1/pieces` is.** The pieces list and get routes are `securityAccess.unscoped(ALL_PRINCIPAL_TYPES)` and take `projectId` straight from the query with no membership check, yet they call the same `pieceSetService.getForProject`. `/current` is strictly tighter: it runs `securityAccess.project(..., { type: ProjectResourceType.QUERY })`, so the caller must at least be a member. Anyone hardening this path should start at `piece-metadata-controller.ts`, not at `/current`. What contains it today is that `getForProject` resolves *both* the project and the set against the principal's `platformId` — never derive the platform from `project.platformId` instead, or an unscoped caller starts reading another platform's visibility.
