...
- **Fixing that ACL without also fixing `enqueueBundleJob` regresses within seconds — the S3 cache re-poisons itself.** S3 is consulted *before* the CDN, and `enqueueBundleJob` fires on every S3 **miss**, before the CDN branch is reached, then unconditionally caches `npmTarballUrl` — the unbundled copy. So purging the `pieces/` prefix buys exactly one good request per piece: miss → enqueue → CDN serves the bundle, then the job writes the npm tarball back and every later request is unbundled again. Correct order: (1) make `pieces/retro/*` readable, (2) change the job to cache the CDN artifact when one exists — 1 and 2 must ship together, (3) purge the S3 `pieces/` prefix, (4) bump `LATEST_CACHE_VERSION` so worker bun stores drop their already-installed trees (`pieceCheckIfAlreadyInstalled` short-circuits on a `ready` marker, and the volume survives deploys — only `deleteStaleCache` removing non-current versions clears it). `cdnVerifiedUrls` needs nothing: it is positive-only, so it never holds a stale negative.
