...
- **`USE_CDN_FOR_BUNDLES` does not mean "pieces are single files" — and the CDN is currently empty.** It is an *app-side* prop (default `'true'`, read in `piece-bundle.ts`, never on the worker) that only chooses the download **URL**. Resolution order is archive → S3 (`AP_S3_BUCKET`, set on cloud) → CDN (OFFICIAL only) → npm. Measured: `cdn.activepieces.com/pieces/retro/` 404s for both old *and* current piece versions, so in practice npm serves everything and the flag changes nothing today. Whether a piece is self-contained is a property of **how that version was published**, not of this flag. Note also `enqueueBundleJob` fills the S3 cache from `npmTarballUrl`, so S3 mirrors npm rather than the CDN.
