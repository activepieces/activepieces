...
- **Chunk embeddings are produced lazily, on the first search — not on upload.** The upload path still only *accepts* an embedding, so chunks land with `embedding IS NULL`; `executeKnowledgeBaseTool` calls `knowledgeBaseService.embedPendingChunks` before searching, which embeds whatever is still NULL in batches of 50 and writes each vector back by chunk id. Between #14699 (which deleted the piece-side producer when the step became a thin client) and this restoration there was no producer at all, and every search silently returned an empty answer for every provider. If searches come back empty again, check that a producer still runs — the search-side filter `embedding IS NOT NULL` makes a missing producer look like "no relevant information found" rather than an error. The other half of the old dance, `POST /files/search` plus `/:id/chunks` and `/:id/store-chunks`, still exists for callers that bring their own vector; nothing in the repo calls it now.
