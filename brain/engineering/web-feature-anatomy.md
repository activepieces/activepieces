...
- **`overflow-y-auto` alone silently turns on horizontal scrolling too.** CSS computes `overflow-x: visible` to `auto` whenever the other axis is not `visible`, so a scroll container clips on *both* axes. Any decoration that paints outside an element's box — Tailwind's `ring-offset-*` halo, `hover:scale-*` — then bleeds past the container and pops a phantom scrollbar. Pair `overflow-y-auto` with an explicit `overflow-x-hidden` plus padding sized for the bleed. Bit `run-details/iteration-rail.tsx`, whose batch dots are `size-2.5` in a row exactly `size-2.5` tall.
