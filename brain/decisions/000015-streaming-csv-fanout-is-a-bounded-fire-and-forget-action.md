...
## Context
The driver is fanning a large CSV out to one subflow run per batch of rows without the parent step ever holding the file. This was designed while framework read-side streaming was still deferred as YAGNI (see [000008](./000008-streaming-file-writes-go-through-the-app-one-path.md)), so a piece reading its own source stream was the only available path.
...
- **A plain `Property.File` input** is impossible here: the engine materializes it into an `ApFile` Buffer before `run()` starts, so the whole file is in memory before any streaming code executes. A URL the piece streams itself was the only shape that avoided that.
...
Read-side streaming has since landed independently: `Property.File({ streaming: true })` resolves to an `ApStreamingFile` with a `body: Readable` (pieces-framework ≥ 0.35.0, [000014](./000014-streaming-file-inputs-resolve-to-a-lazy-apstreamingfile.md)). The URL input is therefore no longer the *only* streaming-capable shape — moving the input to a streaming `Property.File` would additionally accept the builder's file picker and base64 data URLs, and hand the `fetch` to the engine. It is a follow-up, not part of this decision; the URL input ships as-is and the fan-out half is unaffected either way.
