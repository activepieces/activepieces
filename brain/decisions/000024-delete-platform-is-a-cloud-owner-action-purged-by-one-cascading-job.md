...
- **Explicit ordered deletes in the job over flipping the four blockers to `ON DELETE CASCADE`.** Cascade would make correctness the schema's job, and the ordered list is one a human maintains forever — but buying that costs a destructive migration (`DROP CONSTRAINT` on `project` and `signing_key`, both hot tables) to change how a once-per-tenant Cloud-only job behaves. The job already has to enumerate the nine unconstrained tables by hand, so the migration removes four names from a list it does not remove. Nine tables of hand-maintenance is the same failure mode as thirteen; the DDL is the only new risk. Revisit if the list starts drifting in practice.
