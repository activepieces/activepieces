...
- **The piece-bundle CDN tier is dead weight until the bucket ACL is opened.** The `pieces/retro/` prefix of the `activepieces-cdn` DO Spaces bucket *is* seeded (a 2026-06-28 snapshot, ~1735 tarballs, roughly one version per minor line) but every object returns **`403 AccessDenied`** — on `cdn.activepieces.com` and the Spaces origin alike, while `cdn.activepieces.com/pieces/<logo>.png` returns 200, so it is the object ACL and not the CDN. `cdnBundleExists` (`piece-bundle.ts`) counts only `2xx` as present and caches **hits only** (`cdnVerifiedUrls`), so turning `AP_USE_CDN_FOR_BUNDLES` on today buys one wasted `HEAD` per resolve and still falls back to npm. Two related sharp edges: `release-pieces.yml` does **not** mirror to the bucket, so any version published after the snapshot permanently misses; and `safeHttp.axios` sets no `timeout`, so an egress policy that blackholes the CDN hangs the existence check for the OS TCP connect timeout on the piece-install path instead of failing fast.
...
- **`resolvedStepOutputCache` is an unbounded `Map`.** `FlowExecutorContext` caches materialised log-slice downloads (keyed by `fileId`) in an in-process `Map` with no eviction. A run with many distinct large slices (each just over the 32 KB threshold) can inflate the engine heap proportionally — there is no byte-budget cap. Tracked in SRE-206 (Phase 1: bound with LRU eviction).
- **The engine process assumes one operation at a time.** `flowRunProgressReporter` (and, on the sqlite-state-store branch, `runStateStore`) are module-level singletons initialized per `executeOperation` in `worker-socket.ts` and torn down in its `finally`. Nothing in the RPC server enforces serial dispatch — a concurrent op on the same process would share/clobber that global state (e.g. dispose another run's state store mid-flight). Any new per-run engine state must either live on the operation's context or keep honoring this one-op-per-process assumption.
- **A leftover run-state sqlite file in a reused `SANDBOX_PROCESS` box is a cross-tenant data leak, not just a disk leak.** The store writes to `/box` (`TMPDIR`, `run-state-store.ts` `getBasePath`), and in `SANDBOX_PROCESS` a code piece runs as plain Node in a spawned child (`no-op-code-sandbox.ts`) with full `fs` inside that box — so `require('fs').readdirSync('/box')` can list and read any leftover `<runId>.sqlite`, which holds a prior run's step outputs. The store's own read guards (runId-keyed path, `rmSync` before open, `CREATE TABLE`-throws→in-memory fallback) protect the *next run's store object* but do nothing against this direct `fs` read. Exposure is bounded to **`SANDBOX_PROCESS` + `REUSE_SANDBOX=true`**: the isolate box is the tenant boundary, reuse keeps `/box` across runs (no `isolate --init` between reused runs), and a leftover file crosses it. Safe by default — `SANDBOX_PROCESS` reuse is off by default, and the two reuse-by-default modes (`SANDBOX_CODE_ONLY`, `UNSANDBOXED`... the latter has no boundary anyway) plus `SANDBOX_CODE_AND_PROCESS` run code in a v8 isolate with no `fs`. The likely trigger is not "`dispose()` threw" but an engine **OOM-killed/timed-out before the `finally` dispose ran** (SIGKILL skips it). Fixed by an **idempotent sweep**: `runStateStore.init()` (called per operation in `worker-socket.ts`, before the flow runs) now `rmSync`s every `store-*` dir under the box parent before opening the new DB, so a reused box can't carry a prior tenant's file into the next run — matching the `actionRunCache.sweep` recompute-don't-accumulate pattern (decision 000016). **The sweep is gated on `isSandboxed` and that gate is load-bearing, not cosmetic:** only in sandboxed modes is the parent the private per-box `/box`; in the other modes it's the shared `AP_FLOWS_CACHE_PATH`, where a blanket `store-*` sweep would race concurrent engine processes and delete a *live* run's DB (and there's no box boundary to protect there anyway). Per-run cleanup could never cover the SIGKILL case; the sweep recomputes from the live filesystem instead of trusting the prior run's teardown. Testing note: `run-state-store.test.ts` needs Node ≥22.5 (`node:sqlite`) so it won't load on older local runners — it runs in CI. The store's *sandboxed* parent is `os.tmpdir()`, which reads `TMPDIR` per call (not cached), so a test exercises the sweep by setting `TMPDIR` + `AP_EXECUTION_MODE=SANDBOX_PROCESS` and deriving the parent from `os.tmpdir()` at call time.
- **"Offloaded to the run state store" is encoded as `output === undefined` on a non-loop step.** There is no explicit marker: a step whose payload lives in sqlite is a stripped in-memory shell whose `output`/`input` are `undefined` (loops are never offloaded). All reads must go through `storedStepReader` in `run-state-store.ts` (`read`/`readJson`, memory-first then store, falling back to the shell) — sniffing `step.output` directly reintroduces the bug class this helper removed. Corollary: the reporter's final backup streams the log out of the store, so `flowRunProgressReporter.shutdown()` must complete before `runStateStore.dispose()` — `disposePerRunState()` in `worker-socket.ts` owns that ordering.
