...
- **`deleteFiles` succeeding does not mean the objects are gone.** `DeleteObjectsCommand` reports per-object failures in `response.Errors` and does **not** throw, and `Quiet: true` only suppresses the success entries — so a request that "worked" can still have left objects behind. `deleteFiles` therefore returns the keys S3 refused; any caller whose correctness depends on the prefix being empty afterwards (a one-shot purge writing a completion marker, say) must check that list rather than trust the absence of an exception. Callers that are just best-effort cleanup can keep ignoring the return.
