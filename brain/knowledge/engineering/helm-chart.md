...
- **The chart renders an Argo Rollouts `Rollout` by default, which most self-hosters' clusters cannot accept.** `workloadType` defaults to `rollout`, so `templates/deployment.yaml` emits `apiVersion: argoproj.io/v1alpha1, kind: Rollout` — the blue/green shape our own deploy wants. Without the argoproj CRDs installed the apply fails with `no matches for kind "Rollout"`, and the filename `deployment.yaml` hides this from anyone skimming the chart: there is no plain `Deployment` path at all, only `Rollout` or `workloadType: statefulset`. Self-hosted installs should set `workloadType: statefulset`.
- **Overriding `activepiecesEnvVariables` in your own values file merges with the shipped default instead of replacing it.** Helm deep-merges maps, so adding one secret name leaves all ten shipped ones in place and re-introduces the duplicate-env-name trap above. To take full control of the env list, set `activepiecesEnvVariables: null` first and put everything in `activepiecesConfig`; a `null` value does delete the parent key.
