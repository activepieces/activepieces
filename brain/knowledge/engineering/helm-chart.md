...
Only two secrets, both `data: {}` with mittwald `secret-generator` annotations that fill them in-cluster: `<release>-secrets` (`AP_ENCRYPTION_KEY`) and `<release>-jwt-secret` (`AP_JWT_SECRET`). Postgres and Redis come from the Bitnami subcharts (off by default, images from `bitnamilegacy/*`). The workload is an Argo `Rollout` when the cluster serves `argoproj.io/v1alpha1/Rollout`, else a `StatefulSet` (`activepieces.workloadType` helper).
