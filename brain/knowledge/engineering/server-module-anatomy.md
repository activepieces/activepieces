...
- **`ActivepiecesError.message` is the bare error code — the detail only lives in `error.error.params`.** The constructor builds `super(code + (message ? ': ' + message : ''))`, and almost nothing passes that optional second argument, so a `VALIDATION` error carrying `params: { message: 'endpoint X is not approved' }` throws with `message === 'VALIDATION'`. `expect(...).rejects.toThrow('endpoint X')` therefore fails with the useless `expected ... to throw error including 'endpoint X' but got 'VALIDATION'`. Assert on the structured payload instead — capture the error, check `error.error.code`, and read `params.message` through an `'message' in params` narrowing (the pattern in `test/unit/app/ee/authentication/saml-authn/saml-attributes.test.ts`).
