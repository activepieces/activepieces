...
- **Adding an index? Plain `CREATE INDEX IF NOT EXISTS` in the migration — no `CONCURRENTLY`, no `DROP … IF EXISTS` prelude, no invalid-index dance. TypeORM runs migrations before the healthcheck, and on any table small enough for a boot-time build to finish inside the deploy window this is the right shape.** The exception is a new index on an existing production-hot table (`file`, `flow_run`, `flow_version`, `audit_event`, `flow`, …): those take hours to build at cloud scale, a boot-time `CREATE INDEX` blocks the healthcheck and the deploy rolls back on top of a half-built index, and `CONCURRENTLY` inside a migration has its own tar pit (invalid-index leftovers, `IF NOT EXISTS` matching on name-only, `transaction = false` losing all rollback). For those, hand the index to the platform team to build ahead of the deploy, and let the migration's `IF NOT EXISTS` no-op when it runs. Ask before you assume a table is "small enough" — the definition of large is empirical, not by row count.
- **New migrations no longer add a paired sqlite file.** The `migration/sqlite/` directory still exists and is registered in `sqlite-connection.ts`, so grep will find dozens of `*Sqlite.ts` siblings for old changes — but since ~mid-2026 (`1795000000000-AddExternalIdToFolder` onward: `1806`, `1810`, `1812`, `1815`, `1820`, `1821`, `1829`–`1836`), new work handles PGlite via an inline `isPGlite()` branch inside the postgres file and no sqlite counterpart. Follow that; do not resurrect the paired-file convention because the CLI still emits it or because the sqlite dir looks active. sqlite migrations only exist for `AP_EDITION=ce` on native `sqlite3`, which is not the documented one-line install path (PGlite is).
