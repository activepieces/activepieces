...
- **A bare `SET` on a pooled connection outlives your query.** `databaseConnection().query('SET statement_timeout = ...')` runs on whichever pooled connection you happened to get and stays set on it after release, so a later unrelated request inherits it. Scope it to a transaction instead: `runner.startTransaction()` then `SELECT set_config($1, $2, true)` — the `true` is `is_local`, the bindable equivalent of `SET LOCAL`, so the value can be a parameter instead of string-interpolated SQL. Verified on PG14: `SHOW statement_timeout` reads `5s` inside the transaction and `0` right after `COMMIT`. And roll back before `release()` — a query runner released mid-transaction hands the pool a connection sitting in `idle in transaction`, which blocks autovacuum.
