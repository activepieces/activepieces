...
- **`flowStructureUtil.findPathToStep` is O(n²) on its own, so calling it once per step node makes the canvas O(n³).** It walks `getAllSteps(trigger)` and then calls `getAllSteps(step)` *again* for every step to test containment. One call is fine; a call inside a per-node hook is not, and every step node on the canvas runs its own copy of the run-details hooks (`ApStepNodeStatusInRun` → `useStepOutputInRun` → `useBatchLogs`). Memoising on `[trigger, stepName]` does not save you — the trigger object is new after every flow operation, so the whole grid recomputes on each edit, whether or not the flow has a run or a batch step. Measured on a straight-line flow: 100 steps 31.8ms, 200 steps 173.7ms of blocking main-thread work *per edit*. The fix is ordering, not caching — filter to the few steps you care about **before** the containment test (`getAllSteps(trigger).filter(type === X).filter(isChildOf)` instead of `findPathToStep(...).filter(type === X)`), which took the same measurements to 0.7ms and 2.3ms. Before adding any tree walk to a per-node hook, check what it costs times the node count.
