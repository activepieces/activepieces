...
- **The OAuth2 authorization code is decoded exactly once, and the redirect page is where that happens.** The provider percent-encodes the code into the redirect query, the redirect page decodes it once, and `credentialsOauth2Service.claim` re-encodes it once via `URLSearchParams` for the token POST — so anything in between must pass the code through verbatim. There are **two** redirect senders, not one: the SPA route (`params.get('code')`) and a server-rendered HTML page on `GET /redirect` in `app.ts` (`request.query.code` → Mustache → `getAttribute('content')`, which round-trips faithfully because the HTML parser undoes Mustache's `&#x2F;`/`&#x3D;` escaping). Both post the once-decoded value to the single `message` listener in `oauth2-utils.ts`, so a stray extra `decodeURIComponent` there corrupted every code containing `%2F`/`%3D` (INVALID_CLAIM at token exchange) and threw `URIError` on a lone `%`, hanging the connect popup — `getCode` has no rejection path, so an unresolved promise never surfaces as an error. Codes with a **raw** `+` are still read as a space, correctly: form-urlencoded is what RFC 6749 §4.1.2 specifies, so the provider must send `%2B`.
