...
- **A stored header value is bound to the URL it was saved against.** `update()` answers 400 if the request changes `url` while carrying any stored value over (a `null` for a stored key, or an omitted `headers` field entirely — both branches, or it is bypassed by omission). Without that rule "write-only" is decorative: anyone who can edit a destination points it at their own endpoint and the next audit event delivers the secret for them. For the same reason `POST /test` is a `z.strictObject` that takes no `destinationId` and rejects a `null` header — it can only send what the caller typed.
- **The `mapper` column is public by design.** It is unencrypted, `GET` returns it verbatim, and `/test` echoes the rendered body back to the browser. The PostHog preset seeds an `api_key` field there because PostHog authenticates in the body; that is a knowing accept, documented in `event-streaming.mdx`, not an oversight to "fix" by encrypting the column — it is a template an admin must read and edit.
