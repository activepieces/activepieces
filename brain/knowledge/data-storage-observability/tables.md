...
- JSON import into an **existing** table is client-orchestrated and destructive-first: `tableHooks.importTableIntoExisting` calls `tablesApi.clear()`, deletes every field, renames, then recreates — separate HTTP calls with no transaction and no rollback, so anything that fails after the clear leaves the table empty with no undo. The uploaded file is arbitrary JSON, so everything the sequence will send is validated up front: the table shape against `TableTemplate.pick({ name, fields })` and every field against the server's own `CreateFieldRequest`. Validate against the request schema the server applies, never a looser local one — `TableTemplate.fields[].type` is a bare `z.string()`, so a bogus field type passes it and only 400s at `fieldsApi.create`, after the wipe. Still unguarded: `MAX_FIELDS_PER_TABLE`, record limits, and any network failure mid-sequence.
