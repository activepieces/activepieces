...
- **The per-platform gate is an uncached `findOneBy` on every telemetry event, and that is a known, accepted cost.** `getOrCreateForPlatform` matches `platformPlanService`'s shape (find → `distributedLock` → re-check → insert) and neither caches the row. Creation is once per platform and irrelevant; the read is what scales. Two paths make it interesting: `mcp.tool.called` fires per tool invocation and pays two queries (`projectService.getOne` is also uncached), so it scales with agent traffic rather than human traffic; and the daily `RUN_TELEMETRY` job double-gates — the batched `filterProjectsWithProductTelemetryEnabled` already answers the question, then the per-row `trackProject` asks it again, so N rows cost 2N queries in one burst. Deliberately left as-is at current volumes (decided Sep 2026); the tripwire to watch after rollout is that job's runtime on the largest tenant, not MCP latency. Fixes, if it ever bites: drop the redundant per-row gate first, then cache the boolean. **Because it is a read — and a `distributedLock` plus an INSERT the first time a platform emits anything — every emission site must be fire-and-forget.** `rejectedPromiseHandler` is the idiom and only attaches a `.catch`; the one site that was awaited, `authenticationUtils.sendTelemetry`, sat on the signup response and paid the lock on `linkOwnerToPlatform`, the very first signup. A test that asserts `trackProject.mock.calls` synchronously after a request is the tripwire for anyone who re-introduces an `await` — assert with `vi.waitFor` instead.
