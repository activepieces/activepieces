...
- **A platform can curate which external MCP hosts agents may reach** via `platform.mcpServerEndpointAllowlist` — host or `*.`-wildcard entries (`mcp.acme.com`, `*.acme.com`); whole-TLD wildcards, full URLs and single-label hosts are refused at entry validation. Opt-in: an empty or unset list allows every endpoint, so existing installs are unaffected. Matching lives in one place, `mcpEndpointAllowlistUtil` in `@activepieces/shared`, shared by the validate endpoint and the run gate so the two cannot drift. It is a *host* allowlist — it cannot narrow an approved host down to one path — and it stops at the registrable domain, so `*.co.uk`-style public suffixes are the admin's own footgun.
- **Enforce the allowlist where the agent run is enqueued, not where the flow version is served.** The first attempt stripped off-list tools out of `settings.input.agentTools` inside `getFlowVersion`, which was right while the AI piece connected from inside the sandbox. Once tool execution moved server-side the tools travel as `EXECUTE_AGENT_RUN` job data, and a stored agent's tools never pass through a flow version at all — so the chat/agent path was entirely ungated. Both producers (`agent-run-controller` for a flow step, `agent-conversation-controller` for a chat agent) now call `agentHelpers.assertMcpEndpointsApproved` beside the existing `assertRunProviderConfigured`; anything that grows into a third producer needs the same line. The strip also billed every flow run two extra queries on the hottest RPC.
...
- `packages/web/src/app/routes/platform/setup/mcp/` — platform MCP setup page, including the Approved endpoints editor for the external-MCP allowlist
