...
- **Discovery resolves both host and prefix per request, by matching the request host against the configured base URLs.** `domainHelper.getPublicUrlFromRequest` compares `networkUtils.getRequestHost(req)` against `AP_FRONTEND_URL` and `AP_MCP_URL`, and on a match emits that entry's full base URL *verbatim, protocol included*; an unmatched host falls back to the old behaviour — request origin plus `AP_FRONTEND_URL`'s pathname — which is what keeps Cloud custom domains working. So `mcp.example.com` (root) and `example.com/activepieces` can serve one instance at once, each with its own prefix. Match on host rather than origin: a proxy that forwards `x-forwarded-host` but omits `x-forwarded-proto` would otherwise miss an `https://` entry and silently advertise the wrong prefix over `http`. `AP_MCP_URL` must use a different hostname from `AP_FRONTEND_URL` — sharing one with a different path is rejected at startup, because the proxy strips the prefix before the server sees it. `401`s carry an RFC 9728 `WWW-Authenticate: Bearer resource_metadata="…"` header. Host-root `.well-known/oauth-*` must still be forwarded to AP by the operator.
...
- **A dedicated MCP host needs no SPA and no `/api`, because consent is served from the frontend base.** When a request matches `AP_MCP_URL`, `/authorize` redirects to `domainHelper.getPublicUrl({ path: '/mcp-authorize' })` rather than following the request host — so the user signs in once, on the main host, where their session token already lives (it is `localStorage`, and therefore per-origin: consent on the MCP host would demand a second login). That leaves the MCP host serving only `/mcp`, `/mcp/platform`, `/.well-known/oauth-*`, `/authorize`, `/token`, `/register` and `/revoke` — all mounted at the domain root in `server.ts`, outside the `/api` prefix — which is what makes the tight WAF rule set that motivates a separate host actually possible. The redirect is conditional on matching `AP_MCP_URL` on purpose: making it unconditional would bounce Cloud custom-domain users off their own domain.
