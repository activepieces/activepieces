...
- **The platform server's selected project is live mutable state, so resolve it once per call and thread it.** `mcpProjectSelection.get` is an uncached Redis read, its default scope is `{ platformId, userId }` (per *user*, not per session), and the transport is stateless (`sessionIdGenerator: undefined`, a fresh server per POST) — so two concurrent requests from the same user share one key, and `ap_set_project_context` can flip it mid-call. Anything downstream of a tool call that re-reads the key instead of reusing what `execute` resolved will disagree with it: `ap_run_action` runs a real engine flow run, so the window is seconds wide, and a deferred read after the response lands in whatever project was selected *by then*. This bites attribution hardest — a row or file written from a second read names a project the action never touched, silently and plausibly.
