...
**Revocation List** — the Redis keys (`mcp_oauth:revoked_grant:<grantId>`) naming grants revoked while their access tokens are still unexpired, read on every `POST /mcp`. TTL outlives the access token, so it empties itself. _Avoid_: "blacklist", "denylist"
