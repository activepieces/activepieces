...
[models.dev](https://models.dev) (MIT), generated by `npm run sync-model-catalog` into a **vendored**
`model-catalog.generated.json` in `@activepieces/server-utils` — no runtime fetch, so self-hosted and
air-gapped installs get it in the image. A weekly workflow regenerates it and opens a PR only when the
data actually moved. `modelCatalog.lookup({ provider, modelId })` is the single accessor; enrichment
