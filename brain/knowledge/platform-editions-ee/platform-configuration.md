...
- **What blocks `DELETE FROM platform`** is now two constraints: `project` and `signing_key`, both `RESTRICT`. It used to be four — `tag` and `piece_tag` were `NO ACTION` and were dropped once it was clear the tables had outlived their feature. The remaining FKs already `CASCADE`. Platform deletion failing for one tenant and not another almost always means a new blocking FK, not a member count.
- **That cascade list grows on its own, and the count in your head is probably stale.** Measured on prod 2026-08-31: **23** FKs reference `platform(id)`, **21** of them `CASCADE` — up from the 18/14 this page recorded earlier. Nobody added them deliberately for teardown; they arrived with `agent_conversation`, `chat_personalization`, `piece_set`, `user_memory`, `ai_tool_config` and friends. Re-derive it rather than trusting a written number: `SELECT conrelid::regclass, confdeltype FROM pg_constraint WHERE confrelid='platform'::regclass AND contype='f'`.
- **`DELETE FROM platform` fires all 21 cascades as one statement, and prod's `statement_timeout` is 60s.** That one row expands into a delete across every cascading child, `audit_event` among them — a 571 GB / ~432M-row table as of 2026-08-31. Even a tenant with only ~77k audit rows measured **23s** standalone, so it fits on a quiet database and blows past 60s under normal production contention. The `HARD_DELETE_PLATFORM` job then dies at the very last step, after the projects and children are already gone, and retries forever hitting the same wall — the tenant sits half-purged with its `platform` row alive. Fixed by running the platform-row and per-project deletes inside a transaction that does `SET LOCAL statement_timeout` first (`PLATFORM_TEARDOWN_STATEMENT_TIMEOUT_MS`, 5 min), the same idiom `flow-run-module.ts` uses for run telemetry. Any new tenant-sized delete added to that job needs the same treatment.
- **A permanently failed `HARD_DELETE_PLATFORM` is silent.** It exhausts its 25 attempts and stops; there is no alert, and `removeOnFail` drops the job record after 30 days — so the evidence that a purge never happened expires before anyone looks. Members stay `INACTIVE` with their data intact indefinitely. Check for stragglers with `getJobs(['failed'])` on `system-job-queue` rather than waiting to be told.
