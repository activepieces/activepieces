...
- **Third-party tag failures surface as `Script error.` with no stack.** A cross-origin script (GTM tag, Clarity) that throws reaches `window.onerror` with `event.error == null` and the message `Script error.`; nothing in it is actionable and it is not our code. `main.tsx` drops those via `errorReporting.isOpaqueCrossOriginScriptError` **only when `location.pathname` starts with `/embed`** (the memory router keeps that path for the whole embedded session), so a vendor's broken tag cannot flood Sentry while Turnstile and other cross-origin scripts on normal pages still report as before. Same-origin app errors always carry `event.error` and are unaffected. A tag script that fails to load removes its own `<script>` element and warns, so `isScriptLoaded` does not mistake a dead element for a loaded one.
