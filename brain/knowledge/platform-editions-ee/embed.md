...
- **Third-party tag failures surface as `Script error.` with no stack.** A cross-origin script (GTM tag, Clarity) that throws reaches `window.onerror` with `event.error == null` and the message `Script error.`; nothing in it is actionable and it is not our code. `main.tsx` drops those via `errorReporting.isOpaqueCrossOriginScriptError` before reporting, so a vendor's broken tag cannot flood Sentry. Same-origin app errors always carry `event.error` and are unaffected.
