...
- **Automatic trial activation** — `/sign-in?licenseKey=…` (any origin; self-hosters swap the domain) lets sales hand a customer one link instead of a key to paste. `useStashTrialKeyFromUrl` in `AuthLanding` moves the key into `sessionStorage` and scrubs the query param; `<AutomaticTrialActivation />`, rendered inside `AllowOnlyLoggedInUserOnlyGuard`, opens the Activate Trial Key dialog prefilled and auto-submitting against the existing `POST /v1/platform-billing/activate` once a platform exists, then swaps that same dialog's own content to a success view rather than opening a second one (no toast either — `useUpdateLisenceKey` takes `messages.success: null`). Both are keyed off `isTrialKey`, so the manual Activate Trial Key dialog on the billing page shows the success view too; a plain licence key still closes and toasts. It is a component rather than a hook because the guard's early returns would make a hook call conditional. Web-only — no endpoint, no migration. **The link is built in the console** (`packages/web/src/lib/activation-link.ts`, surfaced as "Copy activation link" in the license-key table and on the key-created page) and consumed here, with nothing but the `licenseKey` query-param name joining the two repos — rename the param or move the route and the console keeps handing out links that silently do nothing.
