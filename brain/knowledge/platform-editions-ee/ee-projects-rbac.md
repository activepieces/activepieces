...
- **A project role is deleted by name, so two roles sharing one name lose both.** `DELETE /v1/project-roles/:name` resolves to `repo.delete({ name, platformId })`, which matches every row with that name, and `project_member.projectRole` cascades on `projectRoleId`. `create()` always refused a duplicate, `update()` did not, so renaming B to A was enough to reach that state, and it happened on a dev database without anyone trying. Closed by a unique index on `("platformId", LOWER("name"))` for platform-owned roles, with `getOne` already comparing `LOWER(name)` so the index matches the rule the API enforces. The upgrade renames existing duplicates rather than deleting them: the copy with the most members keeps the name, creation time breaks ties, the rest take the first free ` (2)`, ` (3)` suffix. There is no marker distinguishing a generated name from a chosen one, so `SELECT id, name FROM project_role WHERE name LIKE '% (%)'` lists candidates, not confirmed renames.
