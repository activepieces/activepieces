...
- **Engine and piece child split one memory budget; neither may claim all of it.** `engineNodeArgs` sets `--max-old-space-size` as the *only* bound on engine memory (isolate passes no `--mem`/`--cg-mem`), so a child spawned without it runs on V8's default heap — but a child spawned with the *engine's* value is just as wrong, because both processes are resident at once and two full allowances under one container limit is the same kernel-OOM path. `node-args.ts` owns the split (`engineHeapMb` / `pieceChildHeapMb`), the child's share reaches the engine as `AP_PIECE_MEMORY_LIMIT_MB`, and `piece-runner` substitutes it into the inherited execArgv — `--expose-gc` and `--no-node-snapshot` carry over untouched. Halving the engine's share is only safe because pieces left it: measured engine RSS is ~113-155 MB of a 1 GB budget across a 2,000-run smoke test.
