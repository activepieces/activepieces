...
- **The child must be spawned with the engine's own node flags** (`[...process.execArgv, entry]`). `engineNodeArgs` sets `--max-old-space-size` as the *only* bound on engine memory — isolate passes no `--mem`/`--cg-mem` — so a piece child spawned without it runs on V8's default heap, outgrows the container and is SIGKILLed by the kernel OOM killer, taking the worker with it instead of failing as a bounded piece error. `--expose-gc` and `--no-node-snapshot` ride along for the same reason they exist on the engine.
