...
The host is started with `child_process.fork(process.argv[1], ['--piece-host'])` — `fork` because it launches the engine's *own* bundle (no second build), auto-wires the IPC channel the RPC rides, and gives a real second OS process with its own V8 heap that is reclaimed on exit. `worker_threads` was rejected: a thread is a separate isolate but the **same process**, so memory stays in one address space (shared OOM blast radius), a native crash can take the whole engine down, and many piece deps/native addons are not multi-isolate-safe — process isolation is the property we actually need. `spawn` would be equivalent to `fork` but hand-rolled; `vm`/`isolated-vm` stays in-process and can't run full pieces (Node built-ins). Cost: each fork re-parses the bundle, amortized by reusing one host across an operation's piece calls.
...
- Gating is `AP_REUSE_SANDBOX=true` **and** a non-isolate mode (`SANDBOX_CODE_ONLY` / `UNSANDBOXED`) — never fork a host inside the isolate jail.
- **The engine gets a filtered env, not the worker's.** `create-sandbox-for-job.ts` builds the engine's env explicitly (`baseEnv`) plus an `AP_SANDBOX_PROPAGATED_ENV_VARS` allowlist — so a var set on the container/worker (like `AP_REUSE_SANDBOX`) is invisible to the engine unless it's added to `baseEnv` or the allowlist. This silently defeated the gate at first (0 hosts, engine still grew); the fix propagates `AP_REUSE_SANDBOX` in `baseEnv`. Any future engine-read env var needs the same treatment.
- Verified on a preview instance (`SANDBOX_CODE_ONLY`, reuse on): 60 runs over 4 distinct pieces kept the reused engine RSS flat (+1–3 MB) while per-operation `--piece-host` children spawned (~113–158 MB) and were killed; a failing piece surfaced as a FAILED USER step, not `INTERNAL_ERROR`.
