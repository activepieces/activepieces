...
- **A pipelined write has to inspect its own results, or fail-closed is a fiction.** ioredis resolves `multi().exec()` with a `[error, result]` tuple per command instead of rejecting, so a single `SETEX` refused by Redis — `OOM`, a replica in read-only mode — used to leave `putBatch` reporting success. Both revoke endpoints would then have returned `204` with the row marked revoked and the key missing, which is the precise failure this whole decision exists to prevent, arriving silently. `exec()` also resolves `null` when the transaction is discarded, which is likewise not success. Both batch helpers now throw on either. Anything else added to this file that pipelines commands needs the same treatment.
