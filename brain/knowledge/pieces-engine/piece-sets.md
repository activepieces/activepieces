...
- The **whole** `/v1/piece-sets` module is behind that flag, `GET` included — so on a locked plan the web list query is `enabled: false`, the table is simply empty, and row actions never render. Only toolbar/entry points need a UI guard. The `LockedAlert` + `RequestTrial featureKey="ENTERPRISE_PIECES"` lives once on `PlatformPiecesPage`, above the section body, since the same flag gates both the Pieces and Piece Sets sections; the details route redirects back to `/platform/setup/pieces/piece-sets` rather than hanging on a spinner waiting for a query that will never run.
...
- **A piece set gates visibility, not execution.** `isPieceVisible` / `isComponentVisible` are only consulted when listing pieces, so a flow built before a piece was excluded — or one in a project later reassigned to a stricter set — keeps running that piece, and a step calling a now-hidden action still executes. The piece vanishes from the builder, which reads as "off" but is not. Publish validation and the pre-run checks do not consult the set either.
...
- `packages/web/src/app/routes/platform/setup/pieces/piece-sets/` — management UI and dialogs
