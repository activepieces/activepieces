...
### ⚠️ Gotchas
- **`extractAgentToolPieceRefs` is dead code — do not extend it, and do not trust it as the provisioning path for agent tools.** It still walks `step.settings.input['agentTools']` in `flow-provisioning.ts` and unions the result into `resolvePieces`, but since the agent step became a thin client (#14699, #14730) a configured piece tool no longer runs in the flow's sandbox at all: `agent-worker-tools.ts` → RPC `executePieceTool` → `piece-tool-runner.ts` → `flow-run-utils.ts` → `actionRunService` submits a **separate `EXECUTE_ACTION` job that resolves its own piece** from `pieceName@pieceVersion`. The flow bundle only ever needs `@activepieces/piece-ai`. So the function now installs packages into a sandbox nothing loads them from — a pure waste, safe to delete.
  - The lesson it was originally written for still holds and is worth keeping: **provisioning must not be stricter than the engine.** It strict-`safeParse`d each entry against `AgentPieceTool` and silently `return []`ed on failure, while the engine tolerated the legacy flat `predefinedInput` shape — so pieces went un-provisioned and runs died `INTERNAL_ERROR` with an empty `failedStep`. Any future validate-then-provision pair must fail the same way on both sides.
