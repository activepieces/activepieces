...
**Pool Server**:
The single `/execute` HTTP endpoint the Cloud Run image runs. Shipped as a **dedicated, minimal image** — only the pool + engine + isolate + bun + the server, no app and no worker poller. One self-contained call: fully-resolved-job-in → result-out (Cloud Run cannot guarantee request affinity across separate calls, so the lifecycle cannot be split over the wire). Wraps the pure pool at concurrency 1; holds no `apiClient`.
