...
- **The PR template has TWO required sections and `breaking-change-check` fails if either is left unedited** — `### Breaking change?` *and* `### Security impact?`. Tick exactly one box in each (ticking two in one section also fails). Copy the option text from `.github/pull_request_template.md`; the parser matches `- [x] no` / `- [x] yes` under each heading, so keep the `no`/`yes` as the first word after the checkbox.
  - `Breaking change?` has **three** options: `no`, `yes — technical` (removed/renamed API field or endpoint, dropped column, new required field, removed/required env var), `yes — functional` (default/limit/behaviour change, new self-hosted setup step).
  - `Security impact?` has **two**: `no`, `yes — security-sensitive` (touches authentication, secrets, permissions/roles, cryptography, SSRF / outbound HTTP, or file handling). This one is a **declaration only** — no label, no docs entry, nothing else is coupled to it.
- **If the change is breaking, the label and the docs entry must travel together** — the check's coupling is bidirectional and it also cross-checks the template answer, so label, docs entry and ticked box must all agree:
  - apply the **`⛓️‍💥 breaking-change`** label (in addition to the primary label above), **and**
  - add an entry to `docs/install/reference/breaking-changes.mdx`. It must have a `####` title **and** at least one body line — a bare heading, a `---`, or loose prose does not count as an entry.
  - Ticking `no` while carrying the label fails, and so does adding a docs entry without the label.
- **Never `gh run rerun` a `pull_request`-triggered check to pick up a PR description edit.** A rerun replays the *original* event payload, so it re-reads the stale body and fails again — and because it is a later attempt on an older run, its failure shadows a newer run that already passed. Edit the body (which fires `edited`, a trigger these workflows listen for) and let the fresh run supersede. To check the answer locally without any run at all: `PR_BODY="$(gh pr view <n> --json body --jq .body)" PR_LABELS="$(gh pr view <n> --json labels --jq '[.labels[].name]')" BASE_REF=main npx tsx tools/scripts/breaking-change-check.ts`
