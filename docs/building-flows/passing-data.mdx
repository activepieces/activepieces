...
<Note>If there is a step that has a property which is not present in the "Data to insert" dropdown, you can use the step name to resolve the path to it by typing `${step_name.path_to_property}`.
This is used with JSON inputs like the body of an HTTP request </Note>
