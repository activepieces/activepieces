...
[Passing data](/flows/passing-data) lets you insert values from previous steps into the current step's inputs. Data manipulation lets you **transform that data** before it gets used — combine two values into one, change the format, do a calculation, fall back to a default when something is empty, or branch on a condition.
...
- [Data Manipulation Functions](/flows/formula-reference) — every available function, grouped by category, with signature and example
- [Passing Data](/flows/passing-data) — using mentions to insert values from previous steps
