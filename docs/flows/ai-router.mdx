---
title: "AI Router"
description: "Ask one question and a purpose-built evaluation model picks the route"
icon: "shuffle"
---
...
One model call decides, and exactly one route runs.
...
approval thresholds. It is free, instant and gives the same answer forever. It is also the only one
that can run **several branches at once**; the AI Router always takes exactly one.
...
| Field | What it does |
|---|---|
| **Input** | The text the model reads. Reference a previous step, e.g. `{{trigger.body.message}}`. |
| **Question** | One question, answered by exactly one route. |
...
| **Confidence floor** | Optional. Below it, the flow takes the Otherwise route. |
...
A confidence floor works per route here: a route the model affirmed but was only 40% sure about is
dropped, and if the floor rejects every route, Otherwise runs.
...
A route picked at 91% and one picked at 34% look identical in a normal run log. Use the spread to
tell a routing rule you can trust from one you cannot, and tighten the descriptions that come out
close.
...
The AI Router runs on a key the instance holds, so there is no connection to set up and no AI credit
is consumed. Set `AP_AI_GATEWAY_API_KEY` (see
[environment variables](/install/reference/environment-variables)). Leave it unset and the step
simply does not appear in the step list.
