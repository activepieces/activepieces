...
| Worker concurrency (per worker) | 1 | `AP_WORKER_CONCURRENCY` | `5` (set to `1` — see [Recommended Setup](/install/architecture/recommended-setup)) |
...
<Tip>
At the recommended concurrency of 1, the worker container's memory cap (1 GB)
is the memory limit for the flow. If a flow exhausts it, that one worker is
OOM-killed and the job is requeued. About 300 MB of the budget is overhead for a
warm process with pieces loaded.
</Tip>
