...
| `to_json` | `to_json(value)` | Turns any value into its JSON text, so a list or an object can sit inside a sentence. | `to_json([1, 2, 3])` → `[1,2,3]` |
...
| `to_epoch` | `to_epoch(date; unit)` | Turns a date into the number of seconds or milliseconds since 1 January 1970. | `to_epoch(2025-01-15T00:00:00Z; ms)` → `1736899200000` |
