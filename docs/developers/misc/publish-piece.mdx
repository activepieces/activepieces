...
The CLI scans the `packages/pieces/custom/` directory for the specified piece. It checks the **name** and **version** in the `package.json` file. If the piece is not already published, it builds, packages, and uploads it to the platform using the API.
...
```bash
AP_API_KEY=your_api_key_here npm run publish-piece -- --apiUrl https://INSTANCE_URL/api --name PIECE_NAME
```
...
4. Once the pull request is merged to the main branch, manually run the CLI or use a GitHub/GitLab Action to trigger the publishing process.
