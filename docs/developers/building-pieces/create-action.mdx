...
This tutorial will guide you through the process of creating a Gelato piece with an action that fetches random icecream flavor. It assumes that you are familiar with the following:
...
Now let's create authentication for this piece,which requires API Key in headers.
...
This will create new typescript file at `packages/pieces/community/gelato/src/lib/actions`,the `get-icecream-flavor.ts` file should contain following code.
...
export const getIcecreamFlavor = createAction({
  name: 'fetch_random_icecream_flavor', // Must be a unique across the piece, this shouldn't be changed.
  auth: gelatoAuth,
  ...
  async run(context) {
    const res = await httpClient.sendRequest<string[]>({
      ...
      url: 'https://cloud.activepieces.com/api/v1/webhooks/lH51pkBqJxXSaT9sWPdNg/sync',
      headers: {
