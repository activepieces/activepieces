...
This will create new action file at `packages/pieces/community/clickup/src/lib/actions`,the `create-task-tag.ts` file should contain following code.
...
- `run` function: The logic for the action is implemented within the run function. This function is called when the action is executed. It takes a single argument, context, which contains the values of the action’s properties.
