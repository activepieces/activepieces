...
<Steps>
  ...
  <Step>
    ...
    <Warning>
     ...
     - Here you need to map the attribute value from your Okta user profile. if you already have a role field in the Okta user profile that matches exactly with platformRole value ( `ADMIN`, `MEMBER`, `OPERATOR` ) than you can select it directly with `Map from Okta Profile` option, otherwise you can you an [Expression](https://developer.okta.com/docs/reference/okta-expression-language/) to return one of the 3 roles based on other fields in the Okta user profile.
...
In Activepieces application page go to **Provisioning -> To App** and enable what actions you want to be applied to activepieces when change in Okta

![Okta role expression example](/resources/screenshots/scim/okta/provision-actions.png)

Now in the **Assignments** tab you can:
- Choose to provision indivisule user or groups, note that groups in Okta will be projects in activepieces
...
- To push groups to activepieces go to **Push groups** tab and click on push button, find the group and save.
...
<Tip>
  Created users in activepieces will recieve an email with an invitation link, when clicked they will be redirected to sign in with `SAML`
</Tip>


### Deprovision

Users state switch to `INACTIVE` in Activepieces only when they are deactivated in Okta . suspension or deletion in Okta does not reflect in Activepieces because of Okta's design
...
<Warning>
  Deleting a group will delete the whole project in activepieces with it's flows and connections . users linked to that group won't be affected
</Warning>
