...
Agent actions are recorded too. When an agent runs a piece action that writes something — sending an email, updating a row, posting a message — the event names the agent, the app, the action, the account it used and where the run came from. Reads are not recorded, and the values the agent passed to the action are never stored.
