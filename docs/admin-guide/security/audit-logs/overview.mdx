...
Agent actions are recorded too. When an agent runs a **piece action** that writes something — sending an email, updating a row, posting a message — the event names the agent, the app, the action, the account it used and where the run came from. Reads are not recorded, and the values the agent passed to the action are never stored. The other tool types an agent can carry do not write this event: a flow tool is recorded as a flow run instead, and an MCP server tool is not recorded at all, so neither shows up here even though both can change things.
...
The full list of documented events. We add new ones as the platform grows; the source of truth for every event we emit is the [event schema in the codebase](https://github.com/activepieces/activepieces/blob/main/packages/core/shared/src/lib/ee/audit-events/index.ts).
...
### Agents
- [Agent action executed](/admin-guide/security/audit-logs/agent-action-executed)
