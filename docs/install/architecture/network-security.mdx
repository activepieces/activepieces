...
<Tip>
`AP_NETWORK_MODE` defaults to `NONE`. Set it to `STRICT` to install the in-process SSRF guard described below.
</Tip>

| Value | Effect |
|---|---|
| `NONE` | No outbound guard. User code can reach any host the worker can reach. |
...
The guard is independent of the sandbox execution mode — it is installed in every mode when `AP_NETWORK_MODE=STRICT`. See [Sandboxing](./sandboxing) for what each sandbox mode isolates at the process level; network security is orthogonal — it constrains what code is allowed to *reach*, regardless of *how* it runs.
...
With `AP_NETWORK_MODE=STRICT` you should see an `SSRFBlockedError`. With `NONE` the request will succeed if the host allows it — confirming the guard is off.

## Rolling Out

1. Start in `NONE` (default) and identify any internal services that legitimate flows need to reach (internal APIs, databases used by Code steps, etc.).
