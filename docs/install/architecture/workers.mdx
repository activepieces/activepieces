...
For the full sizing recommendation (worker/app sizes, the 1:10 app:worker ratio, the slot-based capacity formula, and database sizing), see [Production Setup](../configure-operate/production-setup).
...
A worker that crashes, is evicted, or loses its Redis lease mid-run is survivable: BullMQ requeues the job, another worker picks it up, and durable replay skips every already-completed step. You do not need to drain traffic before rolling workers.
