...
| Name                      | Supports NPM in Code Piece | Requires Docker to be Privileged | Performance           | Secure |
|---------------------------|----------------------------|---------------------------------|-----------------------|----------|
| No Sandboxing             | ✅                         | No                               | Fast & Lightweight   | ❌       |
| V8 Sandboxing             | ❌                         | No                              | Fast & Lightweight    | ✅      |
| Kernel Namespaces Sandboxing | ✅                      | Yes                             | Slow & CPU Intensive  | ✅      |
...
The flow execution is isolated in their own namespaces, which means pieces are no longer global thread, So the user can run bash scripts and use the file system safely as It's limited and will be removed after the execution, in this mode the user can use any **NPM package** in their code piece.
