...
Short answer: throughput keeps climbing to 777 req/s at 160 workers, but not proportionally — the per-worker rate peaks at 80 workers and the shared database, not the worker fleet, is what limits it after that.
...
|---|---|---|---|---|---|
| 4 · 40   | 213 | 529m / 3000m | 123m / 2000m | 537m / 1000m | 81m / 500m |
| 8 · 80   | 484 | 1096m / 3000m | 256m / 2000m | 523m / 1000m | 82m / 500m |
| 12 · 120 | 641 | 1689m / 3000m | 337m / 2000m | 599m / 1000m | 92m / 500m |
| 16 · 160 | 777 | 2738m / 3000m | 781m / 2000m | 611m / 1000m | 91m / 500m |

**Postgres is the tier that runs out first.** It climbs 529m → 2738m as the fleet quadruples — 5.2× the CPU for 4× the workers, ending at **91% of its 3-core reservation**. Its cost per unit of work is near-constant (~2.5m of CPU per req/s at every tier), so it grows with *throughput*, not with fleet size, and there is no setting that makes it stop growing. Redis follows the same curve further from its ceiling (123m → 781m of 2000m).

The other two tiers are nowhere near saturation and stay flat: workers sit at ≤0.1 of their 0.5-core cap at every fleet size, and apps hold ~0.52–0.61 of a core per pod because the 1:10 ratio adds app capacity in step with the load. So when the curve bends (see below), neither the workers nor the app tier is the reason — the shared database is.

<Note>
The singletons here are deliberately over-provisioned (3 vCPU Postgres, `max_connections=2000`, durability off, data dir on tmpfs) to push their limit as far out as possible. A **managed** Postgres at the 2 vCPU / 4 GB this documentation recommends, with fsync on and real disks, will reach its own ceiling **earlier** than this one did. Size the database against your peak throughput, not against your worker count.
</Note>
...
- **Past ~80 workers the shared database becomes the constraint.** Postgres CPU tracks throughput almost exactly and reaches 91% of its reservation at 160 workers. Each additional worker still contributes, but it queues a little longer behind the same single database, so per-flow time stretches (the measured `sandbox run` phase goes 146 ms at 80 workers to 181 ms at 160) and the per-worker rate drops.

The **concurrency-1 worker model** sets the shape: each worker is busy for the *whole* per-flow time — engine run plus the end-of-run run-log persistence it finishes before taking the next job — so fleet throughput is `workers ÷ per-flow-time`. That is linear in the fleet only while per-flow time holds constant, which it does until the database starts absorbing the load. (The synchronous response reaches the client sooner than that — it is sent at the response step, before the worker wraps up the log write — so client-perceived latency is lower than the worker-busy time that sets throughput.)

**What this means for sizing:** worker count is the right lever up to roughly this scale, and past it the database is what you scale next. Do not read the worker fleet as the only dial.
...
- **Postgres + Redis**: in-cluster singletons, deliberately over-provisioned to push their limit as far out as possible — Postgres at 3 vCPU / 3 GB with `max_connections=2000` (the default 100 would starve the app pools past ~10 apps), durability off, and its data dir on tmpfs; Redis at 2 vCPU / 2 GB with `io-threads`. Even so, Postgres reaches 91% of its 3 cores at the top tier and is what bends the curve
