...
npx @activepieces/cli benchmark --url https://your-instance.example.com --email admin@company.com --password '...'
...
Infra round-trip (server-measured, in-region — authoritative, not reachable from the CLI)
  ...
  app      : app-7984d59d8c v0.86.2 | 1.5 core | cpu 73.2% | ram 2.0GB 34.1% | disk 12%  (this instance — one of N behind the load balancer)
  workers  : 4 connected
...
| **Infra round-trip** | Authoritative **in-region** DB / Redis / **S3** write+read latency, the effective execution/storage config, the **app instance** that served the request (CPU/RAM/disk — one of N replicas, since apps don't self-register), and the worker fleet with live per-worker CPU. The S3 round-trip is the same cost folded into every run's end-of-run log backup — a slow object store shows up here **and** in RUN. Self-hosted only. | `GET /v1/health/diagnostics` |
