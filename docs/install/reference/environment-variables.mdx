...
| `AP_MAX_BARRIER_SIGNALS` | Starting value for the maximum number of things a single barrier may wait on — the approvers on one approval, or the batches a fan-out dispatched. Each platform's setting is created from it on first read; after that, change it at Platform → Infrastructure → Configurations. | `10000` |
