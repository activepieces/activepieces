...
the address as disposable, toxic, a spam trap, an abuse address, globally
suppressed, or on a domain whose mail is forwarded elsewhere (`mx_forward`, the
shape of a throwaway-domain farm). Role addresses such as `info@` and `sales@`
are accepted, and so is an address whose mailbox ZeroBounce cannot confirm. A
member who already holds an accepted invitation is never refused. One credit is spent per address, and only
