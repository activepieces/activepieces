...
| `AP_MAX_STREAM_FILE_SIZE_MB` | Maximum size (MB) for a single streamed file — via `files.writeStream` or a large binary/multipart webhook body streamed to storage. Only applies when `AP_FILE_STORAGE_LOCATION` is `S3`; on `DB` storage the `AP_MAX_FILE_SIZE_MB` cap applies instead. | `1024` |
