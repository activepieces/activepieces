...
| `AP_MCP_URL` | Optional URL used to build the MCP server URL returned to MCP clients (Claude Desktop, Cursor, etc.), for routing MCP traffic through a different hostname than `AP_FRONTEND_URL`. Falls back to `AP_FRONTEND_URL` when unset. | `None` |
