...
Set `AP_MCP_URL` when you want MCP traffic separated from the main app — usually
to apply distinct WAF rules or to observe it independently. Discovery, the
...
your proxy is expected to strip it before forwarding. It must use a different
hostname from `AP_FRONTEND_URL`; sharing a hostname with a different path is
rejected at startup, because once the proxy has stripped the prefix the server
