...
| `AP_MAX_BARRIER_SIGNALS` | Maximum number of things a single barrier may wait on — batches dispatched by a Process in Batches step, or approvers on one approval. Batch size is derived or clamped so the count fits. | `10000` |
