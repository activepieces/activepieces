...
| `AP_CONTAINER_TYPE` | Which services run in the container: `APP` (API only), `WORKER` (worker only), `WORKER_AND_APP` (both), or `MIGRATION` (run database migrations and seeds, then exit — for a pre-deploy Job / initContainer / helm pre-install **and** pre-upgrade hook that runs before app pods on both fresh installs and upgrades, so migrations don't block app startup). | `WORKER_AND_APP` |
