...
| `AP_TELEMETRY_ENABLED` | Starting value for product analytics. Each platform's setting is created from it on first read; after that, change it at Platform Admin → Account → Configurations. | `true` |
...
| Variable | Description | Default |
|---|---|---|
| `AP_EXECUTION_DATA_RETENTION_DAYS` | Days to retain execution data, logs, and events. Should be **strictly greater** than `AP_PAUSED_FLOW_TIMEOUT_DAYS` — when the two are equal, a valid resume firing on the retention boundary can race the file-cleanup job and find its state files gone. A 1-2 day margin closes the race. | `30` |
| `AP_PAUSED_FLOW_TIMEOUT_DAYS` | Maximum pause duration for a paused flow, in days. Cannot exceed `AP_EXECUTION_DATA_RETENTION_DAYS`, and should be at least 1 day smaller than it (see above). Measured cumulatively from `flow_run.created`, not per-pause — a flow with stacked delays whose sum exceeds this fails at waitpoint creation. Webhook waitpoints without an explicit expiry default to this deadline. | `30` |
...
| `AP_AI_CREDIT_USD_VALUE` | Dollar value of one AI credit. A call on the Activepieces AI provider is billed the dollar cost the provider reports for it, divided by this. A call on your own API key is billed one flat credit and ignores this. | `0.0005` |
...
| `AP_MAX_BARRIER_SIGNALS` | Maximum number of things a single barrier may wait on — the approvers on one approval, or the batches a fan-out dispatched. The cap is stored per platform and created from this value the first time that platform's configuration is read, so changing it later only affects platforms that have not been read yet. Capped at `10000`; a higher value is clamped down to it. | `10000` |
...
| Variable | Description | Default |
|---|---|---|
| `AP_ZEROBOUNCE_API_KEY` | ZeroBounce API key. Set it to check each new address against ZeroBounce before an account is created, whether it arrives through the sign-up form or through Sign in with Google. | `None` |
...
suppressed, or reports that its domain cannot receive mail at all: no DNS
records, a send-only domain, or a mail server at an unroutable address. Role
addresses such as `info@` and `sales@` are accepted, and so is an address whose
mailbox ZeroBounce cannot confirm. A member who already holds an accepted
invitation is never refused. One credit is spent per address, and only for an
address that has no account yet. If ZeroBounce cannot be reached, or answers
that the key is invalid or out of credits, the sign-up is allowed through and
the reason is logged — a validation outage never blocks sign-up.

The check covers the two public doors — password sign-up and Sign in with
Google — and skips accounts a platform provisions itself through SSO (SAML) or
embedding (managed authentication), since those are vouched for by the platform.
A Google sign-up costs a credit only the first time an address is seen. A refused
Google sign-up answers exactly like a failed Google token exchange
(`INVALID_CREDENTIALS`) and the page shows a generic "Sign-in failed", so the
response never says *why*; as on the email path, the server log is the only
record.

ZeroBounce resolves DNS from the public internet. If your members' mail domains
are only resolvable inside your own network, leave the key unset.
