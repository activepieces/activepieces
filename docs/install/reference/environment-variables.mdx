...
| `AP_SANDBOX_MEMORY_LIMIT` | Maximum memory (KB) a single sandboxed engine process can use. Each process runs at most one execution at a time. Capped at the container's memory limit minus 320 MB reserved for the worker, divided by `AP_WORKER_CONCURRENCY`, so the engine heaps cannot together exceed the container. | `1048576` |
...
| Variable | Description | Default |
|---|---|---|
| `AP_ALLOW_DISPOSABLE_EMAILS` | Accept addresses from throwaway email providers. | `false` |
