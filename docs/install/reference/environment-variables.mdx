...
| `AP_WORKER_QUEUE` | Subscribes the worker to a single job-class queue. Set to `syncJobs` to dedicate the worker to synchronous webhook runs — see [Sync Webhook Workers](/install/configure-operate/sync-webhook-workers). Ignored when `AP_WORKER_GROUP_ID` is set. | `None` |
