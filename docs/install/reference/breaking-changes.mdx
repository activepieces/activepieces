...
One smaller change comes with it. The Activepieces AI provider now refuses a call from a version of the AI piece that cannot report what it cost, with a message asking you to republish. The migration above moves existing flows onto a version that can, so this is only reachable by a step pinned back deliberately or a worker still serving an older copy of the piece.
