...
If your network policy restricts outbound traffic and you allowed only `cdn.activepieces.com` after 0.87.0, allow `registry.npmjs.org` as well, or set `AP_USE_CDN_FOR_BUNDLES=true` to keep official bundles coming from the CDN. Otherwise nothing.
