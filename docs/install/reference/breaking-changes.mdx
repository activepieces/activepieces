...
**Action:** only if a flow has a multi-select set to a dynamic value that does not produce a list, such as a single item or a comma-separated line like `a,b`. Those steps used to run and now fail with the message above. Edit the field so it produces a list (`["a","b"]`), or switch it back to picking options. Checkboxes need nothing: they already failed in this case and now fail in fewer cases.
