...
The table is no longer only a constant in the release. On start-up the server reads `https://cdn.activepieces.com/ai/pricing.json` and refreshes it every hour, so prices can change without a deploy. If the file cannot be read, is malformed, or lists fewer than 80% of the models the release shipped with, the server keeps using the table built into the release and logs a warning — it never charges from a partial file.
