...
Nothing, unless you already have duplicates. The upgrade adds a unique index on `("platformId", "name")` for platform-owned roles, and any existing duplicates are renamed first, oldest kept as-is and the rest suffixed ` (2)`, ` (3)` and so on. Member assignments are untouched — no role is deleted to make room.

Check afterwards whether any role gained a suffix and rename it to something meaningful:
