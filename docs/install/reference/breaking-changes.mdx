...
Three differences are worth knowing before you upgrade:

- **A custom API call is refused.** An agent step can no longer use a piece's `custom_api_call` action. An unattended agent choosing a URL and a method with your connection's credentials is not something we will do with nobody watching. Replace it with a specific action from the same piece.
...
Nothing, unless a step used a custom API call. Check any agent step whose tools included one, and swap it for a specific action before upgrading.
