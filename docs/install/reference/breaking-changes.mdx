...
**If you ran 0.88.2–0.88.4 with SMTP configured, assume some accounts depend on the code flow.** Two groups do, and
the second is easy to miss:

- Accounts **created** by the code flow. They hold a random password nobody was ever shown, so taking the flow away
  removes the only way in.
- Accounts that signed up **with** a password but had not verified their email before their first code sign-in.
  Verifying a code marks the identity verified and replaces the stored hash with a random one, so the password its
  owner chose stopped working at that moment. They will remember setting a password, and it will still be refused.

There is no reliable marker for either group in the database — a verified code deletes its `otp` row — so treat any
`EMAIL`-provider account that cannot sign in with a password as one of them, whether or not its owner recalls
setting one.
