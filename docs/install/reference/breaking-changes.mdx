...
A platform's plan, limits, and credit balance are no longer derived from the contents of a license key. They are resolved from our billing service and cached on the platform, and a license key is activated through it rather than being read locally. The key itself is now only a handle — Activepieces never inspects it.
