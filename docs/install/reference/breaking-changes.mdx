...
The MCP page now hands out the platform server address, `/mcp/platform`, rather than the project one, and the project settings dialog no longer carries an MCP Server tab.
...
There is no environment variable and no migration. If you use the platform-wide server and have switched tools off per project, review the Tools tab of the MCP page for each of those projects before you upgrade: those settings were doing nothing for platform clients and now take effect. A connected agent that relied on a tool you had switched off will start being refused.
...
A client that is already connected keeps the tool list it fetched when it connected, because the server does not notify clients when the list changes. Until it reconnects, such a client still shows a tool that the platform has since switched off, and a call to that tool fails with a generic "tool not found" error rather than an explanation.
