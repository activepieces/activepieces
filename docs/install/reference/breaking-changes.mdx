...
Nothing, if your members use the built-in roles. If you have built a custom project role and expect its holders to connect an MCP client, add the `READ_MCP` permission to it — otherwise their existing MCP clients stop working when you upgrade, rather than at the next token expiry. There is no environment variable, migration, or new setup step.
