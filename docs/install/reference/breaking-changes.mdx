...
Nothing, and no new configuration: the key is derived from a secret your instance already has. Any code already sent before you upgrade stops working, because a code stored the old way can no longer match, so anyone mid-sign-in at that moment needs to request a fresh one. Every code expires within ten minutes anyway, so the window where that can happen is small, and the next request replaces the old value.
