...
does not register, so `/authn/local/forget-password` is not served and nothing else can write a password. Set one for
each affected account directly — `UPDATE user_identity SET password = '<bcrypt hash>' WHERE email = '…'` — and have
the user change it after signing in. Do this **before** you upgrade if you can, so nobody is locked out in between.
