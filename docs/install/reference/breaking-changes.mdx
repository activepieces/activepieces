...
`AP_USE_CDN_FOR_BUNDLES` goes back to defaulting to `false`, reversing the change in 0.87.0. Official piece bundles that are not already cached in your own S3 bucket are downloaded from `registry.npmjs.org` instead of `cdn.activepieces.com`. When the flag is switched on, the CDN now serves the repackaged, self-contained tarballs from `https://cdn.activepieces.com/pieces/bundled/`.

#### What you need to do

Nothing. Set `AP_USE_CDN_FOR_BUNDLES=true` if you want official bundles to keep coming from the CDN, and allow outbound traffic to `cdn.activepieces.com` if your network policy restricts it.
...
<Note>
Superseded in 0.89.0: the bundled blocklist was replaced by a ZeroBounce check that runs only when `AP_ZEROBOUNCE_API_KEY` is set. See the 0.89.0 entry.
</Note>
