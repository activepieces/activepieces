...
Expired codes are also swept every ten minutes now, rather than sitting in the table until the same person happens to request another one.

#### What you need to do

Nothing, and no new configuration: the key is derived from a secret your instance already has. The upgrade clears any codes outstanding at that moment, because a stored plaintext and a stored digest are indistinguishable, so anyone who is mid-sign-in when you upgrade needs to request a fresh code. Every code expires within ten minutes anyway, so the window where that can happen is small.
