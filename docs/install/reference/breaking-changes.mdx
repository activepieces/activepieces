...
#### Addresses on mail-forwarding domains are refused at sign-up when ZeroBounce is configured

With `AP_ZEROBOUNCE_API_KEY` set, a new address is checked against ZeroBounce before an account is created. ZeroBounce's `mx_forward` verdict, a domain whose MX records forward into another mailbox, is now refused alongside disposable, toxic, possible-trap and globally suppressed addresses. Throwaway-domain farms look exactly like this (many cheap domains routed into one inbox), but so can a custom domain forwarded to a personal mailbox through a service such as Cloudflare Email Routing. Role addresses such as `info@` are still accepted. Nothing changes for an installation without the key.

#### What you need to do

Nothing if `AP_ZEROBOUNCE_API_KEY` is not set. If it is, a member whose domain forwards its mail is now silently refused like any other refused address. Invite them to the platform or a project first: an address that holds an accepted invitation is never refused. To keep the previous behaviour, unset the key.
