...
#### Every AI step is moved to the piece version that reports what a call costs

Flows were spread across several versions of the AI piece. Anything at or below 0.4.4 was moved to 0.4.5 by an earlier upgrade pass, Run Agent steps were pinned to 0.6.0 when that step became a thin client, and anything newer was whatever the flow was last saved with. None of those versions report what a managed AI call cost, so none of them can be billed on cost. Every `@activepieces/piece-ai` step below 0.11.0 is now moved to 0.11.0 the next time the flow is read, compared with semver so a step already ahead of that is left alone.

Two consequences follow from the version move rather than from the migration itself.

A Run Agent step pinned at 0.6.0 has been running the old behaviour where a dead agent turn reported success. It now raises instead, as described under *A Run Agent step whose turn dies now fails its flow run* below. That change shipped in 0.9.0, but a pinned step never received it until now.

The model dropdown on an AI step lists only the models Activepieces offers on its own credits. A step saved earlier against a model outside that list keeps running on the model it stored; the dropdown simply shows nothing selected until someone picks again. Bring-your-own provider keys are unaffected and still list their vendor's full catalogue.

#### What you need to do

Nothing to configure or migrate. Review any flow with a Run Agent step that was relying on a dead agent turn reporting success — the *What you need to do* under that entry lists the two options. If an AI step's model dropdown looks empty when you next open it, the stored model is still in use; re-pick it only if you want to change it.

#### Activepieces AI credits are charged on what a call cost, not per message

An AI step running on Activepieces AI credits was charged a fixed number of credits per message, set per model. That number is roughly right for a thousand-token call and wrong either side of it: a short prompt was over-charged, and a long one cost Activepieces far more than it charged. Nothing about the size of a request affected the price.

Managed AI calls are now charged on the cost the provider reports for each request, converted to credits at a fixed rate. The credit cost of a step therefore scales with the size of the call: a short prompt costs less than it did, and a long one costs proportionally more. A model that is genuinely free costs nothing. Bring-your-own provider keys are not affected and keep their existing per-message charge.

Two smaller changes come with it. The Activepieces AI provider now refuses a flow whose AI step predates the version that reports cost, with a message asking you to republish — the migration above moves existing flows for you, so this is only reachable by a step that was pinned back deliberately. And a request for that provider is refused outright if it does not come from the AI piece.

#### What you need to do

Nothing to configure or migrate, and no new environment variable — the conversion rate has a default. Expect your credit usage graph to change shape on the day this ships: flows that send large prompts or long documents to Activepieces AI credits will consume noticeably more credits than before, and flows sending short prompts slightly fewer. If a flow processes large inputs on managed credits, check its credit consumption after the first few runs and switch it to your own provider key if the new cost does not suit you.
