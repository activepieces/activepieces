...
The in-app AI Chat is not affected. It keeps checking each tool against the member's role, as before, whether or not that role grants `READ_MCP`.
...
Nothing, if your members use the built-in roles. If you have built a custom project role and expect its holders to connect an MCP client, open the role in **Platform Admin → Security → Project Roles** and set **MCP** to at least **Read** — otherwise their existing MCP clients stop working when you upgrade, rather than at the next token expiry. There is no environment variable, migration, or new setup step.
