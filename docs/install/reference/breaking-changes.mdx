...
Text that spells out a list (`["year","month"]`, `[]`) or a checkbox value (`true`, `false`) is now turned back into a real list or true/false, so fields that were quietly broken work again without being edited. Any other text in a multi-select now stops the step before it runs, with `Expected array, received: <your value>`, which names the field and the value. An optional field left empty now counts as not filled in, so an untouched optional checkbox no longer fails its step.
...
**Action:** only if a flow has a multi-select set to a dynamic value that does not produce a list. That covers a typed-in single item or a comma-separated line like `a,b`, and also a reference such as `{{ trigger.body.tag }}` that arrives carrying one value instead of a list. The second case does not show up in the flow itself, it depends on the incoming data, so the error message is what will point you at it. Those steps used to run and now fail with the message above. Edit the field so it produces a list (`["a","b"]`, or wrap the reference in one), or switch it back to picking options. Checkboxes need nothing: they already failed in this case and now fail in fewer cases.
