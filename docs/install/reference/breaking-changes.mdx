...
Nothing, and no new configuration: the key is derived from a secret your instance already has. The upgrade converts codes that are still in flight as it goes, so a sign-in already underway when you deploy still completes, and email verification links — which live a day rather than ten minutes — keep working. Codes that had already expired are cleared out at the same time.

Rolling back is the one case that costs a code: the older build cannot read the converted values, so the rollback clears the table and anyone mid-sign-in asks for a fresh one.
