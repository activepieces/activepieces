...
#### Project role names must be unique per platform

Nothing stopped two project roles on the same platform sharing a name. That mattered because a role is deleted by name: `DELETE /v1/project-roles/:name` removed every row with that name, and the cascade took each of their project member assignments with it. Deleting one of two roles called `Contractor` silently deleted the other and unassigned everybody holding it.

Creating a role already refused a duplicate name, but renaming one did not, so a rename was enough to get into that state. Renaming now refuses too, and both refusals return `409 Conflict` with code `VALIDATION`. Creating a duplicate previously returned `404` with code `ENTITY_NOT_FOUND`, which was wrong for a name clash.

#### What you need to do

Nothing, unless you already have duplicates. The upgrade adds a unique index on `("platformId", LOWER("name"))` for platform-owned roles, and renames any existing duplicates first. Names differing only in case count as duplicates, because the API has always matched a role name case-insensitively. Whichever copy has the most members keeps the name — so the role people actually hold is the one that stays recognisable — and the rest are suffixed ` (2)`, ` (3)` and so on, skipping any suffix already in use. Creation time only breaks a tie on member count. Member assignments are untouched: no role is deleted to make room.

Afterwards you may want to give any renamed role a meaningful name. There is no marker distinguishing a name the upgrade generated from one you chose, so this lists candidates rather than confirmed renames — a role you had already called `Reporting (2)` will show up too:

```sql
SELECT id, name FROM project_role WHERE name LIKE '% (%)';
```

If you call the API, a duplicate name now fails with `409` rather than `404` on create, and rename can fail where it previously succeeded.
