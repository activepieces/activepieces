...
The chart's default `container.port` is now `8080` instead of `80`, and the chart sets `AP_PORT` to match. A port above 1024 lets the pod run as a non-root user with all capabilities dropped, which OpenShift's `restricted-v2` SCC and the `restricted` Pod Security Standard require. The Service still listens on port 80, so an Ingress or client that goes through the Service is unaffected.
