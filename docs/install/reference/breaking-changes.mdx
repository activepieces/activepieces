...
An agent using your own API key costs more than it did. The old scheme charged one credit for the whole turn no matter how many times the agent went back to the model; the new one charges a credit per model call, so a turn that uses three tools costs roughly three or four credits rather than one plus its tool calls.
