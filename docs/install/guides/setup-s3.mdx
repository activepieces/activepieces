...
When S3 is configured, the piece sync uploads piece bundles to the `pieces/` prefix of the bucket. Workers install registry pieces from there instead of npm: a worker asks the API for the bundle, the API returns a **freshly generated presigned URL**, and the worker downloads directly from S3. If S3 is not configured or a bundle is missing, the worker falls back to downloading the piece from npm.

The bucket stays **fully private** — no public access or bucket policy is required. Presigned URLs are generated per request and short-lived, so nothing is stored or exposed.
