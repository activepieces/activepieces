...
<Accordion title="Prefer to set it up yourself?" icon="wrench">
The script only writes a `.env` and a `docker-compose.yml`, then starts them. You can do the same by hand, which is the right choice if you are air-gapped, if your change process needs a compose file it can review, or if you are pointing Activepieces at a database you already run.
