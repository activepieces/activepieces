...
The fastest way in. One line writes the config, generates your secrets, and starts everything.
...
## Install with Docker Compose directly

Prefer to run Docker Compose yourself instead of the one-line installer? These are the same files the script writes, done by hand.

Choose this if you are air-gapped, if your change process needs a compose file it can review, or if you are pointing Activepieces at a database you already run.
