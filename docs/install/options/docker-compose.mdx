...
<Tip>
Use `--port <port>` if 8080 is taken, and `--dir <path>` to install somewhere other than `./activepieces`. The [script](https://github.com/activepieces/activepieces/blob/main/tools/install.sh) is open source if you want to read it first.
</Tip>
