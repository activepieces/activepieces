...
Then point the app at them. The passwords are copied straight from their existing secrets:

```bash
umask 077
printf 'AP_POSTGRES_HOST=%s-postgresql\nAP_POSTGRES_PORT=5432\nAP_POSTGRES_DATABASE=activepieces\nAP_POSTGRES_USERNAME=postgres\nAP_POSTGRES_PASSWORD=%s\n' \
  "$RELEASE" "$(kubectl get secret "$RELEASE-postgresql" -o jsonpath='{.data.postgres-password}' | base64 -d)" > db.env
printf 'AP_REDIS_HOST=%s-redis-master\nAP_REDIS_PORT=6379\nAP_REDIS_PASSWORD=%s\n' \
  "$RELEASE" "$(kubectl get secret "$RELEASE-redis" -o jsonpath='{.data.redis-password}' | base64 -d)" > redis.env
kubectl create secret generic activepieces-db-secrets --from-env-file=db.env
kubectl create secret generic activepieces-redis-secrets --from-env-file=redis.env
rm db.env redis.env
```

If you used an external database instead, create the same two secrets as described in [Database and Redis](#database-and-redis).
...
**3. Keep your keys.** The encryption key and JWT secret in `RELEASE-activepieces-secrets` and `RELEASE-activepieces-jwt-secret` (or `RELEASE-secrets` and `RELEASE-jwt-secret` when the release name contains `activepieces`) are carried over automatically. If you deploy with `helm template`, Argo CD or Kustomize, the chart cannot see them, so copy them into `activepieces-auth-secrets` first:

```bash
FULLNAME="$RELEASE-activepieces"
case "$RELEASE" in *activepieces*) FULLNAME="$RELEASE" ;; esac
kubectl create secret generic activepieces-auth-secrets \
  --from-file=AP_ENCRYPTION_KEY=<(kubectl get secret "$FULLNAME-secrets" -o jsonpath='{.data.encryption-key}' | base64 -d) \
  --from-file=AP_JWT_SECRET=<(kubectl get secret "$FULLNAME-jwt-secret" -o jsonpath='{.data.jwt-secret}' | base64 -d)
```
