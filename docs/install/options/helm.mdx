...
The pod listens on `container.port` (default `8080`), and the chart passes it to the app as `AP_PORT`. A port above 1024 lets the container run as a non-root user with every capability dropped. The Service still exposes port 80 (`service.port`). If you set `AP_PORT` yourself, in `activepiecesConfig` or in a secret listed under `activepiecesEnvVariables`, the chart leaves it alone: set `container.port` to the same number.
