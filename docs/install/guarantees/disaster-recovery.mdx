...
Sync webhooks are far less exposed: the caller blocks on the result, so a loss surfaces as an error or timeout it can retry rather than as a silent drop. The run record itself is still written asynchronously (see below).

**Also at risk — production run records that have not been flushed yet.** Production run metadata is written through a Redis-backed upsert queue: the run is merged into a Redis hash and a deduplicated job flushes it to Postgres, so several status transitions on a short run cost one Postgres write instead of one each. A run whose first flush has not committed when Redis is lost leaves no `flow_run` row at all, so it will not appear in the runs view to be retried. The window is normally sub-second and scales with `AP_RUNS_METADATA_UPDATE_CONCURRENCY` (default 2); it widens under queue backlog. Test runs are written directly to Postgres and are not exposed.

**Not lost — everything else.** Schedules, renewals, resume timers, and run logs all have their source of truth in Postgres or S3:
...
- **Webhook ingest stops.** Flow resolution is served from a Redis cache, but the payload is persisted to the `file` table before the job is enqueued, so async webhooks fail with a `5xx` for the duration of the failover just as sync ones do. The sender can see the error and retry — which is the behaviour you want during a control-plane outage, but plan for it if your upstream does not retry.
...
| Scenario | RPO | RTO |
|----------|-----|-----|
| Redis node loss | ≤1s of accepted async webhooks, plus production runs whose metadata flush had not committed | Minutes: Redis failover + app restart + bulk-retry |
