...
- `AP_API_URL` environment variable has been removed. The worker now derives the API URL from `AP_FRONTEND_URL` and `AP_CONTAINER_TYPE` automatically.
...
- The flow schema version has been bumped from `17` to `18`. Existing flows are migrated automatically.

### Do you need to take action?
- If you are using the `GET /v1/queue-metrics` endpoint, switch to `GET /v1/worker-machines`.
- If you are using `AP_WORKER_CONCURRENCY` or `AP_AGENTS_WORKER_CONCURRENCY`, remove them and scale by adding more worker replicas instead.
- If you are using `AP_CONTAINER_TYPE`, note the updated values: `APP`, `WORKER`, or `WORKER_AND_APP`.
- If you are using `AP_API_URL`, remove it. The worker now automatically derives the API URL from `AP_FRONTEND_URL`.
