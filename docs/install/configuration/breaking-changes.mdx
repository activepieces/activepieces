...
- Embedding now requires setting allowed embed origins which means only the domains that are listed in the field can embed the app, it does support wildcards. there's also new env variable `AP_ALLOWED_EMBED_ORIGINS` for setting pre-allowed origins. there's also an new endpoint for adding these origins [check docs](/endpoints/embedding/add-allowed-embed-origins)
