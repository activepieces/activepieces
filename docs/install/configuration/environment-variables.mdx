...
| `OTEL_EXPORTER_OTLP_HEADERS` | Headers for OTLP exporter (comma-separated key=value pairs) | `None` | `Authorization=Bearer token,api-key=value` |

**Note**: Both `AP_OTEL_ENABLED` and `AP_OTEL_EXPORTER_OTLP_ENDPOINT` must be set for OpenTelemetry to be enabled.

#### Axiom Logging Configuration

| Variable | Description | Default Value | Example |
|----------|-------------|---------------|---------|
| `AP_AXIOM_TOKEN` | Axiom authentication token | `None` | `your-axiom-token` |
| `AP_AXIOM_DATASET` | Axiom dataset name | `None` | `activepieces-traces` |
