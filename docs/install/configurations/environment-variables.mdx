...
The queue UI is a tool that allows you to monitor the queue and debug scheduled jobs. It is only available when using Redis as the queue mode. In production mode, you can find it at `/api/ui`, while in development mode, it is located at `/ui`.
