...
The list in the app shows only the events a self-hosted instance can actually send. Account and sign-in events — creating an account, signing in, the emailed sign-in codes, captcha and federated login — are sent only from Activepieces Cloud, so they are not listed and never leave a self-hosted instance.

A self-hosted instance still records *that* an account exists — the edition, version and the date it was first seen — so we can tell how many self-hosted deployments are in use. That carries no name or email address: those are attached on Cloud only.