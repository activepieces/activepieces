...
To make ActivePieces handle more work, you can add more instances. For a good setup, set the worker concurrency in the environment variable to 20 on each machine.

With this setup, it can handle more than **20 million jobs**.

The main thing to consider when deciding to scale is how often it checks for new items. For example, if you have a 5-minute check, it will run 8,640 times in a month.