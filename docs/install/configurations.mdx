...
The non-sandboxed mode runs the code directly on the host machine without any restrictions. This mode doesn't require privileged access to Docker, but it should only be used if you trust the users in the activepiece.