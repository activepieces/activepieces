...
### Platform vs Project MCP Server

**Platform MCP Server:** This provides platform-wide MCP configuration. The Platform MCP Server lets platform admins configure the MCP server used by the Activepieces AI Chat assistant and external MCP clients. It can only be accessed by the platform admin from the platform admin dashboard

![Platform Mcp Server](/images/platform-mcp-server.png "Platform Mcp Server")

**Project MCP Server:** The Project MCP is restricted to the specific project it belong to and can only access connects, flows or tools in that project.<br />
...
- _"Create a flow that sends a Slack message when a new row is added to Google Sheets"_
- _"Check my flow for any issues before publishing"_
- _"Create a Contacts table and add 3 records"_
- _"Show me the last failed run and what went wrong"_
- _"What Slack triggers are available?"_
