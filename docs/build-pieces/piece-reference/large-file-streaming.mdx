...
Streaming removes the *memory* ceiling, not the destination API's own limit. Two services cap
what a single request can carry, and the pieces below upload in one request:

- **Dropbox** — `Upload file` uses `/2/files/upload`, which
  [Dropbox documents as being for files under 150 MB](https://developers.dropbox.com/dbx-performance-guide);
  larger files need the chunked `upload_session` endpoints, which the piece does not use yet.
- **Microsoft SharePoint** — `Upload File` uses Graph's simple `PUT …/content`, which
  [supports files up to 250 MB](https://learn.microsoft.com/en-us/graph/api/driveitem-put-content).
  Its Microsoft OneDrive sibling is not affected: `Upload file` there switches to a chunked
  upload session above 4 MiB.

Both are pre-existing API limits rather than streaming limits, but they bound how large a file
these two actions can actually move.
