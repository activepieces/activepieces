...
<Note>
Large binary bodies (e.g. `application/octet-stream`, images, video, PDFs) and multipart file uploads are streamed straight to storage instead of being buffered — `context.payload.body` receives a file-reference URL rather than the raw bytes. The per-file ceiling is `AP_MAX_STREAM_FILE_SIZE_MB` (S3 storage only; on `DB` storage the smaller `AP_MAX_FILE_SIZE_MB` cap applies). This is async-only territory: a **synchronous** webhook streaming a huge upload will hit the HTTP timeout, so use published flows for large ingestion.
</Note>
