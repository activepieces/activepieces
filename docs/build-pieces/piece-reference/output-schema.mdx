...
`sensitive: true`. The engine redacts it to `**REDACTED**` in the step's persisted run-log output
before it's stored — the real value is still used at runtime by the flow itself, only the log is
affected:
