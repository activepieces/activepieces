...
<Tip>
If your action returns an object keyed by opaque IDs or slugs, you can add `__displayName` to each child object so the data selector shows a readable label instead of the raw key. See [Output Display Names](/build-pieces/piece-reference/output-display-names).
</Tip>
