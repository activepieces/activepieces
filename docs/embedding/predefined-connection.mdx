...
<Steps>
  <Step title="Create an API Key">
    Go to <b>Platform Admin → Security → API Keys</b> and create an API key. Save it for use in the next step.
  </Step>
...
<Steps>
    <Step title="Modify Trigger / Action Definition">
       Go to `createTrigger` or `createAction` and set `requireAuth` to `false`, this will hide the connections dropdown in the piece settings,
       next we need to fetch it based on naming convention.
    </Step>
    <Step title="Fetch the connection">
        Here is example how you can fetch the connection value based on naming convention.
...
const fetchConnection = async (connections: ConnectionsManager, projectExternalId: string | undefined): Promise<typeof auth> => {
   if (isNil(projectExternalId)) {
      throw new Error('This project is missing an external id');
   }
   // This is the naming convention we have chosen earlier for the connections.
   ...
   return connection as typeof auth;
}
