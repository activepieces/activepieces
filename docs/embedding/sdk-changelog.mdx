...
- Added `embedding.categoryPriority` parameter to the [configure](./embed-builder#configure-parameters) method **(value: string[])**. Takes a list of piece category names (matched case-insensitively, e.g. `['ARTIFICIAL_INTELLIGENCE', 'my-category']`). Each listed category is rendered as a named group at the top of the step picker's Explore and Apps tabs, in the configured order, and pieces in those categories are ranked first in step-picker search results. Custom pieces can declare their own category strings via `createPiece({ categories: [...] })`.
