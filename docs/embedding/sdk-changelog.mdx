...
### 09/10/2026 (0.16.0)
- SDK URL: https://cdn.activepieces.com/sdk/embed/0.16.0.js
- Added `embedding.analytics.gtmContainerId` and `embedding.analytics.clarityProjectId` parameters to the [configure](./embed-builder#configure-parameters) method **(value: string)**. Loads Google Tag Manager and/or Microsoft Clarity inside the embedded app so you get user-behaviour analytics on the builder; scripts on your own page cannot see into the iframe. Only ids are accepted, never script code, and the ids are validated before anything loads. Both are off when unset. This version requires an Activepieces release that includes [#PR_NUMBER](https://github.com/activepieces/activepieces/pull/PR_NUMBER).
