---
title: "SDK Changelog"
description: "A log of all notable changes to Ensemble SDK"
icon: "code-commit"
---
...
<Warning>
**Breaking Change:** <br></br> <br></br> If your Ensemble image version is < 0.45.0 and (you are using the connect method from the embed SDk, and need the connection externalId to be returned after the user creates it OR if you want to reconnect a specific connection with an externalId), you must upgrade your Ensemble image to >= 0.45.0
</Warning>
...
- add custom navigation handler ([#4500](https://github.com/ensemble/ensemble/pull/4500))
- allow passing a predefined name for connection in connect method ([#4485](https://github.com/ensemble/ensemble/pull/4485))
- add changelog ([#4503](https://github.com/ensemble/ensemble/pull/4503))


### 26/01/2025 (0.3.3)

- This version requires you to update Ensemble to 0.39.8
...
- This version requires you to update Ensemble to 0.41.0
...
- This version requires you to update Ensemble to 0.56.0
...
- SDK URL: https://cdn.ensemble.com/sdk/embed/0.5.0-rc.0.js
- This version requires you to **upgrade Ensemble to [0.64.0-rc.0](https://github.com/ensemble/ensemble/pkgs/container/activepieces/438888138?tag=0.64.0-rc.0)**
...
- SDK URL: https://cdn.ensemble.com/sdk/embed/0.5.0-rc.1.js
- This version requires you to **upgrade Ensemble to [0.64.0-rc.0](https://github.com/ensemble/ensemble/pkgs/container/activepieces/438888138?tag=0.64.0-rc.0)**
- Revert back the `prefix` parameter from the [configure](./embed-builder#configure-parameters) method.


### 03/07/2025 (0.5.0)
- SDK URL: https://cdn.ensemble.com/sdk/embed/0.5.0.js
- This version requires you to **upgrade Ensemble to [0.64.2](https://github.com/ensemble/ensemble/releases/tag/0.64.2)**
- Added `embedding.hideDuplicateFlow` parameter to the [configure](./embed-builder#configure-parameters) method **(value: true | false)**. 
- Added `embedding.builder.homeButtonIcon` parameter to the [configure](./embed-builder#configure-parameters) method **(value: 'logo' | 'back')**, if set to **'back'** the tooltip shown on hovering the home button is removed.
- Added `embedding.locale` parameter to the [configure](./embed-builder#configure-parameters) method, it takes [ISO 639-1](https://en.wikipedia.org/wiki/List_of_ISO_639_language_codes) locale codes, here are the ones supported: **('en' | 'nl' | 'it' | 'de' | 'fr' | 'bg' | 'uk' | 'hu' | 'es' | 'ja' | 'id' | 'vi' | 'zh' | 'pt')**
- Added `embedding.styling.mode` parameter to [configure](./embed-builder#configure-parameters) method **(value: 'light' | 'dark')**
...
### 07/07/2025 (0.6.0)
- SDK URL: https://cdn.ensemble.com/sdk/embed/0.6.0.js
- This version requires you to **upgrade Ensemble to 0.65.1 (we will link it here once we do so)**
