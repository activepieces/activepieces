...
| embedding.categoryPriority | ❌ | string[] | A list of piece category names (matched case-insensitively). Each listed category shows as a named group at the top of the step picker's Explore and Apps tabs, in the configured order, and its pieces rank first in step-picker search results. (added in [0.12.0](./sdk-changelog#07%2F08%2F2026-0-12-0)) |
