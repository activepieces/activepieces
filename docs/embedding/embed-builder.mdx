---
title: "Embed Builder"
description: "Render the Activepieces builder inside your application"
icon: "wrench"
---

<Snippet file="embed-feature.mdx" />
...
| embedding.formulasDocsUrl | ❌ | string | Where the "See All" link in the formula editor points. Leave it unset (the default) and the link is hidden entirely, so embedded users are never sent to the Activepieces docs. Set it to your own formula reference to show the link pointing there. (added in [0.15.0](./sdk-changelog#08%2F01%2F2026-0-15-0)) |
| embedding.analytics.gtmContainerId | ❌ | string | Loads Google Tag Manager inside the embedded app using this container id (e.g. `GTM-XXXXXXX`), so the tags you configure in GTM (analytics, session recording) run on the builder and dashboard. Scripts on your own page cannot see into the iframe, so this is the only way to track it. Off when unset. (added in [0.16.0](./sdk-changelog#09%2F10%2F2026-0-16-0)) |
| embedding.analytics.clarityProjectId | ❌ | string | Loads Microsoft Clarity inside the embedded app using this project id. Session recording on a large flow canvas can be heavy, so test it on a real flow before rolling out. Off when unset. (added in [0.16.0](./sdk-changelog#09%2F10%2F2026-0-16-0)) |
...
<Warning>
**Before enabling `embedding.analytics`:** the GTM container and Clarity tag run inside the embedded app with your user's session, so any tag you publish can read what that user can. Clarity masks input fields by default but records everything else on screen, including step outputs, sample data, and connection names in the builder. You own the consent banner and privacy disclosure for that data. Session recording also mirrors the page in memory, so test it on your largest flows before rolling it out widely.
</Warning>
