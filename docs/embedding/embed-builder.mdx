...
This documentation explains how to embed the Ensemble iframe inside your application and customize it.
...
```html
<script src="https://cdn.ensemble.com/sdk/embed/0.4.0.js">
</script>
<script>
...
| --- | --- | --- | --- |
| instanceUrl| ✅ |  string | The url of the instance hosting Ensemble, could be https://cloud.ensemble.com if you are a cloud user. |
| jwtToken | ✅  |  string | The jwt token you generated to authenticate your users to Ensemble. |
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<Ensemble_url>`. For example if the prefix is `/automation` and the Ensemble url is `/flows` the full url would be `/automation/flows`. |
| embedding.containerId | ❌ | string | The html element's id that is going to be containing Ensemble's iframe. |
...
| navigation.handler  | ❌ | `({route:string}) => void`  | This callback will be triggered each time a route in Ensemble changes, you can read more about it [here](/embedding/navigation) |
