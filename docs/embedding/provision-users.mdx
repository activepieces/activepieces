...
<Steps>
  ...
  <Step title="Step 2: Generate a JWT">
    ...
    | version           | Token payload version. Use `"v4"` to assign a piece set via `pieceSet`, or `"v3"` for the legacy `piecesTags` approach |
    | pieceSet          | (v4) Key of the piece set to assign to the project, check [customize pieces](/embedding/customize-pieces)  |
    | piecesFilterType  | (v3, legacy) Customize the project pieces, check [customize pieces](/embedding/customize-pieces)  |
    | piecesTags        | (v3, legacy) Customize the project pieces, check [customize pieces](/embedding/customize-pieces)  |
