...
By default, navigating within your embedded instance of Ensemble doesn't affect the client's browser history or viewed URL. Ensemble only provide a **handler**, that trigger on every route change in the **iframe**.

## Automatically Sync URL 

You can use the following snippet when configuring the SDK, which will implement a handler that syncs the Ensemble iframe with your browser:
...
window.addEventListener("popstate", () => {
  const route = activepieces.extractEnsembleRouteFromUrl({ vendorUrl: window.location.href });
