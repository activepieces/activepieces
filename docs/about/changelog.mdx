...
Activepieces now ships with a built-in **network proxy that protects your automations from network attacks**. When turned on, flows can still reach the public internet, but they can no longer reach sensitive addresses on your own network — like your database, your Redis cache, your cloud provider's metadata service, or anything else running inside your private infrastructure. That closes a common class of attack where a malicious link or prompt tricks a flow into calling an internal-only URL.
