...
- If `AP_SCHEDULED_WORKER_CONCURRENCY` and `AP_FLOW_WORKER_CONCURRENCY` are set to zero on the app server, workers will stop processing the queues. To resolve this, refer to the [Separate Worker from App](https://www.activepieces.com/docs/install/configuration/seperate-workers) documentation and use `AP_CONTAINER_TYPE`.
