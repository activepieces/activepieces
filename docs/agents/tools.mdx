...
| Flow tool | Yes, naming the flow the agent chose |
| MCP server tool | No |
| Knowledge base search | No, it is a read |

MCP server tools are the remaining gap. They carry no read-or-write marking, so recording them would fill the log with lookups; if you need a trail for a step, use a piece action or a flow.
