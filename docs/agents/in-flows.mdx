...
Reference them like any other step:

```
{{step_2.urgency}}
```

Now you can route on `needs_human`, sort by `urgency`, and drop `category` into the Slack message.

<Tip>
Ask for the fewest fields the next step needs. Every extra field is one more thing to get wrong.
</Tip>

## Decide what it fills in

Every input on a piece tool is one of three modes, so you can pin what must be exact and leave the rest to the agent.
