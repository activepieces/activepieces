...
- **Normal hotfix**:
  ...
  3. Manually trigger `continuous-delivery-stg.yml` with action `cloud-hotfix` **on the hotfix branch** (not `main`). This builds the image, deploys it to staging, and promotes it to production.
  4. Merge the hotfix branch into `main`. This automatically triggers `tag-release-candidate`, which SSHes to staging (the hotfix image is still running there) and retrags it as `release-candidate`. Sunday's scheduled run will re-deploy the hotfix image — a safe no-op.
...
- **`cloud-hotfix`** — Build, deploy to staging, then promote to production. Must be triggered **on the hotfix branch** (`deploy/cloud/YYYY-MM-DD`), not `main`. After promotion, merge the hotfix branch into `main` — this automatically updates `release-candidate` to the hotfix image so Sunday's scheduled run is a no-op.
