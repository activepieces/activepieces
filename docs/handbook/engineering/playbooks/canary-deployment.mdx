...
Trigger the **Continuous Delivery — Canary** workflow from GitHub Actions (`continuous-delivery-canary.yml`). It accepts these optional inputs:

| Input | Description |
|---|---|
| `platform_ids` | Comma-separated platform IDs to route to this canary build. Leave blank to deploy without changing routing. |
...
2. Checks if any new migrations are breaking — fails the workflow if they are (override with `force_breaking_migrations=true`)
3. Deploys the canary app (`config/app-canary.yml`) and canary workers
4. If `platform_ids` is provided, calls `POST /v1/admin/platforms/canary` on the primary app for each ID — enabling canary routing immediately without a restart
...
Dispatch [Continuous Delivery — Cloud](/handbook/engineering/playbooks/releases#continuous-delivery-cloud-yml-%E2%80%94-production-promotion) with **image_tag = canary**
