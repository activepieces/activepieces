...
Trigger the **Continuous Delivery — Canary** workflow from GitHub Actions (`continuous-delivery-canary.yml`). It accepts one optional input:
...
2. Deploys the canary app (`config/app-canary.yml`) and canary workers
3. If `platform_ids` is provided, calls `POST /v1/admin/platforms/canary` on the primary app for each ID — enabling canary routing immediately without a restart
