...
| `CONTEXT-MAP.md` + per-context `CONTEXT.md` | glossary | When pinning down terminology | Domain vocabulary (ubiquitous language) + `_Avoid_` aliases |
...
- Read the relevant `CONTEXT.md` (via `CONTEXT-MAP.md`) and `docs/adr/` entries for the modules you'll touch
...
4. **Read the domain model yourself.** Before starting a feature in a module, spend 2 minutes reading its co-located `CONTEXT.md` (via `CONTEXT-MAP.md`) and the relevant `docs/adr/` entries. You'll ask better questions.
