...
- **Hotfix**: Merge to `main`, then manually dispatch `continuous-delivery-stg.yml` with action `cloud-hotfix`. This builds, deploys to staging, promotes to production, and advances the `release-candidate` Docker tag and git tag to the hotfix commit. Sunday's scheduled run will then re-deploy what is already on prod — a safe no-op.
