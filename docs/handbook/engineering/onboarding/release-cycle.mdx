...
- **Hotfix**: Apply your fix to the current `deploy/cloud/YYYY-MM-DD` branch, then manually dispatch `continuous-delivery-stg.yml` with action `cloud-hotfix` **on that branch** (not `main`). This builds, deploys to staging, and promotes to production. After promotion, merge the hotfix branch into `main` — this automatically triggers `tag-release-candidate`, which SSHes to staging (the hotfix image is still running there) and retrags it as `release-candidate`. Sunday's scheduled run will then re-deploy what is already on prod — a safe no-op.
