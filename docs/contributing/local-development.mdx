...
4. Run `cd src/packages/backend && pnpm install && pnpm start`.
5. Open another terminal and run `cd src/packages/frontend && pnpm install && pnpm start`.
6. The backend will run at `localhost:3000` and the frontend will run at `localhost:4200`.
